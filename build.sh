...
LINGMO_VERTYPE="beta"
LINGMO_SYS_VERSION="3.0"
LINGMO_BUILD_VER="20250111-a"
...
target_image_name() {
	...
	if [ "$IMAGE_TYPE" = "live" ]; then
		if [ "$LINGMO_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_VERTYPE-$LINGMO_BUILD_VER-desktop-$LINGMO_ARCH.$IMAGE_EXT"
		else
      echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_VERTYPE-desktop-$LINGMO_VARIANT-$LINGMO_ARCH.$IMAGE_EXT"
		fi
	else
		if [ "$LINGMO_VARIANT" = "default" ]; then
...
debug "Generating Verification File"
run_and_log cd $TARGET_DIR/ && ../make-sums
