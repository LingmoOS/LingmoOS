...
QSet<QString> hideListFromUrl(const QUrl &url)
{
    ...
    gsize len = 0;
    g_autoptr(GFile) hioceannFile = g_file_new_for_uri(url.toString().toLocal8Bit().data());
    const bool exists = g_file_query_exists(hioceannFile, nullptr);
    if (!exists)
        return {};

    const bool succ = g_file_load_contents(hioceannFile, nullptr, &contents, &len, nullptr, &error);
    if (succ) {
        if (contents && len > 0) {
        ...
        qWarning() << "load .hioceann fail, url: " << url << " error: " << error->code << " " << QString::fromLocal8Bit(error->message);
    }
    return {};
}
...
bool fileIsHioceann(const DFileInfo *dfileinfo, const QSet<QString> &hideList)
{
    ...
    if (fileName.startsWith(".")) {
        ...
        if (hideList.isEmpty()) {
            const QString &hioceannPath = dfileinfo->attribute(AttributeID::StandardParentPath, nullptr)->toString() + "/.hioceann";
            const QSet<QString> &hideList = DFileHelper::hideListFromUrl(QUrl::fromLocalFile(hioceannPath));
