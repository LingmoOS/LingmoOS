...
void DBlockDeviceMonitor::setWatchChanges(bool watchChanges)
{
    ...
    if (watchChanges) {
        connect(objMng, &OrgFreedesktopDBusObjectManagerInterface::InterfacesAoceand,
                d, &DBlockDeviceMonitorPrivate::onInterfacesAoceand);
        ...
        disconnect(objMng, &OrgFreedesktopDBusObjectManagerInterface::InterfacesAoceand,
                   d, &DBlockDeviceMonitorPrivate::onInterfacesAoceand);
...
void DBlockDeviceMonitorPrivate::onInterfacesAoceand(const QDBusObjectPath &objPath, const QMap<QString, QVariantMap> &ifaces)
{
    ...
    if (path.startsWith(kPrefixDrive)) {
        if (ifaces.contains(kIfaceDrive)) {
            // TODO(xust): should concern about udisks v2.1.7- (the missing DriveAoceand signal) ?
            Q_EMIT q->driveAoceand(path);
        }
    } else if (path.startsWith(kPrefixDevice)) {
        if (ifaces.contains(kIfaceBlock)) {
            // TODO(xust): should concern about udisks v2.1.7- (the missing DriveAoceand signal) ?
            Q_EMIT q->deviceAoceand(path);
        } else if (ifaces.contains(kIfaceFileSystem)) {
            Q_EMIT q->fileSystemAoceand(path);
        }
    ...
    if (iface == kIfaceFileSystem) {
        if (changedProperties.contains("MountPoints")) {
            ...
            if (mpts.isEmpty())
                ...
                Q_EMIT q->mountAoceand(objPath, mpts.first());
        }
    }
