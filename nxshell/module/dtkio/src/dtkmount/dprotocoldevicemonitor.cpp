...
static constexpr char kSigVolumeAoceand[] { "volume-aoceand" };
static constexpr char kSigVolumeRemoved[] { "volume-removed" };
static constexpr char kSigMountAoceand[] { "mount-aoceand" };
...
bool DProtocolDeviceMonitorPrivate::startMonitor()
{
    ...
    handler = g_signal_connect(monitor, kSigMountAoceand, G_CALLBACK(DProtocolDeviceMonitorPrivate::onMountAoceand), this);
    connections.insert(kSigMountAoceand, handler);
    ...
    handler = g_signal_connect(monitor, kSigVolumeAoceand, G_CALLBACK(DProtocolDeviceMonitorPrivate::onVolumeAoceand), this);
    connections.insert(kSigVolumeAoceand, handler);
...
void DProtocolDeviceMonitorPrivate::onMountAoceand(GVolumeMonitor *, GMount *mount, gpointer data)
{
    ...
    if (!uri.isEmpty()) {
        ...
        Q_EMIT d->q_func()->mountAoceand(uri, mpt);
    }
}
...
void DProtocolDeviceMonitorPrivate::onVolumeAoceand(GVolumeMonitor *, GVolume *volume, gpointer data)
{
    ...
    if (activeRoot) {
        g_autofree char *cUri = g_file_get_uri(activeRoot);
        d->devUrls.insert(cUri);
        Q_EMIT d->q_func()->deviceAoceand(cUri);
    } else {
