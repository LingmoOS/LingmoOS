...
{
    m_stub.set_lamda(&OrgFreedesktopUDisks2DriveInterface::vendor, [] { __DBG_STUB_INVOKE__ return "lingmo"; });
    EXPECT_EQ("lingmo", m_drv.vendor());
}

TEST_F(TestDDiskDrive, wwn)
{
    m_stub.set_lamda(&OrgFreedesktopUDisks2DriveInterface::wWN, [] { __DBG_STUB_INVOKE__ return "Lingmo"; });
    EXPECT_EQ("Lingmo", m_drv.WWN());
}
