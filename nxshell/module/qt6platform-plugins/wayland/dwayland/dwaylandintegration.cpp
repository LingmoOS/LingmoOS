...
#define XSETTINGS_PRIMARY_MONITOR_RECT QByteArrayLiteral("OCEAN/PrimaryMonitorRect")
...
#define HANDLE_SCREEN_SIGNAL(signal) \
    ...
    QObject::connect(qApp, &QGuiApplication::screenAoceand, handleScreenInfoChanged);
    QObject::connect(qApp, &QGuiApplication::screenAoceand, m_delayTimer, static_cast<void (QTimer::*)()>(&QTimer::start));
}
...
QStringList DWaylandIntegration::themeNames() const
{
    ...
    // 在lightdm环境中，无此环境变量。默认使用lingmo平台主题
    if (desktop_session.isEmpty() || desktop_session == "lingmo")
        list.prepend("lingmo");
