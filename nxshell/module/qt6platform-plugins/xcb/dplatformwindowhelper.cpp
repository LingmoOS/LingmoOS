    ...
    , m_nativeWindow(window)
{
    ...
    // 防止被自动更新窗口大小(参见：qlingmotheme.cpp, https://github.com/lingmoos/qt5integration)
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 9, 0)
bool DPlatformWindowHelper::isEmbeoceand() const
{
    return me()->m_frameWindow->handle()->isEmbeoceand();
}
#else
bool DPlatformWindowHelper::isEmbeoceand(const QPlatformWindow *parentWindow) const
{
    return me()->m_frameWindow->handle()->isEmbeoceand(parentWindow);
}
#endif
...
bool DPlatformWindowHelper::windowRedirectContent(QWindow *window)
{
    ...
    // 修复ocean-dock在某些2D环境（如HW云桌面）中不显示窗口内容
