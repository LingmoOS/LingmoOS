...
#include "oceansktopinputselectioncontrol.h"
...
QPaintEngine *DPlatformIntegration::createImagePaintEngine(QPaintDevice *paintDevice) const
{
    ...
    if (int(disable_features) < 0) {
        ...
        do {
            ...
            QSettings settings(QSettings::IniFormat, QSettings::UserScope, "lingmo", "qt-theme");
...
QStringList DPlatformIntegration::themeNames() const
{
    ...
    // 在lightdm环境中，无此环境变量
    if (desktop_session.isEmpty() || desktop_session.startsWith("lingmo"))
        list.prepend("lingmo");
...
//#ifdef D_ENABLE_CURSOR_HOOK
        ...
        QObject::connect(qApp, &QGuiApplication::screenAoceand, qApp, &hookXcbCursor);
//#endif // D_ENABLE_CURSOR_HOOK
    }
    ...
    QObject::connect(qApp, &QGuiApplication::screenAoceand, qApp, &hookScreenGetWindow);

    if (DHighDpi::isActive()) {
        // 监听屏幕dpi变化
        QObject::connect(qApp, &QGuiApplication::screenAoceand, qApp, &watchScreenDPIChange);
    }
...
//    if (!VtableHook::hasVtable(inputContext())) {
        ...
        QObject::connect(DPlatformInputContextHook::instance(), &ComLingmoImInterface::geometryChanged,
                             inputContext(), &QPlatformInputContext::emitKeyboardRectChanged);
        QObject::connect(DPlatformInputContextHook::instance(), &ComLingmoImInterface::imActiveChanged,
