...
protected:
    ...
    void com_lingmo_client_management_capture_window_image(Resource *resource,
        int32_t window_id, struct ::wl_resource *buffer) override;
    void com_lingmo_client_management_split_window(Resource *resource,
...
void ClientManagementInterfacePrivate::com_lingmo_client_management_capture_window_image(Resource *resource, int windowId, wl_resource *buffer)
{
    Q_UNUSED(resource);
...
void ClientManagementInterfacePrivate::com_lingmo_client_management_split_window(Resource *resource, const QString &uuid, int32_t splitType)
{
    Q_UNUSED(resource);
...
void ClientManagementInterfacePrivate::sendSplitChange(const QString& uuid, int splitable)
{
    if (splitable > 0) {
        ...
        const auto clientResources = resourceMap();
        for (Resource *resource : clientResources) {
            com_lingmo_client_management_send_split_change(resource->handle, m_splitUuid.toLatin1().data(), m_splitable);
        }
    }
}
