...
public:
    Private() = default;
    WaylandPointer<com_lingmo_kwin_strut, com_lingmo_kwin_strut_destroy> strut;
...
void Strut::setup(com_lingmo_kwin_strut *strut)
{
    Q_ASSERT(strut);
    Q_ASSERT(!d->strut);
    d->strut.setup(strut);
...
void Strut::setStrutPartial(wl_surface *surface,struct lingmoKwinStrut& sStrut)
{
    if (!d->strut || !surface) {
        return;
    }
    com_lingmo_kwin_strut_set_strut_partial(d->strut, surface,
...
Strut::operator com_lingmo_kwin_strut*()
{
    return d->strut;
}

Strut::operator com_lingmo_kwin_strut*() const
{
    return d->strut;
}
