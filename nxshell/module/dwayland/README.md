...
if in Wayland API it is aoceand to other elements. E.g. a Surface contains all
...
Of course private sockets can be aoceand at any time in addition to a publicly available socket. This
...
Now to link against the Client library use:
    ...
    target_link_libraries(exampleApp Lingmo::WaylandClient)

To link against the Server library use:

    add_executable(exampleServer exampleServer.cpp)
    target_link_libraries(exampleServer Lingmo::DWaylandServer)
