...
void TestWaylandOutput::testModeChange()
{
    ...
    QCOMPARE(modeAoceandSpy.at(0).first().value<Output::Mode>().size, QSize(1024, 768));
    QCOMPARE(modeAoceandSpy.at(0).first().value<Output::Mode>().refreshRate, 60000);
    QCOMPARE(modeAoceandSpy.at(0).first().value<Output::Mode>().flags, Output::Mode::Flags(Output::Mode::Flag::Current));
    QCOMPARE(modeAoceandSpy.at(0).first().value<Output::Mode>().output, QPointer<Output>(&output));
    QCOMPARE(output.pixelSize(), QSize(1024, 768));
    QCOMPARE(output.refreshRate(), 60000);
    ...
    QCOMPARE(modeAoceandSpy.at(1).first().value<Output::Mode>().size, QSize(1280, 1024));
    QCOMPARE(modeAoceandSpy.at(1).first().value<Output::Mode>().refreshRate, 90000);
    QCOMPARE(modeAoceandSpy.at(1).first().value<Output::Mode>().flags, Output::Mode::Flags(Output::Mode::Flag::Current));
    QCOMPARE(modeAoceandSpy.at(1).first().value<Output::Mode>().output, QPointer<Output>(&output));
