...
void TestDataDevice::testSetSelection()
{
    ...
    QSignalSpy mimeTypeAoceandSpy(dataOffer, &KWayland::Client::DataOffer::mimeTypeOffered);
    QVERIFY(mimeTypeAoceandSpy.isValid());
    dataSource->offer(QStringLiteral("text/html"));
    QVERIFY(mimeTypeAoceandSpy.wait());
    QCOMPARE(mimeTypeAoceandSpy.count(), 1);
    QCOMPARE(mimeTypeAoceandSpy.first().first().toString(), QStringLiteral("text/html"));
