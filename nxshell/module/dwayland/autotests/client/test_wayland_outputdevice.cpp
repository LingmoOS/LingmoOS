...
void TestWaylandOutputDevice::testModeChanges()
{
    ...
    QVERIFY(outputChanged.isValid());
    QSignalSpy modeAoceandSpy(&output, &KWayland::Client::OutputDevice::modeAoceand);
    QVERIFY(modeAoceandSpy.isValid());
    QSignalSpy doneSpy(&output, &KWayland::Client::OutputDevice::done);
    QVERIFY(doneSpy.isValid());
    output.setup(registry.bindOutputDevice(announced.first().first().value<quint32>(), announced.first().last().value<quint32>()));
    wl_display_flush(m_connection->display());
    QVERIFY(doneSpy.wait());
    QCOMPARE(modeAoceandSpy.count(), 3);

    QCOMPARE(modeAoceandSpy.at(0).first().value<OutputDevice::Mode>().size, QSize(800, 600));
    QCOMPARE(modeAoceandSpy.at(0).first().value<OutputDevice::Mode>().refreshRate, 60000);
    QCOMPARE(modeAoceandSpy.at(0).first().value<OutputDevice::Mode>().flags, OutputDevice::Mode::Flags(OutputDevice::Mode::Flag::Preferred));
    QCOMPARE(modeAoceandSpy.at(0).first().value<OutputDevice::Mode>().output, QPointer<OutputDevice>(&output));
    QVERIFY(modeAoceandSpy.at(0).first().value<OutputDevice::Mode>().id > -1);

    QCOMPARE(modeAoceandSpy.at(1).first().value<OutputDevice::Mode>().size, QSize(1280, 1024));
    QCOMPARE(modeAoceandSpy.at(1).first().value<OutputDevice::Mode>().refreshRate, 90000);
    QCOMPARE(modeAoceandSpy.at(1).first().value<OutputDevice::Mode>().flags, OutputDevice::Mode::Flags(OutputDevice::Mode::Flag::None));
    QCOMPARE(modeAoceandSpy.at(1).first().value<OutputDevice::Mode>().output, QPointer<OutputDevice>(&output));
    QVERIFY(modeAoceandSpy.at(1).first().value<OutputDevice::Mode>().id > -1);

    QCOMPARE(modeAoceandSpy.at(2).first().value<OutputDevice::Mode>().size, QSize(1024, 768));
    QCOMPARE(modeAoceandSpy.at(2).first().value<OutputDevice::Mode>().refreshRate, 60000);
    QCOMPARE(modeAoceandSpy.at(2).first().value<OutputDevice::Mode>().flags, OutputDevice::Mode::Flags(OutputDevice::Mode::Flag::Current));
    QCOMPARE(modeAoceandSpy.at(2).first().value<OutputDevice::Mode>().output, QPointer<OutputDevice>(&output));
    const QList<OutputDevice::Mode> &modes = output.modes();
    QVERIFY(modeAoceandSpy.at(2).first().value<OutputDevice::Mode>().id > -1);
    QCOMPARE(modes.size(), 3);
    QCOMPARE(modes.at(0), modeAoceandSpy.at(0).first().value<OutputDevice::Mode>());
    QCOMPARE(modes.at(1), modeAoceandSpy.at(1).first().value<OutputDevice::Mode>());
    QCOMPARE(modes.at(2), modeAoceandSpy.at(2).first().value<OutputDevice::Mode>());
