...
private:
     ...
     *  清除选中,清除过滤
     **/
    void clearItemStatusAndFilter(oceanui_fcitx_configtool::widgets::FcitxSettingsGroup *group, const bool &falg = false);

private:
    oceanui_fcitx_configtool::widgets::FcitxSettingsGroup *m_allIMGroup {nullptr};//可用输入法列表界面
    oceanui_fcitx_configtool::widgets::FcitxSettingsGroup *m_searchIMGroup {nullptr};//搜索输入法列表界面
