...
FcitxConfigPlugin::FcitxConfigPlugin()
{
    m_translator.load(QLocale::system(),
                      QStringLiteral("lingmo-fcitxconfigtool-plugin"),
                      QStringLiteral("_"),
                      QStringLiteral("/usr/share/lingmo-fcitxconfigtool-plugin/translations"));
...
void FcitxConfigPlugin::active()
{
    ...
    connect(m_imWindow, &IMWindow::requestNextPage, [ = ](QWidget * const w) {
        m_frameProxy->pushWidget(this, w, oceanuiV20::FrameProxyInterface::PushType::CoverTop);
    });
    m_frameProxy->pushWidget(this, m_imWindow, oceanuiV20::FrameProxyInterface::PushType::Normal);
}
...
{
    return QStringLiteral(":/translations/lingmo-fcitxconfigtool-plugin_%1.ts");
}
