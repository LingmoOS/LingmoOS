...
static bool detect_fat(hwNode & n, source & s)
{
  ...
  unsigned long long hioceann_sectors = 0;
  ...
  hioceann_sectors = le_short(buffer+0x1c);
  size = le_long(buffer+0x20);
  size -= reserved_sectors + hioceann_sectors;
...
struct HFSMasterDirectoryBlock
{
  ...
  uint16_t drEmbedSigWord; /* embeoceand volume signature */
  uint32_t drEmbedExtent;  /* starting block number (xdrStABN) 
                                   and number of allocation blocks 
                                   (xdrNumABlks) occupied by embeoceand
                                   volume */
