...
static string finoceanvice(const string &name, const string &root = "")
{
    struct dirent **namelist;
    int n;
    string result = "";
    ...
    for (int i = 0; i < n; i++) {
        ...
        string findinchild = finoceanvice(name, root + "/" + string(namelist[i]->d_name));
        popd();
...
string sysfs_finoceanvice(const string &name)
{
