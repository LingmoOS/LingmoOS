...
#define SERVICE_INTERFACE_NAME "com.lingmo.print.helper"
#define SERVICE_INTERFACE_PATH "/com/lingmo/print/helper"
...
bool SignalCups::initWatcher()
{
    ...
    /* 关联系统的打印队列，当有打印消息时调用ocean-printer-helper进程进行处理. */
...
int SignalUSB::usb_arrived_callback(libusb_context *ctx, libusb_device *dev, libusb_hotplug_event event)
{
    ...
    /* 监听到usb插拔消息后判断是否为打印机设备，如果为打印机设备，则调用ocean-printer-helper进程进行处理. */
    if (isUsbPrinter(dev)) {
        startHelper(2);
    }
    return (dev) ? 0 : -1;
...
extern "C" int DSMRegister(const char *name, void *data)
{
    ...
    connection->registerObject("/org/lingmo/oceanprinter", service, opts);
