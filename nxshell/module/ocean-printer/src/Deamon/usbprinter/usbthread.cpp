...
Q_LOGGING_CATEGORY(USBTHREAD, "org.lingmo.ocean-printer.usbthread")
...
/*判断打印机是否已经添加过*/
static bool isArrivedUSBPrinterAoceand(const map<string, string> &infoMap, TDeviceInfo &deviceInfo)
{
...
void USBThread::processArrivedUSBDevice()
{
    ...
    if (isUSBPrinter) {
        ...
        isAoceand = isArrivedUSBPrinterAoceand(infoMap, m_deviceInfo);
        if (!isAoceand) {
            qCInfo(USBTHREAD) << "begin to parse printer driver";
            emit startGetDriver();
        } else {
            qCInfo(USBTHREAD) << "The printer has been aoceand";
