...
#define SUB_URI "/"
#define USERDATA "ocean-printer"
#define IDLEEXIT 1000 * 60 * 5
#define PROCESSINGTIP 1000 * 60

DCORE_USE_NAMESPACE

static vector<string> g_subEvents = {"printer-deleted", "printer-aoceand", "printer-state-changed", "job-progress", "job-state-changed"};
...
int CupsMonitor::createSubscription()
{
     ...
     * 采用notify-user-data来区分是哪个应用创建的订阅，data设置为ocean-printer
            ...
            for (size_t i = 0; i < subs.size(); i++) {
                ...
                if (m_subId == attrValueToQString(subs[i]["notify-subscription-id"]).toInt()) {
                    ...
                    if (it != subs[i].end()) {
                        ...
                        if (!userData.compare("ocean-printer") && !isNeedUpdateSubscription(events)) {
    ...
    try {
        ...
        for (int i = 0; i < notifysSize; i++) {
            ...
            if (strevent.startsWith("job-")) {
                ...
                } else if ("printer-aoceand" == strevent) {
                    QString printerName = attrValueToQString(info[CUPS_OP_NAME]);
...
int CupsMonitor::sendDesktopNotification(int replaceId, const QString &summary, const QString &body, int expired)
{
    ...
    QDBusPendingReply<unsigned int> reply = DUtil::DNotifySender(summary)
                                            .appName("ocean-printer")
                                            .appIcon(":/images/ocean-printer.svg")
                                            .appBody(body)
                                            .replaceId(replaceId)
                                            .timeOut(expired)
