...
void handler(int signo)
{
    ...
    if (signo == SIGUSR1) {
        pid_t pid = getpid();
        QProcess process;
        QString cmd = QString("ocean-printer-helper -r %1").arg(pid);
...
int main(int argc, char *argv[])
{
    ...
    qApp->setApplicationName("ocean-printer-helper");
    qApp->setApplicationVersion(DApplication::buildVersion("1.0"));
    qApp->setProductIcon(QIcon(":/images/ocean-printer.svg"));
    ...
    if (!DGuiApplicationHelper::setSingleInstance("ocean-printer-helper")) {
        //进程设置单例失败，杀死原始进程，继续设置单例，虽然返回true，但是实际没有生效，所以先杀死原始进程，再设置新进程单例
        qWarning() << "ocean-printer-helper is running";
        return -1;
    }
    // 绑定SIGUSR1信号
