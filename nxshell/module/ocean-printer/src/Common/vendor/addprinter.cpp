...
static bool isPackageInstallable(const QString &packagename)
{
    ...
    QString srcList = "Dir::Etc::SourceList=/etc/lingmo/lastore-daemon/sources.list.d/local-driver.list";
...
endUpdate:
    qCDebug(COMMONMOUDLE) << "Disconnect com.lingmo.lastore updateSourceChanged";
...
void InstallInterface::propertyChanged(const QDBusMessage &msg)
{
    ...
    qCDebug(COMMONMOUDLE) << "Disconnect com.lingmo.lastore PropertiesChanged";
    QDBusConnection::systemBus().disconnect(getPackageInterface()->service(),
                                            m_jobPath,
                                            "org.freedesktop.DBus.Properties",
                                            "PropertiesChanged",
...
void AddCanonCAPTPrinter::slotProcessFinished(int iCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus != QProcess::NormalExit || 0 != iCode) {
        ...
        if (iCode == 2) { // 佳能capt打印机已经添加，不允许重复添加
            m_strErr = QObject::tr("The Canon capt printer has been aoceand and is not allowed to be aoceand again.");
        } else if (iCode == -11) {
