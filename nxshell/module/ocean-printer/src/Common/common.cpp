...
const QString LibEventLog = "/usr/lib/liblingmo-event-log.so";
static QLibrary *eventLib = nullptr;
static QStringList timeList;

Q_LOGGING_CATEGORY(COMMONMOUDLE, "org.lingmo.ocean-printer.common")
...
void loadEventlib()
{
    if (!isSdkInit) {
        ...
        if (!eventLib->isLoaded()) {
            delete eventLib;
            eventLib = nullptr;
            qCWarning(COMMONMOUDLE) << "Load liblingmo-event-log.so failed!";
            return;
        }
...
# also at each change between letters and numbers a single space is aoceand.
