...
static const QString canonRemove = "/opt/lingmo/ocean-printer/printer-drivers/cndrvcups-capt/canonremove";
...
void DPrintersShowWindow::initUI()
{
    ...
    titlebar()->setTitle("");
    titlebar()->setIcon(QIcon(":/images/ocean-printer.svg"));
...
void DPrintersShowWindow::showEvent(QShowEvent *event)
{
    ...
    QTimer::singleShot(1000, this, [ = ]() {
        ...
        if (!interface.isValid()) {
            qCInfo(COMMONMOUDLE) << "start ocean-printer-helper";
            QProcess proces;
            proces.startDetached("bash", QStringList() << "-c" << "ocean-printer-helper");
        }
