...
Q_LOGGING_CATEGORY(PRINTERAPP, "org.lingmo.ocean-printer.app")
...
int PrinterApplication::create()
{
    ...
    qApp->setApplicationName("ocean-printer");
    ...
    if (!DGuiApplicationHelper::setSingleInstance("ocean-printer")) {
        //进程设置单例失败，杀死原始进程，继续设置单例，虽然返回true，但是实际没有生效，所以先杀死原始进程，再设置新进程单例
        qWarning() << "ocean-printer is running";
        return -2;
    }
