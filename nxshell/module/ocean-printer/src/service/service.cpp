...
int Service::CanonPrinterInstall(const QStringList &args)
{
    ...
    m_proc->start("/opt/lingmo/ocean-printer/printer-drivers/cndrvcups-capt/canonadd", args);
...
int Service::CanonPrinterRemove(const QStringList &args)
{
    ...
    m_proc->start("/opt/lingmo/ocean-printer/printer-drivers/cndrvcups-capt/canonremove", args);
    m_proc->waitForFinished();
    return m_proc->exitCode();
}
...
bool Service::IsAuthorizePath()
{
    ...
    if (name == "/usr/bin/ocean-printer" || name == "/usr/bin/ocean-printer-helper") {
        return true;
    }
    return false;
}
