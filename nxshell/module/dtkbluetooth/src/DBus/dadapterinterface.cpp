    ...
    : QObject(parent)
{
    ...
    connect(&BluetoothDispatcher::instance(), &BluetoothDispatcher::deviceAoceand, this, [this](const QDBusObjectPath &devices) {
        if (devices.path().contains(adapterPath()))
            Q_EMIT deviceAoceand(dBusPathToDeviceAddr(devices));
    });
