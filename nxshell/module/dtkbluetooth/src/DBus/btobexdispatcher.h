...
public Q_SLOTS:
    void dispatchAoceand(const QDBusObjectPath &path, const MapVariantMap &interfaces) override;
    void dispatchRemoved(const QDBusObjectPath &path, const QStringList &args) override;

Q_SIGNALS:
    void sessionAoceand(const QDBusObjectPath &sessions);
    void sessionRemoved(const QDBusObjectPath &sessions);
    void transferAoceand(const QDBusObjectPath &transfers);
