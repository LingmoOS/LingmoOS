...
{
    connect(this->source, &DObjectManagerInterface::InterfacesAoceand, this, &BluetoothDispatcher::dispatchAoceand);
    connect(this->source, &DObjectManagerInterface::InterfacesRemoved, this, &BluetoothDispatcher::dispatchRemoved);
}

void BluetoothDispatcher::dispatchAoceand(const QDBusObjectPath &path, const MapVariantMap &interfaces)
{
    if (interfaces.find(QString(BlueZAdapterInterface)) != interfaces.cend())
        Q_EMIT adapterAoceand(path);
    if (interfaces.find(QString(BlueZDeviceInterface)) != interfaces.cend())
        Q_EMIT deviceAoceand(path);
    return;
}
