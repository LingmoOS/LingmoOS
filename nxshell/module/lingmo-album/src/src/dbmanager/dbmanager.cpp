...
void DBManager::checkDatabase()
{
    ...
    //2.PathHash作为存放在lingmo-album-delete下的文件名，但是为了方便用户维修电脑，把原始文件名带在后面
    //3.数据库里面存的是标准的PathHash，不带文件名
    //4.如果PathHash所代表的文件没有在lingmo-album-delete下找到，则表示是老版本的删除策略，恢复的时候以此为依据
...
void DBManager::insertTrashImgInfos(const DBImgInfoList &infos, bool showWaitDialog)
{
    ...
    //1.生成路径hash，复制图片到lingmo-album-delete，删除原图至回收站
    QStringList pathHashs;
    for (const auto &info : infos) {
        //计算路径hash
