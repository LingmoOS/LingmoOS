        ...
        if (!imgPath.isNull()) {
            ...
            // 202011/12 bug54279
            {
                ...
                if (/*!qEnvironmentVariableIsEmpty("FLATPAK_APPID")*/ 1) {
                    ...
                    // com.lingmo.daemon.Appearance.Set background /home/test/test.png
                    qDebug() << "SettingWallpaper: "
                             << "flatpak" << path;
                    QDBusInterface interfaceV23(
                        "org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1", "org.lingmo.ocean.Appearance1");
                    QDBusInterface interfaceV20(
                        "com.lingmo.daemon.Appearance", "/com/lingmo/daemon/Appearance", "com.lingmo.daemon.Appearance");
                    ...
                    if (interfaceV23.isValid() || interfaceV20.isValid()) {
                        ...
                        if (isWayland) {
                            QDBusInterface interfaceWaylandV23(
                                "org.lingmo.ocean.Display1", "/org/lingmo/ocean/Display1", "org.lingmo.ocean.Display1");
                            if (interfaceWaylandV23.isValid()) {
                                screenname = qvariant_cast<QString>(interfaceWaylandV23.property("Primary"));
                                qDebug() << qPrintable("SetWallpaper: v23 wayland get org.lingmo.ocean.Display1 Primary property");

                            } else {
                                QDBusInterface interfaceWaylandV20(
                                    "com.lingmo.daemon.Display", "/com/lingmo/daemon/Display", "com.lingmo.daemon.Display");
                                screenname = qvariant_cast<QString>(interfaceWaylandV20.property("Primary"));
                                qDebug() << qPrintable(
                                    "SetWallpaper: v20 wayland get com.lingmo.daemon.Display Primary property");
                            }
                        } else {
                        ...
                        if (interfaceV23.isValid()) {
                            ...
                            qDebug() << qPrintable(
                                "SetWallpaper: Using v23 interface org.lingmo.ocean.Appearance1.SetMonitorBackground");
                        ...
                        if (interfaceV20.isValid() && !settingSucc) {
                            ...
                            qDebug() << qPrintable(
                                "SetWallpaper: Using v20 interface com.lingmo.daemon.Appearance.SetMonitorBackground");
                            qDebug() << qPrintable(QString("DBus Param %1, %2").arg(screenname).arg(path));
...
bool FileControl::isCheckOnly()
{
    ...
    QString appName = "lingmo-image-viewer";
    if (m_viewerType == imageViewerSpace::ImgViewerTypeAlbum) {
        appName = "lingmo-album";
    }
    std::string path = ("/home/" + userName + "/.cache/lingmo/" + appName + "/").toStdString();
...
void FileControl::showShortcutPanel(int windowCenterX, int windowCenterY)
{
    ...
    m_shortcutViewProcess->start("lingmo-shortcut-viewer", shortcutString);
}
