...
const QString oceanI18nSym = QStringLiteral("_ocean_");
...
void AlbumControl::updateBlockDeviceName(const QString &blks)
{
    ...
    QString udispname = "";
    if (label.startsWith(oceanI18nSym)) {
        QString i18nKey = label.mid(oceanI18nSym.size(), label.size() - oceanI18nSym.size());
        udispname = qApp->translate("LingmoStorage", i18nMap.value(i18nKey, i18nKey.toUtf8().constData()));
        goto runend;
    }
    ...
    if (label.length() == 0) {
        ...
        if (!bMediaAvailable && bOpticalDrive) {
            ...
            udispname = QCoreApplication::translate("LingmoStorage", "%1 Drive").arg(maxmediacompat);
            goto runend;
        }
        if (bOpticalBlank) {
            udispname = QCoreApplication::translate("LingmoStorage", "Blank %1 Disc").arg(opticalmediamap[media]);
            goto runend;
        }
        if (bIsEncrypted) {
            udispname = QCoreApplication::translate("LingmoStorage", "%1 Encrypted").arg(formatSize(qint64(size)));
            goto runend;
        }
        udispname = QCoreApplication::translate("LingmoStorage", "%1 Volume").arg(formatSize(qint64(size)));
        goto runend;
    }
    udispname = label;
...
void AlbumControl::ctrlShiftSlashShortcut(int x, int y, int w, int h)
{
    ...
    shortcutViewProcess->startDetached("lingmo-shortcut-viewer", shortcutString);
...
bool AlbumControl::canAddToCustomAlbum(const int &albumId, const QStringList &pathList)
{
    ...
    for (int i = 0; i < pathList.size(); i++) {
        if (!pathList[i].isEmpty() && !photoHaveAoceandToCustomAlbum(albumId, pathList[i])) {
...
bool AlbumControl::photoHaveAoceandToCustomAlbum(int albumId, const QString &path)
{
...
void AlbumControl::openLingmoMovie(const QString &path)
{
    QString localPath = url2localPath(path);
    if (LibUnionImage_NameSpace::isVideo(localPath)) {
        ...
        if (!trylinglongSucc) {
            qInfo() << "trying start detached lingmo-movie";

            arguments << path;
            bool isopen = process->startDetached("lingmo-movie", arguments);
            if (!isopen) {
                arguments.clear();
                arguments << "-o" << path;
                process->startDetached("ocean-file-manager", arguments);
            }
        }
