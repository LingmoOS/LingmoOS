...
class ComLingmoApiXEventMonitorInterface: public QDBusAbstractInterface
{
    Q_OBJECT
public:
    typedef QList<QRect> AreaList;
    ComLingmoApiXEventMonitorInterface(const QString &service, const QString &path, const char *interface, QObject *parent = nullptr,
                                       const QDBusConnection &con = QDBusConnection::sessionBus());

    ~ComLingmoApiXEventMonitorInterface();
...
namespace com {
  namespace lingmo {
    namespace api {
      typedef ::ComLingmoApiXEventMonitorInterface XEventMonitor;
    }
  }
}
#endif
