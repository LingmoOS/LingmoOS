...
Q_GLOBAL_STATIC_WITH_ARGS(DTK_CORE_NAMESPACE::DConfig, _d_dconfig, ("org.lingmo.dtk.preference"));
...
void DGuiApplicationHelperPrivate::notifyAppThemeChangedByEvent()
{
    ...
    // 而在lingmo平台下, 系统级别的QPalette来源自 \a applicationPalette()
...
/*!
  ...
  \var DGuiApplicationHelper::Attribute DGuiApplicationHelper::IsLingmoPlatformTheme
  获取当前是否使用lingmo的platformtheme插件，platformtheme插件可以为Qt程序提供特定的控件样式，默认使用chameleon主题。
  ...
  获取当前是否运行在lingmo平板环境中，检测XDG_CURRENT_DESKTOP环境变量是不是tablet结尾。
  
  \var DGuiApplicationHelper::Attribute DGuiApplicationHelper::IsLingmoEnvironment
  获取当前是否运行在lingmo桌面环境中，检测XDG_CURRENT_DESKTOP环境变量是不是lingmo。
 */
...
bool DGuiApplicationHelper::setSingleInstance(const QString &key, DGuiApplicationHelper::SingleScope singleScope)
{
    ...
    // FIX oceanui 使用不同的 key 两次调用 setSingleInstance 后无法启动的问题
    qint64 pid = -1;
...
QStringList DGuiApplicationHelper::userManualPaths(const QString &appName)
{
    ...
    for (int i = 0; i < pathlist.size(); ++i) {
        QString strAssetPath = QStringList{pathlist[i], "lingmo-manual/manual-assets"}.join(QDir::separator());
...
bool DGuiApplicationHelper::testAttribute(DGuiApplicationHelper::Attribute attribute)
{
    ...
    case IsLingmoPlatformTheme:
        if (!QGuiApplicationPrivate::platform_name) {
            return false;
        }
        return QString(typeid(*QGuiApplicationPrivate::platform_theme).name()).contains("QLingmoTheme");
    case IsLingmoEnvironment: {
        const auto &de = QGuiApplicationPrivate::instance()->platformIntegration()->services()->desktopEnvironment();
        return de.toLower().contains("lingmo") || de == "OCEAN";
    }
