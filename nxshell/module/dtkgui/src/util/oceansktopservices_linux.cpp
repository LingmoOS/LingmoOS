...
static QDBusInterface soundEffectInterface()
{
    ...
    bool isNewInterface = activatableServiceNames.contains(QLatin1String("org.lingmo.ocean.SoundEffect1"));
#else
    bool isNewInterface = false; // Qt 5.14 以下就直接用旧的接口
#endif
    const QLatin1String service(isNewInterface ? "org.lingmo.ocean.SoundEffect1" :"com.lingmo.daemon.SoundEffect");
    const QLatin1String path(isNewInterface ? "/org/lingmo/ocean/SoundEffect1" : "/com/lingmo/daemon/SoundEffect");
    const QLatin1String interface(isNewInterface ? "org.lingmo.ocean.SoundEffect1" :"com.lingmo.daemon.SoundEffect");

    // 使用后端 dbus 接口播放系统音频，音频存放目录： /usr/share/sounds/lingmo/stereo/
