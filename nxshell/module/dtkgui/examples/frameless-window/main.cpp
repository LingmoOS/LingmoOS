...
{
    xcb_atom_t _lingmo_no_titlebar = internAtom(QT_STRINGIFY(_LINGMO_NO_TITLEBAR), false);
    if (!isSupported(_lingmo_no_titlebar))
        return false;

    quint8 value = on;
    setWindowProperty(WId, _lingmo_no_titlebar, XCB_ATOM_CARDINAL, &value, 1, 8);

    // force enable window decorations, becouse we need the window shadow and border, its contains in decorations.
    xcb_atom_t _lingmo_force_decorate = internAtom(QT_STRINGIFY(_LINGMO_FORCE_DECORATE), false);
    if (on) {
        quint8 value = on;
...
QObject *createNativeSettingsFor(QWindow *window)
{
    ...
    // https://github.com/lingmoos/qt5platform-plugins/blob/master/src/dnativesettings.h
...
int main(int argc, char *argv[])
{
    ...
    if (setNoTitlebar(window.winId(), true)) {
        ...
        // Like as https://github.com/lingmoos/dtkwidget/blob/master/src/widgets/dtitlebar.cpp
