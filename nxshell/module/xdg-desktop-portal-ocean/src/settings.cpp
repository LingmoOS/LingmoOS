...
Q_LOGGING_CATEGORY(XdgDesktopOCEANSetting, "xdg-ocean-settings")
...
{
    qCDebug(XdgDesktopOCEANSetting) << "Read: group " << group << " key " << key;

    QObject *obj = QObject::parent();

    if (!obj) {
        qCWarning(XdgDesktopOCEANSetting) << "Failed to get dbus context";
        return;
    }
...
{
    qCDebug(XdgDesktopOCEANSetting) << "ReadAll";
    qCDebug(XdgDesktopOCEANSetting) << "ReadAll called with parameters:";
    qCDebug(XdgDesktopOCEANSetting) << "    groups: " << groups;
