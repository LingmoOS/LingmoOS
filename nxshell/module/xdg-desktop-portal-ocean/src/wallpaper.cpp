...
Q_LOGGING_CATEGORY(XdgDesktopOCEANWallpaper, "xdg-ocean-wallpaper")

WallPaperPortal::WallPaperPortal(QObject *parent)
    : QDBusAbstractAdaptor(parent)
    , m_personalizationManager(new PersonalizationManager)
{
    qCDebug(XdgDesktopOCEANWallpaper) << "WallPaper init";
}
                          ...
                          const QVariantMap &options)
{
    ...
    qCDebug(XdgDesktopOCEANWallpaper) << "Start set wallpaper";
    QDBusMessage primaryMsg = QDBusMessage::createMethodCall(QStringLiteral("org.lingmo.ocean.Display1"),
                                                             QStringLiteral("/org/lingmo/ocean/Display1"),
                                                             QStringLiteral("org.freedesktop.DBus.Properties"),
                                                             QStringLiteral("Get"));
    primaryMsg.setArguments({QVariant::fromValue(QStringLiteral("org.lingmo.ocean.Display1")), QVariant::fromValue(QStringLiteral("Primary"))});
    QDBusPendingReply<QDBusVariant> primaryReply = QDBusConnection::sessionBus().call(primaryMsg);
    if (primaryReply.isError()) {
        qCDebug(XdgDesktopOCEANWallpaper) << "setting failed";
        return 1;
    }

    QDBusMessage msg = QDBusMessage::createMethodCall(QStringLiteral("org.lingmo.ocean.Appearance1"),
    ...
    if (pcall.isValid()) {
        qCDebug(XdgDesktopOCEANWallpaper) << "setting succeed";
        return 0;
    }
    qCDebug(XdgDesktopOCEANWallpaper) << "setting failed";
