...
Q_LOGGING_CATEGORY(XdgDesktopOCEANNotification, "xdg-ocean-notification")
...
{
    qCDebug(XdgDesktopOCEANNotification) << "init NotificationPortal";
...
void NotificationPortal::AddNotification(const QString &app_id, const QString &id, const QVariantMap &notification)
{
    if (!m_service->isValid()) {
        qCWarning(XdgDesktopOCEANNotification) << "notification service has error!";
    }

    qCDebug(XdgDesktopOCEANNotification) << "notification add start";
...
{
    Q_UNUSED(app_id)
    qCDebug(XdgDesktopOCEANNotification) << "notification remove start";
