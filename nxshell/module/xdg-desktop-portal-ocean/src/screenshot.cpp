...
Q_LOGGING_CATEGORY(XdgDesktopOCEANScreenShot, "xdg-ocean-screenshot")
    ...
    : QDBusAbstractAdaptor(parent)
{
    ...
    qCDebug(XdgDesktopOCEANScreenShot) << "Screenshot and ColorPicker init";
}
...
uint ScreenshotPortal::PickColor(const QDBusObjectPath &handle,
                                 ...
                                 QVariantMap &results)
{
    qCDebug(XdgDesktopOCEANScreenShot) << "Start ColorPicker";
    ...
    qCDebug(XdgDesktopOCEANScreenShot) << "ColorPicker Failed";
    return 1;
}
...
uint ScreenshotPortal::Screenshot(const QDBusObjectPath &handle,
                                  ...
                                  QVariantMap &results)
{
    qCDebug(XdgDesktopOCEANScreenShot) << "Start screenshot";
    QDBusMessage msg = QDBusMessage::createMethodCall(QStringLiteral("org.kde.KWin"),
    ...
    if (pcall.isValid()) {
        ...
        qCDebug(XdgDesktopOCEANScreenShot) << "Succeed" << QString("Filepath is %1").arg(filepath);
        results.insert(QStringLiteral("uri"), QUrl::fromLocalFile(filepath).toString(QUrl::FullyEncoded));
        return 0;
    }
    qCDebug(XdgDesktopOCEANScreenShot) << "Screenshot Failed";
    return 1;
}
