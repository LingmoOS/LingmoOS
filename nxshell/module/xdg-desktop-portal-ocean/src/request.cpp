...
Q_LOGGING_CATEGORY(XdgDesktopOCEANRequest, "xdg-ocean-request")
    ...
    , m_data(data)
{
    ...
    if (!sessionBus.registerObject(m_handle.path(), this, QDBusConnection::ExportScriptableSlots)) {
        qCDebug(XdgDesktopOCEANRequest) << sessionBus.lastError().message();
        qCDebug(XdgDesktopOCEANRequest) << "Failed to register request object for" << m_handle.path();
        deleteLater();
    }
}
