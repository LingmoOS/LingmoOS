...
void QtPlayer::resetPlayInfo()
{
    ...
    QVariant volumeV = Utils::readDBusProperty("org.lingmo.daemon.Audio1", m_sinkInputPath,
                                               "org.lingmo.daemon.Audio1.SinkInput", "Volume");

    QVariant muteV = Utils::readDBusProperty("org.lingmo.daemon.Audio1", m_sinkInputPath,
                                             "org.lingmo.daemon.Audio1.SinkInput", "Mute");

    if (!volumeV.isValid() || !muteV.isValid()) return;

    QDBusInterface ainterface("org.lingmo.daemon.Audio1", m_sinkInputPath,
...
{
    QVariant v = Utils::readDBusProperty("org.lingmo.daemon.Audio1", "/org/lingmo/daemon/Audio1",
    ...
    for (auto curPath : allSinkInputsList) {
        QVariant nameV = Utils::readDBusProperty("org.lingmo.daemon.Audio1", curPath.path(),
                                                 "org.lingmo.daemon.Audio1.SinkInput", "Name");

        if (!nameV.isValid() || nameV.toString() != "Lingmo Music")
            continue;
