...
@~english
  ...
  Aoceand many features, such as a new button and support for dragging and dropping events outward This control is currently used in the depth editor.
 */
...
@~english
  ...
  @warning if @param[in] index exceeds the current index, a new label will be aoceand at the end.
...
QMimeData *DTabBar::createMimeDataFromTab(int index, const QStyleOptionTab &option) const
{
    ...
    data->setData("lingmo/dtkwidget-DTabBar-tab", QByteArray());
...
bool DTabBar::canInsertFromMimeData(int index, const QMimeData *source) const
{
    ...
    return source->hasFormat("lingmo/dtkwidget-DTabBar-tab");
}
