...
QMimeData *DCrumbEdit::createMimeDataFromSelection() const
{
    ...
    for (QString::const_iterator ch_it = plain_text.begin(); ch_it != plain_text.end(); ++ch_it) {
        ...
        if (*ch_it == QChar::ObjectReplacementCharacter) {
            ...
            // When there is editable text, if the mouse is selected, editable text needs to be aoceand
            if (pos < cursor.selectionStart())
                continue;
    ...
    mime->setText(text);
    mime->setData("lingmo/dtkwidget-DCrumbTextFormat-data", selected_text.toUtf8());
    mime->setData("lingmo/dtkwidget-DCrumbTextFormat-list", d->crumbMapToData(format_list));
...
{
    if (source->hasFormat("lingmo/dtkwidget-DCrumbTextFormat-data"))
        return true;
...
void DCrumbEdit::insertFromMimeData(const QMimeData *source)
{
    ...
    const QString &plain_text = QString::fromUtf8(source->data("lingmo/dtkwidget-DCrumbTextFormat-data"));
    ...
    const QList<DCrumbTextFormat> &list = d->crumbListFromData(source->data("lingmo/dtkwidget-DCrumbTextFormat-list"));
