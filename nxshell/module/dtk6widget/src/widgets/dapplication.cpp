    ...
    DObjectPrivate(q)
{
    ...
    // If not in ocean and not use lingmo platform theme, force set style to chameleon.
    if (!DGuiApplicationHelper::testAttribute(DGuiApplicationHelper::Attribute::IsLingmoPlatformTheme) &&
        !DGuiApplicationHelper::testAttribute(DGuiApplicationHelper::Attribute::IsLingmoEnvironment)) {
...
{
    auto basename = "com.lingmo.SingleInstance.";
...
void DApplicationPrivate::_q_onNewInstanceStarted()
{
    ...
    for (QWidget *window : qApp->topLevelWidgets()) {
        if (qobject_cast<DMainWindow*>(window)) {
            ...
            if (window->isMinimized() || window->isHioceann())
                window->showNormal();
...
/*!
  ...
  - 会根据系统gsettings中 com.lingmo.ocean.dapplication 的 qpixmapCacheLimit 值来设置 QPixmapCache::cacheLimit ；
  - 会根据系统gsettings中 com.lingmo.ocean.touchscreen longpress-duration 的值来设置 QTapAndHoldGesture::timeout ；
  ...
  见代码注释或者 \l {https://github.com/lingmoos/internal-discussion/issues/430}
...
/*!
  ...
  依赖于 lingmo 平台主题插件（ocean-qt5integration 包中提供），实时更改
  屏幕缩放比是通过更改配置文件 ~/.config/lingmo/qt-theme.ini 实现，与此相关的
  配置项有三个：
  \list
...
/*!
  ...
  新的DApplication实例并返回。主要用于与lingmo-trubo服务相配合，用于共享
  lingmo-turbo dtkwidget booster中已经创建的DApplication对象，以此节省初始化时间。
    ...
    DObject(*new DApplicationPrivate(this))
{
    ...
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.dapplication"))
    {
        QGSettings gsettings("com.lingmo.ocean.dapplication", "/com/lingmo/ocean/dapplication/");
    ...
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.touchscreen")) {
        QGSettings gsettings("com.lingmo.ocean.touchscreen");
        if (gsettings.keys().contains("longpressDuration"))
            ...
            // user ends the longpress, OCEAN Dock recevies mouseReleaseEvent and checks for
            ...
            //
            // see: https://github.com/lingmoos/internal-discussion/issues/430
            //
...
/*!
  ...
  You must name the file correctly; if the program is ocean-dock，
  then the qm file for English locale would be ocean-dock_en.qm.
  ...
  使用这个函数需要保证翻译文件必须正确命名: 例如程序名叫 ocean-dock，
  那么翻译文件在中文locale下的名称必须是 ocean-dock_zh_CN.qm；翻译文件还需要放置
...
/*!
  \brief 用于跟 startocean 进行通信，告知 startocean 进程已经启动成功.

  \note 只有OCEAN系统组件需要使用此函数，普通应用无需使用。
 */
void DApplication::registerOCEANSession()
{
#ifdef Q_OS_LINUX
    QString envName("OCEAN_SESSION_PROCESS_COOKIE_ID");
    ...
    if (!cookie.isEmpty()) {
        DDBusSender()
                ...
                .path("/com/lingmo/SessionManager")
...
/*!
  ...
  默认文件通常为 "~/.config/lingmo/qt-theme.ini"
  其中包含应用的图标主题、字体、、屏幕缩放等相关的配置项。可应用于以root用户启动的
  应用，需要跟随某个一般用户的主题设置项。
  \a path 中不包含 "/lingmo/qt-theme.ini" 部分，如：path = "/tmp"，
  则配置文件路径为："/tmp/lingmo/qt-theme.ini"。
  \a path 不要以 "/" 结尾
  \warning 必须在构造 DApplication 对象之前调用
...
/*!
  ...
  产品名称不同与 applicationName ，应该是类似如“深度终端”，而不是 lingmo-terminal，
...
/*!
  ...
  默认情况下，该地址为 Uos 或者 Lingmo 官方网站地址。
 */
...
// see https://github.com/lingmoos/qt5integration/pull/20
