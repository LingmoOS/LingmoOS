...
void DTitlebarPrivate::init()
{
    ...
    auto config = new DConfig("org.lingmo.dtk.preference", "", q);
    bool isUpdated = config->value("featureUpdated", false).toBool();
    DStyle::setRedPointVisible(optionButton, isUpdated);

    uiPreferDonfig = new DConfig("org.lingmo.dtk.preference", "", q);
...
void DTitlebarPrivate::_q_addDefaultMenuItems()
{
    ...
    if (!switchThemeMenu && DGuiApplicationHelper::testAttribute(DGuiApplicationHelper::Attribute::IsLingmoPlatformTheme)) {
    ...
    // add feedback menu item for lingmo or uos application
    if (!feedbackAction && qApp->organizationName() == "lingmo" && !QStandardPaths::findExecutable("lingmo-feedback").isEmpty()) {
    ...
    // add toolbarAction menu item for lingmo or uos application
...
void DTitlebarPrivate::_q_feedbackActionTriggered() {
    QProcess::startDetached("lingmo-feedback", { qApp->applicationName() });
}
...
void DTitlebar::showMenu()
{
    ...
    if (d->menu) {
        ...
        DConfig config("org.lingmo.dtk.preference");
        bool isUpdated = config.value("featureUpdated", false).toBool();
...
/*!
  ...
  \param[in] while visible is true ，Replace the system title bar, otherwise the system title bar is hioceann.
 */
