...
void MainWindow::translateAppName()
{
    ...
    QDBusPendingCall call = DDBusSender()
            .service("com.lingmo.ocean.daemon.Launcher")
            .interface("com.lingmo.ocean.daemon.Launcher")
            .path("/com/lingmo/ocean/daemon/Launcher")
...
void Content::onCustomContextMenuRequested(QWidget *widget, const QString &appid, const QString &resource, const QString &subpath, const QString &key)
{
    ...
    QString setCmd = QString("ocean-dconfig set %1 -r %2 %3 -v %4").arg(appid).arg(resource).arg(key).arg(value);
    QString getCmd = QString("ocean-dconfig get %1 -r %2 %3").arg(appid).arg(resource).arg(key);
    if (!subpath.isEmpty()) {
