...
_ocean-dconfig() {
  ...
  local -a commands=(
    ...
    'gui:gui模式，用于启动GUI工具，需要安装对应的GUI工具ocean-dconfig-editor'
  )
  ...
  args)
    ...
    'list' | 'set' | 'get' | 'reset' | 'watch' | '-a')
      local result=($(ocean-dconfig list))
      compadd -a result
      ret=0
      ;;
    '-r')
      local a=$words[$(($last_index-1))]
		  local result=($(ocean-dconfig list -a $a))
      compadd -a result
      ret=0
      ;;
    '-k')
      ...
      local r=$words[$(($last_index-1))]
		  local result=($(ocean-dconfig --get -a $a -r $r))
      compadd -a result
      ret=0
      ;;
    *)
