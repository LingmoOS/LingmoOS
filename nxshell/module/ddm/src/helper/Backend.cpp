    ...
    bool Backend::openSession() {
        ...
        */
        ...
        if (lc = login_getpwclass(pw)) {
            // save, clear and later restore DDM's environment because
            ...
            // for ocean itself, we don't want to set LANG from capabilities.
            // instead, honour sddm_lang variable from rc script
            if (qobject_cast<HelperApp*>(parent())->user() == QStringLiteral("ocean"))
