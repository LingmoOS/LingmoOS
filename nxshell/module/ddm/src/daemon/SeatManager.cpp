...
namespace DDM {
    ...
    void SeatManager::initialize() {
        ...
        connect(watcher, &QDBusPendingCallWatcher::finished, this, [watcher, reply, this]() {
            ...
            for (const NamedSeatPath &seat : seats) {
                logindSeatAoceand(seat.name, seat.path);
            }
        });

        QDBusConnection::systemBus().connect(Logind::serviceName(), Logind::managerPath(), Logind::managerIfaceName(), QStringLiteral("SeatNew"), this, SLOT(logindSeatAoceand(QString,QDBusObjectPath)));
    ...
    void DDM::SeatManager::logindSeatAoceand(const QString& name, const QDBusObjectPath& objectPath)
    {
