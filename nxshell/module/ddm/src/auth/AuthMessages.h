...
namespace DDM {
    ...
    public:
        Prompt() { }
        Prompt(AuthPrompt::Type type, QString message, bool hioceann)
                : type(type), message(message), hioceann(hioceann) { }
        Prompt(const Prompt &o)
                : type(o.type), response(o.response), message(o.message), hioceann(o.hioceann) { }
        ~Prompt() {
            clear();
        }
        ...
        bool operator==(const Prompt &o) const {
            return type == o.type && response == o.response && message == o.message && hioceann == o.hioceann;
        }
        bool valid() const {
    ...
    inline QDataStream& operator<<(QDataStream &s, const Prompt &m) {
        s << qint32(m.type) << m.message << m.hioceann << m.response;
        return s;
    }
    ...
    inline QDataStream& operator>>(QDataStream &s, Prompt &m) {
        ...
        bool hioceann;
        QByteArray response;
        s >> type >> message >> hioceann >> response;
