      ...
      mMem(this)
{
    ...
    QDBusConnection::connectToBus(QDBusConnection::SessionBus, QString(name))
            .registerObject("/org/lingmo/SystemMonitorDaemon", this, opts);
}
...
void SystemMonitorService::showLingmoSystemMoniter()
{
    PrintDBusCaller()

            QString cmd("gdbus call -e -d  com.lingmo.SystemMonitorServer -o /com/lingmo/SystemMonitorServer -m com.lingmo.SystemMonitorServer.showLingmoSystemMoniter");
...
bool SystemMonitorService::checkCpuAlarm()
{
    ...
    if (mCpuUsage >= mAlarmCpuUsage && diffTime >= timeGap) {
        ...
        QString cmd = QString("gdbus call -e -d  com.lingmo.SystemMonitorServer -o /com/lingmo/SystemMonitorServer -m com.lingmo.SystemMonitorServer.showCpuAlarmNotify \"%1\" ").arg(QString::number(mCpuUsage));
        QTimer::singleShot(100, this, [=]() { QProcess::startDetached(cmd); });
    }
...
bool SystemMonitorService::checkMemoryAlarm()
{
    ...
    if (mMemoryUsage >= mAlarmMemoryUsage && diffTime > timeGap) {
        mLastAlarmTimeStamp = curTimeStamp;
        QString cmd = QString("gdbus call -e -d  com.lingmo.SystemMonitorServer -o /com/lingmo/SystemMonitorServer -m com.lingmo.SystemMonitorServer.showMemoryAlarmNotify \"%1\" ").arg(QString::number(mMemoryUsage));
        QTimer::singleShot(100, this, [=]() { QProcess::startDetached(cmd); });
    }
