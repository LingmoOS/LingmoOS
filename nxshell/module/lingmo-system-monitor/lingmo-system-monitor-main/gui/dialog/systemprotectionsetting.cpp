...
SystemProtectionSetting::SystemProtectionSetting(QObject *parent)
    : QObject(parent), mBackend(nullptr), mDsettings(nullptr), mDaemonInterface("org.lingmo.SystemMonitorDaemon", "/org/lingmo/SystemMonitorDaemon", "org.lingmo.SystemMonitorDaemon")
{
...
QPair<QWidget *, QWidget *> SystemProtectionSetting::createSettingLinkButtonHandle(QObject *obj)
{
    ...
    option->connect(button, &DCommandLinkButton::clicked, option, [=]() {
        qCDebug(app) << __FUNCTION__ << __LINE__ << "，will jump to ocean setting center notification page!";
...
void SystemProtectionSetting::onMessgaeSetting(QVariant value)
{
    Q_UNUSED(value);
    QString genericName;
    QFile desktopFile("/usr/share/applications/lingmo-system-monitor.desktop");
    ...
    if (genericName.isEmpty() == false) {
        // 跳转到设置页并指定Item
        QDBusMessage showOCEANControlCenterPage = QDBusMessage::createMethodCall(common::systemInfo().ControlCenterService,
        ...
        showOCEANControlCenterPage.setArguments(args);

        QDBusMessage replyMsg = QDBusConnection::sessionBus().call(showOCEANControlCenterPage);

        if (replyMsg.type() == QDBusMessage::ErrorMessage) {
            qCWarning(app) << __FUNCTION__ << __LINE__ << ", ocean control center dbus method call fail , error name :"
                       << replyMsg.errorName() << " , error msg :" << replyMsg.errorMessage();
        }

    } else {
        // 跳转到设置页
        // qdbus org.lingmo.ocean.ControlCenter1 /org/lingmo/ocean/ControlCenter1 org.lingmo.ocean.ControlCenter1.ShowModule notification

        QDBusMessage showOCEANControlCenter;
        if (!common::systemInfo().isOldVersion()) {
            showOCEANControlCenter = QDBusMessage::createMethodCall(common::systemInfo().ControlCenterService,
                                                                  common::systemInfo().ControlCenterPath,
                                                                  common::systemInfo().ControlCenterInterface,
                                                                  "ShowPage");
        } else {
            showOCEANControlCenter = QDBusMessage::createMethodCall("com.lingmo.ocean.ControlCenter",
                                                                  "/com/lingmo/ocean/ControlCenter",
                                                                  "com.lingmo.ocean.ControlCenter",
                                                                  "ShowModule");
        }
        ...
        QList<QVariant> args;
        args << "notification";
        showOCEANControlCenter.setArguments(args);

        QDBusMessage replyMsg = QDBusConnection::sessionBus().call(showOCEANControlCenter);
...
void SystemProtectionSetting::onSettingItemChanged(const QString &key, const QVariant &value)
{
    ...
    //QString cmd("qdbus org.lingmo.SystemMonitorDaemon /org/lingmo/SystemMonitorDaemon org.lingmo.SystemMonitorDaemon.");
    //qdbus 改为gdbus
    QString cmd("gdbus call -e -d  org.lingmo.SystemMonitorDaemon -o /org/lingmo/SystemMonitorDaemon -m org.lingmo.SystemMonitorDaemon.");
    bool needCall = false;
