...
void XWinKillPreviewWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    if (WaylandCentered) {
        ...
        for (QVector<ClientManagement::WindowState>::iterator it = m_windowStates.end() - 1;
            ...
            if (getpid() == it->pid || QString::fromStdString(it->resourceName) == "ocean-desktop" || it->isMinimized
                || QString::fromStdString(it->resourceName) == "lingmo-lingmoid-client"
                || QString::fromStdString(it->resourceName) == "ocean-dock")
                continue;
        ...
        for (QVector<ClientManagement::WindowState>::iterator it = m_windowStates.end() - 1;
            ...
            if (selRect.contains(pos)) {
                ...
                for (QVector<ClientManagement::WindowState>::iterator iter = m_windowStates.end() - 1;
                     iter != it; --iter) {
                    if (QString::fromStdString(iter->resourceName) == "ocean-desktop" || iter->isMinimized || QString::fromStdString(iter->resourceName) == "lingmo-lingmoid-client"
                        || QString::fromStdString(iter->resourceName) == "ocean-dock" || getpid() == iter->pid)
                        continue;
...
void XWinKillPreviewWidget::initUI()
{
    ...
    m_defaultCursor = QCursor(Qt::ForbioceannCursor);
