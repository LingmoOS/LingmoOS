...
void MonitorPlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    qApp->setApplicationName("lingmo-system-monitor-plugin");
    qApp->loadTranslator();
    ...
    QDBusConnection::sessionBus().connect("com.lingmo.SystemMonitorPluginPopup", "/com/lingmo/SystemMonitorPluginPopup", "com.lingmo.SystemMonitorPluginPopup", "sysMonPopVisibleChanged", this, SLOT(onSysMonPopVisibleChanged(bool)));
#endif

    calcCpuRate(m_totalCPU, m_availableCPU);
    calcNetRate(m_down, m_upload);
#ifdef OCEAN_DOCK_NEW_VERSION
...
const QString MonitorPlugin::itemCommand(const QString &itemKey)
{
    if (itemKey == "system-monitor") {
        ...
        // DBusInterface::getInstance()->showOrHideLingmoSystemMonitorPluginPopupWidget();
        openSystemMonitor();
    }
    return "";

    //    return "dbus-send --print-reply --dest=com.lingmo.SystemMonitorPluginPopup /com/lingmo/SystemMonitorPluginPopup com.lingmo.SystemMonitorPluginPopup.showOrHideLingmoSystemMonitorPluginPopupWidget";
}
...
void MonitorPlugin::openSystemMonitor()
{
    auto launchProcessByAM = [](){
        ...
        if (reply.type() != QDBusMessage::ReplyMessage) {
            qCWarning(app) << "Launch lingmo-system-monitor main process error:" << reply.errorMessage();
            return;
        }
    };
    launchProcessByAM();
