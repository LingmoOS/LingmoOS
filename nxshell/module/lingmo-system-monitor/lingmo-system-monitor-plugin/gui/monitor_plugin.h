...
#ifdef  OCEAN_DOCK_MAJORV6
    #define  OCEAN_DOCK_NEW_VERSION
#endif
#endif

#ifdef USE_API_QUICKPANEL20
#include <ocean-dock/pluginsiteminterface_v2.h>
...
#define PLUGIN_METADATA()  Q_PLUGIN_METADATA(IID "com.lingmo.dock.PluginsItemInterface" FILE "system-monitor.json")
#endif
...
public:
    ...
    //!
    //! \brief pluginName 返回插件名称，用于在ocean-dock内部管理插件时使用
    //! \return 插件名称
    //!
    ...
    //!
    //! \brief itemWidget 返回插件主控件，用于显示在ocean-dock面板上
    ...
    //!
    //! \brief displayModeChanged ocean-dock显示模式发生改变时此接口被调用
    //! \param displayMode
    //!
