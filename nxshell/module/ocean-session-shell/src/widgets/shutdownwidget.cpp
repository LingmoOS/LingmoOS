...
void ShutdownWidget::initUI()
{
    ...
    if (findValueByQSettings<bool>(OCEANSESSIONCC::session_ui_configs, "Shutdown", "enableSystemMonitor", true)) {
        QFile file("/usr/bin/lingmo-system-monitor");
        if (file.exists()) {
...
void ShutdownWidget::runSystemMonitor()
{
    auto launchProcessByAM = []() {
        ...
        if (reply.type() == QDBusMessage::ReplyMessage) {
            qDebug() << "Launch lingmo-system-monitor successful!";
        } else {
...
void ShutdownWidget::updateLocale(std::shared_ptr<User> user)
{
    Q_UNUSED(user)
    //只有登陆界面才需要根据系统切换语言
    if(qApp->applicationName() == "ocean-lock") return;
