...
{
    m_inhibitorBlacklists << "NetworkManager" << "ModemManager" << "org.lingmo.ocean.Power1";
            ...
            for (int i = 0; i < inhibitList.count(); i++) {
                ...
                if (inhibitor.what.split(':', behavior).contains(type)
                    ...
                    if (connection.interface()->isServiceRegistered(inhibitor.who)) {
                        ...
                        QDBusInterface ifc(inhibitor.who, "/org/lingmo/ocean/InhibitHint1", "org.lingmo.ocean.InhibitHint1", connection);
