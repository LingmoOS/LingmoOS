...
#define PAM_SERVICE_LINGMO_NAME "ocean-lock"
...
LingmoAuthFramework::LingmoAuthFramework(QObject *parent)
    : QObject(parent)
    , m_authenticateInter(new AuthInter(AUTHRNTICATESERVICE, "/org/lingmo/ocean/Authenticate1", QDBusConnection::systemBus(), this))
    ...
    , m_RSA(nullptr)
{
    ...
    connect(m_authenticateInter, &AuthInter::FrameworkStateChanged, this, &LingmoAuthFramework::FrameworkStateChanged);
    connect(m_authenticateInter, &AuthInter::LimitUpdated, this, &LingmoAuthFramework::LimitsInfoChanged);
    connect(m_authenticateInter, &AuthInter::SupportedFlagsChanged, this, &LingmoAuthFramework::SupportedMixAuthFlagsChanged);
    connect(m_authenticateInter, &AuthInter::SupportEncryptsChanged, this, &LingmoAuthFramework::SupportedEncryptsChanged);

    /* 暂时将加密方式固定，后续有修改再调整 */
    setEncryption(0, {1});
}

LingmoAuthFramework::~LingmoAuthFramework()
{
 ...
 * @param account
 */
void LingmoAuthFramework::CreateAuthenticate(const QString &account)
{
 ...
 * @return void*
 */
void *LingmoAuthFramework::PAMAuthWorker(void *arg)
{
    LingmoAuthFramework *authFramework = static_cast<LingmoAuthFramework *>(arg);
    if (authFramework != nullptr) {
        authFramework->PAMAuthentication(authFramework->m_account);
    } else {
        qCritical() << "pam auth worker lingmo framework is nullptr";
    }
    return nullptr;
}
 ...
 * @param account
 */
void LingmoAuthFramework::PAMAuthentication(const QString &account)
{
    pam_handle_t *m_pamHandle = nullptr;
    pam_conv conv = {PAMConversation, static_cast<void *>(this)};
    const char *serviceName = isLingmoAuth() ? PAM_SERVICE_LINGMO_NAME : PAM_SERVICE_SYSTEM_NAME;
 ...
 * @return int
 */
int LingmoAuthFramework::PAMConversation(int num_msg, const pam_message **msg, pam_response **resp, void *app_data)
{
    LingmoAuthFramework *app_ptr = static_cast<LingmoAuthFramework *>(app_data);
    struct pam_response *aresp = nullptr;
    int idx = 0;

    QPointer<LingmoAuthFramework> isThreadAlive(app_ptr);
    if (!isThreadAlive) {
    ...
    for (idx = 0; idx < num_msg; ++idx) {
        ...
        case PAM_PROMPT_ECHO_OFF: {
            ...
            if (!QPointer<LingmoAuthFramework>(app_ptr)) {
                qCritical() << "pam: lingmo auth framework is null";
                return PAM_CONV_ERR;
            }
 ...
 * @param token
 */
void LingmoAuthFramework::SendToken(const QString &token)
{
    if (!m_waitToken) {
        return;
    }
 ...
 * @param message
 */
void LingmoAuthFramework::UpdateAuthState(const int state, const QString &message)
{
 ...
 * @brief 结束 PAM 认证服务
 */
void LingmoAuthFramework::DestroyAuthenticate()
{
 ...
 * @param method
 */
void LingmoAuthFramework::setEncryption(const int type, ArrayInt method)
{
    m_encryptType = type;
 ...
 * @brief 初始化加密服务
 */
void LingmoAuthFramework::initEncryptionService()
{
 ...
 * @param account
 */
void LingmoAuthFramework::encryptSymmtricKey(const QString &account)
{
 ...
 */
void LingmoAuthFramework::CreateAuthController(const QString &account, const int authType, const int appType)
{
    ...
    AuthControllerInter *authControllerInter = new AuthControllerInter("org.lingmo.ocean.Authenticate1", authControllerInterPath, QDBusConnection::systemBus(), this);
    m_authenticateControllers->insert(account, authControllerInter);

    connect(authControllerInter, &AuthControllerInter::FactorsInfoChanged, this, &LingmoAuthFramework::FactorsInfoChanged);
    connect(authControllerInter, &AuthControllerInter::IsFuzzyMFAChanged, this, &LingmoAuthFramework::FuzzyMFAChanged);
    connect(authControllerInter, &AuthControllerInter::IsMFAChanged, this, &LingmoAuthFramework::MFAFlagChanged);
    connect(authControllerInter, &AuthControllerInter::PINLenChanged, this, &LingmoAuthFramework::PINLenChanged);
    connect(authControllerInter, &AuthControllerInter::PromptChanged, this, &LingmoAuthFramework::PromptChanged);
 ...
 * @param account 用户名
 */
void LingmoAuthFramework::DestroyAuthController(const QString &account)
{
 ...
 */
void LingmoAuthFramework::StartAuthentication(const QString &account, const int authType, const int timeout)
{
    if (!m_authenticateControllers->contains(account)) {
        return;
    }
 ...
 * @param authType  认证类型
 */
void LingmoAuthFramework::EndAuthentication(const QString &account, const int authType)
{
    if (!m_authenticateControllers->contains(account)) {
        return;
    }
 ...
 * @param token     密文
 */
void LingmoAuthFramework::SendTokenToAuth(const QString &account, const int authType, const QString &token)
{
    if (!m_authenticateControllers->contains(account)) {
        return;
    }
 ...
 * @param flag
 */
void LingmoAuthFramework::SetAuthQuitFlag(const QString &account, const int flag)
{
    if (!m_authenticateControllers->contains(account)) {
        return;
    }
 ...
 * @return int
 */
int LingmoAuthFramework::GetSupportedMixAuthFlags() const
{
 ...
 * @return QString
 */
QString LingmoAuthFramework::GetPreOneKeyLogin(const int flag) const
{
 ...
 * @return int
 */
int LingmoAuthFramework::GetFrameworkState() const
{
 ...
 * @return QString 加密类型
 */
QString LingmoAuthFramework::GetSupportedEncrypts() const
{
 ...
 * @return QString  时间
 */
QString LingmoAuthFramework::GetLimitedInfo(const QString &account) const
{
 ...
 * @return int
 */
int LingmoAuthFramework::GetMFAFlag(const QString &account) const
{
    if (!m_authenticateControllers->contains(account)) {
        return 0;
    }
 ...
 * @return int
 */
int LingmoAuthFramework::GetPINLen(const QString &account) const
{
    if (!m_authenticateControllers->contains(account)) {
        return 0;
    }
 ...
 * @return int
 */
int LingmoAuthFramework::GetFuzzyMFA(const QString &account) const
{
    if (!m_authenticateControllers->contains(account)) {
        return 0;
    }
 ...
 * @return QString
 */
QString LingmoAuthFramework::GetPrompt(const QString &account) const
{
    if (!m_authenticateControllers->contains(account)) {
        return QString();
    }
 ...
 * @return
 */
bool LingmoAuthFramework::SetPrivilegesEnable(const QString &account, const QString &path)
{
    if (!m_authenticateControllers->contains(account)) {
        return false;
    }
 ...
 * @param account
 */
void LingmoAuthFramework::SetPrivilegesDisable(const QString &account)
{
    if (!m_authenticateControllers->contains(account)) {
        return;
    }
 ...
 * @return MFAInfoList
 */
MFAInfoList LingmoAuthFramework::GetFactorsInfo(const QString &account) const
{
    if (!m_authenticateControllers->contains(account)) {
        return MFAInfoList();
    }
 ...
 */
QString LingmoAuthFramework::AuthSessionPath(const QString &account) const
{
    if (!m_authenticateControllers->contains(account)) {
        return QString();
    }
...
bool LingmoAuthFramework::authSessionExist(const QString &account) const
{
    return m_authenticateControllers->contains(account) && m_authenticateControllers->value(account)->isValid();
}

/**
 * @brief LingmoAuthFramework::isLingmoAuthValid
 * 判断DA服务是否存在以及DA服务是否可用
 * @return DA认证是否可用
 */
bool LingmoAuthFramework::isLingmoAuthValid() const
{
    qDebug() << Q_FUNC_INFO
