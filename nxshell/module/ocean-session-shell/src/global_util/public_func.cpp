...
bool isLingmoAuth()
{
    const char* controlId = "com.lingmo.ocean.auth.control";
    if (QGSettings::isSchemaInstalled(controlId)) {
        const char *controlPath = "/com/lingmo/ocean/auth/control/";
        QGSettings controlObj(controlId, controlPath);
        const QString &key = "useLingmoAuth";
        bool useLingmoAuth = controlObj.keys().contains(key) && controlObj.get(key).toBool();
    #ifdef QT_DEBUG
        qDebug() << "use lingmo auth: " << useLingmoAuth;
    #endif
        return useLingmoAuth;
    }
    return true;
}
...
{
    return APP_TYPE_LOCK == appType ? OCEANSESSIONCC::LOCK_DCONFIG_SOURCE : OCEANSESSIONCC::LOGIN_DCONFIG_SOURCE;
}
...
void loadTranslation(const QString &locale)
{
    ...
    translator.load("/usr/share/ocean-session-shell/translations/ocean-session-shell_" + locale.split(".").first());
