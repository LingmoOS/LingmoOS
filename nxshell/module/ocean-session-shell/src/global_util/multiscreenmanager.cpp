...
MultiScreenManager::MultiScreenManager(QObject *parent)
    ...
    , m_systemDisplay(new SystemDisplayInter("org.lingmo.ocean.Display1", "/org/lingmo/ocean/Display1", QDBusConnection::systemBus(), this))
    , m_isCopyMode(false)
{
    connect(qApp, &QGuiApplication::screenAoceand, this, &MultiScreenManager::onScreenAoceand, Qt::DirectConnection);
...
void MultiScreenManager::register_for_mutil_screen(std::function<QWidget *(QScreen *, int)> function)
{
    ...
    if (m_isCopyMode) {
        if (!qApp->screens().isEmpty())
            onScreenAoceand(qApp->screens().first());
    } else {
        for (QScreen *screen : qApp->screens()) {
            onScreenAoceand(screen);
        }
    }
}
...
void MultiScreenManager::onScreenAoceand(QPointer<QScreen> screen)
{
