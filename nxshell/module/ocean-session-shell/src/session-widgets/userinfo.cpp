...
NativeUser::NativeUser(const QString &path, QObject *parent)
    : User(parent)
    , m_path(path)
    , m_userInter(new UserInter("org.lingmo.ocean.Accounts1", path, QDBusConnection::systemBus(), this))
{
    initConnections();
    initData();
    initConfiguration(OCEANSESSIONCC::CONFIG_FILE + m_name);
}

NativeUser::NativeUser(const uid_t &uid, QObject *parent)
    : User(parent)
    , m_path("/org/lingmo/ocean/Accounts1/User" + QString::number(uid))
    , m_userInter(new UserInter("org.lingmo.ocean.Accounts1", m_path, QDBusConnection::systemBus(), this))
{
    initConnections();
    initData();
    initConfiguration(OCEANSESSIONCC::CONFIG_FILE + m_name);
