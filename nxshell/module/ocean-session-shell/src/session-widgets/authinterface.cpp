...
AuthInterface::AuthInterface(SessionBaseModel *const model, QObject *parent)
    : QObject(parent)
    , m_model(model)
    , m_accountsInter(new AccountsInter("org.lingmo.ocean.Accounts1", "/org/lingmo/ocean/Accounts1", QDBusConnection::systemBus(), this))
    , m_loginedInter(new LoginedInter("org.lingmo.ocean.Accounts1", "/org/lingmo/ocean/Logined", QDBusConnection::systemBus(), this))
    , m_login1Inter(new DBusLogin1Manager("org.freedesktop.login1", "/org/freedesktop/login1", QDBusConnection::systemBus(), this))
    , m_powerManagerInter(new PowerManagerInter("org.lingmo.ocean.PowerManager1", "/org/lingmo/ocean/PowerManager1", QDBusConnection::systemBus(), this))
    , m_authenticateInter(new Authenticate("org.lingmo.ocean.Authenticate1", "/org/lingmo/ocean/Authenticate1", QDBusConnection::systemBus(), this))
    ...
    , m_loginUserList(0)
{
    if (m_login1Inter->isValid()) {
        ...
        if (m_model->appType() == AppType::Lock) {
            // v23上m_login1Inter->GetSessionByPID(0)接口已不可用，使用org.lingmo.Session获取
            QDBusInterface inter("org.lingmo.ocean.Session1", "/org/lingmo/ocean/Session1",
                                 "org.lingmo.ocean.Session1", QDBusConnection::sessionBus());
            QDBusReply<QString> reply = inter.call("GetSessionPath");
            if (reply.isValid())
                sessionSelf = reply.value();
            else
                qWarning() << "org.lingmo.ocean.Session1 get session path has error!";
        } else {
            // AppType::Login
...
void AuthInterface::onUserAoceand(const QString &user)
{
...
void AuthInterface::initDBus()
{
    ...
    connect(m_accountsInter, &AccountsInter::UserAoceand, this, &AuthInterface::onUserAoceand, Qt::QueuedConnection);
...
bool AuthInterface::isLingmo()
{
    // 这是临时的选项，只在Lingmo下启用同步认证功能，其他发行版下禁用。
#ifdef QT_DEBUG
    return true;
#else
    return getGSettings("","useLingmoAuth").toBool();
#endif
}
...
{
    // 替换接口org.freedesktop.login1 org.lingmo.ocean.SessionManager1,原接口的是否支持待机和休眠的信息不准确
