...
LockWorker::LockWorker(SessionBaseModel *const model, QObject *parent)
    ...
    , m_authFramework(new LingmoAuthFramework(this))
    , m_lockInter(new DBusLockService("org.lingmo.ocean.LockService1", "/org/lingmo/ocean/LockService1", QDBusConnection::systemBus(), this))
    , m_hotZoneInter(new DBusHotzone("org.lingmo.ocean.Zone1", "/org/lingmo/ocean/Zone1", QDBusConnection::sessionBus(), this))
    , m_sessionManagerInter(new SessionManagerInter("org.lingmo.ocean.SessionManager1", "/org/lingmo/ocean/SessionManager1", QDBusConnection::sessionBus(), this))
    ...
    , m_kwinInter(nullptr)
{
    ...
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.session-shell")) {
        m_gsettings = new QGSettings("com.lingmo.ocean.session-shell", "/com/lingmo/ocean/session-shell/", this);
    ...
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.power")) {
        m_powerGsettings = new QGSettings("com.lingmo.ocean.power", "/com/lingmo/ocean/power/", this);
    }
...
void LockWorker::initConnections()
{
   // 监听用户的新增、删除、上次登出用户、用户列表变化
    connect(m_accountsInter, &AccountsInter::UserAoceand, m_model, static_cast<void (SessionBaseModel::*)(const QString &)>(&SessionBaseModel::addUser));
    ...
    // connect(m_accountsInter, &AccountsInter::UserListChanged, m_model, &SessionBaseModel::updateUserList);  // UserListChanged信号的处理， 改用UserAoceand和UserDeleted信号替代
    ...
    /* org.lingmo.ocean.Authenticate1 */
    connect(m_authFramework, &LingmoAuthFramework::FrameworkStateChanged, this, &LockWorker::onFrameworkStateChanged);
    connect(m_authFramework, &LingmoAuthFramework::LimitsInfoChanged, this, [this](const QString &account) {
        qDebug() << "LingmoAuthFramework::LimitsInfoChanged:" << account;
        if (account == m_model->currentUser()->name()) {
            m_model->updateLimitedInfo(m_authFramework->GetLimitedInfo(account));
        }
    });
    connect(m_authFramework, &LingmoAuthFramework::SupportedEncryptsChanged, m_model, &SessionBaseModel::updateSupportedEncryptionType);
    connect(m_authFramework, &LingmoAuthFramework::SupportedMixAuthFlagsChanged, m_model, &SessionBaseModel::updateSupportedMixAuthFlags);
    connect(m_authFramework, &LingmoAuthFramework::FuzzyMFAChanged, m_model, &SessionBaseModel::updateFuzzyMFA);
    connect(m_authFramework, &LingmoAuthFramework::MFAFlagChanged, m_model, &SessionBaseModel::updateMFAFlag);
    connect(m_authFramework, &LingmoAuthFramework::PINLenChanged, m_model, &SessionBaseModel::updatePINLen);
    connect(m_authFramework, &LingmoAuthFramework::PromptChanged, m_model, &SessionBaseModel::updatePrompt);
    connect(m_authFramework, &LingmoAuthFramework::AuthStateChanged, this, &LockWorker::onAuthStateChanged);
    connect(m_authFramework, &LingmoAuthFramework::FactorsInfoChanged, m_model, &SessionBaseModel::updateFactorsInfo);
    ...
    /* org.lingmo.ocean.SessionManager1 */
    ...
    connect(m_limitsUpdateTimer, &QTimer::timeout, this, [this] {
        if (m_authFramework->isLingmoAuthValid())
    ...
    connect(m_dbusInter, &DBusObjectInter::NameOwnerChanged, this, [ = ](const QString &name, const QString &oldOwner, const QString &newOwner) {
        Q_UNUSED(oldOwner)
        if (name == "org.lingmo.ocean.Authenticate1" && newOwner != "" && m_model->visible() && m_sessionManagerInter->locked()) {
            m_resetSessionTimer->stop();
            endAuthentication(m_account, AT_All);
            createAuthentication(m_model->currentUser()->name());
        }
...
void LockWorker::initData()
{
    ...
    /* com.lingmo.udcp.iam */
    QDBusInterface ifc("com.lingmo.udcp.iam", "/com/lingmo/udcp/iam", "com.lingmo.udcp.iam", QDBusConnection::systemBus(), this);
...
void LockWorker::switchToUser(std::shared_ptr<User> user)
{
    ...
    QMetaObject::invokeMethod(this, [ user ] {
        QProcess::startDetached("ocean-switchtogreeter", (user->isLogin() ? QStringList() << user->name() : QStringList()));
    }, Qt::QueuedConnection);
}
