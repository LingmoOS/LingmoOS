    ...
    , m_autoExitTimer(nullptr)
{
    ...
    if (connection) {
        xcb_atom_t cook = internAtom(connection, "_LINGMO_LOCK_SCREEN", false);
        xcb_change_property(connection, XCB_PROP_MODE_REPLACE, static_cast<xcb_window_t>(winId()),
                            cook, XCB_ATOM_ATOM, 32, 1, &cook);

        // x11下通过窗口属性设置锁屏启动动效
        xcb_atom_t startup = internAtom(connection, "_LINGMO_NET_STARTUP", false);
    ...
    connect(model, &SessionBaseModel::prepareForSleep, this, [ = ](bool isSleep) {
        ...
        if (!isSleep) {
            ...
            if (QGSettings::isSchemaInstalled("com.lingmo.ocean.power")) {
                QGSettings powerSettings("com.lingmo.ocean.power", QByteArray(), this);
                if (!powerSettings.get("sleep-lock").toBool()) {
                    qDebug() << "sleep-lock is off, prepare unlock";
                    // 待机唤醒的一瞬间，会延迟收到DBusLogin1Manager::PrepareForSleep（true）, 进而最终传入到ocean-session的locked状态也是延迟的
                    // ocean-session 延迟收到locked后会重新发送锁定信号，导致这里unlock后又被重新lock
                    // 设置一个100ms的延时，等待ocean-session的locked状态和ocean-lock的locked状态完全同步后再进行隐藏锁屏的动作
...
{
    QDBusInterface powerInter("org.lingmo.ocean.Power1","/org/lingmo/ocean/Power1","org.lingmo.ocean.Power1");
