...
#define LOCKSERVICE_PATH "/org/lingmo/ocean/LockService1"
#define LOCKSERVICE_NAME "org.lingmo.ocean.LockService1"
#define SECURITYENHANCE_PATH "/com/lingmo/daemon/SecurityEnhance"
#define SECURITYENHANCE_NAME "com.lingmo.daemon.SecurityEnhance"

using PowerInter = org::lingmo::ocean::Power1;
...
GreeterWorker::GreeterWorker(SessionBaseModel *const model, QObject *parent)
    ...
    , m_soundPlayerInter(new SoundThemePlayerInter("org.lingmo.ocean.SoundThemePlayer1", "/org/lingmo/ocean/SoundThemePlayer1", QDBusConnection::systemBus(), this))
    ...
    , m_retryAuth(false)
{
    ...
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.session-shell")) {
        QGSettings gsetting("com.lingmo.ocean.session-shell", "/com/lingmo/ocean/session-shell/", this);
...
void GreeterWorker::initConnections()
{
    ...
    connect(m_authFramework, &LingmoAuthFramework::FrameworkStateChanged, m_model, &SessionBaseModel::updateFrameworkState);
    ...
    /* org.lingmo.ocean.Authenticate1.Session */
    connect(m_authFramework, &LingmoAuthFramework::AuthStateChanged, this, &GreeterWorker::onAuthStateChanged);
...
void GreeterWorker::initConfiguration()
{
    ...
    if (QFile::exists("/etc/lingmo/no_suspend")) {
    ...
    if (m_model->currentUser() != nullptr && getNumLockState(m_model->currentUser()->name()) == NUM_LOCK_UNKNOWN) {
        PowerInter powerInter("org.lingmo.ocean.Power1", "/org/lingmo/ocean/Power1", QDBusConnection::systemBus(), this);
...
void GreeterWorker::createAuthentication(const QString &account)
{
    ...
    case Available:
        ...
        m_authFramework->SetAuthQuitFlag(account, LingmoAuthFramework::ManualQuit);
...
void GreeterWorker::checkDBusServer(bool isValid)
{
    if (isValid) {
        ...
        QTimer::singleShot(300, this, [ = ] {
            qWarning() << "org.lingmo.ocean.Accounts1 is not start, rechecking!";
            checkDBusServer(m_accountsInter->isValid());
        });
    }
}
