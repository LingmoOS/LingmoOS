...
int main(int argc, char *argv[])
{
    ...
    app->setApplicationName("org.lingmo.ocean.lock");
    ...
    if (worker->isLocked()) {
        //如果当前Session被锁定，则启动ocean-lock时直接进入锁屏状态
        //兼容在desktop文件中添加-l启动参数来解决杀掉ocean-lock后启动器无法打开的问题,BUG91126
    ...
    if (!conn.registerService(DBUS_LOCK_NAME) ||
        ...
        !app->setSingleInstance(QString("ocean-lock%1").arg(getuid()), DApplication::UserScope)) {
        qDebug() << "register dbus failed"<< "maybe lockFront is running..." << conn.lastError();

        if (!runDaemon) {
            const char *lockFrontInter = "org.lingmo.ocean.LockFront1";
            const char *shutdownFrontInter = "org.lingmo.ocean.ShutdownFront1";
            if (showUserList) {
