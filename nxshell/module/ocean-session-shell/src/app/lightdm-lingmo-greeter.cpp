...
{
    QSettings settings(OCEANSESSIONCC::DEFAULT_CURSOR_THEME, QSettings::IniFormat);
    //The default cursor theme path
    qDebug() << "Theme Path:" << OCEANSESSIONCC::DEFAULT_CURSOR_THEME;
...
int main(int argc, char* argv[])
{
    ...
    setenv("DSG_APP_ID", "org.lingmo.ocean.lightdm-lingmo-greeter", 1);

    DGuiApplicationHelper::setAttribute(DGuiApplicationHelper::UseInactiveColorGroup, false);
    // 设置缩放，文件存在的情况下，由后端去设置，否则前端自行设置
    if (!QFile::exists("/etc/lightdm/lingmo/xsettingsd.conf")) {
    ...
    qApp->setApplicationName("org.lingmo.ocean.lightdm-lingmo-greeter");
    ...
    const QString serviceName = "org.lingmo.ocean.Accounts1";
    ...
    if (!interface->isServiceRegistered(serviceName)) {
        ...
        qDebug() << "waiting for lingmo accounts service";
