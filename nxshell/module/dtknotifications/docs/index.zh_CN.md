...
dtknotifications是对于lingmo/UOS系统上的org.freedesktop.Notifications的dbus接口和com.lingmo.ocean.Notification的dbus接口的封装，同时使用Qt以及Linux原生接口实现了一部分功能，其目的是在于方便第三方开发者轻松且快速的调用接口进行开发。
...
int main(int argc, char **argv)
{
    ...
    QObject::connect(&manager, &DNotificationManager::recordAoceand, []{ qDebug() << "recordAoceand"; });
    ...
    QObject::connect(&manager, &DNotificationManager::appAoceand, []{ qDebug() << "appAoceand"; });
    ...
    for (auto record : records.value()) {
        ...
        if (record.appName == "lingmo-screen-recorder") {
            offsetId = record.id;
