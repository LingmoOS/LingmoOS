...
QWaylandShellIntegration *QKWaylandShellIntegrationPlugin::create(const QString &key, const QStringList &paramList)
{
    ...
    //创建oceanshell
    connect(registry, &Registry::oceanShellAnnounced,
            &DWaylandShellManager::createOCEANShell);

    //创建oceanseat
    connect(registry, &Registry::oceanSeatAnnounced,
            &DWaylandShellManager::createOCEANSeat);

    connect(registry, &Registry::interfacesAnnounced, [] {
        DWaylandShellManager::createOCEANPointer();
        DWaylandShellManager::createOCEANKeyboard();
        DWaylandShellManager::createOCEANFakeInput();
    });
