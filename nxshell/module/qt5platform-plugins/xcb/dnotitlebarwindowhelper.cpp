...
DNoTitlebarWindowHelper::~DNoTitlebarWindowHelper()
{
    ...
    if (m_window->handle()) { // 当本地窗口还存在时，移除设置过的窗口属性
        Utility::clearWindowProperty(m_windowID, Utility::internAtom(_LINGMO_SCISSOR_WINDOW));
...
void DNoTitlebarWindowHelper::updateClipPathFromProperty()
{
    ...
    static xcb_atom_t _lingmo_scissor_window = Utility::internAtom(_LINGMO_SCISSOR_WINDOW, false);
    ...
    if (!path.isEmpty()) {
        ...
        ds << m_clipPath;
        Utility::setWindowProperty(m_windowID, _lingmo_scissor_window, _lingmo_scissor_window, data.constData(), data.length(), 8);
    } else {
        m_clipPath = QPainterPath();
        Utility::clearWindowProperty(m_windowID, _lingmo_scissor_window);
    }
