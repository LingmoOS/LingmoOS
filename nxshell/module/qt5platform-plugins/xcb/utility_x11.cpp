...
void Utility::updateMousePointForWindowMove(quint32 WId, bool finished/* = false*/)
{
    ...
    xev.type = internAtom("_LINGMO_MOVE_UPDATE");
    xev.window = WId;
    xev.format = 32;
...
{
    quint8 value = on;
    setWindowProperty(WId, DXcbWMSupport::instance()->_lingmo_no_titlebar, XCB_ATOM_CARDINAL, &value, 1, 8);

    // 默认为使用noborder属性的窗口强制打开窗口标题栏
    xcb_atom_t _lingmo_force_decorate = internAtom("_LINGMO_FORCE_DECORATE", false);
    if (on) {
        quint8 value = on;
        setWindowProperty(WId, _lingmo_force_decorate, XCB_ATOM_CARDINAL, &value, 1, 8);
    } else {
        clearWindowProperty(WId, _lingmo_force_decorate);
    }
}
...
void Utility::splitWindowOnScreenByType(quint32 WId, quint32 position, quint32 type)
{
    ...
    xev.response_type = XCB_CLIENT_MESSAGE;
    xev.type = internAtom("_LINGMO_SPLIT_WINDOW", false);
    xev.window = WId;
    xev.format = 32;
...
{
    auto propAtom = internAtom("_LINGMO_NET_SUPPORTED");
...
bool Utility::setEnableBlurWindow(const quint32 WId, bool enable)
{
    ...
    clearWindowProperty(WId, DXcbWMSupport::instance()->_net_wm_lingmo_blur_region_mask);
...
bool Utility::blurWindowBackground(const quint32 WId, const QVector<BlurArea> &areas)
{
    ...
    if (DXcbWMSupport::instance()->isLingmoWM()) {
        xcb_atom_t atom = DXcbWMSupport::instance()->_net_wm_lingmo_blur_region_rounded_atom;
...
{
    if (!DXcbWMSupport::instance()->isLingmoWM() || maskImage.format() != QImage::Format_Alpha8)
        return false;
    ...
    clearWindowProperty(WId, DXcbWMSupport::instance()->_net_wm_lingmo_blur_region_rounded_atom);
    setWindowProperty(WId, DXcbWMSupport::instance()->_net_wm_lingmo_blur_region_mask,
                      DXcbWMSupport::instance()->_net_wm_lingmo_blur_region_mask,
...
{
    xcb_atom_t atom = DXcbWMSupport::instance()->_lingmo_wallpaper;
...
{
    clearWindowProperty(WId, DXcbWMSupport::instance()->_lingmo_wallpaper);
}
