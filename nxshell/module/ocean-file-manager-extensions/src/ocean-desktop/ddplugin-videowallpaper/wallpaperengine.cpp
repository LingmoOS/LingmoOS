...
bool WallpaperEngine::init()
{
    ...
    if (!registerMenu()) {
        // waiting canvas menu
        dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &WallpaperEngine::registerMenu);
    }
...
bool WallpaperEngine::registerMenu()
{
    if (dfmplugin_menu_util::menuSceneContains("CanvasMenu")) {
        ...
        dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &WallpaperEngine::registerMenu);
        return true;
    } else
        return false;
}
...
void WallpaperEngine::checkResouce()
{
    if (d->videos.isEmpty()) {
       ...
       QDBusPendingReply<uint> p = notify.asyncCall(QString("Notify"),
                        ...
                        static_cast<uint>(0),
                        QString("lingmo-toggle-desktop"),   // icon
                        text,
