...
// 打印水印插件
static const char *g_WaterMarkEnv = "LINGMO_WATERMARK";
...
{
    bool bRet = false;
    // 优先采用V23接口，低版本OCEAN还未提供区分V23/V20接口
    QDBusInterface dockDbusInterfaceV23(
        "org.lingmo.ocean.daemon.Dock1", "/org/lingmo/ocean/daemon/Dock1", "org.lingmo.ocean.daemon.Dock1");
    if (dockDbusInterfaceV23.isValid()) {
        QDBusReply<void> reply = dockDbusInterfaceV23.call("ActivateWindow", winId);
        if (!reply.isValid()) {
            qWarning() << qPrintable("Call v23 org.lingmo.ocean.daemon.Dock1 failed") << reply.error();
        } else {
            return true;
        }
    }
    ...
    QDBusInterface dockDbusInterfaceV20(
        "com.lingmo.ocean.daemon.Dock", "/com/lingmo/ocean/daemon/Dock", "com.lingmo.ocean.daemon.Dock");
    if (dockDbusInterfaceV20.isValid() && !bRet) {
        QDBusReply<void> reply = dockDbusInterfaceV20.call("ActivateWindow", winId);
        if (!reply.isValid()) {
            qWarning() << qPrintable("Call v20 com.lingmo.ocean.daemon.Dock failed") << reply.error();
            bRet = false;
        } else {
            return true;
        }
...
/**
   ...
   @note 命令行参数示例：
    lingmo-image-viewer --config=[Base64 code data] /path/to/file
 */
...
void PermissionConfig::detectWaterMarkPluginExists()
{
    ...
    QString pluginPath = QLatin1String("/usr/share/lingmo/dtk/plugins/printsupport");
#else
