...
LoginSafetySrvModel::LoginSafetySrvModel(QObject *parent)
    : QObject(parent)
    , m_gSettings(new QGSettings("com.lingmo.ocean.lingmo-defender", QByteArray(), this))
    , m_pwdConfInter(new QDBusInterface("com.lingmo.daemon.PasswdConf",
                                        "/com/lingmo/daemon/PasswdConf",
                                        "com.lingmo.daemon.PasswdConf",
                                        QDBusConnection::systemBus(), this))
    , m_accountInter(new AccountInter("com.lingmo.daemon.Accounts",
...
qint64 LoginSafetySrvModel::daysToPasswdLastChanged()
{
    ...
    // 创建接口
    QDBusInterface *accountUserInter = new QDBusInterface("com.lingmo.daemon.Accounts",
                                                          userDbusPath, "com.lingmo.daemon.Accounts.User",
                                                          QDBusConnection::systemBus(), this);
    // 获取上次密码修改日期距原始日期的天数
...
void LoginSafetySrvModel::showNeedChangePwdSysNotify(qint64 days)
{
    ...
    QString openSysSafetyPageCmd = QString("qdbus,com.lingmo.ocean.ControlCenter,"
                                           "/com/lingmo/ocean/ControlCenter,"
                                           "com.lingmo.ocean.ControlCenter.ShowPage,"
                                           "%1,")
                                       .arg("accounts");
