...
#define X_LINGMO_VENDOR_STR "lingmo"
#define ONLY_SHOW_IN_VALUE_LINGMO "Lingmo"
...
void GetProcInfoJob::updateLauncherItemInfoList()
{
    ...
    for (const QFileInfo &info : desktopFileList) {
        ...
        // "OnlyShowIn"属性为空或Lingmo，则显示
        QString onlyShowIn = readIniSettingMethod.value("OnlyShowIn").toString();
        if (!onlyShowIn.isEmpty() && ONLY_SHOW_IN_VALUE_LINGMO != onlyShowIn) {
            continue;
        }
        // 桌面文件路径
        procInfo.sDesktopPath = desktopPath;
        // 应用名称
        QString xLingmoVendor = readIniSettingMethod.value("X-Lingmo-Vendor").toString();
        if (X_LINGMO_VENDOR_STR == xLingmoVendor) {
