...
#define DEFENDER_DATA_DIR_PATH "/usr/share/lingmo-defender/" // 数据库路径
#define LOCAL_CACHE_DB_NAME "localcache.db" // 数据库路径
#define TABLE_NAME_SETTINGS "Settings"
#define GSETTING_SCHEMA_XML_PATH "/usr/share/glib-2.0/schemas/com.lingmo.ocean.lingmo-defender.gschema.xml"
...
QVariant SettingsSql::getValue(const QString &key)
{
    ...
    if (doc.object().contains("value")) {
        retValue = doc.object().value("value");
    }
    return retValue;
}
...
void SettingsSql::setDefaultKeyValues()
{
    ...
    while (!xmlReader.atEnd()) {
        if (xmlReader.isStartElement() && "schema" == xmlReader.name()) {
            ...
            if ("/com/lingmo/ocean/lingmo-defender/" == schemaPath
                && LINGMO_DEFENDER_GSETTING_PATH == schemaId) {
