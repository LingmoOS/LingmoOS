    ...
    , m_isWorking(false)
{
    ...
    mainLayout->setContentsMargins(0, 5, 0, 5); // 上下需要留白，模拟窗口内空隙
    ...
    // 根据UI整改要求,字体设置为 T7 MEDIUM #001a2e
    DFontSizeManager::instance()->bind(m_headLabel, DFontSizeManager::T7);
    QPalette headPal;
    ...
    // 根据UI整改要求,字体设置为 T8 normal #526a7f
    DFontSizeManager::instance()->bind(m_headTipLabel, DFontSizeManager::T8);
    QPalette tipPal;
...
void CleanerResultItemWidget::setAsRoot()
{
    ...
    DFontSizeManager::instance()->bind(m_headLabel, DFontSizeManager::T6);
