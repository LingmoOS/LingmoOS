...
#define Startup_Disable ":/icons/lingmo/builtin/icons/oceanui_startup_disable_29px.svg" // 状态资源图片 - 不允许
#define Startup_Enable ":/icons/lingmo/builtin/icons/oceanui_startup_enable_29px.svg" // 状态资源图片 - 允许
...
StartupWidget::StartupWidget(InvokerFactoryInterface *invokeFactory, QWidget *parent)
    : DFrame(parent)
...
{
    if (invokeFactory) {
        m_dataInterfaceInvoker = invokeFactory->CreateInvoker("com.lingmo.defender.datainterface", "/com/lingmo/defender/datainterface", "com.lingmo.defender.datainterface", ConnectType::SESSION, this);
        m_monitorInvoker = invokeFactory->CreateInvoker("com.lingmo.defender.MonitorNetFlow", "/com/lingmo/defender/MonitorNetFlow", "com.lingmo.defender.MonitorNetFlow", ConnectType::SYSTEM, this);
    }
    ...
    // 后台自启动数据刷新信号连接
    m_dataInterfaceInvoker->Connect("AccessRefreshData", this, SLOT(refreshData(bool, QString)));
}
...
void StartupWidget::getAppsData()
{
    ...
    // 取所有应用数据信息
    bool sus;
    QVariantList susArgs = m_dataInterfaceInvoker->Invoke("startLauncherManage").arguments();
    if (susArgs.isEmpty()) {
        return;
    }
    sus = susArgs.takeFirst().toBool();
    if (sus) {
        QVariantList infoArgs = m_dataInterfaceInvoker->Invoke("getAppsInfoEnable").arguments();
        if (infoArgs.isEmpty()) {
            return;
        }
        QString sEnableInfo = infoArgs.takeFirst().toString();
        infoArgs = m_dataInterfaceInvoker->Invoke("getAppsInfoDisable").arguments();
        if (infoArgs.isEmpty()) {
            return;
        }
        QString sDisableInfo = infoArgs.takeFirst().toString();
        ...
        auto formatData = [&](QString info, QMap<QString, QStringList> &map) {
            ...
            for (int i = 0; i < sList.count(); ++i) {
                ...
                if (sDetailInfoList.count() >= 5) {
                    QList<QString> sList;
                    sList.append(sDetailInfoList.at(0));
                    sList.append(sDetailInfoList.at(1));
                    sList.append(sDetailInfoList.at(2));
                    sList.append(sDetailInfoList.at(3));
                    map[sDetailInfoList.at(4)] = sList;
                }
            }
        };
...
void StartupWidget::loadAppData(int nRow, QString sPath, QString sName, QString sIcon, QString sId, QString sRealName)
{
    // 判断该应用状态（是否是自启动）
    bool bStatus;
    QVariantList statusArgs = m_dataInterfaceInvoker->Invoke("isAutostart", QList<QVariant>() << sPath).arguments();
    if (statusArgs.isEmpty()) {
        bStatus = false;
    } else {
        bStatus = statusArgs.takeFirst().toBool();
    }
    // 设置每一行数据
...
void StartupWidget::onTableBtnClicked()
{
    ...
    // 执行自启动开启/关闭操作
    bool isExeAutoStart;
    QVariantList args = m_dataInterfaceInvoker->Invoke("exeAutostart", QList<QVariant>() << nStatus << sPath).arguments();
    if (args.isEmpty()) {
        return;
    }
    isExeAutoStart = args.takeFirst().toBool();
    if (isExeAutoStart) {
        ...
        params << SECURITY_LOG_TYPE_TOOL;
        nStatus ? params << tr("Enabled auto startup for %1").arg(m_appNamefromId[sID]) : params << tr("Disabled auto startup for %1").arg(m_appNamefromId[sID]);
        m_monitorInvoker->Invoke("AddSecurityLog", params);
        showTotalInfo();
    }
}
