...
void PolicyKitHelper::confirmChangeIsbSaveRecord()
{
    ...
    // 权限验证
    isbChange = checkAuthorization("com.lingmo.lingmo-defender.checkAuthentication", getpid());
    if (isbChange) {
...
void PolicyKitHelper::confirmClearUsbConnectionLog()
{
    bool clear = false;
    // 权限验证
    clear = checkAuthorization("com.lingmo.lingmo-defender.checkAuthentication", getpid());
    if (clear) {
