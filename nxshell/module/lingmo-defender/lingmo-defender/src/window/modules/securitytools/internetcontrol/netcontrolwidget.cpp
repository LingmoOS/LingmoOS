    ...
    , m_isTipDialogFlag(false)
{
    ...
    // 初始化安全中心gsetting配置
    m_DataInterFaceServer = invokerFactory->CreateInvoker("com.lingmo.defender.datainterface", "/com/lingmo/defender/datainterface", "com.lingmo.defender.datainterface", ConnectType::SESSION, this);
    m_monitorInterFaceServer = invokerFactory->CreateInvoker("com.lingmo.defender.MonitorNetFlow", "/com/lingmo/defender/MonitorNetFlow", "com.lingmo.defender.MonitorNetFlow", ConnectType::SYSTEM, this);
    m_gsetting = invokerFactory->CreateSettings("com.lingmo.ocean.lingmo-defender", QByteArray(), this);
    // 初始化
    initUi();
    initData();
