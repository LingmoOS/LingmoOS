    ...
    , m_developModeChecking(false)
{
    ...
    if (factory) {
        //使用GSetting读取参数
        m_gSettingsInvokerInter = factory->CreateSettings(LINGMO_DEFENDER_GSETTING_PATH, "", this);
        m_defenderDataInvokerInter = factory->CreateInvoker("com.lingmo.defender.datainterface",
                                                            "/com/lingmo/defender/datainterface",
                                                            "com.lingmo.defender.datainterface",
                                                            ConnectType::SESSION, this);
        // 控制中心dbus服务对象
        m_controlCenterInvokerInter = factory->CreateInvoker("com.lingmo.ocean.ControlCenter",
                                                             "/com/lingmo/ocean/ControlCenter",
                                                             "com.lingmo.ocean.ControlCenter",
                                                             ConnectType::SESSION, this);
        // 安全中心dbus服务对象
        m_defenderWindowInvokerInter = factory->CreateInvoker("com.lingmo.defender.hmiscreen",
                                                              "/com/lingmo/defender/hmiscreen",
                                                              "com.lingmo.defender.hmiscreen",
                                                              ConnectType::SESSION, this);
        // 同步助手dbus服务，用于查询是否处于开发者模式
        m_helperInvokerInter = factory->CreateInvoker("com.lingmo.sync.Helper",
                                                      "/com/lingmo/sync/Helper",
                                                      "com.lingmo.sync.Helper",
                                                      ConnectType::SYSTEM, this);
        // 磁盘管理dbus服务
        m_diskManagerInvokerInter = factory->CreateInvoker("com.lingmo.diskmanager",
                                                           "/com/lingmo/diskmanager",
                                                           "com.lingmo.diskmanager",
                                                           ConnectType::SYSTEM, this);
        // 流量监控系统dbus服务
        m_netFlowMonitorInvokerInter = factory->CreateInvoker("com.lingmo.defender.MonitorNetFlow",
                                                              "/com/lingmo/defender/MonitorNetFlow",
                                                              "com.lingmo.defender.MonitorNetFlow",
                                                              ConnectType::SYSTEM, this);
        // 系统语言选择器dbus服务
        m_langSelectInvokerInter = factory->CreateInvoker("com.lingmo.daemon.LangSelector",
                                                          "/com/lingmo/daemon/LangSelector",
                                                          "com.lingmo.daemon.LangSelector",
                                                          ConnectType::SESSION, this);
    }
...
bool HomePageModel::isDiskManagerInterValid()
{
    QProcess proc;
    // 查询是否已经安装 磁盘管理器
    proc.start("dpkg -s lingmo-diskmanager");
    proc.waitForFinished();
    QString status = proc.readAllStandardOutput();

    // 若返回结果以 "Package: lingmo-diskmanager\nStatus: install"，则说明已安装
    return status.startsWith("Package: lingmo-diskmanager\nStatus: install");
}
