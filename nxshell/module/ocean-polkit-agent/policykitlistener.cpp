    ...
    , m_showInfoSuccess(false)
{
    ...
    if (!sessionBus.registerService("org.lingmo.ocean.Polkit1.AuthAgent")) {
        qWarning() << "Register auth agent service failed!";
    }
    if (!sessionBus.registerObject("/com/lingmo/ocean/Polkit1/AuthAgent", this,
...
static bool isAccountLocked(const PolkitQt1::Identity &identity)
{
    ...
    QDBusMessage msg = QDBusMessage::createMethodCall("org.lingmo.ocean.Authenticate1",
