...
#include <libintl.h>
#include <ocean-shell/dlayershellwindow.h>
...
void AuthDialog::setError(const QString &error, bool alertImmediately)
{
    ...
    if ("Finger moved too fast, please do not lift until prompted" == error) {
        ...
        dgetText = QString(dgettext("lingmo-authentication", error.toUtf8()));
    }
    m_errorMsg = dgetText;
    if (alertImmediately)
...
void AuthDialog::setAuthInfo(const QString &info)
{
    ...
    m_passwordInput->lineEdit()->setPlaceholderText(QString(dgettext("lingmo-authentication", info.toStdString().c_str())));
...
bool AuthDialog::hasSecurityHighLever(QString userName)
{
    ...
    QDBusInterface securityEnhance("com.lingmo.daemon.SecurityEnhance",
                                   "/com/lingmo/daemon/SecurityEnhance",
                                   "com.lingmo.daemon.SecurityEnhance",
...
{
    // Clears the combobox in the case some user be aoceand
    m_adminsCombo->clear();
...
{
    QDBusInterface accounts("org.lingmo.ocean.Accounts1", "/org/lingmo/ocean/Accounts1", "org.lingmo.ocean.Accounts1", QDBusConnection::systemBus());
    ...
    if (reply.isValid()) {
        ...
        if (!path.isEmpty()) {
            QDBusInterface accounts_user("org.lingmo.ocean.Accounts1", path, "org.lingmo.ocean.Accounts1.User", QDBusConnection::systemBus());
...
{
    const QString path = "/usr/share/ocean-session-shell/ocean-session-shell.conf";
    int count = 5;
    QFile file(path);
    if (!file.exists()) {
        return count;
