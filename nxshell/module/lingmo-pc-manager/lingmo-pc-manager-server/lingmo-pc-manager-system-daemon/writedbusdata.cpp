...
#include "../../lingmo-pc-manager/src/window/modules/common/database/trashcleanuninstallappsql.h"
#include "disk/disk.h"
    ...
    , m_isInitSqlDatabase(false)
{
    ...
    // 扫描冗余文件并删除
    scanFile("/usr/share/lingmo-pc-manager/config/");
}
...
// 递归函数-获取/usr/share/lingmo-pc-manager/config/目录下所有的lock文件
...
bool WriteDBusData::openLocalDb()
{
    ...
    m_localCacheDb.setDatabaseName("/usr/share/lingmo-pc-manager/localcache.db");
    m_localCacheDb.setUserName("root");
    m_localCacheDb.setPassword("lingmo-pc-manager");
