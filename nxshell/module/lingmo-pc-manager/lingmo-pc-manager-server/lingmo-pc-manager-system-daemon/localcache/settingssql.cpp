// Copyright (C) 2019 ~ 2021 Lingmo Technology Co., Ltd.
...
#define DEFENDER_DATA_DIR_PATH "/usr/share/lingmo-pc-manager/" // 数据库路径
#define LOCAL_CACHE_DB_NAME "localcache.db"                    // 数据库路径
#define TABLE_NAME_SETTINGS "Settings"
#define GSETTING_SCHEMA_XML_PATH \
    "/usr/share/glib-2.0/schemas/com.lingmo.ocean.lingmo-pc-manager.gschema.xml"
...
void SettingsSql::setDefaultKeyValues()
{
    ...
    while (!xmlReader.atEnd()) {
        if (xmlReader.isStartElement() && "schema" == xmlReader.name()) {
            ...
            if ("/com/lingmo/ocean/lingmo-pc-manager/" == schemaPath
                && LINGMO_PC_MANAGER_GSETTING_PATH == schemaId) {
