...
bool CmdTool::parseOemTomlInfo(const QString filename)
{
    ...
    qCInfo(appLog) << "Toml File name  is: /etc/lingmo/hardware/" << filename;

    QString curPathFile =  "/etc/lingmo/hardware/" + filename;
    ...
    if (!tomlFileRead) {
        curPathFile = "/etc/lingmo/hardware/oeminfodebug.toml"; //方便调试，并不开放给用户用
        QFile file(curPathFile);
        if (file.open(QIODevice::ReadOnly)) {
            info = file.readAll().data();
            tomlFileRead = true;
