...
void PageSingleInfo::slotShowMenu(const QPoint &)
{
    ...
    //ocean-printer未安装，updateDriver不可选
    DevicePrint *printer = qobject_cast<DevicePrint *>(mp_Device);
    if (printer && !PageInfo::packageHasInstalled("ocean-printer")) {
        mp_updateDriver->setEnabled(false);
    }
...
{
    //打印设备更新驱动时，通过ocean-printer来操作
    if (mp_Device->hardwareClass() == "printer") {
        if (!QProcess::startDetached("ocean-printer"))
            qCInfo(appLog) << "ocean-printer startDetached error";
        return;
    }
