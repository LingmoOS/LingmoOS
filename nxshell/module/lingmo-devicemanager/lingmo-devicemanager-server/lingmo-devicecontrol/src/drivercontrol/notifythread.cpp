...
void NotifyThread::run()
{
    ...
    forever {
        ...
        //通知前端 example: runuser -l Username -c "XDG_RUNTIME_DIR=\"/run/user/\$(id -u $Username)\" /usr/bin/lingmo-devicemanager notify zh_CN"
        QString strCmd=QString("XDG_RUNTIME_DIR=\"/run/user/\$(id -u %1)\" /usr/bin/lingmo-devicemanager notify %2").arg(strUsername).arg(name);
