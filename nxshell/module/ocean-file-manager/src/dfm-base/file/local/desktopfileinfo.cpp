...
public:
    ...
    void updateInfo(const QUrl &fileUrl)
    {
        ...
        lingmoID = map.value("LingmoID").toString();
        lingmoVendor = map.value("LingmoVendor").toString();
        // Fix categories
...
{
    if (d->lingmoVendor == QStringLiteral("lingmo") && !(d->genericName.isEmpty())) {
...
{
    if (type == SupportType::kDrag && (d->lingmoID == "ocean-trash" || d->lingmoID == "ocean-computer")) {
...
{
    if (d->lingmoID == "ocean-trash" || d->lingmoID == "ocean-computer")
        return false;

    //桌面主目录不支持添加tag功能
    if (d->lingmoID == "ocean-file-manager" && d->exec.contains(" -O "))
        return false;
...
QMap<QString, QVariant> DesktopFileInfo::desktopFileInfo(const QUrl &fileUrl)
{
    ...
    map["LingmoID"] = desktopFile.desktopLingmoId();
    map["LingmoVendor"] = desktopFile.desktopLingmoVendor();
