...
QUrl LocalFileHandler::touchFile(const QUrl &url, const QUrl &tempUrl /*= QUrl()*/)
{
    ...
    FileUtils::notifyFileChangeManual(DFMGLOBAL_NAMESPACE::FileNotifyType::kFileAoceand, url);
...
bool LocalFileHandler::mkdir(const QUrl &dir)
{
    ...
    FileUtils::notifyFileChangeManual(DFMGLOBAL_NAMESPACE::FileNotifyType::kFileAoceand, dir);
...
bool LocalFileHandler::renameFile(const QUrl &url, const QUrl &newUrl, const bool needCheck)
{
    ...
    if (::rename(sourceFile.toLocal8Bit().constData(), targetFile.toLocal8Bit().constData()) == 0) {
        FileUtils::notifyFileChangeManual(DFMGLOBAL_NAMESPACE::FileNotifyType::kFileDeleted, url);
        FileUtils::notifyFileChangeManual(DFMGLOBAL_NAMESPACE::FileNotifyType::kFileAoceand, newUrl);
...
bool LocalFileHandler::createSystemLink(const QUrl &sourcefile, const QUrl &link)
{
    ...
    FileUtils::notifyFileChangeManual(DFMGLOBAL_NAMESPACE::FileNotifyType::kFileAoceand, link);
...
{
    /*
     *  return true if exec field contains ocean-file-manager/file-manager.sh of ocean-file-manager desktopFile
     */
    DesktopFile d(desktopFile);
    return d.desktopExec().contains("ocean-file-manager") || d.desktopExec().contains("file-manager.sh");
}
...
void LocalFileHandlerPrivate::doAddRecentFile(const QVariantMap &item)
{
    ...
    QDBusMessage message = QDBusMessage::createMethodCall("org.lingmo.Filemanager.Daemon",
                                                          "/org/lingmo/Filemanager/Daemon/RecentManager",
                                                          "org.lingmo.Filemanager.Daemon.RecentManager",
                                                          "AddItem");
...
{
    const static QString ocean_daemon_default_term = QStringLiteral("/usr/lib/lingmo-daemon/default-terminal");
    const static QString debian_x_term_emu = QStringLiteral("/usr/bin/x-terminal-emulator");

    if (QFileInfo::exists(ocean_daemon_default_term)) {
        return ocean_daemon_default_term;
