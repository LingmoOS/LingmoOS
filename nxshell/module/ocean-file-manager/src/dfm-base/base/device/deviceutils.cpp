...
QString DeviceUtils::convertSuitableDisplayName(const QVariantMap &devInfo)
{
    ...
    if (mpt == "/" || idLabel.startsWith("_ocean_")) {
...
QString DeviceUtils::nameOfSystemDisk(const QVariantMap &datas)
{
    ...
    if (!canPowerOff && !mountPoint.isEmpty()) {
        if (label.startsWith("_ocean_data"))
            return QObject::tr("Data Disk");
        if (label.startsWith("_ocean_"))
...
bool DeviceUtils::checkDiskEncrypted()
{
    ...
    std::call_once(flag, [&] {
        QSettings settings("/etc/lingmo/lingmo-user-experience", QSettings::IniFormat);
...
QStringList DeviceUtils::encryptedDisks()
{
    ...
    std::call_once(flag, [&] {
        // The `lingmo-installer.conf` file is either in the `/etc` directory
        // or in the `/etc/lingmo-installer` directory
        DFMIO::DFile file("/etc/lingmo-installer.conf");
        QString conf = file.exists() ? "/etc/lingmo-installer.conf" : "/etc/lingmo-installer/lingmo-installer.conf";
