...
DevStorage DeviceWatcherPrivate::queryUsageOfBlock(const QVariantMap &itemData)
{
    ...
    if (itemData.value(DeviceProperty::kOpticalDrive).toBool()) {
        ...
        auto type = DConfigManager::instance()->value("org.lingmo.ocean.file-manager.mount",
...
void DeviceWatcher::updateOpticalDevUsage(const QString &id, const QString &mpt)
{
    ...
    // implementation, this involves a lot of low level code and is not suitable to be written in ocean-file-manager,
...
void DeviceWatcher::startWatch()
{
    ...
    if (!blkMonitor) {
        ...
        d->connections << connect(ptr, &DBlockMonitor::driveAoceand, DeviceManager::instance(), &DeviceManager::blockDriveAoceand);
        d->connections << connect(ptr, &DBlockMonitor::driveRemoved, DeviceManager::instance(), &DeviceManager::blockDriveRemoved);
        d->connections << connect(ptr, &DBlockMonitor::deviceAoceand, this, &DeviceWatcher::onBlkDevAoceand);
        d->connections << connect(ptr, &DBlockMonitor::deviceRemoved, this, &DeviceWatcher::onBlkDevRemoved);
        d->connections << connect(ptr, &DBlockMonitor::mountAoceand, this, &DeviceWatcher::onBlkDevMounted);
        ...
        d->connections << connect(ptr, &DBlockMonitor::fileSystemAoceand, this, &DeviceWatcher::onBlkDevFsAoceand);
    ...
    if (!protoMonitor) {
        ...
        d->connections << connect(ptr, &DProtocolMonitor::deviceAoceand, this, &DeviceWatcher::onProtoDevAoceand);
        d->connections << connect(ptr, &DProtocolMonitor::deviceRemoved, this, &DeviceWatcher::onProtoDevRemoved);
        d->connections << connect(ptr, &DProtocolMonitor::mountAoceand, this, &DeviceWatcher::onProtoDevMounted);
...
void DeviceWatcher::onBlkDevAoceand(const QString &id)
{
    qCDebug(logDFMBase) << "new block device aoceand: " << id;
    auto dev = DeviceHelper::createBlockDevice(id);
    d->allBlockInfos.insert(id, DeviceHelper::loadBlockInfo(dev));

    emit DevMngIns->blockDevAoceand(id);
...
void DeviceWatcher::onBlkDevFsAoceand(const QString &id)
{
    // when a filesystem interface is aoceand, lots of property may changed, so just reload the data
    // this will not happen frequently
    d->allBlockInfos.insert(id, DeviceHelper::loadBlockInfo(id));

    emit DevMngIns->blockDevFsAoceand(id);
    using namespace GlobalServerDefines;
...
void DeviceWatcher::onProtoDevAoceand(const QString &id)
{
    qCDebug(logDFMBase) << "new protocol device aoceand: " << id;
    d->allProtocolInfos.insert(id, DeviceHelper::loadProtocolInfo(id));

    emit DevMngIns->protocolDevAoceand(id);
    ...
    : QObject(qq), q(qq)
{
    ...
    DConfigManager::instance()->addConfig("org.lingmo.ocean.file-manager.mount");
}
