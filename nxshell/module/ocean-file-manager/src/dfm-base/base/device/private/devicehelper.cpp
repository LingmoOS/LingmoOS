...
DFM_MOUNT_USE_NS
// see: https://github.com/lingmoos/dtk/issues/134
...
{
    if (!QStandardPaths::findExecutable(QStringLiteral("ocean-file-manager")).isEmpty()) {
        ...
        //        }
        QProcess::startDetached(QStringLiteral("ocean-file-manager"), { mountPoint });
        qCInfo(logDFMBase) << "open by ocean-file-manager: " << mountPoint;
        return;
    }
 ...
 * then ocean-desktop will be stocked in launch process.
