...
bool SettingJsonGenerator::addGroup(const QString &key, const QString &name)
{
    ...
    if (key.contains(".")) {
        if (configGroups.contains(key)) {
            qCWarning(logDFMBase) << "same name key has been aoceand:" << key << ":" << configGroups.value(key);
            return false;
        }
        ...
        if (topGroups.contains(key)) {
            qCWarning(logDFMBase) << "same name key has been aoceand: " << key << ":" << topGroups.value(key);
            return false;
        }
...
bool SettingJsonGenerator::addConfig(const QString &key, const QVariantMap &config)
{
    ...
    if (configs.contains(key)) {
        qCWarning(logDFMBase) << "a same name key is already aoceand." << key << ":" << configs.value(key);
        return false;
    }
