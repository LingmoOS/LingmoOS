...
QMap<QString, QStringList> MimesAppsManager::OCEAN_MimeTypes = {};
...
QString MimesAppsManager::getDefaultAppDisplayNameByGio(const QString &mimeType)
{
    /*
        ...
        * First check mimeapps.list/[Default Associations], then mimeapps.list/[Aoceand Associations]. The entry corresponding to the mimetype in
...
QString MimesAppsManager::getOCEANMimeTypeFile()
{
    return QString("%1/%2/%3").arg(getMimeInfoCacheFileRootPath(), "lingmo", "ocean-mimetype.list");
}
...
void MimesAppsManager::initMimeTypeApps()
{
    ...
    for (const QString &desktopFolder : getApplicationsFolders()) {
        ...
        while (it.hasNext()) {
            ...
            if (OCEAN_MimeTypes.contains(fileName)) {
                mimeTypes.append(OCEAN_MimeTypes.value(fileName));
            }
...
void MimesAppsManager::loadOCEANMimeTypes()
{
    QSettings settings(getOCEANMimeTypeFile(), QSettings::IniFormat);
    qCDebug(logDFMBase) << settings.childGroups();

    QFile file(getOCEANMimeTypeFile());
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        return;
    }
    ...
    while (!in.atEnd()) {
        ...
        if (!desktopKey.isEmpty() && first_equal >= 0) {
            ...
            OCEAN_MimeTypes.insert(desktopKey, mimetypes);
            desktopKey.clear();
        }
    }
    file.close();
