...
#    define APPEARANCE_SERVICE "org.lingmo.ocean.Appearance1"
#    define APPEARANCE_PATH "/org/lingmo/ocean/Appearance1"
#else
#    define APPEARANCE_SERVICE "com.lingmo.daemon.Appearance"
#    define APPEARANCE_PATH "/com/lingmo/daemon/Appearance"
#endif
using namespace GlobalDConfDefines::ConfigPath;

namespace dfmbase {

static constexpr char kOCEANTrashId[] { "ocean-trash" };
static constexpr char kOCEANComputerId[] { "ocean-computer" };
static constexpr char kOCEANHomeId[] { "ocean-home" };
...
bool FileUtils::isTrashDesktopFile(const QUrl &url)
{
    if (isDesktopFileSuffix(url)) {
        ...
        return df.desktopLingmoId() == kOCEANTrashId;
    }
    return false;
}
...
bool FileUtils::isComputerDesktopFile(const QUrl &url)
{
    if (isDesktopFileSuffix(url)) {
        DesktopFile df(url.toLocalFile());
        return df.desktopLingmoId() == kOCEANComputerId;
    }
    return false;
}
...
bool FileUtils::isHomeDesktopFile(const QUrl &url)
{
    if (isDesktopFileSuffix(url)) {
        DesktopFile df(url.toLocalFile());
        return df.desktopLingmoId() == kOCEANHomeId;
    }
    return false;
}
...
void FileUtils::notifyFileChangeManual(DFMGLOBAL_NAMESPACE::FileNotifyType type, const QUrl &url)
{
    ...
    switch (type) {
    case DFMGLOBAL_NAMESPACE::FileNotifyType::kFileAoceand:
        watcher->notifyFileAoceand(url);
        return;
...
{
    if (QFileInfo::exists("/var/lib/lingmo/permission-manager/wallpaper_locked")) {
        QDBusInterface notify("org.freedesktop.Notifications", "/org/freedesktop/Notifications", "org.freedesktop.Notifications");
        notify.asyncCall(QString("Notify"),
                         QString("ocean-file-manager"),   // title
                         static_cast<uint>(0),
                         QString("ocean-file-manager"),   // icon
...
{
    static QUrl trash = QUrl::fromLocalFile(StandardPaths::location(StandardPaths::kDesktopPath) + "/ocean-trash.desktop");
    return trash;
}

QUrl DesktopAppUrl::computerDesktopFileUrl()
{
    static QUrl computer = QUrl::fromLocalFile(StandardPaths::location(StandardPaths::kDesktopPath) + "/ocean-computer.desktop");
    return computer;
}

QUrl DesktopAppUrl::homeDesktopFileUrl()
{
    static QUrl home = QUrl::fromLocalFile(StandardPaths::location(StandardPaths::kDesktopPath) + "/ocean-home.desktop");
    return home;
}
