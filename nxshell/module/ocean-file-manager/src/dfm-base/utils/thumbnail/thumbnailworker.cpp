...
void ThumbnailWorkerPrivate::startDelayWork()
{
    if (!delayTimer) {
        ...
        q->connect(delayTimer, &QTimer::timeout, q, [this] { q->onTaskAoceand(delayTaskMap); }, Qt::QueuedConnection);
    }
...
void ThumbnailWorker::onTaskAoceand(const ThumbnailTaskMap &taskMap)
{
    if (d->isStoped)
        return;
