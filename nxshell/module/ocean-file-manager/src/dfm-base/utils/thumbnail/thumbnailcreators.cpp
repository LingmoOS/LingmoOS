...
QImage ThumbnailCreators::djvuThumbnailCreator(const QString &filePath, ThumbnailSize size)
{
    ...
    const QString &readerBinary = QStandardPaths::findExecutable("lingmo-reader");
    if (readerBinary.isEmpty())
        return img;
    //! 使用子进程来调用lingmo-reader程序生成djvu格式文件缩略图
    QProcess process;
    ...
    if (!process.waitForFinished() || process.exitCode() != 0) {
        qCWarning(logDFMBase) << "thumbnail: lingmo-reader execute failed:"
                              << process.errorString()
                              << filePath;
