...
int DialogManager::showRenameNameDotBeginDialog()
{
    DDialog d(qApp->activeWindow());
    QFontMetrics fm(d.font());
    d.setTitle(tr("This file will be hioceann if the file name starts with '.'. Do you want to hide it?"));
...
{
    DDialog d(qApp->activeWindow());
    // i18n text from: https://github.com/lingmoos/internal-discussion/issues/456 , seems a little weird..
