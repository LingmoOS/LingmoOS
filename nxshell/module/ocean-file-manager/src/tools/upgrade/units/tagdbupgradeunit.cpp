...
static constexpr char kTagOldDbName2[] = ".__lingmo.db";
...
TagDbUpgradeUnit::~TagDbUpgradeUnit()
{
    ...
    if (lingmoDbHandle) {
        delete lingmoDbHandle;
        lingmoDbHandle = nullptr;
    }
    if (newTagDbhandle) {
...
{
    // read old table
    const auto &filePropertyBean = lingmoDbHandle->query<OldFileProperty>().toBeans();
...
bool TagDbUpgradeUnit::checkOldDatabase()
{
    ...
    // check ".__lingmo.db" database table
    lingmoDbHandle = new SqliteHandle(dbPath2);
    if (!chechTable(lingmoDbHandle, kTagOldDb2TableFileProperty))
        return false;
...
bool TagDbUpgradeUnit::checkNewDatabase()
{
    ...
    const auto &dbPath = DFMUtils::buildFilePath(StandardPaths::location(StandardPaths::kApplicationConfigPath).toLocal8Bit(),
                                                 "/lingmo/ocean-file-manager/database",
                                                 nullptr);
