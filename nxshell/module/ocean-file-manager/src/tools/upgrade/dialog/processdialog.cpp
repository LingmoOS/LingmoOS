...
void ProcessDialog::initialize(bool desktop)
{
    ...
    setIcon(QIcon::fromTheme("ocean-file-manager"));
}

bool ProcessDialog::execDialog()
{
    const QString exe = onDesktop ? QString("/usr/libexec/ocean-file-manager") : QString("/usr/bin/ocean-shell");
...
{
    if (killed && !onDesktop) {
        qCInfo(logToolUpgrade) << "restart ocean-shell service...";
        QProcess::startDetached("systemctl", { "--user", "restart", "ocean-shell@OCEAN.service" });
    }
}
...
bool ProcessDialog::isEqual(const QString &link, QString match) const
{
    ...
    // the exePath will contain suffix (deleted) like /usr/bin/ocean-desktop (deleted) after upgrading.
