...
{
    static const QStringList processList { "/usr/bin/dmcg", "/usr/bin/ocean-file-manager"};
    return processList;
}

const QString Utils::devConfigPath()
{
    static const QString path { "/etc/lingmo/devAccessConfig.json" };
    return path;
}

const QString Utils::valultConfigPath()
{
    static const QString path { "/etc/lingmo/vaultAccessConfig.json" };
    return path;
}
