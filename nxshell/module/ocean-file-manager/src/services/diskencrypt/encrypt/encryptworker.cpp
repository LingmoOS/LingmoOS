...
int PrencryptWorker::writeEncryptParams(const QString &device)
{
    ...
    obj.insert("device-name", params.value(encrypt_param_keys::kKeyDeviceName).toString());   // the device name display in ocean-file-manager
...
{
    QFile cfg("/etc/default/grub.d/13_lingmo_ab_recovery.cfg");
