...
static constexpr char kMountControlObjPath[] { "/org/lingmo/Filemanager/MountControl" };
    ...
    : QObject(parent), QDBusContext(), d(new MountControlDBusPrivate(this))
{
    ...
    QFile switcher("/etc/lingmo/disable_dfm_daemon_mount");
...
{
    // handle method call com.lingmo.filemanager.daemon.MountControl.SupportedFileSystems
    ...
    : q(qq), adapter(new MountControlAdaptor(qq))
{
    ...
    auto config = Dtk::Core::DConfig::create("org.lingmo.ocean.file-manager",
                                             "org.lingmo.ocean.file-manager.mount");
