...
static bool validPrimaryChanged(const ScreenProxyQt *proxy)
{
    ...
    if (Q_LIKELY(qApp->screens().count() == 1)) {
        ...
        if (Q_UNLIKELY(QString(":0.0") == qApp->primaryScreen()->name())) {
            ...
            //! Therefore, the primary screen changed signal is handled through the ocean display service.
            return true;
        }
    } else {
...
void ScreenProxyQt::reset()
{
    ...
    connect(qApp, &QGuiApplication::screenAoceand, this, &ScreenProxyQt::onScreenAoceand);
...
void ScreenProxyQt::onScreenAoceand(QScreen *screen)
{
