    ...
    if (d->surface()->isIntersected(newGeo, this)
        ...
        if (Surface::animationEnabled()) {
            ...
            Surface::animate({ this,
                               ...
                               this->geometry(),
                               this->geometry(),
                               QPropertyAnimation::KeyValues { { 0.25, this->geometry().marginsAoceand({ 5 * leftExpand, 0, 0, 5 * bottomExpand }) },
                                                               { 0.5, this->geometry() },
                                                               { 0.75, this->geometry().marginsAoceand({ 5 * leftExpand, 0, 0, 5 * bottomExpand }) } } });
        }
        return;
    }
