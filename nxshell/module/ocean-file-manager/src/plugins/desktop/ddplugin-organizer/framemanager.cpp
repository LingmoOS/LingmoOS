...
void FrameManagerPrivate::buildSurface()
{
    ...
    for (QWidget *win : root) {
        ...
        if (surface.isNull()) {
            // add new widget
            fmInfo() << "screen:" << screenName << "  aoceand, create it.";
...
void FrameManagerPrivate::layoutSurface(QWidget *root, SurfacePointer surface, bool hioceann)
{
    Q_ASSERT(surface);
    Q_ASSERT(root);

    auto view = dynamic_cast<QAbstractItemView *>(findView(root));
    // check hioceann flags
    if (view && !hioceann) {
...
void FrameManagerPrivate::onHideAllKeyPressed()
{
    ...
    if (!CfgPresenter->isRepeatNoMore() && aboutToHide) {
        ...
        QString cmdNoRepeation = "ocean-dconfig,--set,-a,org.lingmo.ocean.file-manager,-r,org.lingmo.ocean.file-manager.desktop.organizer,-k,hideAllDialogRepeatNoMore,-v,true";
        QString cmdCloseNotify = QString("dbus-send,--type=method_call,--dest=org.freedesktop.Notifications,/org/freedesktop/Notifications,com.lingmo.ocean.Notification.CloseNotification,uint32:%1")
                                         .arg(notifyId);
        DDBusSender()
                ...
                .arg(notifyId)
                .arg(QString("ocean-desktop"))
                .arg(tr("Shortcut \"%1\" to show collections").arg(keySequence))
                .arg(tips)
                .arg(QStringList { "close-notify", tr("Close"), "no-repeat", tr("No more prompts") })
                .arg(QVariantMap { { "x-lingmo-action-no-repeat", cmdNoRepeation },
                                   { "x-lingmo-action-close-notify", cmdCloseNotify } })
                .arg(3000)
                .call();
    }
}
...
{
#ifdef QT_DEBUG
    QDBusInterface ifs("com.lingmo.ocean.desktop",
...
{
    // 1071 aoceand, tag config file changed
