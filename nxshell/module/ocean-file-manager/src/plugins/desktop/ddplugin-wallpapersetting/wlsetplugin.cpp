...
void WlSetPlugin::registerDBus()
{
    ...
    if (!conn.registerObject("/org/lingmo/ocean/desktop/wallpapersettings", "org.lingmo.ocean.desktop.wallpapersettings", ifs, registerOptions)) {
        fmCritical() << "org.lingmo.ocean.desktop.wallpapersettings register object failed" << conn.lastError();
        delete ifs;
    }
}
...
{
    if (DFMBASE_NAMESPACE::SysInfoUtils::isLingmo23()) {
        show(name, (int)WallpaperSettings::Mode::WallpaperMode);
    } else {
        fmDebug() << "call ControlCenter serivce by dbus.";
        QDBusMessage msg = QDBusMessage::createMethodCall("org.lingmo.ocean.ControlCenter1", "/org/lingmo/ocean/ControlCenter1",
                                                          "org.lingmo.ocean.ControlCenter1", "ShowPage");
...
bool EventHandle::wallpaperSetting(const QString &name)
{
    QDBusMessage msg = QDBusMessage::createMethodCall("com.lingmo.ocean.ControlCenter", "/com/lingmo/ocean/ControlCenter",
