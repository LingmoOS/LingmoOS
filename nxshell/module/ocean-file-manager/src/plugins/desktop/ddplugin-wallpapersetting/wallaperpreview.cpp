    ...
    : QObject(parent)
{
    ...
    inter = new BackgroudInter("org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1",
                          QDBusConnection::sessionBus(), this);
    inter->setTimeout(1000);
    fmDebug() << "create org.lingmo.ocean.Appearance1 end";
#else
    fmDebug() << "create com.lingmo.wm";
    inter = new BackgroudInter("com.lingmo.wm", "/com/lingmo/wm",
                          QDBusConnection::sessionBus(), this);
    inter->setTimeout(1000);
    fmDebug() << "create com.lingmo.wm end";
#endif
    ...
    if ((DisplayMode::kShowonly == mode) || (DisplayMode::kDuplicate == mode) // 仅显示和复制
        ...
        for (ScreenPointer s : screens) {
            ...
            if (!wid.isNull()) {
                ...
                // 添加缺少的数据
                fmDebug() << "screen:" << s->name() << " aoceand, create it.";
                wid = createWidget(s);
