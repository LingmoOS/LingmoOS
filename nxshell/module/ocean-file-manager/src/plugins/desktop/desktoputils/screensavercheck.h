...
static inline bool enableScreensaver()
{
    ...
    qInfo() << "check com.lingmo.ScreenSaver";
    ...
    if (response.type() == QDBusMessage::ReplyMessage && !response.arguments().isEmpty()
        && response.arguments().first().toList().contains("com.lingmo.ScreenSaver")) {
        qDebug() << "com.lingmo.ScreenSaver is ok";
    } else {
...
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
    QGSettings desktopSettings("com.lingmo.ocean.filemanager.desktop", "/com/lingmo/ocean/filemanager/desktop/");
