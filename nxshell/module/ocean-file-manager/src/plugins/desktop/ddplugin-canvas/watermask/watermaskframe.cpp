...
void WaterMaskFrame::stateChanged(int state, int prop)
{
    ...
    fmInfo() << "reply ActiveState is" << state << prop << "show" << showSate
            << "cn" << cn << this;
    curState = state;
    curProperty = static_cast<LingmoLicenseHelper::LicenseProperty>(prop);

    // 已授权, 判断版本
    if (state == LingmoLicenseHelper::Authorized && showSate) {
        if (prop == LingmoLicenseHelper::LicenseProperty::Secretssecurity) {
        ...
        } else if (prop == LingmoLicenseHelper::LicenseProperty::Government) {   //gov
        ...
        } else if (prop == LingmoLicenseHelper::LicenseProperty::Enterprise) {   // com
   ...
   if (!showSate) {
    ...
    case LingmoLicenseHelper::TrialExpired: {
        ...
        break;
    case LingmoLicenseHelper::Authorized:
        textLabel->setText("");
        break;
