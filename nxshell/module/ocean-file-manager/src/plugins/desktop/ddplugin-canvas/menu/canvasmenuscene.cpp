...
bool CanvasMenuScene::initialize(const QVariantHash &params)
{
    ...
    d->isOCEANDesktopFileIncluded = tmpParams.value(MenuParamKey::kIsOCEANDesktopFileIncluded, false).toBool();
    ...
    if (!d->isOCEANDesktopFileIncluded) {
        // oem menu
    ...
    // the scene aoceand by binding must be initializeed after 'defalut scene'.
    ...
    if (d->predicateAction.values().contains(action)) {
        ...
        if (actionId == ActionID::kDisplaySettings) {
            ...
            QDBusMessage msg = QDBusMessage::createMethodCall("com.lingmo.ocean.ControlCenter", "/com/lingmo/ocean/ControlCenter",
                                                              "com.lingmo.ocean.ControlCenter", "ShowModule");
