...
{
    QString err;
    DConfigManager::instance()->addConfig("org.lingmo.ocean.file-manager.desktop.sys-watermask", &err);
...
void CanvasPlugin::registerDBus()
{
    ...
    if (!conn.registerObject("/org/lingmo/ocean/desktop/canvas", "org.lingmo.ocean.desktop.canvas", ifs, registerOptions)) {
        fmCritical() << "org.lingmo.ocean.desktop.canvas register object failed" << conn.lastError();
        delete ifs;
    } else {
