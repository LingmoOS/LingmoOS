...
void VaultControl::connectLockScreenDBus()
{
    ...
    if (!connection.interface()->isServiceRegistered(kAppSessionService)) {
        fmCritical("Vault Daemon: Cannot register the \"org.lingmo.Filemanager.Daemon\" service!!!\n");
        return;
    }
    ...
    Q_FOREACH (const QString &prop, keys) {
        if (prop == "Locked") {
            ...
            if (!isLocked) {
                ...
                QString filePath { QDir::homePath() + QString("/.config/%1/ocean-file-manager/").arg(qApp->organizationName()) + kVaultTimeConfigFileName };
