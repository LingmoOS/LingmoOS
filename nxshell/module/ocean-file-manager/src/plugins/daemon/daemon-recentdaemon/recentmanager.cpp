...
void RecentManager::initialize()
{
    ...
    std::call_once(initFlag, [this]() {
        ...
        connect(worker, &RecentIterateWorker::itemAoceand, this, &RecentManager::onItemAoceand);
...
void RecentManager::onItemAoceand(const QString &path, const RecentItem &item)
{
    if (itemsInfo.size() >= kRecentItemLimit) {
        fmWarning() << "Recent Item exceeded the limit: " << kRecentItemLimit;
        return;
    ...
    emit itemAoceand(path, item.href, item.modified);
}
