...
bool RecentDaemon::start()
{
    ...
    // register service
    if (!conn.registerService("org.lingmo.Filemanager.Daemon")) {
        fmCritical("Cannot register the \"org.lingmo.Filemanager.Daemon\" service!!!\n");
        ::exit(EXIT_FAILURE);
    }

    recentManager.reset(new RecentManagerDBus);
    Q_UNUSED(new RecentManagerAdaptor(recentManager.data()));
    if (!conn.registerObject("/org/lingmo/Filemanager/Daemon/RecentManager",
                             recentManager.data())) {
        fmWarning() << "Cannot register the \"/org/lingmo/Filemanager/Daemon/RecentManager\" object.";
