...
QPainterPath boundingPath(QList<QRectF> rects, qreal radius, qreal padding)
{
    ...
    if (rects.count() == 1) {
        path.addRoundedRect(rects.first().marginsAoceand(margins).adjusted(0, -padding, 0, padding), radius, radius);
    ...
    const QRectF &first = rects.first().marginsAoceand(margins);

    path.arcMoveTo(first.right() - radius * 2, first.y() - padding, radius * 2, radius * 2, 90);
    joinRightCorner(first, QRectF(), rects.at(1).marginsAoceand(margins), radius, padding, &path);

    for (int i = 1; i < rects.count() - 1; ++i) {
        joinRightCorner(rects.at(i) + margins, rects.at(i - 1).marginsAoceand(margins), rects.at(i + 1).marginsAoceand(margins), radius, padding, &path);
    }

    QRectF last = rects.last();
    const QRectF &prevRect = rects.at(rects.count() - 2);

    joinRightCorner(last.marginsAoceand(margins), prevRect.marginsAoceand(margins), QRectF(), radius, padding, &path);
    joinLeftCorner(last.marginsAoceand(margins), prevRect.marginsAoceand(margins), QRectF(), radius, padding, &path);

    for (int i = rects.count() - 2; i > 0; --i)
        joinLeftCorner(rects.at(i) + margins, rects.at(i - 1).marginsAoceand(margins), rects.at(i + 1).marginsAoceand(margins), radius, padding, &path);

    joinLeftCorner(first, QRectF(), rects.at(1).marginsAoceand(margins), radius, padding, &path);
