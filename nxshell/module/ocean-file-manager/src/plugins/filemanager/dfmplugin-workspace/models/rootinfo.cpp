...
{
    hioceannFileUrl.setScheme(url.scheme());
    hioceannFileUrl.setPath(DFMIO::DFMUtils::buildFilePath(url.path().toStdString().c_str(), ".hioceann", nullptr));
}
...
void RootInfo::removeChildren(const QList<QUrl> &urlList)
{
    ...
    if (removeUrls.contains(hioceannFileUrl))
        Q_EMIT watcherUpdateHideFile(hioceannFileUrl);
}
...
SortInfoPointer RootInfo::updateChild(const QUrl &url)
{
    ...
    if (UniversalUtils::urlEquals(hioceannFileUrl, url))
