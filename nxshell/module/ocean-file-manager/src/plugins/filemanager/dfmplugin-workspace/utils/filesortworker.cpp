...
void FileSortWorker::handleWatcherUpdateHideFile(const QUrl &hidUrl)
{
    if (isCanceled)
        return;
    auto hioceannFileInfo = InfoFactory::create<FileInfo>(hidUrl);
    if (!hioceannFileInfo)
        return;
    auto hidlist = DFMUtils::hideListFromUrl(QUrl::fromLocalFile(hioceannFileInfo->pathOf(PathInfoType::kFilePath)));
