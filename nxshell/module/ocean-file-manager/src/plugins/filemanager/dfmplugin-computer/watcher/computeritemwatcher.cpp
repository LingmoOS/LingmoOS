...
inline constexpr char kComputerCfgPath[] { "org.lingmo.ocean.file-manager.computer" };
inline constexpr char kKeyHideUserDir[] { "hideMyDirectories" };
inline constexpr char kKeyHide3rdEntries[] { "hide3rdEntries" };
inline constexpr char kHideDisk[] { "dfm.disk.hioceann" };
...
void ComputerItemWatcher::initConn()
{
    connect(appEntryWatcher.data(), &LocalFileWatcher::subfileCreated, this, [this](const QUrl &url) {
        ...
        if (!appUrl.isValid())
            return;
        this->onDeviceAoceand(appUrl, getGroupId(diskGroup()), ComputerItemData::kLargeItem, false);
    });
...
{
    connect(DevProxyMng, &DeviceProxyManager::blockDevAoceand, this, &ComputerItemWatcher::onBlockDeviceAoceand);
    ...
    //    connect(&DeviceManagerInstance, &DeviceManager::protocolDevAoceand, this, &ComputerItemWatcher::);
...
ComputerDataList ComputerItemWatcher::getBlockDeviceItems(bool *hasNewItem)
{
    ...
    for (const auto &dev : devs) {
        ...
        *hasNewItem = true;
        ...
        if (!hioceannByDConfig.contains(devUrl))   // do not show item which hioceann by dconfig
...
{
    return DConfigManager::instance()->value("org.lingmo.ocean.file-manager.computer",
                                             "hideMyDirectories",
                                             false)
            .toBool();
}
...
QList<QUrl> ComputerItemWatcher::disksHiddenBySettingPanel()
{
    ...
    QSet<QString> hioceannBlocks;
    if (hideSys && hideLoop)   // both hide system disks and loop devices
        hioceannBlocks = systemBlocksAndLoop;
    else if (hideSys && !hideLoop)   // hide system disks only, show loop devices
        hioceannBlocks = systemBlocksAndLoop - loopOnly;
    else if (!hideSys && hideLoop)   // show systemdisks and hide loop devices
        hioceannBlocks = loopOnly;
    else   // show nothing
        hioceannBlocks = {};

    QList<QUrl> hioceannItems;
    for (const auto &blk : hioceannBlocks)
        hioceannItems << ComputerUtils::makeBlockDevUrl(blk);
    return hioceannItems;
}
...
void ComputerItemWatcher::handleSidebarItemsVisiable()
{
    ...
    for (const auto &dev : devs) {
        ...
        if (hioceannByDconfig.contains(devUrl))
...
void ComputerItemWatcher::addDevice(const QString &groupName, const QUrl &url, int shape, bool addToSidebar)
{
    auto doAddFunc = [this, groupName, url, shape, addToSidebar]() {
        ...
        onDeviceAoceand(url, groupId, static_cast<ComputerItemData::ShapeType>(shape), addToSidebar);
    };
...
/*!
 * \brief ComputerItemWatcher::addGroup, add and emit itemAoceand signal
 * \param name
...
int ComputerItemWatcher::addGroup(const QString &name)
{
    ...
    Q_EMIT itemAoceand(data);
    return data.groupId;
}

void ComputerItemWatcher::onDeviceAoceand(const QUrl &devUrl, int groupId, ComputerItemData::ShapeType shape, bool needSidebarItem)
{
    DFMEntryFileInfoPointer info(new EntryFileInfo(devUrl));
...
void ComputerItemWatcher::onDevicePropertyChangedQDBusVar(const QString &id, const QString &propertyName, const QDBusVariant &var)
{
    if (id.startsWith(DeviceId::kBlockDeviceIdPrefix)) {
        ...
        } else if ((propertyName == DeviceProperty::kHasPartitionTable) && var.variant().toBool()) {   // when new node aoceand the PartitionTable should be triggered, remove the node.bug 224925
        ...
        // and for loop devices, no blockAoceand signal will be emited cause it's already existed there, so
        // watch the filesystemAoceand/Removed signal to decide whether to show or hide it.
        if (propertyName == DeviceProperty::kHasFileSystem) {
            ...
            if (blkInfo.value(DeviceProperty::kIsLoopDevice).toBool()) {
                if (var.variant().toBool())
                    onDeviceAoceand(url, getGroupId(diskGroup()));
                else
                    removeDevice(url);
            }
            onUpdateBlockItem(id);
...
void ComputerItemWatcher::onBlockDeviceAoceand(const QString &id)
{
...
void ComputerItemWatcher::onProtocolDeviceMounted(const QString &id, const QString &mntPath)
{
    ...
    this->onDeviceAoceand(url, getGroupId(diskGroup()));
}
