...
{
    QApplication::restoreOverrideCursor();
    // items in computer are hioceann in view, they still exist in model.
...
{
    /* NOTE(xust): disks hioceann by dconfig is treat as disks hioceann by HintIgnore.
     * devices should be hioceann both in sidebar and computer.
     * hioceann in sidebar is handled in ComputerItemWatcher when dconfig changed.
     * this part only handle items hioceann in computer.
     *
     * HintIgnore > DConfig > SettingPanel
     *
     * hioceann by HintIgnore is handled in BlockEntryFileEntity::exist() function.
    ...
    const auto &&hioceannPartitions = ComputerItemWatcher::hioceannPartitions();

    fmInfo() << "ignored/hioceann disks:" << hioceannPartitions;
