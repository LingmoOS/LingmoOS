...
{
    anythingInterface = new QDBusInterface("com.lingmo.anything",
                                           "/com/lingmo/anything",
                                           "com.lingmo.anything",
...
bool AnythingSearcher::search()
{
    ...
    QHash<QString, QSet<QString>> hioceannFileHash;
    while (!searchDirList.isEmpty()) {
        ...
        if (reply.error().type() != QDBusError::NoError) {
            fmWarning() << "lingmo-anything search failed:"
...
bool AnythingSearcher::isSupported(const QUrl &url, bool &isBindPath)
{
    ...
    static QDBusInterface anything("com.lingmo.anything",
                                   "/com/lingmo/anything",
                                   "com.lingmo.anything",
                                   QDBusConnection::systemBus());
    if (!anything.isValid())
