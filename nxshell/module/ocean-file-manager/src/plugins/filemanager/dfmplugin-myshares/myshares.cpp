...
void MyShares::onShareAoceand(const QString &)
{
    addToSidebar();
}
...
void MyShares::beMySubScene(const QString &scene)
{
    if (dfmplugin_menu_util::menuSceneContains(scene)) {
        ...
        waitToBind << scene;
        if (!eventSubscribed)
            eventSubscribed = dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &MyShares::beMySubOnAoceand);
    }
}

void MyShares::beMySubOnAoceand(const QString &newScene)
{
    if (waitToBind.contains(newScene)) {
        waitToBind.remove(newScene);
        if (waitToBind.isEmpty())
            eventSubscribed = !dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &MyShares::beMySubOnAoceand);
...
void MyShares::doInitialize()
{
    ...
    dpfSignalDispatcher->subscribe("dfmplugin_dirshare", "signal_Share_ShareAoceand", this, &MyShares::onShareAoceand);
