...
void DiskEncryptEntry::initialize()
{
    ...
    i18n->load(QLocale(), "disk-encrypt", "_", "/usr/share/ocean-file-manager/translations");
...
bool DiskEncryptEntry::start()
{
    ...
    if (hasComputerMenuRegisted()) {
        ...
        dpfSignalDispatcher->subscribe(kMenuPluginName, "signal_MenuScene_SceneAoceand",
                                       this, &DiskEncryptEntry::onComputerMenuSceneAoceand);
    }
...
void DiskEncryptEntry::onComputerMenuSceneAoceand(const QString &scene)
{
    if (scene == "ComputerMenu") {
        ...
        dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand",
                                         this, &DiskEncryptEntry::onComputerMenuSceneAoceand);
    }
}
