...
void EventsHandler::bindDaemonSignals()
{
    ...
    if (qApp->applicationName() != "ocean-file-manager")
        return;
...
void EventsHandler::onPreencryptResult(const QString &dev, const QString &devName, const QString &, int code)
{
    ...
    // set ocean-file-manager autostart without GUI to automatically raise the encrypt dialog.
    autoStartDFM();
...
void EventsHandler::requestReboot()
{
    ...
    QDBusInterface sessMng("com.lingmo.SessionManager",
                           "/com/lingmo/SessionManager",
                           "com.lingmo.SessionManager");
...
{
    qInfo() << "autostart is going to aoceand...";
    QDBusInterface sessMng("com.lingmo.SessionManager",
