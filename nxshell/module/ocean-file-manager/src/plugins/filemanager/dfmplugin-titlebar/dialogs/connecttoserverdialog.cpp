...
void ConnectToServerDialog::initializeUi()
{
    ...
    // init collection empty area
    {
        ...
        if (DGuiApplicationHelper::instance()->themeType() == DGuiApplicationHelper::LightType)
            emptyIcon->setPixmap(QIcon(":icons/lingmo/builtin/light/icons/no_favorites_yet.svg").pixmap({ 145, 145 }));
        else if (DGuiApplicationHelper::instance()->themeType() == DGuiApplicationHelper::DarkType)
            emptyIcon->setPixmap(QIcon(":icons/lingmo/builtin/dark/icons/no_favorites_yet.svg").pixmap({ 145, 145 }));
