...
static constexpr char kInterfaceService[] { "org.lingmo.Filemanager.AccessControlManager" };
static constexpr char kInterfacePath[] { "/org/lingmo/Filemanager/AccessControlManager" };
static constexpr char kInterfaceInterface[] { "org.lingmo.Filemanager.AccessControlManager" };
...
namespace LingmoPwdCheck {
// the liblingmo-pw-check-dev is required for liblingmo_pw_check.so -_-
static constexpr char kLibraryName[] { "liblingmo_pw_check.so.1" };
static constexpr char kInterfacePwdCheck[] { "lingmo_pw_check" };
...
{
    QLibrary lib(LingmoPwdCheck::kLibraryName);
    if (lib.load()) {
        lingmoPwCheck = reinterpret_cast<LingmoPwCheckFunc>(lib.resolve(LingmoPwdCheck::kInterfacePwdCheck));
        getPasswdLevel = reinterpret_cast<GetPasswdLevelFunc>(lib.resolve(LingmoPwdCheck::kInterfaceGetPwdLevel));
        errToString = reinterpret_cast<ErrToStringFunc>(lib.resolve(LingmoPwdCheck::kInterfaceErrToString));
    }
}
...
bool DPCConfirmWidget::checkPasswdComplexity(const QString &pwd, QString *msg)
{
    ...
    // not loaded liblingmo_pw_check.so
    if (!getPasswdLevel || !lingmoPwCheck || !errToString)
        return true;
    ...
    int type = lingmoPwCheck(userName.toLocal8Bit().data(), newPwdArray.data(), 3, nullptr);
    if (type != 0) {
