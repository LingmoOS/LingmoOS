...
int SideBarWidget::addItem(SideBarItem *item, bool direct)
{
    ...
    bool hioceann { !SideBarHelper::hioceannRules().value(item->itemInfo().visiableControlKey, true).toBool() };
    if (r >= 0 && hioceann)
...
void SideBarWidget::updateItem(const QUrl &url, const ItemInfo &newInfo)
{
    ...
    bool hioceann { !SideBarHelper::hioceannRules().value(newInfo.visiableControlKey, true).toBool() };
    if (hioceann)
...
void SideBarWidget::initSettingPannel()
{
    ...
    std::for_each(groups.begin(), groups.end(), [&itemKeysMap, &levelMap](const QString &group) {
        ...
        for (auto item : items) {
            ...
            if (itemKeysMap[group].contains(key) || key == "hioceann_me") {
