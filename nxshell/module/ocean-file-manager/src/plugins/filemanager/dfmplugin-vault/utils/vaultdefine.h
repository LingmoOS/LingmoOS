...
inline constexpr char kRootProxy[] { "pkexec lingmo-vault-authenticateProxy" };

// NOTE: Not DBus! This is polkit policy (from com.lingmo.filemanager.vault.policy)
inline constexpr char kPolkitVaultCreate[] { "com.lingmo.filemanager.daemon.VaultManager.Create" };
inline constexpr char kPolkitVaultRemove[] { "com.lingmo.filemanager.daemon.VaultManager.Remove" };
...
{
    return QString("ps -xo pid,cmd | grep /usr/bin/lingmo-compressor | grep ") + value + QString(" | grep -v grep | awk '{print $1}'");
}

//!! 保险箱时间配置文件
inline constexpr char kVaultTimeConfigFileSuffix[] { "/../ocean-file-manager/vaultTimeConfig.json" };
inline constexpr char kVaultTimeConfigFile[] { "/../ocean-file-manager/vaultTimeConfig" };
