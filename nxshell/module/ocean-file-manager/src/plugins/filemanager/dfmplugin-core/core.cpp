...
#include <dfm-mount/oceanvicemanager.h>
...
void Core::connectToServer()
{
    ...
    if (!DevProxyMng->initService()) {
        // active server
        QDBusInterface ifs("org.lingmo.Filemanager.Daemon",
                           "/org/lingmo/Filemanager/Daemon");
        ifs.asyncCall("Ping");
...
void Core::onAllPluginsStarted()
{
    ...
    // ocean-select-dialog also uses the core plugin, don't start filemanger window
    QString &&curAppName { qApp->applicationName() };
    if (!WindowUtils::isWayLand())
        qDebug() << "start" << DAPP_USING_ATT(DAPP_ATT_WIDGETS, 122);
    if (curAppName == "ocean-file-manager")
