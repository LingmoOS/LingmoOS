...
void RecentManager::resetRecentNodes()
{
    ...
    for (const auto &value : topLevelList) {
        ...
        if (!map.isEmpty()) {
            ...
            onItemAoceand(path, href, modified);
        } else {
...
void RecentManager::init()
{
    recentDBusInterce.reset(
            new RecentManagerDBusInterface("org.lingmo.Filemanager.Daemon",
    ...
    connect(recentDBusInterce.data(), &RecentManagerDBusInterface::ReloadFinished,
            this, [this](qint64 timestamp) {
                ...
                std::call_once(flag, [this]() {
                    // 初始化的过程中可能会发送大量信号
                    connect(recentDBusInterce.data(), &RecentManagerDBusInterface::ItemAoceand, this, &RecentManager::onItemAoceand);
...
void RecentManager::onItemAoceand(const QString &path, const QString &href, qint64 modified)
{
    if (path.isEmpty())
        return;
    ...
    fmDebug() << "recent item aoceand:" << url;
    RecentItem item;
    item.fileInfo = info;
