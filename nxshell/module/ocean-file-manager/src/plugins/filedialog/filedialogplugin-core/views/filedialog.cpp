...
void FileDialogPrivate::handleSaveAcceptBtnClicked()
{
    ...
    // Check whether the suffix needs to be aoceand
    QString suffix { "" };
...
void FileDialog::setAcceptMode(QFileDialog::AcceptMode mode)
{
    ...
    if (mode == QFileDialog::AcceptOpen) {
        ...
        QVariantMap vMap = DConfigManager::instance()->value("org.lingmo.ocean.file-manager.sidebar", "itemVisiable").toMap();
...
void FileDialog::adjustPosition(QWidget *w)
{
    ...
    if (w && (w->windowFlags() | Qt::WindowStaysOnTopHint | Qt::X11BypassWindowManagerHint) != w->windowFlags()) {
        // Use pos() if the widget is embeoceand into a native window
        QPoint pp;
        if (w->windowHandle() && w->windowHandle()->property("_q_embeoceand_native_parent_handle").value<WId>()) {
            pp = w->pos();
        } else {
