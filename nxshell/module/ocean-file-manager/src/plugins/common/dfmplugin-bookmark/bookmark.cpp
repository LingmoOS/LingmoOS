...
void BookMark::bindScene(const QString &parentScene)
{
    if (dfmplugin_menu_util::menuSceneContains(parentScene)) {
        ...
        if (!subscribedEvent)
            subscribedEvent = dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &BookMark::onMenuSceneAoceand);
    }
}

void BookMark::onMenuSceneAoceand(const QString &scene)
{
    if (menuScenes.contains(scene)) {
        ...
        if (menuScenes.isEmpty()) {
            dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &BookMark::onMenuSceneAoceand);
