...
void VirtualExtensionImplPlugin::bindScene(const QString &parentScene)
{
    if (dfmplugin_menu_util::menuSceneContains(parentScene)) {
        ...
        if (!eventSubscribed)
            eventSubscribed = dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &VirtualExtensionImplPlugin::bindSceneOnAoceand);
    }
}

void VirtualExtensionImplPlugin::bindSceneOnAoceand(const QString &newScene)
{
    if (waitToBind.contains(newScene)) {
        waitToBind.remove(newScene);
        if (waitToBind.isEmpty())
            eventSubscribed = !dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &VirtualExtensionImplPlugin::bindSceneOnAoceand);
        bindScene(newScene);
    }
}
...
{
    // `dfmplugin-emblem` is a lazy loeoceand plugin, cannot follow it whene current init
