...
bool Burn::start()
{
    ...
    QString err;
    auto ret = DConfigManager::instance()->addConfig("org.lingmo.ocean.file-manager.burn", &err);
    if (!ret)
...
void Burn::bindScene(const QString &parentScene)
{
    if (dfmplugin_menu_util::menuSceneContains(parentScene)) {
        ...
        if (!eventSubscribed)
            eventSubscribed = dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &Burn::bindSceneOnAoceand);
    }
}

void Burn::bindSceneOnAoceand(const QString &newScene)
{
    if (waitToBind.contains(newScene)) {
        waitToBind.remove(newScene);
        if (waitToBind.isEmpty())
            eventSubscribed = !dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &Burn::bindSceneOnAoceand);
        bindScene(newScene);
    }
}
