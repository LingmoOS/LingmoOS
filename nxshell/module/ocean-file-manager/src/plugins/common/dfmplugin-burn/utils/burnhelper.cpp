...
#define DISCBURN_STAGING "discburn"
    return QUrl::fromLocalFile(QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation)   // ~/.cache
                               + "/" + qApp->organizationName() + "/" DISCBURN_STAGING "/"   // ~/.cache/lingmo/discburn/
                               + dev.replace('/', '_'));
}
...
{
    const auto &&ret = DConfigManager::instance()->value("org.lingmo.ocean.file-manager.burn", "burnEnable");
    return ret.isValid() ? ret.toBool() : true;
}

bool BurnHelper::burnIsOnLocalStaging(const QUrl &url)
{
    if (!url.path().contains("/.cache/lingmo/discburn/_dev_"))
        return false;
