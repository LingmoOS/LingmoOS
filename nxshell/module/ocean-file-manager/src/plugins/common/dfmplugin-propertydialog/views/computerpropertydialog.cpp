...
#    define SYSTEM_INFO_SERVICE "com.lingmo.daemon.SystemInfo"
#    define SYSTEM_INFO_PATH "/com/lingmo/daemon/SystemInfo"
#else
#    define SYSTEM_INFO_SERVICE "org.lingmo.ocean.SystemInfo1"
#    define SYSTEM_INFO_PATH "/org/lingmo/ocean/SystemInfo1"
#endif
    ...
    if (LSysInfo::isLingmo()) {
        if (LSysInfo::uosType() == LSysInfo::UosServer) {
            ...
            if (LSysInfo::UosEdition::UosProfessional == LSysInfo::uosEditionType()
                ...
                lingmoLicenseInfo.setTimeout(1000);
                if (!lingmoLicenseInfo.isValid()) {
                    fmWarning() << "Dbus com.lingmo.license is not valid!";
                    return defaultEdition;
                }
                fmInfo() << "Start call Dbus com.lingmo.license AuthorizationState";
                int activeInfo = lingmoLicenseInfo.property("AuthorizationState").toInt();
                fmInfo() << "End call Dbus com.lingmo.license AuthorizationState";
                if (kActivated == activeInfo) {
                    fmInfo() << "Start call Dbus com.lingmo.license ServiceProperty";
