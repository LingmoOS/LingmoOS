...
void FileTagCacheWorker::onTagAoceand(const QVariantMap &tags)
{
    FileTagCache::instance().addTags(tags);
    emit FileTagCacheIns.newTagsAoceand(tags);
}
...
void FileTagCacheController::init()
{
    ...
    connect(TagProxyHandleIns, &TagProxyHandle::newTagsAoceand, cacheWorker.data(), &FileTagCacheWorker::onTagAoceand);
