...
void DirShare::initialize()
{
    ...
    connect(UserShareHelperInstance, &UserShareHelper::shareAoceand, this, &DirShare::onShareStateChanged);
...
void DirShare::bindScene(const QString &parentScene)
{
    if (dfmplugin_menu_util::menuSceneContains(parentScene)) {
        ...
        if (!eventSubscribed)
            eventSubscribed = dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &DirShare::bindSceneOnAoceand);
    }
}

void DirShare::bindSceneOnAoceand(const QString &newScene)
{
    if (waitToBind.contains(newScene)) {
        waitToBind.remove(newScene);
        if (waitToBind.isEmpty())
            eventSubscribed = !dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &DirShare::bindSceneOnAoceand);
        bindScene(newScene);
    }
}
