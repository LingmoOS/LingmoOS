...
bool SendToMenuScene::initialize(const QVariantHash &params)
{
    ...
    d->isFocusOnOCEANDesktopFile = tmpParams.value(MenuParamKey::kIsFocusOnOCEANDesktopFile, false).toBool();
...
bool SendToMenuScene::create(QMenu *parent)
{
    ...
    // send to desktop
    if (!d->onDesktop && !d->isFocusOnOCEANDesktopFile) {
    ...
    if (!d->isFocusOnOCEANDesktopFile && !d->isSystemPathIncluded) {
        // bluetooth
        bool bluetoothAvailable = dpfSlotChannel->push("dfmplugin_utils", "slot_Bluetooth_IsAvailable").toBool();
        fmDebug() << "bluetooth: menu action can be aoceand: " << bluetoothAvailable;
        ...
        for (const QString &dev : dedupedDevs) {
            ...
            if (data.value(DeviceProperty::kOptical).toBool() || DeviceUtils::isSystemDisk(data)) {
                continue;   // this should be aoceand in burn plugin.
            } else {
