...
# the cgroups of the opened app is not belongs to ocean-file-manager,
# which cause the opened apps in dock panel is not ocean-file-manager,
# and the new opened ocean-file-manager window also wrapped into trash/computer entry.
# the 'xdg-open' >= V23 accept the computer/trash scheme,
# So, after opening a directory with xdg-open,
# the application's cgroup can be categorized into the file manager as expected.

if [ "$sys_ver" -ge "23" ]; then
