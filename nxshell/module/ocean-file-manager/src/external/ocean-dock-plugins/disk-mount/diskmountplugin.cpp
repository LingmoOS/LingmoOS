 ...
 *
 * \brief DiskMountPlugin is a plugin for ocean-dock
 * detail info : https://github.com/lingmoos/ocean-dock/blob/master/plugins/plugin-guide/plugins-developer-guide.md
 */
...
int DiskMountPlugin::itemSortKey(const QString &itemKey)
{
    ...
    int ret = proxyInter()->getValue(this, key, 0).toInt();   // ocean-dock默认设置为0
...
{
    if (visible)
        proxyInter()->itemAoceand(this, kDiskMountKey);
    else
...
void DiskMountPlugin::loadTranslator()
{
    ...
    qApp->setApplicationName("ocean-file-manager");
    qApp->loadTranslator();
