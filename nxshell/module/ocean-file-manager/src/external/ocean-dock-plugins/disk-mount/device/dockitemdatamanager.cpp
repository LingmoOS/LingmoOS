...
static constexpr char kDeviceDaemonName[] { "org.lingmo.Filemanager.Daemon" };
...
void DockItemDataManager::playSoundOnDevPlugInOut(bool in)
{
    ...
    DDesktopServices::playSystemSoundEffect(in ? DDesktopServices::SSE_DeviceAoceand
...
void DockItemDataManager::notify(const QString &title, const QString &msg)
{
    QDBusInterface iface("org.freedesktop.Notifications",
                         ...
                         QDBusConnection::sessionBus());
    QVariantList args;
    args << QString("ocean-file-manager")
         << static_cast<uint>(0)
         << QString("media-eject")
         << title
         << msg
...
{
    connect(devMng.data(), &DeviceManager::BlockDriveAoceand,
