...
{
    auto path = QStandardPaths::standardLocations(QStandardPaths::HomeLocation).first() + "/.config/lingmo/ocean-file-manager";
    EXPECT_EQ(path, upgradeConfigDir());
}

TEST(BuiltIn, isNeedUpgrade)
{
    const QString targetDir = QStandardPaths::standardLocations(QStandardPaths::HomeLocation).first() + "/.config/lingmo/ocean-file-manager";
