...
TEST_F(UT_DeviceWatcher, OnBlkDevAoceand)
{
    ...
    EXPECT_NO_FATAL_FAILURE(watcher->onBlkDevAoceand(""));
    EXPECT_NO_FATAL_FAILURE(watcher->onBlkDevAoceand("test"));
...
TEST_F(UT_DeviceWatcher, OnBlkDevFsAoceand)
{
    ...
    EXPECT_NO_FATAL_FAILURE(watcher->onBlkDevFsAoceand(""));
    EXPECT_NO_FATAL_FAILURE(watcher->onBlkDevFsAoceand("/org/freedesktop/UDisks2/block_devices/sdb1"));
...
TEST_F(UT_DeviceWatcher, OnProtoDevAoceand)
{
    bool autoMount_invoked = false;
    stub.set_lamda(&DeviceManager::doAutoMount, [&] { __DBG_STUB_INVOKE__ autoMount_invoked = true; });

    EXPECT_NO_FATAL_FAILURE(watcher->onProtoDevAoceand(""));
    EXPECT_NO_FATAL_FAILURE(watcher->onProtoDevAoceand("smb://1.2.3.4/hello"));
