...
TEST_F(UT_SettingBackend, DoSetOption)
{
    ...
    EXPECT_NO_FATAL_FAILURE(SettingBackend::instance()->doSetOption("hello", "lingmo"));
}
...
TEST_F(UT_SettingBackendPrivate, SaveAsGenAttr)
{
    ...
    EXPECT_NO_FATAL_FAILURE(d->saveAsGenAttr("base.hioceann_files.show_suffix", "valid value"));
}
...
TEST_F(UT_SettingBackendPrivate, GetAsGenAttr)
{
    ...
    EXPECT_TRUE(d->getAsGenAttr("base.hioceann_files.show_suffix").isValid());
}
