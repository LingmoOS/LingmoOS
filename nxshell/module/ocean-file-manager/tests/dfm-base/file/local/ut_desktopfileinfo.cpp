...
TEST_F(UT_DesktopFileInfo, testDesktopName)
{
    ...
    stub.set_lamda(&DesktopFile::desktopLingmoVendor, []{ __DBG_STUB_INVOKE__ return QString("lingmo");});
    stub.set_lamda(&DesktopFile::desktopDisplayName, []{ __DBG_STUB_INVOKE__ return QString("lingmo");});
    DesktopFileInfo info(url);
    QSharedPointer<dfmio::DFileInfo> dfileinfo (new dfmio::DFileInfo(url));
    stub.set_lamda(VADDR(ProxyFileInfo, refresh), []{ __DBG_STUB_INVOKE__ });
    EXPECT_EQ(QString("lingmo"), info.desktopName());

    stub.set_lamda(&DesktopFile::desktopDisplayName, []{ __DBG_STUB_INVOKE__ return QString("");});
    stub.set_lamda(&DesktopFile::desktopLocalName, []{ __DBG_STUB_INVOKE__ return QString("lingmo");});
    info.refresh();
    EXPECT_EQ(QString("lingmo"), info.desktopName());
...
TEST_F(UT_DesktopFileInfo, testDesktopFileInfoSupportedOfAttributes)
{
    ...
    QUrl url = QUrl::fromLocalFile(QDir::currentPath());
    stub_ext::StubExt stub;
    stub.set_lamda(&DesktopFile::desktopLingmoId, []{ __DBG_STUB_INVOKE__ return QString("ocean-computer");});
    DesktopFileInfo info(url);
    ...
    stub.set_lamda(&DesktopFile::desktopLingmoId, []{ __DBG_STUB_INVOKE__ return QString("ocean-file-manager");});
    stub.set_lamda(VADDR(ProxyFileInfo, refresh), []{ __DBG_STUB_INVOKE__ });
