...
TEST(wallPaperSettings, Init)
{
    ...
    EXPECT_EQ(d->wmInter->service(), QString("com.lingmo.wm"));
    EXPECT_TRUE(d->regionMonitor);
    EXPECT_EQ(d->appearanceIfs->service(), QString("com.lingmo.daemon.Appearance"));
    EXPECT_EQ(d->screenSaverIfs->service(), QString("com.lingmo.ScreenSaver"));
...
{
    stub.set_lamda(&ComLingmoScreenSaverInterface::ConfigurableItems, []() {
        __DBG_STUB_INVOKE__
    ...
    list << "screenSaver1"
         << "screenSaver2"
         << "flurry";
    stub.set_lamda(&ComLingmoScreenSaverInterface::allScreenSaver, [&list]() {
        __DBG_STUB_INVOKE__
        return list;
    });
    ...
    stub.set_lamda(&ComLingmoScreenSaverInterface::GetScreenSaverCover, []() {
        __DBG_STUB_INVOKE__
        return QDBusPendingReply<QString>();
    });
    QString currentItem = "screenSaver1";
    stub.set_lamda(&ComLingmoScreenSaverInterface::currentScreenSaver, [&currentItem]() {
        __DBG_STUB_INVOKE__
        return currentItem;
    });
