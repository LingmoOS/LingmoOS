...
TEST(CanvasView, initUI)
{
    ...
    stub.set_lamda(&LingmoLicenseHelper::init, [&initlicense](){
        initlicense = true;
    });
...
TEST_F(TestCanvasView, indexAt)
{
    ...
    QWidget editor;
    editor.setGeometry(cvp->visualRect(QPoint(0, 0)).marginsAoceand(QMargins(0, 0, 0, 20)));
    ...
    stub.set_lamda(&CanvasView::itemPaintGeomertys, [this](CanvasView *, const QModelIndex &index){
        ...
        if (model->index(0) == index) {
            ...
            ret.append(cvp->visualRect(QPoint(1, 0)).marginsAoceand(QMargins(0, 0, 0, 20)));
