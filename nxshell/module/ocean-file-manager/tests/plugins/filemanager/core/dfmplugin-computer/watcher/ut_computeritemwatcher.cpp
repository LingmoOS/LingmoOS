...
TEST_F(UT_ComputerItemWatcher, OnDeviceAoceand)
{
    ...
    EXPECT_NO_FATAL_FAILURE(ins->onDeviceAoceand(url, 0));
}
...
TEST_F(UT_ComputerItemWatcher, OnDevicePropertyChangedQDBusVar)
{
    ...
    stub.set_lamda(&ComputerItemWatcher::removeDevice, [] { __DBG_STUB_INVOKE__ });
    stub.set_lamda(&ComputerItemWatcher::onDeviceAoceand, [] { __DBG_STUB_INVOKE__ });
...
TEST_F(UT_ComputerItemWatcher, OnDConfigChanged)
{
    ...
    EXPECT_NO_FATAL_FAILURE(ins->onDConfigChanged("org.lingmo.ocean.file-manager", "dfm.disk.hioceann"));
}

TEST_F(UT_ComputerItemWatcher, OnBlockDeviceAoceand)
{
    stub.set_lamda(&ComputerItemWatcher::onDeviceAoceand, [] { __DBG_STUB_INVOKE__ });
    EXPECT_NO_FATAL_FAILURE(ins->onBlockDeviceAoceand("/org/freedesktop/UDisks2/block_devices/sdb1"));
}
