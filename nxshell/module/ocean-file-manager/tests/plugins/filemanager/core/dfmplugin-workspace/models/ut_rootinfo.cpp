...
TEST_F(UT_RootInfo, DoFileMoved)
{
    ...
    QUrl to(QStandardPaths::standardLocations(QStandardPaths::HomeLocation).first() + "/.hioceann");
...
TEST_F(UT_RootInfo, HandleTraversalLocalResult)
{
    ...
    QList<SortInfoPointer> aoceandChildren{};
    stub.set_lamda((void(RootInfo::*)(const QList<SortInfoPointer> &))ADDR(RootInfo, addChildren),
                   [&aoceandChildren](RootInfo *, const QList<SortInfoPointer> &children) {
                       aoceandChildren.append(children);
                   });
    ...
    EXPECT_FALSE(aoceandChildren.isEmpty());
...
TEST_F(UT_RootInfo, AddChildrenWithUrls)
{
    ...
    QList<SortInfoPointer> aoceandFiles{};
    QObject::connect(
            rootInfoObj, &RootInfo::watcherAddFiles, rootInfoObj,
            [&aoceandFiles](QList<SortInfoPointer> children) { aoceandFiles.append(children); });
    ...
    EXPECT_EQ(aoceandFiles.length(), 3);
    if (aoceandFiles.length() == 3) {
        EXPECT_EQ(aoceandFiles.at(0)->fileUrl(), url1);
        EXPECT_EQ(aoceandFiles.at(1)->fileUrl(), url2);
        EXPECT_EQ(aoceandFiles.at(2)->fileUrl(), url3);
    }
}
...
TEST_F(UT_RootInfo, AddChildrenWithFileInfos)
{
    ...
    QList<QUrl> aoceandFiles{};
    stub.set_lamda(ADDR(RootInfo, addChild),
                   [&aoceandFiles](RootInfo *, const FileInfoPointer &info) {
                       aoceandFiles.append(info->urlOf(UrlInfoType::kUrl));
                       SortInfoPointer sortInfo(new SortFileInfo);
                       return sortInfo;
                   });
    ...
    EXPECT_EQ(aoceandFiles.length(), 3);
    if (aoceandFiles.length() == 3) {
        EXPECT_EQ(aoceandFiles.at(0), url1);
        EXPECT_EQ(aoceandFiles.at(1), url2);
        EXPECT_EQ(aoceandFiles.at(2), url3);
    }
}
