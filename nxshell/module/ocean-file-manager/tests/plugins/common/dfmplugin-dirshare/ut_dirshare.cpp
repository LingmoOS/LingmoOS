...
TEST_F(UT_DirShare, BindScene)
{
    ...
    typedef bool (dpf::EventDispatcherManager::*Subscribe)(const QString &, const QString &, DirShare *, decltype(&DirShare::bindSceneOnAoceand));
...
TEST_F(UT_DirShare, BindSceneOnAoceand)
{
    EXPECT_NO_FATAL_FAILURE(ins->waitToBind.insert("Hello"));
    EXPECT_TRUE(ins->waitToBind.count() == 1);

    typedef bool (dpf::EventDispatcherManager::*Unsubscribe)(const QString &, const QString &, DirShare *, decltype(&DirShare::bindSceneOnAoceand));
    ...
    stub.set_lamda(&DirShare::bindScene, [] { __DBG_STUB_INVOKE__ });
    EXPECT_NO_FATAL_FAILURE(ins->bindSceneOnAoceand("Hello"));
    EXPECT_NO_FATAL_FAILURE(ins->bindSceneOnAoceand("World"));
}
