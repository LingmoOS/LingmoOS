...
1. 單擊桌面底部的 ![lingmo_launcher ](../common/lingmo_launcher.svg)，進入啟動器介面。
2. 上下滾動滑鼠滾輪瀏覽或透過搜尋找到檔案管理器![lingmo_file_manager](../common/lingmo_file_manager.svg)，單擊執行。
3. 右鍵單擊 ![lingmo_file_manager](../common/lingmo_file_manager.svg)，您可以：
   - 單擊 **建立桌面捷徑**，在桌面建立捷徑。
...
- 在任務欄右鍵單擊 ![lingmo_file_manager](../common/lingmo_file_manager.svg)，選擇 **全部關閉** 來關閉檔案管理器。
...
單擊選單欄上的圖示![icon_view](../common/icon_view.svg) 、![list_view](../common/list_view.svg) 、![list_view](../common/treelist_view.svg)或者使用快捷鍵 **Ctrl + 1** 、 **Ctrl + 2** 和 **Ctrl + 3** 來切換圖示檢視、列表檢視和樹形列表檢視。
...
- 樹形列表檢視：樹形展開文件目錄，顯示文件圖示或縮圖、名稱、修改時間、大小、類型等資訊。
...
2. 選擇 **病毒查殺**，彈出安全中心病毒查殺界面，您可以對該文件（夾）進行安全掃描，具體操作請參閱安全中心的 [病毒查殺](dman:///lingmo-defender#病毒查殺)。
...
前提條件：傳送端與接收端藍牙設備已配對並成功連接。具體連接方法可參考 [連接藍牙設備](dman:///ocean#連接藍牙設備)。
...
- 勾選 **電腦工作區隱藏內建磁碟**，電腦介面將不顯示本機磁碟。
- 勾選 **電腦工作區隱藏迴環分區**，電腦介面將不顯示loop裝置。
- 勾選 **在磁碟圖示上顯示文件系統標籤**。
