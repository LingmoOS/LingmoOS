...
2. 上下滾動鼠標滾輪瀏覽或通過搜索，找到檔案管理員圖標 ![lingmo_file_manager](../common/lingmo_file_manager.svg)，單擊運行。
...
- 在任務欄右鍵單擊 ![lingmo_file_manager](../common/lingmo_file_manager.svg)，選擇 **關閉所有**，關閉檔案管理員。
...
* 樹形列表視圖：樹形展開文件目錄，顯示文件圖標或縮略圖、名稱、修改時間、大小、類型等資訊。
...
1. 在檔案管理員界面，右鍵單擊文件（夾）。
2. 選擇 **病毒查殺**，彈出安全中心病毒查殺界面，您可以對該文件（夾）進行安全掃描，具體操作請參閱安全中心 [病毒查殺](dman:///lingmo-defender#病毒查殺)。
...
前提條件：發送端與接收端藍牙設備已配對並成功連接。具體連接方法請參閱 [連接藍牙設備](dman:///ocean#連接藍牙設備)。
