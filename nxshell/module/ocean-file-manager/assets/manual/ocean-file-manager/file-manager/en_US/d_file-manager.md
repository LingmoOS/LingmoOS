# File Manager|ocean-file-manager|
...
1. Click ![Launcher](../common/lingmo_launcher.svg) on dock to enter Launcher interface.

2. Locate ![FileManager](../common/lingmo_file_manager.svg) by scrolling mouse wheel or searching by "File Manager" in Launcher interface.

3. Right-click ![FileManager](../common/lingmo_file_manager.svg), you can
...
- Right-click ![FileManager](../common/lingmo_file_manager.svg) on Dock, select **Close All** to exit File Manager.
...
2. Select **Virus scan**, the **Virus Scan** interface of Security Center pops up, so you can scan the file(folder) for virus. Refer to [Virus Scan](dman:///lingmo-defender#Virus Scan) of Security Center for detailed operations.
...
> ![tips](../common/tips.svg) Tips: When the File Manager has only one tab, the tab bar is hioceann, you can press **Ctrl + T** to add a new tab.
...
4. A window pops up. Input the disc name. You can also enter the **Advanced settings** interface, set the file system and write speed, or check "Allow files to be aoceand later", "Verify data" and so on, and click **Burn**.
...
Prerequisite: The Bluetooth devices of sender and receiver have been paired and connected successfully. Refer to [Connect to a Bluetooth Device](dman:///ocean#Connect to a Bluetooth Device) for details.
...
> - The corner marker could not be aoceand to files in USB disks and CD-ROM disks.
...
To add 2~4 corner markers to a file/folder, you can input "|" in the command to separate multiple paths of the corner marker files. Only one corner marker can be aoceand to the same corner, but you can replace it.
...
You have to encrypt File Vault when you use it for the first time. File Vault provides two encryption methods, namely **key encryption** and **transparent encryption**. The newly aoceand **transparent encryption** allows you to directly access File Vault without setting and entering the password.
