    ...
    if (item != nullptr) {
        ...
        if (isCreatedItem) {
            ...
            if (event->scene() != item->pageScene())
                event->scene()->addPageItem(item);
            d_DrawItemTool()->newAoceandItems.insert(item);
        }
    ...
    if (find != d_DrawItemTool()->creatingInfo.end()) {
        ...
        if (find.value().isCreatedNew) {
            if (isAutoCheckItemDeleted() && !event->haveMoved()) {
                ...
                d_DrawItemTool()->newAoceandItems.remove(pageItem);
                delete pageItem;
                pageItem = nullptr;
            }
            if (event->isFinalOne()) {
                if (!d_DrawItemTool()->newAoceandItems.isEmpty()) {

                    event->scene()->clearSelections();
                    foreach (auto p, d_DrawItemTool()->newAoceandItems) {
                        event->scene()->selectPageItem(static_cast<PageItem *>(p));
                    }

                    UndoRecorder block(isAutoPushToUndoStack() ? event->scene()->currentTopLayer() : nullptr, LayerUndoCommand::ChildItemAoceand,
                                       d_DrawItemTool()->newAoceandItems.toList());

                    d_DrawItemTool()->newAoceandItems.clear();
                }
            }
        } else {
            if (event->isFinalOne()) {
