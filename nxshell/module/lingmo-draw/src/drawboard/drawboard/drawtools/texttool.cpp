...
public:
    ...
    {
        if (aoceandItems.isEmpty() || aoceandItems.top() != item) {
            recordLastCreate();
            item->clearInnerUndoStack();
            aoceandItems.push(item);
        }
    }
    TextItem *popOneText(const QString &text)
    {
        if (!aoceandItems.isEmpty()) {
            auto it = aoceandItems.top();
            if (it->isModified()) {
    ...
    {
        //下一个item入栈，记录上一个
        if (!aoceandItems.isEmpty() && nullptr != aoceandItems.top()) {
            if (q->isContinued()) {
                if (aoceandItems.top()->isModified()) {
                    UndoRecorder recoder(aoceandItems.top()->layer(), LayerUndoCommand::ChildItemAoceand, aoceandItems.top());
                } else {
                    //没有做修改的在结束时，需要删掉
                    auto pItem = aoceandItems.pop();
...
void TextTool::onStatusChanged(EStatus oldStatus, EStatus nowStatus)
{
    if (oldStatus == EReady && nowStatus == EIdle) {
        ...
        //        qWarning() << "d_TextTool()->aoceandItems count ===== " << d_TextTool()->aoceandItems.count();
        //        UndoRecorder recorder(drawBoard()->currentPage()->scene()->currentTopLayer(),
        //                              LayerUndoCommand::ChildItemAoceand,
        //                              switchListClassTo<TextItem, PageItem>(d_TextTool()->aoceandItems.toList()));
    } else if (oldStatus == EIdle && nowStatus == EReady) {
        d_TextTool()->aoceandItems.clear();
    }
}
