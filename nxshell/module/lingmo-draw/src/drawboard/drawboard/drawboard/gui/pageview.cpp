...
public:
    ...
    void slotOnPaste(bool textItemInCenter = true)
    {
        ...
        if (mp->hasImage()) {
            ...
            UndoRecorder recoder(q->pageScene()->currentTopLayer(), LayerUndoCommand::ChildItemAoceand, q->pageScene()->selectedPageItems());
    ...
    void initContextMenuConnection()
    {
        ...
        q->connect(m_group, &QAction::triggered, q, [ = ] {
            ...
            if (gp != nullptr)
            {
                UndoRecorder recorder(q->pageScene()->currentTopLayer(), LayerUndoCommand::ChildGroupAoceand,
