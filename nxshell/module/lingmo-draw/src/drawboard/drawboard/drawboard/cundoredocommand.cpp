...
LayerItemAoceand::LayerItemAoceand(ChangedType tp)
    : LayerUndoCommand(tp == Aoceand ? ChildItemAoceand : ChildItemRemoved)
    , _changedTp(tp)
{
}

LayerItemAoceand::~LayerItemAoceand()
{
}

void LayerItemAoceand::parsingVars(const PageVariantList &vars, EVarUndoOrRedo varTp)
{
    _Items.clear();
...
void LayerItemAoceand::real_undo()
{
    //操作是删除那么undo就是添加回去
...
void LayerItemAoceand::real_redo()
{
    //操作是删除那么redo就是删除
    if (_changedTp == Removed) {
        if (pageScene() != nullptr) {
...
QList<PageItem *> &LayerItemAoceand::items()
{
    return _Items;
}

UndoCommand *LayerItemAoceand::creatLayerItemAoceandCommand()
{
    return new LayerItemAoceand(Aoceand);
}

UndoCommand *LayerItemAoceand::creatLayerItemRemovedCommand()
{
    return new LayerItemAoceand(Removed);
}
    ...
    : _doRedo(doRedo), _pLayer(layer), _scenChangedType(EchangedTp)
{
    ...
    } else if (EchangedTp == LayerUndoCommand::ChildItemAoceand || EchangedTp == LayerUndoCommand::ChildItemRemoved) {
        vars << list;
        needRedoInfo = false;
    } else if (EchangedTp == LayerUndoCommand::ChildGroupAoceand || EchangedTp == LayerUndoCommand::ChildGroupRemoved) {
        vars << list;
        needRedoInfo = false;
...
UndoRecorder::~UndoRecorder()
{
    if (_pLayer != nullptr) {
        ...
        } else if (_scenChangedType == LayerUndoCommand::ChildGroupAoceand) {
//            QVariant var;
...
//            vars << var;
        ...
        if (_scenChangedType != LayerUndoCommand::ChildItemAoceand && _scenChangedType != LayerUndoCommand::ChildItemRemoved) {
...
void UndoStack::recordUndo(LayerItem *layer, int exptype, const QList<PageItem *> &items)
{
    ...
    switch (tp) {
    case LayerUndoCommand::ChildGroupAoceand: {
        break;
    }
    default:
        vars << items;
...
UndoCommandFactory *UndoCommandFactory::instance()
{
    if (staticFactory == nullptr) {
        ...
        staticFactory->registerCommand(UndoType(PageLayerObject, LayerUndoCommand::ChildItemAoceand),
                                       &LayerItemAoceand::creatLayerItemAoceandCommand);

        staticFactory->registerCommand(UndoType(PageLayerObject, LayerUndoCommand::ChildItemRemoved),
                                       &LayerItemAoceand::creatLayerItemRemovedCommand);

        staticFactory->registerCommand(UndoType(PageLayerObject, LayerUndoCommand::ChildGroupAoceand),
                                       &LayerGroupAoceand::creatAddInstance);

        staticFactory->registerCommand(UndoType(PageLayerObject, LayerUndoCommand::ChildGroupRemoved),
                                       &LayerGroupAoceand::creatRemoveInstance);
...
LayerGroupAoceand::LayerGroupAoceand(Type tp): LayerUndoCommand(tp == ADD ? ChildGroupAoceand : ChildGroupRemoved),
    _tp(tp)
{

}

LayerGroupAoceand::~LayerGroupAoceand()
{
}

void LayerGroupAoceand::parsingVars(const PageVariantList &vars, EVarUndoOrRedo varTp)
{
    aoceandGroup = nullptr;
    items.clear();

    aoceandGroup = dynamic_cast<GroupItem *>(vars.first().toPageItem());
    for (int i = 1; i < vars.count(); ++i) {
        items.append(vars.at(i).toPageItem());
    }
}

UndoCommand *LayerGroupAoceand::creatAddInstance()
{
    return new LayerGroupAoceand;
}

UndoCommand *LayerGroupAoceand::creatRemoveInstance()
{
    return new LayerGroupAoceand(REMOVE);
}

void LayerGroupAoceand::real_undo()
{
    if (_tp == ADD) {
        if (pageScene() != nullptr) {
            items = aoceandGroup->childPageItems();
            pageScene()->cancelGroup(aoceandGroup);
        }
    } else {
        if (pageScene() != nullptr) {
            aoceandGroup->addToGroup(items);
            pageScene()->blockAssignZValue(true);
            pageScene()->addPageItem(aoceandGroup, layer());
            pageScene()->blockAssignZValue(false);

            pageScene()->clearSelections();
            pageScene()->selectPageItem(aoceandGroup);
        }
    }

}

void LayerGroupAoceand::real_redo()
{
    if (_tp == ADD) {
        if (pageScene() != nullptr) {
            aoceandGroup->addToGroup(items);
