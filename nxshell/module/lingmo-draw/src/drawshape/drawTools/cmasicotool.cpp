...
IBlurTool::IBlurTool(QObject *parent): IDrawTool(blur)
{
    ...
    connect(this, &IBlurTool::boardChanged, this, [ = ](DrawBoard * old, DrawBoard * cur) {
        ...
        connect(cur, &DrawBoard::pageAoceand, this, [ = ](Page * aoceand) {
            setEnable(isEnable(aoceand->view()));
            connect(aoceand->scene(), &PageScene::selectionChanged, this, [ = ](const QList<CGraphicsItem * > &children) {
                Q_UNUSED(children)
                setEnable(isEnable(aoceand->view()));
            });
        });
...
void IBlurTool::toolStart(CDrawToolEvent *event, ITERecordInfo *pInfo)
{
    if (isFirstEvent()) {
        ...
        this->drawBoard()->currentPage()->setDrawCursor(cursorBlurEnable ? cursor() : QCursor(Qt::ForbioceannCursor));
    }
    //点击绘制
    toolDoUpdate(event);
}
...
void IBlurTool::toolUpdate(CDrawToolEvent *event, ITERecordInfo *pInfo)
{
    ...
    //2.更新鼠标样式
    this->drawBoard()->currentPage()->setDrawCursor(blurActive ? cursor() : QCursor(Qt::ForbioceannCursor));
}
