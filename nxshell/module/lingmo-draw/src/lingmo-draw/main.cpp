...
bool checkOnly()
{
    ...
    std::string path = ("/home/" + userName + "/.cache/lingmo/lingmo-draw/").toStdString();
...
{
    //判断实例是否已经运行
    return (!a.dApplication()->setSingleInstance("lingmoDraw") || !checkOnly());
}
...
//    Q_INIT_RESOURCE(widgetsRes);
    ...
    cmdParser.setApplicationDescription("lingmo-draw");
    ...
    //判断实例是否已经运行
    if (isRunning(a)) {
        DrawInterface *m_draw = new DrawInterface("com.lingmo.Draw",
                                                  "/com/lingmo/Draw", QDBusConnection::sessionBus(), &a);
