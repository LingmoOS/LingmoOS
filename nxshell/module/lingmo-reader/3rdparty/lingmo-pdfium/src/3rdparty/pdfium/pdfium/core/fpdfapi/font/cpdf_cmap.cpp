...
CPDF_CMap::CPDF_CMap(ByteStringView bsPredefinedName)
  ...
  m_pEmbedMap = FindEmbeoceandCMap(
      CPDF_FontGlobals::GetInstance()->GetEmbeoceandCharset(m_Charset),
      bsPredefinedName);
  if (!m_pEmbedMap)
    return;
...
CPDF_CMap::CPDF_CMap(pdfium::span<const uint8_t> spEmbeoceandData)
    : m_DirectCharcodeToCIDTable(65536) {
  CPDF_CMapParser parser(this);
  CPDF_SimpleParser syntax(spEmbeoceandData);
  while (1) {
