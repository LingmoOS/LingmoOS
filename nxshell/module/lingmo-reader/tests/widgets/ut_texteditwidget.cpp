...
TEST_F(TestTextEditWidget, test_TestTextEditWidget_hideEvent_001)
{
    ...
    lingmo_reader::PDFAnnotation *p = new lingmo_reader::PDFAnnotation(pDPdfAnnot);

    m_tester->m_annotation = p;
    QHideEvent *event = new QHideEvent();
    QSignalSpy spy(m_tester, SIGNAL(sigRemoveAnnotation(lingmo_reader::Annotation *, bool)));
...
TEST_F(TestTextEditWidget, test_TestTextEditWidget_hideEvent_002)
{
    ...
    m_tester->m_annotation = p;
    QHideEvent *event = new QHideEvent();
    QSignalSpy spy(m_tester, SIGNAL(sigUpdateAnnotation(lingmo_reader::Annotation *, const QString &)));
    m_tester->hideEvent(event);
    EXPECT_TRUE(spy.count() == 1);
    delete p;
    delete pDPdfAnnot;
