...
BrowserAnnotation::BrowserAnnotation(QGraphicsItem *parent, QRectF rect, lingmo_reader::Annotation *annotation, qreal scalefactor) : QGraphicsItem(parent),
    m_annotation(annotation), m_rect(rect), m_parent(parent), m_scaleFactor(scalefactor)
{
    if (nullptr == m_annotation)
        return;

    if (m_annotation->type() == lingmo_reader::Annotation::AText)
        setZValue(lingmo_reader::Z_ORDER_ICON);
    else
        setZValue(lingmo_reader::Z_ORDER_HIGHLIGHT);
}
...
lingmo_reader::Annotation *BrowserAnnotation::annotation()
{
    return m_annotation;
}
