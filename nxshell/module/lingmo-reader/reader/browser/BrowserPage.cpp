...
QList<lingmo_reader::Annotation *> BrowserPage::annotations()
{
    if (!m_hasLoadedAnnotation) {
...
bool BrowserPage::updateAnnotation(lingmo_reader::Annotation *annotation, const QString &text, const QColor &color)
{
...
Annotation *BrowserPage::addHighlightAnnotation(QString text, QColor color)
{
    ...
    const QList<lingmo_reader::Word> &twords = m_sheet->renderer()->getWords(itemIndex());
...
bool BrowserPage::hasAnnotation(lingmo_reader::Annotation *annotation)
{
    ...
    if (iconAnnot) {
        foreach (BrowserAnnotation *annotation, m_annotationItems) {
            if (annotation && annotation->isSame(iconAnnot)) {
                ...
                if (iconAnnot->type() == lingmo_reader::Annotation::AText)
...
bool BrowserPage::removeAllAnnotation()
{
    ...
    for (int index = 0; index < m_annotations.size(); index++) {
        lingmo_reader::Annotation *annota = m_annotations.at(index);
...
bool BrowserPage::removeAnnotation(lingmo_reader::Annotation *annota)
{
    if (nullptr == annota)
        return false;
...
Annotation *BrowserPage::addIconAnnotation(const QRectF &rect, const QString &text)
{
    ...
    if (annot) {
        ...
        if (annot->type() == lingmo_reader::Annotation::AText) {
            if (m_lastClickIconAnnotationItem) {
                m_lastClickIconAnnotationItem->setScaleFactor(m_scaleFactor);
                m_lastClickIconAnnotationItem->setDrawSelectRect(true);
            }
