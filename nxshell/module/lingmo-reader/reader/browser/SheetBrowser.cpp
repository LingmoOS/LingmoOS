...
{
    lingmo_reader::Document *document = nullptr;
    ...
    lingmo_reader::Page *page = document->page(0);
...
void SheetBrowser::showNoteEditWidget(lingmo_reader::Annotation *annotation, const QPoint &point)
{
...
QList<lingmo_reader::Annotation *> SheetBrowser::annotations()
{
    QList<lingmo_reader::Annotation *> list;
...
bool SheetBrowser::removeAnnotation(lingmo_reader::Annotation *annotation)
{
    bool ret = false;
    int pageIndex = -1;
    foreach (BrowserPage *item, m_items) {
...
bool SheetBrowser::updateAnnotation(lingmo_reader::Annotation *annotation, const QString &text, QColor color)
{
...
void SheetBrowser::onRemoveAnnotation(lingmo_reader::Annotation *annotation, bool tips)
{
    m_sheet->removeAnnotation(annotation, tips);
}

void SheetBrowser::onUpdateAnnotation(lingmo_reader::Annotation *annotation, const QString &text)
{
...
void SheetBrowser::jumpToHighLight(lingmo_reader::Annotation *annotation, const int index)
{
...
void SheetBrowser::mousePressEvent(QMouseEvent *event)
{
    ...
    if (!m_startPinch && (QGraphicsView::NoDrag == dragMode() || QGraphicsView::RubberBandDrag == dragMode())) {
        ...
        if (btn == Qt::LeftButton) {
            ...
            lingmo_reader::Annotation *clickAnno = getClickAnnot(page, m_selectPressedPos, true);
            ...
            if (clickAnno && clickAnno->type() == lingmo_reader::Annotation::AText) {
            ...
            connect(&menu, &BrowserMenu::signalMenuItemClicked, [ & ](const QString & objectname) {
                ...
                if (objectname == "Copy") {
                    ...
                    if (annotation)  {
                        ...
                        lingmo_reader::Annotation *addAnnot = nullptr;
            ...
            if (annotation && annotation->annotationType() == lingmo_reader::Annotation::AText) {
                if (m_lastSelectIconAnnotPage)
                    m_lastSelectIconAnnotPage->setDrawMoveIconRect(false);
                //文字注释(图标)
            ...
            } else if (annotation && annotation->annotationType() == lingmo_reader::Annotation::AHighlight) {
                //文字高亮注释
...
void SheetBrowser::mouseReleaseEvent(QMouseEvent *event)
{
    if (!m_startPinch && (QGraphicsView::NoDrag == dragMode() || QGraphicsView::RubberBandDrag == dragMode())) {
        if (event->button() == Qt::LeftButton) {
            ...
            lingmo_reader::Annotation *clickAnno = getClickAnnot(page, mousepoint);
...
void SheetBrowser::handleSearchResultComming(const lingmo_reader::SearchResult &res)
{
    if (res.page <= 0)
        return;
...
bool SheetBrowser::setDocTapGestrue(QPoint mousePos)
{
    ...
    if ((browserAnno && browserAnno->annotationType() == lingmo_reader::Annotation::AText) || magnifierOpened() || this->isLink(mousePos)) {
        return false;
    }
