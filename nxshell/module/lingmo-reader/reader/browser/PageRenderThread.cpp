...
{
    qRegisterMetaType<lingmo_reader::Document *>("lingmo_reader::Document *");
    qRegisterMetaType<QList<lingmo_reader::Word>>("QList<lingmo_reader::Word>");
    qRegisterMetaType<QList<lingmo_reader::Annotation *>>("QList<lingmo_reader::Annotation *>");
    qRegisterMetaType<QList<lingmo_reader::Page *>>("QList<lingmo_reader::Page *>");
    qRegisterMetaType<lingmo_reader::Document::Error>("lingmo_reader::Document::Error");
...
bool PageRenderThread::execNextDocPageAnnotationTask()
{
    ...
    const QList<lingmo_reader::Annotation *> annots = task.sheet->renderer()->getAnnotations(task.page->itemIndex());
...
bool PageRenderThread::execNextDocOpenTask()
{
    ...
    lingmo_reader::Document::Error error = lingmo_reader::Document::NoError;

    qDebug() << "PageRenderThread::execNextDocOpenTask" <<  task.sheet->convertedFileDir();
    lingmo_reader::Document *document = lingmo_reader::DocumentFactory::getDocument(task.sheet->fileType(), filePath, task.sheet->convertedFileDir(), task.password, &(task.sheet->m_process), error);

    if (nullptr == document) {
        emit sigDocOpenTask(task, error, nullptr, QList<lingmo_reader::Page *>());
    } else {
        int pagesNumber = document->pageCount();

        QList<lingmo_reader::Page *> pages;

        for (int i = 0; i < pagesNumber; ++i) {
            lingmo_reader::Page *page = document->page(i);
        ...
        if (pages.count() == pagesNumber) {
            emit sigDocOpenTask(task, lingmo_reader::Document::NoError, document, pages);
        } else {
            qDeleteAll(pages);
            ...
            delete document;
            emit sigDocOpenTask(task, lingmo_reader::Document::FileDamaged, nullptr, QList<lingmo_reader::Page *>());
        }
    }
...
void PageRenderThread::onDocPageWordTaskFinished(DocPageWordTask task, QList<lingmo_reader::Word> words)
{
    if (DocSheet::existSheet(task.sheet)) {
...
void PageRenderThread::onDocPageAnnotationTaskFinished(DocPageAnnotationTask task, QList<lingmo_reader::Annotation *> annots)
{
    if (DocSheet::existSheet(task.sheet)) {
...
void PageRenderThread::onDocOpenTask(DocOpenTask task, lingmo_reader::Document::Error error, lingmo_reader::Document *document, QList<lingmo_reader::Page *> pages)
{
    if (DocSheet::existSheet(task.sheet)) {
