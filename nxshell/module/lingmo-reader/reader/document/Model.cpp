...
namespace lingmo_reader {
lingmo_reader::Document *lingmo_reader::DocumentFactory::getDocument(const int &fileType,
                                                                     const QString &filePath,
                                                                     const QString &convertedFileDir,
                                                                     const QString &password,
                                                                     QProcess **pprocess,
                                                                     lingmo_reader::Document::Error &error)
{
    ...
    if (Dr::PDF == fileType) {
        qDebug() << "当前文档类型为: PDF";
        document = lingmo_reader::PDFDocument::loadDocument(filePath, password, error);
    } else if (Dr::DJVU == fileType) {
        qDebug() << "当前文档类型为: DJVU";
        document = lingmo_reader::DjVuDocument::loadDocument(filePath, error);
    } else if (Dr::DOCX == fileType) {
        qDebug() << "当前文档类型为: DOCX";
        if (nullptr == pprocess) {
            error = lingmo_reader::Document::ConvertFailed;
            return nullptr;
        }
        ...
        *pprocess = &converter2;
        ...
        QString htmltopdfCommand = prefix + "/lib/lingmo-reader/htmltopdf " +  tmpHtmlFilePath + " " + realFilePath;
        ...
        *pprocess = nullptr;
        document = lingmo_reader::PDFDocument::loadDocument(realFilePath, password, error);
    }
