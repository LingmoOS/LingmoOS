...
/**
 * @brief pathControl,判断lingmo-reader是否被禁止访问sPath。
 * @param sPath 要访问的目录
 * @return lingmo-reader是否被禁止访问
 */
static bool pathControl(const QString &sPath) noexcept
{
    ...
    if (reply.type() == QDBusMessage::ReplyMessage) {
        /**
         * lValue：被禁止读取的应用列表，lingmo-reader在列表中返回true
         */
        QList<QString> lValue = reply.arguments().takeFirst().toStringList();
        QString strApp = QStandardPaths::findExecutable("lingmo-reader");
...
//    bool result = QProcess::startDetached(QString("ocean-file-manager \"%1\" --show-item").arg(filePath));
//    if (!result) {
...
void CentralDocPage::onOpened(DocSheet *sheet, lingmo_reader::Document::Error error)
{
    if (lingmo_reader::Document::FileError == error || lingmo_reader::Document::FileDamaged == error || lingmo_reader::Document::ConvertFailed == error) {
        ...
        if (lingmo_reader::Document::FileError == error)
            showTips(nullptr, tr("Open failed"), 1);
        else if (lingmo_reader::Document::FileDamaged == error)
            showTips(nullptr, tr("Please check if the file is damaged"), 1);
        else if (lingmo_reader::Document::ConvertFailed == error)
