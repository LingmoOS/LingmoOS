...
bool SheetRenderer::openFileExec(const QString &password)
{
    ...
    return lingmo_reader::Document::NoError == m_error;
}
...
lingmo_reader::Link SheetRenderer::getLinkAtPoint(int index, const QPointF &point)
{
    if (m_pages.count() <= index)
        return lingmo_reader::Link();

    return  m_pages.value(index)->getLinkAtPoint(point);
}

QList<lingmo_reader::Word> SheetRenderer::getWords(int index)
{
    if (m_pages.count() <= index)
        return QList<lingmo_reader::Word>();

    return m_pages.value(index)->words();
}

QList<lingmo_reader::Annotation *> SheetRenderer::getAnnotations(int index)
{
    if (m_pages.count() <= index)
        return QList<lingmo_reader::Annotation *>();
...
lingmo_reader::Annotation *SheetRenderer::addHighlightAnnotation(int index, const QList<QRectF> &boundaries, const QString &text, const QColor &color)
{
    if (m_pages.count() <= index)
        return nullptr;
...
bool SheetRenderer::removeAnnotation(int index, lingmo_reader::Annotation *annotation)
{
    if (m_pages.count() <= index)
        return false;
...
bool SheetRenderer::updateAnnotation(int index, lingmo_reader::Annotation *annotation, const QString &text, const QColor &color)
{
    if (m_pages.count() <= index)
        return false;
...
lingmo_reader::Annotation *SheetRenderer::addIconAnnotation(int index, const QRectF &rect, const QString &text)
{
    if (m_pages.count() <= index)
        return nullptr;
...
lingmo_reader::Annotation *SheetRenderer::moveIconAnnotation(int index, lingmo_reader::Annotation *annot, const QRectF &rect)
{
    if (m_pages.count() <= index)
        return nullptr;
...
QVector<lingmo_reader::PageSection> SheetRenderer::search(int index, const QString &text, bool matchCase, bool wholeWords)
{
    if (m_pages.count() <= index)
        return QVector<lingmo_reader::PageSection>();
...
bool SheetRenderer::inLink(int index, const QPointF &pos)
{
    ...
    lingmo_reader::Link link = m_pages.value(index)->getLinkAtPoint(pos);
...
lingmo_reader::Outline SheetRenderer::outline()
{
    if (m_document == nullptr)
        return lingmo_reader::Outline();

    return m_document->outline();
}

lingmo_reader::Properties SheetRenderer::properties() const
{
    if (m_document == nullptr)
        return lingmo_reader::Properties();
...
void SheetRenderer::handleOpened(lingmo_reader::Document::Error error, lingmo_reader::Document *document, QList<lingmo_reader::Page *> pages)
{
    m_error = error;
