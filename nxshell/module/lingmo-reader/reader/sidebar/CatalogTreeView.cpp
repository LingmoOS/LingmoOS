...
void CatalogTreeView::parseCatalogData(const lingmo_reader::Section &ol, QStandardItem *parentItem)
{
...
void CatalogTreeView::handleOpenSuccess()
{
    ...
    if (model) {
        ...
        const lingmo_reader::Outline &ol = m_sheet->outline();
        for (const lingmo_reader::Section &s : ol) {   //root
            if (s.nIndex >= 0) {
                auto itemList = getItemList(s.title, s.nIndex, s.offsetPointF.x(), s.offsetPointF.y());
