...
void NotesWidget::addNoteItem(lingmo_reader::Annotation *anno)
{
...
void NotesWidget::deleteNoteItem(lingmo_reader::Annotation *anno)
{
...
void NotesWidget::handleAnntationMsg(const int &msgType, lingmo_reader::Annotation *anno)
{
...
{
    const QList< lingmo_reader::Annotation * > &annotationlst = m_sheet->annotations();
    QList<ImagePageInfo_t> pageSrclst;
    int pagesNum = annotationlst.size();
    for (int index = 0; index < pagesNum; index++) {
        lingmo_reader::Annotation *annotion = annotationlst.at(index);
