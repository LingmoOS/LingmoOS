...
make -j8
#生成asan日志和ut测试xml结果
./tests/lingmo-voice-note-test --gtest_output=xml:./report/report_lingmo-voice-note.xml
...
mv ./html/index.html ./html/cov_lingmo-voice-note.html
#对asan、ut、代码覆盖率结果收集至指定文件夹
cp -r html ../$reportdir/
cp -r report ../$reportdir/
cp -r asan*.log* ../$reportdir/asan_lingmo-voice-note.log
