...
{
    // 初始化字体服务Dus接口
    m_appearanceDBusInterface = new QDBusInterface(LINGMO_DAEMON_APPEARANCE_SERVICE,
                                                   LINGMO_DAEMON_APPEARANCE_PATH,
                                                   LINGMO_DAEMON_APPEARANCE_INTERFACE,
                                                   QDBusConnection::sessionBus());
    if (m_appearanceDBusInterface->isValid()) {
        qInfo() << "字体服务初始化成功！字体服务： " << LINGMO_DAEMON_APPEARANCE_SERVICE << " 地址：" << LINGMO_DAEMON_APPEARANCE_PATH << " 接口：" << LINGMO_DAEMON_APPEARANCE_INTERFACE;
        //获取默认字体
        ...
        qWarning() << "初始化失败！字体服务 (" << LINGMO_DAEMON_APPEARANCE_SERVICE << ") 不存在";
    }
}
