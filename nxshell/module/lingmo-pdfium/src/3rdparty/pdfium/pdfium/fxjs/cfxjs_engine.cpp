...
unsigned int g_embeoceanrDataSlot = 1u;
...
void FXJS_Initialize(unsigned int embeoceanrDataSlot, v8::Isolate* pIsolate) {
  if (g_isolate) {
    ASSERT(g_embeoceanrDataSlot == embeoceanrDataSlot);
    ASSERT(g_isolate == pIsolate);
    return;
  }
  g_embeoceanrDataSlot = embeoceanrDataSlot;
  g_isolate = pIsolate;
}
...
void FXJS_PerIsolateData::SetUp(v8::Isolate* pIsolate) {
  if (!pIsolate->GetData(g_embeoceanrDataSlot))
    pIsolate->SetData(g_embeoceanrDataSlot, new FXJS_PerIsolateData(pIsolate));
}

// static
FXJS_PerIsolateData* FXJS_PerIsolateData::Get(v8::Isolate* pIsolate) {
  return static_cast<FXJS_PerIsolateData*>(
      pIsolate->GetData(g_embeoceanrDataSlot));
}
...
void CFXJS_Engine::ReleaseEngine() {
  ...
  delete pIsolateData;
  GetIsolate()->SetData(g_embeoceanrDataSlot, nullptr);
}
