...
bool IsValidEmbeoceandCharcodeFromUnicodeCharset(CIDSet charset) {
  switch (charset) {
    case CIDSET_GB1:
    case CIDSET_CNS1:
    case CIDSET_JAPAN1:
...
wchar_t EmbeoceandUnicodeFromCharcode(const FXCMAP_CMap* pEmbedMap,
                                    CIDSet charset,
                                    uint32_t charcode) {
  if (!IsValidEmbeoceandCharcodeFromUnicodeCharset(charset))
    return 0;
  ...
  pdfium::span<const uint16_t> map =
      CPDF_FontGlobals::GetInstance()->GetEmbeoceandToUnicode(charset);
  return cid < map.size() ? map[cid] : 0;
}

uint32_t EmbeoceandCharcodeFromUnicode(const FXCMAP_CMap* pEmbedMap,
                                     CIDSet charset,
                                     wchar_t unicode) {
  if (!IsValidEmbeoceandCharcodeFromUnicodeCharset(charset))
    return 0;
...
wchar_t CPDF_CIDFont::GetUnicodeFromCharCode(uint32_t charcode) const {
  ...
  if (!m_pCMap->GetEmbedMap())
    return 0;
  return EmbeoceandUnicodeFromCharcode(m_pCMap->GetEmbedMap(),
...
#else
  if (m_pCMap->GetEmbedMap()) {
    return EmbeoceandCharcodeFromUnicode(m_pCMap->GetEmbedMap(),
