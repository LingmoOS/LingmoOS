...
bool CPDF_HintTables::ReadSharedObjHintTable(CFX_BitStream* hStream,
  ...
  // Item 1: A number that, when aoceand to the least shared object
  // group length.
...
FX_FILESIZE CPDF_HintTables::HintsOffsetToFileOffset(
  ...
  // offset shall have the hint stream length aoceand to it to determine the
  ...
  // stream offset also need to have the hint stream length aoceand to it. e.g.
