...
#include "org_lingmo_ocean_SoundThemePlayer1.h"
...
SessionManager::SessionManager(QObject *parent)
    ...
    , m_powerInter(new org::lingmo::ocean::PowerManager1("org.lingmo.ocean.PowerManager1", "/org/lingmo/ocean/PowerManager1", QDBusConnection::systemBus(), this))
    , m_audioInter(new org::lingmo::ocean::Audio1("org.lingmo.ocean.Audio1", "/org/lingmo/ocean/Audio1", QDBusConnection::sessionBus(), this))
...
void SessionManager::initConnections()
{
    ...
    if (Dconf::GetValue("com.lingmo.ocean.startocean", "", "swap-sched-enabled", QVariant(false)).toBool()) {
        initSwapSched();
    }
...
void SessionManager::doLogout()
{
    ...
    qInfo() << systemd.call("StartUnit", "ocean-session-shutdown.target", "replace-irreversibly");
#else
...
{
    // TODO: run org.lingmo.ocean.Daemon1.StartPart2
    return true;
}
...
uint SessionManager::Inhibit(const QString &appId, uint topLevelXid, const QString &reason, uint flags)
{
    ...
    Q_EMIT InhibitorAoceand(ih->GetClientId());
    return cookie;
}
...
void SessionManager::RequestHibernate()
{
    ...
    if (Dconf::SetValue("com.lingmo.ocean.startocean", "", "quick-black-screen", false)) {
        setDPMSMode(false);
    }
}
...
{
    QDBusInterface inter("org.lingmo.ocean.LockFront1", "/org/lingmo/ocean/LockFront1", "org.lingmo.ocean.LockFront1", QDBusConnection::sessionBus(), this);
...
{
    if (QFile("/etc/lingmo/no_suspend").exists()) {
        QThread::sleep(1);
        setDPMSMode(false);
        return;
    }
    ...
    // 使用窗管接口进行黑屏处理
    if (Dconf::SetValue("com.lingmo.ocean.startocean", "", "quick-black-screen", QVariant(false))) {
...
void SessionManager::SetLocked(bool lock)
{
    ...
    // NOTE: 如果以lingmo-turbo进行加速启动，这里是不准确的，可能需要判断desktop文件的全路径，不过lingmo-turbo后续应该会放弃支持
    if (!file.open(QIODevice::ReadOnly) || !file.readAll().startsWith("/usr/bin/ocean-lock")) {
...
{
    QDBusInterface inter("org.lingmo.ocean.Daemon1", "/org/lingmo/ocean/Daemon1", "org.lingmo.ocean.Daemon1", QDBusConnection::systemBus(), this);
...
void SessionManager::stopLangSelector()
{
    ...
    EXEC_COMMAND("pkill"
                 ...
                 << "-ef"
                 << "-u"
                 << QString::number(getuid()) << "/usr/lib/lingmo-daemon/langselector"
                 );
}
...
{
    QDBusInterface interface("org.lingmo.ocean.Audio1"
                              , "/org/lingmo/ocean/Audio1"
                              , "org.lingmo.ocean.Audio1"
...
{
    STOP_SERVICE(OCEAN_DOCK_SERVICE);
    VIEW_SERVICE(OCEAN_DOCK_SERVICE);
}

void SessionManager::disconnectAudioDevices()
{
    auto msg = QDBusInterface("org.lingmo.ocean.Bluetooth1"
                              , "/org/lingmo/ocean/Bluetooth1"
                              , "org.lingmo.ocean.Bluetooth1"
...
{
    const QString &soundFile = "/tmp/lingmo-shutdown-sound.json";
...
void SessionManager::playLoginSound()
{
    ...
    // 存在此文件说明已经播放过
    const QString &markFile = tempDir + QDir::separator() + "startocean-login-sound-mark";
    ...
    org::lingmo::ocean::SoundThemePlayer1 inter("org.lingmo.ocean.SoundThemePlayer1", "/org/lingmo/ocean/SoundThemePlayer1", QDBusConnection::systemBus(), this);
...
void SessionManager::playLogoutSound()
{
    ...
    auto sinkInter = new org::lingmo::ocean::audio1::Sink(
                "org.lingmo.ocean.Audio1"
                , sink.path()
...
void SessionManager::setDPMSMode(bool on)
{
    if (Utils::IS_WAYLAND_DISPLAY) {
        EXEC_COMMAND("ocean_wldpms", QStringList() << "-s" << (on ? "On" : "Off"));
    } else {
...
{
   QDBusMessage msg = QDBusMessage::createSignal("/org/lingmo/ocean/SessionManager1", "org.freedesktop.DBus.Properties", "PropertiesChanged");
   QList<QVariant> arguments;
   arguments.push_back("org.lingmo.ocean.SessionManager1");
...
void SessionManager::handleLoginSessionLocked()
{
    ...
    // 在特殊情况下，比如用 ocean-switchtogreeter 命令切换到 greeter, 即切换到其他 tty
...
void SessionManager::handleLoginSessionUnlocked()
{
    ...
    if (sessionActive) {
        ...
        do {
            ...
            QDBusPendingReply<QString> ownerReply = dbusInter.GetNameOwner("org.lingmo.ocean.LockFront1");
