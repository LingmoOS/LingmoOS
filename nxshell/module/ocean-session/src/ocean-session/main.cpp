...
int main(int argc, char *argv[])
{
    ...
    app.setApplicationName("ocean-session");

    QCommandLineParser parser;
    parser.setApplicationDescription("ocean-session");
    ...
    if (!parser.isSet(systemd)) {
        ...
        QString dmService = "ocean-session.target";
        ...
        QDBusServiceWatcher *watcher = new QDBusServiceWatcher("org.lingmo.ocean.Session1", QDBusConnection::sessionBus(), QDBusServiceWatcher::WatchForUnregistration);
        watcher->connect(watcher, &QDBusServiceWatcher::serviceUnregistered, [&] {
            qInfo() << "ocean session exit";
            startSystemdUnit(systemdDBus, "ocean-session-exit-task.service", "replace");
            qApp->quit();
        });
        pid_t curPid = getpid();
    ...
    QDBusConnection::sessionBus().registerService("org.lingmo.ocean.Session1");
    QDBusConnection::sessionBus().registerObject("/org/lingmo/ocean/Session1", "org.lingmo.ocean.Session1", session);

    SessionManager::instance()->init();
    new SessionManager1Adaptor(SessionManager::instance());
    QDBusConnection::sessionBus().registerService("org.lingmo.ocean.SessionManager1");
    QDBusConnection::sessionBus().registerObject("/org/lingmo/ocean/SessionManager1", "org.lingmo.ocean.SessionManager1", SessionManager::instance());

    auto *wmSwitcher = new WMSwitcher();
    new WMSwitcher1Adaptor(wmSwitcher);
    QDBusConnection::sessionBus().registerService("org.lingmo.ocean.WMSwitcher1");
    QDBusConnection::sessionBus().registerObject("/org/lingmo/ocean/WMSwitcher1", wmSwitcher);
    ...
    // cpu iowait状态检测
    if (Utils::SettingValue("com.lingmo.ocean.startocean", QByteArray(), "iowait-enabled", false).toBool()) {
        QtConcurrent::run([] {
    ...
    QtConcurrent::run([&session](){
        ...
        while ((len = fifo->Read(CurSessionPid)) > 0) {
            ...
            qInfo() << "ocean-session pid: " << CurSessionPid;
            if (ok && pid > 0) {
