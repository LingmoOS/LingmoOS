...
void OthersManager::launchWmChooser()
{
    const QString &config = QStandardPaths::standardLocations(QStandardPaths::ConfigLocation).first()
            + "/lingmo/lingmo-wm-switcher/config.json";
    if (!Utils::IS_WAYLAND_DISPLAY && inVM()) {
        if (!QFile(config).exists())
            return;

        EXEC_COMMAND("ocean-wm-chooser", QStringList() << "-c" << config);
    }

    // kwin存在的情况
    if (!QStandardPaths::findExecutable("lingmo-kwin_x11").isEmpty() && QFile(config).exists()) {
        ...
        bool compositingEnabled = (lastWm == "lingmo-wm");

        // 更新kwinrc的配置文件
        const QString &kwinRc = QStandardPaths::standardLocations(QStandardPaths::ConfigLocation).first() + "/lingmo-kwinrc";
