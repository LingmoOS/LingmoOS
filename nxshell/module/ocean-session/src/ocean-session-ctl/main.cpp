...
int main(int argc, char *argv[])
{
    ...
    parser.setApplicationDescription("ocean-session-ctl");
    parser.addHelpOption();
    parser.addVersionOption();

    QCommandLineOption shutdown(QStringList{ "s", "shutdown", "shutdown ocean"});
    ...
    if (parser.isSet(logout)) {
        org::lingmo::ocean::Session1 session("org.lingmo.ocean.Session1", "/org/lingmo/ocean/Session1", QDBusConnection::sessionBus());
        session.Logout();
    ...
    if (isShutdown) {
        // kill startocean-session or call login1
