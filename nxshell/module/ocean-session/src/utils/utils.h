...
#define OCEAN_DOCK_SERVICE "ocean-dock.service"

// sound
#define SOUND_EFFECT_SCHEMA "com.lingmo.ocean.sound-effect"
#define SOUND_EFFECT_ENABLED_KEY "enabled"
#define SOUND_EFFECT_PLAYER_KEY "player"

// appearance
#define APPEARANCE_SCHEMA "com.lingmo.ocean.appearance"
...
/**
 * @brief SettingsPtr 根据给定信息返回一个QGSettings指针
 * @param module 传入QGSettings构造函数时，会添加"com.lingmo.ocean.dock.module."前缀
 * @param path If non-empty, specifies the path for a relocatable schema
 * @param parent 创建指针的付对象
 * @return
 */
...
{
    return SettingsPtr("com.lingmo.ocean.dock.module." + module, path, parent);
}
 ...
 * @example Utils::SettingValue("com.lingmo.ocean.startocean", QByteArray(), "quick-black-screen", false).toBool()
 */
