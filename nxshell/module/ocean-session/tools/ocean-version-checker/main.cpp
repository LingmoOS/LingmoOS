...
bool isLingmoVersionChanged()
{
    ...
    const QString &welcomePath = QStandardPaths::standardLocations(QStandardPaths::ConfigLocation).first()
            ...
            + "lingmo"
            + QDir::separator()
            + "ocean-version-checker"
            + QDir::separator();
    const QString &welcomeConf = "ocean-welcome.conf";
    ...
    // ocean-welcome.conf中不存在version时，为第一次进入系统或进入当前用户，不显示此界面
...
void maybeOpenWelcome()
{
    ...
    QProcess::startDetached("/usr/lib/lingmo-daemon/ocean-welcome");
}
...
int main(int argc, char *argv[])
{
    ...
    qDebug() << "lingmo version checker init";
    maybeOpenWelcome();
    qDebug() << "lingmo version checker init finished";
