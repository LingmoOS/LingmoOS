...
int start()
{
    ...
    if (!Utils::SettingValue("com.lingmo.ocean.startocean", QByteArray(), "login-reminder", false).toBool())
        return 0;

    // 登录后展示横幅通知信息
    QDBusInterface userInter("org.lingmo.ocean.Accounts1", QString("/org/lingmo/ocean/Accounts1/User%1").arg(getuid()), "org.lingmo.ocean.Accounts1.User", QDBusConnection::systemBus());
...
#define SECONDS_PER_DAY (60 * 60 * 24)
    ...
    QVariantMap hints;
    hints.insert(QString("x-lingmo-action-details"),QVariant(QString("/usr/bin/ocean-hints-dialog, %1, %2").arg(title).arg(content)));
    QDBusPendingReply<uint> notifyIdReply = notifyInter->call("Notify", "ocean-control-center", uint(0), "preferences-system", title, body, QStringList() << "details" << QObject::tr("Details"), hints, int(0));
