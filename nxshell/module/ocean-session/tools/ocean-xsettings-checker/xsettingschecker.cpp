...
XSettingsChecker::XSettingsChecker(QObject *parent)
    : QObject(parent) // TODO XSettings1服务当前仍处于startocean中，后期可能会独立出来
    , m_xSettingsInter(new QDBusInterface("org.lingmo.ocean.XSettings1", "/org/lingmo/ocean/XSettings1", "org.lingmo.ocean.XSettings1", QDBusConnection::sessionBus(), this))
{
...
void XSettingsChecker::initQtTheme()
{
    ...
    if (iconTheme.isEmpty()) {
        iconTheme = Utils::SettingValue("com.lingmo.ocean.appearance", QByteArray(), "icon-theme", QString()).toString();
    ...
    if (fontSize - 0 < (1e-6)) {
        fontSize = Utils::SettingValue("com.lingmo.ocean.appearance", QByteArray(), "font-size", 0.0).toDouble();
