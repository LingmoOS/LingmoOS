...
char *smart_cat(const char *dest, const char c, const char *str1)
{
	...
	if(my_cat == NULL)
	{
        fprintf(stderr,"lingmo-camera: FATAL memory allocation failure (smart_cat): %s\n", strerror(errno));
		exit(-1);
	}
	char *my_p = my_cat;
	...
	if(debug_level > 1)
        printf("lingmo-camera: (smart_cat) dest=%s(%i) len_c=%c(%i) len_str1=%s(%i) => %s\n",
...
char *get_file_basename(const char *filename)
{
	...
	if(debug_level > 1)
        printf("lingmo-camera: basename for %s is %s\n", filename, basename);
...
char *get_file_pathname(const char *filename)
{
	...
	if(debug_level > 1)
        printf("lingmo-camera: path for %s is %s\n", filename, pathname);
...
char *get_file_extension(const char *filename)
{
	...
	if(debug_level > 1)
        printf("lingmo-camera: extension for %s is %s\n", filename, extname);
...
char *set_file_extension(const char *filename, const char *ext)
{
	...
	if(debug_level > 1)
        printf("lingmo-camera: changed file extension to %s\n", new_filename);
	return new_filename;
}
...
unsigned long long get_file_suffix(const char *path, const char* filename)
{
	...
	if(dirp == NULL)
	{
        fprintf(stderr, "lingmo-camera: Error Couldn't open %s directory\n", path);
		return suffix;
	}
	size = dirent_buf_size(dirp);
	if(size == 0)
	{
        perror("lingmo-camera: dirent_buf_size returned 0");
		closedir(dirp);
		return suffix;
	}
        ...
        while (dirp)
	{
            ...
            {
		if(debug_level > 3)
            printf("lingmo-camera: (get_file_suffix) checking %s\n", ent->d_name);
		if (strncmp(ent->d_name, noextname, noextsize) == 0)
		{
			if(debug_level > 3)
                printf("lingmo-camera: (get_file_suffix) prefix matched (%s)\n", noextname);
				...
				(extension == NULL && ext == NULL))
			{
				...
				if(debug_level > 3)
                    printf("lingmo-camera: (get_file_suffix) matched with suffix %s\n", sfixstr);
            ...
            {
                if(errno)
                    fprintf(stderr,"lingmo-camera: error while reading dir: %s\n", strerror(errno));
                ...
                if(debug_level > 1)
                    printf("lingmo-camera: (get_file_suffix) %s has sufix %llu\n", filename, suffix);
                return suffix;
            }
        }

        fprintf(stderr,"lingmo-camera: error while reading dir: null DIR pointer while readind entries\n");
...
char *add_file_suffix(const char *path, const char *filename)
{
	...
	if(new_name == NULL)
	{
        fprintf(stderr,"lingmo-camera: FATAL memory allocation failure (add_file_suffix): %s\n", strerror(errno));
		exit(-1);
	}
