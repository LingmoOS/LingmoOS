...
copy_test_files() {
     cp -r /data/source/lingmo-camera/jpegtest/*.jpg ~/Pictures/相机/
     cp -r /data/source/lingmo-camera/jpegtest/*.jpg ~/Pictures/Camera/
     cp -r /data/source/lingmo-camera/webmtest/*.webm ~/Videos/相机/
     cp -r /data/source/lingmo-camera/webmtest/*.webm ~/Videos/Camera/
}
...
create_coverage_report(){
     ...
     PROJECT_NAME=lingmo-camera-test #可执行程序的文件名
     PROJECT_REALNAME=lingmo-camera  #项目名称

     echo " ===================CREAT LCOV REPROT==================== "
     lcov --directory ./tests/CMakeFiles/${PROJECT_REALNAME}.dir --zerocounters
     ASAN_OPTIONS="fast_unwind_on_malloc=1" ./tests/${PROJECT_NAME} --gtest_output=xml:./report/report_lingmo-camera.xml
     lcov --directory . --capture --output-file ./html/${PROJECT_REALNAME}_Coverage.info
     echo " =================== do filter begin ==================== "
     lcov --remove ./html/${PROJECT_REALNAME}_Coverage.info 'CMakeFiles/${PROJECT_NAME}.dir/lingmo-camera-test_autogen/*/*' '${PROJECT_NAME}_autogen/*/*' 'googletest/*/*' '*/usr/include/*' '*/tests/*' '*/src/src/qtsingleapplication/*' '*/src/src/basepub/printoptionspage.cpp' '*/src/src/dbus_adpator.cpp' '*/src/src/settings_translation.cpp' '/usr/local/*' '*/config.h' '*/src/src/thumbnailsbar.*' '*/src/src/thumbwidget.*' -o ./html/${PROJECT_REALNAME}_Coverage_fileter.info
     echo " =================== do filter end ==================== "
