...
int main(int argc, char *argv[])
{
    ...
    if (bWayland) {
        ...
        DConfig *dconfig = DConfig::create("org.lingmo.camera", "org.lingmo.camera.encode");
    ...
    qApp->setObjectName("lingmo-camera");
#ifndef __mips__
    ...
    qApp->setAttribute(Qt::AA_UseHighDpiPixmaps);
    qApp->setOrganizationName("lingmo");
    //设置应用名称
    qApp->setApplicationName("lingmo-camera");
    //加载翻译
    ...
    QIcon myIcon = QIcon::fromTheme("lingmo-camera");
    ...
    if (!qApp->setSingleInstance("lingmo-camera")) {
        qDebug() << "another lingmo camera instance has started";
        QDBusInterface iface("com.lingmo.camera", QDir::separator(), "com.lingmo.camera");
        if (iface.isValid()) {
            qDebug() << "lingmo-camera raise";
    ...
    QDBusConnection::sessionBus().registerService("com.lingmo.camera");
