...
{
    char *config_path = smart_cat(getenv("HOME"), '/', ".config/lingmo/lingmo-camera");
    mkdir(config_path, 0777);
    char *config_file = smart_cat(config_path, '/', "lingmo-camera");
    /*释放字符串内存*/
    free(config_path);
    ...
    {
        free(config_file);
        fprintf(stderr, "lingmo-camera: couldn't open %s for read: %s\n", config_path, strerror(errno));
        return -1;
    }
    while(fgets(bufr, MAXLINE, fp) != NULL)
    {
        ...
        {
            fprintf(stderr, "lingmo-camera: (config) skiping invalid config entry at line %i\n", line);
            if(token)
                free(token);
            if(value)
                free(value);
...
{
    QString config_file = QString(getenv("HOME")) + QDir::separator() + QString(".config") + QDir::separator() + QString("lingmo") +
                          QDir::separator() + QString("lingmo-camera") + QDir::separator() + QString("lingmo-camera");
    ...
    {
        fprintf(stderr, "lingmo-camera: couldn't open %s for write: %s\n", filename, strerror(errno));
        free(filename);
        return -1;
    }
