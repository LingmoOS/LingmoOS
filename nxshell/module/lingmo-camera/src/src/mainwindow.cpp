...
#include "shortcut.h"
#include "ac-lingmo-camera-define.h"
#include "imageitem.h"
...
#ifndef UNITTEST
        QProcess::startDetached("lingmo-shortcut-viewer", shortcutString);
#endif
    });
...
void CMainWindow::loadAfterShow()
{
    ...
    QString strCache = QString(getenv("HOME")) + QDir::separator() + QString(".cache")
            + QDir::separator() + QString("lingmo") + QDir::separator()
            + QString("lingmo-camera") + QDir::separator();
    QDir dir;
    dir.mkpath(strCache);
    initRightButtons();
    initTabOrder();
...
{
    m_pTitlebar->titlebar()->setIcon(QIcon::fromTheme("lingmo-camera"));
    m_pTitlebar->raise();
...
void CMainWindow::initConnection()
{
    ...
    QDBusConnection::sessionBus().connect("com.lingmo.SessionManager", "/com/lingmo/SessionManager",
...
#ifdef OS_BUILD_V23
    QDBusConnection::sessionBus().connect("org.lingmo.daemon.Timedate1", "/org/lingmo/daemon/Timedate1",
                                          "org.lingmo.daemon.Timedate1", "TimeUpdate", this,
                                          SLOT(onLocalTimeChanged()));
#else
    QDBusConnection::sessionBus().connect("com.lingmo.daemon.Timedate", "/com/lingmo/daemon/Timedate",
                                          "com.lingmo.daemon.Timedate", "TimeUpdate", this,
                                          SLOT(onLocalTimeChanged()));
#endif
