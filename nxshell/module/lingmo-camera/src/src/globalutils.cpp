...
// 相机配置文件
const QString CAMERA_CONF_PATH = "/usr/share/lingmo-camera/camera.conf";
...
bool GlobalUtils::isLowPerformanceBoard()
{
    ...
    QDBusInterface *monitorInterface = new QDBusInterface("com.lingmo.system.SystemInfo", "/com/lingmo/system/SystemInfo", "org.freedesktop.DBus.Properties", QDBusConnection::systemBus());
    if (!monitorInterface->isValid())
        return false;

    DMIInfo dmiInfo;
    QDBusReply<QDBusVariant> replay = monitorInterface->call("Get", "com.lingmo.system.SystemInfo", "DMIInfo");
