...
{
    QFileInfo fileInfo(m_path);
    QString program("ocean-file-manager");  // dbus调用失败，通过文管打开
    ...
    if (osMajor >= kMinOsEdition) {
        ...
        if (fileInfo.suffix() == "jpg") {
            ...
            QDBusMessage message =
                QDBusMessage::createMethodCall("com.lingmo.imageViewer", "/", "com.lingmo.imageViewer", "openImageFile");
            message << m_path;
            QDBusMessage retMessage = QDBusConnection::sessionBus().call(message);

            if (retMessage.type() != QDBusMessage::ErrorMessage) {
                ret = true;
                qDebug() << "[dbus] Open it with lingmo-image-viewer";
            } else {
            ...
            arguments << "run"
                      ...
                      << "--"
                      << "lingmo-movie" << m_path;
            qDebug() << "[process] Open it with lingmo-movie (ll-cli)";
            ...
            if (retMessage.type() != QDBusMessage::ErrorMessage) {
                ret = true;
                qDebug() << "[dbus] Open it with lingmo-movie";
            } else {
                qWarning() << retMessage.errorMessage();
            }
#endif
    ...
    if (!ret) {
        if (fileInfo.suffix() == "jpg") {
            program = "lingmo-image-viewer";  //用看图打开
            arguments << m_path;
            qDebug() << "[process] Open it with lingmo-image-viewer";
        } else {
            program = "lingmo-movie"; //用影院打开
            arguments << m_path;
            qDebug() << "[process] Open it with lingmo-movie";
        }
        ret = myProcess->startDetached(program, arguments);
    }
