    ...
    , m_appsListModel(new AppsListModel(this))
{
    ...
    auto addAppItem = [=] (const QString &appId) {
        ...
        m_appsSourceModel->appAoceand(item);
    };

    for (const auto &app : m_setting->apps()) {
        addAppItem(app);
    }
    connect(m_setting, &NotificationSetting::appAoceand, this, [=] (const QString &appId) {
        addAppItem(appId);
    });
...
void NotificationModel::appAoceand(AppItemModel *item)
{
    m_appsSourceModel->appAoceand(item);
}
