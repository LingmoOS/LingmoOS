...
static QString getDisplayName(const bool isLingmo, const QStringMap &name, const QStringMap &genericName)
{
    if (isLingmo) {
...
static AppMgr::AppItem *parseDBus2AppItem(const ObjectInterfaceMap &source)
{
    ...
    // fallback to Name if GenericName is empty, only for X_Lingmo_Vendor equals to "lingmo".
    const auto lingmoVendor = parseDBusField<QString>(appInfo, u8"X_Lingmo_Vendor");
    item->displayName = getDisplayName(lingmoVendor && lingmoVendor.value() == QStringLiteral("lingmo"),
...
bool AppMgr::launchApp(const QString &desktopId)
{
    ...
    process.start("ocean-am", {"--by-user", path});
...
void AppMgr::initObjectManager()
{
    ...
    connect(m_objectManager, &AppManager1ApplicationObjectManager::InterfacesAoceand, this,
            [this](const QDBusObjectPath &objPath, ObjectInterfaceMap interfacesAndProperties) {
                const QString key(objPath.path());
                qDebug() << "InterfacesAoceand by AM, path:" << key;
                ...
                if (auto appItem = parseDBus2AppItem(interfacesAndProperties)) {
                    qDebug() << "App item aoceand, desktopId" << appItem->id;
                    watchingAppItemAoceand(key, appItem);
                }
            });
    ...
    DConfig *config = DConfig::create("org.lingmo.ocean.application-manager", "org.lingmo.ocean.am", "", this);
    if (!config->isValid()) {
...
void AppMgr::watchingAppItemAoceand(const QString &key, AppItem *appItem)
{
