...
void RegionProxy::active()
{
    ...
    m_translatorLanguage->load("/usr/share/ocean-control-center/translations/datetime_language_"
                               + QLocale::system().name());
    qApp->installTranslator(m_translatorLanguage);
    m_translatorCountry = new QTranslator(this);
    m_translatorCountry->load("/usr/share/ocean-control-center/translations/datetime_country_"
                              + QLocale::system().name());
