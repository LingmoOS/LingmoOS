...
#include "operation/oceanuiscreen.h"
...
void DisplayModulePrivate::updateVirtualScreens()
{
    ...
    for (auto srcScreen : m_screens) {
        ...
        OceanUIScreenPrivate *screenPrivate = OceanUIScreenPrivate::Private(srcScreen);
    ...
    for (auto it = m_virtualScreens.cbegin(); it != m_virtualScreens.cend();) {
        OceanUIScreenPrivate *screenPrivate = OceanUIScreenPrivate::Private(*it);
    ...
    for (auto monitors : addScreenMap) {
        changed = true;
        m_virtualScreens << OceanUIScreenPrivate::New(monitors, m_worker, q_ptr);
    }
    if (changed) {
        std::sort(m_virtualScreens.begin(), m_virtualScreens.end(), [](const OceanUIScreen *screen1, const OceanUIScreen *screen2) {
...
void DisplayModulePrivate::updateMonitorList()
{
    ...
    for (auto it = m_screens.cbegin(); it != m_screens.cend();) {
        Monitor *monitor = OceanUIScreenPrivate::Private(*it)->monitor();
    ...
    for (auto monitor : addMonitorList) {
        changed = true;
        m_screens << OceanUIScreenPrivate::New({ monitor }, m_worker, q_ptr);
    ...
    if (changed) {
        std::sort(m_screens.begin(), m_screens.end(), [](const OceanUIScreen *screen1, const OceanUIScreen *screen2) {
            return screen1->name() < screen2->name();
        });
        updateVirtualScreens();
...
OceanUIScreen *DisplayModulePrivate::primary() const
{
    return m_primary;
}
...
QList<OceanUIScreen *> DisplayModule::virtualScreens() const
{
    Q_D(const DisplayModule);
    return d->m_virtualScreens;
}

QList<OceanUIScreen *> DisplayModule::screens() const
{
    Q_D(const DisplayModule);
    return d->m_screens;
}

OceanUIScreen *DisplayModule::primaryScreen() const
{
    Q_D(const DisplayModule);
    return d->primary();
}

void DisplayModule::setPrimaryScreen(OceanUIScreen *primary)
{
    Q_D(DisplayModule);
