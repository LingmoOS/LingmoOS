...
Q_LOGGING_CATEGORY(DdcDisplayWorker, "oceanui-display-worker")


const QString DisplayInterface("org.lingmo.ocean.Display1");
...
DisplayWorker::DisplayWorker(DisplayModel *model, QObject *parent, bool isSync)
    ...
    , m_dconfig(DTK_CORE_NAMESPACE::DConfig::create("org.lingmo.ocean.control-center", QStringLiteral("org.lingmo.ocean.control-center.display"), QString(), this))
{
...
void DisplayWorker::active()
{
    if (WQt::Utils::isTreeland()) {
        ...
        //    m_model->setResolutionRefreshEnable(m_oceanuiSettings->get(GSETTINGS_SHOW_MUTILSCREEN).toBool());
        //    m_model->setBrightnessEnable(m_oceanuiSettings->get(GSETTINGS_BRIGHTNESS_ENABLE).toBool());
    }
}
...
constexpr static int wlRotate2oceanui(int wlRotate) {
    switch (wlRotate) {
    ...
    case WL_OUTPUT_TRANSFORM_270:
        return 8;
    default:
        qWarning("oceanui dont support FLIPPED");
        return 0;
    }
}

constexpr static int oceanuiRotate2wl(int oceanuiRotate) {
    switch (oceanuiRotate) {
    case 1:
    ...
    default:
        qWarning("unkone oceanuiRotate, feedback to normal");
        return WL_OUTPUT_TRANSFORM_NORMAL;
    }
}
    ...
    if (WQt::Utils::isTreeland()) {
        ...
        for (auto it(m_wl_monitors.cbegin()); it != m_wl_monitors.cend(); ++it) {
            ...
            if (m_model->displayMode() == MERGE_MODE || it.key() == mon) {
                cfgHead->setTransform(oceanuiRotate2wl(rotate));
            }
        }
        opCfg->apply();
    } else {
...
void DisplayWorker::monitorAoceand(const QString &path)
{
...
void DisplayWorker::wlMonitorAoceand(WQt::OutputHead *head)
{
    Monitor *mon = new Monitor(this);
    ...
    connect(head, &WQt::OutputHead::changed, mon, [mon, head](WQt::OutputHead::Property type) {
        ...
        case WQt::OutputHead::Transform:
            mon->setRotate(wlRotate2oceanui(head->property(WQt::OutputHead::Transform).toInt()));
            break;
