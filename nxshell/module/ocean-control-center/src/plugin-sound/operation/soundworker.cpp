...
Q_LOGGING_CATEGORY(DdcSoundWorker, "oceanui-sound-worker")
...
{
    QString path = QString("qrc:/icons/lingmo/builtin/icons/oceanui_volume%1.svg").arg(m_playAniIconIndex);
...
void SoundWorker::onSoundPlayingChanged()
{
    ...
    QString path("");
    if (m_sound->isPlaying()) {
        path = QString("qrc:/icons/lingmo/builtin/icons/sound_preview_%1.svg").arg(m_playAniIconIndex);
        m_playAniIconIndex++;
