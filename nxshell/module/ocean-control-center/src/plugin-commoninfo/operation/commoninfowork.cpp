...
static QString getUserExpContent()
{
    ...
    if (false == newfile.exists()) {
        userExpContent = getLicensePath("/usr/share/lingmo-lingmoid-client/privacy/User-Experience-Program-License-Agreement/User-Experience-Program-License-Agreement-CN-%1.md", "");
        QFile file(userExpContent);
        if (false == file.exists()) {
            userExpContent = getLicensePath("/usr/share/lingmo-lingmoid-client/privacy/User-Experience-Program-License-Agreement-%1.md", "");
        }
    }
    return userExpContent;
}
...
static void notifyInfo(const QString &summary)
{
    DUtil::DNotifySender(summary)
            ...
            .appName(QObject::tr("ocean-control-center"))
            .timeOut(5000)
            .call();
}
                                 ...
                                 QDBusConnection::systemBus(), this))
{
    ...
    //监听开发者在线认证失败的错误接口信息
    connect(m_commonInfoProxy, &CommonInfoProxy::LingmoIdError, this, &CommonInfoWork::lingmoIdErrorSlot);
...
void CommonInfoWork::initGrubAnimationModel()
{
    ...
    if (IS_COMMUNITY_SYSTEM) {
        scaleThemeList.append(QPair<int, QString>(2, "lingmo-hidpi-ssd-logo"));
        scaleThemeList.append(QPair<int, QString>(1, "lingmo-ssd-logo"));
    } else {
...
void CommonInfoWork::jumpToSecurityCenter()
{
    ...
    .path("/com/lingmo/defender/hmiscreen")
...
void CommonInfoWork::importCertificate(QString filePath)
{
    ...
    if (msg.type() == QDBusMessage::MessageType::ErrorMessage) {
        //系统通知弹窗qdbus 接口
        QDBusInterface  tInterNotify("org.lingmo.ocean.Notification1",
                                    "/org/lingmo/ocean/Notification1",
                                    "org.lingmo.ocean.Notification1",
                                    QDBusConnection::sessionBus());

               //初始化Notify 七个参数
        QString in0(QObject::tr("ocean-control-center"));
        uint in1 = 101;
...
void CommonInfoWork::setUeProgram(bool enabled)
{
    ...
    if (enabled) {
        ...
        if (!SYSTEM_LOCAL_LIST.contains(QLocale::system().name()))
            pathType = "-e";
        m_process->start("ocean-license-dialog",
                                      QStringList() << "-t" << m_title << pathType << m_content << "-a" << allowContent);
        qDebug()<<" Deliver content QStringList() = "<<"ocean-license-dialog"
...
void CommonInfoWork::setEnableDeveloperMode(bool enabled)
{
    ...
    // license内容
    QString content = getDevelopModeLicense(":/systeminfo/license/lingmo-end-user-license-agreement_developer_community_%1.txt", "");
    ...
    m_process = new QProcess(this);
    m_process->start("ocean-license-dialog", QStringList() << "-t" << title << pathType << contentPath << "-a" << allowContent);
...
std::pair<int, QString> CommonInfoWork::getPlyMouthInformation()
{
    ...
    static QStringList ScaleLowDpiThemeNames = {"lingmo-logo", "lingmo-ssd-logo", "uos-ssd-logo"};
    static QStringList ScaleHighDpiThemeNames = {"lingmo-hidpi-logo", "lingmo-hidpi-ssd-logo", "uos-hidpi-ssd-logo"};
...
void CommonInfoWork::lingmoIdErrorSlot(int code, const QString &msg)
{
    Q_UNUSED(code);
...
void CommonInfoWork::setPlymouthFactor(int factor)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, watcher, call] {
        if (call.isError()) {
            qCWarning(OceanUICommonInfoWork) << "DBus Error: " << call.error();
        }
        auto [factor, themeName] = getPlyMouthInformation();
        m_commomModel->setPlymouthTheme(themeName);
        m_commomModel->setPlymouthScale(factor);
