...
// static Q_LOGGING_CATEGORY(oceanuiLog, "ocean.oceanui.NavigationModel");
...
void NavigationModel::setNavigationObject(const QVector<OceanUIObject *> &objs)
{
    beginResetModel();
    m_data.clear();
    for (auto &&obj : objs) {
...
QVariant NavigationModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.row() & 1) {
        ...
        const OceanUIObject *item = m_data.at(i);
        switch (role) {
        case Qt::DisplayRole:
