...
struct SearchData
{
    ...
    const OceanUIObject *obj;
    const OceanUIObject *ancestors;
    QList<int> weight;

    explicit SearchData(const OceanUIObject *o)
        : obj(o)
    {
        ancestors = o;
        while (ancestors) {
            weight.prepend(ancestors->weight());
            const OceanUIObject *objParent = OceanUIObject::Private::FromObject(ancestors)->getParent();
...
protected:
    void addObject(OceanUIObject *obj, const QString &text, const QString &url);
    // Basic functionality:
...
void SearchSourceModel::addSearchData(OceanUIObject *obj, const QString &text, const QString &url)
{
    if (!obj) {
        return;
    }
    ...
    if (text.isEmpty() && url.isEmpty()) {
        QVector<OceanUIObject *> objs(obj->getChildren());
        while (!objs.isEmpty()) {
...
void SearchSourceModel::removeSearchData(const OceanUIObject *obj, const QString &text)
{
    if (!obj) {
        return;
    }
...
void SearchSourceModel::addObject(OceanUIObject *obj, const QString &text, const QString &url)
{
...
void SearchModel::addSearchData(OceanUIObject *obj, const QString &text, const QString &url)
{
    static_cast<SearchSourceModel *>(sourceModel())->addSearchData(obj, text, url);
}

void SearchModel::removeSearchData(const OceanUIObject *obj, const QString &text)
{
...
bool SearchModel::filterAcceptsRow(int source_row, const QModelIndex &source_parent) const
{
    ...
    const OceanUIObject *p = OceanUIObject::Private::FromObject(data->obj)->getParent();
    while (p && p->name() != "root") {
