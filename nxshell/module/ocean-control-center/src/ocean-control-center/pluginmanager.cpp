...
static Q_LOGGING_CATEGORY(oceanuiLog, "ocean.oceanui.plugin");
...
struct PluginData
{
    QString name;
    QString path;
    OceanUIObject *module;
    OceanUIObject *mainObj;
    OceanUIObject *soObj;
    QObject *data;
    QThread *thread;
    uint status;
    ...
    if (QFile::exists(soPath)) {
        ...
        if (!loader.isLoaded()) {
            ...
            do {
                ...
                if (iid.isEmpty() || iid != QString(qobject_interface_iid<OceanUIFactory *>())) {
                ...
                OceanUIFactory *factory = qobject_cast<OceanUIFactory *>(loader.instance());
                if (!factory) {
                    m_pManager->updatePluginStatus(m_data, DataErr, "The plugin isn't a OceanUIFactory." + soPath);
                    loader.unload();
                    break;
                }
...
PluginManager::PluginManager(OceanUIManager *parent)
    : QObject(parent)
    , m_manager(parent)
    ...
    , m_isDeleting(false)
{
    ...
    connect(m_manager, &OceanUIManager::hideModuleChanged, this, &PluginManager::onHideModuleChanged);
}
...
PluginManager::~PluginManager()
{
    ...
    for (auto &&data : m_plugins) {
        if (data->data) {
            qCDebug(oceanuiLog()) << "delete so" << data->name;
            delete data->data;
            data->data = nullptr;
        }
        delete data;
    ...
    if (plugin->status & PluginEnd) {
        ...
        if (plugin->module) {
            ...
            if (plugin->module->isVisibleToApp()) {
                ...
                connect(plugin->module, &OceanUIObject::visibleToAppChanged, this, &PluginManager::onVisibleToAppChanged);
                updatePluginStatus(plugin, PluginEnd);
            }
        } else {
            threadPool()->start(new LoadPluginTask(plugin, this));
        ...
        OceanUIManager::installTranslator(plugin->name);
        loadModule(plugin);
    } else {
        loadMetaData(plugin);
    }
...
void PluginManager::updatePluginStatus(PluginData *plugin, uint status, const QString &log)
{
    ...
    if (status & PluginErrMask) {
        qCWarning(oceanuiLog()) << plugin->name << ": status" << QString::number(plugin->status, 16) << log;
    } else {
        qCDebug(oceanuiLog()) << plugin->name << ": status" << QString::number(plugin->status, 16) << log;
    }
...
void PluginManager::createModule(QQmlComponent *component)
{
    ...
    if (component->isError()) {
        ...
        plugin->module = qobject_cast<OceanUIObject *>(object);
...
void PluginManager::createMain(QQmlComponent *component)
{
    ...
    if (component->isError()) {
        ...
        context->setContextProperties({ { "oceanuiData", QVariant::fromValue(plugin->data) }, { "oceanuiModule", QVariant::fromValue(plugin->module) } });
        ...
        plugin->mainObj = qobject_cast<OceanUIObject *>(object);
    ...
    if (plugin->mainObj) {
        if (plugin->mainObj->name().isEmpty() || (plugin->module && plugin->mainObj->name() == plugin->module->name())) {
            ...
            if (plugin->module) {
                ...
                connect(plugin->mainObj, &OceanUIObject::pageChanged, plugin->module, &OceanUIObject::setPage);
                connect(plugin->mainObj, &OceanUIObject::displayNameChanged, plugin->module, &OceanUIObject::setDisplayName);
                connect(plugin->mainObj, &OceanUIObject::descriptionChanged, plugin->module, &OceanUIObject::setDescription);
                connect(plugin->mainObj, &OceanUIObject::iconChanged, plugin->module, &OceanUIObject::setIcon);
                connect(plugin->mainObj, &OceanUIObject::badgeChanged, plugin->module, &OceanUIObject::setBadge);
                connect(plugin->mainObj, &OceanUIObject::visibleChanged, plugin->module, &OceanUIObject::setVisible);
                connect(plugin->mainObj, &OceanUIObject::active, plugin->module, &OceanUIObject::active);
                connect(plugin->mainObj, &OceanUIObject::deactive, plugin->module, &OceanUIObject::deactive);
            }
        } else {
        }
    } else {
        updatePluginStatus(plugin, MainObjErr, "The plugin isn't main OceanUIObject");
    }
...
void PluginManager::loadModules(OceanUIObject *root, bool async, const QStringList &dirs)
{
    if (!root)
        return;
    m_rootModule = root;
    qCDebug(oceanuiLog()) << "plugin dir:" << dirs;
        ...
        for (auto &&plugin : m_plugins) {
            if (plugin->thread && plugin->thread->isRunning()) {
                ...
                if (plugin->thread->isRunning()) {
                    qCWarning(oceanuiLog()) << plugin->name << ": status" << QString::number(plugin->status, 16) << plugin->thread << "thread exit timeout";
...
Q_DECLARE_METATYPE(oceanuiV25::PluginData *)
