...
static Q_LOGGING_CATEGORY(oceanuiLog, "ocean.oceanui.DBusAdaptor");
...
ControlCenterDBusAdaptor::ControlCenterDBusAdaptor(OceanUIManager *parent)
    : QDBusAbstractAdaptor(parent)
{
    parent->mainWindow()->installEventFilter(this);
    connect(parent, &OceanUIManager::activeObjectChanged, this, &ControlCenterDBusAdaptor::updatePage);
}

ControlCenterDBusAdaptor::~ControlCenterDBusAdaptor() { }

OceanUIManager *ControlCenterDBusAdaptor::parent() const
{
    return static_cast<OceanUIManager *>(QObject::parent());
}
...
{
    qCDebug(oceanuiLog()) << "exit pid:" << QCoreApplication::applicationPid();
...
{
    QDBusMessage msg = QDBusMessage::createSignal(OceanUIDBusPath, DBusProperties, DBusPropertiesChanged);
    msg << OceanUIDBusInterface << QVariantMap({ { "Page", page() }, { "Path", path() } }) << QStringList();
...
{
    QDBusMessage msg = QDBusMessage::createSignal(OceanUIDBusPath, DBusProperties, DBusPropertiesChanged);
    msg << OceanUIDBusInterface << QVariantMap({ { "Rect", rect() } }) << QStringList();
    QDBusConnection::sessionBus().asyncCall(msg);
}

DBusControlCenterGrandSearchService::DBusControlCenterGrandSearchService(OceanUIManager *parent)
    : QDBusAbstractAdaptor(parent)
...
OceanUIManager *DBusControlCenterGrandSearchService::parent() const
{
    return static_cast<OceanUIManager *>(QObject::parent());
}
