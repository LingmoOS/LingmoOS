...
D.DialogWindow {
    id: dialog
    property string userId: oceanuiData.currentUserId()
    property string currentAvatar: oceanuiData.avatar()
    width: 640
    height: 500
        ...
        Rectangle {
            ...
            ListModel {
                ...
                ListElement {
                    name: qsTr("Animal")
                    filter: "icons/animal"
                    icon: "oceanui_user_animal"
                    sections: ""
                }
                ListElement {
                    name: qsTr("Illustration")
                    filter: "icons/illustration"
                    icon: "oceanui_user_funny"
                    sections: ""
                }
                ListElement {
                    name: qsTr("Emoji")
                    filter: "icons/emoji"
                    icon: "oceanui_user_emoji"
                    sections: ""
                }
                ListElement {
                    name: qsTr("custom")
                    filter: "icons/local"
                    icon: "oceanui_user_custom"
                    sections: ""
                }
            }
        ...
        ColumnLayout {
            ...
            CustomAvatarEmpatyArea {
                ...
                function needShow() {
                    ...
                    let icons = oceanuiData.avatars(dialog.userId, scrollView.filter, "")
                ...
                ColumnLayout {
                    ...
                    Repeater {
                        ...
                        AvatarGridView {
                            ...
                            Connections {
                                ...
                                function onAvatarChanged(userId, avatar) {
                                    if (userId === dialog.userId) {
                                        ...
                                        view.model = []
                                        view.model = oceanuiData.avatars(dialog.userId, scrollView.filter, modelData)
                            ...
                            model: oceanuiData.avatars(dialog.userId, scrollView.filter, modelData)
                            isCustom: scrollView.isCustom
                            onIsCustomChanged: {
                                if (isCustom && !customEmptyAvatar.visible) {
                                    let icons = oceanuiData.avatars(dialog.userId, scrollView.filter, modelData)
                                    if (icons.length > 1)
