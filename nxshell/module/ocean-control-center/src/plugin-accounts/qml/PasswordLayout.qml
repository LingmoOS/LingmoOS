...
ColumnLayout {
    ...
    property string name: oceanuiData.userName(pwdLayout.userId)
            ...
            function onPasswordModifyFinished(id, code, msg) {
                ...
                if (code === 0) {
                    if (pwdContainter.eidtItems[2])
                        oceanuiData.setPasswordHint(pwdLayout.userId, pwdContainter.eidtItems[2].text)
                    pwdLayout.requestClose()
                    return
                }
                let info = oceanuiData.checkPasswordResult(code, msg, pwdLayout.name, pwdContainter.eidtItems[0].text)
                // wrong password
    ...
    Rectangle {
        ...
        function checkPassword() {
            ...
            if (currentPwd.visible) {
                let alertText = oceanuiData.checkPassword(pwdLayout.name, currentPwd.edit.text)
                if (alertText.length > 0) {
                    edit0.showAlertText(alertText)
                    return false
                }
            }
            alertText = oceanuiData.checkPassword(pwdLayout.name, edit0.text)
            if (alertText.length > 0) {
                edit0.showAlertText(alertText)
                return false
            }
            alertText = oceanuiData.checkPassword(pwdLayout.name, edit1.text)
            if (alertText.length > 0) {
                ...
                delegate: D.ItemDelegate {
                    ...
                    contentItem: PasswordItem {
                        ...
                        onTextChanged: function(text) {
                            if (index == 0) {
                                if (text.length > 0) {
                                    let lvl = oceanuiData.passwordLevel(text)
