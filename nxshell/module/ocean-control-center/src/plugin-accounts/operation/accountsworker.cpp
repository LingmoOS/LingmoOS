...
AccountsWorker::AccountsWorker(UserModel *userList, QObject *parent)
    ...
    , m_userQInter(new UserDBusProxy(QString("/org/lingmo/ocean/Accounts1/User%1").arg(getuid()), this))
    ...
    , m_userModel(userList)
{
    ...
    connect(m_accountsInter, &AccountsDBusProxy::UserAoceand, this, &AccountsWorker::addUser, Qt::QueuedConnection);
...
void AccountsWorker::checkPwdLimitLevel(int level)
{
    ...
    QDBusInterface interface(QStringLiteral("com.lingmo.defender.daemonservice"),
                             QStringLiteral("/com/lingmo/defender/daemonservice"),
                             QStringLiteral("com.lingmo.defender.daemonservice"));
...
void AccountsWorker::addUser(const QString &userPath)
{
    ...
    if(!userPath.contains("/org/lingmo/ocean/Accounts1"))
        return;
...
void AccountsWorker::setNopasswdLogin(User *user, const bool nopasswdLogin)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [=] {
        ...
        QStringList restartLockCommand = QStringList { "--user", "restart", "ocean-lock.service" };
...
void AccountsWorker::ADDomainHandle(const QString &server, const QString &admin, const QString &password)
{
    ...
    if (!exitCode) {
        ...
        // save config
        QFile file("/etc/lingmo/ocean-session-ui.conf");
        QFile tmpFile("/tmp/.ocean-session-ui.conf");

        if (file.exists() && file.open(QIODevice::Text | QIODevice::ReadOnly)) {
            qDebug() << file.copy("/tmp/.ocean-session-ui.conf");
        }
        if (tmpFile.open(QIODevice::Text | QIODevice::ReadWrite)) {
            QSettings setting("/tmp/.ocean-session-ui.conf", QSettings::IniFormat);
            setting.setValue("loginPromptInput", !isJoin);
            setting.sync();
            QProcess::execute("pkexec", QStringList() << "cp"
                                                      << "/tmp/.ocean-session-ui.conf"
                                                      << "/etc/lingmo/ocean-session-ui.conf");
            tmpFile.remove();
        }
    } else {
...
CreationResult *AccountsWorker::createAccountInternal(const User *user)
{
    ...
    if (!validation) {
        result->setType(CreationResult::UserNameError);
        result->setMessage(dgettext("ocean-daemon", reply.argumentAt(1).toString().toUtf8().data()));
        return result;
    }
    ...
    authenticationResult = Authority::instance()->checkAuthorizationSync("org.lingmo.ocean.accounts.user-administration", UnixProcessSubject(getpid()),
