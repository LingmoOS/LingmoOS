...
UpdateWork::UpdateWork(UpdateModel* model, QObject *parent)
    : QObject{ parent }
    , m_model(model)
    , m_updateInterface(new QDBusInterface("org.lingmo.UpdateManager1", "/org/lingmo/UpdateManager1", "org.lingmo.UpdateManager1", QDBusConnection::systemBus()))
{
    initData();
    initConnection();
}
...
void UpdateWork::onActionBtnClicked()
{
    ...
    if (m_model->getUpdateState() == "success") {
        DDBusSender()
            .service("com.lingmo.ocean.shutdownFront")
            .interface("com.lingmo.ocean.shutdownFront")
            .path("/com/lingmo/ocean/shutdownFront")
            .method("Restart")
            .call();
        return;
    }
...
void UpdateWork::onPropertiesChanged(const QDBusMessage &message)
{
    ...
    if (interfaceName == UpdateManagerInterface) {
        ...
        for (int i = 0; i < keys.size(); i++) {
            qInfo() << "org.lingmo.UpdateManager1 key : " << keys.at(i);
