...
D.ApplicationWindow {
    ...
    Shortcut {
        ...
        onActivated: OceanUIApp.showHelp()
        onActivatedAmbiguously: OceanUIApp.showHelp()
    }
    D.TitleBar {
        id: titleBar
        ...
        menu: Menu {
            ...
            D.MenuSeparator {}
            D.HelpAction {
                onTriggered: OceanUIApp.showHelp()
            }
            D.AboutAction {
        ...
        aboutDialog: D.AboutDialog {
            ...
            companyLogo: "file://" + DTK.lingmoDistributionOrgLogo
            websiteName: DTK.lingmoWebsiteName
            websiteLink: DTK.lingmoWebsiteLink
        ...
        SwipeView {
            ...
            Connections {
                ...
                function onActiveObjectChanged(activeObject) {
                    if (stackView.currentIndex !== 0 && OceanUIApp.root === OceanUIApp.activeObject) {
                        homePage.contentVisible = true
                        secondPage.visible = true
                        stackView.currentIndex = 0
                        hideTimer.restart()
                    } else if (stackView.currentIndex !== 1 && OceanUIApp.root !== OceanUIApp.activeObject) {
                        homePage.contentVisible = true
                        secondPage.visible = true
    ...
    Component.onCompleted: {
        root.width = OceanUIApp.width
        root.height = OceanUIApp.height
        root.x = (Screen.width - root.width) / 2
        root.y = (Screen.height - root.height) / 2
        OceanUIApp.root.page = rootLayout
        centralView.contentItem = OceanUIApp.root.getSectionItem(centralView)
    }
}
