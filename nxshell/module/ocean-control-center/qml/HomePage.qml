...
Control {
    ...
    function updateMargin() {
        if (width > grid.cellWidth) {
            ...
            var length = oceanuiObj.children.length
            if (length < count) {
                count = length
            }
    ...
    GridView {
        ...
        Keys.onPressed: function (event) {
            ...
            case Qt.Key_Enter:
            case Qt.Key_Return:
                var obj = oceanuiModel.getObject(currentIndex)
                if (obj) {
                    OceanUIApp.showPage(obj)
                }
                break
            default:
                return
                        ...
                        function updateDescription() {
                            if (model.item.description === "" && model.item.children.length > 0) {
                                ...
                                for (var i = 0; i < len && descs.length < 3; ++i) {
                                    if (model.item.children[i].pageType & OceanUIObject.Menu) {
