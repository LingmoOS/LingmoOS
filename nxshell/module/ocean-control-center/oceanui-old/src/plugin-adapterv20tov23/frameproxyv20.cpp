...
void FrameProxyV20::pushWidget(ModuleInterface *const inter, QWidget *const w, oceanuiV20::FrameProxyInterface::PushType type)
{
...
{
    if (m_moduleMap.contains(inter))
        m_moduleMap.value(inter)->setHioceann(!visible);
}
...
void FrameProxyV20::showModulePage(const QString &module, const QString &page, bool animation)
{
    ...
    if (!page.isEmpty())
        arg += "/" + page;
    QDBusMessage message = QDBusMessage::createMethodCall("com.lingmo.ocean.ControlCenter", "/com/lingmo/ocean/ControlCenter", "com.lingmo.ocean.ControlCenter", "ShowPage");
    message << arg;
...
void FrameProxyV20::setModuleVisible(const QString &module, bool visible)
{
    ...
    if (find_it != m_moduleMap.cend())
        find_it.value()->setHioceann(!visible);
}
