...
const QString Path  = "/org/lingmo/ocean/Notification1";
...
{
    connect(m_dbus, &NotificationDBusProxy::AppAoceandSignal, this, &NotificationWorker::onAppAoceand);
...
void NotificationWorker::initAppSetting()
{
    ...
    connect(timer, &QTimer::timeout, [this, appList, timer]() {
        if (appList->isEmpty()) {
            ...
            onAppAoceand(appList->takeFirst());
        }
    });
    timer->start(10);
}

void NotificationWorker::onAppAoceand(const QString &id)
{
