...
Q_LOGGING_CATEGORY(OceanUINotifyModule, "oceanui-notify-module");
    ...
    , m_appNameList{}
{
    ...
    setIcon(DIconTheme::findQIcon("oceanui_nav_notification"));
    if (m_model) {
        delete m_model;
    }
    m_model = new NotificationModel(this);
    m_worker = new NotificationWorker(m_model, this);
    connect(m_model, &NotificationModel::appListAoceand, this, &NotificationModule::onAppListAoceand);
    // clang-format off
...
void NotificationModule::onAppListAoceand(AppItemModel *item)
{
    QString softName = item->getAppName();
    qCInfo(OceanUINotifyModule) << "App" << softName << "aoceand";
...
void NotificationModule::onAppListRemoved(AppItemModel *item)
{
    ...
    qCInfo(OceanUINotifyModule) << "App" << item->getAppName() << "removed";
    if (index >= 0) {
