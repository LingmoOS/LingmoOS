...
#include "widgets/oceanuislider.h"
...
static const QMap<QString, QString> &pluginIconMap = {{"AiAssistant",    "oceanui_dock_assistant"}
    ...
    : PageModule("dock", tr("Dock"), QString(), nullptr), m_screenTitle(new ItemModule("screenTitle", tr("Multiple Displays"))), m_screen(new ItemModule("screen", tr("Show Dock"), this, &DockModuleObject::initScreen))
{
    ...
    m_displayProxy.reset(new QDBusInterface("org.lingmo.ocean.Display1", "/org/lingmo/ocean/Display1",
                                            "org.lingmo.ocean.Display1", QDBusConnection::sessionBus(), this));
    // 当任务栏服务未注册或当前只有一个屏幕或当前有多个屏幕但设置为复制模式时均不显示多屏设置项
    connect(qApp, &QApplication::screenAoceand, this, &DockModuleObject::updateScreenVisible);
    connect(qApp, &QApplication::screenRemoved, this, &DockModuleObject::updateScreenVisible);
    QDBusConnection::sessionBus().connect("org.lingmo.ocean.Display1", "/org/lingmo/ocean/Display1",
    ...
    QDBusInterface dockInter("org.lingmo.ocean.Dock1", "/org/lingmo/ocean/Dock1", "org.lingmo.ocean.Dock1", QDBusConnection::sessionBus(), this);
...
QIcon DockModuleObject::getIcon(const QString &oceanuiIcon, bool isDeactivate, QString itemKey) const
{
    auto originPixmap = QIcon(oceanuiIcon).pixmap(ICON_SIZE, ICON_SIZE);
    ...
    if (pluginIcon.isNull()) {
        pluginIcon = QIcon::fromTheme("oceanui_dock_plug_in");
    }
...
void DockModuleObject::initStatus(ComboxWidget *widget)
{
    ...
    widget->setComboxOption(QStringList() << tr("Keep shown") << tr("Keep hioceann") << tr("Smart hide"));
...
void DockModuleObject::initSizeSlider(TitledSliderItem *slider)
{
    ...
    // ocean-shell dock panel min size:37
...
void DockModuleObject::initScreenTitle(TitleLabel *label)
{
    ...
    connect(qApp, &QApplication::screenAoceand, label, [=]
...
QWidget *DockModuleObject::initScreen(DCC_NAMESPACE::ModuleObject *module)
{
    ...
    connect(qApp, &QApplication::screenAoceand, widget, [widget]
...
void DockModuleObject::initPluginView(DListView *view)
{
    ...
    auto initPluginModel = [=] (DockItemInfos plugins) {
        for (DockItemInfo dockItem : plugins)
        {
            ...
            leftAction->setIcon(getIcon(dockItem.oceanui_icon, !view->isActiveWindow(), dockItem.itemKey));
            ...
            item->setData(dockItem.oceanui_icon, Dtk::UserRole + 2);
...
void DockModuleObject::onDisplayPropertiesChanged(const QDBusMessage &dbusMessage)
{
    ...
    if (interfaceName != "org.lingmo.ocean.Display1")
        return;
