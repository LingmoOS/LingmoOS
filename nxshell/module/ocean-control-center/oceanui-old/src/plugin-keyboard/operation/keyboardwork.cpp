    ...
    , m_translatorLanguage(nullptr)
{
    ...
    connect(m_keyboardDBusProxy, &KeyboardDBusProxy::Aoceand, this, &KeyboardWorker::onAoceand);
...
void KeyboardWorker::windowSwitch()
{
    ...
    if (!licenseInfo.isValid()) {
        qDebug() << "com.lingmo.license error ," << licenseInfo.lastError().name();
        return;
    }
...
void KeyboardWorker::active()
{
    if (!m_translatorLanguage) {
        ...
        m_translatorLanguage->load("/usr/share/ocean-control-center/translations/keyboard_language_" + QLocale::system().name());
...
void KeyboardWorker::onAoceand(const QString &in0, int in1)
{
    QDBusPendingReply<QString> reply = m_keyboardDBusProxy->GetShortcut(in0, in1);
    QDBusPendingCallWatcher *watcher = new QDBusPendingCallWatcher(reply, this);
    connect(watcher, &QDBusPendingCallWatcher::finished, this, &KeyboardWorker::onAoceandFinished);
}
...
void KeyboardWorker::onAoceandFinished(QDBusPendingCallWatcher *watch)
{
    QDBusPendingReply<QString> reply = *watch;
...
void KeyboardWorker::onLocalListsFinished(QDBusPendingCallWatcher *watch)
{
    ...
    for (int i = 0; i!=list.size(); ++i) {
        ...
        md.setText(QString("%1 - %2").arg(list.at(i).name).arg(QCoreApplication::translate("oceanui::keyboard::Language",list.at(i).name.toUtf8().data())));
        m_datas.append(md);
    }
...
void KeyboardWorker::onPinyin()
{
    m_letters.clear();
    m_metaDatas.clear();
    QDBusInterface dbus_pinyin("org.lingmo.ocean.Pinyin1", "/org/lingmo/ocean/Pinyin1",
                               "org.lingmo.ocean.Pinyin1");
