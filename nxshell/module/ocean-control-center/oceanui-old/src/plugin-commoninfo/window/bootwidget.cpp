...
Q_LOGGING_CATEGORY(OceanUICommonInfoBoot, "oceanui-commoninfo-bootwidget");
...
void BootWidget::onCurrentItem(const QModelIndex &curIndex)
{
    ...
    qCDebug(OceanUICommonInfoBoot) << "current text" << curText << "selectedText" << selectedText;
    ...
    if (curText != selectedText) {
        qCInfo(OceanUICommonInfoBoot) << "start to set first menu to "<< curText;
