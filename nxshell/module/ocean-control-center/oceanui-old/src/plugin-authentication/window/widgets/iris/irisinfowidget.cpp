...
void IrisInfoWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (m_state == CharaMangerModel::AddInfoState::StartState
        ...
        if (DGuiApplicationHelper::instance()->themeType() == DGuiApplicationHelper::LightType) {
            backRes = ":/icons/lingmo/builtin/icons/oceanui_auth_iris-light.svg";
        } else if (DGuiApplicationHelper::instance()->themeType() == DGuiApplicationHelper::DarkType) {
            backRes = ":/icons/lingmo/builtin/icons/oceanui_auth_iris-dark.svg";
        }
    } else if (m_state == CharaMangerModel::AddInfoState::Success) {
        backRes = ":/icons/lingmo/builtin/icons/oceanui_auth_success.svg";
    } else if (m_state == CharaMangerModel::AddInfoState::Fail) {
        backRes = ":/icons/lingmo/builtin/icons/oceanui_auth_fail.svg";
    } else {
    ...
    if (m_state == CharaMangerModel::AddInfoState::Processing) {
        ...
        painter.translate( -(width() / 2), -(height() / 2));
        QPixmap insidePix(":/icons/lingmo/builtin/icons/oceanui_auth_circle_inside.svg");
        ...
        painter.translate( -(width() / 2), -(height() / 2));
        QPixmap outsidePix(":/icons/lingmo/builtin/icons/oceanui_auth_circle_outside.svg");
