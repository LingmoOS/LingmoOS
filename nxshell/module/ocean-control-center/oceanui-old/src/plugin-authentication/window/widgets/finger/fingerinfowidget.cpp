...
void FingerInfoWidget::setStatueMsg(const QString &title, const QString &msg, bool reset)
{
    ...
    if (!m_reset) {
        ...
        if (m_pro == 0) {
            m_view->setPictureSequence(QStringList() << QString(":/icons/lingmo/builtin/icons/%1/icons/finger/fingerprint_light.svg").arg(m_theme));
        } else {
            m_view->setPictureSequence(QStringList() << QString(":/icons/lingmo/builtin/icons/%1/icons/finger/fingerprint_animation_light_%2.svg")
...
void FingerInfoWidget::setProsses(int pro)
{
    ...
    if (m_pro == 0) {
        ...
        if (m_pro > 0 && m_pro < 35) {
            ...
            m_defTitle = tr("Fingerprint aoceand");
            m_defTip = tr("");
        }
    }
    m_msgTimer->stop();
