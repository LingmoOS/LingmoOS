...
#include "faceioceantailwidget.h"
...
AuthenticationModule::AuthenticationModule(QObject *parent)
    : HListModule("authentication", tr("Biometric Authentication"), QString(), DIconTheme::findQIcon("oceanui_nav_authentication"), parent)
    , m_model(new CharaMangerModel(this))
    , m_work(new CharaMangerWorker(m_model, this))
{
    setHioceann(!m_model->charaVaild());
