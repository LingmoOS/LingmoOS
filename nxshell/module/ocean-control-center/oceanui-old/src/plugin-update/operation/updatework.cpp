...
Q_LOGGING_CATEGORY(OceanUIUpdateWork, "oceanui-update-work")

#define MIN_NM_ACTIVE 50
#define UPDATE_PACKAGE_SIZE 0

const QString ChangeLogFile = "/usr/share/lingmo/release-note/UpdateInfo.json";
const QString ChangeLogDic = "/usr/share/lingmo/";
const QString UpdateLogTmpFile = "/tmp/lingmo-update-log.json";
...
inline void notifyError(const QString &summary, const QString &body)
{
    DUtil::DNotifySender(summary)
            .appIcon("ocean-control-center")
            .appName("ocean-control-center")
            .appBody(body)
            .timeOut(5000)
            .call();
}
...
UpdateWorker::UpdateWorker(UpdateModel *model, QObject *parent)
    ...
    , m_updateConfig(DConfig::create("org.lingmo.ocean.control-center", QStringLiteral("org.lingmo.ocean.control-center.update"), QString(), this))
{
}
...
void UpdateWorker::getLicenseState()
{
    ...
    if (!licenseInfo.isValid()) {
        qCDebug(OceanUIUpdateWork) << "com.lingmo.license error ," << licenseInfo.lastError().name();
        return;
    }
    UiActiveState reply =
            static_cast<UiActiveState>(licenseInfo.property("AuthorizationState").toInt());
    qCDebug(OceanUIUpdateWork) << "Authorization State:" << reply;
...
void UpdateWorker::checkForUpdates()
{
    setOnBattery(m_updateInter->onBattery());
    if (checkDbusIsValid()) {
        qCDebug(OceanUIUpdateWork) << " checkDbusIsValid . do nothing";
        return;
    }
...
void UpdateWorker::requestUpdateLog()
{
    ...
    qCDebug(OceanUIUpdateWork) << "request url : " << url;
    http->get(request);
}
...
void UpdateWorker::setUpdateInfo()
{
    ...
    qCDebug(OceanUIUpdateWork) << " UpdateWorker::setUpdateInfo() ";
    ...
    qCDebug(OceanUIUpdateWork) << "systemUpdate packages:" << m_systemPackages;
    qCDebug(OceanUIUpdateWork) << "safeUpdate packages:" << m_safePackages;
    qCDebug(OceanUIUpdateWork) << "unkonowUpdate packages:" << m_unknownPackages;

    if (m_model->status() == UpdatesStatus::UpdateFailed) {
        qCDebug(OceanUIUpdateWork) << " [UpdateWork] The status is error. Current status : "
                               << m_model->status();
        return;
    }
    ...
    if (updateCount < 1) {
        QFile file("/tmp/.oceanui-update-successd");
        if (file.exists()) {
    ...
    qCDebug(OceanUIUpdateWork) << " UpdateWorker::setUpdateInfo: updateInfoMap.count()"
                           << updateInfoMap.count();

    if (updateInfoMap.count() == 0) {
        m_model->setStatus(UpdatesStatus::Updated, __LINE__);
    } else {
        qCDebug(OceanUIUpdateWork) << "UpdateWorker::setAppUpdateInfo: downloadSize = "
                               << m_downloadSize;
...
{
    qCDebug(OceanUIUpdateWork) << "getAllUpdateInfo";
...
void UpdateWorker::getItemInfo(QJsonValue jsonValue, UpdateItemInfo *itemInfo)
{
    ...
    qCDebug(OceanUIUpdateWork) << "UpdateWorker::getItemInfo  itemInfo->name() == " << itemInfo->name();
...
CheckUpdateJobRet UpdateWorker::createCheckUpdateJob(const QString &jobPath)
{
    ...
    qCDebug(OceanUIUpdateWork) << " Get Job: " << ret.jobID << ret.jobDescription
...
void UpdateWorker::distUpgrade(ClassifyUpdateType updateType)
{
    ...
    qCDebug(OceanUIUpdateWork) << "distUpgrade:"
...
{
    qCDebug(OceanUIUpdateWork) << "setUpdateMode"
...
{
    qCInfo(OceanUIUpdateWork) << "Handle reply of update log";
    ...
    if (respondBody.isEmpty()) {
        qCWarning(OceanUIUpdateWork) << "Request body is empty";
        return;
    }
...
{
    QScopedPointer<DConfig> dconfig(DConfig::create("org.lingmo.ocean.control-center",
                                                    QStringLiteral("org.lingmo.ocean.control-center.update")));
    ...
    if (dconfig) {
        ...
        if (!updateLogAddress.isEmpty()) {
            qCDebug(OceanUIUpdateWork) << " updateLogAddress " << updateLogAddress;
...
int UpdateWorker::isUnstableResource() const
{
    ...
    QScopedPointer<DConfig> config(DConfig::create("org.lingmo.unstable", "org.lingmo.unstable"));
    if (!config) {
        qInfo() << "Can not find org.lingmo.unstable or an error occurred in DTK";
...
{
    qCDebug(OceanUIUpdateWork) << "[setCheckUpdatesJob] start status : " << m_model->status();
    UpdatesStatus state = m_model->status();
...
void UpdateWorker::setUpdateItemProgress(UpdateItemInfo *itemInfo, double value)
{
    ...
    if (itemInfo) {
        if (!getNotUpdateState()) {
            qCDebug(OceanUIUpdateWork) << " Now can't to update continue...";
            resetDownloadInfo();
            return;
        }
        ...
        qCDebug(OceanUIUpdateWork) << "[wubw download] DownloadInfo is nullptr , waitfor download info";
    }
}
...
void UpdateWorker::onJobListChanged(const QList<QDBusObjectPath> &jobs)
{
    ...
    for (const auto &job : jobs) {
        ...
        qCDebug(OceanUIUpdateWork) << "[wubw] onJobListChanged, id : " << id
                               << " , m_jobPath : " << m_jobPath;
        if ((id == "update_source" || id == "custom_update") && m_checkUpdateJob == nullptr) {
            ...
            qCDebug(OceanUIUpdateWork) << "Install id: " + id + ", nothing to do";
        }
    }
}
...
void UpdateWorker::onUnkonwnUpdateInstallProgressChanged(double value)
{
    ...
    qCDebug(OceanUIUpdateWork) << "onUnkonwnUpdateInstallProgressChanged : " << value;
    setUpdateItemProgress(itemInfo, value);
}

void UpdateWorker::onCheckUpdateStatusChanged(const QString &value)
{
    qCDebug(OceanUIUpdateWork) << "[setCheckUpdatesJob]status is: " << value;
...
{
    qCDebug(OceanUIUpdateWork) << "job description: " << jobDescription;

    m_model->setClassityUpdateJonError(ClassifyUpdateType::Invalid,
                                       analyzeJobErrorMessage(jobDescription));
    m_model->setStatus(UpdatesStatus::UpdateFailed, __LINE__);
    qCDebug(OceanUIUpdateWork) << Q_FUNC_INFO << "UpdateFailed , jobDescription : " << jobDescription;
...
void UpdateWorker::onRequestOpenAppStore()
{
    ...
    qCDebug(OceanUIUpdateWork) << reply.errorMessage();
}
...
UpdateErrorType UpdateWorker::analyzeJobErrorMessage(QString jobDescription)
{
    ...
    if (err_rpt.error != QJsonParseError::NoError) {
        qCDebug(OceanUIUpdateWork) << "更新失败JSON格式错误";
...
void UpdateWorker::onClassityDownloadStatusChanged(const ClassifyUpdateType type,
                                                   const QString &value)
{
    qCDebug(OceanUIUpdateWork) << "onClassityDownloadStatusChanged ::" << type << "status :: " << value;
    if (value == "running" || value == "ready") {
        m_model->setClassifyUpdateTypeStatus(type, UpdatesStatus::Downloading);
    } else if (value == "failed") {
        QPointer<UpdateJobDBusProxy> job = getDownloadJob(type);
        qCDebug(OceanUIUpdateWork) << "onClassityDownloadStatusChanged ::" << type
...
void UpdateWorker::onClassityInstallStatusChanged(const ClassifyUpdateType type,
                                                  const QString &value)
{
    qCDebug(OceanUIUpdateWork) << "onClassityInstallStatusChanged ::" << type << "status :: " << value;
    if (value == "ready") {
        m_model->setClassifyUpdateTypeStatus(type, UpdatesStatus::Downloaded);
        ...
        qCDebug(OceanUIUpdateWork) << "onClassityInstallStatusChanged ::" << type
                               << "job->description() :: " << job->description();
        m_model->setClassityUpdateJonError(type, analyzeJobErrorMessage(job->description()));
        m_model->setClassifyUpdateTypeStatus(type, UpdatesStatus::UpdateFailed);
        cleanLastoreJob(job);
...
{
    qCDebug(OceanUIUpdateWork) << " ---- UpdatablePackages = " << updatablePackages.count();
                                            ...
                                            QString message)
{
    /*
        ...
        -9: 本作的仓库本不存在;
    */
    qCDebug(OceanUIUpdateWork) << " Atomic State : " << state << "operate: " << operate
    ...
    default:
        ...
        qCDebug(OceanUIUpdateWork) << "handleAtomicStateChanged"
...
{
    // 处理正在备份的状态
    qCDebug(OceanUIUpdateWork) << Q_FUNC_INFO << " [AtomicUpdateing] 可以备份, 开始备份...";
...
void UpdateWorker::onFixError(const ClassifyUpdateType &updateType, const QString &errorType)
{
    ...
    QDBusInterface lastoreManager("org.lingmo.ocean.Lastore1",
                                  "/org/lingmo/ocean/Lastore1",
                                  "org.lingmo.ocean.Lastore1.Manager",
                                  QDBusConnection::systemBus());
    if (!lastoreManager.isValid()) {
        qCDebug(OceanUIUpdateWork) << "com.lingmo.license error ," << lastoreManager.lastError().name();
        return;
    }
    ...
    if (reply.isValid()) {
        ...
        connect(m_fixErrorJob, &UpdateJobDBusProxy::StatusChanged, this, [=](const QString status) {
            if (status == "succeed" || status == "failed" || status == "end") {
                qCDebug(OceanUIUpdateWork) << "m_fixErrorJob ---status :" << status;
...
void UpdateWorker::onRequestLastoreHeartBeat()
{
    QDBusInterface lastoreManager("org.lingmo.ocean.Lastore1",
                                  "/org/lingmo/ocean/Lastore1",
                                  "org.lingmo.ocean.Lastore1.Updater",
                                  QDBusConnection::systemBus());
    if (!lastoreManager.isValid()) {
        qCDebug(OceanUIUpdateWork) << "com.lingmo.license error ," << lastoreManager.lastError().name();
        return;
...
std::optional<QString> UpdateWorker::getTestingChannelSource()
{
    ...
    qCDebug(OceanUIUpdateWork) << "sourceFile" << sourceFile;
    QFile f(sourceFile);
...
CanExitTestingChannelStatus UpdateWorker::checkCanExitTestingChannelDialog()
{
    ...
    if (!testChannelSrc.has_value()) {
        qCDebug(OceanUIUpdateWork) << "Not have source file";
    ...
    if (dpkgL.exitStatus() == QProcess::CrashExit) {
        qCWarning(OceanUIUpdateWork) << "run dpkg error:" << dpkgL.readAllStandardError();
        return CanExitTestingChannelStatus::CheckError;
    }
    ...
    for (const QString &line : listpkgpre) {
        ...
        if (!pkg.contains("ocean") && !pkg.contains("lingmo") && !pkg.contains("dtk")
            && !pkg.contains("uos")) {
            continue;
        }
...
void UpdateWorker::setTestingChannelEnable(const bool &enable)
{
    ...
    if (!enable) {
        if (m_updateInter->PackageExists(TestingChannelPackage)) {
            qCDebug(OceanUIUpdateWork) << "Testing:"
            ...
            if (exitStatus == CanExitTestingChannelStatus::CheckOk) {
                ...
                m_model->setTestingChannelStatus(TestingChannelStatus::Joined);
                qCDebug(OceanUIUpdateWork) << "Cancel to leave testingChannel";
                return;
            }
        }
