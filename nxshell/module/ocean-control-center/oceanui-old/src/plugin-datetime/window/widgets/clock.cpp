...
{
    m_hour = getPixmap("oceanui_noun_hour", pointSize);
    m_min = getPixmap("oceanui_noun_minute", pointSize);
    m_sec = getPixmap("oceanui_noun_second", pointSize);
...
void Clock::paintEvent(QPaintEvent *event)
{
    ...
    do {
        ...
        if (nightMode == m_isBlack && !m_plat.isNull())
            break;
        if (nightMode) {
            m_plat = getPixmap("oceanui_clock_black", clockSize);
            m_isBlack = true;
        } else {
            m_plat = getPixmap("oceanui_clock_white", clockSize);
            m_isBlack = false;
        }
    } while (false);
