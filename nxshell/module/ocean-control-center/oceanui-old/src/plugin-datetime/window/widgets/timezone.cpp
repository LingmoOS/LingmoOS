...
#if USE_LINGMO_ZONE
const QString kZoneTabFileLingmo = QStringLiteral(LINGMO_TIME_ZONE_PATH);
const QString kZoneTabFile = std::visit([] {
    if (QFile(kZoneTabFileLingmo).exists()) {
        return kZoneTabFileLingmo;
    }
...
#if USE_LINGMO_ZONE
    if (kZoneTabFile == kZoneTabFileLingmo && QFile(kZoneTabFile).exists()) {
        return true;
    }
#endif
