...
Q_LOGGING_CATEGORY(DdcDateTimeTimeSettingModule, "oceanui-datetime-timesettingmodule")
...
void TimeSettingModule::initTimeSetting(SettingsGroup *datetimeGroup)
{
    ...
    int nIndex = QFontDatabase::addApplicationFont(":/icons/lingmo/builtin/resource/lingmodigitaltimes-Regular.ttf");
    if (nIndex != -1) {
...
void TimeSettingModule::onConfirmButtonClicked()
{
    if (m_autoSyncTimeSwitch->checked() && m_ntpServerList->comboBox()->currentText() == tr("Customize")) {
        ...
        //        QGSettings("com.lingmo.ocean.control-center","/com/lingmo/ocean/control-center/").set("custom-ntpserver", m_customNtpServer);
