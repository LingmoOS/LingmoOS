...
void SystemInfoModule::initChildModule()
{
    ...
    // 二级菜单--协议与隐私政策
    ModuleObject *moduleAgreement = new VListModule("agreement", tr("Agreements and Privacy Policy"), DIconTheme::findQIcon("oceanui_version"), this);

    // 三级菜单--协议与隐私政策-版本协议
    ModuleObject *moduleEdition = new PageModule("editionLicense", tr("Edition License"), DIconTheme::findQIcon("oceanui_version"), moduleAgreement);
    ...
    if (auto licensepath = DCC_LICENSE::isEndUserAgreementExist(); licensepath.exist) {
        ...
        ModuleObject *moduleUserAgreement = new PageModule("endUserLicenseAgreement", tr("End User License Agreement"), DIconTheme::findQIcon("oceanui_protocol"), moduleAgreement);
    ...
    // 三级菜单--协议与隐私政策-隐私政策
    ModuleObject *modulePolicy = new PageModule("privacyPolicy", tr("Privacy Policy"), DIconTheme::findQIcon("oceanui_privacy_policy"), moduleAgreement);
...
const QString systemCopyright()
{
    ...
    if (oem_copyright.isEmpty()) {
        if (useLingmoCopyright())
            return QApplication::translate("LogoModule", "Copyright© 2011-%1 Lingmo Community")
...
ModuleObject *SystemInfoPlugin::module()
{
    ...
    moduleInterface->setIcon(DIconTheme::findQIcon("oceanui_nav_systeminfo"));
