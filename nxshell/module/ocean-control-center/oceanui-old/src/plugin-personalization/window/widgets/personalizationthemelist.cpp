    ...
    , m_listview(new DListView)
{
    ...
    auto *buttonTuple = new ButtonTuple(oceanuiV23::ButtonTuple::Save, this);
...
void PersonalizationThemeList::setModel(ThemeModel *const model)
{
    ...
    connect(m_model, &ThemeModel::itemAoceand, this, &PersonalizationThemeList::onAddItem);
    connect(m_model, &ThemeModel::picAoceand, this, &PersonalizationThemeList::onSetPic);
...
void PersonalizationThemeList::onAddItem(const QJsonObject &json)
{
    ...
    if (json["type"] == "gtk") {
        ...
        } else if (id == "lingmo-dark") {
            item->setText(tr("Dark"));
        } else if (id == "lingmo-auto") {
        ...
        item->setText(id == "lingmo" ? QString("lingmo (%1)").arg(tr("Default")) : name);
    } else {
        // cursor use "id" as title
        item->setText(id == "lingmo" ? QString("lingmo (%1)").arg(tr("Default")) : id);
    }
