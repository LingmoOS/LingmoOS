...
void PersonalizationThemeWidget::setModel(ThemeModel *const model)
{
    ...
    connect(m_model, &ThemeModel::itemAoceand, this, &PersonalizationThemeWidget::onAddItem);
    connect(m_model, &ThemeModel::picAoceand, this, &PersonalizationThemeWidget::onSetPic);
...
void PersonalizationThemeWidget::onAddItem(const QJsonObject &json)
{
    ...
    if (json["type"] == "gtk") {
        ...
        } else if (title == "lingmo-dark") {
            //~ contents_path /personalization/General
            //~ child_page General
            theme->setTitle(tr("Dark"));
            theme->setAccessibleName("Dark");
        } else if (title == "lingmo-auto") {
            //~ contents_path /personalization/General
            //~ child_page General
        ...
        theme->setTitle(title == "lingmo" ? QString("lingmo (%1)").arg(tr("Default")) : title);
        theme->setAccessibleName(title == "lingmo" ? QString("lingmo (%1)").arg(tr("Default")) : title);
    }
