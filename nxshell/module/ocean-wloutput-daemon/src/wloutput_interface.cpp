...
void wloutput_interface::StartWork()
{
    QObject::connect(m_pConnectThread, &ConnectionThread::connected, this, [ this ] {
        ...
        connect(m_pRegisry, &Registry::oceanSeatAnnounced, this,
                [ = ](quint32 name, quint32 version) {
            qDebug() << "create oceanseat";
            m_oceanSeat = m_pRegisry->createOCEANSeat(name, version, this);
        }
        );
        ...
        connect(m_pRegisry, &Registry::interfacesAnnounced, this, [ this ] {
            ...
            if (m_oceanSeat) {
                m_oceanPointer = m_oceanSeat->createDDePointer(this);
                connect(m_oceanPointer, &OCEANPointer::buttonStateChanged, this,
                        [this] (const QPointF &pos, quint32 button, KWayland::Client::OCEANPointer::ButtonState state) {
                    if (state == OCEANPointer::ButtonState::Released) {
                ...
                connect(m_oceanPointer, &OCEANPointer::motion, this, [this] (const QPointF &pos) {
                    Q_EMIT CursorMove(pos.x(), pos.y());
                }
                );
                connect(m_oceanPointer, &OCEANPointer::axisChanged, this, [this] (quint32 time, KWayland::Client::OCEANPointer::Axis axis, qreal delta) {
                    Q_EMIT AxisChanged(int(axis), delta);
                });

                m_oceanTouch = m_oceanSeat->createOCEANTouch(this);
                connect(m_oceanTouch, &OCEANTouch::touchDown, this,
                        [this] (int32_t kwaylandId, const QPointF &pos) {
                    m_touchMap.insert(kwaylandId, pos);
                    // ocean-session-daemon 监听此信号
                    Q_EMIT ButtonPress(kwaylandId, pos.x(), pos.y());
                }
                );
                connect(m_oceanTouch, &OCEANTouch::touchMotion, this, [this] (int32_t kwaylandId, const QPointF &pos) {
                    // 更新滑动位置
                    m_touchMap.insert(kwaylandId, pos);
                });

                connect(m_oceanTouch, &OCEANTouch::touchUp, this,
...
void wloutput_interface::onDeviceChanged(OutputDeviceV2 *dev)
{
    ...
    if (uuid2OutputDevice.find(uuid) == uuid2OutputDevice.end()) {
        ...
        qDebug() << "OutputDeviceV2::Aoceand uuid=" << uuid;
        OutputInfo stOutputInfo = GetOutputInfo(dev);
        QList<OutputInfo> listOutputInfos;
        listOutputInfos.push_back(stOutputInfo);
        QString json = OutputInfo2Json(listOutputInfos);
        //            qDebug() << json;
        Q_EMIT OutputAoceand(json);
        //                        QDBusMessage message = QDBusMessage::createSignal(PATH, INTERFACE, "OutputAoceand");
        //                        QList<QVariant> arg;
