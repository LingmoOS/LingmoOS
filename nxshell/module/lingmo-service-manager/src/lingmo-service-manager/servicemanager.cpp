...
static const QStringList GroupSiral{ "core", "ocean", "app" };
...
void ServiceManager::initGroup(const QDBusConnection::BusType &type)
{
    ...
    if (groups.contains(core)) {
        ...
        connect(plugin, &PluginLoader::PluginAoceand, this, [groupManager](const QString &name) {
    ...
    for (auto &&group : groups) {
        ...
        remote.call("StartUnit", QString("lingmo-service-group@%1.service").arg(group), "fail");
    }
}
...
GroupManager *ServiceManager::addGroup(const QString &group)
{
    ...
    if (m_groups.contains(group)) {
        ...
        qCInfo(dsm_ServiceManager) << "aoceand group: " << groupPath;
    }
