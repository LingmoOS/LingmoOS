...
本篇文章将介绍 Lingmo 后端服务框架如何使用，以及接口、配置的实现。
...
1. 如图所示，lingmo-service-manager 由 systemd 服务拉起来；
...
- <font color=DodgerBlue>group</font>: [可选]插件按进程分组 core|ocean|app，默认分组为 app
...
```shell
/usr/share/lingmo-service-manager/system/demo.json
```

**session**:

```shell
/usr/share/lingmo-service-manager/user/demo.json
```
...
**实现的 so 文件安装路径为 `${CMAKE_INSTALL_LIBDIR}/lingmo-service-manager/`**
...
```shell
/usr/share/lingmo-service-manager/other/demo.json
```
  ...
  "policy": [
    {
      ...
      "interfaces": [
        {
          "interface": "org.lingmo.service.demo",
          "whitelist": "w1",
          // "permission":true,
          "methods": [
            {
...
1. system
   ...
   ```bash
   sudo systemctl restart lingmo-service-manager.service
   ```

2. session

   ```bash
   systemctl --user restart lingmo-service-manager.service
   ```
...
function (install_dbus_service arg)
    if (${ARGC} EQUAL 1)
        file(GENERATE OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${ARGV0}.service CONTENT "[D-BUS Service]\nName=${ARGV0}\nExec=/usr/bin/lingmo-service-manager -n ${ARGV0}\nSystemdService=lingmo-service-plugin@${ARGV0}.service")
...
[D-BUS Service]
Name=org.lingmo.service.demo
Exec=/usr/bin/lingmo-service-manager -n org.lingmo.service.demo
User=root
SystemdService=lingmo-service-plugin@org.lingmo.service.demo.service
```
...
```bash
lingmo-service-group@app.service
```

所以在调试时，只需启动插件服务即可：

```bash
sudo systemctl restart lingmo-service-group@app.service
```

对于非常驻插件，可以单独进程启动：

```bash
sudo systemctl restart lingmo-service-plugin@org.lingmo.service.demo.service
```
...
```bash
sudo journalctl -x -e -u lingmo-service-group@app.service
```

若是临时应用，可按服务名称查看：

```bash
sudo journalctl -x -e -u lingmo-service-plugin@org.lingmo.service.demo.service
```
...
- 2023/03/16:
  - 支持插件以临时进程进行启动
  - 服务名称改名：lingmo-service-plugin -> lingmo-service-group
  - 优化注意事项，建议详细查看注意事项
  - 新增日志查看方式
