...
bool setDisablePluginList()
{
    ...
    process.setArguments(QStringList() << "get" << "com.lingmo.ocean.dock.disableplugins" << "disable-plugins-list");
    process.start();
    ...
    if (endPos > startPos + 1) {
        ...
        if (!list.contains("liboverlay-warning.so")) {
            ...
            process.setArguments(
                    QStringList() << "set" << "com.lingmo.ocean.dock.disableplugins" << "disable-plugins-list" << disable_plugins_list);
            process.start();
            process.waitForFinished(-1);
        ...
        process.setArguments(
                QStringList() << "set" << "com.lingmo.ocean.dock.disableplugins" << "disable-plugins-list" << QString("[\"%1\"]").arg("liboverlay-warning.so"));
        process.start();
        process.waitForFinished(-1);
        err = process.readAllStandardError();
        if (process.exitStatus() != QProcess::NormalExit && process.exitCode() == 0) {
...
bool resetDisablePluginList()
{
    QProcess process;
    process.setProgram("gsettings");
    process.setArguments(QStringList() << "reset" << "com.lingmo.ocean.dock.disableplugins" << "disable-plugins-list");
    process.start();
    // Wait for process to finish without timeout.
    process.waitForFinished(-1);
    QString output = process.readAllStandardOutput();
