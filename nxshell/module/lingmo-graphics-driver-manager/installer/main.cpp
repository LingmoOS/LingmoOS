...
const QString GraphicMangerServiceName = "com.lingmo.graphicmanger";
const QString GraphicMangerPath = "/com/lingmo/graphicmanger";
...
ComLingmoDaemonGraphicsDriverInterface *g_graphicsDriver = nullptr;
...
void show_success_dialog()
{
    ...
    DDialog *d = dialog(message.arg(new_driver), "://resources/icons/lingmo-graphics-driver-manager-success.svg");
...
void show_fail_dialog()
{
    ...
    DDialog *d = dialog(message.arg(new_driver).arg(old_driver), "://resources/icons/lingmo-dialog-warning.svg");
    d->addButton(qApp->translate("main", "Submit Feedback"));

    QObject::connect(d, &DDialog::buttonClicked, [=] { QProcess::startDetached("lingmo-feedback"); });
...
int show_install_dialog() {
    ...
    My_DDialog *installDialog = my_dialog(qApp->translate("main", "Updating the driver, please wait..."), "://resources/icons/lingmo-graphics-driver-manager-installing.svg");
    ...
    if (realInstallReply.isValid()) {
        QObject::connect(g_graphicsDriver, &ComLingmoDaemonGraphicsDriverInterface::ReportProgress, [=](QString ratio){
            if (ratio.toInt() >= 100) {
...
int main(int argc, char *args[])
{
    ...
    dapp.setOrganizationName("lingmo");
    dapp.setApplicationName("lingmo-graphics-driver-manager-installer");

    QTranslator translator;
    translator.load(QString(TRANSLATIONS_DIR"lingmo-graphics-driver-manager_%1.qm").arg(QLocale::system().name()));
    dapp.installTranslator(&translator);

    DLogManager::registerConsoleAppender();
    DLogManager::registerFileAppender();

    g_graphicsDriver = new ComLingmoDaemonGraphicsDriverInterface(
