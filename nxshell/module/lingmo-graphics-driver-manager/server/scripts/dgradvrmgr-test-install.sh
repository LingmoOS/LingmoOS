...
if [[ -z "${isInOverlayRoot}" ]]; then
    rm  -f /etc/xdg/autostart/lingmo-gradvrmgr-test-installer.desktop
...
/usr/sbin/overlayroot-chroot rm -f /etc/xdg/autostart/lingmo-gradvrmgr-test-installer.desktop
...
cp -f ${TEST_INSTALLER_DESKTOP_FILE_SOURCE} /usr/share/applications/lingmo-graphics-driver-manager.desktop
