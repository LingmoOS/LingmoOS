...
int main(int argc, char *argv[])
{
    ...
    QSettings settings("/usr/lib/lingmo-graphics-driver-manager/working-dir/config.conf", QSettings::IniFormat);
    const QString lang = settings.value("lang", "en_US").toString();
    QTranslator trans;
    trans.load(QString(TRANSLATIONS_DIR"/lingmo-graphics-driver-manager_%1.qm").arg(lang));
    app.installTranslator(&trans);

    //设置日志
    const QString logFormat = "%{time}{yyyyMMdd.HH:mm:ss.zzz}[%{type:1}][%{function:-40} %{line:-4} %{threadid:-8} ] %{message}\n";
    const QString log_file(QString("/var/log/lingmo-graphics-driver-manager-server.log"));
