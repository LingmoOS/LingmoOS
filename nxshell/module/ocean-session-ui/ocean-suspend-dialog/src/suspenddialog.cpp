...
{
    org::lingmo::ocean::Display1 *displayInter = new org::lingmo::ocean::Display1("org.lingmo.ocean.Display1", "/org/lingmo/ocean/Display1", QDBusConnection::sessionBus(), this);
    QList<QDBusObjectPath> screenList = displayInter->monitors();
    for (int i = 0; i < screenList.length(); i++) {
        MonitorInter *monitor = new MonitorInter("org.lingmo.ocean.Display1", screenList[i].path(), QDBusConnection::sessionBus());
