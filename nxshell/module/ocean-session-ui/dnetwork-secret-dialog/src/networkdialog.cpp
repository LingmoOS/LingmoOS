...
static const QString NetworkDialogApp = "ocean-network-dialog"; //网络列表执行文件
...
void NetworkDialog::onPassword(QLocalSocket *socket, const QByteArray &data)
{
    ...
    if (m_needIdentity) {
        // 如果从networkcore获取的用户身份信息为空,则使用调用者传进来的数据.(用于兼容ocean-network-core低版本情况)
...
bool NetworkDialog::ConnectToServer()
{
    ...
    QDBusMessage dbusMessage = QDBusMessage::createMethodCall("org.lingmo.ocean.SessionManager1", "/org/lingmo/ocean/SessionManager1", "org.freedesktop.DBus.Properties", "Get");
    dbusMessage << "org.lingmo.ocean.SessionManager1" << "Locked";
