...
using namespace org::lingmo::ocean;
...
int main(int argc, char *argv[])
{
    ...
    if (translator.load("/usr/share/ocean-session-ui/translations/ocean-session-ui_" +
                    QLocale::system().name())) {
    ...
    Display1 display("org.lingmo.ocean.Display1", "/org/lingmo/ocean/Display1", QDBusConnection::sessionBus());
    // 只有一块屏幕时不显示触摸选择提示框
