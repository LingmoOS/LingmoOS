...
const QStringList BlackListDesktopSuffix = { "google-chrome.desktop",
                                             "lingmo-activator.desktop",
                                             "im-launch.desktop",
                                             "polkit-ocean-authentication-agent-1.desktop",
...
ProcessInfoManager::ProcessInfoManager(QObject *parent)
    ...
    , m_startManagerInter(new StartManagerInter("org.lingmo.ocean.StartManager1", "/org/lingmo/ocean/StartManager1", QDBusConnection::sessionBus(), this))
...
void ProcessInfoManager::scanProcessInfos()
{
    ...
    for (auto it(apps.cbegin()); it != apps.cend(); ++it)
        appendCGroupPath(QString("/%1@ocean/uiapps/%2").arg(m_sessionId).arg(it.key()), it.value());
