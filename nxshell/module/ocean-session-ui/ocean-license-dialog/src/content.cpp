...
void Content::updateContent()
{
    ...
    QString tsFilePath;
    if (m_hasCn && m_hasEn) {
        tsFilePath = QString("/usr/share/ocean-session-ui/translations/ocean-session-ui_%1").arg(m_isCn ? "zh_CN" : "en_US");
    } else {
        tsFilePath = QString("/usr/share/ocean-session-ui/translations/ocean-session-ui_%1").arg(QLocale::system().name());
    }
