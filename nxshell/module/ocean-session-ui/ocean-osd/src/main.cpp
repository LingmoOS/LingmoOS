...
{
    if (!(QString(getenv("XDG_CURRENT_DESKTOP")) == QStringLiteral("Lingmo") || QString(getenv("XDG_CURRENT_DESKTOP")) == QStringLiteral("OCEAN"))) {
        qDebug() << "I only run the Lingmo Desktop!";
        return -1;
    }
    ...
    app->setApplicationName("ocean-osd");
    app->setApplicationVersion("1.0");
    if(!app->setSingleInstance(app->applicationName())) {
        qDebug() << "ocean-osd is running...";
        return 1;
    }
    ...
    translator.load("/usr/share/ocean-session-ui/translations/ocean-session-ui_" + QLocale::system().name());
    ...
    connection.interface()->registerService("org.lingmo.ocean.Osd1",
                                            QDBusConnectionInterface::ReplaceExistingService,
                                            QDBusConnectionInterface::AllowReplacement);

    connection.registerObject("/", "org.lingmo.ocean.Osd1", &m, QDBusConnection::ExportAllSlots);
    ...
    OCEANNotifyDBus oceannotify(&manager);
