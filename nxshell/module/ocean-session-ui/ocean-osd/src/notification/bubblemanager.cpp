...
#include "org_lingmo_ocean_display1.h"
#include "org_lingmo_ocean_display1_monitor.h"

Q_LOGGING_CATEGORY(notifiyBubbleLog, "ocean.notifycation.bubblemanger")

using DisplayInter = org::lingmo::ocean::Display1;
using MonitorInter = org::lingmo::ocean::display1::Monitor;
    ...
    , m_trickTimer(new QTimer(this))
{
    ...
    if (useBuiltinBubble()) {
        ...
        m_soundeffectInter = new SoundeffectInter(SoundEffectDaemonDBusServie, SoundEffectDaemonDBusPath,
                                                   QDBusConnection::sessionBus(), this);
        m_appearance = new Appearance("org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1", QDBusConnection::sessionBus(), this);
        m_dockInter = new DBusDockInterface(this);
        m_gestureInter = new GestureInter("org.lingmo.ocean.Gesture1"
                                          , "/org/lingmo/ocean/Gesture1"
...
DCORE_USE_NAMESPACE
    ...
    if (calledFromDBus()) {
        QGSettings oem_setting("com.lingmo.ocean.notifications", "/com/lingmo/ocean/notifications/");
        if (oem_setting.keys().contains("notifycationClosed") && oem_setting.get("notifycationClosed").toBool())
            return 0;

        QGSettings setting("com.lingmo.ocean.osd", "/com/lingmo/ocean/osd/");
    ...
    if (useBuiltinBubble()) {
        ...
        if (!m_displayInter->isValid()) {
            qWarning() << "The name org.lingmo.ocean.Display1 is invalid";
            return 0;
        }
    }
    ...
    if (!calcReplaceId(notification)) {
        ...
        if (systemNotification) { // 系统通知
            ...
            if (showInNotifyCenter) { // 开启在通知中心显示才加入通知中心
                m_persistence->addOne(notification);
                emitRecordAoceand(notification);
            }
        } else { // 锁屏显示通知或者未锁屏状态
...
void BubbleManager::pushBubble(EntityPtr notify)
{
    ...
    if (m_bubbleList.size() == BubbleEntities + BubbleOverLap) {
        ...
        // emit RecordAoceand for oldEntities entity
        emitRecordAoceand(m_bubbleList.last()->entity());
    ...
    for (QPointer<Bubble> &bubble : m_bubbleList) {
        if (bubble) {
            if (bubble->entity()->isShowInNotifyCenter()) {
                m_persistence->addOne(bubble->entity());
                emitRecordAoceand(bubble->entity());
            }
...
{
    static bool isTreeLand = qEnvironmentVariable("OCEAN_CURRENT_COMPOSITOR") == QString("TreeLand");
...
void BubbleManager::emitRecordAoceand(EntityPtr notify)
{
    if (!notify->isShowInNotifyCenter())
        return;
    emitRecordAoceand(notify->storageId());
}

void BubbleManager::emitRecordAoceand(const QString &id)
{
    if (!id.isEmpty())
        Q_EMIT RecordAoceand(id);
}
...
QRect BubbleManager::calcDisplayRect()
{
    ...
    for (const auto &screen : screenList) {
        MonitorInter monitor("org.lingmo.ocean.Display1", screen.path(), QDBusConnection::sessionBus());
...
void BubbleManager::registerAsService()
{
    ...
    oceannotifyConnect.interface()->registerService(OCEANNotifyDBusServer,
                                                  QDBusConnectionInterface::ReplaceExistingService,
                                                  QDBusConnectionInterface::AllowReplacement);
    oceannotifyConnect.registerObject(OCEANNotifyDBusPath, this);
}
...
void BubbleManager::initConnections()
{
    ...
    connect(m_notifySettings, &NotifySettings::appAoceandSignal, this, [ = ] (const QString &id) {
        Q_EMIT AppAoceandSignal(id);
        Q_EMIT appAoceand(m_notifySettings->getAppSettings_v1(id));
    });
    ...
    if (notify->replacesId() == NoReplaceId) {
        ...
        for (int i = 0; i < m_bubbleList.size(); ++i) {
            ...
            if (bubble->entity()->replacesId() == notify->replacesId()
                ...
                // emit RecordAoceand for replaced entity
                emitRecordAoceand(m_bubbleList.at(i)->entity());
                if (i != 0) {
        ...
        for (int i = 0; i < m_oldEntities.size(); ++i) {
            if (m_oldEntities.at(i)->replacesId() == notify->replacesId()
                ...
                // emit RecordAoceand for notProcessedYet entity
                emitRecordAoceand(m_oldEntities.at(i));
