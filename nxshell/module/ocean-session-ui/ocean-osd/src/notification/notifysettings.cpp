...
const QString schemaKey = "com.lingmo.ocean.notifications";
const QString schemaPath = "/com/lingmo/ocean/notifications/";
const QString appSchemaKey = "com.lingmo.ocean.notifications.applications";
const QString appSchemaPath = "/com/lingmo/ocean/notifications/applications/%1/";
...
static LauncherItemInfo fromObjectInterfaceMapToItemInfo(const QDBusObjectPath &path, const ObjectInterfaceMap &valueMap)
{
    ...
    for (const QVariantMap &mapInter : valueMap) {
        ...
        info.icon = icon;
        bool isLingmoVendor = qdbus_cast<QString>(mapInter["X_Lingmo_Vendor"]) == QStringLiteral("lingmo");
                                              ...
                                              this))
{
    ...
    if (!QGSettings::isSchemaInstalled("com.lingmo.ocean.notification")) {
    ...
    connect(m_applicationObjectInter, &ApplicationObjectManager1::InterfacesAoceand, this, [this](const QDBusObjectPath &object_path, ObjectInterfaceMap interfaces) {
        LauncherItemInfo info = fromObjectInterfaceMapToItemInfo(object_path, interfaces);
        qCInfo(SessionNotifySettings) << "Iterface aoceand, aoceand id=" << info.id;
        if (info.noDisplay) {
            return;
        }
        appAoceand(info);
...
void NotifySettings::appAoceand(const LauncherItemInfo &info)
{
    QStringList appList = m_systemSetting->get("app-list").toStringList();
    ...
    Q_EMIT appAoceandSignal(info.id);
}
