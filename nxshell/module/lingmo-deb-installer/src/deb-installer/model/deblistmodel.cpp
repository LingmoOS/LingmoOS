...
void DebListModel::slotShowDevelopModeWindow()
{
    ...
    if (osVerStr == "20") {  // V20模式
        QDBusInterface interface("com.lingmo.ocean.ControlCenter", "/com/lingmo/ocean/ControlCenter", "com.lingmo.ocean.ControlCenter");
        ...
        if (unlock->exitCode() != QProcess::NormalExit) {
            QDBusInterface interface(
                "org.lingmo.ocean.ControlCenter1", "/org/lingmo/ocean/ControlCenter1", "org.lingmo.ocean.ControlCenter1");
            if (interface.isValid()) {
                interface.call("ShowPage", "commoninfo", "Developer Mode");
            }
        ...
        qWarning() << qPrintable("Unknown OS version, connot open ocean-control-center");
    }
    ...
    if (dependStatus.canInstallCompatible() && supportCompatible()) {
        ...
        if (Utils::checkPackageContainsDebConf(sPackageName)) {  // 检查当前包是否需要配置
            m_procInstallConfig->start("pkexec",
                                       QStringList() << "pkexec"
                                                     << "lingmo-deb-installer-dependsInstall"
