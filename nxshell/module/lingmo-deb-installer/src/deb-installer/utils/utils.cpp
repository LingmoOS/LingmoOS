...
Utils::VerifyResultCode Utils::Digital_Verify(const QString &filepath_name)
{
    ...
    QString verifyfilename = "lingmo-deb-verify";
    ...
    if (result_verify_file) {
        QProcess proc;
        QString program = "/usr/bin/lingmo-deb-verify";
...
QIcon Utils::packageIcon(Pkg::PackageType type)
{
    if (Pkg::Uab == type) {
        ...
        if (kUabIcon.isNull()) {
            ...
            Dtk::Gui::DDciIcon dciIcon(QString(":/icons/lingmo/uab/uos-application-bundle.dci"));
