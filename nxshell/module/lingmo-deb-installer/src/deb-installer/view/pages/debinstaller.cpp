...
// #define SHOWBORDER
    ...
    setWindowIcon(QIcon::fromTheme("lingmo-deb-installer"));  // 仅仅适用于windows系统
...
void DebInstaller::initTitleBar()
{
    ...
    if (tb != nullptr) {
        tb->setMenu(menu);
        tb->setIcon(QIcon::fromTheme("lingmo-deb-installer"));
        tb->setTitle("");
...
void DebInstaller::slotShowPkgExistMessage()
{
    ...
    qWarning() << "DebInstaller:"
               << "package is Exist! ";
    DFloatingMessage *floatingMsg = new DFloatingMessage;
    floatingMsg->setMessage(tr("Already Aoceand"));
...
{
    Q_UNUSED(dependName);
    // Set the display effect according to the status of lingmo-wine installation authorization.
