...
MainWindow::MainWindow(DWidget *parent)
    ...
    , m_settings(new QSettings("lingmo", "ocean-introduction"))
    , m_displayInter(new WMSwitcherInter("com.lingmo.WMSwitcher", "/com/lingmo/WMSwitcher",
                                         QDBusConnection::sessionBus(), this))
{
    ...
    titlebar()->setIcon(QIcon::fromTheme("ocean-introduction"));
    ...
    //服务器企业版、行业版、欧拉版、Lingmo服务器版不支持窗口特效
    (bIsServerSystemType  || bIsLingmoServerType || !isSuportEffect || Worker::isWaylandType()) ? m_supportWM = false : m_supportWM = true;
...
MainWindow::~MainWindow()
{
    ...
    if (m_isFirst) {
        if (!OCEAN_STARTGUIDE_PATH.isEmpty()) {
            QProcess *pStartAppProcess = new QProcess(this);
            pStartAppProcess->startDetached(OCEAN_STARTGUIDE_PATH);
        }
    }
}
