...
Q_LOGGING_CATEGORY(SHUTDOWN, "org.lingmo.ocean.dock.shutdown")
...
ShutdownPlugin::ShutdownPlugin(QObject *parent)
    ...
    , m_powerManagerInter(new DBusPowerManager("org.lingmo.daemon.PowerManager1", "/org/lingmo/daemon/PowerManager1", QDBusConnection::systemBus(), this))
    , m_dconfig(DConfig::create("org.lingmo.ocean.tray-loader", "org.lingmo.ocean.dock.plugin.shutdown", QString(), this))
    , m_lastoreDConfig(DConfig::create("org.lingmo.lastore", "org.lingmo.lastore", "", this))
{
...
void ShutdownPlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    // transfer config
    QSettings settings("lingmo", "ocean-dock-shutdown");
...
bool ShutdownPlugin::pluginIsDisable()
{
    ...
    auto dconfig = Dtk::Core::DConfig::create("org.lingmo.ocean.tray-loader", "org.lingmo.ocean.dock.plugin.common", "");
    if (dconfig) {
...
const QString ShutdownPlugin::itemContextMenu(const QString &itemKey)
{
    ...
    if (!QFile::exists(ICBC_CONF_FILE)) {
        // 读取com.lingmo.ocean.session-shell切换用户配置项
        enum SwitchUserConfig {
            AlwaysShow = 0,
            OnDemand,
            Disabled
        } switchUserConfig = OnDemand;
#if 0 // TODO 等待v25 ocean-session-shell重构添加配置为dconfig
...
void ShutdownPlugin::invokedMenuItem(const QString &itemKey, const QString &menuId, const bool checked)
{
    ...
    if (menuId == "power") {
        ...
        if (QFile::exists(ICBC_CONF_FILE)) {
            QDBusMessage send = QDBusMessage::createMethodCall("org.lingmo.ocean.LockFront1", "/org/lingmo/ocean/LockFront1", "org.lingmo.ocean.LockFront1", "SwitchTTYAndShow");
            ...
            QProcess::execute("bash -c \"originmap=$(setxkbmap -query | grep option | awk -F ' ' '{print $2}');/usr/bin/setxkbmap -option grab:break_actions&&/usr/bin/xdotool key XF86Ungrab&&dbus-send --print-reply --dest=org.lingmo.ocean.LockFront1 /org/lingmo/ocean/LockFront1 org.lingmo.ocean.LockFront1.Show&&setxkbmap -option $originmap\"");
        }
    } else
        DDBusSender()
