...
SoundController::SoundController(QObject *parent)
    : QObject(parent)
    , m_audioInter(new DBusAudio("org.lingmo.ocean.Audio1", "/org/lingmo/ocean/Audio1", QDBusConnection::sessionBus(), this))
    , m_defaultSinkInter(nullptr)
    , m_dconfig(DConfig::create("org.lingmo.ocean.tray-loader", "org.lingmo.ocean.dock.plugin.sound", QString(), this))
{
...
void SoundController::onDefaultSinkChanged(const QDBusObjectPath &path)
{
    ...
    m_defaultSinkInter = new DBusSink("org.lingmo.ocean.Audio1", path.path(), QDBusConnection::sessionBus(), this);
