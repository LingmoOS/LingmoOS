...
const QString NotificationPlugin::itemCommand(const QString &itemKey)
{
    ...
    if (!newNotifications()) {
        return QString("dbus-send --session --print-reply --dest=org.lingmo.ocean.Widgets1 /org/lingmo/ocean/Widgets1 org.lingmo.ocean.Widgets1.Toggle");
    }
    return QString("dbus-send --session --print-reply --dest=org.lingmo.ocean.shell /org/lingmo/ocean/shell/notification/center org.lingmo.ocean.shell.notification.center.Toggle");
}
...
void NotificationPlugin::invokedMenuItem(const QString &itemKey, const QString &menuId, const bool checked)
{
    ...
    if (menuId == TOGGLE_DND) {
        ...
        DDBusSender().service("org.lingmo.ocean.ControlCenter1")
