...
void Notification::watchNotification(bool newNotification)
{
    auto ret = QtConcurrent::run([this, newNotification](){
        m_dbus.reset(new QDBusInterface("org.lingmo.ocean.Notification1", "/org/lingmo/ocean/Notification1", "org.lingmo.ocean.Notification1"));
        // Refresh icon for the first time, cause org.lingmo.ocean.Notification1 might depend on dock's DBus,
        // we should not call org.lingmo.ocean.Notification1 in the main thread before dock's dbus is initialized.
        ...
        QDBusConnection::sessionBus().connect("org.lingmo.ocean.Notification1",
