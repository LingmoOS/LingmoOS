...
using SysPowerInter = org::lingmo::ocean::Power1;
...
private:
    PerformanceModeController()
        ...
        , m_sysPowerInter(new SysPowerInter("org.lingmo.ocean.Power1", "/org/lingmo/ocean/Power1", QDBusConnection::systemBus(), this))
    {
        ...
        QDBusInterface interface("org.lingmo.ocean.Power1",
                                 "/org/lingmo/ocean/Power1",
        ...
        for (const auto &pair : powerModeList) {
            bool powerModeData = getPowerModeProperty(interface, "org.lingmo.ocean.Power1", powerModeProperty.value(pair.first));
            if (powerModeData) {
        ...
        QDBusMessage reply = interface.call("Get", "org.lingmo.ocean.Power1", "Mode");
        QList<QVariant> outArgs = reply.arguments();
        if (outArgs.length() > 0) {
