...
插件是在不需要改动并重新编译主程序本身的情况下扩展主程序功能的一种机制。ocean-dock 插件是根据 Qt 插件标准所开发的共享库文件(.so)，可以通过实现 Qt 的插件标准和 ocean-dock 提供的接口来完成 ocean-dock 的功能扩展。可以通过以下链接查看关于 Qt 插件更详细的介绍：
...
在 ocean-dock 启动时会启动一个线程去检测目录 `/usr/lib/ocean-dock/plugins` 下的所有文件，并检测是否是一个正常的动态库文件，如果是则尝试加载。尝试加载即检测库文件的元数据，插件的元数据定义在一个 JSON 文件中，这个后文会介绍。如果元数据检测通过就开始检查插件是否实现了 ocean-dock 指定的接口，这一步也通过之后就会开始初始化插件，获取插件提供的控件，进而将控件显示在任务栏上。

## 接口列表

这里先介绍 ocean-dock 都提供了哪些接口，可作为一个手册查看。注意，为 ocean-dock 编写插件并不是要实现所有接口，这些接口提供了 ocean-dock 允许的各种可能的功能，插件开发者可以根据需求实现自己需要的接口。后续的插件示例也会用到这里列出的部分接口。接口定义的文件一般在系统如下位置：

> /usr/include/ocean-dock/pluginproxyinterface.h
> /usr/include/ocean-dock/pluginsiteminterface.h
> /usr/include/ocean-dock/constants.h
...
| 名称                 | 简介                                                                       | 必须实现 |
| -------------------- | -------------------------------------------------------------------------- | -------- |
| pluginName           | 返回插件名称，用于在 ocean-dock 内部管理插件时使用                           | 是       |
| pluginDisplayName    | 返回插件名称，用于在界面上显示                                             | 否       |
| init                 | 插件初始化入口函数，参数 proxyInter 可认为是主程序本身                     | 是       |
| itemWidget           | 返回插件主控件，用于显示在 ocean-dock 面板上                                 | 是       |
...
| displayModeChanged   | ocean-dock 显示模式发生改变时此接口被调用                                    | 否       |
| positionChanged      | ocean-dock 位置变化时时此接口被调用                                          | 否       |
| refreshIcon          | 当插件控件的图标需要更新时此接口被调用                                     | 否       |
| displayMode          | 用于插件主动获取 ocean-dock 当前的显示模式                                   | 否       |
| position             | 用于插件主动获取 ocean-dock 当前的位置                                       | 否       |

### PluginProxyInterface

由于上面介绍的接口对于插件来说基本都是被动的，即插件本身无法确定这些接口什么时候会被调用，很明显对于插件机制来说这是不完整的，因此便有了 PluginProxyInterface。它定义了一些让插件主动调用以控制 ocean-dock 的一些行为的接口。

PluginProxyInterface 的具体实例可以认为是抽象了的 ocean-dock 主程序，或者是 ocean-dock 中所有插件的管理员，这个实例将会通过 PluginsItemInterface 中的 init 接口传递给插件，因此在上述 init 接口中总是会先把这个传入的对象保存起来以供后续使用。PluginProxyInterface 类中的接口如下表所示。

| 名称                        | 简介                                                                                                                               |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| itemAoceand                   | 向 ocean-dock 添加新的主控件（一个插件可以添加多个主控件，它们之间使用 itemKey 来区分）                                              |
| itemUpdate                  | 通知 ocean-dock 有主控件需要更新                                                                                                     |
| itemRemoved                 | 从 ocean-dock 移除主控件                                                                                                             |
| requestWindowAutoHide       | 设置 ocean-dock 是否允许隐藏，通常用在任务栏被设置为智能隐藏或始终隐藏而插件又需要让 ocean-dock 保持显示状态来显示一些重要信息的场景下 |
| requestRefreshWindowVisible | 通知 ocean-dock 更新隐藏状态                                                                                                         |
| requestSetAppletVisible     | 通知 ocean-dock 显示或隐藏插件的弹出面板（鼠标左键点击后弹出的控件）                                                                 |
| saveValue                   | 统一的配置保存函数                                                                                                                 |
| getValue                    | 统一的配置读取函数                                                                                                                 |

## 构建一个 ocean-dock 插件

接下来将介绍一个简单的 ocean-dock 插件的开发过程，插件开发者可据此熟悉 ocean-dock 插件开发的步骤，以便创造出更多具有丰富功能的插件。
...
pkg_check_modules(DdeDockInterface REQUIRED ocean-dock)
...
`home-monitor.json`文件是插件的元数据文件，指明了当前插件所使用的 ocean-dock 的接口版本。ocean-dock 在加载此插件时，会检测自己的接口版本是否与插件的接口版本一致，当双方的接口版本不一致或者不兼容时，ocean-dock 为了安全将阻止加载对应的插件。另外，元数据文件是在源码中使用特定的宏加载到插件中的。在 ocean-dock 内建的插件代码中，可以找到当前具体的接口版本，在本书编写时最新 的版本是 1.2。代码如下:
...
另外还可以指定 D-Bus 服务(可选的，且仅支持 session D-Bus)，ocean-dock 在加载插件时会检查此插件所依赖的 D-Bus 服务，如果服务没有启动则不会初始化这个插件，直到服务启动。下面的代码表示依赖 dbus 地址为“com.lingmo.daemon.Network”的 D-Bus 服务。代码如下：

```json
{
  "api": "1.2",
  "depends-daemon-dbus-service": "com.lingmo.daemon.Network"
}
```

#### 插件核心类

`homemonitorplugin.h` 声明了类 `HomeMonitorPlugin`，它继承（实现）了前面提到的 `PluginItemInterface`，这代表它是一个实现了 ocean-dock 接口的插件。

下面是最小化实现了 ocean-dock 接口的插件的源码，只实现了必须实现的接口。注意:下文的代码只是为了简述开发插件的主要过程，详细的示例代码可查看 home-monitor 目录下的内容。

```c++
#ifndef HOMEMONITORPLUGIN_H
#define HOMEMONITORPLUGIN_H

#include <ocean-dock/pluginsiteminterface.h>
...
class HomeMonitorPlugin : public QObject, PluginsItemInterface
{
    ...
    // 插件元数据
    Q_PLUGIN_METADATA(IID "com.lingmo.dock.PluginsItemInterface" FILE "home-monitor.json")
...
void HomeMonitorPlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    m_proxyInter->itemAoceand(this, pluginName());
}
...
QWidget *HomeMonitorPlugin::itemWidget(const QString &itemKey)
{
    ...
    if (!item) {
        ...
        item->setPixmap(QIcon::fromTheme("lingmo-diskmanager").pixmap(QSize(20, 20))); // QIcon::fromTheme会从主题里面查找图标
    }
...
插件的基本结构搭建好之后，应该测试这个插件能否被 ocean-dock 正确加载，这时候如果测试出有问题可以及时处理。
...
可以看到有 `home-monitor.so`文件被安装在了 ocean-dock 的插件目录。

```sh
install -m 755 -p ./home-monitor/libhome-monitor.so /usr/lib/ocean-dock/plugins/libhome-monitor.so
```

或者将 so 文件放在~/.local/lib/ocean-dock/plugins/目录下，dock 会加载用户本地的插件。

```sh
install -m 755 -p ./home-monitor/libhome-monitor.so ~/.local/lib/ocean-dock/plugins/libhome-monitor.so
```

#### 测试加载

执行 `pkill ocean-dock` 命令“杀死”任务栏，看门狗会自动启动任务栏，此时任务栏上会多了一个磁盘样式的插件。此时这个插件只能展示一个简单的图标，没有任何其它的功能，下面将增加弹窗、右键菜单、提示等功能，来完成我们的预期功能。

### 实现预期功能

#### 创建插件主控件

创建新文件 informationwidget.h 和 informationwidget.cpp，用于创建控件类：InformationWidget，这个控件用于显示在 ocean-dock 上。
...
#### 添加主控件到 ocean-dock 面板上

在插件核心类的 init 方法中，获取到了 PluginProxyInterface 对象，调用此对象的 itemAoceand 接口即可实现向 ocean-dock 面板添加项目。第二个参数为 QString 类型，代表了本插件所提供的主控件的 ID，当一个插件提供多个主控件时，不同主控件之间要保证 ID 唯一。代码如下:
...
调用 itemAoceand 之后，ocean-dock 会在合适的时机调用插件的 itemWidget 接口以获取需要显示的控件。如果插件提供多个主控件到 ocean-dock 上，那么插件核心类应该在 itemWidget 接口中分析参数 itemKey，并返回与之对应的控件对象，当插件只有一个可显示项目时，itemKey 可以忽略(但不建议忽略)。代码如下:
...
编译、安装、重启 ocean-dock，就可以看到主控件在 ocean-dock 面板上出现了，如下图所示：
...
此时就会引入一个新的问题，插件允许被禁用，那么在 ocean-dock 启动时，插件有可能处于禁用状态，那么在初始化插件时就不能直接将主控件添加到 ocean-dock 中，而是应该判断当前是否是禁用状态，修改接口 init 的实现：
...
ocean-dock 发现鼠标悬停在插件上时，会调用这个接口获取控件并显示出来，如下图所示：
...
编译，安装，重启 ocean-dock，点击主控件即可看到弹出的 applet 控件，如下图所示：
...
编译，安装，重启 ocean-dock 之后右键点击主控件即可看到弹出右键菜单，如下图所示：
