...
Q_LOGGING_CATEGORY(EYE_COMFORT, "org.lingmo.ocean.dock.eye-comfort-mode")
...
void EyeComfortModePlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    connect(&EyeComfortModeController::ref(), &EyeComfortModeController::supportColorTemperatureChanged, this, [this] (bool support) {
        support ? m_proxyInter->itemAoceand(this, EYE_COMFORT_MODE_KEY) : m_proxyInter->itemRemoved(this, EYE_COMFORT_MODE_KEY);
        notifySupportFlagChanged(support);
    });
    if (!pluginIsDisable()) {
        m_proxyInter->itemAoceand(this, EYE_COMFORT_MODE_KEY);
    }
