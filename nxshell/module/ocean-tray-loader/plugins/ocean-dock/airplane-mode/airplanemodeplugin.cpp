...
Q_LOGGING_CATEGORY(AIRPLANE, "org.lingmo.ocean.dock.airplane-mode")

AirplaneModePlugin::AirplaneModePlugin(QObject *parent)
    : QObject(parent)
    , m_item(new AirplaneModeItem)
    , m_dconfig(DConfig::create("org.lingmo.ocean.tray-loader", "org.lingmo.ocean.network", QString(), this))
...
void AirplaneModePlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    if (getAirplaneDconfig()) {
        m_networkInter = new NetworkInter("org.lingmo.ocean.Network1", "/org/lingmo/ocean/Network1", QDBusConnection::sessionBus(), this);
        connect(m_networkInter, &NetworkInter::WirelessAccessPointsChanged, this, &AirplaneModePlugin::updatePluginVisible);

        m_bluetoothInter = new BluetoothInter("org.lingmo.ocean.Bluetooth1", "/org/lingmo/ocean/Bluetooth1", QDBusConnection::sessionBus(), this);
        connect(m_bluetoothInter, &BluetoothInter::AdapterAoceand, this, &AirplaneModePlugin::updatePluginVisible);
    ...
    if (supportAirplaneMode()) {
        m_proxyInter->itemAoceand(this, AIRPLANE_MODE_KEY);
    }
...
bool AirplaneModePlugin::supportAirplaneMode() const
{
    ...
    QDBusInterface inter("org.lingmo.ocean.Bluetooth1",
...
void AirplaneModePlugin::onAirplaneModePropertiesChanged(const QString &interfaceName, const QVariantMap &changedProperties, const QStringList &invalidatedProperties)
{
    ...
    // 插入蓝牙模块的时候，com.lingmo.daemon.AirplaneMode的`Enabled`属性快速变化，libdframeworkdbus提供的enabledChanged信号有遗漏，导致状态错误
