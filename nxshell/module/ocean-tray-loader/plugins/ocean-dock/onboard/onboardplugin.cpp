...
void OnboardPlugin::invokedMenuItem(const QString &itemKey, const QString &menuId, const bool checked)
{
    ...
    DBusDock DockInter("com.lingmo.ocean.daemon.Dock",
                       "/com/lingmo/ocean/daemon/Dock",
                       QDBusConnection::sessionBus(), this);
    const auto &entries = DockInter.entries();
    for (const auto &entry : entries) {
        DockEntryInter AppInter("com.lingmo.ocean.daemon.Dock",
                                entry.path(),
                                QDBusConnection::sessionBus(), this);
