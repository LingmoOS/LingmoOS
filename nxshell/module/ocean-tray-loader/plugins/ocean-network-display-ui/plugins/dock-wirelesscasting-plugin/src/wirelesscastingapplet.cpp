...
void WirelessCastingApplet::initUI()
{
    ...
    m_displaySetting->setOceanUIModule("display");
    ...
    , m_disconnMonitor(new CancelButton)
{
    ...
    auto updateThemeType = [this] (DGuiApplicationHelper::ColorType type) {
        if (type == DGuiApplicationHelper::ColorType::LightType) {
            m_iconConnected->setPixmap(DIcon::loadNxPixmap(":/icons/lingmo/builtin/light/icons/success_128px.svg"));
            m_iconNoDevice->setPixmap(DIcon::loadNxPixmap(":/icons/lingmo/builtin/light/icons/none_128px.svg"));
        } else {
            m_iconConnected->setPixmap(DIcon::loadNxPixmap(":/icons/lingmo/builtin/dark/icons/success_128px.svg"));
            m_iconNoDevice->setPixmap(DIcon::loadNxPixmap(":/icons/lingmo/builtin/dark/icons/none_128px.svg"));
        }
    };
