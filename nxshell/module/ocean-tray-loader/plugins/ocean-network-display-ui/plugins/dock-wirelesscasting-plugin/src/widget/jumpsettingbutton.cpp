...
void JumpSettingButton::mouseReleaseEvent(QMouseEvent* event)
{
    if (underMouse()) {
        Q_EMIT clicked();
        if (m_autoShowPage && !m_oceanuiModule.isEmpty()) {
            DDBusSender()
...
void JumpSettingButton::setOceanUIModule(const QString &module)
{
    m_oceanuiModule = module;
}
