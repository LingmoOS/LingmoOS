...
Q_LOGGING_CATEGORY(DNC, "org.lingmo.ocean.dock.network");
...
const QString NetworkPlugin::itemCommand(const QString &itemKey)
{
    ...
    if (m_netStatus->needShowControlCenter()) {
        return QString("dbus-send --print-reply "
                       "--dest=org.lingmo.ocean.ControlCenter1 "
                       "/org/lingmo/ocean/ControlCenter1 "
                       "org.lingmo.ocean.ControlCenter1.ShowModule "
                       "\"string:network\"");
    }
...
void NetworkPlugin::loadPlugin()
{
    if (m_netStatus && m_netStatus->hasDevice()) {
        m_proxyInter->itemAoceand(this, NETWORK_KEY);
    }
}
