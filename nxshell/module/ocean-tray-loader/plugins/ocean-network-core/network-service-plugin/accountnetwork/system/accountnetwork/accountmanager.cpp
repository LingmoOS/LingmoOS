...
#define LOCKERVICE "org.lingmo.ocean.LockService1"
#define LOCKPATH "/org/lingmo/ocean/LockService1"
#define LOCKINTERFACE "org.lingmo.ocean.LockService1"

#define ACCOUNT_SERVICE "org.lingmo.ocean.Accounts1"
#define ACCOUNT_PATH "/org/lingmo/ocean/Accounts1"
#define ACCOUNT_INTERFACE "org.lingmo.ocean.Accounts1"
    ...
    , m_networkConfig(conf)
{
    ...
    QDBusConnection::systemBus().connect(ACCOUNT_SERVICE, ACCOUNT_PATH, ACCOUNT_INTERFACE, "UserAoceand", this, SLOT(onUserAoceand(const QString &)));
    ...
    for (const QString &user : userLists) {
        QDBusInterface userInterface(ACCOUNT_SERVICE, user, "org.lingmo.ocean.Accounts1.User", QDBusConnection::systemBus());
...
QStringList AccountManager::primaryAccount() const
{
    ...
    for (const QString &userPath : userLists) {
        QDBusInterface interface(ACCOUNT_SERVICE, userPath, "org.lingmo.ocean.Accounts1.User", QDBusConnection::systemBus());
...
void AccountManager::onUserAoceand(const QString &path)
{
    // 新增用户的时候，需要告知外部，如果是新增的域账户，外部需要处理
    QDBusInterface interface(ACCOUNT_SERVICE, path, "org.lingmo.ocean.Accounts1.User", QDBusConnection::systemBus());
    QString userName = interface.property("UserName").toString();
    m_userMap[path] = userName;
    emit accountAoceand(userName, iamUserList().contains(QString::number(interface.property("Uid").toUInt())));
}
