...
NetSecretAgent::NetSecretAgent(PasswordCallbackFunc fun, bool greeter, QObject *parent)
    : NetworkManager::SecretAgent(QStringLiteral("com.lingmo.system.network.SecretAgent"), parent)
    , NetSecretAgentInterface(fun)
{
    m_greeter = greeter;
    qCInfo(DNC) << "Register secret agent, com.lingmo.system.network.SecretAgent mode: " << (m_greeter ? "greeter" : "lock");
