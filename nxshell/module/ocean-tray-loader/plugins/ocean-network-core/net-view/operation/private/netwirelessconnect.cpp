...
NetWirelessConnect::NetWirelessConnect(ocean::network::WirelessDevice *device, ocean::network::AccessPoints *ap, QObject *parent)
    : QObject(parent)
    , m_device(device)
    , m_accessPoint(ap)
    , m_needUpdate(false)
...
WirelessSecuritySetting::KeyMgmt NetWirelessConnect::getKeyMgmtByAp(ocean::network::AccessPoints *ap)
{
    if (nullptr == ap) {
