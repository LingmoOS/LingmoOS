...
HotspotControllerInter::HotspotControllerInter(NetworkInter *networkInter, QObject *parent)
...
HotspotControllerInter::~HotspotControllerInter()
{
}

void HotspotControllerInter::setEnabled(WirelessDevice *device, const bool enable)
{
...
bool HotspotControllerInter::enabled(WirelessDevice *device)
{
    return device->hotspotEnabled();
}

bool HotspotControllerInter::supportHotspot()
{
    return m_devices.size() > 0;
}

void HotspotControllerInter::connectItem(HotspotItem *item)
{
    // 获取当前连接的UUID和设备的path
    m_networkInter->ActivateConnection(item->connection()->uuid(), QDBusObjectPath(item->devicePath()));
}

void HotspotControllerInter::connectItem(WirelessDevice *device, const QString &uuid)
{
...
void HotspotControllerInter::disconnectItem(WirelessDevice *device)
{
...
QList<HotspotItem *> HotspotControllerInter::items(WirelessDevice *device)
{
    // 获取当前设备对应的所有的热点
...
QList<WirelessDevice *> HotspotControllerInter::devices()
{
    return m_devices;
}

HotspotItem *HotspotControllerInter::findItem(WirelessDevice *device, const QJsonObject &json)
{
    // 从列表中查找指定UUID的连接
    for (HotspotItem *item : m_hotspotItems) {
        if (item->device() == device
...
bool HotspotControllerInter::isHotspotConnection(const QString &uuid)
{
    for (HotspotItem *item : m_hotspotItems) {
...
void HotspotControllerInter::updateActiveConnection(const QJsonObject &activeConnections)
{
...
WirelessDevice *HotspotControllerInter::findDevice(const QString &path)
{
...
void HotspotControllerInter::updateDevices(const QList<NetworkDeviceBase *> &devices)
{
    ...
    if (newDevices.size() > 0)
        Q_EMIT deviceAoceand(newDevices);
...
void HotspotControllerInter::updateConnections(const QJsonArray &jsons)
{
    qCDebug(DNC) << jsons;
