...
Q_SIGNALS:
    void deviceAoceand(QList<NetworkDeviceBase *>);                                        // 新增设备发出的信号
...
Q_SIGNALS:
    ...
    void removed();                                                                      // 设备移除
    // 无线网络信号
    void networkAoceand(const QList<AccessPoints *> &);                                    // wlan新增网络
    ...
    void wirelessConnectionAoceand(const QList<WirelessConnection *> &);                     // 新增连接
...
public:
    ...
    virtual bool hioceann() const = 0;                                            // 是否为隐藏网络
