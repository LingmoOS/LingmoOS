...
{
    if (!dConfig)
        dConfig = Dtk::Core::DConfig::create("org.lingmo.ocean.network", "org.lingmo.ocean.network");

    // if (!gSettingsConfig && QGSettings::isSchemaInstalled("com.lingmo.ocean.control-center"))
    //     gSettingsConfig = new QGSettings("com.lingmo.ocean.control-center", QByteArray(), this);

    // if (!gAirplaneConfig && QGSettings::isSchemaInstalled("com.lingmo.ocean.dock.module.airplane-mode"))
    //     gAirplaneConfig = new QGSettings("com.lingmo.ocean.dock.module.airplane-mode", QByteArray(), this);
    ...
    // if (gSettingsConfig) {
    //     connect(gSettingsConfig, &QGSettings::changed, this, &ConfigWatcher::onOceanUIConfigChanged);
...
void ConfigWatcher::onOceanUIConfigChanged(const QString &key)
{
    if (key == "wireless") {
...
ocean::network::ProxyMethod ConfigWatcher::proxyMethod() const
{
    if (m_lastProxyMethod == "auto")
        return ocean::network::ProxyMethod::Auto;

    return ocean::network::ProxyMethod::Manual;
}

void ConfigWatcher::setProxyMethod(const ocean::network::ProxyMethod &method)
{
    if (dConfig && dConfig->keyList().contains("lastProxyMethod")) {
        dConfig->setValue("lastProxyMethod", (method == ocean::network::ProxyMethod::Auto ? "auto" : "manual"));
    }
}
