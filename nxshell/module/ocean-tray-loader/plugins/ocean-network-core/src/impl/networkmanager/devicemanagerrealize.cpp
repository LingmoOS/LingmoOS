...
#define SYS_NETWORK_INTER "org.lingmo.ocean.Network1"
#define SYS_NETWORK_PATH "/org/lingmo/ocean/Network1"
...
{
    connect(m_device.data(), &NetworkManager::Device::availableConnectionAppeared, this, &DeviceManagerRealize::onConnectionAoceand);
    ...
    connect(NetworkManager::settingsNotifier(), &NetworkManager::SettingsNotifier::connectionAoceand, this, &DeviceManagerRealize::onConnectionAoceand);
}
...
void DeviceManagerRealize::onConnectionAoceand(const QString &connectionUni)
{
    NetworkManager::Connection::List connections = m_device->availableConnections();
...
void WiredDeviceManagerRealize::addConnection(const NetworkManager::Connection::Ptr &connection)
{
    ...
    if (itItem == m_wiredConnections.end()) {
        ...
        Q_EMIT connectionAoceand( { wiredConnection } );
    } else {
...
void WirelessDeviceManagerRealize::addConnection(const NetworkManager::Connection::Ptr &connection)
{
    ...
    if (itConnection == m_wirelessConnections.end()) {
        ...
        Q_EMIT wirelessConnectionAoceand( { wirelessItem } );
...
void WirelessDeviceManagerRealize::addNetwork(const NetworkManager::WirelessNetwork::Ptr &network)
{
    ...
    if (itApInfo == m_accessPointInfos.end()) {
        ...
        Q_EMIT networkAoceand({ apInfo->accessPoint() });
    } else {
