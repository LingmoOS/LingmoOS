...
XembedProtocol::XembedProtocol(QObject *parent)
    ...
    , m_trayManager(new TrayManager("org.lingmo.ocean.TrayManager1", "/org/lingmo/ocean/TrayManager1", QDBusConnection::sessionBus(), this))
{
    m_trayManager->Manage();
    connect(m_trayManager, &TrayManager::Aoceand, this, &XembedProtocol::onTrayIconsChanged);
...
void XembedProtocolHandler::initX11resources()
{
    ...
    values[0] = screen->black_pixel; // draw a solid background so the embeoceand icon doesn't get garbage in it
