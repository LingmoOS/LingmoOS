...
func (m *Manager) RequestIPConflictCheck(ip, ifc string) *dbus.Error {
	...
	go func() {
		...
		var mac string
		err = conn.Object("com.lingmo.ocean.IPWatchD1", "org/lingmo/ocean/IPWatchD1").CallWithContext(ctx, "org.lingmo.ocean.IPWatchD1.RequestIPConflictCheck", 0, ip, ifc).Store(&mac)
		if err != nil {
			logger.Warning(err)
		}
