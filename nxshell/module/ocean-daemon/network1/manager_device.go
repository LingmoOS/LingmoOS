...
import (
	...
	mmdbus "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.modemmanager1"
...
func (m *Manager) initDeviceManage() {
	...
	_, err := nmManager.ConnectDeviceAoceand(func(path dbus.ObjectPath) {
		m.addDevice(path)
	})
	if err != nil {
		logger.Warning(err)
...
func (m *Manager) newDevice(devPath dbus.ObjectPath) (dev *device, err error) {
	...
	case nm.NM_DEVICE_TYPE_WIFI:
		...
		err = nmDevWireless.ActiveAccessPoint().ConnectChanged(func(hasValue bool,
			...
			// ocean dont need try to re active connection or may cause error connection in OPT env
