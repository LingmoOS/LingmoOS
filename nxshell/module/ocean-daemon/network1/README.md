**描述**: lingmo 网络后端, 主要是对 NetworkManager DBus 接口进行包装,
...
- **examples**: 调用 lingmo 网络后端 DBus 接口的 shell, python 脚本示
  例.
...
- **manager_config.go**: 处理 lingmo 网络后端的配置文件
  (~/.config/lingmo/network.json), 主要保存一些网络开关状态和 VPN 自动
  连接的配置.
...
- **manager_switch.go**: 处理设备开关及相关 DBus 接口. lingmo 为每个网
...
- `xxxJSON`: JSON string 类型, 为了方便前后端交互, 同时减少 DBus 参数数
  量, lingmo 网络后端用了很多 JSON string 类型的参数
...
- WiFi AccessPoint
  ...
  - **signal** `AccessPointAoceand func(devPath, apJSON string)`
...
### org.lingmo.ocean.Network1.ConnectionSession
...
详细 DBus 接口信息请参考
[godoc 文档](https://godoc.org/github.com/lingmoos/ocean-daemon/network).
...
1. 单独执行网络后端
   ```sh
   env OCEAN_DEBUG=t go test -tags dev -gocheck.f TestMain
   ```

## 自动化测试

目前配合 ansbile/docker/openwrt 等技术, 易经实现 lingmo 网络功能自动化
测试, 具体请参考
[lingmo-network-tests](https://github.com/x-lingmo/lingmo-network-tests)

如果要通过 iperf3 等工具测试网卡驱动的稳定性, 则参考 [network-testing-experiments](https://github.com/x-lingmo/network-testing-experiments)
...
1. 获取 lingmo 网络后端提供的所有网络设备信息

   ```sh
   dbus-send --print-reply --dest=org.lingmo.ocean.Network1 \
       /org/lingmo/ocean/Network1 org.freedesktop.DBus.Properties.Get \
       string:"org.lingmo.ocean.Network1" string:"Devices"
   ```

1. 设置 lingmo 网络后端提供的 WiFi 无线, VPN 等开关状态

   ```sh
   dbus-send --print-reply --type=method_call \
       --dest=org.lingmo.ocean.Network1 /org/lingmo/ocean/Network1 \
       org.freedesktop.DBus.Properties.Set \
       string:"org.lingmo.ocean.Network1" string:"NetworkingEnabled" variant:boolean:"true"
   dbus-send --print-reply --type=method_call \
       --dest=org.lingmo.ocean.Network1 /org/lingmo/ocean/Network1 \
       org.freedesktop.DBus.Properties.Set \
       string:"org.lingmo.ocean.Network1" string:"WirelessEnabled" variant:boolean:"true"
    dbus-send --print-reply --type=method_call \
       --dest=org.lingmo.ocean.Network1 /org/lingmo/ocean/Network1 \
       org.freedesktop.DBus.Properties.Set \
       string:"org.lingmo.ocean.Network1" string:"VpnEnabled" variant:boolean:"true"
   ```
...
- nm-connection-editor, network-manaager-gnome 包提供的配置工具, 简单
  ...
  建议手动将 nm-applet 禁用, 否则可能和 lingmo 网络后端有冲突
...
- dbus-send, dbus-monitor, 终端 DBus 工具
  ...
  1. 获取 lingmo 网络后端 DBus 接口 XML 配置

     ```sh
     dbus-send --type=method_call --print-reply --dest=org.lingmo.ocean.Network1 /org/lingmo/ocean/Network1 org.freedesktop.DBus.Introspectable.Introspect | sed 1d | sed -e '1s/^   string "//' | sed '$s/"$//'
     ```
...
1. 设置系统代理, 但并未生效
   ...
   目前 lingmo 仅支持 GNOME gsettings 系统代理和环境变量系统代理, 如果
