...
import (
	...
	"github.com/lingmoos/ocean-daemon/network1/nm"
	"github.com/lingmoos/ocean-daemon/network1/proxychains"
	...
	secrets "github.com/lingmoos/go-dbus-factory/session/org.freedesktop.secrets"
	airplanemode "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.airplanemode1"
	ipwatchd "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.ipwatchd1"
	sysNetwork "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.network1"
	login1 "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.login1"
	nmdbus "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.networkmanager"
...
const (
	dbusServiceName = "org.lingmo.ocean.Network1"
	dbusPath        = "/org/lingmo/ocean/Network1"
	dbusInterface   = dbusServiceName
)

const (
	daemonConfigPath                   = "org.lingmo.ocean.daemon"
	networkConfigPath                  = "org.lingmo.ocean.daemon.network"
	...
	networkCoreDsgConfigPath    = "/usr/share/dsg/configs/org.lingmo.ocean.network/org.lingmo.ocean.network.json"
	networkCoreConfigPath       = "org.lingmo.ocean.network"
	oceanNetworkCoreConfigPath    = networkCoreConfigPath
...
type Manager struct {
	...
	// dsg config : org.lingmo.ocean.daemon.network
	...
	// dsg config : org.lingmo.ocean.network : LoadServiceFromNM
	loadServiceFromNM       bool
	enableLocalConnectivity bool

	//nolint
	signals *struct {
		AccessPointAoceand, AccessPointRemoved, AccessPointPropertiesChanged struct {
...
func (m *Manager) init() {
	...
	go func() {
		...
		err = nmAgentManager.Register(0, "org.lingmo.ocean.Network1.SecretAgent")
		if err != nil {
...
func (m *Manager) loadEnableConnectivity(ds configManager.ConfigManager) {
	networkCoreConfigManagerPath, err := ds.AcquireManager(0, networkCoreConfigPath, oceanNetworkCoreConfigPath, "")
	if err != nil {
		logger.Warning(err)
		return
	}
...
func (m *Manager) getLoadServiceFromNM(ds configManager.ConfigManager) (ret bool) {
	if dutils.IsFileExist(networkCoreDsgConfigPath) {
		configManagerPath, err := ds.AcquireManager(0, networkCoreConfigPath, oceanNetworkCoreConfigPath, "")
		if err != nil {
			logger.Warning(err)
			return
		}
		oceanNetworkCoreConfigManager, err := configManager.NewManager(m.sysSigLoop.Conn(), configManagerPath)
		if err != nil {
			logger.Warning(err)
			return
		}
		getDSettingsLoadServiceFromNM := func() bool {
			v, err := oceanNetworkCoreConfigManager.Value(0, dsettingsLoadServiceFromNM)
			if err != nil {
				logger.Warning(err)
				return false
			}
		...
		logger.Warning("[init] DConfig file not exist : /usr/share/dsg/configs/org.lingmo.ocean.network/org.lingmo.ocean.network.json")
	}
...
func (m *Manager) initNMObjManager(systemBus *dbus.Conn) {
	...
	_, err := objManager.ConnectInterfacesAoceand(func(objectPath dbus.ObjectPath,
...
func (m *Manager) doPortalAuthentication() {
	err := exec.Command("pgrep", "startocean").Run()
	if err != nil {
		return
	}
	...
	// get url
	detectUrl := "http://detectportal.lingmo.com"
