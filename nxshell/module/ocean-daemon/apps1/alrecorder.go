...
const (
	dbusServiceName         = "org.lingmo.ocean.Apps1"
	dbusPath                = "/org/lingmo/ocean/Apps1"
...
// handleAoceand handle desktop file aoceand
func (r *ALRecorder) handleAoceand(file string) {
	r.subRecordersMutex.RLock()
	defer r.subRecordersMutex.RUnlock()
	...
	for _, sr := range r.subRecorders {
		if strings.HasPrefix(file, sr.root) {
			rel, _ := filepath.Rel(sr.root, file)
			sr.handleAoceand(removeDesktopExt(rel))
			return
		}
	}
}
