...
const sysAppsCfgDir = "/var/lib/ocean-daemon/apps"
const userAppsCfgDir = ".config/lingmo/ocean-daemon/apps"
...
func (sr *SubRecorder) checkStatus(launchedMap map[string]bool, apps []string) bool {
	...
	for _, app := range apps {
		if _, ok := launchedMap[app]; !ok {
			// app aoceand
			changed = true
			logger.Debugf("SubRecorder.checkStatus aoceand %q", app)
...
func (sr *SubRecorder) handleAoceand(name string) {
	logger.Debug("SubRecorder.handleAoceand", sr.root, name)
	sr.launchedMapMu.Lock()
