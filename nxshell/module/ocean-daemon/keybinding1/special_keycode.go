...
func (m *Manager) handleOpenControlCenter() {
	cmd := "dbus-send --session --dest=org.lingmo.ocean.ControlCenter1 --print-reply /org/lingmo/ocean/ControlCenter1 org.lingmo.ocean.ControlCenter1.Show"
	m.execCmd(cmd, false)
}
...
func (m *Manager) handlePower() {
	...
	case powerActionShowUI:
		cmd := "/usr/lib/lingmo-daemon/ocean-shutdown.sh"
		go func() {
			locked, err := m.sessionManager.Locked().Get(0)
			if err != nil {
				logger.Warning("sessionManager get locked error:", err)
...
func (m *Manager) handleOpenDeviceManager() {
	...
	err := m.execCmd("lingmo-devicemanager", true)
	if err != nil {
		logger.Warning(err)
	}
}
