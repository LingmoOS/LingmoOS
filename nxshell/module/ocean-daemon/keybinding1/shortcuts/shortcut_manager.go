...
import (
	...
	"github.com/lingmoos/ocean-daemon/keybinding1/util"
	...
	"github.com/lingmoos/go-x11-client/ext/record"
	"github.com/lingmoos/go-x11-client/util/keybind"
	"github.com/lingmoos/go-x11-client/util/keysyms"
	"github.com/lingmoos/go-x11-client/util/wm/ewmh"
)
...
// 注意: 从D-BUS启动ocean-system-daemon的时候x会取不到环境变量,只能把获取pid放到ocean-session-daemon
...
// 初始化go-dbus-factory system DBUS : org.lingmo.ocean.Daemon1
		...
		if ok {
			...
			if shortcut != nil && shortcut.GetType() == ShortcutTypeSystem &&
				(strings.HasPrefix(shortcut.GetId(), "screenshot") ||
					strings.HasPrefix(shortcut.GetId(), "lingmo-screen-recorder")) &&
			...
			if shortcut != nil && shortcut.GetType() == ShortcutTypeWM && shortcut.GetId() == "show-desktop" {
				go func() {
					cmd := "dbus-send --print-reply --dest=org.lingmo.ocean.Launcher1 /org/lingmo/ocean/Launcher1 org.lingmo.ocean.Launcher1.Hide"
...
func (sm *ShortcutManager) AddSystemById(gsettings *gio.Settings, wmObj wm.Wm, id string) {
	...
	if id == "terminal-quake" && strings.Contains(cmd, "lingmo-terminal") {
		termPath, _ := exec.LookPath("lingmo-terminal")
		if termPath == "" {
			return
		}
	}
...
func (sm *ShortcutManager) AddSystem(gsettings, gsPlatform, gsEnable *gio.Settings, wmObj wm.Wm) {
	...
	for _, id := range gsettings.ListKeys() {
		if id == "lingmo-screen-recorder" || id == "wm-switcher" {
			if !allow && id == "wm-switcher" {
				logger.Debugf("com.lingmo.wm.compositingAllowSwitch is false, filter %s", id)
				continue
			}
...
func (sm *ShortcutManager) AddWM(gsettings *gio.Settings, wmObj wm.Wm) {
	...
	releaseType := getLingmoReleaseType()
	for _, id := range gsettings.ListKeys() {
