...
import (
	...
	"github.com/lingmoos/ocean-daemon/keybinding1/shortcuts"
	configManager "github.com/lingmoos/go-dbus-factory/org.desktopspec.ConfigManager"
	lockfront "github.com/lingmoos/go-dbus-factory/session/com.lingmo.ocean.lockfront"
	...
	inputdevices "github.com/lingmoos/go-dbus-factory/session/org.lingmo.ocean.inputdevices1"
	kwayland "github.com/lingmoos/go-dbus-factory/session/org.lingmo.ocean.kwayland1"
	network "github.com/lingmoos/go-dbus-factory/session/org.lingmo.ocean.network1"
	...
	airplanemode "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.airplanemode1"
	backlight "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.backlighthelper1"
	keyevent "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.keyevent1"
	power "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.power1"
	systeminfo "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.systeminfo1"
	DisplayManager "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.DisplayManager"
	login1 "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.login1"
	networkmanager "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.networkmanager"
	gio "github.com/lingmoos/go-gir/gio-2.0"
	"github.com/lingmoos/go-lib/dbusutil"
	"github.com/lingmoos/go-lib/dbusutil/gsprop"
	...
	"github.com/lingmoos/go-x11-client/util/keysyms"
)
...
const (
	...
	shortcutSignalAoceand   = "Aoceand"
	shortcutSignalDeleted = "Deleted"

	gsSchemaKeyboard          = "com.lingmo.ocean.keyboard"
	...
	gsSchemaSystem         = "com.lingmo.ocean.keybinding.system"
	gsSchemaSystemPlatform = "com.lingmo.ocean.keybinding.system.platform"
	gsSchemaSystemEnable   = "com.lingmo.ocean.keybinding.system.enable"
	gsSchemaMediaKey       = "com.lingmo.ocean.keybinding.mediakey"
	gsSchemaGnomeWM        = "com.lingmo.wrap.gnome.desktop.wm.keybindings"
	gsSchemaSessionPower   = "com.lingmo.ocean.power"

	customConfigFile = "lingmo/ocean-daemon/keybinding/custom.ini"
	CapslockKey      = 58
	NumlockKey       = 69
	KeyPress         = 1
)
...
const (
	DSettingsAppID                         = "org.lingmo.ocean.daemon"
	DSettingsKeyBindingName                = "org.lingmo.ocean.daemon.keybinding"
...
type Manager struct {
	...
	// nolint
	signals *struct {
		Aoceand, Deleted, Changed struct {
			id  string
			typ int32
		}
...
var kwinSysActionCmdMap = map[string]string{
	...
	"Lingmo Picker":         "color-picker",           // ctrl+alt+v
	"System Monitor":        "system-monitor",         // ctrl+alt+escape
	"Screen Recorder":       "lingmo-screen-recorder", // lingmo-screen-recorder ctrl+alt+r
...
func (m *Manager) listenKeyboardEvent(systemBus *dbus.Conn) {
	err := systemBus.Object("org.lingmo.ocean.Gesture1",
		"/org/lingmo/ocean/Gesture1").AddMatchSignal("org.lingmo.ocean.Gesture1", "KeyboardEvent").Err
	if err != nil {
		logger.Warning(err)
	}
	m.systemSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: "org.lingmo.ocean.Gesture1.KeyboardEvent",
	}, func(sig *dbus.Signal) {
		if len(sig.Body) > 1 {
...
func (m *Manager) handleKeyEventByWayland(changKey string) {
	...
	if action.Type == shortcuts.ActionTypeSystemShutdown {
		...
		err := m.execCmd("dbus-send --session --dest=org.lingmo.ocean.ControlCenter1  --print-reply /org/lingmo/ocean/ControlCenter1 org.lingmo.ocean.ControlCenter1.Show",
			false)
		if err != nil {
		...
		// and check if Wifi shortcut effected by OCEAN software
		...
		if !isWaylandGrabed {
			if _useWayland {
				...
				time.Sleep(200 * time.Millisecond) // + 添加200ms延时，保证在ocean-system-daemon中先获取状态；
...
func (m *Manager) execCmd(cmd string, viaStartocean bool) error {
	if cmd == "" {
		logger.Debug("cmd is empty")
		return nil
	}
	if strings.HasPrefix(cmd, "dbus-send ") || !viaStartocean {
...
func (m *Manager) handleCheckCamera() error {
	...
	if checkProRunning(cmd) {
		cmd = "killall lingmo-camera"
		viaStartocean = false
	}
	return m.execCmd(cmd, viaStartocean)
}
