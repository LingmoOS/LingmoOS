...
func showOSD(signal string) {
	...
	go sessionDBus.Object("org.lingmo.ocean.Osd1", "/").Call("org.lingmo.ocean.Osd1.ShowOSD", 0, signal)
}

const sessionManagerDest = "org.lingmo.ocean.SessionManager1"
const sessionManagerObjPath = "/org/lingmo/ocean/SessionManager1"
...
func (m *Manager) systemShutdown() {
	...
	// 快捷键关机流程：判断是否存在多用户或任何shutdown阻塞项(block or delay),存在则跳转ocean-shutdown界面，不存在进行sessionManager的注销
	if !m.canShutdown() {
...
func (m *Manager) systemTurnOffScreen() {
	...
	if useWayland {
		err = exec.Command("ocean_wldpms", "-s", "Off").Run()
	} else {
...
func doPrepareSuspend() {
	sessionDBus, _ := dbus.SessionBus()
	obj := sessionDBus.Object("org.lingmo.ocean.Power1", "/org/lingmo/ocean/Power1")
	err := obj.Call("org.lingmo.ocean.Power1.SetPrepareSuspend", 0, suspendStateButtonClick).Err
	if err != nil {
		logger.Warning(err)
	}
}
...
func undoPrepareSuspend() {
	sessionDBus, _ := dbus.SessionBus()
	obj := sessionDBus.Object("org.lingmo.ocean.Power1", "/org/lingmo/ocean/Power1")
	err := obj.Call("org.lingmo.ocean.Power.SetPrepareSuspend", 0, suspendStateFinish).Err
	if err != nil {
		logger.Warning(err)
	}
}
