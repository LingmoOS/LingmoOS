...
import (
	...
	"github.com/lingmoos/ocean-api/dxinput"
	"github.com/lingmoos/ocean-api/dxinput/common"
	"github.com/lingmoos/ocean-api/dxinput/kwayland"
	kwin "github.com/lingmoos/go-dbus-factory/session/org.kde.kwin"
)
...
func (m *Manager) handleInputDeviceChanged(stop bool) {
	...
	aoceandID, err := m.kwinManager.ConnectDeviceAoceand(func(sysName string) {
		logger.Debug("[Device Aoceand]:", sysName)
		doHandleKWinDeviceAoceand(sysName)
		m.setWheelSpeed()
	})
	if err == nil {
		m.kwinIdList = append(m.kwinIdList, aoceandID)
	}
...
func doHandleKWinDeviceAoceand(sysName string) {
	info, err := kwayland.NewDeviceInfo(sysName)
	if err != nil {
		logger.Debug("[Device Aoceand] Failed to new device:", err)
		return
	}
	if info == nil {
		return
	}
	logger.Debug("[Device Changed] aoceand:", info.Id, info.Type, info.Name, info.Enabled)
	switch info.Type {
