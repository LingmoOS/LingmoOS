...
func Test_StrList(t *testing.T) {
	...
	ret, aoceand := addItemToList("456", list)
	assert.Len(t, ret, 4)
	assert.True(t, aoceand)

	ret, aoceand = addItemToList("123", list)
	assert.Len(t, ret, 3)
	assert.False(t, aoceand)
...
func Test_SyndaemonExist(t *testing.T) {
	...
	assert.True(t, isProcessExist("testdata/ocean-desktop-cmdline", "ocean-desktop"))
}
