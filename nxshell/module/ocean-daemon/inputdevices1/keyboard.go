...
import (
	...
	"github.com/lingmoos/go-x11-client/util/wm/icccm"
)

const (
	kbdSchema = "com.lingmo.ocean.keyboard"
...
func (kbd *Keyboard) addUserLayout(layout string) {
	kbd.PropsMu.Lock()
	newLayoutList, aoceand := addItemToList(layout, kbd.UserLayoutList)
	kbd.PropsMu.Unlock()
...
func (kbd *Keyboard) addUserOption(option string) {
	...
	ret, aoceand := addItemToList(option, kbd.UserOptionList.Get())
	if !aoceand {
		return
	}
...
func (kbd *Keyboard) shouldUseOCEANKwin() bool {
