...
const (
	...
	oceanLogindConf               = "lingmo-daemon.conf"
...
func (d *Daemon) SetLogindTTY(sender dbus.Sender, NAutoVTs int, resetCustom bool, live bool) *dbus.Error {
	// checkAuth root进程和ocean组件管控应用可以调用
	var cmd string
	...
	if uid != 0 {
		...
		if cmd != "/usr/bin/ocean-component-control" {
	...
	oceanLogindConfPath := filepath.Join(logindConfDir, oceanLogindConf)
	if resetCustom {
		err := os.RemoveAll(oceanLogindConfPath)
		if err != nil {
			logger.Warning(err)
			return dbusutil.ToError(err)
		}
		...
		// 增加tty配置,用于禁用tty启动
		logindOCEANConf := keyfile.NewKeyFile()
		logindOCEANConf.SetInt(systemdLogindConfigSection, systemdLogindConfigNAutoVTs, NAutoVTs)
		err := logindOCEANConf.SaveToFile(oceanLogindConfPath)
		if err != nil {
			logger.Warning(err)
			return dbusutil.ToError(err)
		}
