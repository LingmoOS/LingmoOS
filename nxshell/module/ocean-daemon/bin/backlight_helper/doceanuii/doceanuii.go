...
func (d *doceanuii) newDisplayHandle(idx int) *displayHandle {
	dh := &displayHandle{
		idx: idx,
	}
...
func newDDCCI() (*doceanuii, error) {
	...
	if status < C.int(0) {
		return nil, fmt.Errorf("brightness: Error doceanuii init: %d", status)
	}
...
func (d *doceanuii) RefreshDisplays() error {
	d.listMu.Lock()
...
func (d *doceanuii) initDisplay(idx int) error {
...
func (d *doceanuii) SupportBrightness(edidBase64 string) bool {
	d.listMu.Lock()
	defer d.listMu.Unlock()
...
func (d *doceanuii) GetBrightness(edidBase64 string) (brightness int, err error) {
	d.listMu.Lock()
	defer d.listMu.Unlock()
...
func (d *doceanuii) SetBrightness(edidBase64 string, percent int) error {
	d.listMu.Lock()
	defer d.listMu.Unlock()
...
func (d *doceanuii) getDisplayHandleByIdx(idx int) *displayHandle {
	for _, handle := range d.displayHandleMap {
		if handle.idx == idx {
...
func (d *doceanuii) findMonitorIndex(edidBase64 string) (int, bool) {
	...
	for edidIter, idxIter := range d.displayHandleMap {
		...
		// doceanuii只存储128个字节，所以之比较前128个字节
...
func (d *doceanuii) getDisplayInfoByIdx(idx int) *C.DDCA_Display_Info {
