...
import (
	...
	"github.com/lingmoos/ocean-daemon/bin/backlight_helper/doceanuii"
...
const (
	dbusServiceName = "org.lingmo.ocean.BacklightHelper1"
	dbusPath        = "/org/lingmo/ocean/BacklightHelper1"
...
func main() {
	...
	m.configManagerPath, err = cfgManger.AcquireManager(0, "org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.brightness", "")
	if err != nil {
		// 即使配置服务或者接口不存在，也要设置默认配置，不能影响基本亮度调节
		logger.Warning("failed to get config Manager:", err)
		m.gsSupportDoceanuii = false
		m.gsBackendHold = false
	} else {
		m.gsSupportDoceanuii = m.getBacklightGs("supportDoceanuii")
		m.gsBackendHold = m.getBacklightGs("isBacklightHelperHold")
	}

	if m.gsSupportDoceanuii {
		doceanuiiManager, err := doceanuii.NewManager(service)
		if err != nil {
			logger.Warning(err)
		} else {
			err = service.Export(doceanuii.DbusPath, doceanuiiManager)
			if err != nil {
