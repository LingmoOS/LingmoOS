...
import (
	...
	"github.com/lingmoos/ocean-api/session"
	"github.com/lingmoos/ocean-daemon/calltrace"
	"github.com/lingmoos/ocean-daemon/loader"
	"github.com/lingmoos/go-gir/glib-2.0"
...
func (s *SessionDaemon) initDsgConfig() error {
	...
	err = systemConnObj.Call(configManagerId+".acquireManager", 0, "org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.loader", "").Store(&s.configManagerPath)
	if err != nil {
		logger.Warning(err)
		return err
	}
	...
	s.systemSigLoop.AddHandler(&dbusutil.SignalRule{
		...
		if strings.Contains(string(sig.Name), "org.desktopspec.ConfigManager.Manager.valueChanged") &&
			strings.Contains(string(sig.Path), "org_lingmo_ocean_daemon_loader") && len(sig.Body) >= 1 {
...
func (s *SessionDaemon) StartPart2() *dbus.Error {
	if !hasOCEANCookie {
		return dbusutil.ToError(errors.New("env OCEAN_SESSION_PROCESS_COOKIE_ID is empty"))
	}
	// start part2
