...
import (
	...
	"github.com/lingmoos/go-lib/proxy"
	"github.com/lingmoos/go-lib/utils"
	"github.com/lingmoos/go-lib/xdg/basedir"
)

var logger = log.NewLogger("daemon/ocean-session-daemon")
var hasOCEANCookie bool
var hasTreeLand bool

var treeLandNotAllowModules = []string{"x-event-monitor", "keybinding", "screensaver"}
...
func allowRun() bool {
	if os.Getenv("OCEAN_SESSION_PROCESS_COOKIE_ID") != "" {
		hasOCEANCookie = true
		return true
	}
	...
	sessionManagerObj := systemBus.Object("org.lingmo.ocean.SessionManager1",
		"/org/lingmo/ocean/SessionManager1")
	var allowRun bool
	err = sessionManagerObj.Call("org.lingmo.ocean.SessionManager1.AllowSessionDaemonRun",
...
// migrate user env from ~/.pam_environment to ~/.ocean-env
func migrateUserEnv() error {
	_, err := os.Stat(userenv.DefaultFile())
	if os.IsNotExist(err) {
		// when ~/.ocean-env does not exist, read ~/.pam_environment,
		...
		// save the current env to ~/.ocean-env
...
func syncConfigToSoundThemePlayer() error {
	...
	soundEffectGs := gio.NewSettings("com.lingmo.ocean.sound-effect")
