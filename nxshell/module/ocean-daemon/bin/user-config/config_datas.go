...
import (
	...
	"github.com/lingmoos/go-lib/archive"
...
func CopyUserDatas(user string) {
	...
	err = copyLingmoManuals(info.Home, lang)
	if err != nil {
		fmt.Printf("Copy lingmo manuals for '%s' failed: %v\n", user, err)
	}
...
func copyLingmoManuals(home, lang string) error {
	var (
	...
	src := path.Join("/usr/share/doc/lingmo-manuals", lang)
...
func copySoundThemeData(home, lang string) error {
	src := "/usr/share/lingmo-sample-music/playlist.m3u"
	if !dutils.IsFileExist(src) {
...
func copyBroswerConfig(home, lang string) error {
	...
	var (
		override   = "/usr/share/lingmo-default-settings/google-chrome/override-chrome-config.tar"
		configLang = fmt.Sprintf("/usr/share/lingmo-default-settings/google-chrome/chrome-config-%s.tar", lang)
		config     = "/usr/share/lingmo-default-settings/google-chrome/chrome-config.tar"
