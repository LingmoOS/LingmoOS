...
const (
	updateNotifyShow         = "ocean-control-center"          // 无论控制中心状态，都需要发送的通知
	updateNotifyShowOptional = "ocean-control-center-optional" // 根据控制中心更新模块焦点状态,选择性的发通知(由ocean-session-daemon的lastore agent判断后控制)
)
	...
	if appName == updateNotifyShowOptional {
		...
		if a.isWaylandSession {
			...
			if err != nil {
				...
				wInfo, err := kwayland.NewWindow(a.sessionService.Conn(), dbus.ObjectPath(fmt.Sprintf("/org/lingmo/ocean/KWayland1/PlasmaWindow_%v", winId)))
				if err != nil {
					logger.Warning(err)
				} else {
					name, err := wInfo.AppId(0)
					if err == nil {
						if strings.Contains(name, "ocean-control-center") {
							// 焦点在控制中心上,需要判断是否为更新模块
							currentModule, err := a.controlCenter.CurrentModule().Get(0)
							if err != nil {
								logger.Warning(err)
							} else if currentModule == "update" {
								logger.Info("update module of ocean-control-center is in the foreground, don't need send notify")
								needSend = false
							}
						} else if strings.Contains(name, "ocean-lock") {
			...
			if err != nil {
				logger.Warning(err)
			} else if strings.Contains(string(output), "ocean-control-center") {
				// 焦点在控制中心上,需要判断是否为更新模块
				currentModule, err := a.controlCenter.CurrentModule().Get(0)
				if err != nil {
					logger.Warning(err)
			...
			} else if strings.Contains(string(output), "ocean-lock") {
				// 前台应用在模态更新界面时,不发送通知(TODO: 如果后台更新时发生了锁屏，需要增加判断是否发通知)
				needSend = false
			}
		}
