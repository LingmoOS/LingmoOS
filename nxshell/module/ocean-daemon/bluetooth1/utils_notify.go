...
const (
	...
	notifyDdeDialogPath = "/usr/lib/lingmo-daemon/ocean-bluetooth-dialog"
	// notification window stay time
	notifyTimerDuration = 30 * time.Second
)

const bluetoothDialog string = "ocean-bluetooth-dialog"
...
func notify(icon, summary, body string) {
	...
	nid, err := globalNotifications.Notify(0, Tr("ocean-control-center"), nid, icon,
...
func notifyPassiveConnect(dev *DeviceInfo, pinCode string) error {
	...
	hints := map[string]dbus.Variant{"x-lingmo-action-pair": dbus.MakeVariant(cmd)}
	...
	// notify connect request to ocean-control-center
	// set notify time out as -1, default time out is 5 seconds
	nid, err = globalNotifications.Notify(0, Tr("ocean-control-center"), nid, notifyIconBluetoothConnected,
