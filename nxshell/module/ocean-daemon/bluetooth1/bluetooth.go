...
import (
	...
	audio "github.com/lingmoos/go-dbus-factory/session/org.lingmo.ocean.audio1"
	mpris2 "github.com/lingmoos/go-dbus-factory/session/org.mpris.mediaplayer2"
	obex "github.com/lingmoos/go-dbus-factory/system/org.bluez.obex"
	airplanemode "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.airplanemode1"
	sysbt "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.bluetooth1"
...
const (
	dbusServiceName = "org.lingmo.ocean.Bluetooth1"
	dbusPath        = "/org/lingmo/ocean/Bluetooth1"
	dbusInterface   = dbusServiceName
	configManagerId = "org.desktopspec.ConfigManager"
)

const (
	bluetoothSchema = "com.lingmo.ocean.bluetooth"
...
type Bluetooth struct {
	...
	signals *struct {
		...
		AdapterAoceand, AdapterRemoved, AdapterPropertiesChanged struct {
			adapterJSON string
		}

		DeviceAoceand, DeviceRemoved, DevicePropertiesChanged struct {
			devJSON string
		}
...
func newBluetooth(service *dbusutil.Service) (b *Bluetooth) {
	...
	err = systemConnObj.Call(configManagerId+".acquireManager", 0, "org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.bluetooth", "").Store(&b.configManagerPath)
	if err != nil {
		logger.Warning(err)
		return nil
	}
...
func (b *Bluetooth) init() {
	...
	_, err = b.sysBt.ConnectAdapterAoceand(func(adapterJSON string) {
		...
		err = b.service.Emit(b, "AdapterAoceand", adapterJSON)
		if err != nil {
			logger.Warning(err)
		}
	})
	...
	if err == nil {
		...
		if err == nil {
			...
			for _, adapterInfo := range adapterInfos {
				...
				err = b.service.Emit(b, "AdapterAoceand", string(data))
				if err != nil {
					logger.Warning(err)
					continue
				}
	...
	_, err = b.sysBt.ConnectDeviceAoceand(func(deviceJSON string) {
		devInfo, err := unmarshalDeviceInfo(deviceJSON)
		if err != nil {
			logger.Warning(err)
		}
		logger.Debug("DeviceAoceand", devInfo.Alias, devInfo.Path)
		b.devices.addOrUpdateDevice(devInfo)
		err = b.service.Emit(b, "DeviceAoceand", deviceJSON)
		if err != nil {
			logger.Warning(err)
		}
	})
