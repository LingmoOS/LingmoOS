...
const (
	dbusFormatPath        = "/org/lingmo/ocean/Format1"
	dbusFormatInterface   = "org.lingmo.ocean.Format1"
...
func newManagerFormat(service *dbusutil.Service) (*ManagerFormat, error) {
	...
	err = systemConnObj.Call(configManagerId+".acquireManager", 0, "org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.format", "").Store(&m.configManagerPath)
	if err != nil {
		logger.Warning(err)
	}
...
func (m *ManagerFormat) listenDsgPropChanged() {
	...
	m.systemSigLoop.AddHandler(&dbusutil.SignalRule{
		...
		if strings.Contains(string(sig.Name), "org.desktopspec.ConfigManager.Manager.valueChanged") &&
			strings.Contains(string(sig.Path), "org_lingmo_ocean_daemon_format") {
