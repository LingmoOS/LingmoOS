...
func (d *Daemon) Start() (err error) {
	...
	err = serverObj.ConnectChanged(d.manager, "PowerSavingModeEnabled", func(change *dbusutil.PropertyChanged) {
		...
		// 历史版本只有节能和平衡之间的切换
		if enabled {
			d.manager.doSetMode(oceanPowerSave)
		} else {
			d.manager.doSetMode(oceanBalance)
		}
	...
	if d.manager.enablePerformanceInBoot() {
		...
		handlerId, err = d.manager.displayManager.ConnectSessionAoceand(func(session dbus.ObjectPath) {
