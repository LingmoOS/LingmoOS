...
import (
	...
	"github.com/lingmoos/ocean-api/powersupply"
	"github.com/lingmoos/ocean-api/powersupply/battery"
	...
	gudev "github.com/lingmoos/go-gir/gudev-1.0"
...
const (
	...
	dsettingsPowerName                            = "org.lingmo.ocean.daemon.power"
...
//go:generate dbusutil-gen -type Manager,Battery -import github.com/lingmoos/ocean-api/powersupply/battery manager.go battery.go
...
const (
	oceanPowerSave   = "powersave"
	oceanBalance     = "balance"
	oceanPerformance = "performance"
	oceanLowBattery  = "lowBattery" // 内部使用，在对外暴露的时候，会切换成powersave
)
...
func (m *Manager) initDsgConfig() error {
	logger.Info("org.lingmo.ocean.Power1 module start init dconfig.")
	// dsg 配置
	...
	_, _ = dsPower.ConnectValueChanged(func(key string) {
		logger.Info("dconfig org.lingmo.ocean.daemon.power valueChanged, key : ", key)
		switch key {
		...
		case dsettingsMode:
			...
			if oceanPowerSave == oldMode || oceanPowerSave == newMode {
				m.PropsMu.Lock()
...
func (m *Manager) addAndExportBattery(dev *gudev.Device) {
	bat, aoceand := m.addBattery(dev)
	if aoceand {
		err := m.service.Export(bat.getObjPath(), bat)
		if err == nil {
			m.emitBatteryAoceand(bat)
		} else {
...
func (m *Manager) emitBatteryAoceand(bat *Battery) {
	err := m.service.Emit(m, "BatteryAoceand", bat.getObjPath())
	if err != nil {
		logger.Warning(err)
	}
}
...
const configFile = "/var/lib/ocean-daemon/power/config.json"
...
func (m *Manager) doSetMode(mode string) {
	...
	if mode == oceanPowerSave && m.batteryLow {
		mode = oceanLowBattery
	}
	fixMode := mode
	if fixMode == oceanLowBattery {
		fixMode = oceanPowerSave
	...
	if m.lastMode != mode && mode != oceanPowerSave && mode != oceanLowBattery {
		m.lastMode = mode
	}
	if modeChanged {
