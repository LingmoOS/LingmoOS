...
import (
	...
	"github.com/lingmoos/go-lib/cgroup"
	"github.com/lingmoos/go-lib/dbusutil"
	"github.com/lingmoos/go-lib/log"
)

const (
	dbusServiceName = "org.lingmo.ocean.SwapSchedHelper1"
	dbusPath        = "/org/lingmo/ocean/SwapSchedHelper1"
...
func (sw *Helper) Prepare(sessionID string) *dbus.Error {
	...
	err = createOCEANCGroups(uid, sessionID)
	if err != nil {
...
func (sw *Helper) init() {
	...
	_, err := sw.loginManager.ConnectSessionRemoved(
		func(sessionID string, sessionPath dbus.ObjectPath) {
			...
			_, err := os.Stat(filepath.Join(memMountPoint, sessionID+"@ocean"))
			if err == nil {
				// path exit
				go func() {
					time.Sleep(10 * time.Second)
					err := deleteOCEANCGroups(sessionID)
					if err != nil {
						logger.Warning("failed to delete OCEAN cgroups:", err)
					}
				}()
			}
		})
...
func createOCEANCGroups(uid uint32, sessionID string) error {
	dir := sessionID + "@ocean/"
...
func deleteOCEANCGroups(sessionID string) error {
	logger.Debugf("delete cgroup for session %s", sessionID)
	cg := newCgroup(sessionID + "@ocean")
