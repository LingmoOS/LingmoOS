...
import (
	...
	bluez "github.com/lingmoos/go-dbus-factory/system/org.bluez"
...
const (
	...
	dsettingsBluetoothName       = "org.lingmo.ocean.daemon.bluetooth"
...
func (b *SysBluetooth) init() {
	...
	_, err = b.objectManager.ConnectInterfacesAoceand(b.handleInterfacesAoceand)
	if err != nil {
		logger.Warning(err)
	}
...
func (b *SysBluetooth) handleSessionNew(sessionId string, sessionPath dbus.ObjectPath) {
	logger.Info("session aoceand", sessionId, sessionPath)
	...
	newlyAoceand := b.userAgents.addSession(uidStr, session)
	if newlyAoceand {
...
func (b *SysBluetooth) handleInterfacesAoceand(path dbus.ObjectPath, data map[string]map[string]dbus.Variant) {
...
func (b *SysBluetooth) addDeviceWithCount(devPath dbus.ObjectPath, count int) {
	logger.Debug("receive signal device aoceand", devPath)
...
func (b *SysBluetooth) addAdapter(adapterPath dbus.ObjectPath) {
	logger.Debug("receive signal adapter aoceand", adapterPath)
