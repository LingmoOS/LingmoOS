...
func (a *adapter) notifyAdapterAoceand() {
	logger.Info("AdapterAoceand", a)
	err := _bt.service.Emit(_bt, "AdapterAoceand", marshalJSON(a))
	if err != nil {
		logger.Warning(err)
	}
	_bt.updateState()
...
func (a *adapter) connectProperties() {
	...
	err = a.core.Adapter().Powered().ConnectChanged(func(hasValue bool, value bool) {
		...
		if a.Powered != value && time.Since(a.poweredActionTime) < time.Second {
			logger.Debug("sync power status because ocean power status opt is working")
