...
func Test_DataManager(t *testing.T) {
	...
	for k, v := range testData {
		err := dm.SetData(uadpDataDir, "ocean.out", k, []byte(aesKey), []byte(v))
		assert.NoError(t, err)
	}

	for k, v := range testData {
		key, data, err := dm.GetData("ocean.out", k)
		assert.NoError(t, err)
	...
	assert.Equal(t, len(testData), len(dm.ListName("ocean.out")))
	dm.DeleteProcess("ocean.out")
	assert.Equal(t, 0, len(dm.ListName("ocean.out")))
}
