...
#define OCEAN_TC_KEY_BASE 0x81000001
#define OCEAN_TC_KEY_MAX (OCEAN_TC_KEY_BASE + 64)

// 为了避免被内核摔黑锅，使用 come to nothing 来表示函数调用失败
#define OCEAN_TC_CHECK(FUNC, ...)                                     \
        do                                                          \
        {                                                           \
...
bool cryptoInit(TC_HANDLE* handle)
{
    oceanTcInit();
    // 初始化可信环境
    OCEAN_TC_CHECK(TC_Start, "tabrmd", handle);
...
{
    // 创建主键并获取索引
    OCEAN_TC_CHECK(TC_CreatePrimary, handle, TC_SHA256, TC_TPM2_RH_ENDORSEMENT, NULL, TC_RSA, auth, PrimaryIndex);

    // 创建密钥并获取索引
    uint32_t rawKeyIndex;
    OCEAN_TC_CHECK(TC_Create, handle, TC_SHA256, *PrimaryIndex, auth, TC_RSA, auth, &rawKeyIndex);
    OCEAN_TC_CHECK(TC_Load, handle, rawKeyIndex, auth);

    // 创建持久化密钥
    for(*keyIndex = OCEAN_TC_KEY_BASE; (*keyIndex) < OCEAN_TC_KEY_MAX; (*keyIndex)++)
    {
...
{
    OCEAN_TC_CHECK(TC_EvictControl, handle, false, keyIndex, keyIndex, TC_TPM2_RH_OWNER, NULL);
...
{
    // 加密
    OCEAN_TC_CHECK(TC_Encrypt, handle, keyIndex, auth, TC_RSA, input, output);
...
{
    // 解密
    OCEAN_TC_CHECK(TC_Decrypt, handle, keyIndex, auth, TC_RSA, input, output);
