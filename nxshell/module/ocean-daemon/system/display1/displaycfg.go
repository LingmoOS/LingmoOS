...
const (
	dbusServiceName     = "org.lingmo.ocean.Display1"
	dbusInterfaceName   = dbusServiceName
	dbusPath            = "/org/lingmo/ocean/Display1"
	configFilePath      = "/var/lib/ocean-daemon/display/config.json"
	rendererConfigPath  = "/var/lib/ocean-daemon/display/rendererConfig.json"
...
func (d *Display) doDetectSupportWayland(sender dbus.Sender) (bool, error) {
	...
	if binExist("glxinfo") {
		...
		var cmd *exec.Cmd
		if execPath == "/usr/bin/lightdm-lingmo-greeter" {
