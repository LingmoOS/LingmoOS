...
func Test_AdminUser(t *testing.T) {
	...
	list, err := getAdminUserList("testdata/group", "testdata/sudoers_lingmo")
	assert.NoError(t, err)
...
func Test_GetAdmGroup(t *testing.T) {
	groups, users, err := getAdmGroupAndUser("testdata/sudoers_lingmo")
	assert.NoError(t, err)
