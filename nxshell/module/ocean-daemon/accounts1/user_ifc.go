...
import (
	...
	"github.com/lingmoos/ocean-api/lang_info"
	"github.com/lingmoos/ocean-daemon/accounts1/users"
	"github.com/lingmoos/ocean-daemon/common/sessionmsg"
	"github.com/lingmoos/go-lib/dbusutil"
	"github.com/lingmoos/go-lib/gdkpixbuf"
	"github.com/lingmoos/go-lib/imgutil"
...
const (
	userDBusPathPrefix = "/org/lingmo/ocean/Accounts1/User"
	userDBusInterface  = "org.lingmo.ocean.Accounts1.User"
	controlCenter      = "ocean-control-center"
...
func (u *User) SetFullName(sender dbus.Sender, name string) *dbus.Error {
	...
	if !u.checkIsControlCenter(sender) {
		logger.Debug("[SetLocale] access denied: only ocean-control-center allowed")
		return dbusutil.ToError(fmt.Errorf("only ocean-control-center allowed to call this method"))
	}
...
func (u *User) SetLocale(sender dbus.Sender, locale string) *dbus.Error {
	...
	if !u.checkIsLingmoDaemon(sender) {
		logger.Debug("[SetLocale] access denied: only lingmo daemons allowed")
		return dbusutil.ToError(fmt.Errorf("only lingmo daemons allowed to call this method"))
	}
...
func (u *User) SetIconFile(sender dbus.Sender, iconURI string) *dbus.Error {
	...
	newIconURI, aoceand, err := u.setIconFile(iconURI, len(oldIconPath) > 0)
	if err != nil {
	...
	if aoceand {
		// newIconURI should be custom icon if aoceand is true
...
func (u *User) UpdateWechatAuthState() *dbus.Error {
	...
	var uosid string
	err := syncObj.Call("com.lingmo.sync.Helper.UOSID", 0).Store(&uosid)
	if err != nil {
		logger.Warning(err)
		return dbusutil.ToError(err)
	}
	var ubid string
	err = syncObj.Call("com.lingmo.sync.Helper.LocalBindCheck", 0, uosid, u.UUID).Store(&ubid)
	if err != nil {
		logger.Warning(err)
		return dbusutil.ToError(err)
	}
	...
	err = syncObj.Call("com.lingmo.sync.Helper.LocalBindCheckWechat", 0, uosid, u.UUID).Store(&isBindWechat, &wechatNickName)
	if err != nil {
		logger.Warning(err)
		return dbusutil.ToError(err)
	}
...
func (u *User) SetGreeterBackground(sender dbus.Sender, bg string) *dbus.Error {
	...
	if dutils.IsFileExist("/var/lib/lingmo/permission-manager/wallpaper_locked") {
...
func copyTempIconFile(src string, username string) (string, error) {
	// make ocean-daemon dir
	daemonDir := "/var/cache/lingmo/ocean-daemon"
...
/* secret question */
// #nosec G101
const secretQuestionDirectory = "/var/lib/ocean-daemon/secret-question/"
