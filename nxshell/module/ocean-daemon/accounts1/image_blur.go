...
const (
	imageBlurDBusPath      = "/org/lingmo/ocean/ImageBlur1"
	imageBlurDBusInterface = "org.lingmo.ocean.ImageBlur1"
)
...
func (ib *ImageBlur) gen(file, blurFile string) {
	...
	go func() {
		...
		output, err := exec.Command("/usr/lib/lingmo-api/image-blur-helper", file).CombinedOutput()
		if len(output) > 0 {
