...
var pwdChangerUserName = "lingmo-password-admin"                               //#nosec G101
const resetPwdDialogPath = "/usr/lib/ocean-control-center/reset-password-dialog" //#nosec G101
...
func newCaller(service *dbusutil.Service, sender dbus.Sender) (ret *caller, err error) {
	...
	var app string
	switch exe {
	case "/usr/bin/ocean-control-center":
		app = "control-center"
	case "/usr/bin/ocean-lock":
		app = "lock"
	case "/usr/bin/lightdm-lingmo-greeter":
		app = "greeter"
	default:
...
// 此函数负责将来自 caller 的 xauth 凭证发送给 lingmo-password-admin 用户
...
// 此函数负责临时允许 lingmo-password-admin 用户访问目标用户的 wayland socket
func newPwdChangerWayland(caller *caller, u *User, base *pwdChangerBase) (ret *pwdChangerWayland, err error) {
	...
	base.cmd.Env = append(base.cmd.Env,
		...
		"QT_WAYLAND_SHELL_INTEGRATION=kwayland-shell", // env copy from startocean-wayland
...
func removeLoginKeyring(user *User) (err error) {
	...
	dir := path.Join(user.HomeDir, "/.local/share/lingmo-keyrings-wb")
