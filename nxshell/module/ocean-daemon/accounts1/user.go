...
import (
	...
	authenticate "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.authenticate1"
	uadp "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.uadp1"
...
const (
	defaultUserBackgroundDir = "/usr/share/wallpapers/lingmo/"
	defaultUserIcon          = "file:///var/lib/AccountsService/icons/default"

	controlCenterPath = "/usr/bin/ocean-control-center"
	lingmoDaemonDir   = "/usr/lib/lingmo-daemon/"
...
const (
	lingmoThemePath         = "/usr/share/lingmo-themes/"
...
func getThemeLockBackground(theme string) string {
	kf, err := dutils.NewKeyFileFromFile(
		path.Join(lingmoThemePath, theme, themeFile))
	if err != nil {
...
func getDefaultUserBackground() string {
	bg := "file://" + filepath.Join(defaultUserBackgroundDir, "lingmo-default.jpg")
...
func (u *User) checkIsLingmoDaemon(sender dbus.Sender) bool {
	pid, err := u.service.GetConnPID(string(sender))
	if err != nil {
		logger.Warning(err)
		return false
	...
	if strings.HasPrefix(abs, lingmoDaemonDir) {
		return true
	}
...
func getTempFile() (string, error) {
	tmpfile, err := os.CreateTemp("", "ocean-daemon-accounts")
	if err != nil {
		return "", err
	}
	name := tmpfile.Name()
...
func getUserSession(homeDir string) string {
	...
	if !ok {
		...
		case 1:
			v = list[0]
		default:
			if strv.Strv(list).Contains("lingmo.desktop") {
				v = "lingmo.desktop"
			} else {
				v = list[0]
			}
...
func loadUserConfigInfo(u *User) {
	...
	// CustomIcon is the newly aoceand field in the configuration file
	if u.customIcon == "" {
