...
import (
	...
	"github.com/lingmoos/ocean-daemon/accounts1/checkers"
	...
	"github.com/lingmoos/go-lib/users/passwd"
	dutils "github.com/lingmoos/go-lib/utils"
)

const (
	nilObjPath      = dbus.ObjectPath("/")
	dbusServiceName = "org.lingmo.ocean.Accounts1"
	dbusPath        = "/org/lingmo/ocean/Accounts1"
	dbusInterface   = "org.lingmo.ocean.Accounts1"
)
...
func (m *Manager) CreateUser(sender dbus.Sender,
	...
	ch := make(chan string)
	m.usersMapMu.Lock()
	m.userAoceandChanMap[name] = ch
	m.usersMapMu.Unlock()
	defer func() {
		m.usersMapMu.Lock()
		delete(m.userAoceandChanMap, name)
		m.usersMapMu.Unlock()
		close(ch)
	}()
...
func (m *Manager) EnablePasswdChangedHandler(sender dbus.Sender, enable bool) *dbus.Error {
	...
	if enable {
		...
		m.modifyUserConfig(userDBusPathPrefix + rootUid) // root账户的信息需要更新（lingmo安装器会在后配置界面修改语言）
	}
	return nil
}
