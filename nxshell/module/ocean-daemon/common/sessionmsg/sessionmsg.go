...
// 用于ocean-system-daemon 的各个模块通过 system bus 发送消息到 session。
// ocean-session-daemon 的 service_trigger 模块负责首先接受处理这个消息。
...
import (
	...
	"github.com/lingmoos/go-lib/multierr"
)
...
type BodyNotify struct {
	...
	Summary       *LocalizeStr
	Body          *LocalizeStr
	AppName       string // 如果为空，则为 ocean-control-center
...
// 这个 agent 实际在 ocean-session-daemon 中的 service_trigger 模块中实现的
const (
	AgentPath         = "/org/lingmo/ocean/ServiceTrigger1/Agent"
	AgentIfc          = "org.lingmo.ocean.ServiceTrigger1.Agent"
