...
func (c *dconfigLogCollector) updateSyncConfig(service *dbusutil.Service) error {
	...
	var ret bool
	err := syncObj.Call("com.lingmo.sync.Helper.IsDeveloperMode", 0).Store(&ret)
	if err != nil {
		logger.Warning(err)
		return err
	}
...
func (c *dconfigLogCollector) getLastoreBoolProp(service *dbusutil.Service, propName string) (bool, error) {
	lastoreObj := service.Conn().Object("org.lingmo.ocean.Lastore1", "/org/lingmo/ocean/Lastore1")
	var ret dbus.Variant
	err := lastoreObj.Call("org.freedesktop.DBus.Properties.Get", 0, "org.lingmo.ocean.Lastore1.Updater", propName).Store(&ret)
	if err != nil {
		logger.Warning(err)
		return false, err
	}
