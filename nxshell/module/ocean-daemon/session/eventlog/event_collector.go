...
const (
	dbusServiceName = "org.lingmo.ocean.EventLog1"
	dbusPath        = "/org/lingmo/ocean/EventLog1"
	dbusInterface   = dbusServiceName
)

var (
	userExpPath    = "/var/public/lingmo-user-experience/user"
	defaultExpPath = "/etc/lingmo/lingmo-user-experience"
	varTmpExpPath  = "/var/tmp/lingmo/lingmo-user-experience/state/"
...
func (e *EventLog) syncUserExpState() {
	...
	} else if dutils.IsFileExist("/var/tmp/lingmo/lingmo-user-experience/state") { // 非当前用户创建的state/uid/info 时
		infos, err := os.ReadDir("/var/tmp/lingmo/lingmo-user-experience/state")
		if err != nil {
			logger.Warning(err)
		} else {
			for _, info := range infos {
				// 只会有一个文件
				content, err := os.ReadFile(filepath.Join("/var/tmp/lingmo/lingmo-user-experience/state", info.Name(), "info"))
				if err != nil {
					logger.Warning(err)
				} else {
