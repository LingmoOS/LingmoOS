...
import (
	...
	xscreensaver "github.com/lingmoos/go-x11-client/ext/screensaver"
...
func (psp *powerSavePlan) HandleIdleOn() {
	...
	_, err := os.Stat("/etc/lingmo/no_suspend")
	if err == nil {
...
func (psp *powerSavePlan) handleIdleOff() {
	...
	psp.manager.setOCEANBlackScreenActive(false)
	psp.resetBrightness()
...
func (psp *powerSavePlan) isWindowFullScreenAndFocused(xid x.Window) (bool, error) {
	...
	for _, s := range states {
		...
		// 后端的代码之前是基于lingmo-wm这个窗口适配，现在换成了kwin,state中没有 focus 这个属性了
...
func (psp *powerSavePlan) ConnectIdle() error {
	...
	err = sessionBus.Object("org.lingmo.ocean.KWayland1",
		"/org/lingmo/ocean/KWayland1/Output").AddMatchSignal("org.lingmo.ocean.KWayland1.Idle", "IdleTimeout").Err
	if err != nil {
		logger.Warning(err)
		return err
	}
	...
	sessionSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: "org.lingmo.ocean.KWayland1.Idle.IdleTimeout",
	}, func(sig *dbus.Signal) {
		if strings.HasPrefix(string(sig.Path),
			"/org/lingmo/ocean/KWayland1/") &&
			len(sig.Body) == 1 {
