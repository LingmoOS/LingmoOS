...
func (m *Manager) handleBeforeSuspend() {
	...
	m.setOCEANBlackScreenActive(true)
	if m.UseWayland {
...
func (m *Manager) handleWakeup() {
	...
	if m.UseWayland {
		err := m.service.Conn().Object("org.lingmo.ocean.KWayland1",
			"/org/lingmo/ocean/KWayland1/Output").Call("org.lingmo.ocean.KWayland1.Idle.simulateUserActivity", 0).Err
	...
	time.AfterFunc(time.Duration(m.delayWakeupInterval)*time.Second, func() {
		...
		m.setOCEANBlackScreenActive(false)
	})
...
func (m *Manager) handleWakeupOCEANLowPowerCheck() {
	power := m.helper.Power
...
func (m *Manager) handleWarnLevelChanged(level WarnLevel) {
	...
	case WarnLevelAction:
		...
		m.warnLevelCountTicker = newCountTicker(time.Second, func(count int) {
			if count == 3 {
				// after 3 seconds, lock and then show ocean low power
				if m.SleepLock.Get() {
