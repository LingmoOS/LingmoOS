...
import (
	...
	"github.com/lingmoos/ocean-api/soundutils"
	. "github.com/lingmoos/go-lib/gettext"
	"github.com/lingmoos/go-lib/gsettings"
	"github.com/lingmoos/go-lib/pulse"
	"github.com/lingmoos/go-x11-client/ext/dpms"
)
...
func (m *Manager) setOCEANBlackScreenActive(active bool) {
	if m.delayInActive {
		return
	}

	if !m.SleepLock.Get() {
		logger.Info("setOCEANBlackScreenActive GSettings of sleep-lock is false, Not use ocean black widget")
		return
	}
	...
	bus, err := dbus.SessionBus()
	if err == nil {
		dbusObject := bus.Object("org.lingmo.ocean.BlackScreen1", "/org/lingmo/ocean/BlackScreen1")
		err = dbusObject.Call("org.lingmo.ocean.BlackScreen1.setActive", 0, active).Err
		if err != nil {
			logger.Warning("set blackScreen active failed:", err)
		}
	}
...
func (m *Manager) setDPMSModeOn() {
	...
	autoWm := m.getAutoChangeLingmoWm()
	if autoWm {
		m.tryChangeLingmoWM()
	}
...
const (
	lockFrontServiceName = "org.lingmo.ocean.LockFront1"
	lockFrontIfc         = lockFrontServiceName
	lockFrontObjPath     = "/org/lingmo/ocean/LockFront1"
)

func (m *Manager) tryChangeLingmoWM() bool {
	ret := false
	count := 0
	for {
		if count > 2 {
		...
		if err != nil {
			logger.Warning("tryChangeLingmoWM get CompositingEnabled err : ", err)
			continue
		}
		if !enabled {
			err := m.wmDBus.CompositingEnabled().Set(0, true)
			if err != nil {
				logger.Warning("tryChangeLingmoWM set CompositingEnabled true, err :", err)
				continue
			}

			// ocean-osd需要使用该dconfig值，true不弹该特效osd
			ret = true
			break
		}
	}

	logger.Info(" tryChangeLingmoWM ret : ", ret)
	return ret
}
...
func (m *Manager) doShutdown() {
	if m.hasShutdownInhibit() || m.hasMultipleDisplaySession() {
		...
		err := m.oceanShutdown.Shutdown(0)
		if err != nil {
			logger.Warning(err)
		}
	} else {
...
func doShowOCEANLowPower() {
	logger.Info("Show ocean low power")
	go func() {
		err := exec.Command(cmdOCEANLowPower, "--raise").Run()
		if err != nil {
			logger.Warning(err)
		}
	}()
...
func doCloseOCEANLowPower() {
	logger.Info("Close low power")
	go func() {
		err := exec.Command(cmdOCEANLowPower, "--quit").Run()
		if err != nil {
			logger.Warning(err)
		}
	}()
...
func (m *Manager) sendNotify(icon, summary, body string) {
	...
	_, err := n.Notify(0, "ocean-control-center", 0, icon, summary, body, nil, nil, -1)
	if err != nil {
		logger.Warning(err)
	}
}
...
const (
	lingmoScreensaverDBusServiceName = "com.lingmo.ScreenSaver"
	lingmoScreensaverDBusPath        = "/com/lingmo/ScreenSaver"
	lingmoScreensaverDBusInterface   = lingmoScreensaverDBusServiceName
)
...
func startScreensaver() {
	...
	obj := bus.Object(lingmoScreensaverDBusServiceName, lingmoScreensaverDBusPath)
	err = obj.Call(lingmoScreensaverDBusInterface+".Start", 0).Err
	if err != nil {
		logger.Warning(err)
	}
}
...
func stopScreensaver() {
	...
	obj := bus.Object(lingmoScreensaverDBusServiceName, lingmoScreensaverDBusPath)
	err = obj.Call(lingmoScreensaverDBusInterface+".Stop", 0).Err
	if err != nil {
		logger.Warning(err)
	}
}
...
func (m *Manager) getDpmsList() ([]dbus.Variant, error) {
	...
	sessionObj := sessionBus.Object("org.lingmo.ocean.KWayland1", "/org/lingmo/ocean/KWayland1/DpmsManager")
	var ret []dbus.Variant
	err := sessionObj.Call("org.lingmo.ocean.KWayland1.DpmsManager.dpmsList", 0).Store(&ret)
	if err != nil {
		logger.Warning(err)
		return nil, err
	}
...
func (m *Manager) getDpmsModeByKwin() (int32, error) {
	...
	for i := 0; i < len(list); i++ {
		...
		sessionObj := m.sessionSigLoop.Conn().Object("org.lingmo.ocean.KWayland1", dbus.ObjectPath(v))
		err = sessionObj.Call("org.lingmo.ocean.KWayland1.Dpms.getDpmsMode", 0).Store(&dpmsMode)
		if err != nil {
			logger.Warning(err)
			return dpmsStateOn, err
		}
...
func (m *Manager) setDpmsModeByKwin(mode int32) error {
	...
	for i := 0; i < len(list); i++ {
		v := list[i].Value().(string)
		sessionObj := m.sessionSigLoop.Conn().Object("org.lingmo.ocean.KWayland1", dbus.ObjectPath(v))
		err = sessionObj.Call("org.lingmo.ocean.KWayland1.Dpms.setDpmsMode", 0, int32(mode)).Err
		if err != nil {
			logger.Warning(err)
			return err
		}
