...
import (
	...
	"github.com/lingmoos/ocean-daemon/session/common"
	configManager "github.com/lingmoos/go-dbus-factory/org.desktopspec.ConfigManager"
	calendar "github.com/lingmoos/go-dbus-factory/session/com.lingmo.dataserver.Calendar"
	...
	sessiontimedate "github.com/lingmoos/go-dbus-factory/session/org.lingmo.ocean.timedate1"
	...
	systemPower "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.power1"
	...
	timedate "github.com/lingmoos/go-dbus-factory/system/org.freedesktop.timedate1"
...
const (
	DSettingsAppID        = "org.lingmo.startocean"
	DSettingsDisplayName  = "org.lingmo.Display"
	DSettingsAutoChangeWm = "auto-change-lingmo-wm"
...
type Manager struct {
	...
	oceanShutdown          shutdownfront.ShutdownFront
...
func newManager(service *dbusutil.Service) (*Manager, error) {
	...
	m.oceanShutdown = shutdownfront.NewShutdownFront(sessionBus)
	...
	// 绑定org.lingmo.ocean.Display1的DBus
...
func (m *Manager) initDsg() {
	...
	if !dutils.IsFileExist("/usr/share/dsg/configs/org.lingmo.startocean/org.lingmo.Display.json") {
		logger.Warning(" [initDsg] dconfig file not exist : /usr/share/dsg/configs/org.lingmo.startocean/org.lingmo.Display.json.")
	} else {
		// display
	...
	m.dsPowerConfigManager.ConnectValueChanged(func(key string) {
		...
		logger.Info("DSG org.lingmo.ocean.daemon.power valueChanged, key : ", key)
...
func (m *Manager) shutdownCountdownNotify(count int, playSound bool) {
	...
	hints := map[string]dbus.Variant{"x-lingmo-PlaySound": dbus.MakeVariant(playSound),
		"urgency":                     dbus.MakeVariant(2), //取消关闭按钮
		"x-lingmo-ShowInNotifyCenter": dbus.MakeVariant(false),
		"x-lingmo-ClickToDisappear":   dbus.MakeVariant(false),
		"x-lingmo-DisappearAfterLock": dbus.MakeVariant(false)}
	m.notifyIdMu.Lock()
	nid := m.notifyId
	m.notifyIdMu.Unlock()
	nid, err := m.notify.Notify(0, "ocean-control-center", nid, "preferences-system", title, body, actions, hints, -1)
	if err != nil {
...
func (m *Manager) getAutoChangeLingmoWm() bool {
	if m.dsDisplayConfigManager == nil {
		logger.Warning("getAutoChangeLingmoWm, dsgConfig org.lingmo.startocean auto-change-lingmo-wm not exist")
		return false
	}
	...
	dsAutoChangeLingmoWm := v.Value().(bool)
	logger.Info("Auto Change Lingmo Wm:", dsAutoChangeLingmoWm)

	return dsAutoChangeLingmoWm
}

func (m *Manager) setAutoChangeLingmoWm(value bool) error {
	if m.dsDisplayConfigManager == nil {
		return errors.New("setAutoChangeLingmoWm, dsgConfig org.lingmo.startocean auto-change-lingmo-wm not exist")
	}
...
func (m *Manager) listenEventToHandleIdleOff() error {
	// + 监控按键事件
	err := m.systemSigLoop.Conn().Object("org.lingmo.ocean.Gesture1",
	...
	// + 监控鼠标移动事件
	err = m.sessionSigLoop.Conn().Object("org.lingmo.ocean.KWayland1",
		"/org/lingmo/ocean/KWayland1/Output").AddMatchSignal("org.lingmo.ocean.KWayland1.Output", "CursorMove").Err
	if err != nil {
		logger.Warning(err)
		return err
	}
	m.sessionSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: "org.lingmo.ocean.KWayland1.Output.CursorMove",
	}, func(sig *dbus.Signal) {
		if len(sig.Body) > 1 {
	...
	// + 监控鼠标按下事件
	err = m.sessionSigLoop.Conn().Object("org.lingmo.ocean.KWayland1",
		"/org/lingmo/ocean/KWayland1/Output").AddMatchSignal("org.lingmo.ocean.KWayland1.Output", "ButtonPress").Err
	if err != nil {
		logger.Warning(err)
		return err
	}
	m.sessionSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: "org.lingmo.ocean.KWayland1.Output.ButtonPress",
	}, func(sig *dbus.Signal) {
		if len(sig.Body) > 1 {
			if m.getDPMSMode() != dpmsStateOn {
