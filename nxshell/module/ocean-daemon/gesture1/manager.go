...
import (
	...
	dock "github.com/lingmoos/go-dbus-factory/session/com.lingmo.ocean.daemon.dock"
	notification "github.com/lingmoos/go-dbus-factory/session/com.lingmo.ocean.notification"
	wm "github.com/lingmoos/go-dbus-factory/session/com.lingmo.wm"
	clipboard "github.com/lingmoos/go-dbus-factory/session/org.lingmo.ocean.clipboard1"
	...
	gesture "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.gesture1"
...
const (
	tsSchemaID              = "com.lingmo.ocean.touchscreen"
...
func newManager() (*Manager, error) {
	...
	err = systemConnObj.Call(configManagerId+".acquireManager", 0, "org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.gesture", "").Store(&m.configManagerPath)
	if err != nil {
		logger.Warning(err)
	}
...
func (m *Manager) init() {
	...
	m.systemSigLoop.AddHandler(&dbusutil.SignalRule{
		...
		if strings.Contains(sig.Name, "org.desktopspec.ConfigManager.Manager.valueChanged") &&
			strings.Contains(string(sig.Path), "org_lingmo_ocean_daemon_gesture") && len(sig.Body) >= 1 {
...
func (m *Manager) showWidgets(show bool) error {
	...
	obj := sessionBus.Object("org.lingmo.ocean.Widgets", "/org/lingmo/ocean/Widgets")
	if show {
		err = obj.Call("org.lingmo.ocean.Widgets.Show", 0).Err
	} else {
		err = obj.Call("org.lingmo.ocean.Widgets.Hide", 0).Err
	}
	if err != nil {
		logger.Warning(err)
	}
