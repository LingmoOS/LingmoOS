...
const (
	...
	dSettingsClipboardName              = "org.lingmo.ocean.daemon.clipboard"
...
func initAtoms(xConn *x.Conn) {
	...
	atomFromClipboardManager, _ = xConn.GetAtom("FROM_LINGMO_CLIPBOARD_MANAGER")
...
func (m *Manager) setContent(targetDataMap map[x.Atom]*TargetData) {
	// 给剪贴板数据带上特殊标记，为了让前端 ocean-clipboard 知道是本程序给出的剪贴板数据
