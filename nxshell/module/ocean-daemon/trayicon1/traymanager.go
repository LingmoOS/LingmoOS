...
import (
	...
	"github.com/lingmoos/go-x11-client/ext/composite"
	"github.com/lingmoos/go-x11-client/ext/damage"
)
...
//go:generate dbusutil-gen -type TrayManager,StatusNotifierWatcher -import github.com/lingmoos/go-lib/strv traymanager.go status-notifier-watcher.go
...
type TrayManager struct {
	...
	signals *struct {
		...
		Inited struct{}
		// Aoceand信号会在系统过盘图标增加时被触发。
		// Removed信号会在系统过盘图标被移除时被触发。
		// Changed信号会在系统托盘图标改变后被触发。
		Aoceand, Removed, Changed struct {
			id uint32
		}
	}
}
...
func (m *TrayManager) addIcon(win x.Window) {
	...
	err = m.service.Emit(m, "Aoceand", uint32(win))
	if err != nil {
		logger.Warning(err)
	}
