...
import (
	...
	"github.com/lingmoos/go-x11-client/ext/ge"
	"github.com/lingmoos/go-x11-client/ext/input"
...
func (m *Manager) listenGlobalCursorPressed() error {
	...
	err := sessionBus.Object("org.lingmo.ocean.KWayland1",
...
func (m *Manager) listenGlobalCursorRelease() error {
	sessionBus := m.service.Conn()
	err := sessionBus.Object("org.lingmo.ocean.KWayland1",
		"/org/lingmo/ocean/KWayland1/Output").AddMatchSignal("org.lingmo.ocean.KWayland1.Output", "ButtonRelease").Err
	if err != nil {
		logger.Warning(err)
		return err
	}
	...
	m.sessionSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: "org.lingmo.ocean.KWayland1.Output.ButtonRelease",
	}, func(sig *dbus.Signal) {
		if len(sig.Body) > 1 {
			key := sig.Body[0].(uint32)
			x := sig.Body[1].(uint32)
...
func (m *Manager) listenGlobalAxisChanged() error {
	sessionBus := m.service.Conn()
	err := sessionBus.Object("org.lingmo.ocean.KWayland1",
		"/org/lingmo/ocean/KWayland1/Output").AddMatchSignal("org.lingmo.ocean.KWayland1.Output", "AxisChanged").Err
	if err != nil {
		logger.Warning(err)
		return err
	}
	...
	m.sessionSigLoop.AddHandler(&dbusutil.SignalRule{
		Name: "org.lingmo.ocean.KWayland1.Output.AxisChanged",
	}, func(sig *dbus.Signal) {
		if len(sig.Body) > 1 {
