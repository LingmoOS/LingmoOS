...
import (
	...
	libApps "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.apps1"
	libLastore "github.com/lingmoos/go-dbus-factory/system/org.lingmo.ocean.lastore1"
...
const (
	...
	oceanDataDir              = "/usr/share/ocean/data/"
	appNameTranslationsFile = oceanDataDir + "app_name_translations.json"

	AppStatusCreated  = "created"
	AppStatusModified = "updated"
	AppStatusDeleted  = "deleted"
	lastoreDBusDest   = "com.lingmo.lastore"

	gsSchemaLauncher        = "com.lingmo.ocean.launcher"
...
func (m *Manager) _queryCategoryID(item *Item, pkg string) CategoryID {
	...
	if cid, ok := parseCategoryString(item.xLingmoCategory); ok {
		logger.Debugf("queryCategoryID X-Lingmo %v -> %v", item, cid)
		return cid
	}
...
func (m *Manager) queryPkgName(itemID string, itemPath string) (string, error) {
	...
	if strings.HasPrefix(itemID, "org.lingmo.flatdeb.") {
		return "lingmo-fpapp-" + itemID, nil
	}
...
func getNotifyAppName() string {
	_, err := os.Stat("/usr/share/applications/lingmo-app-store.desktop")
	if err == nil {
		return "lingmo-app-store"
	}
	return "lingmo-appstore"
}
