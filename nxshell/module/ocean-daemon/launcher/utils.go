...
import (
	...
	"github.com/lingmoos/go-lib/xdg/userdir"
)
...
// The default applications module of the OCEAN Control Center
// creates the desktop file with the file name beginning with
// "lingmo-custom" in the applications directory under the XDG
// user data directory.
func isLingmoCustomDesktopFile(file string) bool {
	dir := filepath.Dir(file)
	base := filepath.Base(file)
	userAppDir := getUserAppDir()

	return dir == userAppDir && strings.HasPrefix(base, "lingmo-custom-")
}
