...
import (
	...
	"github.com/lingmoos/ocean-daemon/common/dconfig"
...
func NewItemWithDesktopAppInfo(appInfo *desktopappinfo.DesktopAppInfo) *Item {
	...
	xLingmoCategory, _ := appInfo.GetString(desktopappinfo.MainSection, "X-Lingmo-Category")
	xLingmoVendor, _ := appInfo.GetString(desktopappinfo.MainSection, "X-Lingmo-Vendor")
	...
	getEnableLinglongSuffix := func() {
		dc, err := dconfig.NewDConfig("org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.application", "")
		if err != nil {
	...
	item := &Item{
		...
		xLingmoCategory: strings.ToLower(xLingmoCategory),
	}
