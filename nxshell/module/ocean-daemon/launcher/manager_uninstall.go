...
func (m *Manager) uninstallLingmoWineApp(item *Item) error {
	logger.Debug("uninstallLingmoWineApp", item.Path)
	cmd := exec.Command("/opt/lingmowine/tools/uninstall.sh", item.Path)
	err := cmd.Run()
	go m.notifyUninstallDone(item, err == nil)
	return err
}
...
func (m *Manager) uninstallFlatpakApp(item *Item, fpAppInfo *flatpakAppInfo) error {
	...
	if !userInstallation {
		...
		sysOrUser = "--system"
		pkgFile := filepath.Join("/usr/share/lingmo-flatpak/app/",
...
func (m *Manager) uninstall(id string) error {
	...
	if isWineApp(appInfo) {
		...
		return m.uninstallLingmoWineApp(item)
	}
...
func (m *Manager) monitorJobStatusChange(jobPath string) error {
	...
	rule := dbusutil.NewMatchRuleBuilder().ExtPropertiesChanged(jobPath,
		"com.lingmo.lastore.Job").Sender(lastoreDBusDest).Build()
