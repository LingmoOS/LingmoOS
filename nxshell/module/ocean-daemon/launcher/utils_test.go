...
func Test_getAppIdByFilePath(t *testing.T) {
	...
	id = getAppIdByFilePath("/usr/local/share/applications/lingmo-screenshot.desktop", appDirs)
	assert.Equal(t, id, "lingmo-screenshot")
...
func Test_runeSliceDiff(t *testing.T) {
	...
	popCount, runesPush = runeSliceDiff([]rune("lingmo"), []rune("deeinp"))
