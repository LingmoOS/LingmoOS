...
func (m *Manager) handleAppHiddenChanged() {
	...
	for _, appID := range removed {
		...
		shouldShow := appInfo.ShouldShow() &&
			!isLingmoCustomDesktopFile(appInfo.GetFileName())
...
func diffMapKeys(oldMap, newMap map[string]struct{}) (aoceand, removed []string) {
	...
	for newKey := range newMap {
		if _, ok := oldMap[newKey]; !ok {
			// key found in newMap, but not in oldMap => aoceand
			aoceand = append(aoceand, newKey)
		}
	}
...
func (m *Manager) checkDesktopFile(file string) {
	...
	if appInfo == nil {
		...
		shouldShow := appInfo.ShouldShow() &&
			!isLingmoCustomDesktopFile(appInfo.GetFileName()) &&
			!m.hioceannByGSettingsWithLock(newItem.ID)
