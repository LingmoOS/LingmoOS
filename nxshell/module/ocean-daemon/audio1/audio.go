...
const (
	gsSchemaAudio                 = "com.lingmo.ocean.audio"
	...
	gsSchemaControlCenter = "com.lingmo.ocean.control-center"
	gsKeyDeviceManager    = "device-manage"

	gsSchemaSoundEffect  = "com.lingmo.ocean.sound-effect"
	gsKeyEnabled         = "enabled"
	gsKeyDisableAutoMute = "disable-auto-mute"

	dbusServiceName = "org.lingmo.ocean.Audio1"
	dbusPath        = "/org/lingmo/ocean/Audio1"
...
func newAudio(service *dbusutil.Service) *Audio {
	...
	if isStringInSlice(gio.SettingsListSchemas(), gsSchemaControlCenter) {
		...
		logger.Warning(" [newAudio] gschemas file not exist : /usr/share/glib-2.0/schemas/com.lingmo.ocean.control-center.gschema.xml")
	}
...
func sendNotify(icon, summary, body string) {
	...
	_, err = n.Notify(0, Tr("ocean-control-center"), 0,
		icon, summary, body,
		nil, nil, -1)
...
const (
	dndVirtualSinkName        = "lingmo_network_displays"
...
func (a *Audio) SetPortEnabled(cardId uint32, portName string, enabled bool) *dbus.Error {
	...
	if !a.enableAutoSwitchPort {
		err := errors.New("DConfig of org.lingmo.ocean.daemon.audio autoSwitchPort is false")
		logger.Warning(err)
		return dbusutil.ToError(err)
	}
...
func (a *Audio) initDsgProp() error {
	...
	err = systemConnObj.Call("org.desktopspec.ConfigManager.acquireManager", 0, "org.lingmo.ocean.daemon", "org.lingmo.ocean.daemon.audio", "").Store(&a.configManagerPath)
	if err != nil {
		logger.Warning(err)
		return nil
	}
	...
	a.systemSigLoop.AddHandler(&dbusutil.SignalRule{
		...
		if strings.Contains(sig.Name, "org.desktopspec.ConfigManager.Manager.valueChanged") &&
			strings.Contains(string(sig.Path), "org_lingmo_ocean_daemon_audio") && len(sig.Body) >= 1 {
