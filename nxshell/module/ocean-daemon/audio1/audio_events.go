...
func (a *Audio) handleCardAoceand(idx uint32) {
	// 数据更新在refreshCards中统一处理，这里只做业务逻辑上的响应
	logger.Debugf("card %d aoceand", idx)
...
func (a *Audio) handleSinkAoceand(idx uint32) {
	// 数据更新在refreshSinks中统一处理，这里只做业务逻辑上的响应
	logger.Debugf("sink %d aoceand", idx)
}
...
func (a *Audio) handleSourceAoceand(idx uint32) {
	// 数据更新在refreshSources中统一处理，这里只做业务逻辑上的响应
	logger.Debugf("source %d aoceand", idx)
}
...
func (a *Audio) handleSinkInputAoceand(idx uint32) {
	// 数据更新在refreshSinkInputs中统一处理，这里只做业务逻辑上的响应
	logger.Debugf("sink-input %d aoceand", idx)
}
...
func makeNotifyCmdEnablePort(cardId uint32, portName string) string {
	dest := "org.lingmo.ocean.Audio1"
	path := "/org/lingmo/ocean/Audio1"
	method := "org.lingmo.ocean.Audio1.SetPortEnabled"
...
func (a *Audio) notifyPortDisabled(cardId uint32, port pulse.CardPortInfo) {
	...
	hints := map[string]dbus.Variant{"x-lingmo-action-open": dbus.MakeVariant(cmd)}
	notify := notifications.NewNotifications(session)
	_, err = notify.Notify(
		0,
		gettext.Tr("ocean-control-center"),
		0,
		icon,
		message,
		"",
