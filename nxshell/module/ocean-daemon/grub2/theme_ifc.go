...
func (theme *Theme) GetBackground(sender dbus.Sender) (background string, busErr *dbus.Error) {
	...
	if strings.Contains(themeFile, "/lingmo/") {
	...
	} else if strings.Contains(themeFile, "/lingmo-fallback/") {
		background = fallbackGrubBackground
	} else {
		return "", nil
	}
	...
	if len(background) == 0 {
		...
		tmpBackground := "ocean-grub-background" + path.Ext(defaultGrubBackground)
