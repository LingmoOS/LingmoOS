...
import (
	...
	"github.com/lingmoos/go-lib/procfs"
...
func getModifyFuncPrepareGfxmodeDetect(gfxmodesStr string) func(map[string]string) {
	f := func(params map[string]string) {
		...
		params[grub_common.LingmoGfxmodeDetect] = "1"
		delete(params, grub_common.LingmoGfxmodeAdjusted)
		delete(params, grub_common.LingmoGfxmodeNotSupported)
...
func (g *Grub2) finishGfxmodeDetect(params map[string]string) {
	...
	currentGfxmode, _, err := grub_common.GetBootArgLingmoGfxmode()
	if err != nil {
		g.PropsMu.Lock()
		g.gfxmodeDetectState = gfxmodeDetectStateFailed
		g.PropsMu.Unlock()
		...
		task := modifyTask{
			paramsModifyFunc: func(params map[string]string) {
				params[grub_common.LingmoGfxmodeDetect] = "2"
			},
		}
	...
	task := modifyTask{
		paramsModifyFunc: func(params map[string]string) {
			...
			params[grub_common.LingmoGfxmodeAdjusted] = "1"
			delete(params, grub_common.LingmoGfxmodeDetect)
			if notMax {
				params[grub_common.LingmoGfxmodeNotSupported] = maxGfxmode.String()
			}
		},
...
func NewGrub2(service *dbusutil.Service) *Grub2 {
	...
	params, err := grub_common.LoadOCEANGrubParams()
	if err != nil {
		logger.Warning(err)
	}
...
func getGfxmodesFromBootArg() (grub_common.Gfxmodes, error) {
	_, allGfxmodes, err := grub_common.GetBootArgLingmoGfxmode()
	if err != nil {
		return nil, err
	}
...
func checkInvokePermission(service *dbusutil.Service, sender dbus.Sender) error {
	...
	if uid != 0 {
		...
		if cmd == "/usr/bin/ocean-control-center" || cmd == "/usr/libexec/lastore-daemon/lastore-daemon" {
			return nil
		} else {
...
func getFstartState() (bool, int) {
	if !dutils.IsFileExist(lingmoFstartFile) {
		return false, -1
	}
	content, err := os.ReadFile(lingmoFstartFile)
	if err != nil {
		return false, -1
	}
...
func setFstartState(state bool) error {
	...
	if lineNum == -1 {
		return errors.New(lingmoFstartFile + "is illegal!")
	}
	if value == state {
	...
	if !strings.Contains(line, lingmoFstart) {
		return errors.New(lingmoFstartFile + "is illegal!")
	}
	arg := "true"
	if !state {
	...
	if strings.Trim(strings.TrimSpace(line[strings.Index(line, "=")+1:]), "\"") != arg {
		lines[lineNum] = "export " + lingmoFstart + "=\"" + arg + "\""
		err := os.WriteFile(lingmoFstartFile, []byte(strings.Join(lines, "\n")), 0644)
		if err != nil {
