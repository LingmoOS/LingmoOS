...
func TestVersion(t *testing.T) {
	...
	lingmo, err := getVersionFromLingmo("testdata/lingmo-version")
	assert.Equal(t, lingmo, "2015 Desktop Alpha1")
	assert.NoError(t, err)
...
func TestDistro(t *testing.T) {
	...
	assert.Equal(t, distroId, "Lingmo")
	assert.Equal(t, distroDesc, "Lingmo 2014.3")
...
func TestParseInfoFile(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, v["DISTRIB_ID"], "Lingmo")
	assert.Equal(t, v["DISTRIB_RELEASE"], "2014.3")
	assert.Equal(t, v["DISTRIB_DESCRIPTION"], strconv.Quote("Lingmo 2014.3"))
}
