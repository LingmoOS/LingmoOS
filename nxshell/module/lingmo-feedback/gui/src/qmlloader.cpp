                          ...
                          const QString &content)
{
    ...
    jsonObj.insert("HelpLingmo",helpLingmo);
...
void QmlLoader::showManual()
{
    if (m_manualPro.isNull()) {
        ...
        const QStringList args("lingmo-feedback");
...
bool QmlLoader::draftNotEmpty(const QString &target)
{
    ...
    return draft.title != "" || draft.content != "" || draft.email != "" ||
            ...
            draft.helpLingmo != true || draft.adjunctPathList.length() > 0;
}
...
void QmlLoader::updateUiDraftData(const QString &target)
{
    ...
    QVariant helpLingmoValue = QVariant(draft.helpLingmo);
    QMetaObject::invokeMethod(
                this->rootObject,
                "setSimpleEntries",
                ...
                Q_ARG(QVariant,helpLingmoValue)
                );
}
...
void QmlLoader::parseJsonData(const QByteArray &byteArray, Draft *draft)
{
    ...
    if(jsonError.error == QJsonParseError::NoError)
    {
        if(parseDoucment.isObject())
        {
            ...
            if (obj.contains("HelpLingmo"))
            {
                QJsonValue helpLingmoValue = obj.take("HelpLingmo");
                if (helpLingmoValue.isBool())
                    draft->helpLingmo = helpLingmoValue.toBool();
            }
        }
    }
}
