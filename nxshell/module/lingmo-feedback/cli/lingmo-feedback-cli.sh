...
get_log_path() {
    local _dir="${1}"; shift
    length=`jq --arg _dir $_dir "."$_dir"|length" /var/lib/lingmo-feedback/feedback_logpath.json`
    _length=`expr $length - 1`
    for (( i=0; i<=$_length; i++  ))
    do
        path=`jq -r --arg _dir $_dir --arg i $i "."$_dir"[$i]" /var/lib/lingmo-feedback/feedback_logpath.json`
...
sliceinfo_basic() {
    ...
    if [ -f /etc/lingmo-version ]; then
        msg_title "Lingmo Release"
        msg_code "$(run cat /etc/lingmo-version)"
    fi

    msg_title "Linux Kernel"
    msg_code "$(run uname -a)"

    msg_title "Installed Lingmo Packages"
    msg_code "$(run ${pkgcmd[@]} | grep -i -e 'lingmo-' -e '-lingmo' -e 'ocean-' -e '-ocean' -e 'lastore')"
...
sliceinfo_battery(){
    ...
    msg_code "        "
    gdbus call -y -d com.lingmo.system.Power -o /com/lingmo/system/Power -m com.lingmo.system.Power.GetBatteries
    msg_code "$(run gdbus introspect -y -d com.lingmo.system.Power -o /com/lingmo/system/Power -rp)"
...
sliceinfo_gsettings() {
    pkexec --user $SUDO_USER gsettings list-recursively | grep com.lingmo
}
...
category_ocean() {
    # category_system include ocean-control-center
    category_system
    subcategory_startocean
    subcategory_background
    subcategory_ocean-desktop
    subcategory_ocean-dock
    subcategory_ocean-launcher
    subcategory_ocean-offline-upgrader
}
subcategory_startocean() {
    include_sliceinfo "syslog"
    include_syslog_keyword "startocean"
}
subcategory_background() {
    include_sliceinfo "syslog"
    include_syslog_keyword "startocean"
...
category_ocean-control-center() {
    ...
    # catch all syslog for ocean-daemon
    include_sliceinfo "syslog"
    ...
    get_log_path "ocean_control_center"
}
subcategory_bootmgr() {
    if [ ! "${arg_privacymode}" ]; then
...
subcategory_ocean-offline-upgrader() {
    include_sliceinfo "syslog"
    get_log_path "ocean_offline_upgrader"
}
...
category_lingmo-installer() {
    get_log_path "lingmo_installer"
    if [ ! "${arg_privacymode}" ]; then
        include_sliceinfo "disk"
    fi
}
category_lingmo-store() {
    subcategory_pkglog
    collect_file "lingmo_store" $(find /var/lib/lastore/ -type f 2>/dev/null | grep -v safecache | grep -v tree)
    get_log_path "lingmo_store"
    get_log_path "ocean_control_center"
}
category_lingmo-music() {
    get_log_path "lingmo_music"
    include_sliceinfo "audio"
    include_sliceinfo "driver"
    include_sliceinfo "kernel"
}
category_lingmo-movie() {
    get_log_path "lingmo_movie"
    include_sliceinfo "video"
    include_sliceinfo "driver"
    include_sliceinfo "kernel"
}
category_lingmo-boot-maker() {
    get_log_path "lingmo_boot_maker"
}
category_lingmo-screenshot() {
    get_log_path "lingmo_screenshot"
}
category_lingmo-terminal() {
    get_log_path "lingmo_terminal"
}
category_lingmo-translator() {
    get_log_path "lingmo_translator"
}
category_lingmo-cloud-print() {
    get_log_path "lingmo_cloud_print"
}
category_lingmo-cloud-scan() {
    get_log_path "lingmo_cloud_scan"
}
category_lingmo-file-manager() {
    get_log_path "ocean_file_manager"
}
category_lingmo-image-viewer() {
    get_log_path "lingmo_image_viewer"
}
category_lingmo-remote-assistance() {
    get_log_path "lingmo_remote_assistance"
}
category_lingmo-graphics-driver-manager() {
    get_log_path "lingmo_graphics_driver_manager"
}
###* Main
...
if [ -f /etc/lingmo-version ]; then
    distro_release="$(cat /etc/lingmo-version | grep Version | awk -F'=' '{print $2}')"
fi
...
if [ "${arg_category}" ]; then
    ...
    export TEXTDOMAIN=lingmo-feedback
    uploadPrompt=$(gettext "Please report the bug by uploading %s as an attachment to lingmo feedback: %s, so that lingmo team can locate and solve the problem.")
    uploadPrompt=$(printf "$uploadPrompt\n" $(pwd)/$result_archive "http://feedback.lingmo.org/feedback")
    msg "$uploadPrompt"
