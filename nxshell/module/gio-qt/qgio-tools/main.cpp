...
//    }
    ...
    DGioFile * f = DGioFile::createFromPath("/media/wzc/_ocean_data");
    if (f) {
    ...
    DGioMount * m = DGioMount::createFromPath("/media/wzc/_ocean_data");
    if (m) {
...
//        m->unmount();
    ...
    QObject::connect(&vm, &DGioVolumeManager::mountAoceand, [](QExplicitlySharedDataPointer<DGioMount> mnt){
        qDebug() << "MountAoceand" << mnt->name();
    });
