...
private Q_SLOTS:
    void testCase_Settings() {
        if (DGioSettings::isSchemaInstalled("com.lingmo.ocean.appearance") == false) {
            qWarning() << "com.lingmo.ocean.appearance" << "invalid";
            return;
        }

        DGioSettings settings("com.lingmo.ocean.appearance", this);
