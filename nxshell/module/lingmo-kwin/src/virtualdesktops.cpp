...
{
    std::ifstream file(std::string(getenv("HOME")) + "/.config/lingmo-kwinrc");
    std::string line;
    int count = 0;
...
void VirtualDesktopManager::initShortcuts()
{
    ...
    //Note: The default gesture that provided by kwin is conflicts with ocean-daemon
}
...
QAction *VirtualDesktopManager::addAction(const QString &name, const KLocalizedString &label, uint value, const QKeySequence &key, void (VirtualDesktopManager::*slot)())
{
    ...
    a->setProperty("componentName", QStringLiteral("lingmo-kwin"));
