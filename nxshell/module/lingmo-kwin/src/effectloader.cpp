...
bool ScriptedEffectLoader::loadEffect(const KPluginMetaData &effect, LoadEffectFlags flags)
{
    ...
    if (!effects->waylandDisplay()) {
        ...
        if (name == "kwin4_effect_squash" || name == "kwin4_effect_maximize"
            ...
            KConfigGroup kwinConfig(KSharedConfig::openConfig("lingmo-kwinrc"), "Compositing");
...
{
    return KPackage::PackageLoader::self()->listPackages(s_serviceType, QStringLiteral("lingmo-kwin/effects"));
}

KPluginMetaData ScriptedEffectLoader::findEffect(const QString &name) const
{
    const auto plugins = KPackage::PackageLoader::self()->findPackages(s_serviceType, QStringLiteral("lingmo-kwin/effects"),
...
PluginEffectLoader::PluginEffectLoader(QObject *parent)
    : AbstractEffectLoader(parent)
    , m_pluginSubDirectory(QStringLiteral("lingmo-kwin/effects/plugins"))
{
}
...
bool PluginEffectLoader::loadEffect(const KPluginMetaData &info, LoadEffectFlags flags)
{
    ...
    if (!effects->waylandDisplay()) {
        ...
        if (name == "magiclamp" || name == "slide") {
            ...
            if (kwinConfig.hasKey("com.lingmo.multitaskingEnabled") && kwinConfig.readEntry("com.lingmo.multitaskingEnabled") == "false") {
                return false;
            }
        }
    }
