...
Application::Application(Application::OperationMode mode, int &argc, char **argv)
    ...
    , m_configLock(false)
    , m_config(KSharedConfig::openConfig(QStringLiteral("lingmo-kwinrc")))
    , m_kxkbConfig()
...
{
    KAboutData aboutData(QStringLiteral("lingmo-kwin"), // The program name used internally
...
{
    KLocalizedString::setApplicationDomain("lingmo-kwin");
}
