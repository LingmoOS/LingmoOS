...
#define _LINGMO_NET_EFFECT   "_LINGMO_NET_EFFECT"
#define _LINGMO_NET_STARTUP  "_LINGMO_NET_STARTUP"
#define _LINGMO_NET_RADIUS   "_LINGMO_NET_RADIUS"
#define _LINGMO_NET_SHADOW   "_LINGMO_NET_SHADOW"
...
X11DecorationStyle::effectScenes X11DecorationStyle::getWindowEffectScene()
{
    ...
    if (m_validProperties.testFlag(WindowEffectProperty)) {
        ...
        auto scenAtom = Utils::internAtom(_LINGMO_NET_EFFECT);
...
{
    auto atom = Utils::internAtom(_LINGMO_NET_RADIUS);
...
{
    auto atom = Utils::internAtom(_LINGMO_NET_SHADOW);
    const QByteArray data = Utils::readWindowProperty(m_window->window(), atom, XCB_ATOM_STRING);
    if (!data.isEmpty()) {
...
void X11DecorationStyle::parseWinStartUpEffect()
{
    if (m_validProperties.testFlag(WindowStartUpEffectProperty)) {
        ...
        auto atom = Utils::internAtom(_LINGMO_NET_STARTUP);
