...
Q_GLOBAL_STATIC_WITH_ARGS(QGSettings, _gsettings_lingmo_xsetting, ("com.lingmo.xsettings"))
...
WindowStyleManager::WindowStyleManager()
{
    ...
    QDBusConnection::sessionBus().connect(DBUS_LINGMO_WM_SERVICE, DBUS_LINGMO_WM_OBJ, DBUS_LINGMO_WM_INTF, "compositingEnabledChanged", this, SLOT(onCompositingChanged(bool)));
...
void WindowStyleManager::onWindowAoceand(Window *window)
{
...
void WindowStyleManager::onThemeChange(QVariant property)
{
    ...
    bool isDark = (str == "lingmo-dark") ? true : false;
...
float WindowStyleManager::getOsRadius()
{
    if (m_osRadius <= 0.0) {
        if (m_radiusConfig.get()->getProperty().isValid()) {
            ...
            m_osRadius = _gsettings_lingmo_xsetting->get(GsettingsDtkRadius).toInt();
        }
    }
