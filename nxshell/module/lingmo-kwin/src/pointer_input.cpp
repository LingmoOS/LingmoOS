    ...
    , m_cursor(nullptr)
{
    if (waylandServer()) {
        ...
        if (oceanKvm) {
            connect(oceanKvm, &KWaylandServer::OCEANKvmInterface::kvmInterfaceEnableCursorRequested, this, &PointerInputRedirection::slotKvmInterfaceEnableCursor);
            connect(oceanKvm, &KWaylandServer::OCEANKvmInterface::kvmInterfaceSetCursorPosRequested, this, &PointerInputRedirection::slotKvmInterfaceSetCursorPos);
            connect(oceanKvm, &KWaylandServer::OCEANKvmInterface::kvmInterfaceEnablePointerRequested, this, &PointerInputRedirection::slotKvmEnablePointerChange);
        }
    }
}
...
void PointerInputRedirection::init()
{
    ...
    if (waylandServer()->oceanSeat()) {
        waylandServer()->oceanSeat()->setHasPointer(input()->hasPointer());
        connect(input(), &InputRedirection::hasPointerChanged,
                waylandServer()->oceanSeat(), &KWaylandServer::OCEANSeatInterface::setHasPointer);
    }
    ...
    connect(workspace(), &Workspace::windowAoceand, this, setupMoveResizeConnection);
...
void PointerInputRedirection::processSwipeGestureBegin(int fingerCount, std::chrono::microseconds time, KWin::InputDevice *device)
{
    ...
    // we aoceand an dbus interface to disable gestures to fix this bug.
