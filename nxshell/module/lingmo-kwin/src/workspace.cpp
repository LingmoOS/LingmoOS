    ...
    , m_splitManage(std::make_unique<SplitManage>())
{
    ...
    QDBusConnection::sessionBus().connect(QString(), QString(), DBUS_LINGMO_WM_INTF, "TileActiveWindowChanged", this, SLOT(tileActiveWindow(int)));
    QDBusConnection::sessionBus().connect(QString(), QString(), DBUS_LINGMO_WM_INTF, "ToggleActiveWindowMaximizeChanged", this, SLOT(toggleActiveMaximize()));
}
...
void Workspace::init()
{
    ...
    connect(vds, &VirtualDesktopManager::desktopCreated, this, &Workspace::slotDesktopAoceand);
    ...
    connect(this, &Workspace::windowAoceand, m_windowStyleManager.get(), &WindowStyleManager::onWindowAoceand);
    connect(this, &Workspace::unmanagedAoceand, m_windowStyleManager.get(), &WindowStyleManager::onWindowAoceand);
    connect(this, &Workspace::internalWindowAoceand, m_windowStyleManager.get(), &WindowStyleManager::onWindowAoceand);
    ...
    if (auto server = waylandServer()) {
        connect(server, &WaylandServer::windowAoceand, this, &Workspace::addWaylandWindow);
    ...
    connect(this, &Workspace::windowAoceand, m_placementTracker.get(), &PlacementTracker::add);
    connect(this, &Workspace::windowRemoved, m_placementTracker.get(), &PlacementTracker::remove);

    connect(this, &Workspace::windowAoceand, m_splitManage.get(), &SplitManage::add);
    connect(this, &Workspace::windowRemoved, m_splitManage.get(), &SplitManage::remove);
    connect(this, &Workspace::unmanagedAoceand, m_splitManage.get(), &SplitManage::add);
    connect(this, &Workspace::internalWindowAoceand, m_splitManage.get(), &SplitManage::add);
    ...
    auto req = QDBusConnection::sessionBus().interface()->isServiceRegistered("org.lingmo.ocean.daemon.Dock1");

    if (req.value()) {
        m_dockInter = new org::lingmo::ocean::daemon::Dock1(
            "org.lingmo.ocean.daemon.Dock1",
            "/org/lingmo/ocean/daemon/Dock1",
            QDBusConnection::sessionBus(),
            this);
        if (m_dockInter) {
            setDockLastPosition(m_dockInter->frontendWindowRect());
            connect(m_dockInter, &org::lingmo::ocean::daemon::Dock1::FrontendWindowRectChanged, this, &Workspace::slotDockPositionChanged);
        }
    } else {
        QDBusServiceWatcher *watcher = new QDBusServiceWatcher(this);
        watcher->setConnection(QDBusConnection::sessionBus());
        watcher->addWatchedService("org.lingmo.ocean.daemon.Dock1");
...
Unmanaged *Workspace::createUnmanaged(xcb_window_t windowId)
{
    ...
    Q_EMIT unmanagedAoceand(window);
    reportTimeToSpanEventTracking(createTimeval, window);
    return window;
}
...
void Workspace::addX11Window(X11Window *window)
{
    ...
    addToStack(window);
    updateClientArea(); // This cannot be in manage(), because the window got aoceand only now
    window->updateLayer();
    if (window->isDesktop()) {
        raiseWindow(window);
...
void Workspace::updateToolWindows(bool also_hide)
{
    ...
    // SELI TODO: But maybe it should - what if a new window has been aoceand that's not in stacking order yet?
...
void Workspace::updateOutputs(const QVector<Output *> &outputOrder)
{
    ...
    const auto aoceand = outputsSet - oldOutputsSet;
    for (Output *output : aoceand) {
        output->ref();
        m_tileManagers[output] = std::make_unique<TileManager>(output);
        Q_EMIT outputAoceand(output);
    }
...
void Workspace::slotDesktopAoceand(VirtualDesktop *desktop)
{
...
void Workspace::addInternalWindow(InternalWindow *window)
{
    ...
    Q_EMIT internalWindowAoceand(window);
}
...
KWaylandServer::OCEANShellSurfaceInterface* Workspace::getOCEANShellSurface(Window* c)
{
    if (!c) {
        return nullptr;
    }
...
{
    auto oceanShellSurface = getOCEANShellSurface(window);
    if (!oceanShellSurface) {
        return;
    }
    for (auto output : m_outputs) {
        oceanShellSurface->sendWorkArea(waylandServer()->findWaylandOutput(output), clientArea(MaximizeArea, window, output).toRect());
    }
}
...
void Workspace::setWinSplitState(Window *w, bool isSplit)
{
    ...
    if (w) {
        xcb_intern_atom_cookie_t cookie_st = xcb_intern_atom( connection(), 0, strlen( "_LINGMO_NET_SUPPORTED"), "_LINGMO_NET_SUPPORTED");
            ...
            if (!c->isDock() && !c->isDesktop()) {
                if (list.contains(c)) {
                    if (c->isMinimized()) {
                        ...
                        c->unminimize(true);
                        // 记录窗口的最小化状态
                        c->setProperty("__lingmo_kwin_minimized", true);
                    }
                } else {
                    if (c->property("__lingmo_kwin_minimized").toBool()) {
                        c->setProperty("__lingmo_kwin_minimized", QVariant());
                        // 恢复窗口被预览之前的状态
                        c->minimize(true);
                    }
                }
