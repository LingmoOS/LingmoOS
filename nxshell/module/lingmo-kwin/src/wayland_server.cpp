...
#include "wayland/oceanseat_interface.h"
#include <wayland/oceansecurity_interface.h>
#include <wayland/oceanrestrict_interface.h>
#include <wayland/oceankvm_interface.h>
...
void WaylandServer::registerXdgToplevelWindow(XdgToplevelWindow *window)
{
    ...
    if (auto shellSurface = OCEANShellSurfaceInterface::get(surface)) {
        window->installOCEANShellSurface(shellSurface);
    }
...
void WaylandServer::registerXdgGenericWindow(Window *window)
{
    ...
    if (auto popup = qobject_cast<XdgPopupWindow *>(window)) {
        ...
        if (auto shellSurface = OCEANShellSurfaceInterface::get(window->surface())) {
            popup->installOCEANShellSurface(shellSurface);
        }
        return;
    }
...
void WaylandServer::handleOutputAoceand(Output *output)
{
...
bool WaylandServer::init(InitializationFlags flags)
{
    ...
    connect(this, &WaylandServer::windowAoceand, idleInhibition, &IdleInhibition::registerClient);
    ...
    m_oceanShell = new OCEANShellInterface(m_display, m_display);
    connect(m_oceanShell, &OCEANShellInterface::shellSurfaceCreated,
        [this] (OCEANShellSurfaceInterface *shellSurface) {
            if (XdgSurfaceWindow *client = findXdgSurfaceWindow(shellSurface->surface())) {
                client->installOCEANShellSurface(shellSurface);
            }
    ...
    connect(m_strut, &StrutInterface::setStrut,
        [this] (SurfaceInterface *surface, struct lingmoKwinStrut& strutArea) {
    ...
    m_oceanSeat = new OCEANSeatInterface(m_display, m_display);

    m_oceanSecurity = new OCEANSecurityInterface(m_display, m_display);
    m_seat->addSecurityInterface(m_oceanSecurity);

    m_oceanRestrict = new OCEANRestrictInterface(m_display, m_display);
    m_oceanKvm = new OCEANKvmInterface(m_display, m_display);
...
void WaylandServer::initWorkspace()
{
    ...
    for (Output *output : availableOutputs) {
        handleOutputAoceand(output);
    }
    connect(kwinApp()->outputBackend(), &OutputBackend::outputAoceand, this, &WaylandServer::handleOutputAoceand);
    ...
    connect(workspace(), &Workspace::outputAoceand, this, &WaylandServer::handleOutputEnabled);
...
{
    QList<QByteArray> white_windows = m_oceanRestrict->clientWhitelists();
...
{
    connect(server, &WaylandServer::windowAoceand, this, [this](Window *window) {
