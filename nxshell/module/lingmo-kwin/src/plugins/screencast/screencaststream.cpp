...
bool ScreenCastStream::createStream()
{
    ...
    if (m_cursor.mode == KWaylandServer::ScreencastV1Interface::Embeoceand) {
...
void ScreenCastStream::recordFrame(const QRegion &_damagedRegion)
{
    ...
    if (data || spa_data[0].type == SPA_DATA_MemFd) {
        ...
        if (m_cursor.mode == KWaylandServer::ScreencastV1Interface::Embeoceand && m_cursor.viewport.contains(cursor->pos())) {
