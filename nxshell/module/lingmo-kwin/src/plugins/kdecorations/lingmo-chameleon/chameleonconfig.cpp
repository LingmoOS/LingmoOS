...
#define OCEAN_FORCE_DECORATE "__ocean__force_decorate"
#define OCEAN_NEED_UPDATE_NOBORDER "__ocean__need_update_noborder"
...
ChameleonConfig::ChameleonConfig(QObject *parent)
    : QObject(parent)
{
    m_atom_lingmo_chameleon = KWinUtils::internAtom(_LINGMO_CHAMELEON, false);
    m_atom_lingmo_no_titlebar = KWinUtils::internAtom(_LINGMO_NO_TITLEBAR, false);
    m_atom_lingmo_force_decorate = KWinUtils::internAtom(_LINGMO_FORCE_DECORATE, false);
    m_atom_lingmo_scissor_window = KWinUtils::internAtom(_LINGMO_SCISSOR_WINDOW, false);
...
quint32 ChameleonConfig::atomLingmoChameleon() const
{
    return m_atom_lingmo_chameleon;
}

quint32 ChameleonConfig::atomLingmoNoTitlebar() const
{
    return m_atom_lingmo_no_titlebar;
}

quint32 ChameleonConfig::atomLingmoScissorWindow() const
{
    return m_atom_lingmo_scissor_window;
}
...
{
    KConfig config("lingmo-kwinrc", KConfig::CascadeConfig);
    KConfigGroup group_decoration(&config, "org.kde.kdecoration2");

    bool active = group_decoration.readEntry("library") == "com.lingmo.chameleon";

    setActivated(active);

    KConfigGroup group(&config, "lingmo-chameleon");
    const QString &theme_info = group.readEntry("theme");
    setTheme(theme_info);
}

#define D_KWIN_DEBUG_APP_START_TIME "D_KWIN_DEBUG_APP_START_TIME"
void ChameleonConfig::onClientAoceand(KWin::Window *client)
{
...
void ChameleonConfig::onUnmanagedAoceand(KWin::Unmanaged *client)
{
    QObject *c = reinterpret_cast<QObject*>(client);

    qCDebug(CHAMELEON) << "onUnmanagedAoceand: " << QString("0x%1").arg(c->property("windowId").toULongLong(), 0, 16) << "windowType: " << c->property("windowType").toInt();
...
void ChameleonConfig::onInternalWindowAoceand(KWin::InternalWindow *client)
{
...
void ChameleonConfig::onShellClientAoceand(KWin::WaylandWindow *client)
{
    QObject *c = reinterpret_cast<QObject*>(client);
...
void ChameleonConfig::onWindowPropertyChanged(quint32 windowId, quint32 atom)
{
    if (atom == m_atom_lingmo_no_titlebar) {
        ...
        bool force_decorate = client->property(OCEAN_FORCE_DECORATE).toBool();
...
void ChameleonConfig::updateWindowNoBorderProperty(QObject *window)
{
    ...
    if (window->property(OCEAN_NEED_UPDATE_NOBORDER).toBool()) {
        // 清理掉属性，避免下次重复更新
        window->setProperty(OCEAN_NEED_UPDATE_NOBORDER, QVariant());

        // 应该更新窗口的noBorder属性
        if (window->property(OCEAN_FORCE_DECORATE).toBool()) {
...
{
    // 如果属性__ocean__ignore_blur_behind_changed有效，且函数是因为WindowBlurBehindRole变化被调用
    // 则表示这次变化是由updateWindowBlurArea本身引起的，应该忽略此次调用
    if (role == KWin::WindowBlurBehindRole && window->property("__ocean__ignore_blur_behind_changed").isValid()) {
        // 清理标记的属性
        window->setProperty("__ocean__ignore_blur_behind_changed", QVariant());
        return;
    }
    ...
    if (role != KWin::WindowBlurBehindRole) {
        const QVariant &cache_blur_area = window->property("__ocean__blur_behind_role");
    ...
    if (!blur_area.isValid()) {
        // 清理缓存的属性
        window->setProperty("__ocean__blur_behind_role", QVariant());
        return;
    }
    ...
    if (path.isEmpty() && (qIsNull(radius.x()) || qIsNull(radius.y()))) {
        const QVariant &blur_area = window->property("__ocean__blur_behind_role");
    ...
    // 更新窗口的原始模糊数据, 用于后期的恢复
    if (role == KWin::WindowBlurBehindRole
            || !window->property("__ocean__blur_behind_role").isValid()) {
        window->setProperty("__ocean__blur_behind_role", blur_area);
    }
    ...
    window->setProperty("__ocean__ignore_blur_behind_changed", true);
...
void ChameleonConfig::updateWindowSize()
{
    ...
    const QSize &old_size = window->property("__ocean__old_size").toSize();
    const QSize &size = window->property("size").toSize();

    if (old_size == size)
        return;

    window->setProperty("__ocean_old_size", size);
...
{
    const QByteArray &force_decorate = KWinUtils::instance()->readWindowProperty(client, m_atom_lingmo_force_decorate, XCB_ATOM_CARDINAL);
        ...
        if (set_border) {
            if (client->property("noBorder").toBool()) {
                ...
                if (setWindowOverrideType(client, false)) {
                    ...
                    client->setProperty(OCEAN_NEED_UPDATE_NOBORDER, true);
                } else {
                    client->setProperty("noBorder", false);
                }
                client->setProperty(OCEAN_FORCE_DECORATE, true);
            }
        } else {
            client->setProperty(OCEAN_FORCE_DECORATE, true);
        }
    } else if (client->property(OCEAN_FORCE_DECORATE).toBool()) {
        client->setProperty(OCEAN_FORCE_DECORATE, QVariant());
...
void ChameleonConfig::updateClientClipPath(QObject *client)
{
    ...
    QPainterPath path;
    const QByteArray &clip_data = effect->readProperty(m_atom_lingmo_scissor_window, m_atom_lingmo_scissor_window, 8);
...
void ChameleonConfig::init()
{
    ...
    connect(Workspace::self(), SIGNAL(windowAoceand(KWin::Window*)), this, SLOT(onClientAoceand(KWin::Window*)));
    connect(Workspace::self(), SIGNAL(unmanagedAoceand(KWin::Unmanaged*)), this, SLOT(onUnmanagedAoceand(KWin::Unmanaged*)));
    connect(Workspace::self(), SIGNAL(internalWindowAoceand(KWin::InternalWindow*)), this, SLOT(onInternalWindowAoceand(KWin::InternalWindow*)));
...
void ChameleonConfig::setActivated(const bool active)
{
    ...
    if (active) {
        if (KWinUtils::compositorIsActive()) {
            ...
            KWinUtils::instance()->addSupportedProperty(m_atom_lingmo_scissor_window, false);
        }

        KWinUtils::instance()->addSupportedProperty(m_atom_lingmo_chameleon, false);
        KWinUtils::instance()->addSupportedProperty(m_atom_lingmo_no_titlebar, false);
        KWinUtils::instance()->addSupportedProperty(m_atom_lingmo_force_decorate);

        // 监听属性变化
        KWinUtils::instance()->addWindowPropertyMonitor(m_atom_lingmo_no_titlebar);
        KWinUtils::instance()->addWindowPropertyMonitor(m_atom_lingmo_force_decorate);
        KWinUtils::instance()->addWindowPropertyMonitor(m_atom_lingmo_scissor_window);
        ...
        KWinUtils::instance()->removeSupportedProperty(m_atom_lingmo_scissor_window, false);
        KWinUtils::instance()->removeSupportedProperty(m_atom_lingmo_chameleon, false);
        KWinUtils::instance()->removeSupportedProperty(m_atom_lingmo_no_titlebar, false);
        KWinUtils::instance()->removeSupportedProperty(m_atom_lingmo_force_decorate);

        // 取消监听属性变化
        KWinUtils::instance()->removeWindowPropertyMonitor(m_atom_lingmo_no_titlebar);
        KWinUtils::instance()->removeWindowPropertyMonitor(m_atom_lingmo_force_decorate);
        KWinUtils::instance()->removeWindowPropertyMonitor(m_atom_lingmo_scissor_window);
...
bool ChameleonConfig::setWindowOverrideType(QObject *client, bool enable)
{
    ...
    if (enable && !client->property("__ocean__override_type").toBool()) {
        return false;
    }
    ...
    if (!enable) {
        ...
        if (atom_list.removeAll(_KDE_NET_WM_WINDOW_TYPE_OVERRIDE)) {
            ...
            client->setProperty("__ocean__override_type", true);
        ...
        if (QX11Info::isPlatformX11())
            xcb_flush(QX11Info::connection());
        // 取消窗口override类型标记
        client->setProperty("__ocean__override_type", QVariant());
