...
bool KWinUtils::isLingmoOverride(const QObject *window) const
{
    bool ok = false;
    qulonglong wid;
    QByteArray data;
    ...
    static xcb_atom_t property_atom = internAtom("_LINGMO_OVERRIDE", true);
    if (property_atom == XCB_ATOM_NONE) {
        goto out;
    }
...
KWaylandServer::OCEANShellSurfaceInterface *KWinUtils::getOCEANShellSurface(QObject * shellClient)
{
    if (!shellClient) {
        return nullptr;
    }
    ...
    return Workspace::self()->getOCEANShellSurface(c);
}
