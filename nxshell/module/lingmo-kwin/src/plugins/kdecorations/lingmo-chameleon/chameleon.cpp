...
void Chameleon::init()
{
    ...
    if (!QX11Info::isPlatformX11() && m_client) {
        m_oceanShellSurface = KWinUtils::getOCEANShellSurface(m_client);
        if (m_oceanShellSurface) {
            connect(m_oceanShellSurface, &KWaylandServer::OCEANShellSurfaceInterface::noTitleBarPropertyRequested, this,
                [this, c] (qint32 value) {
            ...
            connect(m_oceanShellSurface, &KWaylandServer::OCEANShellSurfaceInterface::windowRadiusPropertyRequested, this,
...
bool Chameleon::noTitleBar() const
{
    if (m_noTitleBar < 0) {
        ...
        const QByteArray &data = KWinUtils::instance()->readWindowProperty(client().data()->windowId(),
                                                                           ChameleonConfig::instance()->atomLingmoNoTitlebar(),
                                                                           XCB_ATOM_CARDINAL);
