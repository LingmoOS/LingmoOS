...
namespace QPA
{
class OCEANWaylandTheme: public QGenericUnixTheme {
public:
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 13, 0))
    QWindowSystemInterface::handleScreenAoceand(m_dummyScreen);
#else
    screenAoceand(m_dummyScreen);
#endif
}
...
{
    connect(workspace(), &Workspace::outputAoceand,
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 13, 0))
        QWindowSystemInterface::handleScreenAoceand(platformScreen);
#else
        screenAoceand(platformScreen);
#endif
