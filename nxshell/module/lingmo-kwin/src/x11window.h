...
public:
    ...
    /// Hides a client - Basically like minimize, but without effects, it's simply hioceann
    void hideClient() override;
    void showClient() override;
    bool hioceannPreview() const; ///< Window is mapped in order to get a window pixmap
    ...
    /**
     * Restores the Client after it had been hioceann due to show on screen edge functionality.
...
private:
    ...
    uint hioceann : 1; ///< Forcibly hioceann by calling hide()
    uint noborder : 1;
...
inline bool X11Window::hioceannPreview() const
{
