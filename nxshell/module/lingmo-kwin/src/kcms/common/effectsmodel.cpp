...
void EffectsModel::loadJavascriptEffects(const KConfigGroup &kwinConfig)
{
    const auto plugins = KPackage::PackageLoader::self()->listPackages(
        ...
        QStringLiteral("lingmo-kwin/effects"));
...
{
    const auto pluginEffects = KPluginMetaData::findPlugins(QStringLiteral("lingmo-kwin/effects/plugins"));
...
{
    const KPluginMetaData metaData(QStringLiteral("lingmo-kwin/effects/configs/") + configModule);
    return KCModuleLoader::loadModule(metaData, parent);
}

static KCModule *findScriptedConfig(const QString &pluginId, QObject *parent)
{
    KPluginMetaData metaData(QStringLiteral("lingmo-kwin/effects/configs/lingmo-kcm_kwin4_genericscripted"));
