 ...
 * having a hioceann panel there. On X11 (currently only supported backend) the @ref Edge is
...
public:
     ...
     * and if the client is already hioceann, the Edge gets reserved.
     *
     * Once the Edge for the client triggers, the client gets shown again and the Edge unreserved.
     * The idea is that the Edge can only get activated if the client is currently hioceann.
     *
