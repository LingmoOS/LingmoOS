    ...
    , m_touch(touch)
{
    ...
    connect(touch, &Touch::pointAoceand, this, [this](TouchPoint *tp) {
        Q_EMIT touchDown(tp->id(), tp->position(), std::chrono::milliseconds(tp->time()), this);
    });
...
void WaylandInputBackend::initialize()
{
    ...
    if (seat->relativePointerDevice()) {
        Q_EMIT deviceAoceand(seat->relativePointerDevice());
    }
    if (seat->pointerDevice()) {
        Q_EMIT deviceAoceand(seat->pointerDevice());
    }
    if (seat->keyboardDevice()) {
        Q_EMIT deviceAoceand(seat->keyboardDevice());
    }
    if (seat->touchDevice()) {
        Q_EMIT deviceAoceand(seat->touchDevice());
    }

    connect(seat, &WaylandSeat::deviceAoceand, this, &InputBackend::deviceAoceand);
...
void WaylandSeat::createPointerDevice()
{
    ...
    Q_EMIT deviceAoceand(m_pointerDevice.get());
}
...
void WaylandSeat::createRelativePointer()
{
    ...
    if (manager) {
        ...
        Q_EMIT deviceAoceand(m_relativePointerDevice.get());
    }
}
...
void WaylandSeat::createKeyboardDevice()
{
    ...
    Q_EMIT deviceAoceand(m_keyboardDevice.get());
}
...
void WaylandSeat::createTouchDevice()
{
    ...
    Q_EMIT deviceAoceand(m_touchDevice.get());
}
