...
public:
    ConnectionAdaptor(Connection *con)
        ...
        , m_con(con)
    {
        connect(con, &Connection::deviceAoceand, this, [this](LibInput::Device *inputDevice) {
            Q_EMIT deviceAoceand(inputDevice->sysName());
        });
...
Q_SIGNALS:
    void deviceAoceand(QString sysName);
    ...
    , m_mutex(QMutex::Recursive)
{
    ...
    if (waylandServer()) {
        auto oceanKvm = waylandServer()->oceanKvm();
        if (oceanKvm) {
            connect(oceanKvm, &KWaylandServer::OCEANKvmInterface::kvmInterfaceEnablePointerRequested, this, &Connection::slotKvmEnablePointerChange);
            connect(oceanKvm, &KWaylandServer::OCEANKvmInterface::kvmInterfaceEnableKeyboardRequested, this, &Connection::slotKvmEnableKeyboardChange);
        }
    }
}
...
void Connection::processEvents()
{
    ...
    while (m_eventQueue.size() != 0) {
        ...
        switch (event->type()) {
        case LIBINPUT_EVENT_DEVICE_AOCEAND: {
        ...
        case LIBINPUT_EVENT_KEYBOARD_KEY: {
            ...
            if (waylandServer()) {
                auto oceanKvm = waylandServer()->oceanKvm();
                if (oceanKvm) {
                    oceanKvm->setkvmKeyboardTimestamp(static_cast<quint32>(ke->time().count()));
                    oceanKvm->updateKey(ke->key(), 0, ke->state());
                }
            }
            break;
        }
        case LIBINPUT_EVENT_POINTER_AXIS: {
            ...
            for (const InputRedirection::PointerAxis &axis : axes) {
                ...
                if (waylandServer()) {
                    auto oceanKvm = waylandServer()->oceanKvm();
                    if (oceanKvm) {
                        oceanKvm->setKvmPointerTimestamp(static_cast<quint32>(pointerEvent->time().count()));
                        if (axis == InputRedirection::PointerAxis::PointerAxisVertical) {
                            oceanKvm->pointerAxis(Qt::Orientation::Vertical, qRound(pointerEvent->scrollValue(axis)));
                        } else {
                            oceanKvm->pointerAxis(Qt::Orientation::Horizontal, qRound(pointerEvent->scrollValue(axis)));
                        }
                    }
                }
            }
        ...
        case LIBINPUT_EVENT_POINTER_BUTTON: {
            ...
            if (waylandServer()) {
                auto oceanKvm = waylandServer()->oceanKvm();
                if (oceanKvm) {
                    oceanKvm->setKvmPointerTimestamp(static_cast<quint32>(pe->time().count()));
                    oceanKvm->pointerButton(pe->button(), pe->buttonState(), 0, pe->absoluteKvmPos(workspace()->geometry().size()));
                }
            }
            break;
        }
