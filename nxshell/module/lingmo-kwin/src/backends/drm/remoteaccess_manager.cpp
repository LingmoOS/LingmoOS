    ...
    : QObject(parent)
{
    if (waylandServer()) {
        ...
        connect(m_interface, &RemoteAccessManagerInterface::aoceandClient, this, [=]() {
            Q_EMIT aoceandClient();
        });
...
void RemoteAccessManager::passBuffer(Output *output, DrmGpuBuffer *buffer)
{
    ...
    if (ocean_restrict && ocean_restrict->prohibitScreencast() && waylandServer()->hasProhibitWindows()) {
        return;
    }
