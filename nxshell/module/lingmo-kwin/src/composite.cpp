...
#define GSETTINGS_OCEAN_APPEARANCE "com.lingmo.ocean.appearance"
...
bool Compositor::setupStart()
{
    ...
    if (!waylandServer() && m_backend && QGSettings::isSchemaInstalled(GSETTINGS_OCEAN_APPEARANCE)) {
        ...
        if (m_backend->compositingType() == XRenderCompositing) {
            QGSettings(GSETTINGS_OCEAN_APPEARANCE).set("opacity", 1);
        } else {
            KConfigGroup config(KSharedConfig::openConfig("lingmo-kwinrc"), "Compositing");
            float opacity = config.readEntry("OpacityRecord", QVariant(-1.0f)).toFloat();
            if (opacity != -1.0f) {
                QGSettings(GSETTINGS_OCEAN_APPEARANCE).set("opacity", opacity);
            }
        }
    }
...
void Compositor::startupWithWorkspace()
{
    ...
    if (kwinApp()->operationMode() == Application::OperationModeX11) {
        ...
        connect(workspace(), &Workspace::outputAoceand, this, &Compositor::addOutput);
...
void Compositor::addOutput(Output *output)
{
    ...
    connect(Cursors::self(), &Cursors::hioceannChanged, cursorLayer, updateCursorLayer);
...
void Compositor::stop()
{
    ...
    if (!waylandServer() && m_backend && m_backend->compositingType() & OpenGLCompositing
            && QGSettings::isSchemaInstalled(GSETTINGS_OCEAN_APPEARANCE)) {
        KConfigGroup config(KSharedConfig::openConfig("lingmo-kwinrc"), "Compositing");
        float opacity = QGSettings(GSETTINGS_OCEAN_APPEARANCE).get("opacity").toFloat();
    ...
    if (Workspace::self()) {
        ...
        disconnect(workspace(), &Workspace::outputAoceand, this, &Compositor::addOutput);
