...
#define KWinDBusService "org.lingmo.ocean.Appearance1"
#define KWinDBusPath "/org/lingmo/ocean/Appearance1"
#define KWinDBusInterface "org.lingmo.ocean.Appearance1"
...
void CompositedOutlineVisual::show()
{
    ...
    if (!m_qmlComponent) {
        ...
        const QString fileName = QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                                        kwinApp()->config()->group(QStringLiteral("Outline")).readEntry("QmlPath", QStringLiteral("lingmo-kwin/outline/plasma/outline.qml")));
        if (fileName.isEmpty()) {
