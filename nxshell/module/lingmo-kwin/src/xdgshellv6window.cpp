...
#include "wayland/oceanshell_interface.h"
#include "wayland_server.h"
#include "workspace.h"
#include "useractions.h"
#include "wayland/oceanrestrict_interface.h"
    ...
    , m_configureTimer(new QTimer(this))
{
    ...
    connect(treeMonitor, &SubSurfaceMonitor::subSurfaceAoceand,
...
void XdgSurfaceV6Window::handleConfigureAcknowledged(quint32 serial)
{
    ...
    // The xdg protocol does not have a minimized state, and this state needs to be supplemented through the ocean shell protocol
    if (m_oceanShellSurface && isMinimized()) {
        m_oceanShellSurface->setMinimized(false);
        m_oceanShellSurface->setMinimized(isMinimized());
    }
}
...
void XdgSurfaceV6Window::destroyWindow()
{
    ...
    auto ocean_restrict = waylandServer()->oceanRestrict();
    if (ocean_restrict) {
         auto protectedWindowIdLists = ocean_restrict->protectedWindowIdLists();
...
void XdgSurfaceV6Window::installOCEANShellSurface(OCEANShellSurfaceInterface *shellSurface)
{
    m_oceanShellSurface = shellSurface;
    ...
    connect(this, &XdgSurfaceV6Window::frameGeometryChanged, this,
        [this] {
            ...
            if (isDecorated()) {
                ...
                m_oceanShellSurface->sendGeometry(clientGeom.toAlignedRect());
            } else {
                m_oceanShellSurface->sendGeometry(frameGeometry().toAlignedRect());
            }
        }
    );
    ...
    connect(this, &Window::activeChanged, this,
            [this] {
                m_oceanShellSurface->setActive(isActive());
                Q_EMIT workspace()->windowStateChanged();
                }
            );
    connect(this, &Window::fullScreenChanged, this,
            [this] {
                m_oceanShellSurface->setFullscreen(isFullScreen());
                Q_EMIT workspace()->windowStateChanged();
                }
            );
    connect(this, &Window::keepAboveChanged, m_oceanShellSurface, &OCEANShellSurfaceInterface::setKeepAbove);
    connect(this, &Window::keepBelowChanged, m_oceanShellSurface, &OCEANShellSurfaceInterface::setKeepBelow);
    ...
    connect(this, static_cast<void (Window::*)(Window *, MaximizeMode, bool)>(&Window::clientMaximizedStateChanged), this,
        [this] (KWin::Window *c, MaximizeMode mode, bool animated) {
            Q_UNUSED(c);
            m_oceanShellSurface->setMaximized(mode == KWin::MaximizeFull);
            Q_EMIT workspace()->windowStateChanged();
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::activationRequested, this,
        [this] {
            workspace()->activateWindow(this);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::minimizedRequested, this,
        [this] (bool set) {
            if (set) {
                minimize();
            } else {
    ...
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::maximizedRequested, this,
        [this] (bool set) {
            maximize(set ? MaximizeFull : MaximizeRestore);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::keepAboveRequested, this,
        [this] (bool set) {
            setKeepAbove(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::keepBelowRequested, this,
        [this] (bool set) {
            setKeepBelow(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::activeRequested, this,
        [this] (bool set) {
            if (set) {
    ...
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::onAllDesktopsRequested, this,
        [this] (bool set) {
            if (set) {
    ...
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::minimizeableRequested, this,
        [this] (bool set) {
            setMinimizeable(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::maximizeableRequested, this,
        [this] (bool set) {
            setMaximizeable(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::resizableRequested, this,
        [this] (bool set) {
            setResizable(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::closeableRequested, this,
        [this] (bool set) {
            setCloseable(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::acceptFocusRequested, this,
        [this] (bool set) {
            setAcceptFocus(set);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::modalityRequested, this,
        [this] (bool set) {
            setModal(set);
        }
    );
    // porting to do: send the value to ocean-kwin?
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::noTitleBarPropertyRequested, this,
        [this] (qint32 value) {
            m_noTitleBar = value;
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::windowRadiusPropertyRequested, this,
        [this] (QPointF windowRadius) {
            m_windowRadius = windowRadius;
            Q_EMIT waylandWindowRadiusChanged(windowRadius);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::shadowColorPropertyRequested  , this,
        [this] (QString shadowColor) {
            Q_EMIT waylandShadowColorChanged(shadowColor);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::borderWidthPropertyRequested, this,
        [this] (qint32 width) {
            Q_EMIT waylandBorderWidthChanged(width);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::borderColorPropertyRequested, this,
        [this] (QString borderColor) {
            Q_EMIT waylandBorderColorChanged(borderColor);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::splitWindowRequested, this,
        [this] (KWaylandServer::SplitType type) {
            Window::setQuickTileFromMenu(QuickTileMode(int(type)));

        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::windowEffectRequested, this,
        [this] (uint32_t type) {
            Q_EMIT waylandWindowEffectChanged(type);
        }
    );
    connect(m_oceanShellSurface, &OCEANShellSurfaceInterface::windowStartUpEffectRequested, this,
        [this] (uint32_t type) {
...
KWaylandServer::OCEANShellSurfaceInterface *XdgSurfaceV6Window::oceanShellSurface() const {
    return m_oceanShellSurface.data();
}
...
StrutRect XdgToplevelV6Window::strutRect(StrutArea area) const
{
    ...
    KWaylandServer::lingmoKwinStrut strutArea = strut();
...
{
    if (m_oceanShellSurface && !m_acceptFocus) {
...
bool XdgToplevelV6Window::acceptsFocus() const
{
    ...
    if (!m_oceanShellSurface.isNull() && !m_acceptFocus) {
        return m_acceptFocus;
    }
...
void XdgToplevelV6Window::initialize()
{
    ...
    if (m_oceanShellSurface && !m_isSendT && isResizable()) {
        m_oceanShellSurface->sendSplitable(true);
        m_isSendT = true;
    }
...
void XdgToplevelV6Window::configureDecoration()
{
    ...
    if (m_noTitleBar != -1) {
        disconnect(m_oceanShellSurface, &OCEANShellSurfaceInterface::noTitleBarPropertyRequested, this, nullptr);
        Q_EMIT m_oceanShellSurface->noTitleBarPropertyRequested(m_noTitleBar);
    }
    if (!m_windowRadius.isNull()) {
        // disconnect(m_oceanShellSurface, &KWaylandServer::OCEANShellSurfaceInterface::windowRadiusPropertyRequested, this, nullptr);
        Q_EMIT m_oceanShellSurface->windowRadiusPropertyRequested(m_windowRadius);
