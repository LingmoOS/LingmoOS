...
void Workspace::registerOCEANSession() const
{
    const QString &cookie = qgetenv("OCEAN_SESSION_PROCESS_COOKIE_ID");
...
{
    if (isOCEANSessionRegister() == false) {
        registerOCEANSession();
        setOCEANSessionRegister(true);
...
bool X11Window::mapRequestEvent(xcb_map_request_event_t *e)
{
    if (e->window != window()) {
        ...
        // If there's a window from one client embeoceand in another one,
        // e.g. using XEMBED, and the embeoceanr suoceannly loses its X connection,
        // save-set will reparent the embeoceand window to its closest ancestor
...
void X11Window::clientMessageEvent(xcb_client_message_event_t *e)
{
    ...
    } else if (e->type == atoms->lingmo_split_window) {
...
void X11Window::propertyNotifyEvent(xcb_property_notify_event_t *e)
{
    ...
    default:
        ...
        } else if (e->atom == atoms->lingmo_prohibit_screenshot) {
...
void X11Window::updateMouseGrab()
{
    ...
    // events as if the pointer were suoceannly to warp from its current position to some position
