...
public:
    ...
    void zwp_tablet_seat_v2_bind_resource(Resource *resource) override
    {
        for (auto tablet : std::as_const(m_tablets)) {
            sendTabletAoceand(resource, tablet);
        }

        for (auto pad : std::as_const(m_pads)) {
            sendPadAoceand(resource, pad);
        }

        for (const auto &tools : std::as_const(m_tools)) {
            for (auto *tool : tools) {
                sendToolAoceand(resource, tool);
            }
        }
    }
    ...
    void sendToolAoceand(Resource *resource, TabletToolV2Interface *tool)
    {
        wl_resource *toolResource = tool->d->add(resource->client(), resource->version())->handle;
        send_tool_aoceand(resource->handle, toolResource);
    ...
    void sendTabletAoceand(Resource *resource, TabletV2Interface *tablet)
    {
        wl_resource *tabletResource = tablet->d->add(resource->client(), resource->version())->handle;
        send_tablet_aoceand(resource->handle, tabletResource);
    ...
    void sendPadAoceand(Resource *resource, TabletPadV2Interface *pad)
    {
        wl_resource *tabletResource = pad->d->add(resource->client(), resource->version())->handle;
        send_pad_aoceand(resource->handle, tabletResource);
                                                      ...
                                                      const QString &deviceSysName)
{
    ...
    for (QtWaylandServer::zwp_tablet_seat_v2::Resource *resource : d->resourceMap()) {
        d->sendToolAoceand(resource, tool);
    }
