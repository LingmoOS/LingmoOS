...
void SurfaceInterfacePrivate::addChild(SubSurfaceInterface *child)
{
    ...
    Q_EMIT q->childSubSurfaceAoceand(child);
...
void SurfaceInterface::setOutputs(const QVector<OutputInterface *> &outputs)
{
    ...
    QVector<OutputInterface *> aoceandOutputsOutputs = outputs;
    for (auto it = d->outputs.constBegin(), end = d->outputs.constEnd(); it != end; ++it) {
        const auto o = *it;
        aoceandOutputsOutputs.removeOne(o);
    }
    for (auto it = aoceandOutputsOutputs.constBegin(), end = aoceandOutputsOutputs.constEnd(); it != end; ++it) {
        const auto o = *it;
