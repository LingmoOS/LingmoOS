...
#define BUF_SIZE 256
#define CLIPBOARD_NAME "ocean-clipboard-d"
...
void SeatInterfacePrivate::addSecurityInterface(OCEANSecurityInterface* new_security)
{
    if (new_security == oceanSecurity) {
        return;
    }
    oceanSecurity = new_security;
    QObject::connect(oceanSecurity, &OCEANSecurityInterface::copySecurityVerified, q,
...
bool SeatInterfacePrivate::verifySelection(AbstractDataDevice *dataDevice, AbstractDataSource *dataSource)
{
    ...
    int serial = oceanSecurity->doVerifySecurity(OCEANSecurityInterface::SecurityType::SEC_CLIPBOARD_COPY, dataSource->processId(), dataDevice->processId());
    if (serial > 0) {
...
uint32_t SeatInterfacePrivate::verifySelectionForX11(AbstractDataSource *dataSource, int target)
{
    ...
    int serial = oceanSecurity->doVerifySecurity(OCEANSecurityInterface::SecurityType::SEC_CLIPBOARD_COPY, dataSource->processId(), target);
    if (serial > 0) {
...
void SeatInterface::addSecurityInterface(OCEANSecurityInterface* security)
{
