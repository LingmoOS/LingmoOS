...
void TestWindowManagement::testParentWindow()
{
    ...
    auto transient = windowAoceandSpy.first().first().value<KWayland::Client::PlasmaWindow *>();
...
void TestWindowManagement::testGeometry()
{
    ...
    QVERIFY(pm != nullptr);
    QSignalSpy windowAoceandSpy(pm.get(), &KWayland::Client::PlasmaWindowManagement::windowCreated);
    QVERIFY(windowAoceandSpy.wait());
