...
public:
    ...
    bool wheelEvent(WheelEvent *event) override
    {
        ...
        auto seat = waylandServer()->seat();
        auto oceanSeat = waylandServer()->oceanSeat();
        if (pointerSurfaceAllowed()) {
            ...
            if (oceanSeat) {
                oceanSeat->pointerAxis(wheelEvent->orientation(),
...
public:
    ...
    bool wheelEvent(WheelEvent *event) override
    {
        ...
        if (oceanSeat) {
            oceanSeat->pointerAxis(_event->orientation(),
...
public:
    TabletInputFilter()
    {
        ...
        connect(input(), &InputRedirection::deviceAoceand, this, &TabletInputFilter::integrateDevice);
        connect(input(), &InputRedirection::deviceRemoved, this, &TabletInputFilter::removeDevice);

        auto tabletNextOutput = new QAction(this);
        tabletNextOutput->setProperty("componentName", QStringLiteral("lingmo-kwin"));
...
public:
    ...
    bool pointerEvent(MouseEvent *event, quint32 nativeButton) override
    {
        ...
        case QEvent::MouseMove: {
            ...
            if (oceanSeat) {
                oceanSeat->setPointerPos(event->globalPos());
            }
            seat->notifyPointerFrame();
        ...
        case QEvent::MouseButtonPress:
            seat->notifyPointerButton(nativeButton, KWaylandServer::PointerButtonState::Pressed);
            if (oceanSeat) {
                oceanSeat->pointerButtonPressed(nativeButton);
            }
            seat->notifyPointerFrame();
            break;
        case QEvent::MouseButtonRelease:
            ...
            seat->notifyPointerButton(nativeButton, KWaylandServer::PointerButtonState::Released);
            if (oceanSeat) {
                oceanSeat->pointerButtonReleased(nativeButton);
            }
            seat->notifyPointerFrame();
            break;
        default:
...
public:
    void touchDown(qint32 id, const QPointF &pos, std::chrono::microseconds time) override {
        ...
        oceanSeat->setTouchTimestamp(time.count());
        oceanSeat->touchDown(id, pos);
    }
    ...
    void pointerEvent(MouseEvent *event) override {
        ...
        case QEvent::MouseButtonPress: {
            oceanSeat->pointerButtonPressed(event->nativeButton());
            break;
        }
        case QEvent::MouseButtonRelease: {
            oceanSeat->pointerButtonReleased(event->nativeButton());
            break;
        }
        default:
            break;
    ...
    void keyEvent(KeyEvent *event) override {
        ...
        switch (event->type()) {
            case QEvent::KeyPress:
                oceanSeat->setTimestamp(event->timestamp().count());
                oceanSeat->keyPressed(event->nativeScanCode());
                break;
            case QEvent::KeyRelease:
                oceanSeat->setTimestamp(event->timestamp().count());
                oceanSeat->keyReleased(event->nativeScanCode());
                break;
            default:
                break;
        }
...
static const QString s_touchpadComponent = QStringLiteral("lingmo-kcm_touchpad");
...
{
    connect(inputBackend.get(), &InputBackend::deviceAoceand, this, &InputRedirection::addInputDevice);
...
Window *InputRedirection::findManagedToplevel(const QPointF &pos)
{
    ...
    do {
        ...
        if (!window->isOnCurrentActivity() || !window->isOnCurrentDesktop() || window->isMinimized() || window->isHioceannInternal()) {
            continue;
        }
