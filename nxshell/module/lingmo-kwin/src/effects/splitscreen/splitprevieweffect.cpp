...
void SplitPreviewEffect::initTextureMask()
{
    for (Output *output : workspace()->outputs()) {
        ...
        QDBusInterface wmInterface("com.lingmo.wm", "/com/lingmo/wm", "com.lingmo.wm");
...
void SplitPreviewEffect::setActive(bool active)
{
    ...
    if (active) {
        if (!m_effectFrame) {
            m_effectFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/splitscreen/qml/main.qml", false);
        }
