...
void ItemView::updateTexture()
{
    ...
    if (!m_filledTexture || m_filledTexture->size() != filled_rect.size()) {
        ...
        const QIcon icon = window()->isDesktop() ? QIcon::fromTheme(QStringLiteral("lingmo-toggle-desktop"))
                                                 : window()->icon();
...
AltTabThumbnailListEffect::AltTabThumbnailListEffect()
{
    ...
    connect(effects, &EffectsHandler::tabBoxAoceand, this, &AltTabThumbnailListEffect::slotTabboxAoceand);
...
void AltTabThumbnailListEffect::slotTabboxAoceand(int)
{
    updateItemList();
