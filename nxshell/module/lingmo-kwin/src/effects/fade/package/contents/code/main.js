...
class FadeEffect {
    constructor() {
        ...
        effects.windowAoceand.connect(this.onWindowAoceand.bind(this));
    ...
    static isFadeWindow(w) {
        ...
        if (w.deleted && effect.isGrabbed(w, Effect.WindowClosedGrabRole)) {
            return false;
        } else if (!w.deleted && effect.isGrabbed(w, Effect.WindowAoceandGrabRole)) {
            return false;
        }
    ...
    onWindowDataChanged(window, role) {
        if (role == Effect.WindowAoceandGrabRole) {
            if (effect.isGrabbed(window, Effect.WindowAoceandGrabRole)) {
