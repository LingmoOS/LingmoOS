...
GlideEffect::GlideEffect()
{
    ...
    connect(effects, &EffectsHandler::windowAoceand, this, &GlideEffect::windowAoceand);
...
void GlideEffect::windowAoceand(EffectWindow *w)
{
    ...
    const void *addGrab = w->data(WindowAoceandGrabRole).value<void *>();
    if (addGrab && addGrab != this) {
        return;
    }

    w->setData(WindowAoceandGrabRole, QVariant::fromValue(static_cast<void *>(this)));
...
{
    if (role != WindowAoceandGrabRole && role != WindowClosedGrabRole) {
        return;
    }
