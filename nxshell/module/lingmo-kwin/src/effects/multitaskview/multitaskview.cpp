...
Q_GLOBAL_STATIC_WITH_ARGS(QGSettings, _gsettings_ocean_dock, ("com.lingmo.ocean.dock"))
//Q_GLOBAL_STATIC_WITH_ARGS(QGSettings, _gsettings_ocean_dock_primary, ("com.lingmo.ocean.dock.mainwindow"))
...
const char notification_tips[] = "ocean-osd ocean-osd";
const char screen_recorder[] = "lingmo-screen-recorder lingmo-screen-recorder";
const char split_outline[] = "kwin_x11 kwin";
const char fallback_background_name[] = "file:///usr/share/wallpapers/lingmo/desktop.jpg";
    ...
    , m_bgmutex(QMutex::Recursive)
{
    ...
    if (lst.size() > 0) {
        m_lingmowmrcIni = new QSettings(lst[0] + "/lingmowmrc", QSettings::IniFormat);
    }
}
...
{
    if (m_lingmowmrcIni) {
        delete m_lingmowmrcIni;
        m_lingmowmrcIni = nullptr;
    }
}
...
void MultiViewBackgroundManager::cacheWorkspaceBg(BgInfo_st &st)
{
    ...
    if (m_lingmowmrcIni) {
        QString backgroundUri;
        QString strBackgroundPath = QString("%1@%2").arg(st.desktop).arg(st.screenName);
        backgroundUri = m_lingmowmrcIni->value("WorkspaceBackground/" + strBackgroundPath).toString();
...
{
    m_button = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/icon.qml", true);
}
...
MultiViewWorkspace::MultiViewWorkspace(bool flag)
    ...
    , m_desktop(0)
{
    m_backGroundFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/icon.qml", true);
    m_backGroundFrame->setAlignment(Qt::AlignLeft | Qt::AlignTop);

    m_workspaceBgFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/workspacebg.qml", true);
    m_workspaceBgFrame->setAlignment(Qt::AlignLeft | Qt::AlignTop);
    m_workspaceBgFrame->setRadius(10);
    m_hoverFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/hover.qml", true);
...
{
    m_fillFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/hoverbg.qml", false);
    ...
    , m_timerCheckWindowClose(new QTimer(this))
{
    ...
    s->setProperty("componentName", QStringLiteral("lingmo-kwin"));
    ...
    connect(effects, &EffectsHandler::windowAoceand, this, &MultitaskViewEffect::onWindowAoceand);
    ...
    connect(effects, &EffectsHandler::screenAoceand, this, [this] { onCloseEffect(true); });
    connect(effects, &EffectsHandler::screenRemoved, this, [this] { onCloseEffect(true); });

    connect(_gsettings_ocean_dock, &QGSettings::changed, this, &MultitaskViewEffect::onDockChange);
    ...
    QDBusConnection::sessionBus().connect(DBUS_LINGMO_WM_SERVICE, DBUS_LINGMO_WM_OBJ, DBUS_LINGMO_WM_INTF,
...
void MultitaskViewEffect::reconfigure(ReconfigureFlags flags)
{
    ...
    KConfigGroup config_group(KSharedConfig::openConfig("lingmo-kwinrc"), "Compositing");
...
void MultitaskViewEffect::onWindowAoceand(EffectWindow *w)
{
    if (!m_activated)
        return;
    ...
    if (w->isDock()) {
    ...
    } else if (!QX11Info::isPlatformX11() && w->caption() == "org.lingmo.ocean.lock") {
        setActive(false);
        ...
        if (m_isScreenRecorder) {
            ...
            effects->stopMouseInterception(this);
            m_isScreenRecorder = true;
        } else if (w->windowClass() != notification_tips && w->caption() != "lingmo-splitoutline") {
            setActive(false);
        }
    }
}
...
{
    QString position = _gsettings_ocean_dock->get(GsettingsDockPosition).toString();
    int height = _gsettings_ocean_dock->get(GsettingsDockHeight).toInt();
    if (position == "bottom") {
...
void MultitaskViewEffect::initWorkspaceBackground()
{
    ...
    for (; it != m_screenInfoList.end(); ++it) {
        ...
        {
            std::unique_ptr<EffectFrameEx> bgframe = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/hoverbg.qml", false);
            bgframe->setAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
            bgframe->setColor("#E0E0E0");
            bgframe->setRadius(5);
            std::unique_ptr<EffectFrameEx> frame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/text.qml", false);
...
{
    if (!m_hoverWinFrame) {
        m_hoverWinFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/hover.qml", false);
    }

    if (!m_closeWinFrame) {
        m_closeWinFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/icon.qml", true);
    ...
    if (!m_topWinFrame) {
        m_topWinFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/icon.qml", true);
    ...
    if (!m_textWinFrame) {
        m_textWinFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/text.qml", false);
    }

    if (!m_textWinBgFrame) {
        m_textWinBgFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/hoverbg.qml", false);
    ...
    if (!m_previewFrame) {
        m_previewFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/workspacebg.qml", true);
        m_previewFrame->setAlignment(Qt::AlignLeft | Qt::AlignTop);
        m_previewFrame->setRadius(10);
    }

    if (!m_closeWorkspaceFrame) {
        m_closeWorkspaceFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/icon.qml", true);
    ...
    if (!m_dragTipsFrame) {
        m_dragTipsFrame = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/dragtips.qml", false);
    ...
    if (!m_dragTipsFrameShadow) {
        m_dragTipsFrameShadow = effectsEx->effectFrameEx("lingmo-kwin/effects/multitaskview/qml/dragtips.qml", false);
