    ...
    , m_monitorWindow(nullptr)
{
    ...
    connect(effects, &EffectsHandler::windowAoceand, this, &HighlightWindowEffect::slotWindowAoceand);
...
void HighlightWindowEffect::slotWindowAoceand(EffectWindow *w)
{
    if (!m_highlightedWindows.isEmpty()) {
        // On X11, the tabbox may ask us to highlight itself before the windowAoceand signal
...
void HighlightWindowEffect::slotPropertyNotify(EffectWindow *w, long a, EffectWindow *aoceandWindow)
{
    ...
    if (byteData.length() < 1) {
        ...
        if (!aoceandWindow || w != aoceandWindow) {
            finishHighlighting();
        }
        return;
    }
