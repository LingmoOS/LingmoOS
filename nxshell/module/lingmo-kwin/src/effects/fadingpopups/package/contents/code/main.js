...
function isPopupWindow(window) {
    ...
    // animate almost every window that was shown or hioceann. Then it was
...
function isFadingWindow(window) {
    if (window.windowClass == "lingmo-screen-recorder lingmo-screen-recorder" || window.windowClass == "ocean-lock org.lingmo.ocean.lock")
        return false;
    if (window.tooltip)
        return false;
    return true;
...
var fadingPopupsEffect = {
    calcDiagonalPos: function (window, cpos, windowRect) {
        ...
        var height = windowRect.height;
        var padding = 5;
        if (window.windowClass == "ocean-launcher ocean-launcher" && !window.popupMenu) {
    ...
    slotWindowAoceand: function (window, cursorpos) {
    ...
    slotWindowClosed: function (window) {
        ...
        if (window.windowClass != "ocean-launcher ocean-launcher") {
    ...
    init: function () {
        ...
        effects.unmanagedAoceandAnimation.connect(fadingPopupsEffect.slotWindowAoceand);
