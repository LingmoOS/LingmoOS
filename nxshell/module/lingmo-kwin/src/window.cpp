    ...
    if (!isRequestedFullScreen() && isInteractiveMove()) {
        ...
        } else*/
        {
            ...
            if (!m_electricMaximizing) {
                ...
                // otherwise we must make sure it's hioceann
                workspace()->outline()->hide();
            }
        }
    }
    ...
    if (isInteractiveResize()) {
        ...
        if (!isMovable()) { // isMovableAcrossScreens() must have been true to get here
            ...
            if (!isUnrestrictedInteractiveMoveResize()) {
                ...
                // 2. OCEAN window (with titlebar exists, only height is 0)
                //
                // NOTE: apps may set a very small border (e.g ocean apps in non-composited mode),
                ...
                if (!transposed && bTitleRect.height() <= 10) {
                    ...
                    // this contains old ocean windows and lingmo-terminal which have
...
void Window::setupWindowManagementInterface()
{
    ...
    // We need to set `OnAllDesktops` after the actual VD list has been aoceand.
