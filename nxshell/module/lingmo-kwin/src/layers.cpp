...
void Workspace::propagateWindows(bool propagate_new_windows)
{
    ...
    // supportWindow > electric borders > windows > hioceann windows
    ...
    for (int i = stacking_order.size() - 1; i >= 0; --i) {
        ...
        if (!window || window->hioceannPreview()) {
            continue;
        }
    ...
    // when having hioceann previews, stack hioceann windows below everything else
