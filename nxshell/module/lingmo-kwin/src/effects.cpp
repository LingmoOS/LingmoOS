    ...
    , m_trackingCursorChanges(0)
{
    ...
    connect(ws, &Workspace::windowAoceand, this, [this](Window *window) {
    ...
    connect(ws, &Workspace::unmanagedAoceand, this, [this](Unmanaged *u) {
    ...
    connect(ws, &Workspace::internalWindowAoceand, this, [this](InternalWindow *window) {
        setupWindowConnections(window);
        Q_EMIT windowAoceand(window->effectWindow());
    });
    ...
    if (Activities *activities = Workspace::self()->activities()) {
        connect(activities, &Activities::aoceand, this, &EffectsHandler::activityAoceand);
    ...
    connect(tabBox, &TabBox::TabBox::tabBoxAoceand, this, &EffectsHandler::tabBoxAoceand);
...
{ // regardless, unmanaged windows are -yet?- not synced anyway
    ...
    setupUnmanagedConnections(u);
    Q_EMIT windowAoceand(u->effectWindow());
    Q_EMIT unmanagedAoceandAnimation(window->effectWindow(), cursorPos());
}
...
void EffectsHandlerImpl::slotTabAoceand(EffectWindow *w, EffectWindow *to)
{
    Q_EMIT tabAoceand(w, to);
}
...
{
    KConfigGroup kwinConfig(KSharedConfig::openConfig("lingmo-kwinrc"), "Plugins");
...
bool EffectsHandlerImpl::prohibitScreenshot(qulonglong winid) const
{
    ...
    return ocean_restrict->prohibitScreencast() && waylandServer()->hasProhibitWindows();
}
...
void EffectsHandlerImpl::slotOutputAoceand(Output *output)
{
    EffectScreen *screen = new EffectScreenImpl(output, this);
    m_effectScreens.append(screen);
    Q_EMIT screenAoceand(screen);
}
