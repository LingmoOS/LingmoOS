...
void GlobalShortcutsTest::testSetupWindowShortcut()
{
    ...
    QSignalSpy shortcutDialogAoceandSpy(workspace(), &Workspace::internalWindowAoceand);
    workspace()->slotSetupWindowShortcut();
    QTRY_COMPARE(shortcutDialogAoceandSpy.count(), 1);
    auto dialog = shortcutDialogAoceandSpy.first().first().value<InternalWindow *>();
    QVERIFY(dialog);
