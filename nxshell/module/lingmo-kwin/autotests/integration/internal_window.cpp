...
{
    QSignalSpy windowAoceandSpy(workspace(), &Workspace::internalWindowAoceand);
    HelperWindow win;
    ...
    QTRY_COMPARE(windowAoceandSpy.count(), 1);
    QVERIFY(!workspace()->activeWindow());
    InternalWindow *window = windowAoceandSpy.first().first().value<InternalWindow *>();
    QVERIFY(window);
...
void InternalWindowTest::testKeyboard()
{
    ...
    QTRY_COMPARE(windowAoceandSpy.count(), 1);
    auto internalWindow = windowAoceandSpy.first().first().value<InternalWindow *>();
...
void InternalWindowTest::testReentrantMoveResize()
{
    ...
    win.show();
    QTRY_COMPARE(windowAoceandSpy.count(), 1);
    auto window = windowAoceandSpy.first().first().value<InternalWindow *>();
    QVERIFY(window);
...
void InternalWindowTest::testDismissPopup()
{
    ...
    clientToplevel.show();
    QTRY_COMPARE(windowAoceandSpy.count(), 1);
    auto serverToplevel = windowAoceandSpy.last().first().value<InternalWindow *>();
    ...
    clientPopup.show();
    QTRY_COMPARE(windowAoceandSpy.count(), 2);
    auto serverPopup = windowAoceandSpy.last().first().value<InternalWindow *>();
    QVERIFY(serverPopup);
    ...
    QTRY_COMPARE(windowAoceandSpy.count(), 3);
    auto serverOtherToplevel = windowAoceandSpy.last().first().value<InternalWindow *>();
