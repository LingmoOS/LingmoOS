...
void TestXdgShellWindow::testMapUnmap()
{
    ...
    QSignalSpy windowAoceandSpy(workspace(), &Workspace::windowAoceand);
    ...
    QVERIFY(windowAoceandSpy.wait());
    QCOMPARE(windowAoceandSpy.count(), 1);
    Window *window = windowAoceandSpy.last().first().value<Window *>();
    QVERIFY(window);
    ...
    Test::render(surface.get(), QSize(100, 50), Qt::blue);
    QVERIFY(windowAoceandSpy.wait());
    QCOMPARE(windowAoceandSpy.count(), 2);
    window = windowAoceandSpy.last().first().value<Window *>();
    QVERIFY(window);
    QCOMPARE(window->readyForPainting(), true);
...
void TestXdgShellWindow::testUnresponsiveWindow()
{
    ...
    if (windowAoceandSpy.isEmpty()) {
        QVERIFY(windowAoceandSpy.wait());
    }
    ::kill(process->processId(), SIGUSR1); // send a signal to freeze the process

    killWindow = windowAoceandSpy.first().first().value<Window *>();
    QVERIFY(killWindow);
...
void TestXdgShellWindow::testXdgWindowGeometryAttachSubSurface()
{
    ...
    // when a new sub-surface is aoceand and xdg_surface.set_window_geometry is
    // not called again.
