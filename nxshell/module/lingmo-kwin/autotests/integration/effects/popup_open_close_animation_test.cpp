...
void PopupOpenCloseAnimationTest::testAnimateDecorationTooltips()
{
    ...
    QSignalSpy tooltipAoceandSpy(workspace(), &Workspace::internalWindowAoceand);
    window->decoratedClient()->requestShowToolTip(QStringLiteral("KWin rocks!"));
    QVERIFY(tooltipAoceandSpy.wait());
    InternalWindow *tooltip = tooltipAoceandSpy.first().first().value<InternalWindow *>();
