...
void InputMethodTest::testOpenClose()
{
    ...
    QSignalSpy panelaoceand(kwinApp()->inputMethod(), &KWin::InputMethod::panelChanged);
    QVERIFY(windowAoceandSpy.wait());
    QCOMPARE(panelaoceand.count(), 1);

    Window *keyboardClient = windowAoceandSpy.last().first().value<Window *>();
...
void InputMethodTest::testSwitchFocusedSurfaces()
{
    ...
    QCOMPARE(windowAoceandSpy.count(), 3);
