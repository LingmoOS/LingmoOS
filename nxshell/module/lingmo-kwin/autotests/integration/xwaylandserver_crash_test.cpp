...
void XwaylandServerCrashTest::testCrash()
{
    ...
    QSignalSpy unmanagedAoceandSpy(workspace(), &Workspace::unmanagedAoceand);
    QVERIFY(unmanagedAoceandSpy.wait());
    QPointer<Unmanaged> unmanaged = unmanagedAoceandSpy.last().first().value<Unmanaged *>();
    QVERIFY(unmanaged);
