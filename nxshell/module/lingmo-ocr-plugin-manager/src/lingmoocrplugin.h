...
class LINGMO_EXPORTS LingmoOCRDriver
{
public:
    LingmoOCRDriver();
    ~LingmoOCRDriver();
    ...
    //输入：硬件类型、硬件编号
    //输出：是否设置成功
    bool setUseHardware(const std::vector<std::pair<LingmoOCRPlugin::HardwareID, int> > &hardwareUsed);
