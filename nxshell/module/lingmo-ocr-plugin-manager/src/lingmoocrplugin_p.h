...
//插件加载接口，用于加载插件的基础类
//输入：无
//输出：插件基础类指针 LingmoOCRPlugin::Plugin * ，使用 static_cast 即可转换到 void * 类型
...
//建议直接返回LingmoOCRPlugin::VERSION，这个变量位于lingmoocrplugindef.h中
//输入：无
//输出：插件的版本号
...
public:
    ...
    //输入：硬件类型、硬件编号
    //输出：是否设置成功
    virtual bool setUseHardware(const std::vector<std::pair<LingmoOCRPlugin::HardwareID, int> > &hardwareUsed);
