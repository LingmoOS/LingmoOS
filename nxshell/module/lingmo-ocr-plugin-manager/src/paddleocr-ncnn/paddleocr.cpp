...
{
    return LingmoOCRPlugin::VERSION;
}
...
PaddleOCRApp::PaddleOCRApp()
{
    ...
    do {
        ...
        while(token != nullptr) {
            ...
            fullPath = pathPrefix + "lingmo-ocr-plugin-manager/model/";
    ...
    if(currentPath.empty()) {
        fullPath = "/usr/share/lingmo-ocr-plugin-manager/model/";
        if(std::filesystem::exists(fullPath)) {
            currentPath = fullPath;
        } else {
            LINGMO_LOG("cannot find default model");
        }
    }
}
...
void PaddleOCRApp::initNet()
{
    ...
    if (gpuCount != 0) {
        for (auto &eachPair : hardwareUseInfos) {
            if (eachPair.first == LingmoOCRPlugin::HardwareID::GPU_Vulkan) {
...
std::vector<LingmoOCRPlugin::TextBox> PaddleOCRApp::lengthToBox(const std::vector<int> &lengths, std::pair<float, float> basePoint, float rectHeight, float ratio)
{
    std::vector<LingmoOCRPlugin::TextBox> result;
...
bool PaddleOCRApp::setUseHardware(const std::vector<std::pair<LingmoOCRPlugin::HardwareID, int>> &hardwareUsed)
{
    needReset = true;
...
std::vector<LingmoOCRPlugin::HardwareID> PaddleOCRApp::getHardwareSupportList()
{
...
LingmoOCRPlugin::PixelType PaddleOCRApp::getPixelType()
{
    return LingmoOCRPlugin::PixelType::Pixel_BGR;
}
...
std::vector<LingmoOCRPlugin::TextBox> PaddleOCRApp::getTextBoxes()
{
    return textBoxes;
}

std::vector<LingmoOCRPlugin::TextBox> PaddleOCRApp::getCharBoxes(size_t index)
{
    if (index >= charBoxes.size()) {
        return std::vector<LingmoOCRPlugin::TextBox>();
    } else {
