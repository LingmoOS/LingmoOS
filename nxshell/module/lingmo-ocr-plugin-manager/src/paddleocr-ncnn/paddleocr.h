...
class PaddleOCRApp : public LingmoOCRPlugin::Plugin
{
public:
    PaddleOCRApp();
    ~PaddleOCRApp() override;

    bool setUseHardware(const std::vector<std::pair<LingmoOCRPlugin::HardwareID, int> > &hardwareUsed) override;
    std::vector<LingmoOCRPlugin::HardwareID> getHardwareSupportList() override;
    ...
    LingmoOCRPlugin::PixelType getPixelType() override;
    ...
    std::vector<LingmoOCRPlugin::TextBox> getTextBoxes() override;
    std::vector<LingmoOCRPlugin::TextBox> getCharBoxes(size_t index) override;
...
private:
    ...
    std::vector<LingmoOCRPlugin::TextBox> lengthToBox(const std::vector<int> &lengths, std::pair<float, float> basePoint, float rectHeight, float ratio);

    //推理设置缓存
    cv::Mat imageCache;
    std::vector<std::string> supportLanguages = {"zh-Hans_en", "zh-Hant_en", "en"};
    std::vector<LingmoOCRPlugin::HardwareID> supportHardwares = {LingmoOCRPlugin::HardwareID::CPU_Any,
                                                                 LingmoOCRPlugin::HardwareID::GPU_Vulkan};
    std::vector<std::pair<LingmoOCRPlugin::HardwareID, int>> hardwareUseInfos;
    std::string languageUsed = "zh-Hans_en";
    unsigned int maxThreadsUsed = 1;

    //推理结果缓存
    std::vector<LingmoOCRPlugin::TextBox> textBoxes;
    std::vector<std::vector<LingmoOCRPlugin::TextBox>> charBoxes;
