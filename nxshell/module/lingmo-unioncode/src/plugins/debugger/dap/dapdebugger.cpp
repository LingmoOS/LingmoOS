    ...
    : AbstractDebugger(parent), d(new DebuggerPrivate())
{
    ...
    sessionBus.connect(QString(""),
                       "/path",
                       "com.lingmo.unioncode.interface",
                       "dapport",
...
void DAPDebugger::registerDapHandlers()
{
    ...
    // The event indicates that some source has been aoceand, changed,
...
void DAPDebugger::handleEvents(const dpf::Event &event)
{
    ...
    } else if (event.data() == editor.breakpointAoceand.name) {
        QString filePath = event.property(editor.breakpointAoceand.pKeys[0]).toString();
        int line = event.property(editor.breakpointAoceand.pKeys[1]).toInt() + 1;
