...
{
    QList<baseLocatorItem> alreadyAoceand {};
    foreach (auto locator, validLocator) {
        const QList<baseLocatorItem> matchResult = locator->matchesFor(searchText);
        alreadyAoceand.reserve(matchResult.size());
        foreach (const baseLocatorItem &item, matchResult) {
            if (!alreadyAoceand.contains(item))
                alreadyAoceand.append(item);
        }
    }
    model->clear();
    model->addItems(alreadyAoceand);
    return alreadyAoceand;
}
