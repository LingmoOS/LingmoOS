...
bool MessageComponent::createCodeEdit(const MessageData &newData)
{
    ...
    QStringList aoceandLines = newLines.mid(oldLines.count());

    for (int i = 0; i < aoceandLines.count(); ++i) {
        QString aoceandLine = aoceandLines.at(i).trimmed();
        if (aoceandLine.contains("`") || aoceandLine.isEmpty()) {
            if (i != 0) {
                MessageData aoceandMsgData = messageData;
                aoceandMsgData.appendData(aoceandLines.mid(0, i));
                updateMessage(aoceandMsgData);
            }

            QRegExp re("```([a-z]*|[A-Z]*)");
            if (re.exactMatch(aoceandLine) && currentUpdateState == Label) {
                // create new code edit component
                messageData.appendData({ aoceandLine });
                ...
                return true;
            } else if (aoceandLine == "```" && currentUpdateState == CodeEdit) {
                ...
                curUpdateLabel->setWordWrap(true);
                msgLayout->addWidget(curUpdateLabel);
                if (i != (aoceandLines.count() - 1))
                    updateMessage(newData);

                return false;
            } else if (aoceandLine.size() > 4) {  // aoceandline starts with ` but not ```. eg: `123`
                return true;
            }
            return false;
        }
