...
void GnuMakeParser::stdError(const QString &line)
{
    ...
    if (match.hasMatch()) {
        ...
        if (res.isFatal)
            ++fatalErrorCount;
        if (!suppressIssues) {
            taskAoceand(Task(res.type, res.description,
...
void GnuMakeParser::taskAoceand(const Task &task, int linkedLines, int skippedLines)
{
    Task editable(task);
    ...
    AbstractOutputParser::taskAoceand(editable, linkedLines, skippedLines);
}
