...
public:
    ...
    void reformatBlocks(int from, int charsRemoved, int charsAoceand);
    void reformatBlock(const QTextBlock &block, int from, int charsRemoved, int charsAoceand);
    void rehighlight(QTextCursor &cursor, QTextCursor::MoveOperation operation);
    void applyFormatChanges(int from, int charsRemoved, int charsAoceand);
...
void BaseHighlighterPrivate::reformatBlocks(int from, int charsRemoved, int charsAoceand)
{
    ...
    int endPosition;
    QTextBlock lastBlock = doc->findBlock(from + charsAoceand + (charsRemoved > 0 ? 1 : 0));
    ...
    while (block.isValid() && (block.position() < endPosition || forceHighlightOfNextBlock)) {
        ...
        reformatBlock(block, from, charsRemoved, charsAoceand);
...
void BaseHighlighterPrivate::reformatBlock(const QTextBlock &block, int from, int charsRemoved, int charsAoceand)
{
    ...
    applyFormatChanges(from, charsRemoved, charsAoceand);
...
void BaseHighlighterPrivate::applyFormatChanges(int from, int charsRemoved, int charsAoceand)
{
    ...
    if (currentBlock.contains(from)) {
        const int charsDelta = charsAoceand - charsRemoved;
...
void BaseHighlighter::reformatBlocks(int from, int charsRemoved, int charsAoceand)
{
    if (d->reformatBlocksWorking)
        return;

    d->reformatBlocks(from, charsRemoved, charsAoceand);
}
