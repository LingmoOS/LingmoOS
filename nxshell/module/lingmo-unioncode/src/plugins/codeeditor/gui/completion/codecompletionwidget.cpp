...
void CodeCompletionWidget::initConnection()
{
    ...
    connect(editor(), &TextEditor::textAoceand, this, &CodeCompletionWidget::onTextAoceand);
...
void CodeCompletionWidget::onTextAoceand(int pos, int len, int aoceand, const QString &text, int line)
{
    Q_UNUSED(line)
    ...
    if (!editor()->isAutomaticInvocationEnabled() || aoceand != 0) {
