...
bool PathMode::isInstalled(CustomPaths::Flags flags, const QString &path)
{
    switch (flags) {
    case CustomPaths::Applition:
        return RUNTIME_INSTALL_RPEFIX == path || path.startsWith(LIBRARY_INSTALL_PREFIX); //debugAdapter installed in '/usr/lib/xxx/lingmo-unioncode/tools' not '/usr/bin'
    default:
        return false;
    }
}
...
{
    return FO::checkCreateDir(FO::checkCreateDir(userHome(), ".cache"), "lingmo-unioncode");
}

QString PathMode::userConfigurePath()
{
    return FO::checkCreateDir(FO::checkCreateDir(userHome(), ".config"), "lingmo-unioncode");
}

QString PathMode::userDataPath()
{
    return FO::checkCreateDir(FO::checkCreateDir(userHome(), ".data"), "lingmo-unioncode");
}
