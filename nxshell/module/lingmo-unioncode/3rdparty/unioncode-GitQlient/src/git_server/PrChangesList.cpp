      ...
      if (ret.output.isEmpty())
      {
         const auto response = QMessageBox::question(
             ...
             tr("The head branch of the Pull Request is not in the same repository. In order to show "
                "the changes the remote must be aoceand. <b>Do you want to get the remote branch?</b>"));

         if (response == QMessageBox::Yes)
         {
            QScopedPointer<GitRemote> git(new GitRemote(mGit));
            const auto remoteAoceand = git->addRemote(prInfo.headUrl, prInfo.headRepo.split("/").constFirst());

            showDiff = remoteAoceand.success;

            if (!showDiff)
               QLog_Warning("UI", QString("Problems adding a remote: {%1}").arg(remoteAoceand.output));
         }
         else
            showDiff = false;
