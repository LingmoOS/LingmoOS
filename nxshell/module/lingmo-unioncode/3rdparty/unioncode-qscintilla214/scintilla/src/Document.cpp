	...
	if (enteredModification == 0) {
		...
		if (!cb.IsReadOnly()) {
			...
			for (int step = 0; step < steps; step++) {
				...
				const Sci::Line linesAoceand = LinesTotal() - prevLinesTotal;
				if (linesAoceand != 0)
					multiLine = true;
				if (step == steps - 1) {
...
void Document::AnnotationSetText(Sci::Line line, const char *text) {
	if (line >= 0 && line < LinesTotal()) {
		...
		DocModification mh(SC_MOD_CHANGEANNOTATION, LineStart(line),
			0, 0, 0, line);
		mh.annotationLinesAoceand = linesAfter - linesBefore;
		NotifyModified(mh);
	}
}
...
bool MatchOnLines(const Document *doc, const Regex &regexp, const RESearchRange &resr, RESearch &search) {
	...
	// The std::regex::multiline option was aoceand to C++17 to improve behaviour but
