...
public:
	...
	Sci::Position length;
	Sci::Line linesAoceand;	/**< Negative if lines deleted. */
	const char *text;	/**< Only valid for changes to text, not for changes to style. */
	Sci::Line line;
	int foldLevelNow;
	int foldLevelPrev;
	Sci::Line annotationLinesAoceand;
	Sci::Position token;

	DocModification(int modificationType_, Sci::Position position_=0, Sci::Position length_=0,
		Sci::Line linesAoceand_=0, const char *text_=nullptr, Sci::Line line_=0) noexcept :
		modificationType(modificationType_),
		position(position_),
		length(length_),
		linesAoceand(linesAoceand_),
		text(text_),
		line(line_),
		foldLevelNow(0),
		foldLevelPrev(0),
		annotationLinesAoceand(0),
		token(0) {}

	DocModification(int modificationType_, const Action &act, Sci::Line linesAoceand_=0) noexcept :
		modificationType(modificationType_),
 ...
 * and implement the notification methods. It can then be aoceand to the watcher list with AddWatcher.
 */
class DocWatcher {
public:
	virtual ~DocWatcher() {}
