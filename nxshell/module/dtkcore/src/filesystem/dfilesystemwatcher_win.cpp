...
/*!
    ...
    paths can be aoceand using the addPaths() function. Existing paths can
    be removed by using the removePath() and removePaths() functions.

    DFileSystemWatcher examines each path aoceand to it. Files that have
    been aoceand to the DFileSystemWatcher can be accessed using the
...
/*!
    ...
    path is not aoceand if it does not exist, or if it is already being
...
/*!
    ...
    not aoceand if they not exist, or if they are already being
    monitored by the file system watcher.
    ...
    be monitored, and they will be aoceand to the returned QStringList.
...
/*!
    ...
    is modified (e.g., when a file is aoceand or deleted) or removed
