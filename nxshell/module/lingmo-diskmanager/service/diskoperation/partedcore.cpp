...
bool PartedCore::delTempMountFile()
{
    ...
    for (int i = 0; i < mountPath.size(); i++) {
        getxattr(mountPath[i].toStdString().c_str(), "user.lingmo-diskmanager", value, 1024);
        if (QString(value) == "lingmo-diskmanager") {
            d.rmdir(mountPath[i]);
        }
    }
    return true;
...
int PartedCore::getPartitionHiddenFlag()
{
    ...
    if (!file.open(QIODevice::ReadOnly)) { //打开指定文件
        ...
        m_hioceannPartition = file.readAll();
...
//            if(m_hioceannPartition.indexOf(partinfo.m_uuid) != -1 && !partinfo.m_uuid.isEmpty()) {
//                partinfo.m_flag = 1;
//            } else {
//                partinfo.m_flag = 0;
//            }
...
bool PartedCore::createTmpMountDir(const  QString &mountPath)
{
    ...
    //设置文件属性，删除时，按照文件属性删除
    const char *v = "lingmo-diskmanager";
    return setxattr(mountPath.toStdString().c_str(), "user.lingmo-diskmanager", v, strlen(v), 0) == 0;
}
