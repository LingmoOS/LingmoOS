...
void PartitionWidget::onAddPartition()
{
    ...
    if (m_sizeInfo.size() >= 24 || maxAmountPrimReached() == true || (partitionCount + m_sizeInfo.size()) >= device.m_maxPrims) {
        DMessageManager::instance()->sendMessage(this, QIcon(":/icons/lingmo/builtin/warning.svg"), tr("The number of new partitions exceeds the limit"));
        return;
    }
    ...
    if (formate.contains("AES") || formate.contains("SM4")) {
        ...
        if (currentSize <= 52) {
            DMessageManager::instance()->sendMessage(this, QIcon(":/icons/lingmo/builtin/warning.svg"), tr("To create a partition, you need at least 52 MB"));
            return;
        }
    }
