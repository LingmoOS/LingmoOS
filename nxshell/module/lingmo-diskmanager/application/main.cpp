...
int main(int argc, char *argv[])
{
    ...
    const QString acknowledgementLink = "https://www.lingmo.org/acknowledgments/lingmo_reader";
...
//    }
    if (a.setSingleInstance(appName)) {
        ...
        cmd = QString("pidof lingmo-diskmanager-service");

        if (!executCmd(cmd, oldPid, error)) {
            proc.startDetached("/usr/bin/dbus-send --system --type=method_call --dest=com.lingmo.diskmanager /com/lingmo/diskmanager com.lingmo.diskmanager.Quit");
        }

        QStringList argList;
        argList << QString::number(QCoreApplication::applicationPid()) << QDBusConnection::systemBus().baseService();
        proc.startDetached("/usr/bin/lingmo-diskmanager-authenticateProxy", argList);
