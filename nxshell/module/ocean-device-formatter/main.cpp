...
int main(int argc, char *argv[])
{
    ...
    bool isSingletonApp = SingletonApp::instance()->setSingletonApplication("ocean-device-formatter");
    if(!isSingletonApp)
        return 0;
    ...
    translator->load("/usr/share/ocean-device-formatter/translations/ocean-device-formatter_"
                     +QLocale::system().name()+".qm");
    a.installTranslator(translator);

    a.setOrganizationName("lingmo");
    a.setApplicationName(QObject::tr("ocean device formatter"));
    a.setApplicationVersion("1.0");
    a.setWindowIcon(QIcon::fromTheme("ocean-file-manager", QIcon::fromTheme("system-file-manager")));
