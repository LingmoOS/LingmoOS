...
var logger = log.NewLogger("lingmo-system-upgrade/version")

// DBus path and DBus interface
const (
	dbusPath      = "/org/lingmo/SystemUpgrade1/VersionManager"
	dbusInterface = "org.lingmo.SystemUpgrade1.VersionManager"
)

// Configuration files carried by the upgrade tool
const (
	UpgradeConfig = "/etc/lingmo-system-upgrade/upgrade.yaml"
	BackupConfig  = "/etc/lingmo-system-upgrade/backup.yaml"
)
...
func (v *VersionManager) BackupSystem(sender dbus.Sender) *dbus.Error {
	...
	err = dbustools.DBusMethodCaller("org.lingmo.AtomicUpgrade1", "SetDefaultConfig", "/org/lingmo/AtomicUpgrade1", BackupConfig)
	if err != nil {
		return dbusutil.ToError(err)
	}
	...
	err = dbustools.DBusMethodCaller("org.lingmo.AtomicUpgrade1", "Commit", "/org/lingmo/AtomicUpgrade1", string(commitInfoStr))
	if err != nil {
		return dbusutil.ToError(err)
	}
	for {
...
func (v *VersionManager) SetPlymouthTheme(theme string) *dbus.Error {
	themeCacheFile := filepath.Join("/var/cache/lingmo-system-upgrade", "plymouth-default-theme")
...
func commitNewSystemDir(root, repo string) error {
	...
	out, err := exec.Command("/usr/sbin/lingmo-upgrade-manager", "--action=setdefaultconfig", "--data="+UpgradeConfig).CombinedOutput()
	if err != nil {
	...
	out, err = exec.Command("/usr/sbin/lingmo-upgrade-manager", "--action=init", "--subject="+string(commitInfoStr)).CombinedOutput()
	if err != nil {
...
func monitorStateChange(replyCh chan StateChangeReply) error {
	...
	err = systemBus.BusObject().AddMatchSignal("org.lingmo.AtomicUpgrade1", "StateChanged",
		dbus.WithMatchObjectPath("/org/lingmo/AtomicUpgrade1")).Err

	signalCh := make(chan *dbus.Signal, 100)
	systemBus.Signal(signalCh)
	go func() {
		defer func() {
			err = systemBus.BusObject().RemoveMatchSignal("org.lingmo.AtomicUpgrade1", "StateChanged",
				dbus.WithMatchObjectPath("/org/lingmo/AtomicUpgrade1")).Err
		...
		for {
			select {
			case sig := <-signalCh:
				if sig.Path == "/org/lingmo/AtomicUpgrade1" &&
					sig.Name == "org.lingmo.AtomicUpgrade1.StateChanged" {
...
func getActiveVersion() (string, error) {
	version, err := dbustools.GetProperty("org.lingmo.AtomicUpgrade1", "org.lingmo.AtomicUpgrade1", "ActiveVersion", "/org/lingmo/AtomicUpgrade1")
	if err != nil {
...
func (v *VersionManager) setUpgradeVersion() error {
	...
	err = dbustools.DBusMethodCaller("org.lingmo.AtomicUpgrade1", "Rollback", "/org/lingmo/AtomicUpgrade1", v.activeVersion)
	if err != nil {
		return err
	}
