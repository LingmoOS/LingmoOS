...
import (
	...
	"lingmo-system-upgrade-daemon/pkg/module/systeminfo"
	"lingmo-system-upgrade-daemon/pkg/module/user"
)

var logger = log.NewLogger("lingmo-system-upgrade/checker")

// DBus path and DBus interface
const (
	dbusPath      = "/org/lingmo/SystemUpgrade1/Checker"
	dbusInterface = "org.lingmo.SystemUpgrade1.Checker"
)
...
func (c *CheckerManager) Perform(sender dbus.Sender, phase uint64) *dbus.Error {
	...
	if phase == PreUpgrade {
		...
		var state string
		reply, err := dbustools.GetProperty("com.lingmo.license", "com.lingmo.license.Info", "AuthorizationState", "/com/lingmo/license/Info")
		if err != nil {
