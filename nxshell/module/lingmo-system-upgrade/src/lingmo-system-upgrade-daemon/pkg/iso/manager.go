...
import (
	...
	"lingmo-system-upgrade-daemon/pkg/module/atomic"
	"lingmo-system-upgrade-daemon/pkg/module/config"
	"lingmo-system-upgrade-daemon/pkg/module/dbustools"
	"lingmo-system-upgrade-daemon/pkg/module/disk"
)

// DBus path and DBus interface
const (
	dbusPath      = "/org/lingmo/SystemUpgrade1/ISOManager"
	dbusInterface = "org.lingmo.SystemUpgrade1.ISOManager"
)

// Dependent configuration file path
const (
	AtomicRepoCfgLocation   = "/persistent/osroot/config/config.json"
	AtomicConfigLocation    = "/etc/lingmo-upgrade-manager/config.json"
	UpgradeConfigPath       = "/etc/lingmo-system-upgrade"
	UpgradeToolBinaryPath   = "/usr/bin/lingmo-system-upgrade-tool"
	UpgradeToolIconPath     = "/usr/share/icons/hicolor/scalable/apps/upgrade-tool.svg"
	PlymouthThemePath       = "/usr/share/plymouth/themes/lingmo-upgrade"
	SystemdDBusConf         = "/usr/share/dbus-1/system.d/org.lingmo.SystemUpgrade1.conf"
	SystemdDBusService      = "/usr/share/dbus-1/system-services/org.lingmo.SystemUpgrade1.service"
	UpgradeDaemonBinaryPath = "/usr/sbin/lingmo-system-upgrade-daemon"
	TranslationDirPath      = "/usr/share/lingmo-system-upgrade-tool/translations"
	UpgradeDesktopFilePath  = "/usr/share/applications/lingmo-system-upgrade-tool.desktop"
	UpgradeAutoStartFile    = "/etc/xdg/autostart/lingmo-system-upgrade-tool.desktop"
	MigrateFlagsPath        = "/var/cache/lingmo-system-upgrade/migrate.state"
...
var logger = log.NewLogger("lingmo-system-upgrade/iso")
...
func setPlymouthTheme() error {
	path := "/var/cache/lingmo-system-upgrade"
	...
	out, err := exec.Command("/usr/sbin/plymouth-set-default-theme", "-R", "lingmo-upgrade").CombinedOutput()
	if err != nil {
...
func removeLingmoInstaller() error {
	out, err := exec.Command("/usr/bin/apt", "-y", "purge", "lingmo-installer").CombinedOutput()
	if err != nil {
		logger.Warning("failed to purge lingmo-installer:", string(out))
		return err
	}
	return nil
}
...
func PrepareNewRootForCommit(homeDir, isoPath string) (string, string, error) {
	...
	// remove lingmo-installer
	err = removeLingmoInstaller()
	if err != nil {
		return "", "", err
	}
...
func HandleAfterCommit(root, repo string) error {
	...
	out, err := exec.Command("/usr/bin/sh", "-c", "pgrep -f -l lingmo-upgrade-manager | awk -F ' ' '{print $1}'").Output()
	if err == nil {
		if len(out) != 0 {
	...
	return dbustools.DBusMethodCaller("org.lingmo.AtomicUpgrade1", "SetRepoMount", "/org/lingmo/AtomicUpgrade1", repo)
}
