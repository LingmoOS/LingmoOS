...
void ConditionChecklistWidget::initUI()
{
    ...
    if (DBusWorker::getInstance()->GetDistroID() == QString("Lingmo"))
    {
        m_versionResultWidget->setRequirement(tr("Requirement: lingmo (%1)").arg(kLingmoMinimalVersion.toString()));
    }
    else
    {
...
void ConditionChecklistWidget::updateUI()
{
    ...
    if (DBusWorker::getInstance()->GetDistroID() == QString("Lingmo"))
    {
        requiredVersionNumber = kLingmoMinimalVersion;
    }
    else
    {
