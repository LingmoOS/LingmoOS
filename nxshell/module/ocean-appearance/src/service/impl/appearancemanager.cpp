...
bool AppearanceManager::init()
{
    ...
    new ThemeFontSyncConfig("org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1/sync", QSharedPointer<AppearanceManager>(this));
    new BackgroundSyncConfig("org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1/Background", QSharedPointer<AppearanceManager>(this));
...
void AppearanceManager::handleSetScaleFactorStarted()
{
    ...
    m_dbusProxy->Notify("ocean-control-center", "dialog-window-scale", summary, body, {}, {}, 0);
}
...
void AppearanceManager::handleSetScaleFactorDone()
{
    ...
    optionMap["x-lingmo-action-_logout"] = "dbus-send,--type=method_call,--dest=org.lingmo.ocean.SessionManager1,"
                                           "/org/lingmo/ocean/SessionManager1,org.lingmo.ocean.SessionManager1.RequestLogout";
    optionMap["x-lingmo-action-_later"] = "";
    int expireTimeout = 15 * 1000;
    m_dbusProxy->Notify("ocean-control-center", "dialog-window-scale", summary, body, options, optionMap, expireTimeout);
    // 更新ScaleFactor缓存
    getScaleFactor();
}
...
void AppearanceManager::doUpdateWallpaperURIs()
{
    ...
    if (!monitorWallpaperUris.first().startsWith("/usr/share/wallpapers/lingmo")) {
...
void AppearanceManager::loadWSConfig()
{
    ...
    QString fileName = utils::GetUserConfigDir() + "/lingmo/ocean-daemon/appearance/wallpaper-slideshow.json";
...
bool AppearanceManager::doSetGlobalTheme(QString value)
{
    ...
    QString defaultTheme = theme.getStr("Lingmo Theme", "DefaultTheme");
    ...
    QString darkTheme = theme.getStr("Lingmo Theme", "DarkTheme");
...
bool AppearanceManager::doSetGtkTheme(QString value)
{
    ...
    QString oceanKWinTheme;
    if (value == LINGMO) {
        oceanKWinTheme = "light";
    } else if (value == LINGMODARK) {
        oceanKWinTheme = "dark";
    }

    if (!oceanKWinTheme.isEmpty()) {
        m_dbusProxy->SetDecorationLingmoTheme(oceanKWinTheme);
    }
    setGtkTheme(value);
...
QString AppearanceManager::doList(QString type)
{
    if (type == TYPEGTK) {
        ...
        while (iter != gtks.end()) {
            if ((*iter)->getId().startsWith("lingmo")) {
                ++iter;
            } else {
...
QString AppearanceManager::doThumbnail(const QString &type, const QString &name)
{
    if (type == TYPEGTK) {
        ...
        if (gtkThumbnailMap.count(name) == 1) {
            return "/usr/share/ocean-daemon/appearance/" + gtkThumbnailMap[name] + ".svg";
        }
...
bool AppearanceManager::isSkipSetWallpaper(const QString &themePath)
{
    ...
    for (const auto &name : themeNames) {
        QString themeName = theme.getStr("Lingmo Theme", name);
        if (themeName.isEmpty()) {
            continue;
        }

        QString themeValue = theme.getStr(themeName, "Wallpaper");
        if (!themeValue.startsWith("/usr/share/wallpapers/lingmo")) {
            return true;
        }
    }
...
QString AppearanceManager::getWallpaperUri(const QString &index, const QString &monitorName)
{
    ...
    if (wallpaper.isEmpty()) {
        ...
        if (!bglist.isEmpty()) {
            ...
            wallpaper = "file:///usr/share/wallpapers/lingmo/desktop.jpg";
        }
...
bool AppearanceManager::changeBgAfterLogin(QString monitorSpace)
{
    ...
    runDir = runDir + "/ocean-daemon-wallpaper-slideshow-login" + "/" + monitorSpace;
...
bool AppearanceManager::setDQtTheme(QStringList key, QStringList value)
{
    ...
    QString filePath = utils::GetUserConfigDir() + "/lingmo" + "/qt-theme.ini";
