    ...
    , timer(new QTimer(this))
{
    ...
    gtkThumbnailMap["lingmo"] ="light";
    gtkThumbnailMap["lingmo-dark"] ="dark";
    gtkThumbnailMap["lingmo-auto"] ="auto";
}
...
void Subthemes::refreshGlobalThemes()
{
    ...
    for (auto &&info : infos) {
        KeyFile keyFile(',');
        keyFile.loadFile(info->getPath()+"/index.theme");
        info->setName(keyFile.getLocaleStr("Lingmo Theme","Name"));
        info->setComment(keyFile.getLocaleStr("Lingmo Theme","Comment"));
        ...
        info->setHasDark(keyFile.containKey("Lingmo Theme","DarkTheme"));
