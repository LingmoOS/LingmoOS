...
static QVector<QStringList> presentIcons {
    // file manager:
    {"ocean-file-manager", "system-file-manager"},
    // music player:
    {"lingmo-music", "banshee", "amarok", "deadbeef", "clementine", "rhythmbox"},
    // image viewer:
    {"lingmo-image-viewer", "eog", "gthumb", "gwenview", "gpicview", "showfoto", "phototonic"},
    // media/video player:
    {"lingmo-movie", "media-player", "totem", "smplayer", "vlc", "dragonplayer", "kmplayer"},
    // web browser:
...
//    {"accessories-text-editor", "text-editor", "gedit", "kedit", "xfce-edit"},
    // terminal:
//    {"lingmo-terminal", "utilities-terminal", "terminal", "gnome-terminal", "xfce-terminal", "terminator", "openterm"},
};
...
void removeUnusedScaleDirs()
{
    ...
    cacheDir+="/lingmo/ocean-api/theme_thumb";
...
bool genGtk(QString name,int width,int height,double scaleFactor,QString dest)
{
    ...
    if (scaleFactor > 1.7) {
        // 根据 startocean 的逻辑，此种条件下 gtk 窗口放大为 2 倍
    ...
    QString cmd = "/usr/lib/lingmo-api/gtk-thumbnailer";
...
QString getGlobal(QString id, QString descFile, QString gtkTheme)
{
    ...
    QStringList example = keyFile.getStrList("Lingmo Theme", "Example");
    if (!example.isEmpty()) {
        bool isdark = (gtkTheme == "lingmo-dark") ;
