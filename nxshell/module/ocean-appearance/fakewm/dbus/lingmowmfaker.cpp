...
#include <QGSettings>
Q_GLOBAL_STATIC_WITH_ARGS(QGSettings, _gsettings_ocean_appearance, ("com.lingmo.ocean.appearance"))
Q_GLOBAL_STATIC_WITH_ARGS(QGSettings, _gsettings_ocean_zone, ("com.lingmo.ocean.zone"))
...
#define KWinConfigName "lingmo-kwinrc"
...
LingmoWMFaker::LingmoWMFaker(QObject* appearance)
    : QObject(appearance)
    ...
    , m_previewWinMiniPair(QPair<uint, bool>(-1, false))
{
    ...
    connect(_gsettings_ocean_appearance, &QGSettings::changed, this, &LingmoWMFaker::onGsettingsOCEANAppearanceChanged);
    connect(_gsettings_ocean_zone, &QGSettings::changed, this, &LingmoWMFaker::onGsettingsOCEANZoneChanged);
    ...
    for (const QString &key : zoneKeyList) {
        onGsettingsOCEANZoneChanged(key);
    }
#endif // DISABLE_LINGMO_WM
    ...
    QDBusConnection::sessionBus().connect(QStringLiteral("org.lingmo.ocean.Appearance1"),
                                          QStringLiteral("/org/lingmo/ocean/Appearance1"),
                                          QStringLiteral("org.lingmo.ocean.Appearance1"),
...
void LingmoWMFaker::handleWorkspaceCountChanged(quint32 count)
{
    std::lock_guard<std::mutex> guard(WORKSPACE_COUNT_GUARD);
    m_workspaceount = count;
}

void LingmoWMFaker::handleThemeChanged(const QString& key, const QString &value)
{
    if (key == "gtk") {
        SetDecorationLingmoTheme(value.contains("dark") ? "dark" : "light");
    }
}
...
int LingmoWMFaker::cursorSize() const
{
    KConfigGroup mousecfg(KSharedConfig::openConfig("kcminputrc", KConfig::NoGlobals), "Mouse");
    bool ok = false;
    // The default Xcursor theme size of lingmo-kwin is 24
...
{
    return _gsettings_ocean_appearance->get(GsettingsBackgroundUri).toStringList().value(index - 1);
}

static void setWorkspaceBackgroundForLingmoWM(const int index, const QString &uri)
{
    QStringList all_wallpaper = _gsettings_ocean_appearance->get(GsettingsBackgroundUri).toStringList();
    ...
    // 将壁纸设置同步到 lingmo-wm
    _gsettings_ocean_appearance->set(GsettingsBackgroundUri, all_wallpaper);
}
#endif // DISABLE_LINGMO_WM
...
QString LingmoWMFaker::GetWorkspaceBackgroundForMonitor(const int index,const QString &strMonitorName) const
{
    ...
    QDBusInterface interface("org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1", "org.lingmo.ocean.Appearance1");
...
int LingmoWMFaker::GetCurrentWorkspaceInner() const
{
...
void LingmoWMFaker::PreviewWindow(uint xid)
{
    ...
    if (KWindowEffects::isEffectAvailable(KWindowEffects::HighlightWindows)) {
        ...
        // 不过，ocean-dock中不支持此种用法，而且对kwin接口的调用仅仅是fallback，因此直接将xid作为预览请求的controller窗口
...
void LingmoWMFaker::enableEffect(bool on)
{
...
bool LingmoWMFaker::maybeShowWarningDialog()
{
    ...
    if (!enable) {
        return QProcess::startDetached("/usr/lib/lingmo-daemon/ocean-warning-dialog");
    }
...
void LingmoWMFaker::setWorkspaceBackgroundForMonitor(const int index, const QString &strMonitorName, const QString &uri) const
{
...
#ifndef DISABLE_LINGMO_WM
    QStringList allWallpaper = _gsettings_ocean_appearance->get(GsettingsBackgroundUri).toStringList();
    ...
    _gsettings_ocean_appearance->set(GsettingsBackgroundUri, allWallpaper);
#endif // DISABLE_LINGMO_WM
}
...
void LingmoWMFaker::quitTransientBackground()
{
    ...
    if (!m_lingmoWMBackgroundUri.isEmpty()) {
        ...
        QSignalBlocker blocker(_gsettings_ocean_appearance);
        Q_UNUSED(blocker)
        setWorkspaceBackgroundForLingmoWM(GetCurrentWorkspaceInner(), m_lingmoWMBackgroundUri);
        m_lingmoWMBackgroundUri.clear();
    }
#endif // DISABLE_LINGMO_WM
}
...
#ifndef DISABLE_LINGMO_WM
void LingmoWMFaker::onGsettingsOCEANAppearanceChanged(const QString &key)
{
    if (QLatin1String(GsettingsBackgroundUri) == key) {
        const QStringList &uris = _gsettings_ocean_appearance->get(GsettingsBackgroundUri).toStringList();
        ...
        // 更新值
        if (!m_lingmoWMBackgroundUri.isEmpty()) {
            m_lingmoWMBackgroundUri = uris.value(GetCurrentWorkspaceInner());
        }
    }
}
...
void LingmoWMFaker::onGsettingsOCEANZoneChanged(const QString &key)
{
    ...
    const QString &value = _gsettings_ocean_zone->get(key).toString();
...
void LingmoWMFaker::SlotUpdateMultitaskStatus(bool isActive)
{
    m_isMultitaskingActived = isActive;
}
