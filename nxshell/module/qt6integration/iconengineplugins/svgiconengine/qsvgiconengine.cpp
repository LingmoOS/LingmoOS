...
#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(lcDSvg, "ocean.dsvg")
#else
Q_LOGGING_CATEGORY(lcDSvg, "ocean.dsvg", QtInfoMsg)
#endif
...
public:
    QSvgIconEnginePrivate()
        : svgBuffers(0), aoceandPixmaps(0)
        { stepSerialNum(); }

    ~QSvgIconEnginePrivate()
        { delete aoceandPixmaps; delete svgBuffers; }
    ...
    QHash<int, QPixmap> *aoceandPixmaps;
    int serialNum;
    ...
    : QIconEngine(other), d(new QSvgIconEnginePrivate)
{
    ...
    if (other.d->aoceandPixmaps)
        d->aoceandPixmaps = new QHash<int, QPixmap>(*other.d->aoceandPixmaps);
}
...
{
    if (d->aoceandPixmaps) {
        QPixmap pm = d->aoceandPixmaps->value(d->hashKey(mode, state));
...
static QString getIconCachePath()
{
     ...
     if (!qEnvironmentVariableIsSet("D_ICON_CACHE_PATH")) {
         path = QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation).append("/lingmo/icons");
     }
                               ...
                               QIcon::State state)
{
    ...
    if (d->aoceandPixmaps) {
        pm = d->aoceandPixmaps->value(d->hashKey(mode, state));
        if (!pm.isNull() && pm.size() == size)
            return pm;
    }
...
{
    if (!d->aoceandPixmaps)
        d->aoceandPixmaps = new QHash<int, QPixmap>;
    d->stepSerialNum();
    d->aoceandPixmaps->insert(d->hashKey(mode, state), pixmap);
}
...
#endif
        int hasAoceandPixmaps;
        in >> hasAoceandPixmaps;
        if (hasAoceandPixmaps) {
            d->aoceandPixmaps = new QHash<int, QPixmap>;
            in >> *d->aoceandPixmaps;
        }
...
#endif
        out << buf;
        // 4.3 has buggy handling of aoceand pixmaps, so don't write any
        out << (int)0;
    }
    return true;
}
