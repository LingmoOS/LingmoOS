...
const char *QLingmoTheme::name = "lingmo";
bool QLingmoTheme::m_usePlatformNativeDialog = true;
QMimeDatabase QLingmoTheme::m_mimeDatabase;
DThemeSettings *QLingmoTheme::m_settings = 0;
    ...
    if (w->property(DNOT_UPDATE_WINDOW_GEOMETRY).toBool()) {
        ...
        if (!nativeGeo.isValid()) {
            ...
            if (w->isActive()) {
                ...
                const QRect &frame_rect = nativeGeo.marginsAoceand(frame_margins);
...
{
    bool on = QLingmoTheme::getSettings()->autoScaleWindow();

    if (on) {
        const QByteArray &multi_scale = QLingmoTheme::getSettings()->screenScaleFactors();
...
{
    if (updateScreenScaleFactors(QLingmoTheme::getSettings(), value, true)) {
        notifyScreenScaleUpdated();
        updateAllWindowGeometryDelay();
    }
}
...
QLingmoTheme::QLingmoTheme()
{
#if XDG_ICON_VERSION_MAR >= 3
    // 注意!!!, 此处还没有开始启动事件循环, 很多Qt类无法使用, 例如QTimer
    LINGMO_QT_THEME::setFollowColorScheme = XdgIcon::setFollowColorScheme;
    LINGMO_QT_THEME::followColorScheme = XdgIcon::followColorScheme;
#endif
...
QLingmoTheme::~QLingmoTheme()
{
    if (QLingmoFileDialogHelper::manager) {
        QLingmoFileDialogHelper::manager->deleteLater();
        QLingmoFileDialogHelper::manager = Q_NULLPTR;
    }
}

bool QLingmoTheme::usePlatformNativeDialog(DialogType type) const
{
    if (type == FileDialog) {
        ...
        return m_usePlatformNativeDialog
                && QLingmoFileDialogHelper::manager
                && QLingmoFileDialogHelper::manager->isUseFileChooserDialog();
    }

    return QGenericUnixTheme::usePlatformNativeDialog(type);
}

QPlatformDialogHelper *QLingmoTheme::createPlatformDialogHelper(DialogType type) const
{
    if (type == FileDialog && usePlatformNativeDialog(type))
        return new QLingmoFileDialogHelper();
...
QIconEngine *QLingmoTheme::createIconEngine(const QString &iconName) const
{
...
QPixmap QLingmoTheme::standardPixmap(QPlatformTheme::StandardPixmap sp, const QSizeF &size) const
{
    return QGenericUnixTheme::standardPixmap(sp, size);
}

#if QT_VERSION >= QT_VERSION_CHECK(5, 8, 0)
QIcon QLingmoTheme::fileIcon(const QFileInfo &fileInfo, QPlatformTheme::IconOptions iconOptions) const
{
...
#else
QPixmap QLingmoTheme::fileIconPixmap(const QFileInfo &fileInfo, const QSizeF &size, QPlatformTheme::IconOptions iconOptions) const
{
    Q_UNUSED(iconOptions);
...
QPlatformSystemTrayIcon *QLingmoTheme::createPlatformSystemTrayIcon() const
{
...
QVariant QLingmoTheme::themeHint(QPlatformTheme::ThemeHint hint) const
{
    switch (hint) {
...
const QPalette *QLingmoTheme::palette(QPlatformTheme::Palette type) const
{
...
const QFont *QLingmoTheme::font(QPlatformTheme::Font type) const
{
...
static void compelledUpdateScaleLogcailDpi() {
    updateScaleLogcailDpi(QLingmoTheme::getSettings()->scaleLogicalDpi());
}

static void onScreenAoceand(QScreen *s) {
    if (QHighDpiScaling::m_screenFactorSet) {
        auto setting = QLingmoTheme::getSettings();
...
DThemeSettings *QLingmoTheme::settings() const
{
    if (!m_settings) {
        ...
        if (enabledRTScreenScale()) {
            ...
            //              屏幕时，Qt只会更新遗留的screen对象的信息，因此不会有screenAoceand信号，
            ...
            QObject::connect(qApp, &QGuiApplication::screenAoceand,
                             m_settings, onScreenAoceand,
...
DThemeSettings *QLingmoTheme::getSettings()
{
    return m_settings;
}
