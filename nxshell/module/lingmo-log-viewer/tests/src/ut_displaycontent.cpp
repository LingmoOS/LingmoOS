...
TEST_F(DisplayContentlx_UT, slot_appLogs_UT)
{
    ...
    m_content->slot_appLogs(1, "~/.cache/lingmo/lingmo-log-viewer/lingmo-log-viewer.log");
    EXPECT_NE(m_content, nullptr);
    ...
    EXPECT_EQ(m_content->m_curApp,"lingmo-log-viewer")<<"check the status after slot_appLogs()";
}
...
TEST_F(DisplayContentlx_UT, slot_kernData_UT)
{
    ...
    LOG_MSG_JOURNAL kernLog = {"2021-05-21", "UOS", "ocean-daemon", "1122", "DEBUG", "DNF version: 4.2.23"};
...
TEST(DisplayContent_createOOCTable_UT, DisplayContent_createOOCTable_UT_001)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        data.name = "lingmo-installer-init.log";
        data.path = "/var/log/lingmo-installer-init.log";
        list.append(data);
    }
...
TEST(DisplayContent_generateOOCFile_UT, DisplayContent_generateOOCFile_UT_001)
{
    ...
    DisplayContent *p = new DisplayContent(nullptr);
    EXPECT_NE(p, nullptr);
    p->generateOOCFile("/var/log/lingmo-installer-init.log");
    p->deleteLater();
}
...
INSTANTIATE_TEST_CASE_P(DisplayContent, DisplayContent_getAppName_UT, ::testing::Values(DisplayContent_getAppName_UT_Param("", ""), DisplayContent_getAppName_UT_Param("lingmo-log-viewer.log", "日志收集工具"), DisplayContent_getAppName_UT_Param("lingmo-log-viewer", "日志收集工具"), DisplayContent_getAppName_UT_Param("/test/lingmo-log-viewer.log", "日志收集工具")));
...
TEST(DisplayContent_slot_appLogs_UT, DisplayContent_slot_appLogs_UT_001)
{
    ...
    EXPECT_NE(p, nullptr);
    stub.set(ADDR(DisplayContent, generateAppFile), DisplayContent_slot_BtnSelected_UT_generateAppFile);
    p->slot_appLogs(1, Utils::homePath + "/.cache/lingmo/lingmo-log-viewer/lingmo-log-viewer.log");
    p->deleteLater();
}
