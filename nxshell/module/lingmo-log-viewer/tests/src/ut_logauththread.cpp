...
QString stub_dnfReadStream(const QString &filePath)
{
    ...
    QString result;
    if(i % 2 == 0) {
        result = "2021-05-21T02:08:36Z DEBUG 加载插件：builoceanp, changelog, \nconfig-manager, copr, debug, debuginfo-install, download, \ngenerate_completion_cache, needs-restarting, playground, repoclosure, repodiff, repograph, repomanage, reposync";
    } else {
        result = "";
    }
    ++i;
...
{
    Q_UNUSED(filePath);
    return "Aug 21 13:23:33 uos-PC lingmo-lingmoid-daemon[2946]: #033[33m[WAR] manager.go:218:#033[0m skip doSync";
}
