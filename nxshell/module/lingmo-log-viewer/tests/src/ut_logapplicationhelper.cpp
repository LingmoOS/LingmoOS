...
{
    return "NotShowIn testX-Lingmo-Vendor=lingmo";
}

void stub_parseField(QString path, QString name, bool isLingmo, bool isGeneric,
                     bool isName)
{
}
...
{
public:
    LogApplicationHelper_parseField_UT_Param(bool iIsPathEmpty, bool iIsLingmo, bool iIsGeneric, bool iIsName)
    {
...
TEST_P(LogApplicationHelper_parseField_UT, LogApplicationHelper_parseField_UT)
{
    ...
    QString path = param.isPathEmpty ? "" : "../sources/ocean-calendar.log";
    p->generateTransName(path, "ocean-calendar.log", param.isLingmo, param.isGeneric, param.isName);
...
TEST(LogApplicationHelper_getLogFile_UT, LogApplicationHelper_getLogFile_UT_001)
{
    LogApplicationHelper *p = new LogApplicationHelper(nullptr);
    EXPECT_NE(p, nullptr);
    QString path = p->getLogFile("../sources/ocean-calendar.log");
    p->deleteLater();
}
...
TEST(LogApplicationHelper_getPathByAppId_UT, LogApplicationHelper_getPathByAppId_UT)
{
    LogApplicationHelper *p = new LogApplicationHelper(nullptr);
    EXPECT_NE(p, nullptr);
    p->getPathByAppId("lingmo-log-viewer");
    p->deleteLater();
}
