...
class LogViewerPlugin : public QObject, public LogViewerPluginInterface
{
    ...
    //Q_PLUGIN_METADATA(IID "com.lingmo.logviewer.LogViewerPlugin" FILE "LogViewerPlugin.json")
    Q_PLUGIN_METADATA(IID "com.lingmo.logviewer.LogViewerPlugin")
...
public:
    ...
    // param: appid 自研应用id，用于导出指定应用的日志，若为空，什么都不做，返回false（例：导出相册的日志信息，传入lingmo-album即可）
...
private:
     ...
     */
    /**
     * @brief appListOrigin 未经过筛选的内核日志数据   ~/.cache/lingmo/xxx.log(.xxx)
     */
