...
#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(logBackend, "org.lingmo.log.viewer.backend")
#else
Q_LOGGING_CATEGORY(logBackend, "org.lingmo.log.viewer.backend", QtInfoMsg)
#endif
...
LOG_FLAG LogBackend::type2Flag(const QString &type, QString& error)
{
    ...
    if (type == TYPE_SYSTEM)
        ...
        flag = NONE;
        error = QString("Unknown type: %1.").arg(type) + "USEAGE: system(journal log), kernel(kernel log), boot(boot log), dpkg(dpkg log), dnf(dnf log), kwin(Kwin log), xorg(Xorg log), app(lingmo app log), coredump(coredump log)、boot-shutdown-event(boot shutdown event log)、other(other log)、custom(custom log)、audit(audit log)";
    }
