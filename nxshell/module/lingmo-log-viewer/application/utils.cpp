...
#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(logUtils, "org.lingmo.log.viewer.utils")
#else
Q_LOGGING_CATEGORY(logUtils, "org.lingmo.log.viewer.utils", QtInfoMsg)
#endif
...
// 1.sudo权限运行lingmo-log-viewer，QDir::homePath返回的是/root
// 此种情况下，使用freedesktop dbus接口获取当前登录用户家目录，以便能正确导出日志
// 2.systemd服务启动lingmo-log-viewer，QDir::homePath返回的是/，因该方式下freedesktop dbus接口获取为空，只能将/root作为homePath
...
// 高频重复崩溃记录exe路径名单
//const QString COREDUMP_REPEAT_CONFIG_PATH = "/var/cache/lingmo/lingmo-log-viewer/repeatCoredumpApp.list";
const QString COREDUMP_REPEAT_CONFIG_PATH = Utils::homePath + "/.cache/lingmo/lingmo-log-viewer/repeatCoredumpApp.list";
...
bool Utils::isValidUserName(const QString &userName)
{
    ...
    QDBusInterface interface("com.lingmo.daemon.Accounts", "/com/lingmo/daemon/Accounts", "com.lingmo.daemon.Accounts", QDBusConnection::systemBus());
