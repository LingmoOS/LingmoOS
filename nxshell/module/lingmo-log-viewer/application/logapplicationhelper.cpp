...
#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(logAppHelper, "org.lingmo.log.viewer.application.helper")
#else
Q_LOGGING_CATEGORY(logAppHelper, "org.lingmo.log.viewer.application.helper", QtInfoMsg)
#endif
...
// 自研应用日志json配置文件目录
const QString APP_LOG_CONFIG_PATH = "/usr/share/lingmo-log-viewer/lingmo-log.conf.d";
...
const QString DCONFIG_APPID = "org.lingmo.log.viewer";
const QString GSETTING_APPID = "com.lingmo.log.viewer";
...
void LogApplicationHelper::initOtherLog()
{
    ...
    m_other_log_list_temp.append(QStringList() << "ocean-dconfig-daemon" << "/var/log/lingmo/ocean-dconfig-daemon");
    m_other_log_list_temp.append(QStringList() << "ocean-file-manager-daemon" << "/var/log/lingmo/ocean-file-manager-daemon");
    m_other_log_list_temp.append(QStringList() << "lingmo-boot-maker-service.log" << "/var/log/lingmo/lingmo-boot-maker-service.log");

    m_other_log_list_temp.append(QStringList() << "lingmo-diskmanager-service" << "/var/log/lingmo/lingmo-diskmanager-service/Log");
    m_other_log_list_temp.append(QStringList() << "lingmo-home-appstore-daemon" << "/var/log/lingmo/lingmo-home-appstore-daemon");
    m_other_log_list_temp.append(QStringList() << "lingmo-log-viewer-service" << "/var/log/lingmo/lingmo-log-viewer-service/Log");
    m_other_log_list_temp.append(QStringList() << "lingmo-sync-helper_ERROR.log" << "/var/log/lingmo-lingmoid-daemon/lingmo-sync-helper_ERROR.log");

    m_other_log_list_temp.append(QStringList() << "lingmo-sync-helper.log" << "/var/log/lingmo-lingmoid-daemon/lingmo-sync-helper.log");
    m_other_log_list_temp.append(QStringList() << "lingmo-defender-system-daemon.log" << "/var/log/lingmo-defender-system-daemon.log");
    m_other_log_list_temp.append(QStringList() << "lingmo-face.log" << "/var/log/lingmo-face.log");
    m_other_log_list_temp.append(QStringList() << "lingmo-installer-first-boot.log" << "/var/log/lingmo-installer-first-boot.log");

    m_other_log_list_temp.append(QStringList() << "lingmo-installer-init.log" << "/var/log/lingmo-installer-init.log");
    m_other_log_list_temp.append(QStringList() << "lingmo-installer.log" << "/var/log/lingmo-installer.log");
    m_other_log_list_temp.append(QStringList() << "lingmo-monitornetflow.log" << "/var/log/lingmo-monitornetflow.log");
    ...
    m_other_log_list_temp.append(QStringList() << "lingmo-graphics-driver-manager-server.log" << "/var/log/lingmo-graphics-driver-manager-server.log");
    m_other_log_list_temp.append(QStringList() << "uos-activator-kms.log" << "/var/log/uos/uos-activator-kms/uos-activator-kms.log");

    m_other_log_list_temp.append(QStringList() << "lingmo-recovery-gui.log" << "/recovery/lingmo-recovery-gui.log");
...
void LogApplicationHelper::initCustomLog()
{
    ...
    if (!m_pGSettings) {
        if (QGSettings::isSchemaInstalled(GSETTING_APPID.toUtf8())) {
            m_pGSettings = new QGSettings(GSETTING_APPID.toUtf8(), "/com/lingmo/log/viewer/");
...
void LogApplicationHelper::createDesktopFiles()
{
    ...
    QStringList regStr {"lingmo-", "ocean-", "org.lingmo", "com.lingmo", "uos-"};
    for (QString desktop : fileInfoList) {
        //需要符合以lingmo或者ocean开头的应用
        for (auto &it : regStr) {
    ...
    for (QString var : tempDesktopFiles) {
        ...
        while (!fi.atEnd()) {
            ...
            if (!lineStr.contains("X-Lingmo-Vendor", Qt::CaseInsensitive)) {
                continue;
            }
            ...
            if (0 == rval.compare("lingmo", Qt::CaseInsensitive)) {
                isLingmo = true;
                break;
            }
        }
        ...
        if (canDisplay) {
            ...
            generateTransName(filePath, var.split(QDir::separator()).last(), isLingmo, isGeneric, isName);
        }
    }
}
...
void LogApplicationHelper::createLogFiles()
{
    ...
    QString path = homePath + "/.cache/lingmo/";
    QDir appDir(path);
    if (!appDir.exists()) {
        return;
    }
 ...
 * @param name 包名
 * @param isLingmo  是否是lingmo应用
 * @param isGeneric 是否有GenericName字段
 * @param isName 是否有Name字段
 */
void LogApplicationHelper::generateTransName(const QString &path, const QString &name, bool isLingmo, bool isGeneric,
                                      bool isName)
{
    Q_UNUSED(isName)
    QFile fi(path);
            ...
            if (document.isObject()) {
                ...
                if (object.contains("logType") || object.contains("logPath") || object.contains("exec")) {
                    ...
                    if (submodule.logType == "file") {
                        // 对应用日志的logPath做有效性判断，若无效，则填入默认日志路径~/.cache/lingmo
...
void LogApplicationHelper::validityJsonLogPath(SubModuleConfig &submodule)
{
    ...
    // 若logPath无效，则填入默认日志路径~/.cache/lingmo
