...
void LogCollectorMain::initUI()
{
    ...
    /** add titleBar */
    titlebar()->setIcon(QIcon::fromTheme("lingmo-log-viewer"));
    titlebar()->setTitle("");
#ifdef SPLITTER_TYPE
...
{
    m_settings = DSettings::fromJsonFile(":/lingmo-log-viewer-setting.json");
...
void LogCollectorMain::exportAllLogs()
{
    ...
    if (false == authorization) {
        ...
        if (DBusManager::isSEOpen() && DBusManager::isAuditAdmin())
            policyActionId = "com.lingmo.pkexec.logViewerAuth.exportLogsSelf";
        else
            // 其他情况，默认为多用户鉴权
            policyActionId = "com.lingmo.pkexec.logViewerAuth.exportLogs";
