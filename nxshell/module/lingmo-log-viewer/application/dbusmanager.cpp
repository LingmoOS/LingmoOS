...
#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(logDBusManager, "org.lingmo.log.viewer.dbus.manager")
#else
Q_LOGGING_CATEGORY(logDBusManager, "org.lingmo.log.viewer.dbus.manager", QtInfoMsg)
#endif
...
bool DBusManager::isSEOpen()
{
    ...
    if (interfaceSE.isValid()) {
        ...
        if (!reply.error().message().isEmpty())
            qCWarning(logDBusManager) << qPrintable(QString("com.lingmo.daemon.SecurityEnhance.Status DBus error: %1").arg(reply.error().message()));
...
bool DBusManager::isAuditAdmin()
{
    ...
    if (interfaceSE.isValid()) {
        ...
        if (!reply.error().message().isEmpty())
            qCWarning(logDBusManager) << qPrintable(QString("com.lingmo.daemon.SecurityEnhance.GetSEUserByName DBus error: %1").arg(reply.error().message()));
