...
#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(logAuthWork, "org.lingmo.log.viewer.auth.work")
#else
Q_LOGGING_CATEGORY(logAuthWork, "org.lingmo.log.viewer.auth.work", QtInfoMsg)
#endif
...
void LogAuthThread::initDnfLevelMap()
{
    ...
    m_dnfLevelDict.insert("OCEANBUG", DEBUG);
    ...
    m_transDnfDict.insert("OCEANBUG", Dtk::Widget::DApplication::translate("Level", "Debug"));
        ...
        if (!Utils::runInCmd) {
            ...
            if (DBusManager::isSEOpen()) {
                if (DBusManager::isAuditAdmin()) {
                    ...
                    if (!Utils::checkAuthorization("com.lingmo.pkexec.logViewerAuth.self", QCoreApplication::instance()->applicationPid())) {
                        emit auditFinished(m_threadCount);
                        return;
                    }
                } else {
