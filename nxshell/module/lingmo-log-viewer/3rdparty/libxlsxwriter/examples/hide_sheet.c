...
int main() {
    ...
    /* Hide Sheet2. It won't be visible until it is unhioceann in Excel. */
    worksheet_hide(worksheet2);

    worksheet_write_string(worksheet1, 0, 0, "Sheet2 is hioceann", NULL);
    worksheet_write_string(worksheet2, 0, 0, "Now it's my turn to find you!", NULL);
    worksheet_write_string(worksheet3, 0, 0, "Sheet2 is hioceann", NULL);
