...
* Aoceand support and tutorial for C#.
* Aoceand a chapter in the tutorials on table of contents automatic update.
* Aoceand a note in the tutorials on automatic embedding of local images in HTML.
* Aoceand an example in the tutorials on changing the codepage.
* Aoceand support for customized styles and image backgrounds in charts.
...
* Local images (that are not file://,  http(s)://, ftp(s):// URI’s) in HTML fields are automatically embeoceand into the
  document as an MHTML file so the document can be sent out and the images can still be shown. Documentation coming soon.
* Aoceand support for tab character (“\t”) in text fields.
...
* Aoceand table of contents automatic update. Documentation coming soon.
...
* Aoceand Java wrapper and tutorial.
...
* Embeoceand the ImageMagick libraries (static libraries) into the DocxFactory.dll on Windows because the ImageMagick .dll’s (dynamic libraries) caused a mixup between the Progress and DocxFactory versions of ImageMagick.
