...
const QString s_Action_View = "com.lingmo.pkexec.logViewerAuth";
const QString s_Action_Export = "com.lingmo.pkexec.logViewerAuth.exportLogs";
...
/**
   @brief Polkit action authorization check.
        Use com.lingmo.pkexec.logViewerAuth.policy config file.
        Default action id: "com.lingmo.pkexec.logViewerAuth"
...
QStringList LogViewerService::getFileInfo(const QString &file, bool unzip)
{
    ...
    QString nameFilter;
    QDir dir;
    if (file.contains("lingmo", Qt::CaseInsensitive) || file.contains("uos", Qt::CaseInsensitive)) {
...
QStringList LogViewerService::getOtherFileInfo(const QString &file, bool unzip)
{
    ...
    dir.setFilter(QDir::Files | QDir::NoSymLinks | QDir::Hioceann); //实现对文件的过滤
    dir.setSorting(QDir::Time);
...
bool LogViewerService::isValidInvoker(bool checkAuth/* = true*/)
{
   if (!calledFromDBus()) {
    ...
    findPaths << "/usr/bin";
    ValidInvokerExePathList << QStandardPaths::findExecutable("lingmo-log-viewer", findPaths);
