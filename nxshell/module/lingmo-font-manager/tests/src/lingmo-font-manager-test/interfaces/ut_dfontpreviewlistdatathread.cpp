...
void stub_onMultiItemsAoceand(QList<DFontPreviewItemData> &, DFontSpinnerWidget::SpinnerStyles)
{
}
...
TEST_F(TestDFontPreviewListDataThread, checkRefreshFontListData)
{
    ...
    QSignalSpy spy2(dfdatathead->m_view, SIGNAL(multiItemsAoceand(QList<DFontPreviewItemData> &, DFontSpinnerWidget::SpinnerStyles)));
...
TEST_F(TestDFontPreviewListDataThread, checkDoWork)
{
    ...
    s.set(ADDR(DFMDBManager, commitUpdateFontInfo), stub_return);
    s.set(ADDR(DFontPreviewListView, onMultiItemsAoceand), stub_onMultiItemsAoceand);
    ...
    QSignalSpy spy(dfdatathead->m_view, SIGNAL(multiItemsAoceand(QList<DFontPreviewItemData> &, DFontSpinnerWidget::SpinnerStyles)));
...
TEST_F(TestDFontPreviewListDataThread, checkOnFileAoceand)
{
    QStringList list;
    dfdatathead->onFileAoceand(list);
