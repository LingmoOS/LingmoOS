...
DSqliteUtil::DSqliteUtil(const QString &strDatabase)
    ...
    , m_query(nullptr)
{
    QDir dbdir(QDir::homePath() + "/.local/share/lingmo/lingmo-font-manager/");
    if (!dbdir.exists()) {
        dbdir.mkdir(QDir::homePath() + "/.local/share/lingmo/lingmo-font-manager/");
        qDebug() << __FUNCTION__ << QDir::homePath() + "/.local/share/lingmo/lingmo-font-manager/";
    }
