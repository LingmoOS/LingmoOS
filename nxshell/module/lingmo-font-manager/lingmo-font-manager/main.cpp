...
int main(int argc, char *argv[])
{
    ...
    // 依赖DTK的程序，如果要在root下或者非lingmo/uos环境下运行不会发生异常，就需要加上该环境变量
    if (!QString(qgetenv("XDG_CURRENT_DESKTOP")).toLower().startsWith("lingmo")) {
        setenv("XDG_CURRENT_DESKTOP", "Lingmo", 1); //setenv改变或添加一个环境变量
    }
    ...
    app.loadTranslator();
    app.setApplicationName("lingmo-font-manager");
    app.setOrganizationName("lingmo");
    app.setApplicationVersion(VERSION);
    app.setApplicationAcknowledgementPage("https://www.lingmo.org/original/lingmo-font-installer/");
    app.setProductIcon(QIcon::fromTheme(LINGMO_FONT_MANAGER));
...
//    qDebug() << DTK_VERSION << endl;
    ...
    if (dbus.registerService("com.lingmo.FontManager")) {
        dbus.registerObject("/com/lingmo/FontManager", &app, QDBusConnection::ExportScriptableSlots);
        app.parseCmdLine();
        app.activateWindow();
        return app.exec();
    } else {
