...
signals:
    ...
    //批量添加item请求
    void multiItemsAoceand(QList<DFontPreviewItemData> &data, DFontSpinnerWidget::SpinnerStyles styles);
    //启动时批量添加item请求
    // void startupMultiItemsAoceand(QList<DFontPreviewItemData> &data);
    //请求移除某一项的响应函数
    void itemRemoved(DFontPreviewItemData &data);
    //请求安装后的选中响应函数
    void itemsSelected(const QStringList &files, bool isFirstInstall = false);
    void requestInstFontsUiAoceand();
    //请求字体列表数目改变函数
...
public slots:
    ...
    //listview中添加项响应函数
    void onMultiItemsAoceand(QList<DFontPreviewItemData> &data, DFontSpinnerWidget::SpinnerStyles styles);

    //启动时向listview添加项
    void onStartupMultiItemAoceand(QList<DFontPreviewItemData> &data);
