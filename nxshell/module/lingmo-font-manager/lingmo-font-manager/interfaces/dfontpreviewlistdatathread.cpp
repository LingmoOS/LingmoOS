    ...
    , m_mutex(nullptr)
{
    ...
    connect(this, &DFontPreviewListDataThread::requestAoceand, this, &DFontPreviewListDataThread::onFileAoceand);
...
void DFontPreviewListDataThread::doWork()
{
    ...
    Q_EMIT m_view->multiItemsAoceand(m_fontModelList, DFontSpinnerWidget::StartupLoad);
}
...
/*************************************************************************
 ...
 <Note>          null
*************************************************************************/
void DFontPreviewListDataThread::onFileAoceand(const QStringList &files)
{
    if (files.isEmpty()) {
        return;
    }
...
//    Q_EMIT m_view->itemAoceand(itemData);
...
void DFontPreviewListDataThread:: refreshFontListData(bool isStartup, const QStringList &installFont)
{
    ...
    if (!isStartup && !installFont.isEmpty()) {
        Q_EMIT m_view->multiItemsAoceand(datalist, DFontSpinnerWidget::NoLabel);
        Q_EMIT m_view->multiItemsAoceand(m_diffFontModelList, DFontSpinnerWidget::NoLabel);
        Q_EMIT m_view->itemsSelected(installFont);
    } else if (isStartup) {
        QList<DFontPreviewItemData> startuplist = m_fontModelList.mid(0, 50);
        Q_EMIT m_view->multiItemsAoceand(startuplist, DFontSpinnerWidget::Load);
    } else {
        Q_EMIT m_view->multiItemsAoceand(m_fontModelList, DFontSpinnerWidget::Load);
    }
}
...
void DFontPreviewListDataThread::refreshStartupFontListData()
{
    ...
    Q_EMIT m_view->multiItemsAoceand(m_startModelList, DFontSpinnerWidget::StartupLoad);
