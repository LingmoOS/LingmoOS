...
lingmo_font_manager(){
#{{{
    local font_log=~/.cache/lingmo/lingmo-font-manager/lingmo-font-manager.log
    #关闭自体管理器
    kill_lingmo_font_manager(){
    #{{{
        local font_close='N'
        #查找到已经启动自体管理器
        local font_pid=`ps -eo pid,comm | grep 'lingmo-font' | awk '{print $1}'`
    ...
    #检查有没有安装自体管理器
    local font_manager=`which lingmo-font-manager`
    ...
    while [ 1 ] ; do
      ...
      #  font_top=`top -n1 -p $font_pid | grep "lingmo-font-m" | col -xb | tr -s ' '`
      local font_top=`top -n1 -p $font_pid  | grep "lingmo-font-m" | col -xb`
   ...
   # clear;
    ...
    #一 启动时长
    local font_startup_duration=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0001" | sed -e 's/.*LINGMO_FONT_MANAGER-0001\(.*\)#.*/\1/g' -e 's/ //g'`
    echo "启动时长: $font_startup_duration 毫秒" >> $font_result

    #二 从启动到加载字体完成时长
    local font_startup_load_duration=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0007" | sed -e 's/.*LINGMO_FONT_MANAGER-0007\(.*\)#.*/\1/g' -e 's/ //g'`
    ...
    #三 安装
    local font_inst_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0003" | wc -l`
    echo $"安装次数为 $font_inst_times 次" >> $font_result

    # 安装字体个数和安装时长
    local font_inst_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0003"  | sed -e 's/.*LINGMO_FONT_MANAGER-0003\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    ...
    #四 导出字体
    local font_export_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0004" | wc -l`
    echo $"导出次数为 $font_export_times 次" >> $font_result
    local font_export_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0004"  | sed -e 's/.*LINGMO_FONT_MANAGER-0004\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    # 导出字体个数和导出时长
    ...
    #五 删除字体
    local font_delete_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0005" | wc -l`
    echo $"删除次数为 $font_delete_times 次" >> $font_result
    local font_delete_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0005"  | sed -e 's/.*LINGMO_FONT_MANAGER-0005\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    # 删除字体个数和删除时长
    ...
    #六 禁用字体
    local font_deactive_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0009" | wc -l`
    echo $"禁用次数为 $font_deactive_times 次" >> $font_result
    local font_deactive_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0009"  | sed -e 's/.*LINGMO_FONT_MANAGER-0009\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    # 禁用字体个数和禁用时常
    ...
    #七 启用字体
    local font_active_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0008" | wc -l`
    echo $"启用次数为 $font_active_times 次" >> $font_result
    local font_active_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0008"  | sed -e 's/.*LINGMO_FONT_MANAGER-0008\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    # 启用字体个数和启用时长
    ...
    #八 收藏字体
    local font_favorite_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0006" | wc -l`
    echo $"收藏次数为 $font_favorite_times 次" >> $font_result
    local font_favorite_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0006"  | sed -e 's/.*LINGMO_FONT_MANAGER-0006\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    # 收藏字体个数和收藏时长
    ...
    #九 取消收藏字体
    local font_unfavorite_times=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0010" | wc -l`
    echo $"取消收藏次数为 $font_unfavorite_times 次" >> $font_result
    local font_unfavorite_data=`echo "$font_grab" | grep "LINGMO_FONT_MANAGER-0010"  | sed -e 's/.*LINGMO_FONT_MANAGER-0010\(.*\)#.*/\1/g' | sed 's/^[ ]*\(.*\)/\1/g' | nl -w1 -s : -n ln`
    # 取消收藏字体个数和时长
