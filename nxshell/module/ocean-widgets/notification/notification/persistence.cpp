...
{
    qDebug() << "onReceivedRecord() RecordAoceand id" << id;
    // TODO multiple RecordAoceand requests can be merged into one.
...
{
    if (!m_notifyObserver) {
        m_notifyObserver.reset(new QDBusInterface(OCEANNotifyDBusServer, OCEANNotifyDBusPath, OCEANNotifyDBusInterface));
        ...
        bool valid = connection.connect(OCEANNotifyDBusServer, OCEANNotifyDBusPath, OCEANNotifyDBusInterface,
                           "RecordAoceand", this, SLOT(onReceivedRecord(const QString &)));
        if (!valid) {
            qWarning() << "notifyObserver() NotifyConnection is invalid, and can't receive RecordAoceand signal.";
        }
