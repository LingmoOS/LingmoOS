...
void NotifyModel::freeData()
{
    if (!m_notifications.isEmpty() && m_notifications.first()->appName() == m_cacheList.first()->appName()) {
        m_view->createAoceandAnimation(m_cacheList.first(), getAppData(m_cacheList.first()->appName()));
    } else {
...
{
    connect(m_database, &AbstractPersistence::RecordAoceand, this, &NotifyModel::cacheData);
    connect(m_freeTimer, &QTimer::timeout, this, &NotifyModel::freeData);
    connect(m_view, &NotifyListView::aoceandAniFinished, this, &NotifyModel::addNotify);
