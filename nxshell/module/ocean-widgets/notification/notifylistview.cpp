...
void NotifyListView::createAoceandAnimation(EntityPtr entity, const ListItemPtr appItem)
{
    // 正在进行动画
    if (m_aniState)
        return;
    ...
    QWidget *currentWidget = this->indexWidget(index);
    if (!currentWidget) {
        qWarning() << "createAoceandAnimation() it maybe lost notification:" << entity->appName() << entity->id();
        return;
    }

    QParallelAnimationGroup *aoceandAniGroup = new QParallelAnimationGroup(this);
    connect(aoceandAniGroup, &QParallelAnimationGroup::finished, this, [ = ] { //动画完成发出动画完成的信号
        m_aniState = false;
        Q_EMIT aoceandAniFinished(entity);
    });
    ...
    aoceandAniGroup->addAnimation(addAni);
    ...
    if (!appItem->isCollapse() && canShow(appItem->showLast())) {
        for (int i = firstNotificationRow; i < this->model()->rowCount(QModelIndex()); ++i) {
            ...
            aoceandAniGroup->addAnimation(downMoveAni);
        }
    } else {
        ...
        if (!faceWidget) {
            qWarning() << "createAoceandAnimation() error for the notify:" << appItem->appName()
        ...
        aoceandAniGroup->addAnimation(removeAni);
    }
    aoceandAniGroup->start(QPropertyAnimation::DeleteWhenStopped);
    m_aniState = true;
}
