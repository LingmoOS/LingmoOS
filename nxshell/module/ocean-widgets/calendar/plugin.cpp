...
bool CalendarWidget::initialize(const QStringList &arguments) {
    ...
    if (!hasLoaded)
        hasLoaded = BuildinWidgetsHelper::instance()->loadTranslator("ocean-widgets-calendar_");
...
void CalendarWidget::showOCEANCalendar()
{
    qDebug(dwLog()) << "showOCEANCalendar()";
...
{
    return QIcon::fromTheme("ocean-calendar");
}
