...
int main(int argc, char *argv[])
{
    ...
    a.setApplicationName("ocean-widgets");
    ...
    QCommandLineOption showOption(QStringList() << "s"
                                                << "show",
                                  "Show ocean-widgets(hide for default).");
    QCommandLineParser parser;
    parser.setApplicationDescription("ocean-widgets is the desktop widgets service/implementation for OCEAN.\n"
                                     "We can show it by call `qdbus org.lingmo.ocean.Widgets1 /org/lingmo/ocean/Widgets1 org.lingmo.ocean.Widgets1.Show`");
...
//    Dtk::Core::DLogManager::registerFileAppender();
    ...
    if (server.registerService()) {
        qInfo(dwLog()) << "start ocean-widgets successfully.";
    } else {
        qInfo(dwLog()) << "start ocean-widgets failed.";
        return 1;
    }
    server.start();
    if (isShow)
