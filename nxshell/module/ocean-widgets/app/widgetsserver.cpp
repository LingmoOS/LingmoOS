...
#define OCEAN_WIDGETS_SERVICE "org.lingmo.ocean.Widgets1"
...
bool WidgetsServer::registerService()
{
    ...
    if (!bus.registerService(OCEAN_WIDGETS_SERVICE)) {
        qWarning() << QString("Can't register the %1 service, error:%2.").arg(OCEAN_WIDGETS_SERVICE).arg(bus.lastError().message());
        return false;
    }
    if (!bus.registerObject("/org/lingmo/ocean/Widgets1", this)) {
...
void WidgetsServer::SyncWidgets()
{
    ...
    const auto aoceandPluginIds = m_manager->addingPlugins();
    for (auto pluginId : aoceandPluginIds) {
        m_mainView->addPlugin(pluginId);
    }
    qDebug(dwLog()) << " removedPlugins:" << removedPluginIds
                    << "aoceandPlugins:" << aoceandPluginIds;
}
