...
{
    // The same pluginid will be overwritten by later, `OCEAN_WIDGETS_PLUGIN_DIRS` > `./plugins` > `/usr`
    QStringList dirs;
    dirs << PLUGIN_DIRECTORY;
    dirs << QCoreApplication::applicationDirPath() + "/plugins";
    const QString &envPaths = qEnvironmentVariable("OCEAN_WIDGETS_PLUGIN_DIRS");
    if (!envPaths.isEmpty()) {
