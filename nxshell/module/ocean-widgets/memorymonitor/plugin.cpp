...
void MemoryMonitorWidget::updateMemory()
{
    ...
    // This process is the same as `lingmo-system-monitor-plugin`
...
bool MemoryMonitorWidget::initialize(const QStringList &arguments) {
    ...
    if (!hasLoaded)
        hasLoaded = BuildinWidgetsHelper::instance()->loadTranslator("ocean-widgets-memorymonitor_");
...
{
    QDBusInterface interface("com.lingmo.SystemMonitorMain", "/com/lingmo/SystemMonitorMain",
                              "com.lingmo.SystemMonitorMain");
     if (!interface.isValid()) {
         qWarning() << "Get com.lingmo.SystemMonitorMain interface error." << interface.lastError().message();
         return;
     }
