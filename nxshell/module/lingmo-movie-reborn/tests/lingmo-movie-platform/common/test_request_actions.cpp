...
TEST(requestAction, onlineSub)
{
    ...
    QUrl url(QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/天空之眼 高清1080P.mp4"));
...
TEST(requestAction, subtitle)
{
    ...
    QTest::qWait(100);
    engine->loadSubtitle(QFileInfo(QString("/data/source/lingmo-movie-reborn/movie/Hachiko.A.Dog's.Story.ass")));
    QTest::qWait(100);
    w->requestAction(ActionFactory::ActionKind::HideSubtitle);
    QTest::qWait(100);
}
...
TEST(requestAction, settings)
{
    ...
    Settings::get().settings()->setOption("base.screenshot.location",
                                          "/data/source/lingmo-movie-reborn/movie/DMovie");
    QTest::qWait(200);
