...
#ifndef __aarch64__
    player->play(QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4"));
#endif
...
TEST(libdmr, utils)
{
    ...
    bool ret = utils::CompareNames("/data/source/lingmo-movie-reborn/movie/demo.mp4", "/data/source/lingmo-movie-reborn/movie/demo.mp3");
    ret = utils::CompareNames("/data/source/lingmo-movie-reborn/movie/demo.mp4", "/data/source/lingmo-movie-reborn//movie/demo.mp4");
    //QCOMPARE(ret, true);
    utils::UnInhibitPower(20);
    utils::MoveToCenter(w);
    utils::Time2str(90000);
    utils::ValidateScreenshotPath(QString("/data/source/lingmo-movie-reborn"));
    utils::ValidateScreenshotPath(QString("~/uos"));
    utils::MakeRoundedPixmap(QPixmap("/data/source/lingmo-movie-reborn/test.jpg"), 10, 10);
    utils::MakeRoundedPixmap(QSize(20, 30), QPixmap("/data/source/lingmo-movie-reborn/test.jpg"), 10, 10, 20);
    utils::ShowInFileManager("/data/source/lingmo-movie-reborn/movie/demo.mp4");
    utils::ShowInFileManager("/data/source/lingmo-movie-reborn/test");  //path is not exist
}
...
TEST(libdmr, playlistModel)
{
    ...
    QUrl url = QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4");
    bool is = false;
...
TEST(libdmr, movieConfiguration)
{
    MovieConfiguration::get().removeFromListUrl(
                QUrl("/data/source/lingmo-movie-reborn/Hachiko.A.Dog's.Story.ass"),
