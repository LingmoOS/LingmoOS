// Copyright (C) 2020 ~ 2021, Lingmo Technology Co., Ltd. <support@lingmo.org>
...
TEST(MainWindow, init)
{
    ...
    QMimeData mimeData;
    QList<QUrl> urls;
    urls << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4");
...
TEST(MainWindow, nakedstream)
{
    ...
    listPlayFiles << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/test.264");

    w->show();
    engine->playlist().loadPlaylist();
    engine->playlist().clear();
    engine->addPlayFiles(listPlayFiles);
    engine->playByName(QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/test.264"));

    // drop is playing video
    QMimeData mimeData;
    QList<QUrl> urls;
    urls << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/test.264");
    mimeData.setUrls(urls);
    ...
    QTest::qWait(1000);
    engine->playByName(QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4"));
}
...
TEST(MainWindow, tabInteraction)
{
    ...
    listPlayFiles << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4")\
                  << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/天空之眼 高清1080P.mp4")\
                  << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/bensound-sunny.mp3");
...
TEST(MainWindow, loadFile)
{
    ...
    wrongPath << "/data/source/lingmo-movie-reborn/movie/wrong.mp4";
    w->play(wrongPath);
    PlayerEngine *engine =  w->engine();
    QList<QUrl> listPlayFiles;
    listPlayFiles << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4")
...
TEST(MainWindow, DBus)
{
    ...
    DBusUtils utils;
    QVariant v = ApplicationAdaptor::redDBusProperty("com.lingmo.daemon.Audio", "/com/lingmo/daemon/Audio",
                                                     "com.lingmo.daemon.Audio", "SinkInputs");
    ...
    if (v.isValid()) {
        ...
        for (auto curPath : allSinkInputsList) {
            QVariant name = ApplicationAdaptor::redDBusProperty("com.lingmo.daemon.Audio", curPath.path(),
                                                                "com.lingmo.daemon.Audio.SinkInput", "Name");

            QString strMovie = QObject::tr("Movie");
            if (!name.isValid() || (!name.toString().contains(strMovie, Qt::CaseInsensitive) && !name.toString().contains("lingmo-movie", Qt::CaseInsensitive)))
                continue;
    ...
    QVariant method = ApplicationAdaptor::redDBusMethod("com.lingmo.daemon.Audio", "/com/lingmo/daemon/Audio",
                                                        "com.lingmo.daemon.Audio", "SinkInputs");
    QVariant method_invalid = ApplicationAdaptor::redDBusMethod("com.test", "/com/test", "com.test", "SinkInputs");
    method = DBusUtils::redDBusMethod("com.lingmo.daemon.Audio", "/com/lingmo/daemon/Audio",
                                      "com.lingmo.daemon.Audio", "SinkInputs");
    method_invalid = DBusUtils::redDBusMethod("com.test", "/com/test", "com.test", "SinkInputs");
    appAdaptor->Raise();
    appAdaptor->openFile("/data/source/lingmo-movie-reborn/movie/demo.mp4");
...
//    testEventList.addKeyClick(Qt::Key_Slash, Qt::ControlModifier | Qt::ShiftModifier, 500);
    ...
    if (w->engine()->state() == PlayerEngine::Idle) {
        w->engine()->playByName(QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4"));
    }
...
TEST(MainWindow, ViewProgBar)
{
    ...
    pmList.append(QPixmap(QString("/data/source/lingmo-movie-reborn/test.jpg")));
...
TEST(MainWindow, ThumbnailPreview)
{
    ...
    ThumbnailWorker::get().requestThumb(QUrl("/data/source/lingmo-movie-reborn/movie/demo.mp4"),
                                        200);
...
TEST(MainWindow, SettingsDialog)
{
    ...
    if(savePathEdit) {
        ...
        emit savePathEdit->textEdited("/data/source/lingmo-movie-reborn/movie/DMovie");
...
TEST(ToolBox, mainWindowEvent)
{
    ...
    urls << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/demo.mp4")\
         << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/bensound-sunny.mp3") \
         << QUrl::fromLocalFile("/data/source/lingmo-movie-reborn/movie/Hachiko.A.Dog's.Story.ass");
    mimeData.setUrls(urls);
