...
//    DApplication::loadDXcbPlugin();
    ...
    app->setApplicationName("lingmo-movie");
    app->setApplicationVersion(DMR_VERSION);
    app->setProductIcon(QIcon::fromTheme("lingmo-movie"));
    app->setWindowIcon(QIcon::fromTheme("lingmo-movie"));
    QString acknowledgementLink = "https://www.lingmo.org/acknowledgments/lingmo-movie";
    ...
    if (singleton && !runSingleInstance()) {
        qInfo() << "another lingmo movie instance has started";
        if (!toOpenFiles.isEmpty()) {
            QDBusInterface iface("com.lingmo.movie", "/", "com.lingmo.movie");
    ...
    if (CompositingManager::get().composited()) {
        ...
        QDBusConnection::sessionBus().registerService("com.lingmo.movie");
