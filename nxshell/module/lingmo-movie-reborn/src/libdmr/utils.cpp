...
void ShowInFileManager(const QString &path)
{
    ...
    if (fp->error() == QProcess::FailedToStart) {
        // Start ocean-file-manager failed, try nautilus
...
uint32_t InhibitStandby()
{
    QDBusInterface iface("org.freedesktop.ScreenSaver",
                         ...
                         "org.freedesktop.ScreenSaver");
    QDBusReply<uint32_t> reply = iface.call("Inhibit", "lingmo-movie", "playing in fullscreen");
