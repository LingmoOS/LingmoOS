...
CompositingManager::CompositingManager()
{
    ...
    _composited = false;
//    QGSettings gsettings("com.lingmo.lingmo-movie", "/com/lingmo/lingmo-movie/");
...
//    }
    ...
    //单元测试
#ifdef USE_TEST
    utils::getPlayProperty("/data/source/lingmo-movie-reborn/movie/play.conf", m_pMpvConfig);
    if (m_pMpvConfig->contains("vo")) {
        QString value = m_pMpvConfig->find("vo").value();
        if ("libmpv" == value) {
            _composited = true;//libmpv只能走opengl
...
//    // this window hioceann or invisible.
