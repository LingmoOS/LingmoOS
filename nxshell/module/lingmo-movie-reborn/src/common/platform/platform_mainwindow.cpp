...
#include "../accessibility/ac-lingmo-movie-define.h"
...
#define XLINGMO_BLUR_REGION "_NET_WM_LINGMO_BLUR_REGION"
#define XLINGMO_BLUR_REGION_ROUNDED "_NET_WM_LINGMO_BLUR_REGION_ROUNDED"
...
#else
        QString sName = "/data/source/lingmo-movie-reborn/movie/DMovie";
#endif
    ...
    : DMainWindow(nullptr)
{
    ...
    m_pWMDBus = new QDBusInterface("com.lingmo.WMSwitcher", "/com/lingmo/WMSwitcher", "com.lingmo.WMSwitcher", QDBusConnection::sessionBus());
    QDBusReply<QString> reply_string = m_pWMDBus->call("CurrentWM");
    m_bIsWM = reply_string.value().contains("lingmo wm");
    ...
    QDBusConnection::sessionBus().connect("com.lingmo.ocean.shutdownFront", "/com/lingmo/ocean/lockFront",
                                          "com.lingmo.ocean.lockFront", "Visible", this,
                               ...
                               QList<QVariant> args, bool bIsShortcut)
{
...
#else
        QString name("/data/source/lingmo-movie-reborn/movie");
#endif
...
#else
        QStringList filenames;
        filenames << QString("/data/source/lingmo-movie-reborn/movie/demo.mp4")\
                  << QString("/data/source/lingmo-movie-reborn/movie/bensound-sunny.mp3");
#endif
        if (filenames.size()) {
    ...
    case ActionFactory::ActionKind::MovieInfo: {
        if (m_pEngine->state() != PlayerEngine::CoreState::Idle) {
            //fix 107355
            //Add a mouse display to prevent the target is hioceann
...
#else
        filename = QStringList({"/data/source/lingmo-movie-reborn/Hachiko.A.Dog's.Story.ass"});
#endif
...
#undef POPUP_ADAPTER
    ...
    case ActionFactory::ActionKind::ViewShortcut: {
        ...
        m_pShortcutViewProcess->startDetached("lingmo-shortcut-viewer", shortcutString);
...
//        } else {
        // menus  are popped up
        // NOTE: menu keeps focus while hioceann, so focusWindow is not used
...
//        }
        ...
        //reset focus to mainWindow when the titlebar and toolbox is heoceann
...
{
    if (!bIsOnline) {
        this->sendMessage(QIcon(":/icons/lingmo/builtin/icons/ddc_warning_30px.svg"), QObject::tr("Network disconnected"));
    }
}
...
{
    if (msg.contains("lingmo metacity")) {
        m_bIsWM = false;
    } else {
        m_bIsWM = true;
    }
...
void Platform_MainWindow::paintEvent(QPaintEvent *pEvent)
{
    ...
    if (m_pEngine->state() == PlayerEngine::Idle) {
        QImage bg = QIcon::fromTheme("lingmo-movie").pixmap(130, 130).toImage();
