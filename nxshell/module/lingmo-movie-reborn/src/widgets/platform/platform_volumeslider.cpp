...
QString Platform_VolumeSlider::readSinkInputPath()
{
    ...
    QVariant v = ApplicationAdaptor::redDBusProperty("org.lingmo.daemon.Audio1", "/org/lingmo/daemon/Audio1",
                                                     "org.lingmo.daemon.Audio1", "SinkInputs");
    ...
    for (auto curPath : allSinkInputsList) {
        QVariant nameV = ApplicationAdaptor::redDBusProperty("org.lingmo.daemon.Audio1", curPath.path(),
                                                             "org.lingmo.daemon.Audio1.SinkInput", "Name");
        QString strMovie = QObject::tr("Movie");
        if (!nameV.isValid() || (!nameV.toString().contains(strMovie, Qt::CaseInsensitive) && !nameV.toString().contains("lingmo movie", Qt::CaseInsensitive)))
            continue;
...
void Platform_VolumeSlider::setMute(bool muted)
{
    ...
    if (!sinkInputPath.isEmpty()) {
        QDBusInterface ainterface("org.lingmo.daemon.Audio1", sinkInputPath,
                                  "org.lingmo.daemon.Audio1.SinkInput",
...
void Platform_VolumeSlider::refreshIcon()
{
    ...
    if (nValue >= 66)
        m_pBtnChangeMute->setIcon(QIcon::fromTheme("oceanui_volume"));
    else if (nValue >= 33)
        m_pBtnChangeMute->setIcon(QIcon::fromTheme("oceanui_volumemid"));
    else
        m_pBtnChangeMute->setIcon(QIcon::fromTheme("oceanui_volumelow"));

    if (m_bIsMute || nValue == 0)
        m_pBtnChangeMute->setIcon(QIcon::fromTheme("oceanui_mute"));
