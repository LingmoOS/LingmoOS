...
{
    setIcon(QIcon::fromTheme("oceanui_volume"));
...
{
    QIcon icon = QIcon::fromTheme("oceanui_volumedisable");
...
void VolumeButton::changeStyle()
{
    if (m_nVolume >= 66)      //根据音量大小改变图标，更直观的表现
        ...
        setIcon(QIcon::fromTheme("oceanui_volumemid"));
    else
        setIcon(QIcon::fromTheme("oceanui_volumelow"));

    if (m_bMute || m_nVolume == 0)
        setIcon(QIcon::fromTheme("oceanui_mute"));
    update();
}
