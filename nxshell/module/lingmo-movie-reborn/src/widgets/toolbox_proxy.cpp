...
public:
    ThumbnailPreview()
    {
        ...
        m_bIsWM = reply.value().contains("lingmo wm");
...
void ToolboxProxy::updateplaylisticon()
{
    if (m_pListBtn->isChecked() && DGuiApplicationHelper::LightType == DGuiApplicationHelper::instance()->themeType()) {
        m_pListBtn->setIcon(QIcon(":/icons/lingmo/builtin/light/checked/episodes_checked.svg"));
    } else {
        m_pListBtn->setIcon(QIcon::fromTheme("oceanui_episodes"));
    }

    if (m_pMircastBtn->isChecked() && DGuiApplicationHelper::LightType == DGuiApplicationHelper::instance()->themeType()) {
        m_pMircastBtn->setIcon(QIcon(":/icons/lingmo/builtin/light/checked/mircast_chenked.svg"));
    } else {
        m_pMircastBtn->setIcon(QIcon::fromTheme("oceanui_mircast"));
    }
}
...
//    bot_widget->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
    ...
    m_pPrevBtn->setIcon(QIcon::fromTheme("oceanui_last", QIcon(":/icons/lingmo/builtin/light/normal/last_normal.svg")));
    ...
    m_pPlayBtn->setIcon(QIcon::fromTheme("oceanui_play", QIcon(":/icons/lingmo/builtin/light/normal/play_normal.svg")));
    ...
    m_pNextBtn->setIcon(QIcon::fromTheme("oceanui_next", QIcon(":/icons/lingmo/builtin/light/normal/next_normal.svg")));
    ...
    m_pMircastBtn->setIcon(QIcon::fromTheme("oceanui_mircast", QIcon(":/resources/icons/mircast/mircast.svg")));
    ...
    m_pFullScreenBtn->setIcon(QIcon::fromTheme("oceanui_zoomin"));
    ...
    // these tooltips is not used due to lingmo ui design
...
void ToolboxProxy::updateFullState()
{
    ...
    if (isFullscreen) {
        m_pFullScreenBtn->setIcon(QIcon::fromTheme("oceanui_zoomout"));
        if (utils::check_wayland_env())
...
void ToolboxProxy::updatePlayState()
{
    if (((m_mircastWidget->getMircastState() != MircastWidget::Idel) && (m_mircastWidget->getMircastPlayState() == MircastWidget::Play))
        ...
        m_pPlayBtn->setIcon(QIcon::fromTheme("oceanui_suspend", QIcon(":/icons/lingmo/builtin/light/normal/suspend_normal.svg")));
        //lmh0910wayland下用这一套tooltip
        if (utils::check_wayland_env()) {
