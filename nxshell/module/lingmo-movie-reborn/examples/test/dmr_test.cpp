...
int main(int argc, char *argv[])
{
    ...
    bool is = false;
    //获取视频信息 demo
    dmr::MovieInfo mi = playModel->getMovieInfo(QUrl("file:///usr/share/ocean-introduction/demo.mp4"),&is);
    if (is) {
        qInfo() << "# 文件类型:"<< mi.fileType
                ...
                << "# 采样数:"<< mi.sampling << "hz";
        //获取预览图 demo
        QImage img = playModel->getMovieCover(QUrl("file:///usr/share/ocean-introduction/demo.mp4"));
        img.save(QStandardPaths::writableLocation(QStandardPaths::HomeLocation) + "/Desktop/test.png");

        QImage img1 = playModel->getMovieCover(QUrl("file:///usr/share/ocean-introduction/demo.mp4"));
