...
void loadTranslate()
{
    ...
    if (!tnappcoun.isEmpty()) {
        tranlateUrl = QString(":/translations/lingmo-boot-maker_%1_%2.qm").arg(tnapplang).arg(tnappcoun);
    }

    if (!QFile::exists(tranlateUrl)) {
        tranlateUrl = QString(":/translations/lingmo-boot-maker_%1.qm").arg(tnapplang);
    }


    if (!QFile::exists(tranlateUrl)) {
        tranlateUrl = ":/translations/lingmo-boot-maker.qm";
    }
