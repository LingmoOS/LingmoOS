...
void StartManager::slotCloseWindow()
{
    ...
    if (m_windows.isEmpty()) {
        ...
        // 导致被附加到上一文本编辑器进程
        QDBusConnection::sessionBus().unregisterService("com.lingmo.Editor");
...
void StartManager::popupExistTabs(FileTabInfo info)
{
    ...
    int indexid = 0;
    uint winid = 0;
    QDBusInterface dock("com.lingmo.ocean.daemon.Dock",
    ...
    for (auto name : rep.value()) {
        if (name == "lingmo-editor") {
            indexid = rep.value().indexOf(name);
        }
    }

    m_pDock.reset(new Dock("com.lingmo.ocean.daemon.Dock",
                           "/com/lingmo/ocean/daemon/Dock",
                           QDBusConnection::sessionBus(),
                           this
                          )
    ...
    m_pEntry.reset(new Entry("com.lingmo.ocean.daemon.Dock",
                             list[indexid].path(),
                             QDBusConnection::sessionBus(),
                             this));
    ...
    QDBusMessage active = QDBusMessage::createMethodCall("com.lingmo.ocean.daemon.Dock",
                                                         "/com/lingmo/ocean/daemon/Dock",
                                                         "com.lingmo.ocean.daemon.Dock",
                                                         "ActivateWindow");
    active << winid;
