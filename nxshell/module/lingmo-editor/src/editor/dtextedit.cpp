...
void TextEdit::checkBookmarkLineMove(int from, int charsRemoved, int charsAoceand)
{
    Q_UNUSED(charsRemoved);
    Q_UNUSED(charsAoceand);
...
void TextEdit::updateMark(int from, int charsRemoved, int charsAoceand)
{
    //只读模式下实现禁止语音输入的效果
    if (m_readOnlyMode) {
        //undo();
        ...
        for (int i = 0; i < wordMarkSelections.count(); i++) {
            ...
            if (nCurrentPos > nStartPos && nCurrentPos < nEndPos) {
                ...
                if (m_bIsInputMethod) {
                    ...
                    selection.cursor.setPosition(nStartPos, QTextCursor::MoveAnchor);
                    selection.cursor.setPosition(from + charsAoceand, QTextCursor::KeepAnchor);
                }
 ...
 * @param charsAoceand    插入的字符数
 */
void TextEdit::onTextContentChanged(int from, int charsRemoved, int charsAoceand)
{
    Q_UNUSED(charsRemoved)
    ...
    if (m_MidButtonPatse) {
        ...
        QTextCursor cursor = textCursor();
        cursor.setPosition(from);
        cursor.setPosition(from + charsAoceand, QTextCursor::KeepAnchor);
        // 取得已插入的文本信息
