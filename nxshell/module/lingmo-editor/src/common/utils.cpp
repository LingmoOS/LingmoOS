 ...
 *  [debian]    /home/user/.local/share/lingmo/lingmo-editor/
 *  [linglong]  /home/user/.linglong/org.lingmo.editor/share/lingmo/lingmo-editor/
 */
QString Utils::localDataPath()
{
    auto dataPaths = Utils::cleanPath(QStandardPaths::standardLocations(QStandardPaths::DataLocation));
    return dataPaths.isEmpty() ? QDir::homePath() + "/.local/share/lingmo/lingmo-editor/"
                               : dataPaths.first();
}
...
bool Utils::activeWindowFromDock(quintptr winId)
{
    bool bRet = false;
    // 优先采用V23接口
    QDBusInterface dockDbusInterfaceV23("org.lingmo.ocean.daemon.Dock1",
    ...
    if (dockDbusInterfaceV23.isValid()) {
        ...
        if (!reply.isValid()) {
            qDebug() << "call v23 org.lingmo.ocean.daemon.Dock1 failed" << reply.error();
        } else {
            return true;
        }
    }

    QDBusInterface dockDbusInterfaceV20("com.lingmo.ocean.daemon.Dock",
                                        "/com/lingmo/ocean/daemon/Dock",
                                        "com.lingmo.ocean.daemon.Dock");
    if (dockDbusInterfaceV20.isValid() && !bRet) {
        QDBusReply<void> reply = dockDbusInterfaceV20.call("ActivateWindow", winId);
        if (!reply.isValid()) {
            qDebug() << "call v20 com.lingmo.ocean.daemon.Dock failed" << reply.error();
            bRet = false;
        } else {
            return true;
        }
...
QString Utils::getSystemLan()
{
    if (!m_systemLanguage.isEmpty()) {
        ...
        case V23:
            ...
            break;
        default: {
            QDBusInterface ie("com.lingmo.daemon.LangSelector",
