...
/**
   ...
   @details 取得DConfig配置，目前主要用于配置是否提高GB18030识别率。
    配置文件路径为 /usr/share/dsg/configs/org.lingmo.editor/org.lingmo.editor.json
 */
...
{
#ifdef DTKCORE_CLASS_DConfigFile
    dconfig = DConfig::create("org.lingmo.editor", "org.lingmo.editor");
    if (dconfig->isValid()) {
        // 默认提高GB18030识别率
        ...
        qWarning() << qPrintable("org.lingmo.editor DConfig not valid!");
    }
#else
