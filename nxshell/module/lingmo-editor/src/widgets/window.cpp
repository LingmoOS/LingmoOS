      ...
      m_themePath(Settings::instance()->settings->option("advance.editor.theme")->value().toString())
{
    ...
    m_rootSaveDBus = new DBusDaemon::dbus("com.lingmo.editor.daemon", "/", QDBusConnection::systemBus(), this);
    ...
    setWindowIcon(QIcon::fromTheme("lingmo-editor"));
...
void Window::initTitlebar()
{
    ...
    titlebar()->setIcon(QIcon::fromTheme("lingmo-editor"));
...
void Window::addTabWithWrapper(EditWrapper *wrapper, const QString &filepath, const QString &qstrTruePath, const QString &tabName, int index)
{
    ...
    case Utils::V23:
        dbus.systemBus().disconnect("org.lingmo.ocean.Gesture1",
                                    "/org/lingmo/ocean/Gesture1", "org.lingmo.ocean.Gesture1",
                                    "Event",
                                    wrapper->textEditor(), SLOT(fingerZoom(QString, QString, int)));
        break;
    default:
        dbus.systemBus().disconnect("com.lingmo.daemon.Gesture",
                                    "/com/lingmo/daemon/Gesture", "com.lingmo.daemon.Gesture",
                                    "Event",
                                    wrapper->textEditor(), SLOT(fingerZoom(QString, QString, int)));
        break;
    }
    ...
    switch (Utils::getSystemVersion()) {
    case Utils::V23:
        dbus.systemBus().connect("org.lingmo.ocean.Gesture1",
                                 "/org/lingmo/ocean/Gesture1", "org.lingmo.ocean.Gesture1",
                                 "Event",
                                 wrapper->textEditor(), SLOT(fingerZoom(QString, QString, int)));
        break;
    default:
        dbus.systemBus().connect("com.lingmo.daemon.Gesture",
                                 "/com/lingmo/daemon/Gesture", "com.lingmo.daemon.Gesture",
                                 "Event",
                                 wrapper->textEditor(), SLOT(fingerZoom(QString, QString, int)));
        break;
...
void Window::displayShortcuts()
{
    ...
    m_shortcutViewProcess->startDetached("lingmo-shortcut-viewer", shortcutString);
...
DDialog *Window::createDialog(const QString &title, const QString &content)
{
    ...
    dialog->setIcon(QIcon::fromTheme("lingmo-editor"));
...
{
    if (path == LINGMO_THEME) {
        if (DGuiApplicationHelper::instance()->themeType() == DGuiApplicationHelper::LightType) {
            return;
        } else {
            DGuiApplicationHelper::instance()->setPaletteType(DGuiApplicationHelper::LightType);
        }
    } else if (path == LINGMO_DARK_THEME) {
...
//        if (nullptr != m_shortcutViewProcess) {
//            int count = Utils::getProcessCountByName("lingmo-shortcut-viewer");
//            if (count > 0) {
//                Utils::killProcessByName("lingmo-shortcut-viewer");
//            }
//            delete (m_shortcutViewProcess);
//            m_shortcutViewProcess = nullptr;
//        }
