...
# Copyright (C) 2014 Lingmo Technology Co., Ltd.
#
...
def get_all_icons():
    ...
    try:
        metadata = urllib2.urlopen("http://api.lastore.lingmo.org/metadata")
...
def update_record(package_id, package_icon):
    ...
    file_path = os.path.join(dirname, "lingmo/apps/48/", "%s.svg") % package_id
    try:
...
if __name__ == "__main__":
    ...
    for package_id, package_icon in icons.items():
        if (package_id.encode("utf-8").startswith("apps")) and \
            ...
            print "skip apps that are not maintained by lingmo: %s" % package_id