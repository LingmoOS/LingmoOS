    ...
    : PageModule("insider", tr("Technology Preview"), parent)
{
    ...
    DStandardItem *dim = new DStandardItem(tr("Technology preview Input Method (lingmo-im)"));
    dim->setData("lingmo-im", Dtk::UserRole);
...
void InsiderModule::installInputMethod(const QString &packageName) {
    bool isNew = packageName == "lingmo-im";
...
void InsiderModule::installOCEANShell() {
    PKUtils::resolve("ocean-shell").then([this](const PKUtils::PkPackages packages) {
        if (packages.isEmpty()) return;
...
void InsiderModule::checkEnabledInputMethod() {
    ...
    for (int i = 0; i < availableIMCount; i++) {
        ...
        bool isChecked = isDim == (item->data(Dtk::UserRole).toString() == "lingmo-im");
        item->setCheckState(isChecked ? Qt::Checked : Qt::Unchecked);
    }
