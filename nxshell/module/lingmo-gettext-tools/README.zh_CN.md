...
有关如何使用可参考项目 [lingmo-feedback](https://github.com/lingmoos/lingmo-feedback)
...
### 子命令 init
```
lingmo-desktop-ts-convert init $desktopFile $outputTsDir
```
...
### 子命令 desktop2ts
```
lingmo-desktop-ts-convert desktop2ts $desktopFile $outputTsDir
```
...
### 子命令 ts2desktop
```
lingmo-desktop-ts-convert ts2desktop $desktopFile $tsDir $outputDesktopFile
```
...
6. 用 ci 的 [sync-transifex](https://ci.lingmo.io/view/tools/job/sync-transifex/) 上传翻译源文件；
...
## 开源许可证
lingmo-gettext-tools 在 [GPL-2.0-or-later](LICENSE) 下发布。
