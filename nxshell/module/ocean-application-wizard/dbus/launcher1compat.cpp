...
void sendNotification(const QString & displayName, bool successed)
{
    ...
    notifySender = notifySender.appName("lingmo-app-store").appIcon("application-default-icon").timeOut(5000);
    notifySender.call();
}
...
void postUninstallCleanUp(const QString & desktopId)
{
    ...
    // TODO: the legacy ocean-application-manager didn't do this
...
void Launcher1Compat::RequestUninstall(const QString & desktop, bool unused)
{
    ...
    qDebug() << cmdline;
#ifndef QT_DEBUG
    if (!QString(cmdline).endsWith(BINDIR_PREFIX + QStringLiteral("ocean-shell")) &&
        !QString(cmdline).endsWith(BINDIR_PREFIX + QStringLiteral("ocean-launchpad"))) {
    ...
    if (desktopFilePath.contains("/persistent/linglong") || desktopFilePath.contains("/var/lib/linglong")) {
        ...
        if (!succ) {
            ...
            //        here is the logic from the legacy ocean-application-manager which is INCORRECT in that case.
        ...
        // Uninstall regular package via PackageKit or lingmo-store
