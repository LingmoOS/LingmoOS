...
void CDayMonthView::setTheMe(int type)
{
    ...
    if (type == 0 || type == 1) {
        ...
        QFont hLabelF;
        hLabelF.setPixelSize(OCEANCalendar::FontSizeFourteen);
...
void CDayMonthView::initUI()
{
    ...
    m_today->setFixedSize(80, OCEANDayCalendar::D_MLabelHeight);
    QFont todayfont;
    todayfont.setPixelSize(OCEANCalendar::FontSizeFourteen);
    ...
    m_currentMouth->setFixedSize(74, OCEANDayCalendar::D_MLabelHeight);
    QFont mlabelF;
    mlabelF.setWeight(QFont::Medium);
    mlabelF.setPixelSize(OCEANCalendar::FontSizeTwentyfour);
    ...
    m_currentYear->setFixedHeight(OCEANDayCalendar::DWLabelHeight);
    ...
    m_currentLuna->setFixedHeight(OCEANDayCalendar::DHuangLiInfoLabelHeight);
    m_currentLuna->setTextAlign(Qt::AlignCenter);
    QFont hLabelF;
    hLabelF.setPixelSize(OCEANCalendar::FontSizeTwelve);
    ...
    m_yiLabel->setTextInfo(QColor("#7B7B7B "), hLabelF);
    m_yiLabel->setMinimumHeight(OCEANDayCalendar::DHuangLiLabelHeight);
    m_yiLabel->setMaximumHeight(OCEANDayCalendar::DHuangLiLabelMaxHeight);
    ...
    m_jiLabel->setTextInfo(QColor("#7B7B7B "), hLabelF);
    m_jiLabel->setMinimumHeight(OCEANDayCalendar::DHuangLiLabelHeight);
    m_jiLabel->setMaximumHeight(OCEANDayCalendar::DHuangLiLabelMaxHeight);
    ...
    , m_isFocus(false)
{
    ...
    m_dayNumFont.setPixelSize(OCEANCalendar::FontSizeTwelve);
    for (int r = 0; r < 6; ++r) {
        for (int c = 0; c < 7; ++c) {
...
void CDayMonthWidget::paintCell(QWidget *cell)
{
    ...
    if (m_vlineflag.count() == OCEANDayCalendar::PainterCellNum) {
        if (m_vlineflag[pos]) {
            if (m_selectDate.month() == getCellDate(pos).month()) {
                painter.save();
                QPen pen;
...
bool CDayMonthWidget::eventFilter(QObject *o, QEvent *e)
{
    ...
    if (cell && m_cellList.contains(cell)) {
        ...
        //如果需要显示时间小于1900年则退出
        if (date.year() < OCEANCalendar::QueryEarliestYear)
            return false;
                ...
                if (m_dayCreateState != 0) {
                    ...
                    if (widget != nullptr) {
                        ...
                        if (rect.contains(globalPoint)) {
                            ...
                            setCursor(Qt::ForbioceannCursor);
                            m_dayCreateState = 2;
                        }
                    }
                }
