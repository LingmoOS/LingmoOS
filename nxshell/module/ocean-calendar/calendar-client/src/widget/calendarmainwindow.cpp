    ...
    , m_defaultIndex(index)
{
    ...
    //注册光标位置
    CalendarGlobalEnv::getGlobalEnv()->registerKey(OCEANCalendar::CursorPointKey, QPoint());
    //保存主窗口指针
...
void Calendarmainwindow::viewWindow(int type, const bool showAnimation)
{
    ...
    switch (type) {
    case OCEANCalendar::CalendarYearWindow: {
        //更新界面显示
        m_yearwindow->updateData();
    } break;
    case OCEANCalendar::CalendarMonthWindow: {
    } break;
    case OCEANCalendar::CalendarWeekWindow: {
    } break;
    case OCEANCalendar::CalendarDayWindow: {
        m_DayWindow->setTime();
        m_searchflag = true;
    } break;
    }
...
void Calendarmainwindow::slotOpenSchedule(QString job)
{
    ...
    //设置被选中
    m_buttonBox->button(OCEANCalendar::CalendarDayWindow)->setChecked(true);
    //设置获取焦点
    m_buttonBox->button(OCEANCalendar::CalendarDayWindow)->setFocus();
    //切换到日视图
    m_stackWidget->setCurrentIndex(OCEANCalendar::CalendarDayWindow);
    //设置选择时间
...
void Calendarmainwindow::createview()
{
    ...
    //如果为日视图则设置非全天时间定位
    if (m_defaultIndex == OCEANCalendar::CalendarDayWindow) {
        m_DayWindow->setTime();
    }
}
...
void Calendarmainwindow::slotSwitchView(const int viewIndex)
{
    ...
    switch (viewIndex) {
    case 0: {
        if (m_priindex == OCEANCalendar::CalendarYearWindow)
            m_priindex = OCEANCalendar::CalendarDayWindow;
        viewWindow(m_priindex);
        break;
    }
    case 1: {
        viewWindow(OCEANCalendar::CalendarMonthWindow);
        break;
    }
    case 2: {
        viewWindow(OCEANCalendar::CalendarWeekWindow);
        break;
    }
    case 3: {
        viewWindow(OCEANCalendar::CalendarDayWindow);
        break;
    }
    default:
        break;
...
void Calendarmainwindow::slotShowPrivacy()
{
    ...
    if (country == QLocale::China) {
        ...
        if (isCommunityEdition) {
            url = "https://www.lingmo.org/en/agreement/privacy/";
        } else {
...
void Calendarmainwindow::slotShowSyncToast(int syncNum)
{
    ...
    if (preSyncNum != syncNum && syncNum == -1) {
        ...
        //同步中
        removeSyncToast();
        DMessageManager::instance()->sendMessage(this->window(), QIcon::fromTheme(":/icons/lingmo/builtin/icons/ocean_calendar_spinner_32px.svg"), tr("Syncing..."));
        return;
    }
    //
    if (preSyncNum == -1 && syncNum != -1) {
        ...
        case 0: {
            //同步成功
            removeSyncToast();
            DMessageManager::instance()->sendMessage(this->window(), QIcon::fromTheme(":/icons/lingmo/builtin/icons/ocean_calendar_success_200px.png"), tr("Sync successful"));
            break;
        }
        case 1:
        case 2:
        case 3: {
            //同步失败
            removeSyncToast();
            DMessageManager::instance()->sendMessage(this->window(), QIcon::fromTheme(":/icons/lingmo/builtin/icons/ocean_calendar_fail_200px.png"), tr("Sync failed, please try later"));
            break;
        }
        default:
            break;
