...
CWeekNumWidget::CWeekNumWidget(const GetWeekNumOfYear &getWeekNumOfYear, QWidget *parent)
    ...
    , m_isFocus(false)
{
    m_dayNumFont.setPixelSize(OCEANCalendar::FontSizeSixteen);
    ...
    hBoxLayout->setMargin(0);
    //显示周数的widget
    for (int c = 0; c != OCEANWeekCalendar::NumWeeksDisplayed; ++c) {
...
void CWeekNumWidget::setTheMe(int type)
{
    if (type == 0 || type == 1) {
        ...
        m_backgrounoceanfaultColor = Qt::white;
        ...
        m_backgrounoceanfaultColor = "#FFFFFF";
        m_backgrounoceanfaultColor.setAlphaF(0.05);
...
void CWeekNumWidget::resizeEvent(QResizeEvent *event)
{
    ...
    //获取当前cell的宽度
    int w = _allCellWidth / OCEANWeekCalendar::NumWeeksDisplayed;
    //最小显示的宽度
    ...
    if (w < _minWidget) {
        //计算前后需要隐藏的个数
        int t_num = qRound((_minWidget * OCEANWeekCalendar::NumWeeksDisplayed - _allCellWidth) / _minWidget / 2.0);
    ...
    //设置是否显示
    for (int i = 0; i < OCEANWeekCalendar::NumWeeksDisplayed; i++) {
...
void CWeekNumWidget::setSelectedCell(int index)
{
    ...
    if (m_days[index].year() < OCEANCalendar::QueryEarliestYear && dayNum != "1")
        return;
...
{
    for (int i = 0 ; i < OCEANWeekCalendar::NumWeeksDisplayed; ++i) {
        m_days[i]  = m_selectDate.addDays((i - 4) * OCEANWeekCalendar::AFewDaysofWeek);
