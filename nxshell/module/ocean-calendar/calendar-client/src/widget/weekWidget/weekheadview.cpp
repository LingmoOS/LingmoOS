    ...
    , m_touchGesture(this)
{
    ...
    m_monthFont.setPixelSize(OCEANCalendar::FontSizeTwenty);
...
{
    if (vDays.count() != OCEANWeekCalendar::AFewDaysofWeek)
        return;
    m_days = vDays;
    QLocale locale;
...
void CWeekHeadView::paintCell(QWidget *cell)
{
    ...
    //根据周几设置不一样的背景色
    if (d == OCEANWeekCalendar::FirstDayOfWeekend || d == OCEANWeekCalendar::AFewDaysofWeek) {
    ...
    QString dayWeek = locale.dayName(d ? d : OCEANWeekCalendar::AFewDaysofWeek, QLocale::ShortFormat);
