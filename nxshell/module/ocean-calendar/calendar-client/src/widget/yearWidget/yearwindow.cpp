...
{
    Q_UNUSED(event);
    m_topWidget->setGeometry(0, 0, this->width(), OCEANMonthCalendar::M_YTopHeight);
}
...
void CYearWindow::initUI()
{
    ...
    m_prevButton->setFixedWidth(OCEANYearCalendar::Y_MLabelHeight);
    m_prevButton->setFixedHeight(OCEANYearCalendar::Y_MLabelHeight);
    ...
    m_nextButton->setFixedWidth(OCEANYearCalendar::Y_MLabelHeight);
    m_nextButton->setFixedHeight(OCEANYearCalendar::Y_MLabelHeight);

    m_yearLabel = new QLabel(this);
    m_yearLabel->setFixedHeight(OCEANYearCalendar::Y_YLabelHeight);

    QFont t_labelF;
    t_labelF.setWeight(QFont::Medium);
    t_labelF.setPixelSize(OCEANCalendar::FontSizeTwentyfour);
    ...
    m_yearLunarLabel->setFixedSize(OCEANMonthCalendar::M_YLunaLabelWindth, OCEANMonthCalendar::M_YLunaLabelHeight);

    QFont yLabelF;
    yLabelF.setWeight(QFont::Medium);
    yLabelF.setPixelSize(OCEANCalendar::FontSizeFourteen);
    ...
    m_yearLunarDayLabel->setFixedSize(108, OCEANMonthCalendar::M_YLunaLabelHeight);
    ...
    m_todayFrame->setFixedHeight(OCEANYearCalendar::Y_MLabelHeight);
...
void CYearWindow::slotMousePress(const QDate &selectDate, const int pressType)
{
    ...
    case 0: {
        ...
        //使用设置的显示坐标
        QVariant variant;
        CalendarGlobalEnv::getGlobalEnv()->getValueByKey(OCEANCalendar::CursorPointKey, variant);
    ...
    : QWidget(parent)
{
    ...
    m_topWidget->setLayout(yeartitleLayout);
    m_topWidget->setFixedHeight(OCEANMonthCalendar::M_YTopHeight);
    QVBoxLayout *hhLayout = new QVBoxLayout;
    hhLayout->setMargin(0);
    hhLayout->setSpacing(0);
...
void YearFrame::setShowDate(const QDate &selectDate)
{
    ...
    for (int i = 0; i < OCEANYearCalendar::FrameSizeOfEveryYear; i++) {
