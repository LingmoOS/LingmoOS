...
void CMonthScheduleView::updateData()
{
    ...
    //保护数据防止越界
    if (m_data.count() != OCEANMonthCalendar::ItemSizeOfMonthDay || m_cNum < 1)
        return;
    //开始结束时间
...
{
    int brow = static_cast<int>((m_beginDate.daysTo(bgeindate)) / OCEANMonthCalendar::AFewDaysOfWeek);
    int bcol = (m_beginDate.daysTo(bgeindate)) % OCEANMonthCalendar::AFewDaysOfWeek;
    int ecol = (m_beginDate.daysTo(enddate)) % OCEANMonthCalendar::AFewDaysOfWeek;
