...
{
    //设置字体
    m_dayNumFont.setPixelSize(OCEANCalendar::FontSizeTwentyfour);
    m_dayNumFont.setWeight(QFont::Light);

    m_LunerFont.setPixelSize(OCEANCalendar::FontSizeTwelve);
...
void CMonthDayItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (m_Date == QDate::currentDate()) {
        ...
        tFont.setPixelSize(OCEANCalendar::FontSizeTwenty);
    ...
    if (m_LunarVisible) {
        ...
        if (filleRectX > hh) {
            ...
            switch (m_DayStatus) {
            case H_WORK: {
                QPixmap pixmap = DIcon::loadNxPixmap(":/icons/lingmo/builtin/icons/ocean_calendar_ban_32px.svg");
                painter->drawPixmap(fillRectT.toRect(), pixmap);
            } break;
            case H_REST: {
                QPixmap pixmap = DIcon::loadNxPixmap(":/icons/lingmo/builtin/icons/ocean_calendar_xiu.svg");
                painter->drawPixmap(fillRectT.toRect(), pixmap);
            } break;
            default:
                break;
