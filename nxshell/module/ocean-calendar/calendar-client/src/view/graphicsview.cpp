...
DSchedule::Ptr CGraphicsView::getScheduleInfo(const QDateTime &beginDate, const QDateTime &endDate)
{
    ...
    if (beginDate.secsTo(endDate) > 0) {
        ...
        if (beginDate.secsTo(endDate) < OCEANCalendar::ThirtyMinutesWithSec) {
            info->setDtEnd(beginDate.addSecs(OCEANCalendar::ThirtyMinutesWithSec));
        } else {
            info->setDtEnd(endDate);
        }
    } else {
        if (endDate.secsTo(beginDate) < OCEANCalendar::ThirtyMinutesWithSec) {
            info->setDtStart(beginDate.addSecs(-OCEANCalendar::ThirtyMinutesWithSec));
        } else {
...
{
    return moveDateTime.secsTo(m_InfoEndTime) < OCEANCalendar::ThirtyMinutesWithSec ? m_InfoEndTime.addSecs(-OCEANCalendar::ThirtyMinutesWithSec) : moveDateTime;
}

QDateTime CGraphicsView::getDragScheduleInfoEndTime(const QDateTime &moveDateTime)
{
    return m_InfoBeginTime.secsTo(moveDateTime) < OCEANCalendar::ThirtyMinutesWithSec ? m_InfoBeginTime.addSecs(OCEANCalendar::ThirtyMinutesWithSec) : moveDateTime;
}
