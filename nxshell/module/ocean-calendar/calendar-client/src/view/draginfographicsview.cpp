...
void DragInfoGraphicsView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_DragStatus == NONE) {
        ...
        switch (m_DragStatus) {
        case IsCreate:
            if (gDate.date().year() >= OCEANCalendar::QueryEarliestYear && gDate.date().year() <= OCEANCalendar::QueryLatestYear) {
                if (m_PressDate.date().year() >= OCEANCalendar::QueryEarliestYear && m_PressDate.date().year() <= OCEANCalendar::QueryLatestYear) {
...
void DragInfoGraphicsView::dropEvent(QDropEvent *event)
{
    if (event->mimeData()->hasFormat("Info")) {
        if (event->source() != this || m_MoveDate != m_PressDate) {
            ...
            if (startDate.date().year() >= OCEANCalendar::QueryEarliestYear && endDate.date().year() <= OCEANCalendar::QueryLatestYear) {
...
void DragInfoGraphicsView::DragPressEvent(const QPoint &pos, DragInfoItem *item)
{
    ...
    CalendarGlobalEnv::getGlobalEnv()->reviseValue(OCEANCalendar::CursorPointKey, mapToGlobal(pos));
...
void DragInfoGraphicsView::mousePress(const QPoint &point)
{
    ...
    if (infoitem) {
        ...
        if (isCanDragge(infoitem->getData())) {
            ...
            CalendarGlobalEnv::getGlobalEnv()->reviseValue(OCEANCalendar::CursorPointKey, mapToGlobal(point));
