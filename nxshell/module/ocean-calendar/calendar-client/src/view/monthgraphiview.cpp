    ...
    : DragInfoGraphicsView(parent)
{
    ...
    for (int i = 0; i < OCEANMonthCalendar::ItemSizeOfMonthDay; ++i) {
...
void CMonthGraphicsview::updateSize()
{
    ...
    qreal w = m_Scene->width() / OCEANMonthCalendar::AFewDaysOfWeek;
    qreal h = m_Scene->height() / OCEANMonthCalendar::LinesNumOfMonth;
    QRectF rect ;
    int w_offset = 0;
    int h_offset = 0;

    for (int i = 0 ; i < m_DayItem.size(); ++i) {
        h_offset = i / OCEANMonthCalendar::AFewDaysOfWeek;
        w_offset = i % OCEANMonthCalendar::AFewDaysOfWeek;
...
QDateTime CMonthGraphicsview::getPosDate(const QPoint &p)
{
    ...
    int xoffset = qFloor(x / (rect.width() / OCEANMonthCalendar::AFewDaysOfWeek)) % OCEANMonthCalendar::AFewDaysOfWeek;
    int yoffset = qFloor(y / (rect.height() / OCEANMonthCalendar::LinesNumOfMonth)) % OCEANMonthCalendar::LinesNumOfMonth;
...
void CMonthGraphicsview::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    if (item != nullptr) {
        //双击切换视图
        if (item->getDate().year() > OCEANCalendar::QueryEarliestYear) {
    ...
    if (Dayitem != nullptr) {
        ...
        if (point.y() < 38) {
            //双击切换视图
            if (Dayitem->getDate().year() > OCEANCalendar::QueryEarliestYear) {
                emit signalsViewSelectDate(Dayitem->getDate());
            }
        } else {
            //双击新建日程
            if (Dayitem->getDate().year() >= OCEANCalendar::QueryEarliestYear) {
                if (Dayitem->getDate().year() <= OCEANCalendar::QueryLatestYear) {
