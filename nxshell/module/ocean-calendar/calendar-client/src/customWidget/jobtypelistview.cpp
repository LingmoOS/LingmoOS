        ...
        if (indexCurrentHover != m_iIndexCurrentHover) {
            ...
            if (itemJobType->actionList(Qt::Edge::RightEdge).size() > 0) {
                ...
                if (DStyle *ds = qobject_cast<DStyle *>(style())) {
                    ...
                    actionEdit->setIcon(DIcon::loadNxPixmap(":/icons/lingmo/builtin/icons/ocean_calendar_edit_32px.svg"));
                    ...
                    actionDelete->setIcon(DIcon::loadNxPixmap(":/icons/lingmo/builtin/icons/ocean_calendar_delete_32px.svg"));
...
void JobTypeListView::slotImportScheduleType()
{
    ...
    auto typeID = cal->nonKDECustomProperty("X-OCEAN-CALENDAR-TYPE-ID");
    auto typeName = cal->nonKDECustomProperty("X-OCEAN-CALENDAR-TYPE-NAME");
    auto typeColor = cal->nonKDECustomProperty("X-OCEAN-CALENDAR-TYPE-COLOR");
    // 如果没有颜色，使用随机颜色
