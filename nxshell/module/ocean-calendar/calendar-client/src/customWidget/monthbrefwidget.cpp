    ...
    , m_globalData(new GlobalData)
{
    ...
    for (int i = 0; i < OCEANYearCalendar::RectSizeOfEveryMonth; ++i) {
...
void CMonthDayRectWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton && isVisible()) {
        m_pressed = true;
        //保存日程悬浮框显示位置
        CalendarGlobalEnv::getGlobalEnv()->reviseValue(OCEANCalendar::CursorPointKey, mapToGlobal(QPoint(width()/2,height()/2)));
