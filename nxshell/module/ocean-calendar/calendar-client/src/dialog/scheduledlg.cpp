    ...
    , m_createAllDay(isAllDay)
{
    ...
    if (type == 1) {
        ...
        int minnutes = QTime::currentTime().minute() % OCEANCalendar::QuarterOfAnhourWithMinute;

        if (minnutes != 0) {
            minnutes = QTime::currentTime().minute() / OCEANCalendar::QuarterOfAnhourWithMinute *
                       OCEANCalendar::QuarterOfAnhourWithMinute + OCEANCalendar::QuarterOfAnhourWithMinute;
        }
...
void CScheduleDlg::setDate(const QDateTime &date)
{
    ...
    int minnutes = 0;
    //取15的整数倍
    minnutes = date.time().minute() / OCEANCalendar::QuarterOfAnhourWithMinute *
               OCEANCalendar::QuarterOfAnhourWithMinute;
    //如果有余数则添加15分钟
    minnutes = minnutes + (date.time().minute() % OCEANCalendar::QuarterOfAnhourWithMinute == 0 ? 0 : 15);
    if (minnutes == 60) {
        if (hours + 1 == 24) {
...
bool CScheduleDlg::createSchedule(const QString &scheduleTypeId)
{
    ...
    if (beginDateTime > endDateTime) {
        ...
        prompt->setIcon(QIcon(":/icons/lingmo/builtin/icons/ocean_calendar_warning.svg"));
...
void CScheduleDlg::changeEvent(QEvent *event)
{
    ...
    QString str_typelabel = fontWidth_typeLabel.elidedText(tr("Type:"), Qt::ElideRight,
                                                           OCEANCalendar::NewScheduleLabelWidth);
...
void CScheduleDlg::initUI()
{
    ...
    //时间
    {
        ...
        tLabel->setFixedSize(OCEANCalendar::NewScheduleLabelWidth, 25);
...
{
    m_beginDateEdit->setMinimumDate(QDate(OCEANCalendar::QueryEarliestYear, 1, 1)); // 0天
    m_beginDateEdit->setMaximumDate(QDate(OCEANCalendar::QueryLatestYear, 12, 31));
    m_endDateEdit->setMinimumDate(QDate(OCEANCalendar::QueryEarliestYear, 1, 1)); // 0天
    m_endDateEdit->setMaximumDate(QDate(OCEANCalendar::QueryLatestYear, 12, 31));
    return;
}
