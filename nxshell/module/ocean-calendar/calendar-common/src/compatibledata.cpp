...
QJsonObject JobToObject(const DSchedule::Ptr &job)
{
    ...
    obj.insert("Type", OCEAN_Calendar::getOldTypeIDByNewTypeID(job->scheduleTypeID()));
    ...
    obj.insert("Remind", OCEAN_Calendar::getOldRemindByAlarm(job->getAlarmType()));
...
QString OCEAN_Calendar::getNewTypeIDByOldTypeID(int oldTypeID)
{
...
int OCEAN_Calendar::getOldTypeIDByNewTypeID(const QString &newTypeID)
{
    int oldTypeID = 0;
...
QString OCEAN_Calendar::getExternalSchedule(const DSchedule::Map &scheduleMap)
{
    QString strJson;
    QJsonDocument doc;
    QJsonArray jsonarr;
...
void OCEAN_Calendar::setAlarmByOldRemind(const DSchedule::Ptr &schedule, const QString &remind)
{
    //提醒规则
...
QString OCEAN_Calendar::getOldRemindByAlarm(const DSchedule::AlarmType &alarmType)
{
    QString _resultStr {""};
    switch (alarmType) {
...
DSchedule::Ptr OCEAN_Calendar::getScheduleByExported(const QString &scheduleStr)
{
    ...
    if (rootObj.contains("Remind")) {
        OCEAN_Calendar::setAlarmByOldRemind(schedule, rootObj.value("Remind").toString());
    }
    //日程标题
    ...
    if (rootObj.contains("Type")) {
        schedule->setScheduleTypeID(OCEAN_Calendar::getNewTypeIDByOldTypeID(rootObj.value("Type").toInt()));
    }
    //日程开始时间
    ...
    if (rootObj.contains("RRule")) {
        OCEAN_Calendar::setRRuleByOldRRule(schedule, rootObj.value("RRule").toString());
    }
    //重复日程忽略日期集
    if (rootObj.contains("Ignore")) {
        OCEAN_Calendar::setExDate(schedule, rootObj.value("Ignore").toArray());
    }
    return schedule;
}

void OCEAN_Calendar::setRRuleByOldRRule(const DSchedule::Ptr &schedule, const QString &rrule)
{
    //重复规则
...
void OCEAN_Calendar::setExDate(const DSchedule::Ptr &schedule, const QJsonArray &ignore)
{
    KCalendarCore::Recurrence *recurrence = schedule->recurrence();
