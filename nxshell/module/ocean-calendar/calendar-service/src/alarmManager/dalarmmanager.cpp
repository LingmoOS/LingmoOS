...
DAlarmManager::DAlarmManager(QObject *parent)
    : QObject(parent)
{
    m_dbusnotify = new DBusNotify("org.lingmo.ocean.Notification1",
    ...
    if (!m_dbusnotify->isValid()) {
        delete m_dbusnotify;
        m_dbusnotify = new DBusNotify("com.lingmo.ocean.Notification",
...
int DAlarmManager::remindJob(const DRemindData::Ptr &remindData, const DSchedule::Ptr &schedule)
{
    ...
    QString cmd = QString("dbus-send --session --print-reply --dest=com.lingmo.dataserver.Calendar "
                          ...
                          "com.lingmo.dataserver.Calendar.AccountManager.notifyMsgHanding string:%1 string:%2")
    ...
    auto argMake = [&](int operationNum, const QString & text, const QString & transText) {
        ...
        hints.insert("x-lingmo-action-" + text, QString("/bin/bash,-c,%1 int32:%2").arg(cmd).arg(operationNum));
    };
    ...
    QString appicon("ocean-calendar");
    QString appname("ocean-calendar");
