...
{
    // 清理玲珑包的残留
    QFile(m_systemdPath + "com.ocean.calendarserver.calendar.service").remove();
    QFile(m_systemdPath + "com.ocean.calendarserver.calendar.timer").remove();
    QFileInfo fileInfo(m_systemdPath + "timers.target.wants/com.ocean.calendarserver.calendar.timer");
    //如果没有设置定时任务则开启定时任务
    if (!fileInfo.exists()) {
        execLinuxCommand("systemctl --user enable com.ocean.calendarserver.calendar.timer");
        execLinuxCommand("systemctl --user start com.ocean.calendarserver.calendar.timer");
    }
}
...
{
    {
        ...
        QString remindCMD = QString("dbus-send --session --print-reply --dest=com.lingmo.dataserver.Calendar "
                                    "/com/lingmo/dataserver/Calendar/AccountManager "
                                    "com.lingmo.dataserver.Calendar.AccountManager.downloadByAccountID string:%1 ")
                            .arg(accountID);
...
void CSystemdTimerControl::startUploadTask(const int minute)
{
    ...
    }
    {
        ...
        QString remindCMD = QString("dbus-send --session --print-reply --dest=com.lingmo.dataserver.Calendar "
                                    "/com/lingmo/dataserver/Calendar/AccountManager "
                                    "com.lingmo.dataserver.Calendar.AccountManager.uploadNetWorkAccountData ");
...
void CSystemdTimerControl::createService(const QString &name, const SystemDInfo &info)
{
    ...
    QString remindCMD = QString("dbus-send --session --print-reply --dest=com.lingmo.dataserver.Calendar "
                                "/com/lingmo/dataserver/Calendar/AccountManager "
                                "com.lingmo.dataserver.Calendar.AccountManager.remindJob string:%1 string:%2")
                        .arg(info.accountID)
                        .arg(info.alarmID);
