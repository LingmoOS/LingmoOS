...
void DAccountModule::removeDB()
{
    ...
    if(account()->accountType() == DAccount::Type::Account_UnionID){
        QString dbPatch = getHomeConfigPath().append(QString("/lingmo/ocean-calendar-service/"));
        QDir dir(dbPatch);
        if (dir.exists()) {
            QStringList filters;
...
{
    DbusUIOpenSchedule openCalendar("com.lingmo.Calendar",
                                    "/com/lingmo/Calendar",
...
bool DAccountModule::exportSchedule(const QString &icsFilePath, const QString &typeID)
{
    ...
    // 附加扩展信息
    cal->setNonKDECustomProperty("X-OCEAN-CALENDAR-TYPE-ID", typeID);
    cal->setNonKDECustomProperty("X-OCEAN-CALENDAR-TYPE-NAME", typeInfo->displayName());
    cal->setNonKDECustomProperty("X-OCEAN-CALENDAR-TYPE-COLOR", typeInfo->getColorCode());
