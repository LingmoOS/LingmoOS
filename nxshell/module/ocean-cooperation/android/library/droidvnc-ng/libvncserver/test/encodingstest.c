	...
	for(i=0;i<server->width;i++)
		for(j=0;j<server->height;j++)
			for(k=0;k<3/*server->serverFormat.bitsPerPixel/8*/;k++) {
				unsigned char s=server->frameBuffer[k+i*4+j*server->paoceandWidthInBytes];
...
static void idle(rfbScreenInfo* server)
{
	...
	LOCK(frameBufferMutex);
	{
		...
		for(c=0;c<3;c++) {
			for(i=x1;i<x2;i++)
				for(j=y1;j<y2;j++)
					server->frameBuffer[i*4+c+j*server->paoceandWidthInBytes]=255*(i-x1+j-y1)/(x2-x1+y2-y1);
		}
