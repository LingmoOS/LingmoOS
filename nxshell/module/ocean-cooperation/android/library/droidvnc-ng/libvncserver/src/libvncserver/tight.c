        ...
        for (dx = x; dx < x + w; dx += MAX_SPLIT_TILE_SIZE) {
            ...
            if (CheckSolidTile(cl, dx, dy, dw, dh, &colorValue, FALSE)) {
                ...
                fbptr = (cl->scaledScreen->frameBuffer +
                         (cl->scaledScreen->paoceandWidthInBytes * y_best) +
                         (x_best * (cl->scaledScreen->bitsPerPixel / 8)));

                (*cl->translateFn)(cl->translateLookupTable, &cl->screen->serverFormat,
                                   &cl->format, fbptr, cl->beforeEncBuf,
                                   cl->scaledScreen->paoceandWidthInBytes, 1, 1);
...
{                                                                             \
    ...
    fbptr = (uint##bpp##_t *)&cl->scaledScreen->frameBuffer                   \
        [y * cl->scaledScreen->paoceandWidthInBytes + x * (bpp/8)];             \
                                                                              \
    colorValue = *fbptr;                                                      \
    ...
    for (dy = 0; dy < h; dy++) {                                              \
        ...
        fbptr = (uint##bpp##_t *)((uint8_t *)fbptr                            \
                 + cl->scaledScreen->paoceandWidthInBytes);                     \
    }                                                                         \
                                                                              \
            ...
            int h)
{
    ...
    fbptr = (cl->scaledScreen->frameBuffer
             + (cl->scaledScreen->paoceandWidthInBytes * y)
    ...
    if (cl->format.bitsPerPixel == cl->screen->serverFormat.bitsPerPixel &&
        ...
        case 16:
            FastFillPalette16(&palette, cl, (uint16_t *)fbptr, w,
                              cl->scaledScreen->paoceandWidthInBytes / 2, h);
            break;
        default:
            FastFillPalette32(&palette, cl, (uint32_t *)fbptr, w,
                              cl->scaledScreen->paoceandWidthInBytes / 4, h);
        }
        ...
        if(palette.numColors != 0 || cl->turboQualityLevel == -1) {
            (*cl->translateFn)(cl->translateLookupTable,
                               ...
                               cl->beforeEncBuf,
                               cl->scaledScreen->paoceandWidthInBytes, w, h);
        }
    }
    else {
        (*cl->translateFn)(cl->translateLookupTable, &cl->screen->serverFormat,
...
SendJpegRect(rfbClientPtr cl, int x, int y, int w, int h, int quality)
{
    ...
    if (ps == 2) {
        ...
        srcptr = (uint16_t *)&cl->scaledScreen->frameBuffer
            [y * cl->scaledScreen->paoceandWidthInBytes + x * ps];
        dst = tmpbuf;
        for(j = 0; j < h; j++) {
            ...
            srcptr += cl->scaledScreen->paoceandWidthInBytes / ps;
            dst += w * 3;
        }
        srcbuf = tmpbuf;
        pitch = w * 3;
        ...
        if (cl->screen->serverFormat.bigEndian)
            flags ^= TJ_BGR;
        pitch = cl->scaledScreen->paoceandWidthInBytes;
                    ...
                    int count)
{
    ...
    fbptr = (uint32_t *)
        &cl->scaledScreen->frameBuffer[y * cl->scaledScreen->paoceandWidthInBytes + x * 4];
...
PrepareRowForImg##bpp(rfbClientPtr cl, uint8_t *dst, int x, int y, int count) { \
    ...
    int inRed, inGreen, inBlue;                                             \
                                                                            \
    fbptr = (uint##bpp##_t *)                                               \
        &cl->scaledScreen->frameBuffer[y * cl->scaledScreen->paoceandWidthInBytes +       \
                             x * (bpp / 8)];                                \
                                                                            \
    while (count--) {                                                       \
        pix = *fbptr++;                                                     \
