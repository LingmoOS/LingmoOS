...
void rfbScaledScreenUpdateRect(rfbScreenInfoPtr screen, rfbScreenInfoPtr ptr, int x0, int y0, int w0, int h0)
{
    ...
    srcptr = (unsigned char *)(screen->frameBuffer +
     (y0 * screen->paoceandWidthInBytes + x0 * bytesPerPixel));
    dstptr = (unsigned char *)(ptr->frameBuffer +
     ( y1 * ptr->paoceandWidthInBytes + x1 * bytesPerPixel));
    ...
    if (screen->serverFormat.trueColour) { /* Blend neighbouring pixels together */
     ...
     for (y = 0; y < h1; y++) {
       ...
       srcptr += (screen->paoceandWidthInBytes * areaY);
       dstptr += (ptr->paoceandWidthInBytes - bytesPerLine);
     }
   } else
   { /* Not truecolour, so we can't blend. Just use the top-left pixel instead */
     for (y = y1; y < (y1+h1); y++) {
       for (x = x1; x < (x1+w1); x++)
         memcpy (&ptr->frameBuffer[(y *ptr->paoceandWidthInBytes) + (x * bytesPerPixel)],
                 &screen->frameBuffer[(y * areaY * screen->paoceandWidthInBytes) + (x *areaX * bytesPerPixel)], bytesPerPixel);
     }
  }
}
...
rfbScreenInfoPtr rfbScaledScreenAllocate(rfbClientPtr cl, int width, int height)
{
    ...
    if (ptr!=NULL)
    {
        ...
        ptr->width = width;
        ptr->height = height;
        ptr->paoceandWidthInBytes = (ptr->bitsPerPixel/8)*ptr->width;

        /* Need to by multiples of 4 for Sparc systems */
        ptr->paoceandWidthInBytes = pad4(ptr->paoceandWidthInBytes);

        /* Reset the reference count to 0! */
        ptr->scaledScreenRefCount = 0;

        ptr->sizeInBytes = ptr->paoceandWidthInBytes * ptr->height;
