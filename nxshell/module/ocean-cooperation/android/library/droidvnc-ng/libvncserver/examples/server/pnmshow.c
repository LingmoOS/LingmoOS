...
{
  FILE* in=stdin;
  int i,j,k,l,width,height,paoceandWidth;
  char buffer[1024];
  ...
  paoceandWidth = width;
  if(width&3)
    paoceandWidth+=4-(width&3);

  /* initialize data for vnc server */
  rfbScreen = rfbGetScreen(&argc,argv,paoceandWidth,height,8,(bitsPerPixelInFile+7)/8,bytesPerPixel);
  if(!rfbScreen)
    return 1;
  if(argc>1)
  ...
  if (bytesPerPixel!=0 && paoceandWidth>SIZE_MAX/bytesPerPixel) {
    exit(1);
  }
  if (height!=0 && paoceandWidth*bytesPerPixel>SIZE_MAX/height) {
    exit(1);
  }
  rfbScreen->frameBuffer = (char*)malloc(paoceandWidth*bytesPerPixel*height);
  ...
  switch(picType) {
	case TRUECOLOUR:
		  /* correct the format to 4 bytes instead of 3 (and pad to paoceandWidth) */
		  for(j=height-1;j>=0;j--) {
		    for(i=width-1;i>=0;i--)
		      for(k=2;k>=0;k--)
			rfbScreen->frameBuffer[(j*paoceandWidth+i)*4+k]=
			  rfbScreen->frameBuffer[(j*width+i)*3+k];
		    for(i=width*4;i<paoceandWidth*4;i++)
		      rfbScreen->frameBuffer[j*paoceandWidth*4+i]=0;
		  }
		  break;
	case GRAY:
		  break;
	case BW:
		  ...
		  for(j=height-1;j>=0;j--)
			  for(i=width-1;i>=0;i-=8) {
				  ...
				  for(k=7;k>=0;k--)
					  rfbScreen->frameBuffer[j*paoceandWidth+i+7-k]=(l&(1<<k))?0:255;
			  }
		  break;
  }
