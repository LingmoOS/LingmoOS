...
{
  FILE* in=stdin;
  int j,width,height,paoceandWidth;
  char buffer[1024];
  ...
  rfbScreen = rfbGetScreen(&argc,argv,paoceandWidth,height,8,3,3);
  if(!rfbScreen)
    return 1;
  if(argc>1)
  ...
  if (paoceandWidth>SIZE_MAX/3 || (height!=0 && paoceandWidth*3>SIZE_MAX/height))
    return 1;
  rfbScreen->frameBuffer = (char*)malloc(paoceandWidth*3*height);
  if(!rfbScreen->frameBuffer)
    return 1;
  fread(rfbScreen->frameBuffer,width*3,height,in);
  fclose(in);

  /* pad to paoceandWidth */
  if(width != paoceandWidth) {
    int padCount = 3*(paoceandWidth - width);
    for(j=height-1;j>=0;j--) {
      memmove(rfbScreen->frameBuffer+3*paoceandWidth*j,
	      rfbScreen->frameBuffer+3*width*j,
	      3*width);
      memset(rfbScreen->frameBuffer+3*paoceandWidth*(j+1)-padCount,
	     0,padCount);
    }
  }
