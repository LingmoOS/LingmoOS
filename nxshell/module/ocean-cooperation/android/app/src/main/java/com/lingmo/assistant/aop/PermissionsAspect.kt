...
import com.lingmo.assistant.manager.*
import com.lingmo.assistant.other.PermissionCallback
...
class PermissionsAspect {
     ...
     * 方法切入点
     */
    @Pointcut("execution(@com.lingmo.assistant.aop.Permissions * *(..))")
    fun method() {}
        ...
        XXPermissions.with(activity)
            ...
            .request(object : PermissionCallback() {
                ...
                override fun onDenied(permissions: MutableList<String>, never: Boolean) {
                    if (never) {
                        Timber.w("Request permissions were denied forever!")
                        // 如果是被永久拒绝就跳转到应用权限系统设置页面
                        XXPermissions.startPermissionActivity(activity, permissions)
                    } else {
                        Timber.w("Request permissions FAILED!")
                    }
                }
            })
    }
