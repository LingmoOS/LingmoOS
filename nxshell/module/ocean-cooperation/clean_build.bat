...
@REM projects
set COO_PROJECT=ocean-cooperation
...
move %cd%\installer-inno\lingmo-cooperation-* %cd%\installer-inno\%COO_PROJECT%\ > NUL
if ERRORLEVEL 1 goto issfailed
"%INNO_ROOT%\ISCC.exe" /Qp lingmo-%DT_PROJECT%-setup.iss
move %cd%\installer-inno\lingmo-datatransfer-* %cd%\installer-inno\%DT_PROJECT%\ > NUL
if ERRORLEVEL 1 goto issfailed
