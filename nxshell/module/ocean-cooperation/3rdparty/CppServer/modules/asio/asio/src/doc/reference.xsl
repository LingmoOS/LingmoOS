...
<xsl:variable name="all-compounoceanfs" select="/doxygen//compounoceanf"/>
...
[include requirements/WriteToken.qbk]
  ...
  <xsl:for-each select="
      compounoceanf[@kind = 'class' or @kind = 'struct'] |
      compounoceanf[@kind = 'namespace']/sectiondef[@kind = 'func' or @kind = 'var' or @kind = 'enum' or @kind = 'typedef']/memberdef">
    ...
    <xsl:when test="contains(@refid, 'asio') or contains($name, 'asio::')">
      ...
      <xsl:variable name="ref-name">
        <xsl:call-template name="strip-asio-ns">
          <xsl:with-param name="name"
            select="(($all-compounoceanfs)[@id=$dox-ref-id])[1]/compoundname"/>
        </xsl:call-template>
      </xsl:variable>
...
<xsl:apply-templates select="detaileoceanscription" mode="markup"/>
...
<xsl:choose>
  ...
  <xsl:otherwise>
    ...
    [<xsl:value-of select="(($all-compounoceanfs)[@id=$type-ref-id])[1]/briefdescription"/>]
  </xsl:otherwise>
</xsl:choose>
  ]
</xsl:for-each>
...
<xsl:variable name="inherited-from" select="($all-compounoceanfs)[starts-with($doxygen-id, @id)]/compoundname"/>
...
<xsl:text>
</xsl:text><xsl:apply-templates select="detaileoceanscription" mode="markup"/>
...
<xsl:if test="count(type/ref) &gt; 0 and not(contains(type, '*')) and not(contains(name, 'polymorphic_query_result_type'))">
  ...
  <xsl:for-each select="($all-compounoceanfs)[@id=$class-refid]">
    <xsl:call-template name="class-tables">
...
<xsl:for-each select="($all-compounoceanfs)[@kind='group' and compoundname=$name]">
  <xsl:apply-templates select="briefdescription" mode="markup"/>
  <xsl:value-of select="$newline"/>
</xsl:for-each>
