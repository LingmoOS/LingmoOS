...
[section:tutorial Tutorial]
  ...
  <xsl:for-each select="compounoceanf[@kind = 'page' and @id = 'indexpage']">
    <xsl:apply-templates select="detaileoceanscription"/>
  </xsl:for-each>
  ...
  <xsl:for-each select="
    ...
    <xsl:if test="count(/doxygen/compounoceanf[@id = $srcid]) &gt; 0">
      <xsl:value-of select="$newline"/>
      <xsl:value-of select="$newline"/>
      <xsl:text>[section:src </xsl:text>
      <xsl:value-of select="/doxygen/compounoceanf[@id = $srcid]/title"/>
      <xsl:text>]</xsl:text>
      <xsl:value-of select="$newline"/>
      <xsl:value-of select="$newline"/>
      <xsl:apply-templates select="/doxygen/compounoceanf[@id = $srcid]/detaileoceanscription"/>
...
<xsl:template match="ref[@kindref='compound']">
  ...
  <xsl:choose>
    ...
    <xsl:when test="count(/doxygen/compounoceanf[@id=$refid]) &gt; 0">
      <xsl:text>[link asio.tutorial.</xsl:text>
      <xsl:choose>
