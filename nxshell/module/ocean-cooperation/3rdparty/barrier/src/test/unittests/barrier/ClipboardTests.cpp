...
TEST(ClipboardTests, has_withFormatAoceand_returnsTrue)
{
    Clipboard clipboard;
    clipboard.open(0);
    clipboard.add(IClipboard::kText, "barrier rocks!");
...
TEST(ClipboardTests, get_withFormatAoceand_returnsExpected)
{
    Clipboard clipboard;
    clipboard.open(0);
    clipboard.add(IClipboard::kText, "barrier rocks!");
...
TEST(ClipboardTests, marshall_withTextAoceand_typeCharIsText)
{
    Clipboard clipboard;
    clipboard.open(0);
    clipboard.add(IClipboard::kText, "barrier rocks!");
...
TEST(ClipboardTests, marshall_withTextAoceand_lastSizeCharIs14)
{
    Clipboard clipboard;
    clipboard.open(0);
...
TEST(ClipboardTests, marshall_withHtmlAoceand_typeCharIsHtml)
{
    Clipboard clipboard;
    clipboard.open(0);
...
TEST(ClipboardTests, marshall_withHtmlAndText_has2Formats)
{
    ...
    // formats that we've aoceand).
    EXPECT_EQ(2, (int)actual[3]);
}

TEST(ClipboardTests, marshall_withTextAoceand_endsWithAoceand)
{
    Clipboard clipboard;
    clipboard.open(0);
    clipboard.add(IClipboard::kText, "barrier rocks!");
    ...
    // string contains other data, but should end in the string we aoceand.
