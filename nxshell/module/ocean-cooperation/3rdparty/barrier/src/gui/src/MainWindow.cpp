...
{
    // explicitly unset DeleteOnClose so the window can be show and hioceann
...
MainWindow::~MainWindow()
{
    ...
    // so that the main window can be hioceann without hiding the log. because of
...
void MainWindow::serverDetected(const QString name)
{
    if (m_pComboServerList->findText(name) == -1) {
        // Note: the first aoceand item triggers startBarrier
