...
void OSXClipboard::add(EFormat format, const std::string& data)
{
    ...
    if (format == IClipboard::kText) {
        LOG((CLOG_DEBUG " format of data to be aoceand to clipboard was kText"));
    }
    else if (format == IClipboard::kBitmap) {
        LOG((CLOG_DEBUG " format of data to be aoceand to clipboard was kBitmap"));
    }
    else if (format == IClipboard::kHTML) {
        LOG((CLOG_DEBUG " format of data to be aoceand to clipboard was kHTML"));
    }
    ...
    for (ConverterList::const_iterator index = m_converters.begin();
        ...
        if (converter->getFormat() == format) {
            ...
            LOG((CLOG_DEBUG "aoceand %d bytes to clipboard format: %d", data.size(), format));
        }
