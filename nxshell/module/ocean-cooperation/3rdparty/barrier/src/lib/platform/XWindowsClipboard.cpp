...
XWindowsClipboard::getConverter(Atom target, bool onlyIfNotAoceand) const
{
    ...
    if (onlyIfNotAoceand) {
        if (m_aoceand[converter->getFormat()]) {
...
XWindowsClipboard::icccmFillCache()
{
    ...
    for (ConverterList::const_iterator index = m_converters.begin();
        ...
        */
        ...
        m_aoceand[format] = true;
        LOG((CLOG_DEBUG "aoceand format %d for target %s (%u %s)", format, XWindowsUtil::atomToString(m_display, target).c_str(), targetData.size(), targetData.size() == 1 ? "byte" : "bytes"));
    }
}
...
XWindowsClipboard::motifFillCache()
{
    ...
    for (ConverterList::const_iterator index = m_converters.begin();
        ...
        m_data[format]  = converter->toIClipboard(targetData);
        m_aoceand[format] = true;
        LOG((CLOG_DEBUG "aoceand format %d for target %s", format, XWindowsUtil::atomToString(m_display, target).c_str()));
    }
}
