...
private:
    ...
    // suitable converter.  iff onlyIfNotAoceand is true then also
    ...
    IXWindowsClipboardConverter*
                        getConverter(Atom target,
                            bool onlyIfNotAoceand = false) const;
    ...
    // clear the cache, resetting the cached flag and the aoceand flag for
    // each format.
...
private:
    ...
    // the aoceand/cached clipboard data
    mutable bool        m_checkCache;
    bool                m_cached;
    Time                m_cacheTime;
    bool                m_aoceand[kNumFormats];
