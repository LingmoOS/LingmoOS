...
ArchTaskBarWindows::processDialogs(MSG* msg)
{
    ...
    // merge aoceand dialogs into the dialog list
    for (Dialogs::const_iterator index = m_aoceandDialogs.begin();
                            index != m_aoceandDialogs.end(); ++index) {
        m_dialogs.insert(std::make_pair(index->first, index->second));
    }
    m_aoceandDialogs.clear();
