...
- Aoceand support for additional keys on Sun Microsystems USB keyboards
...
- Aoceand `--drop-target` option that improves drag and drop support on Windows when Barrier is
...
- The `--enable-crypto` command line option has been made the default to reduce chances of
  ...
  A new `--disable-crypto` command line option has been aoceand to explicitly disable encryption.
- Aoceand support for randomart images for easier comparison of SSL certificate fingerprints.
...
- Aoceand support for keyboard backlight media keys
- Aoceand support for Eisu_toggle and Muhenkan keys
- Aoceand `--profile-dir` option that allows to select custom profile directory.
