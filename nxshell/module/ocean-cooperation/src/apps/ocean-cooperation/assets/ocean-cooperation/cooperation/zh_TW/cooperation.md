...
找到目標協同裝置（可透過電腦名、IP進行識別），點擊連接![ocean-cooperation](../common/connect.svg)後，完成兩台裝置的連接。

**2. 透過搜尋裝置IP連接裝置**

在搜尋框內，搜尋目標協同裝置的IP，點擊連接![ocean-cooperation](../common/connect.svg)後，完成兩台裝置的連接。

### 文件投送

**1. 透過協同裝置列表-文件投送按鈕進行投送**

當兩台裝置處於協同狀態下，在裝置列表介面點擊![ocean-cooperation](../common/send.svg)，在選擇文件視窗中，選擇需要發送的文件，點擊**確定**，即開始文件投送。
...
您可以透過點擊標題欄![ocean-cooperation](../common/menu.svg)選擇**設定**進入跨端協同的設定介面。
