...
找到目標協同設備（可通過計算機名、IP進行識別），點擊連接![ocean-cooperation](../common/connect.svg)後，完成兩台設備的連接。

**2. 通過搜索設備IP連接設備**

在搜索框內，搜索目標協同設備的IP，點擊連接![ocean-cooperation](../common/connect.svg)後，完成兩台設備的連接。

### 文件投送

**1. 通過協同設備列表-文件投送按鈕進行投送**

當兩台設備處於協同狀態下，在設備列表界面點擊![ocean-cooperation](../common/send.svg)，在選擇文件窗口中，選擇需要發送的文件，點擊**確定**，即開始文件投送。
...
您可以通過點擊標題欄![ocean-cooperation](../common/menu.svg)選擇**設置**進入跨端協同的設置界面。
