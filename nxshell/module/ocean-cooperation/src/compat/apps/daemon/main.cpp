...
static constexpr char kPluginInterface[] { "org.lingmo.plugin.daemon" };
...
#ifdef QT_DEBUG
    const QString &pluginsDir { OCEAN_COOPERATION_PLUGIN_ROOT_DEBUG_DIR };
    qInfo() << QString("Load plugins path : %1").arg(pluginsDir);
    pluginsDirs.push_back(pluginsDir);
#else
    pluginsDirs << QString(OCEAN_COOPERATION_PLUGIN_ROOT_DIR);
    pluginsDirs << QString(LINGMO_DAEMON_PLUGIN_DIR);
...
{
    // qputenv("QT_LOGGING_RULES", "ocean-cooperation-daemon.debug=true");
