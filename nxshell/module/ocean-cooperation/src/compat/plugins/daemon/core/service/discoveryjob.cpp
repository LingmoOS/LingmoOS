...
void DiscoveryJob::announcerRun(const fastring &info)
{
    ...
    ((searchlight::Announcer*)_announcer_p)->start([this](const QString &ip){
        QUNIGO([this, ip](){
            ...
            if (selfIp.empty())
                return;
            RemoteServiceSender sender("ocean-cooperation", ip, 51597, false);
...
{
    if (name.compare("ocean-cooperation") == 0)
...
void DiscoveryJob::notifySearchResult(bool result, const fastring &info)
{
    ...
    SendIpcService::instance()->handleSendToClient("ocean-cooperation", FRONT_SEARCH_IP_DEVICE_RESULT, jsonMsg);
}
