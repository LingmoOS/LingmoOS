...
{
    lingmo_cross::ReportLogManager::instance()->init();
    auto translator = new QTranslator(this);
    translator->load(QLocale(), "cooperation-transfer", "_", "/usr/share/ocean-file-manager/translations");
...
bool CooperationPlugin::start()
{
    ...
    qApp->setApplicationName("ocean-cooperation");
    ConfigManager::instance();
    qApp->setApplicationName(appName);

    // 添加文管设置
    if (appName == "ocean-file-manager")
...
void CooperationPlugin::bindMenuScene()
{
    ...
    if (ret) {
        ...
        dpfSignalDispatcher->subscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &CooperationPlugin::onMenuSceneAoceand);
    }
}

void CooperationPlugin::onMenuSceneAoceand(const QString &scene)
{
    if (scene == kParentScene) {
        dpfSlotChannel->push("dfmplugin_menu", "slot_MenuScene_Bind", CooperationMenuCreator::name(), QString(kParentScene));
        dpfSignalDispatcher->unsubscribe("dfmplugin_menu", "signal_MenuScene_SceneAoceand", this, &CooperationPlugin::onMenuSceneAoceand);
    }
}
