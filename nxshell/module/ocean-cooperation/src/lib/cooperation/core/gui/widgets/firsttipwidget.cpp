...
void FirstTipWidget::themeTypeChanged()
{
    if (CooperationGuiHelper::instance()->isDarkTheme()) {
        ...
        bannerLabel->setPixmap(QIcon::fromTheme(":/icons/lingmo/builtin/dark/icons/banner_128px.png").pixmap(234, 158));

    } else {
        shadowEffect->setColor(QColor(10, 57, 99, 128));
        bannerLabel->setPixmap(QIcon::fromTheme(":/icons/lingmo/builtin/light/icons/banner_128px.png").pixmap(234, 158));
    }
    if (!qApp->property("onlyTransfer").toBool())
        action->setPixmap(QIcon::fromTheme("connect").pixmap(12, 12));
#ifndef __linux__
    action->setPixmap(QIcon::fromTheme(":/icons/lingmo/builtin/texts/connect_18px.svg").pixmap(12, 12));
#endif
}
    ...
    for (int i = 0; i < tips.size(); i++) {
        ...
        if (i % 2 == 0) {
            ...
            if (i + 1 == tips.size()) {
                ...
                QString remainTip;
                if (qApp->property("onlyTransfer").toBool()) {
                    action->setPixmap(QIcon::fromTheme(":/icons/lingmo/builtin/texts/send_18px.svg").pixmap(12, 12));
...
void FirstTipWidget::inittipBtn()
{
    ...
    tipBtn->setIcon(QIcon::fromTheme(":/icons/lingmo/builtin/icons/close_white.svg"));
    tipBtn->setIconSize(QSize(8, 8));
    connect(tipBtn, &QToolButton::clicked, this, [this]() {
        QFile flag(lingmo_cross::CommonUitls::tipConfPath());
