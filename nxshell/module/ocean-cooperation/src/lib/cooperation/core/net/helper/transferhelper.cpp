...
#else
inline constexpr char Khistory[] { ":/icons/lingmo/builtin/texts/history_18px.svg" };
inline constexpr char Ksend[] { ":/icons/lingmo/builtin/texts/send_18px.svg" };
#endif
...
void TransferHelper::updateProgress(int value, const QString &remainTime)
{
    ...
    if (d->role != Server) {
        ...
        // ocean-session-ui 5.7.2.2 版本后，支持设置该属性使消息不进通知中心
        QVariantMap hitMap { { "x-lingmo-ShowInNotifyCenter", false } };
...
{
#ifdef __linux__
    QProcess::execute("ocean-file-manager", QStringList() << path);
#else
