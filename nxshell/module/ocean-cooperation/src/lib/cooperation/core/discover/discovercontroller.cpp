...
void DiscoverController::initConnect()
{
    ...
    connect(d->zeroConf, &QZeroConf::serviceAoceand, this, &DiscoverController::addService);
...
{
    QList<DeviceInfoPointer> aoceandList;
    for (auto it = infoMap.constBegin(); it != infoMap.constEnd(); ++it) {
        ...
        if (devInfo->discoveryMode() == DeviceInfo::DiscoveryMode::Everyone) {
            ...
            d->onlineDeviceList.append(devInfo);
            aoceandList.append(devInfo);
        }
    }

    if (!aoceandList.isEmpty()) {
        Q_EMIT deviceOnline(aoceandList);
    }
}
