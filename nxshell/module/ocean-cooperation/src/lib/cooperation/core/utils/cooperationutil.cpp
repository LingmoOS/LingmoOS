...
QVariantMap CooperationUtil::deviceInfo()
{
    ...
    value = lingmo_cross::BaseUtils::osType();
...
{
    QString ip = lingmo_cross::CommonUitls::getFirstIp().data();
    return ip;
}
...
void CooperationUtil::initNetworkListener()
{
    ...
    emit onlineStateChanged(lingmo_cross::CommonUitls::getFirstIp().c_str());
}

void CooperationUtil::checkNetworkState()
{
    // 网络状态检测
    bool isConnected = lingmo_cross::CommonUitls::getFirstIp().size() > 0;

    if (isConnected != d->isOnline) {
        d->isOnline = isConnected;
        Q_EMIT onlineStateChanged(lingmo_cross::CommonUitls::getFirstIp().c_str());
    }
}
...
void CooperationUtil::showFeatureDisplayDialog(QDialog *dlg1)
{
    ...
    dlg->setTitle(tr("Welcome to ocean-cooperation"));
    dlg->addItem(new DFeatureItem(QIcon::fromTheme(":/icons/lingmo/builtin/icons/tip_kvm.png"),
    ...
    dlg->addItem(new DFeatureItem(QIcon::fromTheme(":/icons/lingmo/builtin/icons/tip_clipboard.png"),
    ...
    dlg->addItem(new DFeatureItem(QIcon::fromTheme(":/icons/lingmo/builtin/icons/tip_file.png"),
    ...
    dlg->addItem(new DFeatureItem(QIcon::fromTheme(":/icons/lingmo/builtin/icons/tip_more.png"),
