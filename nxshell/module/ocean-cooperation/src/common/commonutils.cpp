...
{
    lingmo_cross::Logger::GetInstance().init(logDir().toStdString(), qApp->applicationName().toStdString());
#ifdef QT_DEBUG
    lingmo_cross::g_logLevel = lingmo_cross::debug;
    LOG << "Debug build, set LogLevel " << lingmo_cross::g_logLevel;
#endif
    ...
    lingmo_cross::g_logLevel = static_cast<lingmo_cross::LogLevel>(level);

    QTimer *timer = new QTimer();
    QObject::connect(timer, &QTimer::timeout, [configFile] {
        QSettings settings(configFile, QSettings::IniFormat);
        int level = settings.value("g_minLogLevel", 2).toInt();
        auto logLevel = static_cast<lingmo_cross::LogLevel>(level);
        if (lingmo_cross::g_logLevel != logLevel) {
            lingmo_cross::g_logLevel = logLevel;
...
#endif
    if (detailLog()) {
        lingmo_cross::g_logLevel = lingmo_cross::debug; //详细日志输出
    }
}
...
QString CommonUitls::logDir()
{
    QString logPath = QString("%1/%2/%3/")
                              ...
                              .arg(qApp->applicationName());   //~/.cache/lingmo/xx
