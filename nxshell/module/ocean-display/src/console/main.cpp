...
/** Usage example:
 * ocean-display-console --set output.0.disable output.1.mode.1 output.1.enable"
 *
 * Error codes:
...
int main(int argc, char **argv)
{
    const QString desc = QStringLiteral(
        "ocean-display-console allows to change the screen setup from the command-line.\n"
        "\n"
        "Setting the output configuration is done in an atomic fashion, all settings\n"
        "are applied in a single command.\n"
        "ocean-display-console can be used to enable and disable outputs, to position screens,\n"
        "change resolution (mode setting), etc.. You should put all your options into \n"
        "a single invocation of ocean-display-console, so they can all be applied at once.\n"
        "\n"
        "Usage examples:\n\n"
        "   Show output information:\n"
        "   $ ocean-display-console -o\n"
        ...
        "   $ ocean-display-console output.HDMI-2.disable output.eDP-1.mode.1 output.eDP-1.enable\n"
        "\n   Position the hdmi monitor on the right of the laptop panel\n"
        "   $ ocean-display-console output.HDMI-2.position.0,1280 output.eDP-1.position.0,0\n"
        "\n   Set resolution mode\n"
        "   $ ocean-display-console output.HDMI-2.mode.1920x1080@60 \n"
        "\n   Set scale (note: fractional scaling is only supported on wayland)\n"
        "   $ ocean-display-console output.HDMI-2.scale.2 \n"
        "\n   Set rotation (possible values: none, left, right, inverted)\n"
        "   $ ocean-display-console output.HDMI-2.rotation.left \n");
    /*
        "\nError codes:\n"
        "   2 : general parse error\n"
        "   3 : output id parse error\n"
    ...
    */
    const QString syntax = QStringLiteral(
        ...
        "For example:\n"
        "$ ocean-display-console output.HDMI-2.enable \\ \n"
        "                output.eDP-1.mode.4 \\ \n"
        "                output.eDP-1.position.1280,0\n"
        "Multiple settings are passed in order to have ocean-display-console apply these settings in one go.\n");
