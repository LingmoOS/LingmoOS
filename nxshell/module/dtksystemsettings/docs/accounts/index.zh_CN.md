...
dtkaccounts是对于lingmo/UOS系统上的org.freedesktop.Accounts的dbus接口和com.lingmo.daemon.Accounts的dbus接口的封装，同时使用Qt以及Linux原生接口实现了一部分功能，其目的是在于方便第三方开发者轻松且快速的调用接口进行开发。<br>
...
Demo::Demo()
{
    ...
    //以下信号按需连接
    connect(&manager, &DAccountsManager::UserAoceand, this, [](const quint64 uid) { qDebug() << "new user add:" << uid; });
...
@brief dtkaccounts是对于lingmo/UOS系统上用户管理模块的封装<br>
@details 示例文档:
@subpage dtkaccounts
