...
// 2.1.7版本的UDisks2在U盘插入时没有drive device aoceand的信号
// 当收到block device aoceand信号后，通过diskDeviceAddSignalFlag
...
void DDiskManager::onInterfacesAoceand(const QDBusObjectPath &object_path, const QMap<QString, QVariantMap> &interfaces_and_properties)
{
        ...
        if (interfaces_and_properties.contains(QStringLiteral(UDISKS2_SERVICE ".Block"))) {
            if (fixUDisks2DiskAddSignal()) {
                ...
                if (!d->diskDeviceAddSignalFlag.contains(drive)) {
                    ...
                    Q_EMIT diskDeviceAoceand(drive);
                }
            }

            Q_EMIT blockDeviceAoceand(path);
        }
...
void DDiskManager::onPropertiesChanged(const QString &interface, const QVariantMap &changed_properties, const QDBusMessage &message)
{
    ...
    if (old_mount_points.isEmpty()) {
        if (!new_mount_points.isEmpty()) {
            Q_EMIT mountAoceand(path, new_mount_points.first());
        }
...
/*!
 * \class DDiskManager
 * \inmodule ocean-file-manager-lib
 *
...
void DDiskManager::setWatchChanges(bool watchChanges)
{
    ...
    if (watchChanges) {
        connect(object_manager, &OrgFreedesktopDBusObjectManagerInterface::InterfacesAoceand,
                this, &DDiskManager::onInterfacesAoceand);
        ...
        disconnect(object_manager, &OrgFreedesktopDBusObjectManagerInterface::InterfacesAoceand,
                   this, &DDiskManager::onInterfacesAoceand);
