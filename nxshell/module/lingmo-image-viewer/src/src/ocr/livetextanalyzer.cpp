...
#include <lingmo-ocr-plugin-manager/lingmoocrplugindef.h>
#include <lingmo-ocr-plugin-manager/lingmoocrplugin.h>

#include <QtConcurrent/QtConcurrent>

LiveTextAnalyzer::LiveTextAnalyzer(QObject *parent)
    : QQuickImageProvider(Image)
    , ocrDriver(new LingmoOCRPlugin::LingmoOCRDriver)
{
    ocrDriver->loadDefaultPlugin();
    ocrDriver->setUseHardware({{LingmoOCRPlugin::HardwareID::GPU_Vulkan, 0}});
...
void LiveTextAnalyzer::setImage(const QImage &image)
{
    ...
    ocrDriver->setMatrix(image_copy.height(), image_copy.width(), image_copy.bits(),
                         static_cast<size_t>(image_copy.bytesPerLine()), LingmoOCRPlugin::PixelType::Pixel_RGB);
}
