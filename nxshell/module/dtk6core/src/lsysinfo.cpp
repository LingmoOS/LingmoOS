...
#define OS_VERSION_FILE     LSYSINFO_PREFIX"/etc/os-version"
#define LSB_RELEASE_FILE    LSYSINFO_PREFIX"/etc/lsb-release"
#define OS_RELEASE_FILE     LSYSINFO_PREFIX"/etc/os-release"
#define LINGMO_VERSION_FILE LSYSINFO_PREFIX"/etc/lingmo-version"

static inline bool inTest()
{
    return !QLatin1String(LSYSINFO_PREFIX).isEmpty();
}

DCORE_BEGIN_NAMESPACE

#ifdef QT_DEBUG
Q_LOGGING_CATEGORY(logSysInfo, "dtk.lsysinfo")
#else
...
void DSysInfoPrivate::ensureLingmoInfo()
{
    ...
    if (inTest())
        lingmoTypeMap.clear(); // clear cache for test
    ...
    Q_FOREVER {
        ...
        if (key_value.first == "Version") {
            lingmoVersion = key_value.second;
        } else if (line.startsWith("Type")) {
            if (key_value.first == "Type") {
                lingmoTypeMap[QString()] = QString::fromLatin1(key_value.second);
                ...
                if (!language.isEmpty()) {
                    lingmoTypeMap[QString::fromLatin1(language)] = QString::fromUtf8(key_value.second);
                }
            }
        } else if (key_value.first == "Edition") {
            lingmoEdition = QString::fromUtf8(key_value.second);
        } else if (key_value.first == "Copyright") {
            lingmoCopyright = QString::fromUtf8(key_value.second);
        }

        // lingmoTypeMap may not parse finished(multi language) but !lingmoTypeMap.isEmpty()
//        if (!lingmoTypeMap.isEmpty() && !lingmoEdition.isEmpty() && !lingmoCopyright.isEmpty()) {
//            break;
//        }
    }
...
QString DSysInfo::lingmoCopyright()
{
    ...
    return siGlobal->lingmoCopyright;
}
...
QString DSysInfo::distributionInfoPath()
{
    ...
    return QStandardPaths::locate(QStandardPaths::GenericDataLocation, "lingmo/distribution.info");
#else
...
@~english
  ...
  use \a type as Distribution to get the name of current lingmo distribution itself.
...
QString DSysInfo::distributionOrgName(DSysInfo::OrgType type, const QLocale &locale)
{
    ...
    QString fallback = type == Distribution ? QStringLiteral("Lingmo") : QString();
...
QPair<QString, QString> DSysInfo::distributionOrgWebsite(DSysInfo::OrgType type)
{
    ...
    QString fallbackSiteName = type == Distribution ? QStringLiteral("www.lingmo.org") : QString();
    QString fallbackSiteUrl = type == Distribution ? QStringLiteral("https://www.lingmo.org") : QString();
...
@~english
  ...
  use \a type as Distribution to get the logo of current lingmo distribution itself.
...
@~english
  ...
  Current rule:
   - Professional, Server, Personal edition (LingmoType) will be treat as Enterprise edition.
...
bool DSysInfo::isCommunityEdition()
{
    ...
    QList<LingmoType> enterpriseTypes {
        LingmoProfessional, LingmoServer, LingmoPersonal
    };
