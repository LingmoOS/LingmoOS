# Lingmo Application Specification   {#Specification}
...
````bash
XGD_LOG_HOME_LINGMO:
    Where lingmo-user-specific log should be written.
    XGD_USER_HOME/.log
...
DAPP_LOG_HOME:
    ...
    XGD_LOG_HOME_LINGMO/{Organization-name}/{Application-name}
...
The ocean-dock is offcial application of lingmo and the standard path will be:

````bash
DAPP_CONFIG_HOME:   $HOME/.config/lingmo/ocean-dock
DAPP_LOG_HOME:      $HOME/.log/lingmo/ocean-dock
DAPP_CACHE_HOME:    $HOME/.cache/lingmo/ocean-dock
DAPP_DATA_HOME:     $HOME/.local/share/lingmo/ocean-dock
````
