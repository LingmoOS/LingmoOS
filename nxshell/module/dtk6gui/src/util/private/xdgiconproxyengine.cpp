...
QPixmap XdgIconProxyEngine::followColorPixmap(ScalableEntry *color_entry, const QSize &size, QIcon::Mode mode, QIcon::State state)
{
    ...
    const LINGMO_XDG_THEME::PALETTE_MAP &cache_color_scheme = entryToColorScheme.value(cache_key);

    // 当size为1时表示此svg文件不需要处理ColorScheme标签
    if (!cache_color_scheme.isEmpty() && cache_color_scheme[LINGMO_XDG_THEME::Text].size() == 1)
        return entryPixmap(color_entry, size, mode, state);

    const LINGMO_XDG_THEME::PALETTE_MAP &color_scheme = LINGMO_XDG_THEME::colorScheme.localData();
    ...
    if (pm.isNull()) {
        ...
        if (device.open(QIODevice::ReadOnly)) {
            ...
            style_sheet = qMakePair((mode << 4) | state, STYLE.arg(color_scheme[LINGMO_XDG_THEME::Text], color_scheme[LINGMO_XDG_THEME::Highlight]));
        ...
        if (invalidBuffers) {
            ...
            entryToColorScheme[cache_key] = LINGMO_XDG_THEME::PALETTE_MAP({ {LINGMO_XDG_THEME::Text, "#"} });
            return entryPixmap(color_entry, size, mode, state);
        }
        ...
        filenames[-1] = color_scheme[LINGMO_XDG_THEME::Text] + color_scheme[LINGMO_XDG_THEME::Highlight]; // 在dsvg插件中会为svg图标做缓存，此处是为其添加额外的缓存文件key标识，避免不同color的svg图标会命中同一个缓存文件
        str << QStringLiteral("svg") << filenames << static_cast<int>(0) /*isCompressed*/ << svg_buffers << static_cast<int>(0) /*hasAoceandPimaps*/;
...
QPixmap XdgIconProxyEngine::pixmapByEntry(QIconLoaderEngineEntry *entry, const QSize &size, QIcon::Mode mode, QIcon::State state)
{
    if (!XdgIconFollowColorScheme()) {
        LINGMO_XDG_THEME::colorScheme.setLocalData(LINGMO_XDG_THEME::PALETTE_MAP());
    ...
    if (type_name == QByteArrayLiteral("ScalableFollowsColorEntry")) {
        if (LINGMO_XDG_THEME::colorScheme.localData().isEmpty()) {
            const QPalette &pal = palette(&pixmap);
            LINGMO_XDG_THEME::colorScheme.setLocalData(LINGMO_XDG_THEME::PALETTE_MAP({
                { LINGMO_XDG_THEME::Text, mode == QIcon::Selected ? rgba(pal.highlightedText().color()) : rgba(pal.windowText().color()) },
                { LINGMO_XDG_THEME::Highlight, rgba(pal.highlight().color()) }
            }));
        }
...
void XdgIconProxyEngine::paint(QPainter *painter, const QRect &rect, QIcon::Mode mode, QIcon::State state)
{
    if (painter->device()->devType() == QInternal::Widget
        ...
        && LINGMO_XDG_THEME::colorScheme.localData().isEmpty()) {
