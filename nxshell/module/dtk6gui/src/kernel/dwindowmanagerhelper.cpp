...
/*!
  ...
  \sa {https://github.com/lingmoos/qt5dxcb-plugin/}{dxcb插件}
...
/*!
  ...
  \note 在 dxcb 插件中目前只支持 lingmo-wm 和 kwin 这两种窗管的模糊特效
  \note 只读
 */
...
/*!
  ...
  \note 对于使用系统标题栏的窗口，此功能和具体窗口管理器实现相关，lingmo-wm 中设置
  此标志无效。
...
/*!
  ...
  否则无法通过外部行为调整窗口大小。
  \note 只支持使用系统标题栏的窗口，此功能和具体窗口管理器实现相关，lingmo-wm 中设置
  此标志无效。
...
DWindowManagerHelper::WMName DWindowManagerHelper::windowManagerName() const
{
    ...
    if (wmName == QStringLiteral("Mutter(LingmoGala)")) {
        return LingmoWM;
    }
