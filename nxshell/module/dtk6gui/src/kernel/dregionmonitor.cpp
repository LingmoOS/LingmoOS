...
DRegionMonitorPrivate::DRegionMonitorPrivate(DRegionMonitor *q)
    : DObjectPrivate(q)
{
    if (QDBusConnection::sessionBus().interface()->isServiceRegistered(QLatin1String("org.lingmo.ocean.XEventMonitor1"))) {
        eventInter = new XEventMonitor("org.lingmo.ocean.XEventMonitor1", "/org/lingmo/ocean/XEventMonitor1",
                                       "org.lingmo.ocean.XEventMonitor1", q);
    } else {
        eventInter = new XEventMonitor("com.lingmo.api.XEventMonitor", "/com/lingmo/api/XEventMonitor",
                                       "com.lingmo.api.XEventMonitor", q);
    }
