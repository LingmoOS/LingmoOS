...
int exec(const char *process)
{
    ...
    // lingmo-trubo 启动的程序无法对外暴露环境变量，因此通过root窗口设置启动时间
...
bool checkPid(__pid_t pid)
{
    ...
    while (true) {
        ...
        // 检测是不是lingmo-turbo-booster进程
        char exe_path[100];
        readlink(status, exe_path, 100);
        // 对于lingmo-turbo的进程无法检测是否为子进程，因此默认认为是被ocean-kwin-debug启动的
        if (strstr(exe_path, "/usr/lib/lingmo-turbo") == exe_path)
            return true;
    ...
    if (!dpy) {
        ...
        if (!dpy) {
            fprintf(stderr, "%s:  unable to open display '%s'\n",
                    "ocean-kwin-startup-debug", XDisplayName (displayname));
            exit(1);
        }
    }
...
Eg: ocean-kwin-debug -k --format "%p %t %n" qtcreator
...
Arguments:
The applications.
Eg: ocean-kwin-debug -k --format "%t %n" "app1" "app2" "app3"
)help";
