...
void ScreensaverWorker::requestResource()
{
    ...
    for (const QString &name : saverNameList) {
        ...
        if (name.startsWith("lingmo")) {
            saverNameList.move(saverNameList.indexOf(name), lingmo);
            lingmo++;
        }
    }
