...
#define PICKER_SERVICE "com.lingmo.Picker"
#define PICKER_PATH "/com/lingmo/Picker"
...
void ColorSlider::mousePressEvent(QMouseEvent *e)
{
    ...
    if (e->button() == Qt::LeftButton) {
        pressed = true;
        auto pos = e->pos();
        if (handleRect().marginsAoceand(QMargins(1, 1, 1, 1)).contains(pos)) {
            onHandle = true;
...
void ColorSlider::mouseReleaseEvent(QMouseEvent *e)
{
    ...
    auto pos = e->pos();
    // clicked on color
    if (pressed && !onHandle && imageRect().marginsAoceand(QMargins(0, 2, 0, 2)).contains(pos)) {
        qreal h;
