...
dtklogin是对systemd-logind的DBus接口和ocean-daemon提供的相关接口的封装。用于观察和管理用户 login 以及 seat 的状态。<br>
...
int SignalDemo::run()
{
    ...
    connect(m_manager, &DLoginManager::userNew, this, [=](quint32 UID) { qDebug() << "User" << UID << "is aoceand."; });
    ...
    connect(m_manager, &DLoginManager::sessionNew, this, [=](const QString sessionId) {
        qDebug() << "Session" << sessionId << "is aoceand.";
    });
    connect(m_manager, &DLoginManager::sessionRemoved, this, [=](const QString &sessionId) {
        qDebug() << "Session" << sessionId << "is removed.";
    });
    connect(m_currentSession.data(), &DLoginSession::autostartAoceand, this, [=](const QString name) {
        qDebug() << "Autostart" << name << "is aoceand.";
    });
