    ...
    : QObject(parent)
{
    ...
    const QString &Service = QStringLiteral("com.lingmo.daemon.FakeAccounts");
    const QString &Path = QStringLiteral("/com/lingmo/daemon/FakeAccounts");
    const QString &Interface = QStringLiteral("com.lingmo.daemon.FakeAccounts");
    QDBusConnection Connection = QDBusConnection::sessionBus();
#else
    const QString &Service = QStringLiteral("org.lingmo.ocean.Accounts1");
    const QString &Path = QStringLiteral("/org/lingmo/ocean/Accounts1");
    const QString &Interface = QStringLiteral("org.lingmo.ocean.Accounts1");
    QDBusConnection Connection = QDBusConnection::systemBus();
    Connection
            .connect(Service, Path, Interface, "UserAoceand", this, SLOT(receiveUserAoceand(QString)));
...
void DSystemAccountsInterface::receiveUserAoceand(QString user)
{
    emit ReceivedUserAoceand(std::move(user));
}
