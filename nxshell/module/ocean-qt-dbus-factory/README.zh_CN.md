...
`ocean-qt-dbus-factory` 是用来统一存放 `ocean` 使用到的自动生成的 `Qt DBus` 代码的仓库，这样 `ocean` 开发者就不用在自己的项目中重复创建同样的 `DBus` 代码，或者从其它项目中拷贝文件到自己的项目里面。
...
## 开源许可证
ocean-qt-dbus-factory 在 [GPL-3.0-or-later](LICENSE) 下发布。
