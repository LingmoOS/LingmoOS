...
#include "com_lingmo_daemon_SoundEffect.h"
...
public:
	SoundEffect(QObject *parent = nullptr) :QDBusVirtualObject(parent)
    {
        ...
        m_soundEffect = new com::lingmo::daemon::SoundEffect("com.lingmo.daemon.SoundEffect", "/com/lingmo/daemon/SoundEffect", QDBusConnection::sessionBus(), this);
        signalMonitor();
    }
    virtual	~SoundEffect(){}
    ...
    void signalMonitor()
    {
        connect(m_soundEffect, &com::lingmo::daemon::SoundEffect::EnabledChanged, this, [](bool ret){
            qInfo() << "enable:" << ret;
        });
    }

private:
    com::lingmo::daemon::SoundEffect *m_soundEffect;
};
