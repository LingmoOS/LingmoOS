...
#include "session_org_lingmo_ocean_ControlCenter1.h"
#include "session_org_lingmo_ocean_ControlCenter1_GrandSearch.h"
...
public:
        ...
        if (message.interface() == m_proxyDbusInterface) {
            if (message.member() == "ShowModule") {
                ...
                if (message.arguments().size() == 1) {
                    ...
                    connection.send(message.createErrorReply("com.lingmo.ocean.error.ParamError", "param error"));
                }
                return true;
            }
        }
        // 目前的Proxy封装，对于同PATH下不同Interface没有统一处理，所以在这里处理com.lingmo.ocean.ControlCenter.GrandSearch代理
        if (message.interface() == "com.lingmo.ocean.ControlCenter.GrandSearch") {
            QStringList updaterFilterMethods{"Action", "Search", "Stop"};
            // if (!updaterFilterMethods.contains(message.member())) {
            //     qInfo() << "com.lingmo.ocean.ControlCenter.GrandSearch" << "method-filter:" << message.member() << "is not allowed.";
            //     connection.send(message.createErrorReply("com.lingmo.ocean.error.NotAllowed", "is not allowed"));
            //     return true;
            // }
    ...
    {
        m_dbusProxy = new org::lingmo::ocean::ControlCenter1(m_dbusName, m_dbusPath, QDBusConnection::sessionBus(), this);
        m_dbusGrandSearchProxy = new org::lingmo::ocean::controlcenter1::GrandSearch(m_dbusName, m_dbusPath, QDBusConnection::sessionBus(), this);
        return m_dbusProxy;
    }
private:
    org::lingmo::ocean::ControlCenter1 *m_dbusProxy;
    org::lingmo::ocean::controlcenter1::GrandSearch *m_dbusGrandSearchProxy;
};
