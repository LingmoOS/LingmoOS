...
void ShowDesktopWidget::paintEvent(QPaintEvent *e)
{
    ...
    if (Dock::Fashion == qApp->property(PROP_DISPLAY_MODE).value<Dock::DisplayMode>()) {
        icon = ImageUtil::loadSvg("lingmo-toggle-desktop", QString(), int(size().width() * 0.8), ratio);
    } else {
        icon = ImageUtil::loadSvg("lingmo-toggle-desktop", QString(), int(size().width() * 0.7), ratio);
    }
