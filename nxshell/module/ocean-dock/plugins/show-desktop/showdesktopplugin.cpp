...
void ShowDesktopPlugin::invokedMenuItem(const QString &itemKey, const QString &menuId, const bool checked)
{
    ...
    if (menuId == "show-desktop") {
        ...
        DDBusSender()
            .service("org.lingmo.ocean.Dock1")
            .interface("org.lingmo.ocean.Dock1")
            .path("/org/lingmo/ocean/Dock1")
...
QIcon ShowDesktopPlugin::icon(const DockPart &dockPart, DGuiApplicationHelper::ColorType themeType)
{
    if (dockPart == DockPart::DCCSetting) {
        auto loadsvg = []{ return ImageUtil::loadSvg(":/icons/oceanui-show-desktop.svg", QSize(18, 18));};
        auto icon = QIcon::fromTheme("oceanui-show-desktop", loadsvg());
