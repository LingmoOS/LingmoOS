...
ocean-dock 插件是根据 Qt 插件标准所开发的共享库文件(`so`)，通过实现 Qt 的插件标准和 ocean-dock 提供的接口，共同完成 ocean-dock 的功能扩展。\
...
在 ocean-dock 启动时会跑一个线程去检测目录`/usr/lib/ocean-dock/plugins`下的所有文件，并检测是否是一个正常的动态库文件，如果是则尝试加载。尝试加载即检测库文件的元数据，插件的元数据定义在一个 JSON 文件中，这个后文会介绍，如果元数据检测通过就开始检查插件是否实现了 ocean-dock 指定的接口，这一步也通过之后就会开始初始化插件，获取插件提供的控件，进而将控件显示在任务栏上。

## 接口列表

这里先列出 ocean-dock 都提供了哪些接口，可作为一个手册查看，注意，为 ocean-dock 编写插件并不是要实现所有接口，这些接口提供了 ocean-dock 允许各种可能的功能，插件开发者可以根据自己的需求去实现自己需要的接口。后续的插件示例也将会用到这里列出的部分接口。
...
|名称|简介|
|-|-|
|pluginName | 返回插件名称，用于在 ocean-dock 内部管理插件时使用 `必须实现`|
|pluginDisplayName | 返回插件名称，用于在界面上显示|
|init | 插件初始化入口函数，参数 proxyInter 可认为是主程序的进程 `必须实现`|
|itemWidget | 返回插件主控件，用于显示在 ocean-dock 面板上 `必须实现`|
...
|displayModeChanged | ocean-dock 显示模式发生改变时此接口被调用|
|positionChanged | ocean-dock 位置变化时时此接口被调用|
|refreshIcon | 当插件控件的图标需要更新时此接口被调用|
|displayMode | 用于插件主动获取 ocean-dock 当前的显示模式|
|position | 用于插件主动获取 ocean-dock 当前的位置|
...
由于上面的接口对于插件来说都是被动的，即插件本身无法确定这些接口什么时刻会被调用，很明显这对于插件机制来说是不完整的，因此便有了 PluginProxyInterface，它定义了一些让插件主动调用以控制 ocean-dock 的一些行为的接口。PluginProxyInterface 的具体实例可以认为是抽象了的 ocean-dock 主程序，或者是 ocean-dock 中所有插件的管理员，这个实例将会通过 PluginItemInterface 中的 `init` 接口传递给插件，因此在上述 `init` 接口中总是会先把这个传入的对象保存起来以供后续使用。

|名称|简介|
|-|-|
|itemAoceand | 向 ocean-dock 添加新的主控件（一个插件可以添加多个主控件它们之间使用`ItemKey`区分）|
|itemUpdate | 通知 ocean-dock 有主控件需要更新|
|itemRemoved | 从 ocean-dock 移除主控件|
|updateDockInfo | 当插件图标发生变化的时候，响应该函数 |
|requestWindowAutoHide | 设置 ocean-dock 是否允许隐藏，通常被用在任务栏被设置为智能隐藏或始终隐藏而插件又需要让 ocean-dock 保持显示状态来显示一些重要信息的场景下|
...
接下来将介绍一个简单的 ocean-dock 插件的开发过程，插件开发者可跟随此步骤熟悉为 ocean-dock 开发插件的步骤，以便创造出更多具有丰富功能的插件。
...
下面以 Qt + cmake 为例进行说明，以 Lingmo 15.9 环境为基础，安装如下的包：
...
`home_monitor.json`文件是插件的元数据文件，指明了当前插件所使用的 ocean-dock 的接口版本，ocean-dock 在加载此插件时，会检测自己的接口版本是否与插件的接口版本一致，当双方的接口版本不一致或者不兼容时，ocean-dock 为了安全将阻止加载对应的插件。另外，元数据文件是在源代码中使用特定的宏加载到插件中的。

在 ocean-dock 内建的插件代码中，可以找到当前具体的接口版本，目前最新的版本是 `1.2` 。
...
另外（可选的）还支持指定一个 dbus 服务，dock 在加载插件时会检查此插件所依赖的 dbus 服务，如果服务没有启动则不会初始化这个插件，直到服务启动，如下表示依赖 dbus 地址为 "org.lingmo.ocean.Network1" 的 dbus 服务。

``` json
{
    "api": "2.0.0",
    "depends-daemon-dbus-service": "org.lingmo.ocean.Network1"
}
```
...
```json
{
    "api": "2.0.0",
    "order": 1,
    "depends-daemon-dbus-service": "org.lingmo.ocean.Audio1"
}
```
...
`homemonitorplugin.h` 声明了类 `HomeMonitorPlugin`，它继承（实现）了前面提到的 `PluginItemInterface`，这代表了它是一个实现了 ocean-dock 接口的插件。
...
class HomeMonitorPlugin : public QObject, PluginsItemInterface
{
    ...
    // 插件元数据
    Q_PLUGIN_METADATA(IID "com.lingmo.dock.PluginsItemInterface" FILE "home_monitor.json")
...
QWidget *HomeMonitorPlugin::itemWidget(const QString &itemKey)
{
    ...
    // 这里暂时返回空指针，这意味着插件会被 ocean-dock 加载
    // 但是不会有任何东西被添加到 ocean-dock 上
    return nullptr;
}
...
当插件的基本结构搭建好之后应该测试下这个插件能否被 ocean-dock 正确的加载，这时候测试如果有问题也可以及时处理。
...
可以看到有`home_monitor.so`文件被安装在了 ocean-dock 的插件目录。

``` sh
install -m 755 -p ./home_monitor/libhome_monitor.so /usr/lib/ocean-dock/plugins/libhome_monitor.so
```

或者将so文件放在~/.local/lib/ocean-dock/plugins/目录下，dock会加载用户本地的插件。

``` sh
install -m 755 -p ./home_monitor/libhome_monitor.so ~/.local/lib/ocean-dock/plugins/libhome_monitor.so
```

### 测试加载

执行 `pkill ocean-dock; ocean-dock` 来重新运行 ocean-dock，在终端输出中如果出现以下的输出，说明插件的加载已经正常：
...
在插件核心类的 `init` 方法中获取到了 `PluginProxyInterface` 对象，调用此对象的 `itemAoceand` 接口即可实现向 ocean-dock 面板上添加项目。
...
在调用 `itemAoceand` 之后，ocean-dock 会将该控件在任务栏插件区域显示出来，在展开快捷面板的时候，会将该插件的大图标显示在快捷面板上，快捷面板上显示的内容可以通过如下两种方式来显示
...
ocean-dock 在发现鼠标悬停在插件的控件上时就会调用这个接口拿到相应的控件并显示出来。
...
编译，安装，重启 ocean-dock 之后右键点击主控件即可看到弹出右键菜单。
