...
{
    // 插件有三种状态
    // 1、所有的插件，不管这个插件是否调用itemAoceand方法，只要是通过dock加载的插件（换句话说，也就是在/lib/ocean-dock/plugins目录下的插件）
    // 2、在1的基础上，插件自身调用了itemAoceand方法的插件，例如机器上没有蓝牙设备，那么蓝牙插件就不会调用itemAoceand方法，这时候就不算
...
void DockPluginController::itemAoceand(PluginsItemInterface * const itemInter, const QString &itemKey)
{
    ...
    if (interfaceData.contains(PLUGININFO)) {
        ...
        // 如果插件已经加载，则无需再次加载（此处保证插件出现重复调用itemAoceand的情况）
        if (pluginInfo->m_loaded)
            return;
    } else {
...
void DockPluginController::addPluginItem(PluginsItemInterface * const itemInter, const QString &itemKey)
{
    ...
    m_proxyInter->itemAoceand(itemInter, itemKey);
}
...
void DockPluginController::startLoader(PluginLoader *loader)
{
    ...
    int delay = Utils::SettingValue("com.lingmo.ocean.dock", "/com/lingmo/ocean/dock/", "delay-plugins-time", 0).toInt();
...
void DockPluginController::loadPlugin(const QString &pluginFile)
{
    ...
    if (!interface) {
        ...
        if (interface_v20) {
            ...
            // 将适配器的地址保存到map列表中，因为适配器自己会调用itemAoceand方法，转换成PluginsItemInterface类，但是实际上它
            // 对应的是PluginAdapter类，因此，这个map用于在后面的itemAoceand方法中用来查找
...
bool DockPluginController::pluginCanDock(const QStringList &config, PluginsItemInterface *plugin) const
{
    ...
    // 3、如果该插件并未加载（未调用itemAocean或已经调用itemRemoved)，则该插件不显示
    if (!m_pluginsMap.contains(plugin))
        return false;

    const QMap<QString, QObject *> &pluginMap = m_pluginsMap[plugin];
    // 如果不包含PLUGININFO，说明该插件从未调用itemAoceand方法，无需加载
    if (!pluginMap.contains(PLUGININFO))
        return false;

    // 如果该插件信息的m_loaded为true,说明已经调用过itemAoceand方法，并且之后又调用了itemRemoved方法,则插件也无需加载
    PluginInfo *pluginInfo = static_cast<PluginInfo *>(pluginMap[PLUGININFO]);
    if (!pluginInfo->m_loaded)
        return false;
    ...
    for (PluginsItemInterface *plugin : plugins()) {
        ...
        if (!canDock && isPluginLoaded(plugin)) {
            ...
            if (!pluginNames.contains(plugin->pluginName())) {
                // lingmo-screen-recorder has Attribute_ForceDock flag
                // FIX https://github.com/lingmoos/developer-center/issues/4215
                continue;
            }
