...
static QStringList getPathFromConf(const QString &key) {
    QSettings set("/etc/lingmo/ocean-dock.conf", QSettings::IniFormat);
...
#else
    pluginPaths << QString("/usr/lib/ocean-dock%1").arg(QUICK_PATH)
                << QString("/usr/lib/ocean-dock%1").arg(PLUGIN_PATH)
                << QString("/usr/lib/ocean-dock%1").arg(TRAY_PATH);
