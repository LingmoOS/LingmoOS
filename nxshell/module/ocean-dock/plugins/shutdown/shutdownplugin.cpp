...
ShutdownPlugin::ShutdownPlugin(QObject *parent)
    ...
    , m_powerManagerInter(new DBusPowerManager("org.lingmo.ocean.PowerManager1", "/org/lingmo/ocean/PowerManager1", QDBusConnection::systemBus(), this))
    , m_gsettings(Utils::ModuleSettingsPtr("shutdown", QByteArray(), this))
    , m_sessionShellGsettings(Utils::SettingsPtr("com.lingmo.ocean.session-shell", "/com/lingmo/ocean/session-shell/", this))
{
...
const QString ShutdownPlugin::itemCommand(const QString &itemKey)
{
    ...
    return QString("dbus-send --print-reply --dest=org.lingmo.ocean.ShutdownFront1 /org/lingmo/ocean/ShutdownFront1 org.lingmo.ocean.ShutdownFront1.Show");
}
...
const QString ShutdownPlugin::itemContextMenu(const QString &itemKey)
{
    ...
    if (!QFile::exists(ICBC_CONF_FILE)) {
        // com.lingmo.ocean.session-shell切换用户配置项
...
void ShutdownPlugin::invokedMenuItem(const QString &itemKey, const QString &menuId, const bool checked)
{
    ...
    if (menuId == "power") {
        ...
        if (QFile::exists(ICBC_CONF_FILE)) {
            ...
            DDBusSender()
            .service("org.lingmo.ocean.LockFront1")
            .interface("org.lingmo.ocean.LockFront1")
            .path("/org/lingmo/ocean/LockFront1")
            .method(QString("Show"))
            .call();
        }
    } else
...
QIcon ShutdownPlugin::icon(const DockPart &dockPart, DGuiApplicationHelper::ColorType themeType)
{
    if (dockPart == DockPart::DCCSetting) {
        QString iconFile(":/icons/resources/icons/oceanui_shutdown.svg");
