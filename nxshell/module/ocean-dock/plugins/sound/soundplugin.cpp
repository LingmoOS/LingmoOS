...
#include "sounoceanviceswidget.h"
...
void SoundPlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    if (!pluginIsDisable()) {
        m_proxyInter->itemAoceand(this, SOUND_KEY);
...
bool SoundPlugin::eventHandler(QEvent *event)
{
    ...
    // 获取当前默认的声音设备
    QDBusPendingCall defaultSinkCall = DDBusSender().service("org.lingmo.ocean.Audio1")
            .path("/org/lingmo/ocean/Audio1")
            .interface("org.lingmo.ocean.Audio1")
    ...
    // 获取当前默认声音设备的音量
    DDBusSender sinkDBus = DDBusSender().service("org.lingmo.ocean.Audio1")
            .path(defaultSinkPath).interface("org.lingmo.ocean.Audio1.Sink");
    ...
    // 获取当前默认声音设备的最大音量
    DDBusSender audioDBus = DDBusSender().service("org.lingmo.ocean.Audio1")
            .path("/org/lingmo/ocean/Audio1").interface("org.lingmo.ocean.Audio1");
