...
DBusHandler::DBusHandler(TaskManager *taskmanager, QObject *parent)
    ...
    , m_wm(new com::lingmo::wm("com.lingmo.wm", "/com/lingmo/wm", QDBusConnection::sessionBus(), this))
    , m_wmSwitcher(new org::lingmo::ocean::WMSwitcher1("org.lingmo.ocean.WMSwitcher1", "/org/lingmo/ocean/WMSwitcher1", QDBusConnection::sessionBus(), this))
    , m_kwaylandManager(nullptr)
    , m_xEventMonitor(nullptr)
    , m_launcher(new org::lingmo::ocean::Launcher1(launcherService, launcherPath, QDBusConnection::sessionBus(), this))
{
    ...
    connect(m_wmSwitcher, &org::lingmo::ocean::WMSwitcher1::WMChanged, this, [&](QString name) {m_taskmanager->setWMName(name);});
    if (!isWaylandSession()) {
        m_xEventMonitor = new org::lingmo::ocean::XEventMonitor1("org.lingmo.ocean.XEventMonitor1", "/org/lingmo/ocean/XEventMonitor1", QDBusConnection::sessionBus(), this);
        m_activeWindowMonitorKey = m_xEventMonitor->RegisterFullScreen();
        connect(m_xEventMonitor, &org::lingmo::ocean::XEventMonitor1::ButtonRelease, this, &DBusHandler::onActiveWindowButtonRelease);
    }

    connect(m_launcher,static_cast<void (org::lingmo::ocean::Launcher1::*)(bool)>(&org::lingmo::ocean::Launcher1::VisibleChanged), this, [=] (const bool visible){
...
{
    m_kwaylandManager = new org::lingmo::ocean::kwayland1::WindowManager("org.lingmo.ocean.KWayland1", "/org/lingmo/ocean/KWayland1/WindowManager", QDBusConnection::sessionBus(), this);
    connect(m_kwaylandManager, &org::lingmo::ocean::kwayland1::WindowManager::ActiveWindowChanged, this, &DBusHandler::handleWlActiveWindowChange);
    connect(m_kwaylandManager, &org::lingmo::ocean::kwayland1::WindowManager::WindowCreated, this, [&] (const QString &ObjPath) {
        m_taskmanager->registerWindowWayland(ObjPath);
    });
    connect(m_kwaylandManager, &org::lingmo::ocean::kwayland1::WindowManager::WindowRemove, this, [&] (const QString &ObjPath) {
...
void DBusHandler::launchApp(QString desktopFile, uint32_t timestamp, QStringList files)
{
    if (newStartManagerAvaliable()) {
        ...
        launchAppUsingApplicationManager1(QString{OCEANApplicationManager1ObjectPath} + '/' + objPath, timestamp, files);
    } else {
...
void DBusHandler::launchAppAction(QString desktopFile, QString action, uint32_t timestamp)
{
    if (newStartManagerAvaliable()) {
        auto objPath = desktopEscapeToObjectPath(desktopFile);
        launchAppActionUsingApplicationManager1(QString{OCEANApplicationManager1ObjectPath} + '/' + objPath, action, timestamp);
    } else {
...
{
    QDBusInterface interface = QDBusInterface("org.lingmo.ocean.Application1.Manager", "/org/lingmo/ocean/Application1/Manager", "org.lingmo.ocean.Application1.Manager");
...
{
    QDBusInterface interface = QDBusInterface("org.lingmo.ocean.AlRecorder1", "/org/lingmo/ocean/AlRecorder1", "org.lingmo.ocean.AlRecorder1");
...
{
    return new PlasmaWindow("org.lingmo.ocean.KWayland1", objPath, QDBusConnection::sessionBus(), this);
}
...
void DBusHandler::sendFailedDockNotification(const QString &appName)
{
    QtConcurrent::run(QThreadPool::globalInstance(), [ = ] {
        DDBusSender()
                ...
                .arg(QString("ocean-control-center"))                                            // appname
