...
{
    m_oceanLauncherVisible = visible;
}
...
bool TaskManager::isWindowDockOverlapK(WindowInfoBase *info)
{
    ...
    QStringList appList = {"ocean-desktop", "ocean-lock", "ocean-shutdown"};
...
{
    if (!m_activeWindow || m_oceanLauncherVisible || m_trayGridWidgetVisible || m_popupVisible)
        return false;

    if (!m_isWayland) {
        XWindow activeWinId = m_activeWindow->getXid();

        // ocean launcher is invisible, but it is still active window
        WMClass winClass = XCB->getWMClass(activeWinId);
        if (winClass.instanceName.size() > 0 && winClass.instanceName.c_str() == oceanLauncherWMClass) {
            qInfo() << "shouldHideOnSmartHideMode: active window is ocean launcher";
            return false;
        }
...
void TaskManager::attachOrDetachWindow(WindowInfoBase *info)
{
    ...
    // 在新增窗口后，同步最近打开应用到com.lingmo.ocean.dock.json的DConfig配置文件中
    updateRecentApps();
}
...
{
    return m_oceanLauncherVisible || m_popupVisible || m_trayGridWidgetVisible;
}
