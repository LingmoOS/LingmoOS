...
AppInfo *WindowIdentify::identifyWindowWayland(WindowInfoK *winInfo, QString &innerId)
{
    ...
    if (winInfo->getAppId() == "ocean-desktop" && m_taskmanager->shouldShowOnDock(winInfo)) {
        winInfo->setAppId("ocean-file-manager");
    }
...
AppInfo *WindowIdentify::identifyWindowByWmClass(TaskManager *_taskmanager, WindowInfoX *winInfo, QString &innerId)
{
    ...
    if (wmClass.instanceName.size() > 0) {
        ...
        // try app id org.lingmo.flatdeb.brackets
        //ret = new AppInfo("org.lingmo.flatdeb." + QString(wmClass.instanceName.c_str()).toLower());
        if (DesktopInfo("org.lingmo.flatdeb." + QString(wmClass.instanceName.c_str()).toLower()).isValidDesktop()) {
            AppInfo *appInfo = new AppInfo("org.lingmo.flatdeb." + QString(wmClass.instanceName.c_str()).toLower());
            innerId = appInfo->getInnerId();
            return appInfo;
        }
