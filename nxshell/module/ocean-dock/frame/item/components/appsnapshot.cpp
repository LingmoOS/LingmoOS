...
void AppSnapshot::fetchSnapshot()
{
    ...
    if (isKWinAvailable()) {
        ...
        do {
            // get window image from shm(only for lingmo app)
            QImage qimage;
            info = getImageDSHM();
            if (info) {
...
SHMInfo *AppSnapshot::getImageDSHM()
{
    ...
    Atom atom_prop = XInternAtom(display, "_LINGMO_DXCB_SHM_INFO", true);
    if (!atom_prop) {
        return nullptr;
    }

    Atom actual_type_return_lingmo_shm;
    int actual_format_return_lingmo_shm;
    unsigned long nitems_return_lingmo_shm;
    unsigned long bytes_after_return_lingmo_shm;
    unsigned char *prop_return_lingmo_shm;

    XGetWindowProperty(display, m_wid, atom_prop, 0, 32 * 9, false, AnyPropertyType,
                       &actual_type_return_lingmo_shm, &actual_format_return_lingmo_shm, &nitems_return_lingmo_shm,
                       &bytes_after_return_lingmo_shm, &prop_return_lingmo_shm);

    //qDebug() << actual_type_return_lingmo_shm << actual_format_return_lingmo_shm << nitems_return_lingmo_shm << bytes_after_return_lingmo_shm << prop_return_lingmo_shm;

    return reinterpret_cast<SHMInfo *>(prop_return_lingmo_shm);
}
