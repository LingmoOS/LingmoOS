    ...
    , m_lastWidget(nullptr)
{
    ...
    if (Utils::IS_WAYLAND_DISPLAY)
        QDBusConnection::sessionBus().connect("com.lingmo.ocean.lockFront", "/com/lingmo/ocean/lockFront", "com.lingmo.ocean.lockFront", "Visible", "b", this, SLOT(hide()));
}
...
void DockPopupWindow::onButtonPress(int type, int x, int y, const QString &key)
{
    ...
    if (auto focus = qApp->focusWidget()) {
        ...
        if (className == "QLineEdit") {
            qDebug() << "PopupWindow window will not be hioceann";
            return;
        }
    }
