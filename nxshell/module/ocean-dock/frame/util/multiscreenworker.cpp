// Copyright (C) 2018 ~ 2020 Lingmo Technology Co., Ltd.
...
MultiScreenWorker::MultiScreenWorker(QObject *parent)
    ...
    , m_appearanceInter(new Appearance("org.lingmo.ocean.Appearance1", "/org/lingmo/ocean/Appearance1", QDBusConnection::sessionBus(), this))
 ...
 * org.lingmo.ocean.XEventMonitor1服务比dock晚启动，导致dock启动后的状态错误
 */
...
void MultiScreenWorker::tryToShowDock(int eventX, int eventY)
{
    ...
    if (toScreen != currentScreen) {
        if (!m_delayWakeTimer->isActive()) {
            ...
            m_delayWakeTimer->start(Utils::SettingValue("com.lingmo.ocean.dock.mainwindow", "/com/lingmo/ocean/dock/mainwindow/", MonitorsSwitchTime, 2000).toInt());
        }
    } else {
        // 任务栏隐藏状态，但需要显示
