...
const QString g_cfgPath = QStandardPaths::standardLocations(QStandardPaths::ConfigLocation)[0] + "/ocean-cfg.ini";
...
[[noreturn]] void sig_crash(int sig)
{
    ...
    dir.cdUp();
    QString filePath = dir.path() + "/ocean-collapse.log";
    ...
    QSettings settings(g_cfgPath, QSettings::IniFormat);
    settings.beginGroup("ocean-dock");
...
int main(int argc, char *argv[])
{
    ...
    if (currentDesktop.compare("OCEAN", Qt::CaseInsensitive) == 0 ||
        currentDesktop.compare("lingmo", Qt::CaseInsensitive) == 0) {
    ...
    // NOTE: ocean-shell load ocean-dock plugins and mark loader program DS_APP_ID as ocean-dock
    ...
    app.setApplicationName("ocean-dock");
    app.setApplicationDisplayName("OCEAN Dock");
    ...
    // 启动入参 ocean-dock --help可以看到一下内容， -x不加载插件 -r 一般用在startocean启动任务栏
    QCommandLineOption disablePlugOption(QStringList() << "x" << "disable-plugins", "do not load plugins.");
    QCommandLineOption runOption(QStringList() << "r" << "run-by-starocean", "run by startocean.");
    QCommandLineParser parser;
    parser.setApplicationDescription("OCEAN Dock");
    ...
    if (!app.setSingleInstance(QString("ocean-dock_%1").arg(getuid()))) {
    ...
    QDBusConnection::sessionBus().registerObject("/org/lingmo/ocean/Dock1", "org.lingmo.ocean.Dock1", &windowManager);

    //保证dock daemon接口兼容性,ocean-desktop,ocean-clipboard,lingmo-system-monitor,ocean-osd等在调用。
    QDBusConnection::sessionBus().registerService("org.lingmo.ocean.daemon.Dock1");
    QDBusConnection::sessionBus().registerObject("/org/lingmo/ocean/daemon/Dock1", "org.lingmo.ocean.daemon.Dock1", &windowManager);
