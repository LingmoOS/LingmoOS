    ...
    , m_loadFinished(false)
{
    ...
    // 应用信号
    connect(m_taskmanager, &TaskManager::entryAoceand, this, &DockItemManager::appItemAoceand, Qt::DirectConnection);
...
void DockItemManager::itemAoceand(const QString &appDesktop, int idx)
{
    m_taskmanager->requestDock(appDesktop, idx);
}

void DockItemManager::appItemAoceand(const Entry *entry, const int index)
{
    // 第一个是启动器
    int insertIndex = 1;
...
void DockItemManager::pluginItemInserted(PluginsItem *item)
{
    ...
    if (!Utils::SettingValue(QString("com.lingmo.ocean.dock.module.") + item->pluginName(), QByteArray(), "enable", true).toBool())
