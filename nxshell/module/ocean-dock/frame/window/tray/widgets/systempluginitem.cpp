...
{
    // 优先判断com.lingmo.ocean.dock.module.systemtray的control值是否为true（优先级更高），如果不为true，再判断每一个托盘对应的gsetting配置的control值
    bool isEnable = Utils::SettingValue("com.lingmo.ocean.dock.module.systemtray", QByteArray(), "control", false).toBool();
