...
void WindowManager::sendNotifications()
{
    ...
    QVariantMap hints;
    hints["x-lingmo-action-reload"] = QString("dbus-send,--session,--dest=org.lingmo.ocean.Dock1,--print-reply,/org/lingmo/ocean/Dock1,org.lingmo.ocean.Dock1.ReloadPlugins");
 ...
 * 部分设置未初始化完等，导致任务栏显示的界面异常，所以留下此接口，被startocean延后调用
 */
...
void WindowManager::callShow()
{
    ...
    // 预留200ms提供给窗口初始化再通知startocean，不影响启动速度
...
void WindowManager::onRequestUpdateFrontendGeometry()
{
    ...
    // org.lingmo.ocean.daemon.Dock1的SetFrontendWindowRect接口设置区域时,此区域的高度或宽度不能为0,否则会导致其HideState属性循环切换,造成任务栏循环显示或隐藏
