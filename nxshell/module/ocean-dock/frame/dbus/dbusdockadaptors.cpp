...
DBusDockAdaptors::DBusDockAdaptors(WindowManager* parent)
    ...
    , m_gsettings(Utils::SettingsPtr("com.lingmo.ocean.dock.mainwindow", QByteArray(), this))
...
{
    return Utils::SettingValue("com.lingmo.ocean.dock.mainwindow", QByteArray(), "onlyShowPrimary", false).toBool();
}
...
void DBusDockAdaptors::setShowInPrimary(bool showInPrimary)
{
    ...
    if (Utils::SettingSaveValue("com.lingmo.ocean.dock.mainwindow", QByteArray(), "onlyShowPrimary", showInPrimary)) {
...
bool DBusDockAdaptors::isPluginValid(const QString &name)
{
    ...
    if (!Utils::SettingValue("com.lingmo.ocean.dock.module." + name, QByteArray(), "enable", true).toBool())
        return false;
    ...
    if (name == "lingmo-screen-recorder-plugin")
        return false;
...
QIcon DBusDockAdaptors::getSettingIcon(PluginsItemInterface *plugin, QSize &pixmapSize, DGuiApplicationHelper::ColorType colorType) const
{
    ...
    if (icon.isNull()) {
        icon = QIcon(":/icons/resources/oceanui_dock_plug_in.svg");
