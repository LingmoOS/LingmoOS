...
import (
	"bufio"
	config "lingmo-upgrade-manager/pkg/config/upgrader"
	"lingmo-upgrade-manager/pkg/logger"
	"lingmo-upgrade-manager/pkg/module/bootkitinfo"
	"lingmo-upgrade-manager/pkg/module/chroot"
	"lingmo-upgrade-manager/pkg/module/dirinfo"
	"lingmo-upgrade-manager/pkg/module/fstabinfo"
	"lingmo-upgrade-manager/pkg/module/generator"
	"lingmo-upgrade-manager/pkg/module/grub"
	"lingmo-upgrade-manager/pkg/module/langselector"
	"lingmo-upgrade-manager/pkg/module/mountinfo"
	"lingmo-upgrade-manager/pkg/module/mountpoint"
	"lingmo-upgrade-manager/pkg/module/notify"
	"lingmo-upgrade-manager/pkg/module/plymouth"
	"lingmo-upgrade-manager/pkg/module/records"
	"lingmo-upgrade-manager/pkg/module/repo"
	"lingmo-upgrade-manager/pkg/module/repo/branch"
...
const (
	...
	SelfRecordStatePath    = "/etc/lingmo-upgrade-manager/state.records"
	LocalNotifyDesktopPath = "/usr/share/lingmo-upgrade-manager/lingmo-upgrade-manager-tool.desktop"
	AutoStartDesktopPath   = "/etc/xdg/autostart/lingmo-upgrade-manager-tool.desktop"
...
func (c *Upgrader) Commit(newVersion, subject string, useSysData bool,
	...
	if len(newVersion) == 0 {
		...
		if err != nil {
			logger.Warning("failed add version, from lingmo boot kit, err:", err)
...
func (c *Upgrader) Rollback(version string,
	...
	if isCanRollback && len(backVersion) != 0 {
		...
		out, err := exec.Command("/usr/sbin/lingmo-boot-kit", "--action=mkinitrd").CombinedOutput()
		if err != nil {
...
func (c *Upgrader) AfterRollbackOper(backVersion string, isSuccessful bool) error {
	...
	// save log to /var/log/lingmo-upgrade-manager
...
func (c *Upgrader) SendSystemNotice() error {
	...
	const atomicUpgradeDest = "org.lingmo.AtomicUpgrade1"
	const atomicUpgradePath = "/org/lingmo/AtomicUpgrade1"
	...
	if len(backMsg) != 0 {
		...
		const selfRuning = "/usr/bin/lingmo-upgrade-manager-tool --action=notify"
