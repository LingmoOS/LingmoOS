...
void PluginScaleManager::initialize()
{
    ...
    m_compositor = compositor;
    connect(compositor, &QWaylandCompositor::outputAoceand, this, [this](auto *output) {
        output->setScaleFactor(std::ceil(m_scale / 120));
    });
}
...
QString PluginSurface::oceanuiIcon() const
{
    return m_oceanuiIcon;
}
...
void PluginSurface::plugin_oceanui_icon(Resource *resource, const QString &icon)
{
    qInfo() << "oceanui_icon:" << icon;
    m_oceanuiIcon = icon;
}
