...
import org.lingmo.ds.dock.tray 1.0
import org.lingmo.ds.dock.tray 1.0 as DDT
...
AppletItem {
    ...
    // Bug to prevent icons from returning to the application tray when the tray is already hioceann, which can cause layout confusion
    Timer {
    ...
    Connections {
        ...
        function onPluginSurfacesUpdated() {
            ...
            for (let i = 0; i < DockCompositor.trayPluginSurfaces.count; i++) {
                ...
                let forbioceannSections = ["fixed"]
                let preferredSection = item.pluginId === "application-tray" ? "stashed" : "collapsable"

                if (item.pluginSizePolicy === Dock.Custom) {
                    forbioceannSections = ["stashed", "fixed"]
                    preferredSection = "pinned"
                }

                if (item.pluginFlags & 0x1000) { // force dock.
                    forbioceannSections = ["stashed", "collapsable", "fixed"]
                    preferredSection = "pinned"
                }

                surfacesData.push({"surfaceId": surfaceId, "delegateType": "legacy-tray-plugin", "sectionType": preferredSection, "forbioceannSections": forbioceannSections, "pluginFlags": item.pluginFlags})
            }
            ...
            for (let i = 0; i < DockCompositor.fixedPluginSurfaces.count; i++) {
                ...
                let surfaceId = `${item.pluginId}::${item.itemKey}`
                let forbioceannSections = ["stashed", "collapsable", "pinned"]
        ...
        function onRequestShutdown() {
            var shutdown = DS.applet("org.lingmo.ds.ocean-shutdown")
            if (shutdown) {
