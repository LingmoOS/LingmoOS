...
TraySortOrderModel::TraySortOrderModel(QObject *parent)
    ...
    , m_dconfig(Dtk::Core::DConfig::create("org.lingmo.ocean.shell", "org.lingmo.ds.dock.tray"))
{
    ...
    defaultRoleNames.insert({
        ...
        {TraySortOrderModel::ForbioceannSectionsRole, QByteArrayLiteral("forbioceannSections")}
    });
    setItemRoleNames(defaultRoleNames);

    // init sort order data and hioceann list data
    loadDataFromDConfig();
    ...
    connect(m_dconfig.get(), &Dtk::Core::DConfig::valueChanged, this, [this](const QString &key){
        if (key == QLatin1String("hioceannSurfaceIds")) {
            loadDataFromDConfig();
            updateVisualIndexes();
        }
    });
...
bool TraySortOrderModel::dropToStashTray(const QString &draggedSurfaceId, int dropVisualIndex, bool isBefore)
{
    ...
    if (draggedItem->data(ForbioceannSectionsRole).toStringList().contains(SECTION_STASHED)) return false;
...
bool TraySortOrderModel::dropToDockTray(const QString &draggedSurfaceId, int dropVisualIndex, bool isBefore)
{
    ...
    QStringList forbioceannSections(draggedItems[0]->data(ForbioceannSectionsRole).toStringList());
    ...
    // If it's hioceann, remove it from the hioceann list. updateVisualIndexes() will update the
    ...
    if (m_hioceannIds.contains(draggedSurfaceId)) {
        m_hioceannIds.removeOne(draggedSurfaceId);
    }
    ...
    if (dropOnSurfaceId == QLatin1String("internal/action-toggle-collapse")) {
        ...
        if (isBefore) {
            ...
            if (forbioceannSections.contains(SECTION_COLLAPSABLE)) return false;
            ...
            if (forbioceannSections.contains(SECTION_PINNED)) return false;
            if (targetSection == sourceSection) {
    ...
    if (forbioceannSections.contains(targetSectionName)) return false;
...
{
    if (visible) {
        if (m_hioceannIds.contains(surfaceId)) {
            m_hioceannIds.removeOne(surfaceId);
        }
    } else {
        if (!m_hioceannIds.contains(surfaceId)) {
            m_hioceannIds.append(surfaceId);
        }
    }
    updateVisualIndexes();
}
...
{
    return !m_hioceannIds.contains(surfaceId);
}
...
QString TraySortOrderModel::findSection(const QString &surfaceId, const QString &fallback, const QStringList &forbioceannSections, int pluginFlags)
{
    ...
    if (forbioceannSections.contains(result)) {
...
QStandardItem *TraySortOrderModel::createTrayItem(const QString &name,
                                                  ...
                                                  const QStringList &forbioceannSections,
                                                  int pluginFlags)
{
    QString actualSectionType = findSection(name, sectionType, forbioceannSections, pluginFlags);
    ...
    item->setData(forbioceannSections, TraySortOrderModel::ForbioceannSectionsRole);
...
void TraySortOrderModel::updateVisualIndexes()
{
    ...
    for (const QString & id : std::as_const(m_stashedIds)) {
        ...
        bool itemVisible = (pluginFlags & Dock::Attribute_ForceDock) || !(pluginFlags & Dock::Attribute_ForceDock) || !m_hioceannIds.contains(id);
    ...
    for (const QString & id : std::as_const(m_collapsableIds)) {
        ...
        if (results[0]->data(TraySortOrderModel::VisualIndexRole).toInt() != -1) continue;
        auto pluginFlags = results[0]->data(TraySortOrderModel::PluginFlagsRole).toInt();
        bool itemVisible = (pluginFlags & Dock::Attribute_ForceDock) || !(pluginFlags & Dock::Attribute_CanSetting) || !m_hioceannIds.contains(id);
    ...
    for (const QString & id : std::as_const(m_pinnedIds)) {
        ...
        bool itemVisible = (flags & Dock::Attribute_ForceDock) || !(flags & Dock::Attribute_CanSetting) || !m_hioceannIds.contains(id);
    ...
    // move to other sections. We archive that by setting the 'forbioceannSections' property
...
QString TraySortOrderModel::registerSurfaceId(const QVariantMap & surfaceData)
{
    ...
    QStringList forbioceannSections(surfaceData.value("forbioceannSections").toStringList());
    ...
    if (!results.isEmpty()) {
        ...
        // check if the item is currently in a forbioceann zone
        QString currentSection(result->data(SectionTypeRole).toString());
        if (forbioceannSections.contains(currentSection)) {
            result->setData(findSection(surfaceId, preferredSection, forbioceannSections, pluginFlas), SectionTypeRole);
        }
        return surfaceId;
    }

    appendRow(createTrayItem(surfaceId, preferredSection, delegateType, forbioceannSections, pluginFlas));
    return surfaceId;
}
...
void TraySortOrderModel::loadDataFromDConfig()
{
    ...
    m_hioceannIds = m_dconfig->value("hioceannSurfaceIds").toStringList();
...
void TraySortOrderModel::saveDataToDConfig()
{
    ...
    m_dconfig->setValue("hioceannSurfaceIds", m_hioceannIds);
