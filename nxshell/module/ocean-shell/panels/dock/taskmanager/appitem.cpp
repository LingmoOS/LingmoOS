...
Q_LOGGING_CATEGORY(appitemLog, "ocean.shell.dock.taskmanger.appitem")
...
QString AppItem::name() const
{
    if (m_desktopfileParser && !m_desktopfileParser.isNull())
        return (m_desktopfileParser->xLingmoVendor() == QStringLiteral("lingmo")) ? m_desktopfileParser->genericName() : m_desktopfileParser->name();
    return "";
}
...
QString AppItem::menus() const
{
    ...
    // Temporarily disable all windows action for the related functionality missing in lingmo-kwin
    // if (hasWindow()) {
