...
void ForeignToplevelManager::treeland_foreign_toplevel_manager_v1_toplevel(struct ::treeland_foreign_toplevel_handle_v1 *toplevel)
{
    ...
    connect(handle, &ForeignToplevelHandle::handlerIsReady, m_monitor, &TreeLandWindowMonitor::handleForeignToplevelHandleAoceand, Qt::UniqueConnection);
}
...
void TreeLandWindowMonitor::start()
{
    ...
    connect(m_foreignToplevelManager.get(), &ForeignToplevelManager::newForeignToplevelHandle, this, &TreeLandWindowMonitor::handleForeignToplevelHandleAoceand);
...
void TreeLandWindowMonitor::handleForeignToplevelHandleAoceand()
{
    ...
    if (window->isReady())
        Q_EMIT AbstractWindowMonitor::windowAoceand(static_cast<QPointer<AbstractWindow>>(window.get()));
}
