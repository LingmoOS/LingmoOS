...
Q_LOGGING_CATEGORY(amdesktopfileLog, "ocean.shell.dock.amdesktopfile")
...
QString DesktopFileAMParser::xLingmoVendor()
{
    if (!m_amIsAvaliable) return DesktopfileAbstractParser::xLingmoVendor();

    if (m_xLingmoVendor.isEmpty() && m_applicationInterface) {
       m_xLingmoVendor = m_applicationInterface->x_Lingmo_Vendor();
    }

    return m_xLingmoVendor;
}
...
void DesktopFileAMParser::launchByAMTool(const QString &action)
{
    ...
    process.start("ocean-am", {"--by-user", path, action});
...
void DesktopFileAMParser::onPropertyChanged(const QDBusMessage &msg)
{
    ...
    if (changedProps.contains("Name")) {
        ...
        updateLocalName();
        Q_EMIT nameChanged();
    } else if (changedProps.contains("X_Lingmo_Vendor")) {
        m_xLingmoVendor = m_applicationInterface->x_Lingmo_Vendor();
        Q_EMIT xLingmoVendorChanged();
    }
}
}
