...
NotificationSetting::NotificationSetting(QObject *parent)
    : QObject(parent)
    , m_impl(Dtk::Core::DConfig::create("org.lingmo.ocean.shell", "org.lingmo.ocean.shell.notification", QString(), this))
{
...
void NotificationSetting::onAppsChanged()
{
    ...
    QList<NotificationSetting::AppItem> aoceand;
    ...
    for (auto item : aoceand) {
        qDebug(notifyLog) << "Application aoceand" << item.id;
        Q_EMIT appAoceand(item.id);
    }
