...
Q_LOGGING_CATEGORY(layershell, "ocean.shell.layershell")
    ...
    , m_dlayerShellWindow(DLayerShellWindow::get(m_window))
{
    ...
    connect(qApp, &QGuiApplication::screenAoceand, this, [this] (const QScreen *newScreen) {
...
void LayerShellEmulation::onLayerChanged()
{
    ...
    switch (m_dlayerShellWindow->layer()) {
        ...
        case DLayerShellWindow::LayerOverlay: {
            // on lingmo Notification will be influenced by exclusionZone,
            // while plasma works all right, maybe lingmo kwin bug?
            // FIXME: fix above
            m_window->setFlags(m_window->flags() & ~Qt::WindowStaysOnBottomHint);
