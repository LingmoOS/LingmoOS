...
int main(int argc, char *argv[])
{
    ...
    app.setApplicationName("lingmo-compressor");
    app.loadTranslator();
    app.setApplicationVersion(DApplication::buildVersion(QDate::currentDate().toString("yyyyMMdd")));
    app.setApplicationAcknowledgementPage("https://www.lingmo.org/original/lingmo-compressor/");
    app.setProductIcon(QIcon::fromTheme("lingmo-compressor"));
    ...
    // 设置应用图标
    QIcon appIcon = QIcon::fromTheme("lingmo-compressor");

    if (appIcon.isNull()) {
        appIcon = QIcon(":assets/icons/lingmo/builtin/icons/lingmo-compressor.svg");
    }
    ...
    parser.setApplicationDescription("Lingmo Compressor.");
        ...
        if (iParamCount == 1) {
            ...
            if (strType == "dragdropadd") {
                ...
                for (int i = 1; i < iParamCount; i++) {
                    ...
                    p->setProgramPath(QStandardPaths::findExecutable("lingmo-compressor"));
