...
//        if (desktop_info.getFileName().contains("lingmo-compressor")) {
//            continue;
//        }
...
void OpenWithDialog::initData()
{
    ...
    foreach (const QString &f, mimeAppsManager->DesktopObjs.keys()) {
        ...
        const QString &custom_open_desktop = desktop_info.value("X-OCEAN-File-Manager-Custom-Open").toString();
...
void OpenWithDialog::slotUseOtherApplication()
{
    ...
    if (file_path.endsWith(".desktop")) {
        ...
        desktop.set("X-OCEAN-File-Manager-Custom-Open", m_mimeType.name());
