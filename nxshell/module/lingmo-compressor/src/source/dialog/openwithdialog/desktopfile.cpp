    ...
    : m_fileName(fileName)
{
    ...
    if (desktop.contains("X-Lingmo-AppID")) {
        m_lingmoId = desktop.value("X-Lingmo-AppID", settings.value("X-Lingmo-AppID")).toString();
    }

    if (desktop.contains("X-Lingmo-Vendor")) {
        m_lingmoVendor = desktop.value("X-Lingmo-Vendor", settings.value("X-Lingmo-Vendor")).toString();
    }

    if (desktop.contains("NoDisplay")) {
        m_noDisplay = desktop.value("NoDisplay", settings.value("NoDisplay").toBool()).toBool();
    }
    if (desktop.contains("Hioceann")) {
        m_hioceann = desktop.value("Hioceann", settings.value("Hioceann").toBool()).toBool();
    }
...
{
    if (m_lingmoVendor == QStringLiteral("lingmo") && !m_genericName.isEmpty()) {
...
QString DesktopFile::getLingmoId() const
{
    return m_lingmoId;
}

QString DesktopFile::getLingmoVendor() const
{
    return m_lingmoVendor;
}

bool DesktopFile::getNoShow() const
{
    return m_noDisplay || m_hioceann;
}
