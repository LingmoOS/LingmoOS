...
void SettingDialog::initUI()
{
    ...
    m_settings = DSettings::fromJsonFile(":assets/data/lingmo-compressor.json");

    const QString confDir = DStandardPaths::writableLocation(QStandardPaths::AppConfigLocation); // 换了枚举值，待验证

    const QString confPath = confDir + QDir::separator() + "lingmo-compressor.conf";
...
void SettingDialog::writeConfbf()
{
    ...
    const QString confPath = confDir + QDir::separator() + "lingmo-compressor.confbf";
