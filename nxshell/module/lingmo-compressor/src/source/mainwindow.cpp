...
#include "oceansktopservicesthread.h"
...
void MainWindow::initTitleBar()
{
    ...
    // 设置应用程序图标
    QIcon icon = QIcon::fromTheme("lingmo-compressor");
...
{
    QIcon icon = UiTools::renderSVG(":assets/icons/lingmo/builtin/icons/compress_success_30px.svg", QSize(30, 30));
...
void MainWindow::handleJobErrorFinished(ArchiveJob::JobType eJobType, ErrorType eErrorType)
{
    ...
    case ArchiveJob::JT_Add: {
        ...
        if (StartupType::ST_DragDropAdd != m_eStartupType) {
            m_ePageID = PI_UnCompress;
            QIcon icon = UiTools::renderSVG(":assets/icons/lingmo/builtin/icons/compress_fail_128px.svg", QSize(30, 30));
            switch (eErrorType) {
            // 密码错误
...
void TitleWidget::slotThemeChanged()
{
    ...
    if (DGuiApplicationHelper::LightType == themeType) {    // 浅色
        icon.addFile(":assets/icons/lingmo/builtin/icons/compress_information_15px.svg");
    } else if (DGuiApplicationHelper::DarkType == themeType) {  // 深色
        icon.addFile(":assets/icons/lingmo/builtin/icons/compress_information_dark.svg");
    } else {        // 其它默认
        icon.addFile(":assets/icons/lingmo/builtin/icons/compress_information_15px.svg");
    }
