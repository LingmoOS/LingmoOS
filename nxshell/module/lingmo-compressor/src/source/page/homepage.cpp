...
void HomePage::initUI()
{
    ...
    // 设置首页图标
    m_pixmap = UiTools::renderSVG(":assets/icons/lingmo/builtin/icons/compress_folder_128px.svg", QSize(128, 128));
...
void HomePage::slotThemeChanged()
{
    ...
    if (DGuiApplicationHelper::LightType == themeType) {    // 浅色
        m_pSplitLbl->setPixmap(QPixmap(":assets/icons/lingmo/builtin/light/icons/split_line.svg"));
    } else if (DGuiApplicationHelper::DarkType == themeType) {  // 深色
        m_pSplitLbl->setPixmap(QPixmap(":assets/icons/lingmo/builtin/dark/icons/split_line_dark.svg"));
    } else {        // 其它默认
        m_pSplitLbl->setPixmap(QPixmap(":assets/icons/lingmo/builtin/light/icons/split_line.svg"));
    }
}
