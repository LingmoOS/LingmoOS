...
./bin/tests/lingmo-compressor_test --gtest_output=xml:./report/report_lingmo-compressor.xml
...
mv ./html/index.html ./html/cov_lingmo-compressor.html
#对asan、ut、代码覆盖率结果收集至指定文件夹
cp -r html ../$reportdir/
cp -r report ../$reportdir/
cp -r asan*.log* ../$reportdir/asan_lingmo-compressor.log
