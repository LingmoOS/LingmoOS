...
TEST_F(UT_HomePage, test_slotThemeChanged_001)
{
    ...
    EXPECT_EQ(m_tester->m_pSplitLbl->pixmap()->toImage() == QPixmap(":assets/icons/lingmo/builtin/light/icons/split_line.svg").toImage(), true);
}
...
TEST_F(UT_HomePage, test_slotThemeChanged_002)
{
    ...
    m_tester->slotThemeChanged();
    EXPECT_EQ(m_tester->m_pSplitLbl->pixmap()->toImage() == QPixmap(":assets/icons/lingmo/builtin/dark/icons/split_line_dark.svg").toImage(), true);
}
