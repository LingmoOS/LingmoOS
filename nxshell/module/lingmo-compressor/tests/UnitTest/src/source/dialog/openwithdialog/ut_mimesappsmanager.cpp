...
TEST_F(UT_MimeAppsWorker, test_readData)
{
    ...
    strPath += "/src/desktoplingmo-compressor.desktop";
...
TEST_F(UT_MimesAppsManager, test_getOCEANMimeTypeFile)
{
    EXPECT_EQ(m_tester->getOCEANMimeTypeFile().isEmpty(), false);
}
...
TEST_F(UT_MimesAppsManager, test_loadOCEANMimeTypes)
{
    m_tester->loadOCEANMimeTypes();
}
