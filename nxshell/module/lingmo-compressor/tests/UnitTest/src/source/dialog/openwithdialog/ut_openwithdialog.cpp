...
{
    EXPECT_EQ(m_tester->getProgramPathByExec("/usr/bin/lingmo-compressor"), "/usr/bin/lingmo-compressor");
}

TEST_F(UT_OpenWithDialog, test_getProgramPathByExec_003)
{
    EXPECT_EQ(m_tester->getProgramPathByExec("lingmo-compressor"), QStandardPaths::findExecutable("/usr/bin/lingmo-compressor"));
}
...
TEST_F(UT_OpenWithDialog, testslotUseOtherApplication)
{
    ...
    Stub stub1;
    CommonStub::stub_QFileDialog_getOpenFileName(stub1, QStandardPaths::findExecutable("lingmo-compressor"));
    m_tester->slotUseOtherApplication();
}
