...
void CliPlugin::continueMoving(bool result)
{
    ...
    case Delete:
        m_subOperation = Add;
        if (!setMovingAoceandFiles() || !addFiles(m_tempAoceandFiles, m_passedDestination, m_passedOptions)) {
            finishMoving(false);
        }
        break;
    case Add:
...
bool CliPlugin::setMovingAoceandFiles()
{
    ...
    m_tempAoceandFiles << new Archive::Entry(nullptr, m_passedDestination->name());
