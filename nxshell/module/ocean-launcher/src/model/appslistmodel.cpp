...
static QGSettings *gSetting = SettingsPtr("com.lingmo.ocean.launcher", "/com/lingmo/ocean/launcher/");
...
{
    //从先/etc/lingmo-installer.conf读取不可卸载软件列表
    const QSettings settings("/etc/lingmo-installer.conf", QSettings::IniFormat);
    auto holds_list = settings.value("ocean_launcher_hold_packages").toStringList();
...
AppsListModel::AppsListModel(const AppCategory &category, QObject *parent)
    ...
    , m_actionSettings(SettingsPtr("com.lingmo.ocean.launcher.menu", "/com/lingmo/ocean/launcher/menu/", this))
