...
QGSettings *AppsManager::m_launcherSettings = SettingsPtr("com.lingmo.ocean.launcher", "", nullptr);
QSet<QString> AppsManager::APP_AUTOSTART_CACHE;
QSettings AppsManager::APP_USED_SORTED_LIST("lingmo", "ocean-launcher-app-used-sorted-list");
QSettings AppsManager::APP_CATEGORY_USED_SORTED_LIST("lingmo","ocean-launcher-app-category-used-sorted-list");
...
void AppsManager::registerSettingsFormat()
{
    ...
    m_collectedSetting = new QSettings(JsonFormat, QSettings::UserScope, "lingmo","ocean-launcher-app-collect-list", this);
    m_categorySetting = new QSettings(JsonFormat, QSettings::UserScope, "lingmo","ocean-launcher-app-category-used-sorted-list", this);
    m_fullscreenUsedSortSetting = new QSettings(JsonFormat, QSettings::UserScope, "lingmo","ocean-launcher-fullscreen-app-used-sorted-list", this);
    m_windowedUsedSortSetting = new QSettings(JsonFormat, QSettings::UserScope, "lingmo", "ocean-launcher-windowed-app-used-sorted-list", this);
}
...
AppsManager::AppsManager(QObject *parent)
    ...
    , m_autostartDesktopListSetting(new QSettings("lingmo", AUTOSTART_KEY, this))
    ...
    , m_dropRow(0)
{
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.launcher")) {
        m_filterSetting = new QGSettings("com.lingmo.ocean.launcher", "/com/lingmo/ocean/launcher/");
...
void AppsManager::loadDefaultFavoriteList(const ItemInfoList_v1 &processList)
{
    ...
    appKeyList.append("ocean-control-center");
    appKeyList.append("ocean-file-manager");
    appKeyList.append("lingmo-defender");
    appKeyList.append("lingmo-mail");
...
void AppsManager::refreshCategoryInfoList()
{
    ...
    QStringList filters = SettingValue("com.lingmo.ocean.launcher", "/com/lingmo/ocean/launcher/", "filter-keys").toStringList();
    ...
    if (m_appInfos.isEmpty()) {
        for (int categoryIndex = AppsListModel::Internet; categoryIndex < static_cast<int>(AppsListModel::Others); categoryIndex++) {
            ...
            if (categoryIndex == static_cast<int>(AppsListModel::System) && QFileInfo::exists(APP_CATEGORY_USED_SORTED_LIST.fileName())) {
                ...
                qDebug() << "remove ocean-launcher-app-category-used-sorted-list conf file success!";
            }
        }
    }
...
void AppsManager::updateTrashIconFromInfoList()
{
    auto updateTrashIcon = [=](ItemInfoList_v1 &processList) {
        for (ItemInfo_v1 &info : processList) {
            if (info.m_key == QLatin1String("ocean-trash")) {
...
void AppsManager::readCollectedCacheData()
{
    const QString &filePath = QStandardPaths::writableLocation(QStandardPaths::ConfigLocation)
            + QString("/%1/ocean-launcher-app-collect-list.json").arg(qApp->organizationName());
