...
QDebug operator<<(QDebug argument, const ItemInfo_v3 &info)
{
    argument << "actionName: " << info.m_actionName
             ...
             << ", X_Lingmo_Vendor:" << info.m_X_Lingmo_Vendor
...
bool ItemInfo_v3::operator==(const ItemInfo_v3 &other) const
{
    return (this->m_actionName == other.m_actionName) &&
           ...
           (this->m_X_Lingmo_Vendor == other.m_X_Lingmo_Vendor) &&
    ...
    , m_objManagerDbusInter(new AppManagerDBusProxy(this))
{
    ...
    connect(m_objManagerDbusInter, &AppManagerDBusProxy::InterfacesAoceand, this, [this] (const QDBusObjectPath &objectPath, ObjectInterfaceMap objs){
        qDebug() << "InterfacesAoceand:" << objectPath.path() << objs;
...
void AMInter::buildAppDBusConnection()
{
    for (auto path : m_infos.keys()) {
        ...
        connect(appInter, &ApplicationDBusProxy::InterfacesAoceand, this, &AMInter::onInterfaceAoceand);
...
void AMInter::onInterfaceAoceand(const QDBusObjectPath &objectPath, const ObjectInterfaceMap &interfaces)
{
    Q_UNUSED(interfaces)
...
{
    if (isAutoStarted) {
        qDebug() << "autostartChanged aoceand" << appId;
        Q_EMIT autostartChanged("aoceand", appId);
    } else {
...
bool AMInter::lastoreUninstall(const QString &appId)
{
    ...
    QDBusInterface lastoreDbus("org.lingmo.ocean.Lastore1",
...
ItemInfo_v3 AMInter::itemInfoV3(const ObjectInterfaceMap &objs) const
{
    ...
    for (auto objIter = objs.begin(); objIter != objs.end(); ++objIter) {
        ...
        for (auto interPropIter = interProps.begin(); interPropIter != interProps.end(); ++interPropIter) {
            ...
            } else if (key == "X_Lingmo_Vendor") {
                info_v3.m_X_Lingmo_Vendor =
...
ItemInfo_v2 AMInter::itemInfoV2(const ItemInfo_v3 &itemInfoV3)
{
    ...
    if (itemInfoV3.m_X_Lingmo_Vendor == "lingmo") {
