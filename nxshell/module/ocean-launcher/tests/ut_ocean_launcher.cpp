...
TEST_F(LauncherUnitTest, testStartManagerDBus)
{
    ...
    // 检测是否自启动
    QString appName = QString("lingmo-editor");
...
{
    QDBusInterface inter("com.lingmo.ocean.Lanucher",
                         "/com/lingmo/ocean/Lanucher",
                         "com.lingmo.ocean.Lanucher",
...
TEST_F(LauncherUnitTest, check_gsettings_default)
{
    ...
    QGSettings setting("com.lingmo.ocean.launcher", "/com/lingmo/ocean/launcher/");
...
{
    QDBusInterface launcherBusInter("com.lingmo.ocean.daemon.Launcher"
                            ,"/com/lingmo/ocean/daemon/Launcher"
                            ,"org.freedesktop.DBus.Properties"
                            ,QDBusConnection::sessionBus(), this);
    QVERIFY(launcherBusInter.isValid());

    QDBusReply<QVariant> reply = launcherBusInter.call("Get", "com.lingmo.ocean.daemon.Launcher", "Fullscreen");

    bool fullScreen = QVariant(reply).toBool();
    if (fullScreen == false) {
        launcherBusInter.call("Set", "com.lingmo.ocean.daemon.Launcher", "Fullscreen", true);
        QTest::qWait(100);

        QDBusReply<QVariant> replyGetFullscreen = launcherBusInter.call("Get", "com.lingmo.ocean.daemon.Launcher", "Fullscreen");
        fullScreen = QVariant(replyGetFullscreen).toBool();

        QCOMPARE(fullScreen, true);
    } else {
        launcherBusInter.call("Set", "com.lingmo.ocean.daemon.Launcher", "Fullscreen", false);
        QTest::qWait(100);
    ...
    QDBusPendingReply<> reqUninstall = launcherBusInter.call("RequestUninstall", "lingmo-editor", false);
    reqUninstall.waitForFinished();

    QDBusPendingReply<> setProxyReply = launcherBusInter.call("SetUseProxy", "lingmo-editor", false);
    setProxyReply.waitForFinished();

    QDBusPendingReply<bool> getProxyReply = launcherBusInter.call("GetUseProxy", "lingmo-editor");
    getProxyReply.waitForFinished();

    QVERIFY(getProxyReply.argumentAt(0).toBool());

    QDBusPendingReply<> setDisableScaleReply = launcherBusInter.call("SetDisableScaling", "lingmo-editor", false);
    setDisableScaleReply.waitForFinished();

    QDBusPendingReply<bool> getScaleReply = launcherBusInter.call("GetDisableScaling", "lingmo-editor");
