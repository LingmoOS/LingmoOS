...
1. 确保已经安装所有依赖库。
   ...
   _不同发行版的软件包名称可能不同，如果您的发行版提供了ocean-grand-search，请检查发行版提供的打包脚本。_
...
可执行程序为 `/usr/bin/ocean-grand-search` 和 `/usr/bin/ocean-grand-search-daemon`。
...
ocean-grand-search 在 [GPL-3.0-or-later](LICENSE.txt)下发布。
