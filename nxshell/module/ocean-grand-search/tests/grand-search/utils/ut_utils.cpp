...
TEST(UtilsTest, setWeightMethod)
{
    ...
    item = {"a", "a", "a", "a", GRANDSEARCH_CLASS_FILE_LINGMO, QVariant::fromValue(weight)};
    ...
    // 类目为文件
    item = {"a", "a", "a", "a", GRANDSEARCH_CLASS_FILE_LINGMO, QVariant()};
    result = Utils::setWeightMethod(item);
    QVariantHash hash;
...
TEST(UtilsTest, packageBestMatch)
{
    // todo:最佳匹配增加了应用与设置，该用例需要补充完善
     ...
     // 匹配项中包含文件、应用、设置
     for (int i = 0; i < 5; ++i) {
         item = {"a", "a", "a", "testa", GRANDSEARCH_CLASS_FILE_LINGMO, extra};
         items.append(item);
     }
     map.insert(GRANDSEARCH_GROUP_FILE, items);
     items.clear();
...
TEST(UtilsTest, appIconName)
{
    ...
    QString defaultDesktopFile("/usr/share/applications/ocean-control-center.desktop");
    ...
    // 结果为文件
    item.searcher = GRANDSEARCH_CLASS_FILE_LINGMO;
...
{
    // 后缀判断
    QString path("/usr/share/applications/ocean-control-center.desktop");
...
TEST(UtilsTest, launchAppByGio)
{
    ...
    desktopFile = "/usr/share/applications/ocean-file-manager.desktop";
