...
do
  if [ "$argcommand" = "--help" ] ; then
        ...
        echo "--ut:指定ut项目类型，可以使用类型：no all ocean-grand-search libocean-grand-search-daemon preview-plugin ocean-grand-search-dock-plugin"
...
UT_COMMAND="all"; #运行UT类型 no all ocean-grand-search libocean-grand-search-daemon
REBUILD_PTJ="yes";
...
echo `pwd`
echo "start ocean-grand-search project all CI cases"
