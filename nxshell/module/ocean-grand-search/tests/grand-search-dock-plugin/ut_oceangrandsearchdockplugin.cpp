...
#include "oceangrandsearchdockplugin.h"
...
TEST_F(TestDdeGrandSearchDockPlugin, init)
{
    ...
    if (QGSettings::isSchemaInstalled("com.lingmo.ocean.dock.module.grand-search"))
...
TEST_F(TestDdeGrandSearchDockPlugin, itemSortKey)
{
    ...
    EXPECT_EQ(1, plugin->itemSortKey("ocean-grand-search"));
}
...
TEST_F(TestDdeGrandSearchDockPlugin, setSortKey)
{
    ...
    plugin->setSortKey("ocean-grand-search",1);

    EXPECT_CALL(mock_proxy,getValue).WillOnce(testing::Return(1));

    EXPECT_EQ( plugin->itemSortKey("ocean-grand-search"), 1);
}
