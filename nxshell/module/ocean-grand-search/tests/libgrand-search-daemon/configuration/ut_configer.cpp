...
TEST(UserPreference, ut_initDefault)
{
    ...
    EXPECT_NE(conf.d->m_root->group(GRANDSEARCH_CLASS_FILE_LINGMO).get(), nullptr);
#else
    EXPECT_EQ(conf.d->m_root->group(GRANDSEARCH_CLASS_FILE_LINGMO).get(), nullptr);
#endif
...
#ifdef ENABLE_LINGMOANYTHING
    EXPECT_TRUE(up->value(GRANDSEARCH_CLASS_FILE_LINGMO, false));
#else
    EXPECT_FALSE(up->value(GRANDSEARCH_CLASS_FILE_LINGMO, false));
#endif
