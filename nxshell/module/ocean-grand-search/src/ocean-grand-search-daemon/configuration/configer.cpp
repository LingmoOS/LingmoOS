...
bool ConfigerPrivate::updateConfig1(QSettings *set)
{
    ...
    //文件搜索相关配置
    {
        //初始化文件搜索的子类目
    #ifdef ENABLE_LINGMOANYTHING
        if (UserPreferencePointer conf = m_root->group(GRANDSEARCH_CLASS_FILE_LINGMO)) {
            ...
            //设置是否启用文件搜索项
            searcherConfig->setValue(GRANDSEARCH_CLASS_FILE_LINGMO, on);
        } else {
            qWarning() << "no shuch config:" << GRANDSEARCH_CLASS_FILE_LINGMO;
        }
    #endif
...
void Configer::initDefault()
{
    ...
    //初始化文件搜索的子类目
#ifdef ENABLE_LINGMOANYTHING
    rootData.insert(GRANDSEARCH_CLASS_FILE_LINGMO, QVariant::fromValue(d->fileSearcher()));
#endif
