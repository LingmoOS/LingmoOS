...
void DesktopAppSearcherPrivate::createIndex(DesktopAppSearcherPrivate *d)
{
    ...
    for (auto app = apps.begin(); app != apps.end(); ++app) {
        ...
        info->name = value->oceanDisplayName();
        info->item = app.key();
...
QSet<QString> DesktopAppSearcherPrivate::desktopIndex(const DesktopEntryPointer &app, const QString &locale)
{
    ...
    static const QString XLingmoVendor = "X-Lingmo-Vendor";
    static const QString lingmo = "lingmo";

    //ocean名称,X-Lingmo-Vendor=lingmo
    bool useGeneric = app->stringValue(XLingmoVendor) == lingmo;
