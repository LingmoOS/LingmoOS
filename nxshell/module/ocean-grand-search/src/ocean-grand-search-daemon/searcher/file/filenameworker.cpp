...
{
    // 获取支持的搜索类目
    auto config = ConfigerIns->group(GRANDSEARCH_CLASS_FILE_LINGMO);
...
void FileNameWorkerPrivate::initAnything()
{
    ...
    m_anythingInterface = new ComLingmoAnythingInterface("com.lingmo.anything",
                                                         "/com/lingmo/anything",
...
bool FileNameWorkerPrivate::searchByAnything()
{
    ...
    while (!isResultLimit() && !m_searchDirList.isEmpty()) {
        ...
        if (result.error().type() != QDBusError::NoError) {
            qWarning() << "lingmo-anything search failed:"
        ...
        if (!m_supportParallelSearch)
            searchResults = searchResults.filter(hioceannFileFilter);
