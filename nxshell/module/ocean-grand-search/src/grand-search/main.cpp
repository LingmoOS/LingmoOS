...
int main(int argc, char *argv[])
{
    ...
    app.setApplicationName("ocean-grand-search");
    app.setApplicationDisplayName("OCEAN Grand Search");
    ...
    if (isSetting) { // 设置界面
        // 设置单例运行程序
        if (!app.setSingleInstance("ocean-grand-search-config")) {
        ...
        // 设置单例运行程序
        if (!app.setSingleInstance("ocean-grand-search")) {
            qWarning() << "set single instance failed!I (pid:" << getpid() << ") will exit.";
            return -1;
        }
