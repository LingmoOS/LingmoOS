...
void GroupWidget::showLabel(bool bShow)
{
    ...
    if (bShow && m_searchGroupName == GRANDSEARCH_GROUP_FILE_INFERENCE) {
        // 有无索引文件
        QString idxDir = QStandardPaths::writableLocation(QStandardPaths::HomeLocation) + "/.cache/lingmo/lingmo-ai-daemon/index";
...
void GroupWidget::onOpenConfig(const QString& link)
{
    if (link == "update index") {
        ...
        QProcess::startDetached("ocean-grand-search -s --position aiconfig");
    } else if (link == "config") {
        QProcess::startDetached("ocean-grand-search -s --position aiconfig");
    }
}
