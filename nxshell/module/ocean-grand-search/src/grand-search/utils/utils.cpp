...
#else
static const QString SessionManagerService = "org.lingmo.ocean.SessionManager1";
static const QString StartManagerPath = "/org/lingmo/StartManager1";
static const QString StartManagerInterface = "org.lingmo.ocean.StartManager1";
#endif
...
bool Utils::setWeightMethod(MatchedItem &item)
{
    ...
    if (search == GRANDSEARCH_CLASS_FILE_LINGMO ||
...
void Utils::packageBestMatch(MatchedItemMap &map, int maxQuantity)
{
    ...
    static const QMap<QString, bool> supprotedSeracher = {
        {GRANDSEARCH_CLASS_FILE_LINGMO, true},
...
QString Utils::appIconName(const MatchedItem &item)
{
    ...
    if (item.searcher == GRANDSEARCH_CLASS_APP_DESKTOP) {
        ...
        auto defaultDesktopFile = "/usr/share/applications/ocean-control-center.desktop";
...
{
    if (GRANDSEARCH_CLASS_FILE_LINGMO == item.searcher || GRANDSEARCH_CLASS_FILE_FSEARCH == item.searcher) {
...
bool Utils::openFile(const MatchedItem &item)
{
    ...
    QString defaultDesktopFile = getDefaultAppDesktopFileByMimeType(mimetype);
    if (defaultDesktopFile.isEmpty()) {
        result = QProcess::startDetached(QString("ocean-file-manager"), {QString("-o"), fileUrlString});
...
QIcon Utils::defaultIcon(const MatchedItem &item)
{
    ...
    else if (item.searcher == GRANDSEARCH_CLASS_FILE_LINGMO || item.searcher == GRANDSEARCH_CLASS_FILE_FSEARCH) {
