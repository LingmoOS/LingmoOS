...
#检查有没有lingmo-terminal终端被安装
dt=`which lingmo-terminal`
echo $dt
if [ ! $dt ] ; then
    echo "没有发现 lingmo-terminal 终端被安装" 
    exit 0
fi


version=`lingmo-terminal -v`

version=$(echo $version | sed -e 's/\(.*\)lingmo-terminal\(.*\)/\2/g' -e 's/ //g')

if [ ! "$version" ]; then
#  version=5.2.19
  echo "lingmo-terminal 没有获取到相应的版本号"
  exit 0
fi
...
#检检查是不是使用lingmo-terminal终端
pid=$(ps -eo pid,ppid,sid,comm | grep 'lingmo-terminal' | awk '{print $1;}')

if [ $pid ]; then
    echo "为了准确性,关闭lingmo-terminal 终端,使用其他终端测试" 
    exit 0
fi

dtl=~/.cache/lingmo/lingmo-terminal/lingmo-terminal.log
if [ -f $dtl ] ; then
    rm -rf $dtl
    sleep 0.2
fi
...
do
   ...
   # if [ $i == 1 ]; then
   #     pid=$(ps -eo pid,comm | grep 'lingmo-terminal' | awk '{print $1;}')
...
termpid=$(ps -eo pid,comm | grep 'lingmo-terminal' | awk '{print$1;}')
...
do
 #   curcpu=$(top -n1 | grep "lingmo-terminal" | awk '{print $10;}')
#    curcpu=$(ps -eo pcpu,comm | grep "lingmo-terminal" | awk '{print $1;}')
    curcpu=`top -n 1 -d 1 -p $userpid | grep lingmo | awk '{print $10;}'`
#    echo "====================$curcpu=================="
     if [ $curcpu ] ; then
