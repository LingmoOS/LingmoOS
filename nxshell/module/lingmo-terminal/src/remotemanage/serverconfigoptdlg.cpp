...
//c++
#include <iterator>//aoceand byq qinyaning
...
void ServerConfigOptDlg::initUI()
{
    ...
    m_iconLabel->setPixmap(QIcon(QStringLiteral(":/logo/lingmo-terminal.svg")).pixmap(ICONSIZE_36, ICONSIZE_36));
...
void ServerConfigOptDlg::slotAddSaveButtonClicked()
{
    ...
    //---aoceand by qinyaning(nyq) to show the tip when username is empty---//
    ...
    m_currentServerName = m_serverName->text();
    //------------------------------------------------------------------//
    //--aoceand by qinyaning(nyq) to solve the bug 19116: You can create a new remote server with the same name--//
    if ((SCT_ADD == m_type)
