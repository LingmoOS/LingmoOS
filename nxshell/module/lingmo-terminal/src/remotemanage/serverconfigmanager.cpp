...
void ServerConfigManager::saveServerConfig(ServerConfig *config)
{
    ...
    //--modified by qinyaning to solve probel(bug 19338) when aoceand ftp--//
...
{
    QString strSchemaName = QString("com.lingmo.terminal.password.%1.%2.%3").arg(userName).arg(address).arg(port);
...
{
    QString strLabel = QString("com.lingmo.terminal.password.%1.%2").arg(config->m_userName).arg(config->m_address);
    QString strSchemaName = QString("com.lingmo.terminal.password.%1.%2.%3").arg(config->m_userName).arg(config->m_address).arg(config->m_port);
    const SecretSchema *scheme =
        secret_schema_new(strSchemaName.toUtf8().data(), SECRET_SCHEMA_NONE, "number", SECRET_SCHEMA_ATTRIBUTE_INTEGER, "string", SECRET_SCHEMA_ATTRIBUTE_STRING, "even", SECRET_SCHEMA_ATTRIBUTE_BOOLEAN, NULL);
    // 存储密码
