...
void MainWindow::initWindow()
{
    ...
    setWindowIcon(QIcon::fromTheme("lingmo-terminal"));
...
void MainWindow::showExitConfirmDialog(Utils::CloseType type, int count, QWidget *parent)
{
    ...
    dlg->setIcon(QIcon::fromTheme("lingmo-terminal"));
    ...
    if (QEvent::KeyPress == event->type() && currentActivatedTerminal()) {
        ...
        if (Qt::Key_Return == keyEvent->key()  || Qt::Key_Enter == keyEvent->key()) {
            ...
            if (term->enterSzCommand() && term->isActiveWindow()) {
                //--aoceand by qinyaning(nyq) to slove Unable to download file from server, time: 2020.4.13 18:21--//
                // 先用sz获取输入的文件
                pressEnterKey("\nsz \"${files[@]}\"");
                //-------------------------------------
                // 执行下载操作
...
void NormalWindow::initTitleBar()
{
    ...
    titlebar()->setTitle("");
    titlebar()->setIcon(QIcon::fromTheme("lingmo-terminal"));
...
{
    //--aoceand by qinyaning(nyq) to solve the problem of can't show center--//
 ...
 * 和 窗管沟通后 定义_LINGMO_FORHIBIT_MOVE 属性（1：禁止移动,0:可以移动,默认：可以移动）
 * sendWindowForhibitMove(true)，通过调用xcb相关函数，设置_LINGMO_FORHIBIT_MOVE属性值为1,禁止窗口移动
 */
void QuakeWindow::sendWindowForhibitMove(bool forhibit)
{
    ...
    auto reply = internAtom("_LINGMO_FORHIBIT_MOVE");
