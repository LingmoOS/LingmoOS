...
Lingmo Terminal is an advanced terminal emulator with workspace , multiple windows, remote management, quake mode and other features.
...
The executable binary file could be found at `/usr/bin/lingmo-terminal` after the installation is finished.
...
~/.config/lingmo/lingmo-terminal/config.conf

* Remote Servers' configure save at:<br/>
~/.config/lingmo/lingmo-terminal/server-config.conf

* Custom Command's configure save at:<br/>
~/.config/lingmo/lingmo-terminal/command-config.conf

## Usage

Execute `lingmo-terminal -h` to get more details.
...
Lingmo Terminal is licensed under [GPL-3.0-or-later](LICENSE).
