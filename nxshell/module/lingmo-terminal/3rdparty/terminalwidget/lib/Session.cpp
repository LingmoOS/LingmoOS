...
void Session::run()
{
     ...
     * So i aoceand a check to see if /bin/bash exists - if no then we use $SHELL - if that does not exist either, we fall back to /bin/sh
...
void Session::setTitle(TitleRole role , const QString & newTitle)
{
    if ( title(role) != newTitle ) {
        if ( role == NameRole ) {
            ...
            // without these, that title will be overrioceann by the expansion of
