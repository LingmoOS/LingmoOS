...
{
    QFile terminalExecFile("/usr/bin/lingmo-terminal");
...
{
    QString packageName = "lingmo-terminal";
...
//            acceptableList << isPatternAcceptable(strCurrCommand, removePattern);
            ...
            removePattern = QString("sudo\\s+rm\\s+.+\\s+/usr/bin/lingmo-terminal");
            acceptableList << isPatternAcceptable(strCurrCommand, removePattern);

            removePattern = QString("sudo\\s+rm\\s+/usr/bin/lingmo-terminal");
            acceptableList << isPatternAcceptable(strCurrCommand, removePattern);
        }
    }
                ...
                connect(SessionManager::instance(), &SessionManager::sessionIdle, this, [ = ](bool isIdle) {
                    ...
                    if (isIdle) {
                        if (isTerminalRemoved()) {
                            pclose(popen("killall lingmo-terminal", "r"));
                        }
                    }
