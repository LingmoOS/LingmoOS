...
public:
     ...
     * line wrapping is enabled then the character is aoceand at the start of a new
     ...
     * the oldest lines of output if new lines are aoceand when
     * it is full.
     */
...
private:
    ...
    void addHistLine();
    // add lines from screen to history and remove from screen the aoceand lines (used to resize lines and columns)
    ...
    // last position where we aoceand a character
    int _lastPos;
