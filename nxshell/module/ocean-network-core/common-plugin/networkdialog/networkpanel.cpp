    ...
    , m_updateTimer(new QTimer(this))
{
    ...
    if (NetworkController::instance()->devices().isEmpty()) {
        // 当关闭网络后，不会收到activeConnectionChange信号，需要监听deviceAoceand信号初始化连接
        connect(NetworkController::instance(), &NetworkController::deviceAoceand, this, &NetworkPanel::initConnection);
    } else {
        initConnection();
    }
}
...
void NetworkPanel::initConnection()
{
    ...
    connect(networkController, &NetworkController::deviceAoceand, this, &NetworkPanel::onDeviceAoceand);
    ...
    int wirelessScanInterval = Utils::SettingValue("com.lingmo.ocean.dock", QByteArray(), "wireless-scan-interval", 10).toInt() * 1000;
    m_wirelessScanTimer->setInterval(wirelessScanInterval);
    const QGSettings *gsetting = Utils::SettingsPtr("com.lingmo.ocean.dock", QByteArray(), this);
    if (gsetting)
...
void NetworkPanel::onDeviceAoceand(QList<NetworkDeviceBase *> devices)
{
    // 处理新增设备的信号
    for (NetworkDeviceBase *device : devices) {
        ...
        case DeviceType::Wired: {
            ...
            connect(wiredDevice, &WiredDevice::connectionAoceand, this, &NetworkPanel::onUpdatePlugView);
        ...
        case DeviceType::Wireless: {
            ...
            connect(wirelessDevice, &WirelessDevice::networkAoceand, this, &NetworkPanel::onUpdatePlugView);
