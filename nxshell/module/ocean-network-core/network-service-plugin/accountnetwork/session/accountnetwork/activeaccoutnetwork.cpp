...
#define SYSTEMNETWORKSERVICE "org.lingmo.service.SystemNetwork"
...
#define DEAMON_NETWORK_SERVICE "com.lingmo.daemon.Network"
...
void ActiveAccountNetwork::activeNetwork(const QMap<QString, QString> &network, const QVariantMap &authenInfo)
{
    ...
    if (m_networkregisted && m_secretregisted) {
        ...
        if (!m_networkregisted) {
            qCWarning(DSM) << "com.lingmo.daemon.Network not start,wait to it start";
