...
#define ACCOUNT_SERVICE "com.lingmo.daemon.Accounts"
#define ACCOUNT_PATH "/com/lingmo/daemon/Accounts"
#define ACCOUNT_INTERFACE "com.lingmo.daemon.Accounts"
    ...
    , m_networkConfig(conf)
{
    ...
    for (const QString &user : userLists) {
        QDBusInterface userInterface(ACCOUNT_SERVICE, user, "com.lingmo.daemon.Accounts.User", QDBusConnection::systemBus());
...
QStringList AccountManager::primaryAccount() const
{
    ...
    for (const QString &userPath : userLists) {
        QDBusInterface interface(ACCOUNT_SERVICE, userPath, "com.lingmo.daemon.Accounts.User", QDBusConnection::systemBus());
...
void AccountManager::onUserAoceand(const QString &path)
{
    ...
    QDBusInterface interface(ACCOUNT_SERVICE, path, "com.lingmo.daemon.Accounts.User", QDBusConnection::systemBus());
