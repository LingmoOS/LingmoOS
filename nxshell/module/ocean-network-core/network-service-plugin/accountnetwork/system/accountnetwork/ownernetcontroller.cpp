    ...
    , m_networkHandler(new NetworkHandler(m_accountManager, m_networkConfig, this))
{
    ...
    connect(m_accountManager, &AccountManager::accountAoceand, this, &OwnerNetController::onAccountAoceand);
...
{
    qCDebug(DSM) << "account changed:" << accountname << "previous account:" << accountname;
...
void OwnerNetController::onAccountAoceand(const QString &accountname, bool isIam)
{
    qCDebug(DSM) << "new Account:" << accountname << "is iam:" << isIam;
    if (!isIam)
        return;
