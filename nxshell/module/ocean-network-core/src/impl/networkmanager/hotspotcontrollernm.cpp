...
void HotspotController_NM::onConnectionAoceand(const QString &connectionUni)
{
    ...
    qCDebug(DNC) << "connection aoceand:" << connectionUni;
...
void HotspotController_NM::updateDevices(const QList<NetworkDeviceBase *> &devices)
{
    ...
    for (WirelessDevice *device : newDevices) {
        ...
        // 监听设备连接发生变化的信号
        connect(wirelessDevice.data(), &NetworkManager::WirelessDevice::availableConnectionAppeared, this, &HotspotController_NM::onConnectionAoceand, Qt::UniqueConnection);
    ...
    if (newHotspotItems.size() > 0)
        emit itemAoceand(newHotspotItems);
