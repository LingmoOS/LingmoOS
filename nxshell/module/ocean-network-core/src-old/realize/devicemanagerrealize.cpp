...
{
    QDBusInterface dbusInter("org.lingmo.ocean.Network1", "/org/lingmo/ocean/Network1", "org.lingmo.ocean.Network1", QDBusConnection::systemBus());
...
void DeviceManagerRealize::onWiredConnectionChanged()
{
    ...
    if (newConnection.size() > 0 || rmConnection.size() > 0) {
        if (newConnection.size() > 0)
            Q_EMIT connectionAoceand(newConnection);
