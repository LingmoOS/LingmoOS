...
#include <networkmanagerqt/wireoceanvice.h>
...
const static QString networkService = "org.lingmo.ocean.Network1";
const static QString networkPath = "/org/lingmo/ocean/Network1";
...
NetworkManagerProcesser::NetworkManagerProcesser(QObject *parent)
    ...
    , m_connectivity(ocean::network::Connectivity::Unknownconnectivity)
...
{
    connect(NetworkManager::notifier(), &Notifier::deviceAoceand, this, &NetworkManagerProcesser::onDeviceAoceand);
    ...
    QDBusConnection::systemBus().connect("org.lingmo.ocean.Network1", "/org/lingmo/ocean/Network1", "org.lingmo.ocean.Network1", "DeviceEnabled", this, SLOT(onDeviceEnabledChanged(QDBusObjectPath, bool)));
}
...
ocean::network::Connectivity NetworkManagerProcesser::connectivity()
{
...
void NetworkManagerProcesser::onDeviceAoceand(const QString &uni)
{
        ...
        if (device->managed() && (hasFlag || device->type() == Device::Wifi)) {
            ...
            if (!deviceExist(device->uni())) {
                ...
                if (newDevice) {
                    ...
                    sortDevice();
                    updateDeviceName();
                    Q_EMIT deviceAoceand({ newDevice });
                }
            }
        } else {
...
{
    ocean::network::Connectivity ctity;
    switch (conntity) {
    case NetworkManager::Connectivity::Full: {
        ctity = ocean::network::Connectivity::Full;
        break;
    }
    case NetworkManager::Connectivity::Portal: {
        ctity = ocean::network::Connectivity::Portal;
        break;
    }
    case NetworkManager::Connectivity::Limited: {
        ctity = ocean::network::Connectivity::Limited;
        break;
    }
    case NetworkManager::Connectivity::NoConnectivity: {
        ctity = ocean::network::Connectivity::Noconnectivity;
        break;
    }
    case NetworkManager::Connectivity::UnknownConnectivity: {
        ctity = ocean::network::Connectivity::Unknownconnectivity;
        break;
    }
    }
