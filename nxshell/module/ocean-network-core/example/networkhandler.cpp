...
void NetworkHandler::initConnection()
{
    ...
    connect(controller, &NetworkController::deviceAoceand, this, &NetworkHandler::onDeviceAoceand);
    ...
    connect(vpnController, &VPNController::itemAoceand, this, &NetworkHandler::onItemAoceand);
    ...
    connect(hotspotController, &HotspotController::itemAoceand, this, &NetworkHandler::onHotspotChanged);
    ...
    onDeviceAoceand(controller->devices());
}
...
void NetworkHandler::onDeviceAoceand(QList<NetworkDeviceBase *> devices)
{
    outputMessage("received DeviceAoceand signal:");
    for (int i = 0; i < devices.size(); i++) {
        NetworkDeviceBase *device = devices[i];
        ...
        if (device->deviceType() == DeviceType::Wireless) {
            ...
            connect(wirelessDevice, &WirelessDevice::networkAoceand,
                    this, &NetworkHandler::onNetworkAoceand);
            connect(wirelessDevice, &WirelessDevice::networkAoceand,
...
void NetworkHandler::onItemAoceand(const QList<VPNItem *> vpns)
{
...
void NetworkHandler::onNetworkAoceand(QList<AccessPoints *> newAps)
{
