...
void HotspotPlugin::initConnection() {
  ...
  connect(NetworkManager::notifier(), &NetworkManager::Notifier::deviceAoceand, this,
...
const QString HotspotPlugin::itemCommand(const QString &itemKey) {
  ...
  return QString("dbus-send --print-reply --dest=org.lingmo.ocean.ControlCenter1 /org/lingmo/ocean/ControlCenter1 org.lingmo.ocean.ControlCenter1.ShowPage string:network/personalHotspot");
}
...
{
    if (isEnabled)
        m_proxyInter->itemAoceand(this, hotspot_key);
    else
        m_proxyInter->itemRemoved(this, hotspot_key);
}
