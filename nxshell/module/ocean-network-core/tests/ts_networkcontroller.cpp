...
TEST_F(Tst_NetworkController, dslcontroll_test)
{
    ...
    auto onDeviceChange = [ = ](QList<NetworkDeviceBase *> changeDevices) {
        ...
        for (NetworkDeviceBase *device : devices) {
            ...
            if (device->deviceType() == DeviceType::Wired) {
                ...
                QObject::connect(wirelessDevice, &WirelessDevice::networkAoceand, [ = ] (QList<AccessPoints *> accessPoints) {
                    qInfo() << "new networkAoceand:";
                    for (AccessPoints *ap : accessPoints)
                        qInfo() << "accessPoints aoceand:" << ap->ssid();
    ...
    QObject::connect(m_controller, &NetworkController::deviceAoceand, onDeviceChange);
