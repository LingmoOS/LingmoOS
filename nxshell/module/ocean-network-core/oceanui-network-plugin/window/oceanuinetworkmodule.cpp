...
NetworkModule::NetworkModule(QObject *parent)
    : HListModule("network", tr("Network"), QString(), QIcon::fromTheme("oceanui_nav_network"), parent)
{
...
void NetworkModule::init()
{
    if (m_modules.isEmpty()) {
        connect(NetworkController::instance(), &NetworkController::deviceAoceand, this, &NetworkModule::updateModel);
        connect(NetworkController::instance(), &NetworkController::deviceRemoved, this, &NetworkModule::updateModel);
        connect(NetworkController::instance()->hotspotController(), &HotspotController::deviceAoceand, this, &NetworkModule::updateVisiable);
...
OceanUINetworkPlugin::OceanUINetworkPlugin(QObject *parent)
    : PluginInterface(parent)
    , m_moduleRoot(nullptr)
{
}

OceanUINetworkPlugin::~OceanUINetworkPlugin()
{
    m_moduleRoot = nullptr;
}

QString OceanUINetworkPlugin::name() const
{
    return QStringLiteral("oceanui-network-plugin");
}

ModuleObject *OceanUINetworkPlugin::module()
{
    if (m_moduleRoot)
        return m_moduleRoot;
...
QString OceanUINetworkPlugin::location() const
{
    return "6";
}
