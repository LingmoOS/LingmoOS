...
WiredModule::WiredModule(WiredDevice *dev, QObject *parent)
    : PageModule("wired", dev->deviceName(), QString(), QIcon::fromTheme("oceanui_ethernet"), parent)
    , m_device(dev)
{
...
void WiredModule::initWirelessList(DListView *lvProfiles)
{
    ...
    updateItems();
    connect(m_device, &WiredDevice::connectionAoceand, model, updateItems);
...
void WiredModule::editConnection(ocean::network::ControllItems *item, QWidget *parent)
{
