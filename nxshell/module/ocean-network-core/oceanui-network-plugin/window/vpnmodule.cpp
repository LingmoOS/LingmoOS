...
VPNModule::VPNModule(QObject *parent)
    : PageModule("networkVpn", tr("VPN"), tr("VPN"), QIcon::fromTheme("oceanui_vpn"), parent)
{
    m_modules.append(new WidgetModule<SwitchWidget>("wired_adapter", tr("Wired Network Adapter"), [](SwitchWidget *vpnSwitch) {
        ...
        updateVisible();
        connect(NetworkController::instance()->vpnController(), &VPNController::itemAoceand, vpnSwitch, updateVisible);
    ...
    ModuleObject *extraImport = new WidgetModule<FloatingButton>("addWired", tr("Add Network Connection"), [this](FloatingButton *importVpnBtn) {
        importVpnBtn->setIcon(QIcon::fromTheme("oceanui_vpn_import"));
...
void VPNModule::initVPNList(DListView *vpnView)
{
    ...
    updateItems();
    connect(vpnController, &VPNController::itemAoceand, model, updateItems);
