...
WirelessDeviceModel::WirelessDeviceModel(const WirelessDevice *dev, QWidget *parent)
    ...
    , m_device(dev)
    , m_parent(parent)
    , m_hioceannItem(new ItemAction(nullptr))
{
    m_hioceannItem->iconAction->setIcon(m_parent->style()->standardIcon(QStyle::SP_ArrowRight));
    connect(m_hioceannItem->iconAction, &DViewItemAction::triggered, this, &WirelessDeviceModel::onDetailTriggered);
    m_hioceannItem->item->setText(tr("Connect to hioceann network"));

    addAccessPoints(m_device->accessPointItems());
    connect(m_device, &WirelessDevice::networkAoceand, this, &WirelessDeviceModel::addAccessPoints);
...
QVariant WirelessDeviceModel::data(const QModelIndex &index, int role) const
{
    ...
    if (row >= m_data.size()) {
        return m_hioceannItem->item->data(role);
    }
    ...
    case Qt::DecorationRole: {
        ...
        QString icon = ap->type() == AccessPoints::WlanType::wlan6 ? QLatin1String("oceanui_wireless6-") : QLatin1String("oceanui_wireless-");
        if (strength > 65)
            icon += "8";
        else if (strength > 55)
            icon += "6";
...
void WirelessDeviceModel::removeAccessPoints(QList<ocean::network::AccessPoints *> aps)
{
    for (AccessPoints *ap : aps) {
