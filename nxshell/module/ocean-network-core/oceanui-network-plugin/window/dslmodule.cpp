...
DSLModule::DSLModule(QObject *parent)
    : PageModule("networkDsl", tr("DSL"), tr("DSL"), QIcon::fromTheme("oceanui_dsl"), parent)
{
...
void DSLModule::initDSLList(DListView *lvsettings)
{
    ...
    updateItems();
    connect(dslController, &DSLController::itemAoceand, model, updateItems);
...
void DSLModule::editConnection(ocean::network::ControllItems *item, QWidget *parent)
{
    QString devPath = "/";
    QString connUuid;
    if (item) {
