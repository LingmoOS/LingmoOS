...
NetworkModule::NetworkModule(QObject *parent)
    ...
    , m_initilized(false)
{
    QDBusConnection::sessionBus().connect("org.lingmo.ocean.LockFront1", "/org/lingmo/ocean/LockFront1", "org.lingmo.ocean.LockFront1", "Visible", this, SLOT(updateLockScreenStatus(bool)));
    m_isLockModel = (-1 == qAppName().indexOf("greeter"));
    if (!m_isLockModel) {
        ocean::network::NetworkController::setServiceType(ocean::network::ServiceLoadType::LoadFromManager);
    }
    ...
    if (m_isLockModel) {
        m_networkDialog->setServerName("ocean-network-dialog" + QString::number(getuid()) + "lock");
    } else {
        QDBusMessage lock = QDBusMessage::createMethodCall("org.lingmo.ocean.LockService1", "/org/lingmo/ocean/LockService1", "org.lingmo.ocean.LockService1", "CurrentUser");
        QDBusConnection::systemBus().callWithCallback(lock, this, SLOT(onUserChanged(QString)));
        QDBusConnection::systemBus().connect("org.lingmo.ocean.LockService1", "/org/lingmo/ocean/LockService1", "org.lingmo.ocean.LockService1", "UserChanged", this, SLOT(onUserChanged(QString)));
...
void NetworkModule::onUserChanged(QString json)
{
    ...
    if (doc.isObject()) {
        ...
        QDBusInterface user("org.lingmo.ocean.Accounts1", QString("/org/lingmo/ocean/Accounts1/User%1").arg(uid), "org.lingmo.ocean.Accounts1.User", QDBusConnection::systemBus());
    ...
    if (!m_initilized) {
        ...
        for (ocean::network::NetworkDeviceBase *device : ocean::network::NetworkController::instance()->devices()) {
...
void NetworkModule::installTranslator(QString locale)
{
    ...
    ocean::network::NetworkController::instance()->retranslate();
...
// Delay construct Network to avoid NetworkManager service hasn't ready, e.g: ocean-session-daemon.
...
NetworkPanelContainer::NetworkPanelContainer(ocean::networkplugin::NetworkDialog *dialog, QWidget *parent)
    : QWidget(parent)
