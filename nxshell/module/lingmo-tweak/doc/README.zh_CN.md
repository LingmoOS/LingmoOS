...
lingmo-tweak 会加载插件的主界面到窗口中，主界面的 QML 不能是 Window 元素。
...
lingmo-tweak 内置了执行命令、读写文件及 DBus 调用的接口。

tweak 提供了一个 Tweak 模块，在 qml 文件头部使用 `import org.lingmo.tweak 1.0` 导入 tweak 模块，就可以使用 `Tweak` 对象。
