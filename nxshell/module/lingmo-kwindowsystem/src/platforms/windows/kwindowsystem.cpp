...
//         qDebug() << "what has happened?:" << message->wParam << message->message;
        ...
        case HSHELL_WINDOWCREATED:
            KWindowSystem::s_d_func()->windowAoceand(static_cast<WId>(message->lParam));
            break;
...
void KWindowSystemPrivate::windowAoceand(WId wid)
{
//     qDebug() << "window aoceand!";
    KWindowSystem::s_d_func()->reloadStackList();
    emit KWindowSystem::self()->windowAoceand(wid);
...
void KWindowSystem::forceActiveWindow(WId win, long time)
{
    ...
    // FIXME restoring a hioceann window doesn't work: the window contents just appear white.
