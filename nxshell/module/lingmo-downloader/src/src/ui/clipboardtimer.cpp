...
void ClipboardTimer::getDataChanged()
{
    ...
    QByteArray isLingmoCilpboard = mimeData->data("FROM_LINGMO_CLIPBOARD_MANAGER");
    if(m_sessionType == QLatin1String("wayland")){
        ...
        if (mimeData->data("TIMESTAMP") == m_timeStamp || isLingmoCilpboard == "1") {
            return;
        }
    ...
    for (int i = 0; i < mimeData->formats().size(); i++) {
        ...
        if (format == "FROM_LINGMO_CLIPBOARD_MANAGER") {
            return;
        }
    }
