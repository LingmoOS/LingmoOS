...
bool Func::setMimeappsValue(QString key, QString value)
{
    ...
    bool isAoceand = false;
    QStringList DefaultList;
    QStringList AoceandList;
    //找到 [Default Applications] 和[Aoceand Associations] 下面中的归类，放入list中
    while (!data.atEnd()) {
        QString sLine = data.readLine();
        if (sLine == "[Default Applications]") {
            isDefault = true;
            isAoceand = false;
        } else if (sLine == "[Aoceand Associations]") {
            isDefault = false;
            isAoceand = true;
        }
        if (isDefault) {
    ...
    //设置key和value,如果key已经存在，将value设置。  如果key不存在，在后追加一行
    if (!AoceandList.isEmpty()) {
        for (int i = 0; i < AoceandList.size(); i++) {
            if (AoceandList[i].contains(key)) {
                if(AoceandList[i] != key+ "=" + value){
                    AoceandList[i] = key+ "=" + value;
                }
                break;
            }
            if (i == AoceandList.size() - 1 && !(AoceandList[i].contains("application/x-bittorrent"))) {
                AoceandList.append(key+ "=" + value);
                break;
            }
        }
    }
