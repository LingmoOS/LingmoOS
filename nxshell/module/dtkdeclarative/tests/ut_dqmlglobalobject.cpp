...
TEST_F(ut_DQMLGlobalObject, lingmoWebsite)
{
    ASSERT_TRUE(!pObj->lingmoWebsiteName().isEmpty());
    ASSERT_TRUE(!pObj->lingmoWebsiteLink().isEmpty());
}

TEST_F(ut_DQMLGlobalObject, lingmoDistributionOrgLogo)
{
    ASSERT_TRUE(!pObj->lingmoDistributionOrgLogo().isEmpty());
}
