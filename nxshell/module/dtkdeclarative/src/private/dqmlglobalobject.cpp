...
QString DQMLGlobalObject::lingmoWebsiteName() const
{
    D_DC(DQMLGlobalObject);

    const_cast<DQMLGlobalObjectPrivate*>(d)->ensureWebsiteInfo();

    return d->lingmoWebsiteName;
}

QString DQMLGlobalObject::lingmoWebsiteLink() const
{
    D_DC(DQMLGlobalObject);

    const_cast<DQMLGlobalObjectPrivate*>(d)->ensureWebsiteInfo();

    return d->lingmoWebsiteLink;
}

QString DQMLGlobalObject::lingmoDistributionOrgLogo() const
{
    DCORE_USE_NAMESPACE;
    ...
    return d->lingmoDistributionOrgLogo;
...
bool DQMLGlobalObject::loadTranslator()
{
    ...
    const QString dtkdeclarativeTranslationPath(OCEANCLARATIVE_TRANSLATIONS_DIR);
