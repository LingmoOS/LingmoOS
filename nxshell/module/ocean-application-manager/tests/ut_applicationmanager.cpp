...
public:
    static void SetUpTestCase()
    {
        ...
        QSharedPointer<InstanceService> instance = QSharedPointer<InstanceService>::create(
            InstancePath.path().split('/').last(), ApplicationPath.path(), QString{"/"}, QString{"OCEAN"});
    ...
    const static inline QDBusObjectPath ApplicationPath{QString{OCEANApplicationManager1ObjectPath} + "/test_2dApplication"};
...
TEST_F(TestApplicationManager, identifyService)
{
    ...
    fakeServiceProc.start("/usr/bin/systemd-run",
                          {{QString{R"(--unit=app-OCEAN-test\x2dApplication@%1.service)"}.arg(InstanceId)},
                           {"--user"},
    ...
    ObjectInterfaceMap map{{QString{InstanceInterface},
                            QVariantMap{{QString{"Application"}, ApplicationPath},
                                        ...
                                        {QString{"Launcher"}, QString{"OCEAN"}},
    ...
    ASSERT_TRUE(QProcess::startDetached("/usr/bin/systemd-run",
                                        {{"--scope"},
                                         {QString{R"(--unit=app-OCEAN-test\x2dApplication-%1.scope)"}.arg(InstanceId)},
                                         {"--user"},
                                         {QString{"--working-directory=%1"}.arg(workingDir)},
                                         {"--slice=app.slice"},
                                         {"./fake-process.sh"},
