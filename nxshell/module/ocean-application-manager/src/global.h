...
Q_DECLARE_LOGGING_CATEGORY(OCEANAMProf)
...
inline QString getApplicationLauncherBinary()
{
    static const QString bin = []() {
        auto value = qgetenv("LINGMO_APPLICATION_MANAGER_APP_LAUNCH_HELPER_BIN");
        if (value.isEmpty()) {
            return QString::fromLocal8Bit(ApplicationLaunchHelperBinary);
        }
        qWarning() << "Using app launch helper defined in environment variable LINGMO_APPLICATION_MANAGER_APP_LAUNCH_HELPER_BIN.";
...
{
    if (!appId.isEmpty()) {
        return QString{OCEANApplicationManager1ObjectPath} + "/" + escapeToObjectPath(appId);
    }
    return QString{OCEANApplicationManager1ObjectPath} + "/" + QUuid::createUuid().toString(QUuid::Id128);
}
