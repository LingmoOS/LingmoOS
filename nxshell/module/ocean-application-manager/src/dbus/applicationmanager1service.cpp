...
{
    if (!connection.registerService(OCEANApplicationManager1ServiceName)) {
    ...
    if (!registerObjectToDBus(this, OCEANApplicationManager1ObjectPath, ApplicationManager1Interface)) {
        std::terminate();
    }
    ...
    connect(
        &dispatcher, &SystemdSignalDispatcher::SystemdJobNew, this, [this](QString unitName, QDBusObjectPath systemdUnitPath) {
            ...
            // 比如 ocean-file-manager.service 如果启动的比 AM 晚，那么在 scanInstances 时不会 addInstanceToApplication
    ...
    if (auto *ptr = new (std::nothrow) PropertiesForwarder{OCEANApplicationManager1ObjectPath, this}; ptr == nullptr) {
    ...
    const auto *filename = u8"lingmo-application-manager";
    ...
    if (!value.isEmpty() && value == QString::fromLocal8Bit("wayland")) {
        for (const auto &[app, realExec] : needLaunch.asKeyValueRange()) {
         ...
         constexpr auto XSettings = "org.lingmo.ocean.XSettings1";
...
void ApplicationManager1Service::scanInstances() noexcept
{
    ...
    args << QVariant::fromValue(QStringList{"ocean*", "lingmo*"});
    ...
    for (auto &it : autostartItems) {
        ...
        if (!asApplication.isNull()) {  // modified by application manager
            ...
            if (source.desktopId().isEmpty()) {
                qWarning() << X_Lingmo_GenerateSource << "is" << appSource
...
QSharedPointer<ApplicationService> ApplicationManager1Service::addApplication(DesktopFile desktopFileSource) noexcept
{
    ...
    emit listChanged();
    emit InterfacesAoceand(application->applicationPath(), getChildInterfacesAndPropertiesFromObject(ptr));
