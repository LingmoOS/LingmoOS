...
{
    if (ApplicationFilter::hioceannCheck(entry)) {
        qDebug() << "hioceann check failed.";
        return false;
    }
...
ApplicationService::Launch(const QString &action, const QStringList &fields, const QVariantMap &options, const QString &realExec)
{
    ...
    if (terminal()) {
        ...
        execCmds.push_front("-e");           // run all original execution commands in lingmo-terminal
        execCmds.push_front("--keep-open");  // keep terminal open, prevent exit immediately
        execCmds.push_front("lingmo-terminal");
    }
    ...
    return jobManager.addJob(
        ...
        [this, binary = std::move(bin), commands = std::move(cmds)](const QVariant &variantValue) -> QVariant {
            ...
            if (resourceFile.isEmpty()) {
                newCommands.push_front(QString{R"(--unitName=app-OCEAN-%1@%2.service)"}.arg(
            ...
            newCommands.push_front(QString{R"(--unitName=OCEAN-%1@%2.service)"}.arg(this->id(), instanceRandomUUID));
            QProcess process;
            qDebug() << "launcher :"<<m_launcher<<"run with commands:" << newCommands;
...
QString ApplicationService::X_Lingmo_Vendor() const noexcept
{
    return findEntryValue(DesktopFileEntryKey, "X-Lingmo-Vendor", EntryValueType::String).toString();
}
...
bool ApplicationService::autostartCheck(const QString &filePath) const noexcept
{
    ...
    QString source = s.value(DesktopFileEntryKey, X_Lingmo_GenerateSource).value_or(DesktopEntry::Value{}).toString();
    ...
    auto hioceann = hioceannVal.value().toString();
    return hioceann.compare("false", Qt::CaseInsensitive) == 0;
}
...
void ApplicationService::setAutoStart(bool autostart) noexcept
{
    ...
    if (!m_autostartSource.m_entry.data().isEmpty()) {
        ...
        originalSource = newEntry.value(DesktopFileEntryKey, X_Lingmo_GenerateSource)
...
QStringList ApplicationService::mimeTypes() const noexcept
{
    ...
    for (auto it = cacheList.crbegin(); it != cacheList.crend(); ++it) {
        ...
        std::for_each(list.crbegin(), list.crend(), [&tmp, this](const MimeApps &app) {
            auto [aoceand, removed] = app.queryTypes(id());
            tmp.aoceand.append(std::move(aoceand));
            tmp.removed.append(std::move(removed));
        });
    };

    tmp.aoceand.removeDuplicates();
    tmp.removed.removeDuplicates();
    for (const auto &it : tmp.removed) {
        tmp.aoceand.removeOne(it);
    }

    ret.append(std::move(tmp.aoceand));
                                        ...
                                        const QString &launcher) noexcept
{
    ...
    emit InterfacesAoceand(QDBusObjectPath{objectPath}, getChildInterfacesAndPropertiesFromObject(service));
