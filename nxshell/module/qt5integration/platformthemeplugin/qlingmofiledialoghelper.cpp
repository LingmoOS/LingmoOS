...
#define DIALOG_SERVICE "com.lingmo.filemanager.filedialog"
#endif
...
DFileDialogManager *QLingmoFileDialogHelper::manager = Q_NULLPTR;
QString QLingmoFileDialogHelper::dialogService = QStringLiteral();

QLingmoFileDialogHelper::QLingmoFileDialogHelper()
{
    connect(this, &QLingmoFileDialogHelper::accept, this, [this] {
...
{
    if (auxiliaryWindow) {
        // Note that we must hide auxiliaryWindow if it's modal to avoid direct invocation of deleteLater() to QLingmoFileDialogHelper
        // When someone creates a QFileDialog, runs exec amd uses a timer to invoke QFileDialog::deleteLater later. auxiliaryWindow
        // will just be deleted but not be hioceann. Mainwindow will keep blocked by a dead modal window.
        hideAuxiliaryWindow();
...
void QLingmoFileDialogHelper::onApplicationStateChanged(Qt::ApplicationState state)
{
...
void QLingmoFileDialogHelper::onWindowActiveChanged()
{
    if (!filedlgInterface)
        return;
...
bool QLingmoFileDialogHelper::show(Qt::WindowFlags flags, Qt::WindowModality modality, QWindow *parent)
{
    ensureDialog();
    applyOptions();
    ...
    if (filedlgInterface) {
        ...
        if (modality != Qt::NonModal) {
            ...
            if (modality == Qt::ApplicationModal) {
                connect(qApp, &QGuiApplication::applicationStateChanged, this, &QLingmoFileDialogHelper::onApplicationStateChanged, Qt::UniqueConnection);
                connect(filedlgInterface, &DFileDialogHandle::windowActiveChanged, this, &QLingmoFileDialogHelper::onWindowActiveChanged, Qt::UniqueConnection);
            }
        }
    }
...
void QLingmoFileDialogHelper::exec()
{
...
void QLingmoFileDialogHelper::hide()
{
...
bool QLingmoFileDialogHelper::defaultNameFilterDisables() const
{
...
void QLingmoFileDialogHelper::setDirectory(const QUrl &directory)
{
    if (QLingmoFileDialogHelper::directory() == directory) {
        return;
    }
...
QUrl QLingmoFileDialogHelper::directory() const
{
    qCDebug(fileDialogHelper) << __FUNCTION__;
...
void QLingmoFileDialogHelper::selectFile(const QUrl &fileUrl)
{
...
QList<QUrl> QLingmoFileDialogHelper::selectedFiles() const
{
    qCDebug(fileDialogHelper) << __FUNCTION__;
...
void QLingmoFileDialogHelper::setFilter()
{
    qCDebug(fileDialogHelper) << __FUNCTION__;
...
void QLingmoFileDialogHelper::selectNameFilter(const QString &filter)
{
...
QString QLingmoFileDialogHelper::selectedNameFilter() const
{
    qCDebug(fileDialogHelper) << __FUNCTION__;
...
void QLingmoFileDialogHelper::initDBusFileDialogManager()
{
    if (manager)
        return;
    ...
    const auto *managerObjectPath = "/com/lingmo/filemanager/filedialogmanager";
    ...
    if (QDBusConnection::sessionBus().interface()->isServiceRegistered(dialogService).value()
        || !QStandardPaths::findExecutable("ocean-desktop").isEmpty()) {
        manager = new DFileDialogManager(dialogService, managerObjectPath, QDBusConnection::sessionBus());
    }
}

bool QLingmoFileDialogHelper::iAmFileDialogDBusServer()
{
...
void QLingmoFileDialogHelper::ensureDialog() const
{
    if (filedlgInterface)
        return;
    ...
    if (manager) {
        ...
        if (path.isEmpty()) {
            ...
            auxiliaryWindow->setObjectName("QLingmoFileDialogHelper_auxiliaryWindow");
            ...
            connect(filedlgInterface, &DFileDialogHandle::accepted, this, &QLingmoFileDialogHelper::accept);
            connect(filedlgInterface, &DFileDialogHandle::rejected, this, &QLingmoFileDialogHelper::reject);
            connect(filedlgInterface, &DFileDialogHandle::destroyed, this, &QLingmoFileDialogHelper::reject);
            ...
            connect(heartbeatTimer, &QTimer::timeout, this, [this, heartbeatTimer] {
                ...
                if (reply.isError()) {
                    ...
                    filedlgInterface->QObject::deleteLater();
                    const_cast<QLingmoFileDialogHelper*>(this)->reject();
                }
            });
    ...
    if (!filedlgInterface && qobject_cast<QApplication*>(qGuiApp)) {
        QLingmoTheme::m_usePlatformNativeDialog = false;
    }
}

void QLingmoFileDialogHelper::applyOptions()
{
...
void QLingmoFileDialogHelper::hideAuxiliaryWindow() const
{
