...
static const QString LingmoClientSv = QStringLiteral("com.lingmo.lingmoid.Client");
static const QString LingmoClientPath = QStringLiteral("/com/lingmo/lingmoid/Client");
static const QString LingmoClientIf = QStringLiteral("com.lingmo.lingmoid.Client");

static const QString SyncService = QStringLiteral("com.lingmo.sync.Daemon");
static const QString SyncInterface = QStringLiteral("com.lingmo.sync.Daemon");
static const QString SyncPath = QStringLiteral("/com/lingmo/sync/Daemon");

static const QString LingmoIDService = QStringLiteral("com.lingmo.lingmoid");
static const QString LingmoIDInterface = QStringLiteral("com.lingmo.lingmoid");
static const QString LingmoIDPath = QStringLiteral("/com/lingmo/lingmoid");

static const QString UtcloudPath = QStringLiteral("/com/lingmo/utcloud/Daemon");
static const QString UtcloudInterface = QStringLiteral("com.lingmo.utcloud.Daemon");

// 根据lingmo-lingmoid-daemon决定lingmoid是否显示
static const QString LingmoidDaemonPath = QStringLiteral("/usr/lib/lingmo-lingmoid-daemon/lingmo-lingmoid-daemon");
static const QString DaemonFildPath = QStringLiteral("/usr/lib/lingmo-lingmoid-daemon");
...
SyncWorker::SyncWorker(SyncModel *model, QObject *parent)
    ...
    , m_lingmoId_inter(new LingmoIdProxy(this))
    ...
    , m_watcher(new QFileSystemWatcher(this))
{
    ...
    QDBusConnection::systemBus().connect("com.lingmo.license", "/com/lingmo/license/Info",
                                         "com.lingmo.license.Info", "LicenseStateChange",
    ...
    connect(m_lingmoId_inter, &LingmoIdProxy::UserInfoChanged, [this](const QVariantMap &userinfo){
    ...
    QFileInfo file(LingmoidDaemonPath);
...
void SyncWorker::initData()
{
    ...
    refreshSwitcherDump();
    m_model->setUserinfo(m_lingmoId_inter->userInfo());
    //
    getRSAPubKey();
}
...
void SyncWorker::activate()
{
    ...
    m_lingmoId_inter->setDBusBlockSignals(false);
...
void SyncWorker::deactivate()
{
    ...
    m_lingmoId_inter->setDBusBlockSignals(true);
}
...
{
    m_lingmoId_inter->Login();
}

void SyncWorker::logoutUser()
{
    m_lingmoId_inter->Logout();
}
...
{
    QDBusInterface accountsInter("org.lingmo.ocean.Accounts1",
                                 QString("/org/lingmo/ocean/Accounts1/User%1").arg(getuid()),
    ...
    QDBusPendingCall call = accountsInter.asyncCall("Get", "org.lingmo.ocean.Accounts.User" ,"UUID");
...
{
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusPendingCall call = lingmoIf.asyncCall("GetRSAKey");
    QDBusPendingCallWatcher *watcher = new QDBusPendingCallWatcher(call, this);
...
{
    QDBusInterface lingmoIf(SyncService, UtcloudPath, UtcloudInterface, QDBusConnection::sessionBus());
    QDBusReply<void> reply = lingmoIf.asyncCall("Empty");
    if(!reply.isValid())
    {
...
{
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<void> reply = lingmoIf.asyncCall("UnBindPlatform", "wechat");
    if(!reply.isValid())
    {
...
{
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<void> reply = lingmoIf.asyncCall("FlushUseInfo");
    if(!reply.isValid())
    {
...
{
    QDBusInterface clientIf(LingmoClientSv, LingmoClientPath, LingmoClientIf, QDBusConnection::sessionBus());
...
void SyncWorker::registerPasswd(const QString &strpwd)
{
    ...
    {
        QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
        QDBusReply<void> reply = lingmoIf.call("SetPassword", QString::fromLocal8Bit(encryptPwd.toBase64()));
        if(!reply.isValid())
        {
...
{
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<QString> reply = lingmoIf.call("MeteInfo");
    if(reply.isValid())
    {
...
{
    QString strsession;
    QDBusInterface lingmoIdIf(LingmoIDService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<QByteArray> reply = lingmoIdIf.call("Get");
    if(reply.isValid()) {
...
{
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusPendingCall pendCall = lingmoIf.asyncCall("DeviceList", pageIndex, pageSize);
...
{
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    lingmoIf.asyncCall("DeviceRemove", devid);
}
...
bool SyncWorker::checkPassword(const QString &passwd, QString &encryptPwd, int &count)
{
    ...
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<QString> reply = lingmoIf.call("Checkpwd", QString::fromLocal8Bit(encryptData.toBase64()));
    if(reply.isValid())
    {
...
bool SyncWorker::resetPassword(const QString &oldpwd, const QString &newpwd)
{
    ...
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<void> reply = lingmoIf.call("ResetPassword", oldpwd, QString::fromLocal8Bit(newEncrypt.toBase64()));
    if(reply.isValid())
    {
        return true;
    }
...
int SyncWorker::sendVerifyCode(const QString &straddr)
{
    ...
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<QString> reply = lingmoIf.call("SendCode", QString::fromLocal8Bit(encryptAddr.toBase64()));
    if(reply.isValid())
    {
...
bool SyncWorker::updatePhoneEmail(const QString &phonemail, const QString &verifyCode, QString &rebindAccount, QString &rebindKey)
{
    ...
    QDBusInterface lingmoIf(SyncService, LingmoIDPath, LingmoIDInterface, QDBusConnection::sessionBus());
    QDBusReply<QString> reply = lingmoIf.call("UpdatePhoneEmail", QString::fromLocal8Bit(encryptPM.toBase64()),
...
{
    QDBusInterface utInterface("com.lingmo.sync.Daemon",
                               "/com/lingmo/utcloud/Daemon",
                               "com.lingmo.utcloud.Daemon"
...
void SyncWorker::getUserLingmoidInfo()
{
    getUOSID();
    getUUID();
    getHostName();
...
BindCheckResult SyncWorker::checkLocalBind(const QString &uuid)
{
    ...
    QDBusReply<QString> retLocalBindCheck= m_lingmoId_inter->LocalBindCheck(uuid);
    if (!m_syncHelperInter->isValid()) {
