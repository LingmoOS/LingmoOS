...
void LoginInfoDetailPage::onStateChanged(const std::pair<qint32, QString> &state)
{
    ...
    case SyncState::Succeed:
        ...
        if (m_model->lastSyncTime() > 0) {
            m_stateIcon->setRotatePixmap(QIcon::fromTheme("oceanui_sync_ok").pixmap(QSize(16, 16)));
        } else {
    ...
    case SyncState::Syncing:
        ...
        m_stateIcon->setRotatePixmap(QIcon::fromTheme("oceanui_syncing").pixmap(QSize(16, 16)));
        m_stateIcon->play();
        break;
...
void LoginInfoDetailPage::onBindUserAccountChanged(bool checked)
{
    ...
    if (m_authority) {
        m_authority->checkAuthorization(QString("com.lingmo.daemon.accounts.user-administration"),
