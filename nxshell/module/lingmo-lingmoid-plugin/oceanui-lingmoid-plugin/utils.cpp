...
QString forgetPwdURL()
{
    ...
    if (forgetUrl.isEmpty()) {
        if (qEnvironmentVariableIsEmpty("LINGMO_PRE")) {
            forgetUrl = QStringLiteral("https://login.lingmo.org/view/client/forgot-password");
        } else {
            forgetUrl = QStringLiteral("https://login-pre.lingmo.org/view/client/forgot-password");
        }
    }
...
QString wechatURL()
{
    ...
    if (wechatUrl.isEmpty()) {
        if (qEnvironmentVariableIsEmpty("LINGMO_PRE")) {
            wechatUrl = QStringLiteral("https://login.lingmo.org/view/client/bind-third/wechat");
        } else {
            wechatUrl =
                    QStringLiteral("https://login-pre.lingmo.org/view/client/bind-third/wechat");
        }
    }
...
{
    QDBusInterface appearance_ifc_("org.lingmo.ocean.Appearance1",
                                   "/org/lingmo/ocean/Appearance1",
                                   "org.lingmo.ocean.Appearance1",
                                   QDBusConnection::sessionBus());
    qDebug() << "connect"
             << "com.lingmo.daemon.Appearance" << appearance_ifc_.isValid();
...
QString getStandardFont()
{
    QDBusInterface appearance_ifc_("org.lingmo.ocean.Appearance1",
                                   ...
                                   QDBusConnection::sessionBus());
    qDebug() << "connect"
             << "org.lingmo.ocean.Appearance1" << appearance_ifc_.isValid();
...
{
    QDBusInterface Interface("com.lingmo.lingmoid",
                             "/com/lingmo/lingmoid",
                             "org.freedesktop.DBus.Properties",
                             QDBusConnection::sessionBus());
    QDBusMessage reply = Interface.call("Get", "com.lingmo.lingmoid", "HardwareID");
...
{
    QDBusPendingReply<unsigned int> reply = Dtk::Core::DUtil::DNotifySender("lingmo ID")
                                                    .appName("org.lingmo.ocean.control-center")
                                                    .appIcon("lingmo-id")
                                                    .appBody(strReason)
                                                    .replaceId(0)
                                                    .timeOut(3000)
