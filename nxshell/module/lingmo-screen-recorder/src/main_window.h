...
//    Q_CLASSINFO("D-Bus Interface", "com.lingmo.ScreenRecorder")
...
protected:
    ...
    /**
      ...
      * 采用线程循环监听文件（"/home/" + userName + "/.cache/lingmo/lingmo-screen-recorder/stopRecord.txt"）是否存在且内容是否为1
      * 存在且内容是1：停止录屏
      * 不存在或不为1：不处理
      */
    ...
    /**
     * @brief 采用线程循环监听文件（"/home/" + userName + "/.cache/lingmo/lingmo-screen-recorder/stopRecord.txt"）是否存在且内容是否为1
     */
