...
#include <QProcess>
#include <QKeyEvent>
#include <com_lingmo_daemon_audio.h>
#include <com_lingmo_daemon_audio_sink.h>
...
{
    DDialog warnDlg;
    warnDlg.setIcon(QIcon::fromTheme("lingmo-screen-recorder"));
...
{
    const QString serviceName {"com.lingmo.daemon.Audio"};

    QScopedPointer<com::lingmo::daemon::Audio> audioInterface;
    QScopedPointer<com::lingmo::daemon::audio::Sink> defaultSink;

    audioInterface.reset(
        new com::lingmo::daemon::Audio(
            serviceName,
            "/com/lingmo/daemon/Audio",
            QDBusConnection::sessionBus(),
            nullptr)
    );

    defaultSink.reset(
        new com::lingmo::daemon::audio::Sink(
            serviceName,
