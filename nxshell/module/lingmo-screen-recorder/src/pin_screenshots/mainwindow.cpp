...
void MainWindow::initShortcut()
{
    ...
    connect(Keyhelp, &QShortcut::activated, this, [ = ] {
        ...
        QList<QVariant> arg;
        arg << (QString("lingmo-screen-recorder"))                  // 应用名称
...
void MainWindow::sendNotify(QString savePath, bool bSaveState)
{
    ...
    if (!bSaveState) {
        ...
        unsigned int id = 0;
        int timeout = -1;
        arg << tr("Pin Screenshots") << id << "lingmo-screen-recorder" << QString() << tips << QStringList() << QVariantMap() << timeout;
    ...
    if (!savePath.isEmpty()) {
        ...
        if (!QStandardPaths::findExecutable("ocean-file-manager").isEmpty()) {
            ...
            hints["x-lingmo-action-_open"] = QString("ocean-file-manager,--show-item,%1").arg(savePath);
        } else {
            hints["x-lingmo-action-_open"] = QString("xdg-open,%1").arg(savePath);
        }
