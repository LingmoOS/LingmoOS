...
void MainWindow::initMainWindow()
{
    ...
    connect(qApp, &QGuiApplication::screenAoceand, this, &MainWindow::onExit);
...
void MainWindow::initAttributes()
{
    ...
    if (Utils::isWaylandMode) {
        ...
        QDBusInterface sessionManagerIntert("org.lingmo.ocean.SessionManager1",
    ...
    //检测锁屏的属性是否发生改变
    QDBusConnection::sessionBus().connect("org.lingmo.ocean.SessionManager1",
                                          "/org/lingmo/ocean/SessionManager1",
...
//1050wayland平台上，部分性能差的机型，采用线程循环监听文件（"/home/" + userName + "/.cache/lingmo/lingmo-screen-recorder/stopRecord.txt"）是否存在且内容是否为1
void MainWindow::checkTempFileArm()
{
    qDebug() << __LINE__ << __func__ ;
    QString userName = QDir::homePath().section("/", -1, -1);
    m_tempPath = ("/home/" + userName + "/.cache/lingmo/lingmo-screen-recorder/").toStdString();
...
void MainWindow::checkIsLockScreen()
{
    QDBusInterface sessionManagerIntert("org.lingmo.ocean.SessionManager1",
                                        ...
                                        QDBusConnection::sessionBus());
    if (!sessionManagerIntert.isValid()) {
        qWarning() << "（org.lingmo.ocean.SessionManager1） dbus接口不存在";
        return;
    }
    ...
    //电源界面判断接口
    QDBusInterface oceanLockFront("com.lingmo.ocean.lockFront",
                                "/com/lingmo/ocean/lockFront",
                                "com.lingmo.ocean.lockFront",
                                QDBusConnection::sessionBus());
    if (!oceanLockFront.isValid()) {
        qWarning() << "(QDBusInterface) The 'oceanLockFront' interface does not exist";
        return;
    }
    if (oceanLockFront.property("Visible").isNull()) {
        qWarning() << "(QDBusInterface) The 'Visible' property does not exist of lockFront";
        return;
    }
    bool isLockFront = oceanLockFront.property("Visible").toBool();
...
void MainWindow::sendSavingNotify()
{
    ...
    arg << Utils::appName //(QCoreApplication::applicationName())                 // appname
        << id                                                   // id
        << QString("lingmo-screen-recorder")                     // icon
...
{
    QDBusInterface iface("com.lingmo.Manual.Open",
                         "/com/lingmo/Manual/Open",
                         "com.lingmo.Manual.Open");
    if (iface.isValid()) {
        iface.call("ShowManual", "lingmo-screen-recorder");
...
void MainWindow::topWindow()
{
    ...
    for (int i = t_windowCount - 1; i >= 0; i--) {
        ...
        if (window->flags().testFlag(Qt::Window) || window->flags().testFlag(Qt::Desktop)) {
            // 排除ocean-dock作为顶层窗口
            if (window->wmClass() == "ocean-dock" || window->wmClass() == "ocean-shell") {
                continue;
            }
            //判断窗口是否被最小化
...
void MainWindow::initBackground()
{
    ...
    if (m_backgroundPixmap.isNull()) {
        ...
        shotFailedNotify.Notify(Utils::appName /*QCoreApplication::applicationName()*/, 0, "lingmo-screen-recorder", QString(), tips, QStringList(), QVariantMap(), 5000);
        // if(Utils::isWaylandMode) {
...
void MainWindow::changeShotToolEvent(const QString &func)
{
    ...
    if (func == "ocr") {
        ...
        m_pinInterface = new PinScreenShotsInterface("com.lingmo.PinScreenShots", "/com/lingmo/PinScreenShots", QDBusConnection::sessionBus(), this);
        //保存贴图到剪贴板
        saveScreenShot();
...
void MainWindow::sendNotify(SaveAction saveAction, QString saveFilePath, const bool succeed)
{
    ...
    if (Utils::is3rdInterfaceStart) {
        QDBusMessage msg = QDBusMessage::createSignal("/com/lingmo/Screenshot", "com.lingmo.Screenshot", "Done");
        msg << saveFilePath;
    ...
    if (!succeed) {
        ...
        saveFailedNotify.Notify(Utils::appName /*QCoreApplication::applicationName()*/, 0, "lingmo-screen-recorder", QString(), tips, QStringList(), QVariantMap(), 5000);
        exitApp();
        return;
    }

    QDBusInterface remote_ocean_notify_obj("com.lingmo.ocean.Notification", "/com/lingmo/ocean/Notification", "com.lingmo.ocean.Notification");

    const bool remote_ocean_notify_obj_exist = remote_ocean_notify_obj.isValid();
    ...
    // 保存到剪贴板， 通知不用open
    QString tips;
    if (remote_ocean_notify_obj_exist && saveFilePath.compare(QString(tr("Clipboard")))) {
        ...
        if (!QStandardPaths::findExecutable("ocean-file-manager").isEmpty()) {
            savepathcommand = QString("ocean-file-manager,--show-item,%1").arg(saveFilePath);
        }
        command = QString("xdg-open,%1").arg(saveFilePath);
        qDebug() << "command:" << command;

        hints["x-lingmo-action-_open"] = command;
        hints["x-lingmo-action-_open1"] = savepathcommand;
    }
...
bool MainWindow::saveAction(const QPixmap &pix)
{
    ...
    case SaveToSpecificDir: {
        ...
        // 自动化测试反馈, ocean-desktop里面有2个computer_window. 修改直接调用QFileDialog类的静态函数. 不用创建其对象
...
void MainWindow::onLockScreenEvent(QDBusMessage msg)
{
    ...
    if (interfaceName == "org.lingmo.ocean.SessionManager1") {
...
void MainWindow::onOpenScrollShotHelp()
{
    ...
    QDBusInterface interFace("com.lingmo.Manual.Open",
                             "/com/lingmo/Manual/Open",
                             "com.lingmo.Manual.Open",
                             QDBusConnection::sessionBus());
...
void MainWindow::onViewShortcut()
{
    ...
    shortcutViewProc->startDetached("lingmo-shortcut-viewer", shortcutString);
