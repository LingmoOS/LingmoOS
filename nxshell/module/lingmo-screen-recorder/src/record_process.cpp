...
{
    QDBusInterface timeInterface("com.lingmo.ScreenRecorder.time",
                                 "/com/lingmo/ScreenRecorder/time",
                                 "com.lingmo.ScreenRecorder.time",
...
{
    QDBusInterface shotInterface("com.lingmo.ShotRecorder.PanelStatus",
                                 "/com/lingmo/ShotRecorder/PanelStatus",
                                 "com.lingmo.ShotRecorder.PanelStatus",
                                 QDBusConnection::sessionBus());
...
{
    QDBusInterface recorderInterface("com.lingmo.ShotRecorder.Recorder.PanelStatus",
                                     "/com/lingmo/ShotRecorder/Recorder/PanelStatus",
                                     "com.lingmo.ShotRecorder.Recorder.PanelStatus",
                                     QDBusConnection::sessionBus());
...
void RecordProcess::startRecord()
{
    ...
    if (QDBusMessage::ReplyMessage == message.type()) {
        if (!message.arguments().takeFirst().toBool())
            qDebug() << "ocean dock screen-recorder-plugin did not receive start message!";
    }
}
...
//    EventLogUtils::get().writeLogs(obj);
    if (Utils::isSysHighVersion1040() == true) {
        ...
        if (QDBusMessage::ReplyMessage == message.type()) {
            if (!message.arguments().takeFirst().toBool())
                qDebug() << "ocean dock screen-recorder-plugin did not receive stop message!";
        }
...
void RecordProcess::exitRecord(QString newSavePath)
{
    if (!Utils::isRootUser) {
        ...
        QVariantMap hints;
        hints["x-lingmo-action-_open"] = QString("xdg-open,%1").arg(newSavePath);
        QString savepathcommand = QString("ocean-file-manager,--show-item,%1").arg(newSavePath);
