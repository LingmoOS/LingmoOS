...
#define ShotShartApp "lingmo-screen-recorder"  // 使用截图录屏的翻译
...
void ShotStartPlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    if (m_bDockQuickPanel || !pluginIsDisable()) {
        qCInfo(SHOT_LOG) << "the current plugin has been aoceand to the dock";
        m_proxyInter->itemAoceand(this, pluginName());
    }

    QDBusConnection sessionBus = QDBusConnection::sessionBus();
    if (sessionBus.registerService("com.lingmo.ShotRecorder.PanelStatus") &&
        sessionBus.registerObject("/com/lingmo/ShotRecorder/PanelStatus", this, QDBusConnection::ExportScriptableSlots)) {
...
const QString ShotStartPlugin::itemCommand(const QString &itemKey)
{
    ...
    if (itemKey == ShotShartPlugin) {
        ...
        if (!m_isRecording) {
            ...
            return "dbus-send --print-reply --dest=com.lingmo.Screenshot /com/lingmo/Screenshot "
                   "com.lingmo.Screenshot.StartScreenshot";
        }
    } else {
...
void ShotStartPlugin::onClickQuickPanel()
{
    ...
    if (!m_isRecording) {
        ...
        QDBusInterface shotDBusInterface(
            "com.lingmo.Screenshot", "/com/lingmo/Screenshot", "com.lingmo.Screenshot", QDBusConnection::sessionBus());
