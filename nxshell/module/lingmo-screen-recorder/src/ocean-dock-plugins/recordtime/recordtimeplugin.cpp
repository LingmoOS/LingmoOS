...
{
    return QString("lingmo-screen-recorder-plugin");
}

const QString RecordTimePlugin::pluginDisplayName() const
{
    return QString("lingmo-screen-recorder");
}
...
void RecordTimePlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    if (sessionBus.registerService("com.lingmo.ScreenRecorder.time")
            && sessionBus.registerObject("/com/lingmo/ScreenRecorder/time", this, QDBusConnection::ExportAdaptors)) {
...
{
    // 录屏过程中，killall ocean-dock，恢复时重新初始化
