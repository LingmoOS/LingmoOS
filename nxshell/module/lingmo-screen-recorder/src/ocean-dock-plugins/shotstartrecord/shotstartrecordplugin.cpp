...
#define RecordShartApp "lingmo-screen-recorder"  // 使用截图录屏的翻译
...
void ShotStartRecordPlugin::init(PluginProxyInterface *proxyInter)
{
    ...
    if (m_bDockQuickPanel || !pluginIsDisable()) {
        qCInfo(RECORD_LOG) << "the current plugin has been aoceand to the dock";
        m_proxyInter->itemAoceand(this, pluginName());
    }

    QDBusConnection sessionBus = QDBusConnection::sessionBus();
    if (sessionBus.registerService("com.lingmo.ShotRecorder.Recorder.PanelStatus") &&
        sessionBus.registerObject(
            "/com/lingmo/ShotRecorder/Recorder/PanelStatus", this, QDBusConnection::ExportScriptableSlots)) {
...
/**
 * @brief itemWidget:返回插件主控件，用于ocean-dock面板上显示
 * @param itemKey:控件名称
 */
...
QWidget *ShotStartRecordPlugin::itemTipsWidget(const QString &itemKey)
{
    ...
    if (itemKey != RecordShartPlugin)
        return nullptr;
    m_tipsWidget->setText(tr("Record") + m_iconWidget->getSysShortcuts("lingmo-screen-recorder"));
...
const QString ShotStartRecordPlugin::itemCommand(const QString &itemKey)
{
    ...
    if (itemKey == RecordShartPlugin) {
        ...
        return "dbus-send --print-reply --dest=com.lingmo.ScreenRecorder /com/lingmo/ScreenRecorder "
               "com.lingmo.ScreenRecorder.stopRecord";
    } else {
...
void ShotStartRecordPlugin::onClickQuickPanel()
{
    ...
    QDBusInterface recordDBusInterface(
        "com.lingmo.ScreenRecorder", "/com/lingmo/ScreenRecorder", "com.lingmo.ScreenRecorder", QDBusConnection::sessionBus());
...
void ShotStartRecordPlugin::setTrayIconVisible(bool visible)
{
    ...
    QDBusInterface interface("com.lingmo.ocean.Dock", "/com/lingmo/ocean/Dock", "com.lingmo.ocean.Dock", QDBusConnection::sessionBus());
