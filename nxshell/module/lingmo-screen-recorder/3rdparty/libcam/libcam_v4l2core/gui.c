...
void set_video_name(const char *name)
{
	...
	if(ext == NULL)
	{
		...
		fprintf(stderr, "lingmo-camera: no valid file extension for video file %s\n",
			name);
		fprintf(stderr, "lingmo-camera: using muxer %i\n", get_video_muxer());
...
void set_photo_name(const char *name)
{
	...
	if(ext == NULL)
	{
		...
		fprintf(stderr, "lingmo-camera: no valid file extension for image file %s\n",
			name);
		fprintf(stderr, "lingmo-camera: using format %i\n", get_photo_format());
