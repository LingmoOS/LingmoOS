...
void video_capture_save_video(int value)
{
    ...
    if(debug_level > 1)
        printf("lingmo-camera: save video flag changed to %i\n", save_video);
}
...
audio_context_t *create_audio_context(int api, int device)
{
    ...
    if(my_audio_ctx == NULL)
        fprintf(stderr, "lingmo-camera: couldn't allocate audio context\n");
...
static void *audio_processing_loop(void *data)
{
    ...
    if(debug_level > 1)
        printf("lingmo-camera: audio thread (tid: %u)\n",
            (unsigned int) syscall (SYS_gettid));

    audio_context_t *audio_ctx = get_audio_context();
    if(!audio_ctx)
    {
        fprintf(stderr, "lingmo-camera: no audio context: skiping audio processing\n");
        return ((void *) -1);
    }
...
static void *encoder_loop(__attribute__((unused))void *data)
{
    ...
    if(debug_level > 1)
        printf("lingmo-camera: encoder thread (tid: %u)\n",
            (unsigned int) syscall (SYS_gettid));
    ...
    if(debug_level > 0)
        printf("lingmo-camera: audio [channels= %i; samprate= %i] \n",
            channels, samprate);
        ...
        v4l2core_get_requested_frame_format(my_vd) == V4L2_PIX_FMT_H264)
    {
        ...
        if(debug_level > 0)
            printf("lingmo-camera: storing external pps and sps data in encoder context\n");
        ...
        if(encoder_ctx->h264_pps_size > 0)
        {
            ...
            {
                fprintf(stderr,"lingmo-camera: FATAL memory allocation failure (encoder_loop): %s\n", strerror(errno));
                exit(-1);
            }
...
//    int64_t last_check_pts = 0; /*last pts when disk supervisor called*/
    ...
    {
        if(debug_level > 1)
            printf("lingmo-camera: starting encoder audio thread\n");

        int ret = __THREAD_CREATE(&encoder_audio_thread, audio_processing_loop, (void *) encoder_ctx);

        if(ret)
            fprintf(stderr, "lingmo-camera: encoder audio thread creation failed (%i)\n", ret);
        else if(debug_level > 2)
            printf("lingmo-camera: created audio encoder thread with tid: %u\n",
...
//        }
    ...
    if(debug_level > 1)
        printf("lingmo-camera: video capture terminated - flushing video buffers\n");
    /*flush the video buffer*/
    encoder_flush_video_buffer(encoder_ctx);
    if(debug_level > 1)
        printf("lingmo-camera: flushing video buffers - done\n");

    /*make sure the audio processing thread has stopped*/
    if(encoder_ctx->enc_audio_ctx != NULL && audio_get_channels(audio_ctx) > 0)
    {
        if(debug_level > 1)
            printf("lingmo-camera: join encoder audio thread\n");
...
void *capture_loop(void *data)
{
    ...
    if(debug_level > 1)
        printf("lingmo-camera: capture thread (tid: %u)\n",
            (unsigned int) syscall (SYS_gettid));
    ...
    while(!quit)
    {
        if(restart)
        {
            ...
            if(ret != E_OK)
            {
                fprintf(stderr, "lingmo-camera: could not set the defined stream format\n");
                fprintf(stderr, "lingmo-camera: trying first listed stream format\n");
                ...
                if(ret != E_OK)
                {
                    fprintf(stderr, "lingmo-camera: also could not set the first listed stream format\n");

                    //gui_error("Lingmo-camera error", "could not start a video stream in the device", 1);
            ...
            {
                if(debug_level > 1)
                    printf("lingmo-camera: resolution changed, reseting render\n");
            ...
            if(debug_level > 0)
                printf("lingmo-camera: reset to pixelformat=%x width=%i and height=%i\n",
                    v4l2core_get_requested_frame_format(my_vd),
                    v4l2core_get_frame_width(my_vd),
                    v4l2core_get_frame_height(my_vd));
        ...
        if( frame != NULL)
        {
            ...
            if(save_image)
            {
                ...
                //if(debug_level > 1)
                //	printf("lingmo-camera: saving image to %s\n", img_filename);
            ...
            snprintf(render_caption, 29, "Lingmo-camera  (%2.2f fps)",
...
int start_encoder_thread(void *data)
{
    ...
    if(ret)
        fprintf(stderr, "lingmo-camera: encoder thread creation failed (%i)\n", ret);
    else if(debug_level > 2)
        printf("lingmo-camera: created encoder thread with tid: %u\n",
...
int stop_encoder_thread()
{
    ...
    if(debug_level > 1)
        printf("lingmo-camera: encoder thread terminated and joined\n");
