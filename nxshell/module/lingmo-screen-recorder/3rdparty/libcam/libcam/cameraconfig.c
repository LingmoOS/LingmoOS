...
int config_save(const char *filename)
{
	...
	/*write config data*/
//	fprintf(fp, "#Lingmo-camera %s config file\n", VERSION);
	fprintf(fp, "\n");
...
int config_load(const char *filename)
{
	...
	while(fgets(bufr, MAXLINE, fp) != NULL)
	{
		...
		{
			if(debug_level > 1)
                printf("lingmo-camera: (config) empty or commented line (%i)\n", line);
			continue;
		}
...
//		else
//            fprintf(stderr, "lingmo-camera: (config) skiping invalid entry at line %i ('%s', '%s')\n", line, token, value);
	...
	//if(errno)
	//{
    //	fprintf(stderr, "lingmo-camera: couldn't read line %i of config file: %s\n", line, strerror(errno));
	//	fclose(fp);
	//	return -1;
	//}
