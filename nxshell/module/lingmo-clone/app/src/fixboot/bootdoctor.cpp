...
bool BootDoctor::fix(const QString &partDevice)
{
    ...
    {
        const QString tmp_dir = "/var/cache/lingmo-clone";
        ...
        const QString &repo_mount_point = mount_root + "/lingmo-clone";
        QFile file_boot_fix(mount_root + "/boot_fix.sh");

        do {
            if (!QDir(mount_root).exists("lingmo-clone") && !QDir(mount_root).mkdir("lingmo-clone")) {
                dCError("Create \"lingmo-clone\" dir failed(\"%s\")", qPrintable(mount_root));
                break;
            }
        ...
        if (ok) {
            ...
            if (ok) {
                ...
                process.start("chroot", {mount_root, "./boot_fix.sh", parent_device, is_efi ? "true" : "false", "/lingmo-clone"});
        ...
        QDir(mount_root).rmdir("lingmo-clone");
        file_boot_fix.remove();
