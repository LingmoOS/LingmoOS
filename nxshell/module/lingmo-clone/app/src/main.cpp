...
int main(int argc, char *argv[])
{
    ...
    if (isTUIMode(argc, argv)) {
        ...
        const QString log_file("/var/log/lingmo-clone.log");
        ...
        if (!qApp->setSingleInstance("_lingmo_clone_")) {
        ...
        app->setApplicationDisplayName(QObject::tr("Lingmo Clone"));
        app->setApplicationDescription(QObject::tr("Lingmo Clone is a backup and restore tool in lingmo. "
                                                   "It supports disk or partition clone, backup and restore, and other functions."));
        app->setApplicationAcknowledgementPage("https://www.lingmo.org/acknowledgments/lingmo-clone/");
        a = app;
    }
#endif

    a->setApplicationName("lingmo-clone");
#ifdef ENABLE_GUI
    ...
    QFile arguments_file("/lib/live/mount/medium/.tmp/lingmo-clone.arguments");
    ...
    if (load_arg_from_file) {
        ...
        if (!arguments_file.open(QIODevice::ReadOnly)) {
            qCritical() << "Open \"/lib/live/mount/medium/.tmp/lingmo-clone.arguments\" failed, error:" << arguments_file.errorString();
        } else {
    ...
    dlogger->registerCategoryAppender("lingmo.ghost", consoleAppender);
    dlogger->registerCategoryAppender("lingmo.ghost", rollingFileAppender);
#endif
    ...
    if (Global::debugLevel == 0) {
        QLoggingCategory::setFilterRules("lingmo.ghost.debug=false");
    }
    ...
    if (Global::isTUIMode) {
        ...
        window->setWindowIcon(QIcon::fromTheme("lingmo-clone"));
