...
#include "../corelib/oceanvicediskinfo.h"
    ...
    , m_mode(mode)
{
    ...
    if (mode == GetSaveName) {
        ...
        m_label->setIcon(QIcon::fromTheme("application-x-lingmoclone-dim"), 80);
        setAcceptDrops(true);
    }
...
void SelectFileWidget::dragEnterEvent(QDragEnterEvent *event)
{
    if (event->mimeData()->hasUrls()) {
        ...
        if (type.name() != "application/x-lingmoclone-dim")
            return QWidget::dragEnterEvent(event);
...
void SelectFileWidget::openFileDialog()
{
    ...
    do {
        ...
        env_list << QString("%1=%2").arg(LINGMO_CLONE_OPEN_DIALOG).arg(m_mode == GetSaveName ? LINGMO_CLONE_GET_FILE : "");
        env_list << QString("%1=%2").arg(LINGMO_CLONE_PARENT_WINID).arg(window()->winId());
        env_list << QString("%1=%2").arg(LINGMO_CLONE_TITLE).arg(m_button->text());
        env_list << QString("%1=%2").arg(LINGMO_CLONE_FILE_NAME).arg(m_defaultFileName);
        env_list << QString("%1=%2").arg(LINGMO_CLONE_NAME_FILTER).arg(tr("Lingmo Image File"));
        env_list << QString("QT_QPA_PLATFORMTHEME=lingmo");
    ...
    dialog.setNameFilters(QStringList() << tr("Lingmo Image File") + "(*.dim)");
