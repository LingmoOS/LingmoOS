...
#include "../corelib/oceanvicepartinfo.h"
...
QStringList honestChildEnvironment()
{
    ...
    if (qEnvironmentVariableIsSet("PKEXEC_UID")) {
        ...
        env_list << QString("%1=%2").arg(LINGMO_CLONE_UID).arg(pkexec_uid);
...
{
    QString honest_child = qApp->applicationDirPath() + "/../honest_child/lingmo-clone-honest";

    if (QFile::exists(honest_child))
        return honest_child;

    return QStandardPaths::findExecutable("lingmo-clone-honest");
}
...
bool MainWindow::showFileItem(const QString &file)
{
    ...
    env_list << QString("%1=%2").arg(LINGMO_CLONE_SHOW_FILE).arg(file);
...
bool MainWindow::openUrl(const QUrl &url)
{
    ...
    env_list << honestChildEnvironment();
    env_list << QString("%1=%2").arg(LINGMO_CLONE_OPEN_URL).arg(url.toString());
...
void MainWindow::setStatus(MainWindow::Status status)
{
                ...
                if (Helper::isBlockSpecialFile(target)) {
                    ...
                    if (target_disk_info) {
                        ...
                        if (target_disk_info.type() == DDiskInfo::Part
                                ...
                                && disk_info.childrenPartList().first().isLingmoSystemRoot()
    ...
    case ToLiveSystem: {
        ...
        if (Helper::existLiveSystem()) {
            ...
            page->setMessage(tr("\"%1\" is used, please restart and enter \"Lingmo Recovery\" to continue").arg(busy_device));
        } else {
            m_bottomButton->setText(tr("OK"));
            m_buttonAction = Cancel;
            page = new EndPage(EndPage::Fail);
            page->setMessage(tr("\"%1\" is used, please install \"Lingmo Recovery\" to retry").arg(busy_device));
        }
...
void MainWindow::onButtonClicked()
{
    ...
    case RestartToLiveSystem: {
        ...
        arguments << source_url << target_url
                  ...
                  << "--log-backup" << toSerialUrl("/var/log/lingmo-clone-livesystem.log");
        ...
        if (!Helper::restartToLiveSystem(arguments)) {
            ...
            showErrorMessage(tr("Failed to restart \"Lingmo Recovery\""));
        }
