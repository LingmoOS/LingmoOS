...
Q_LOGGING_CATEGORY(lcLingmoGhost, "lingmo.ghost")
Q_LOGGING_CATEGORY(lcFormat, "lingmo.clone.format")
...
bool Helper::restartToLiveSystem(const QStringList &arguments)
{
    ...
    QFile file("/recovery/.tmp/lingmo-clone.arguments");
    ...
    if (processExec("grub-reboot", {"Lingmo Recovery"}) != 0) {
        dCDebug("Exec grub-reboot \"Lingmo Recovery\" failed");
...
bool Helper::isLingmoSystem(const DPartInfo &part)
{
    ...
    bool is = QFile::exists(mout_root + "/etc/lingmo-version");
