...
#include "oceanvicepartinfo.h"
#include "dpartinfo_p.h"
#include "helper.h"
#include "oceanvicediskinfo.h"
...
public:
    ...
    bool getIsLingmoSystemRoot() const Q_DECL_OVERRIDE;
    void ensureSizeInfo();

    void init(const QJsonObject &obj);

    bool sizeInfoInitialized = false;
    bool isLingmoSystemRootInitialized = false;
};
...
bool DDevicePartInfoPrivate::getIsLingmoSystemRoot() const
{
    if (!isLingmoSystemRootInitialized) {
        const_cast<DDevicePartInfoPrivate*>(this)->isLingmoSystemRoot = Helper::isLingmoSystem(*q);
        const_cast<DDevicePartInfoPrivate*>(this)->isLingmoSystemRootInitialized = true;
    }

    return isLingmoSystemRoot;
}
