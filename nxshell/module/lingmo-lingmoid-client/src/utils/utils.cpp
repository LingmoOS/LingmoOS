...
QString getActiveColor()
{
    ...
    qDebug() << "connect" << "com.lingmo.daemon.Appearance" << appearance_ifc_.isValid();
...
void sendDBusNotify(const QString &message)
{
    ...
    QVariantMap hints;
    hints["x-lingmo-action-_open"] = "ocean-control-center,-m,cloudsync";

    QList<QVariant> argumentList;
    argumentList << "lingmo-lingmoid-client";
    argumentList << static_cast<uint>(0);
    argumentList << "com.lingmo.lingmoid.Client";
    argumentList << "";
...
{
    QString version = QString("%1 (%2)").arg(LSysInfo::uosEditionName())
...
QString windowSizeURL()
{
    ...
    if(windowUrl.isEmpty()) {
        if(!qEnvironmentVariableIsEmpty("LINGMO_PRE")) {
            windowUrl = QStringLiteral("https://login-pre.lingmo.org/view/client/config.json");
        }
        else {
            windowUrl = QStringLiteral("https://login.lingmo.org/view/client/config.json");
        }
    }
