...
class SessionPrivate
{
    explicit SessionPrivate(Session *parent)
        : q_ptr(parent)
    {
        daemonIf = new LingmoIDInterface(Const::SyncDaemonService,
                                         Const::SyncDaemonPath,
                                         QDBusConnection::sessionBus());
    }

    LingmoIDInterface *daemonIf = nullptr;
...
void Session::authorize(const AuthorizeRequest &authReq)
{
    ...
    req.setRawHeader("X-LingmoID-SessionID", sessionID.toLatin1());
    ...
    qDebug() << "get" << req.url() << req.rawHeader("X-LingmoID-SessionID");
