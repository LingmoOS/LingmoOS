...
void ClipboardLoader::doWork(int protocolType)
{
    ...
    if (mimeData->data("FROM_LINGMO_CLIPBOARD_MANAGER") == "1") {
        qDebug() << "FROM_LINGMO_CLIPBOARD_MANAGER";
        return;
    }
...
bool ClipboardLoader::cachePixmap(const QImage &srcPix, ItemInfo &info)
{
    if (initPixPath()) {
        ...
        // "text/uri-list":"file:///${XDG_CACHE_HOME}/lingmo/ocean-clipboard-daemon/clipboard-pix/xxx"
