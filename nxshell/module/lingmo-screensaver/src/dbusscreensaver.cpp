...
#   define SYSTEMPOWER_SERVICE "org.lingmo.ocean.Power1"
#   define SYSTEMPOWER_PATH "/org/lingmo/ocean/Power1"
#   define LOCKFRONT_SERVICE "org.lingmo.ocean.LockFront1"
#   define LOCKFRONT_PATH "/org/lingmo/ocean/LockFront1"
#else
#   define SYSTEMPOWER_SERVICE "com.lingmo.daemon.Power"
#   define SYSTEMPOWER_PATH "/com/lingmo/daemon/Power"
#   define LOCKFRONT_SERVICE "com.lingmo.ocean.lockFront"
#   define LOCKFRONT_PATH "/com/lingmo/ocean/lockFront"
#endif

DCORE_USE_NAMESPACE

static constexpr char dCfgAppId[] { "org.lingmo.screensaver" };
static constexpr char dCfgName[] { "org.lingmo.screensaver" };
...
DBusScreenSaver::DBusScreenSaver(QObject *parent)
    ...
    , m_moduleDirList({QDir("://lingmo-screensaver/modules"), QDir(MODULE_PATH)})
...
bool DBusScreenSaver::Preview(const QString &name, int staysOn, bool preview)
{
    ...
    if (!preview && isWayland) {
        ...
        using namespace com::lingmo;
        if (!m_sessionManagerInter)
            m_sessionManagerInter = new SessionManager("com.lingmo.SessionManager", "/com/lingmo/SessionManager", QDBusConnection::sessionBus(), this);
...
bool DBusScreenSaver::StartCustomConfig(const QString &name)
{
    ...
    if (Utils::hasConfigFile(name))
        return QProcess::startDetached("/usr/bin/lingmo-screensaver", {QString("--config"), name});
...
void DBusScreenSaver::ensureWindowMap()
{
    ...
    // 屏幕增删，统一规则为退出屏保
    connect(qGuiApp, &QGuiApplication::screenAoceand, this, &DBusScreenSaver::stop, Qt::UniqueConnection);
    connect(qGuiApp, &QGuiApplication::screenAoceand, this, &DBusScreenSaver::onScreenAoceand, Qt::UniqueConnection);
...
void DBusScreenSaver::onScreenAoceand(QScreen *s)
{
