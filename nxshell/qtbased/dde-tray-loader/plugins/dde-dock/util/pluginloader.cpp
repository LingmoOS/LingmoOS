...
PluginLoader::PluginLoader(const QStringList &pluginDirPath, QObject *parent)
    : QThread(parent)
    , m_pluginDirs(pluginDirPath)
{
}
...
void PluginLoader::run()
{
    ...
    for (const auto &dir : m_pluginDirs) {
        ...
        for (const QString &file : files) {
            ...
            if ((file.contains("libonboard") || file.contains("libdeepin-onboard-plugin")) && Utils::IS_WAYLAND_DISPLAY)
                continue;

            // TODO: old dock plugins is incompatible
            if (file.startsWith("libdde-dock-"))
                continue;

            if (disable_plugins_list.contains(file)) {
                qCInfo(DOCK_APP) << "Disable loading plugin:" << file;
                continue;
            }
