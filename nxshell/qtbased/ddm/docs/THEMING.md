...
DDM themes are created using the QtQuick framework, a declarative framework to develop next-generation, hardware-accelerated user interfaces with fluid animations. QtQuick offers some basic components.
...
You can test your themes using `ddm-greeter`. Note that in this mode, actions like shutdown, suspend or login will have no effect.

    ddm-greeter --test --theme /path/to/your/theme

If you have compiled DDM with Qt4, you can also use it in a nested X session through Xephyr. To accomplish this use:
