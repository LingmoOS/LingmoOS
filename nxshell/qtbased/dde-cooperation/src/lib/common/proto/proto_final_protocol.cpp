...
size_t FinalSender::send(const ::proto::OriginMessage& value)
{
...
bool FinalReceiver::onReceive(size_t type, const void* data, size_t size)
{
    switch (type)
    {
        case FBE::proto::OriginMessageFinalModel::fbe_type():
        {
...
std::future<::proto::OriginMessage> FinalClient::request(const ::proto::OriginMessage& value, uint64_t timeout)
{
...
bool FinalClient::onReceiveResponse(const ::proto::OriginMessage& response)
{
    std::scoped_lock locker(this->_lock);
