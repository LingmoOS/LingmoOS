...
BOOT_DIRS := $(shell find boot -type d -name 'debian' -exec dirname {} \;)
...
WINDOWS_DIRS := $(shell find windows -type d -name 'debian' -exec dirname {} \;)
PKG_DEB := $(shell find out/ -name '*.deb')
LIBCOREUI_DIRS := libcoreui
CORE32_DIRS := core32

# Define a target for each directory
.PHONY: all config pkg-base $(SHELL_DIRS) $(LIB_DIRS) $(EXTRA_PKG) $(IMAGE_BUILD) $(CORE_DIRS) $(FM_DIRS) $(BOOT_DIRS) $(WINDOWS_DIRS) ${LIBCOREUI_DIRS} ${CORE32_DIRS} clean
...
#	@$(foreach dir,$(EXTRA_PKG),cp -r $(dir)/* $(OUTPUT)/$(dir)/;)
	...
	@$(foreach dir,$(WINDOWS_DIRS),cp -r $(dir)/* $(OUTPUT)/$(dir)/;)
	@$(foreach dir,$(LIBCOREUI_DIRS),cp -r $(dir)/* $(OUTPUT)/$(dir)/;)
	@$(foreach dir,$(CORE32_DIRS),cp -r $(dir)/* $(OUTPUT)/$(dir)/;)
...
pkg-shell: $(SHELL_DIRS) ${WINDOWS_DIRS}
