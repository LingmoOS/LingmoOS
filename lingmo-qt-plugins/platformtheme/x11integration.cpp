...
xcb_connection_t *x11Connection()
{
    ...
    if (const auto *x11 = qGuiApp->nativeInterface<QNativeInterface::QX11Application>()) {
...
xcb_window_t x11RootWindow()
{
    ...
    if (const auto *x11 = qGuiApp->nativeInterface<QNativeInterface::QX11Application>()) {
        if (const auto *display = x11->display()) {
...
bool X11Integration::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (event->type() == QEvent::Show && watched->inherits("QShapedPixmapWindow") && x11Connection()) {
        //static cast should be safe there
        QWindow *w = static_cast<QWindow *>(watched);
        NETWinInfo info(x11Connection(), w->winId(), x11RootWindow(), NET::WMWindowType, NET::Properties2());
...
void X11Integration::installDesktopFileName(QWindow *w)
{
    ...
    NETWinInfo info(x11Connection(), w->winId(), x11RootWindow(), NET::Properties(), NET::Properties2());
