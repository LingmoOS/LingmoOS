...
Prepare_config ()
{
	...
	export LINGMO_SYS_BUILD_CONFIGURATION_VERSION="${LINGMO_SYS_BUILD_CONFIGURATION_VERSION:-${LINGMO_SYSBUILD_VERSION}}"
	...
	LINGMO_SYS_BUILD_MODE="${LINGMO_SYS_BUILD_MODE:-lingmo}"
	LINGMO_SYS_BUILD_DERIVATIVE="false"
	LINGMO_SYS_BUILD_CODENAME=""
	LINGMO_SYS_BUILD_DISTRIBUTION="${LINGMO_SYS_BUILD_DISTRIBUTION:-trixie}"
	...
	export LINGMO_SYS_BUILD_IMAGE_NAME="${LINGMO_SYS_BUILD_IMAGE_NAME:-lingmo-os}"
	...
	case "${LINGMO_SYS_BUILD_PARENT_DISTRIBUTION_BINARY}" in
		sid|unstable)
			...
			LINGMO_SYS_BUILD_PROPOSED_UPDATES="${LINGMO_SYS_BUILD_PROPOSED_UPDATES:-false}"
			;;
	...
	LINGMO_SYS_BUILD_ZSYNC="${LINGMO_SYS_BUILD_ZSYNC:-false}"
	...
	case "${LINGMO_SYS_BUILD_INITRAMFS}" in
		live-boot)
			...
			LINGMO_SYS_BUILD_BOOTAPPEND_LIVE_FAILSAFE="${LINGMO_SYS_BUILD_BOOTAPPEND_LIVE_FAILSAFE:-boot=live components memtest noapic noapm nodma nomce nosmp nosplash vga=788}"
			;;

		none)
			LINGMO_SYS_BUILD_BOOTAPPEND_LIVE="${LINGMO_SYS_BUILD_BOOTAPPEND_LIVE:-quiet splash}"
			LINGMO_SYS_BUILD_BOOTAPPEND_LIVE_FAILSAFE="${LINGMO_SYS_BUILD_BOOTAPPEND_LIVE_FAILSAFE:-memtest noapic noapm nodma nomce nosmp nosplash vga=788}"
			;;
	esac
	...
	LINGMO_SYS_BUILD_ISO_APPLICATION="${LINGMO_SYS_BUILD_ISO_APPLICATION:-Lingmo OS}"
	LINGMO_SYS_BUILD_ISO_PREPARER="${LINGMO_SYS_BUILD_ISO_PREPARER:-lingmo-sysbuild @LINGMO_SYS_BUILD_VERSION@; https://nebula.lingmo.org/lingmo-os/LingmoOS}"
	LINGMO_SYS_BUILD_ISO_PUBLISHER="${LINGMO_SYS_BUILD_ISO_PUBLISHER:-Lingmo OS project; https://lingmo.org; team@lingmo.org}"
	# The string @ISOVOLUME_TS@ must have the same length as the output of `date +%Y%m%d-%H:%M`
	LINGMO_SYS_BUILD_ISO_VOLUME="${LINGMO_SYS_BUILD_ISO_VOLUME:-Lingmo OS ${LINGMO_SYS_BUILD_DISTRIBUTION} @ISOVOLUME_TS@}"

	LINGMO_SYS_BUILD_HDD_LABEL="${LINGMO_SYS_BUILD_HDD_LABEL:-LINGMO_OS_LIVE}"
	...
	if [ -n "${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURES}" ]; then
		...
		Echo_warning "LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURES was renamed to LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURE, please update your config."
	fi
...
Validate_config_permitted_values ()
{
	...
	if [ "${LINGMO_SYS_BUILD_PROPOSED_UPDATES}" != "true" ] && [ "${LINGMO_SYS_BUILD_PROPOSED_UPDATES}" != "false" ]; then
		Echo_error "Value for LINGMO_SYS_BUILD_PROPOSED_UPDATES (--proposed-updates) can only be 'true' or 'false'!"
		exit 1
	fi
	...
	if [ -n "${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURE}" ]; then
		if [ -z "${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_STATIC}" ]; then
			Echo_error "You have not specified the qemu-static binary for ${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURE} (--bootstrap-qemu-static)"
			exit 1
		fi
		if [ ! -e "${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_STATIC}" ]; then
			Echo_error "The qemu-static binary (${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_STATIC}) for ${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURE} was not found on the host"
			exit 1
		fi

		if [ ! -x "${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_STATIC}" ]; then
			Echo_error "The qemu-static binary (${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_STATIC}) for ${LINGMO_SYS_BUILD_BOOTSTRAP_QEMU_ARCHITECTURE} is not executable on the host"
			exit 1
		fi
	fi
}
...
Validate_http_proxy ()
{
	...
	if command -v apt-config >/dev/null; then
		...
		#  This setup is too complex for the purpose of lingmo-sysbuild and will silently be ignored
	...
	# This is the value to use for the the other scripts in lingmo-sysbuild
