...
Create_apt_sources_list ()
{
	...
	case "${1}" in
		chroot)
			PARENT_MIRROR=${LINGMO_SYS_BUILD_PARENT_MIRROR_CHROOT}i
			DESKTOP_LIST=${LINGMO_SYS_BUILD_DESKTOP}
	...
	if [ "${LINGMO_SYS_BUILD_PROPOSED_UPDATES}" = "true" ]; then
		echo "deb ${PARENT_MIRROR} ${PARENT_DISTRIBUTION}-proposed-updates ${LINGMO_SYS_BUILD_PARENT_ARCHIVE_AREAS}" >> "${PARENT_LIST_FILE}"
		echo "deb-src ${PARENT_MIRROR} ${PARENT_DISTRIBUTION}-proposed-updates ${LINGMO_SYS_BUILD_PARENT_ARCHIVE_AREAS}" >> "${PARENT_LIST_FILE}"

		if [ "${LINGMO_SYS_BUILD_DERIVATIVE}" = "true" ]; then
			echo "deb ${MIRROR} ${_DISTRIBUTION}-proposed-updates ${LINGMO_SYS_BUILD_ARCHIVE_AREAS}" >> "${LIST_FILE}"
			echo "deb-src ${MIRROR} ${_DISTRIBUTION}-proposed-updates ${LINGMO_SYS_BUILD_ARCHIVE_AREAS}" >> "${LIST_FILE}"
		fi
	fi
