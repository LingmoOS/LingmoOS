...
# http_proxy: The proxy that is used by lingmo-sysbuild and wget
...
#   Defaults env_keep += "LINGMO_SYSBUILD"
...
Lingmo-sysBuild override: ${LINGMO_SYSBUILD_OVERRIDE}
Lingmo-sysBuild path: ${LINGMO_SYSBUILD}
...
if [ ! -z "${LINGMO_SYSBUILD}" ]; then
	LINGMO_SYSBUILD_OVERRIDE=1
else
	LINGMO_SYSBUILD_OVERRIDE=0
	export LINGMO_SYSBUILD=${PWD}/lingmo-sysbuild
fi
...
if [ ! -d ${LINGMO_SYSBUILD} -a ${LINGMO_SYSBUILD_OVERRIDE} -eq 0 ]; then
	git clone https://salsa.debian.org/live-team/lingmo-sysbuild.git ${LINGMO_SYSBUILD} --single-branch --no-tags
fi

LINGMO_SYS_BUILD_OUTPUT=lb_output.txt
...
if [ ${LINGMO_SYSBUILD_OVERRIDE} -eq 0 ]; then
	pushd ${LINGMO_SYSBUILD} >/dev/null
	git pull ${GIT_OPTIONS}
	git checkout $(git rev-list -n 1 --min-age=${SOURCE_DATE_EPOCH} HEAD) ${GIT_OPTIONS}
	git clean -Xdf ${GIT_OPTIONS}
	output_echo "Info: using lingmo-sysbuild from git version $(git log -n 1 --pretty=format:%H_%aI)"
	popd >/dev/null
else
	output_echo "Info: using local lingmo-sysbuild: $(lb --version)"
fi
...
# Insider knowledge of lingmo-sysbuild:
...
cp -a ${LINGMO_SYSBUILD}/examples/hooks/reproducible/* config/hooks/normal
...
${SUDO} lb build | tee -a $LINGMO_SYS_BUILD_OUTPUT
BUILD_RESULT=$?
set -e
