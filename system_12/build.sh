...
target_image_name() {
	...
	if [ "$IMAGE_TYPE" = "live" ]; then
		if [ "$LINGMO_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$IMGTYPE-$LINGMO_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-linux-$LINGMO_VERSION-$IMGTYPE-$LINGMO_VARIANT-$LINGMO_ARCH.$IMAGE_EXT"
		fi
	else
		if [ "$LINGMO_VARIANT" = "default" ]; then
...
run_and_log cd $TARGET_DIR/iso/ && ./make-sums && rm make-sums
 && cd ../../
