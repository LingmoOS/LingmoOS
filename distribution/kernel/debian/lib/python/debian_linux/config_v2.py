...
@dataclasses.dataclass
class ConfigLingmoarchDefs:
    __arch: Optional[str] = dataclasses.field(default=None, init=False)

    def __post_init_defs__(self, parent: ConfigLingmoarch) -> None:
...
class ConfigKernelarch(ConfigBase):
    debianarch: list[ConfigLingmoarch] = dataclasses.field(
        default_factory=list, metadata={'merge': 'assoclist'},
    )
...
@dataclasses.dataclass
class ConfigLingmoarch(ConfigBase):
    defs: ConfigLingmoarchDefs = dataclasses.field(default_factory=ConfigLingmoarchDefs)
...
class ConfigFeatureset(ConfigBase):
    ...
    def __post_init_hierarchy_featureset__(
        self,
        path: Path,
        debianarch: Optional[ConfigLingmoarch],
    ) -> None:
...
class ConfigMergedKernelarch(ConfigMerged):
    ...
    @property
    def debianarchs(self) -> Iterable[ConfigMergedLingmoarch]:
        for debianarch in self._kernelarch.debianarch:
            yield ConfigMergedLingmoarch(
                root=self._root,
...
class ConfigMergedLingmoarch(ConfigMergedKernelarch):
    _debianarch: ConfigLingmoarch
    ...
    def __init__(
        ...
        **kw: Optional[ConfigBase],
        ...
        if debianarch is not None:
            assert isinstance(debianarch, ConfigLingmoarch)
    ...
    @property
    def defs_debianarch(self) -> ConfigLingmoarchDefs:
...
class ConfigMergedFeatureset(ConfigMergedLingmoarch):
