...
clean() {
	...
	# Live
	run_and_log $SUDO lingmo-sys-build clean --purge
...
# Build parameters for lingmo-sys-build config
...
live)
	...
	run_and_log lingmo-sys-build config -a $LINGMO_ARCH $LINGMO_CONFIG_OPTS "$@"
	[ $? -eq 0 ] || failure

	debug "Stage 2/2 - Build"
	run_and_log $SUDO lingmo-sys-build build
