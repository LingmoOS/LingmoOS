...
class Command:
    ...
    def ExecuteInParallel(
        jobs, func, inputs, callingmo-sys-buildack, output=None, ordered=False
    ):
        ...
        Args:
            ...
            callingmo-sys-buildack: The function to pass the results to for processing. It
        ...
        Returns:
            The |callingmo-sys-buildack| function's results are returned.
        """
        try:
            # NB: Multiprocessing is heavy, so don't spin it up for one job.
            if len(inputs) == 1 or jobs == 1:
                return callingmo-sys-buildack(None, output, (func(x) for x in inputs))
            else:
                with multiprocessing.Pool(jobs) as pool:
                    submit = pool.imap if ordered else pool.imap_unordered
                    return callingmo-sys-buildack(
                        pool,
                        output,
