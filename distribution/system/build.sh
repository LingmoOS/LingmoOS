...
target_image_name() {
	...
	if [ "$IMAGE_TYPE" = "live" ]; then
		if [ "$LINGMO_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_SYS_DIST-$RELEASE_TYPE-$SYSTEM_TYPE-$LINGMO_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_SYS_DIST-$RELEASE_TYPE-$SYSTEM_TYPE-$LINGMO_ARCH.$IMAGE_EXT"
		fi
	else
		if [ "$LINGMO_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_SYS_DIST-installer-$LINGMO_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_SYS_DIST-installer-$LINGMO_VARIANT-$LINGMO_ARCH.$IMAGE_EXT"
		fi
	fi
}
...
print_help() {
	...
	echo
	echo "More information: https://github.com/LingmoOS/LingmoOS"
	exit 0
}
...
if [ -z "$LINGMO_VERSION" ]; then
	LINGMO_VERSION="$(default_version $LINGMO_SYS_DIST)"
fi
...
if grep -q -e "^ID=lingmo" -e "^ID_LIKE=debian" /usr/lib/os-release; then
...
installer)
	...
	if [ "$LINGMO_VARIANT" = "netinst" ]; then
		...
		export KERNEL_PARAMS="debian-installer/theme=Clearlooks"
...
run_and_log cp -v make-sums $TARGET_DIR/iso
run_and_log cd $TARGET_DIR/iso/ && ./make-sums && rm make-sums && cd ../../
