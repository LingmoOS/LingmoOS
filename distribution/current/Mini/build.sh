...
target_image_name() {
	...
	if [ "$IMAGE_TYPE" = "live" ]; then
		if [ "$LINGMO_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_DIST-$RELEASE_TYPE-$SYSTEM_TYPE-$LINGMO_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_DIST-$RELEASE_TYPE-$SYSTEM_TYPE-$LINGMO_ARCH.$IMAGE_EXT"
		fi
	else
		if [ "$LINGMO_VARIANT" = "default" ]; then
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_DIST-installer-$LINGMO_ARCH.$IMAGE_EXT"
		else
			echo "${TARGET_SUBDIR:+$TARGET_SUBDIR/}lingmo-os-$LINGMO_VERSION-$LINGMO_DIST-installer-$LINGMO_VARIANT-$LINGMO_ARCH.$IMAGE_EXT"
		fi
	fi
}
