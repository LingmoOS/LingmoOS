...
{
	setWindowTitle(i18n("Lingmo GPGAssistant"));
	...
	label->setText(i18n("This assistant will first setup some basic configuration options required for Lingmo GPGto work properly. Next, it will allow you to create your own key pair, enabling you to encrypt your files and emails."));
	...
	page_welcome = addPage(page, i18n("Welcome to the Lingmo GPGAssistant"));
	...
	label->setText(i18n("Lingmo GPGneeds to know which GnuPG binary to use."));
	...
	label->setAlignment(Qt::AlignTop);
	label->setWordWrap(true);
	label->setText(i18n("Lingmo GPGneeds to know where your GnuPG configuration file is stored."));
	...
	autostartCB->setText(i18n("Start Lingmo GPGautomatically at Lingmo OS startup."));
	...
	if (!QFile(confPath).exists()) {
		...
		if (QFile(confPathOpt).exists()) {
			confPath = confPathOpt;
                        ...
                        if (KMessageBox::questionYesNo(nullptr, i18n("<qt><b>The GnuPG configuration file was not found</b>. Should Lingmo GPGtry to create a config file ?</qt>"), QString(), KGuiItem(i18n("Create Config")), KGuiItem(i18n("Do Not Create"))) == KMessageBox::Yes) {
				QFile file(confPath);
