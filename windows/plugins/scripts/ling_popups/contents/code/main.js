...
var lingPopupsEffect = {
    loadConfig: function () {
        lingPopupsEffect.fadeInDuration = animationTime(100);
        lingPopupsEffect.fadeOutDuration = animationTime(100) * 4;
    },
    slotWindowAdded: function (window) {
        ...
        window.fadeInAnimation = animate({
            ...
            duration: lingPopupsEffect.fadeInDuration,
            type: Effect.Opacity,
            from: 0.0,
            to: 1.0
        });
    ...
    slotWindowClosed: function (window) {
        ...
        window.fadeOutAnimation = animate({
            ...
            duration: lingPopupsEffect.fadeOutDuration,
            type: Effect.Opacity,
            from: 1.0,
            to: 0.0
        });
    ...
    init: function () {
        lingPopupsEffect.loadConfig();

        effect.configChanged.connect(lingPopupsEffect.loadConfig);
        effects.windowAdded.connect(lingPopupsEffect.slotWindowAdded);
        effects.windowClosed.connect(lingPopupsEffect.slotWindowClosed);
        effects.windowDataChanged.connect(lingPopupsEffect.slotWindowDataChanged);
    }
};

lingPopupsEffect.init();
