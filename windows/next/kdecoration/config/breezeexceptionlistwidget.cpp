...
void ExceptionListWidget::add()
{
    ...
    dialog->setWindowTitle(i18n("New Exception - Lingmo Settings"));
...
void ExceptionListWidget::edit()
{
    ...
    // create dialog
    QPointer<ExceptionDialog> dialog(new ExceptionDialog(m_configuration, m_presetsConfiguration, this));
    dialog->setWindowTitle(i18n("Edit Exception - Lingmo Settings"));
    dialog->setException(exception);
...
bool ExceptionListWidget::checkException(InternalSettingsPtr exception)
{
    while ((exception->exceptionProgramNamePattern().isEmpty() && exception->exceptionWindowPropertyPattern().isEmpty())
        ...
        if (exception->preventApplyOpacityToHeader() && exception->exceptionProgramNamePattern().isEmpty()) {
            QMessageBox::warning(this, i18n("Warning - Lingmo Settings"), i18n("Please provide an application name to match"));
        } else
            QMessageBox::warning(this, i18n("Warning - Lingmo Settings"), i18n("Regular Expression syntax is incorrect"));
        QPointer<ExceptionDialog> dialog(new ExceptionDialog(m_configuration, m_presetsConfiguration, this));
        dialog->setException(exception);
