...
void PresetsModel::exportPreset(KConfig *presetsConfig, const QString &presetName, const QString &filePath)
{
    ...
    KConfigGroup outputGlobalGroup = outputPresetConfig->group("Lingmo Window Decoration Preset File");
    KConfigGroup outputPresetGroup = outputPresetConfig->group(groupName);

    outputGlobalGroup.writeEntry("version", lingmoLongVersion());
...
PresetsModel::importPreset(KConfig *presetsConfig, const QString &filePath, QString &presetName, QString &error, bool forceInvalidVersion, bool markAsBundled)
{
    ...
    if (!(importPresetConfig->hasGroup("Lingmo Window Decoration Preset File")))
        return PresetsErrorFlag::InvalidGlobalGroup;
    KConfigGroup importGlobalGroup = importPresetConfig->group("Lingmo Window Decoration Preset File");
    QString importVersion = importGlobalGroup.readEntry("version");
    bool versionValid = (importVersion == lingmoLongVersion());
...
// copies bundled presets in /usr/lib64/qt5/plugins/plasma/kcms/lingmo/presets into ~/.config/lingmo/lingmorc once per release
void PresetsModel::importBundledPresets(KConfig *presetsConfig)
{
    ...
    if (presetsConfig->hasGroup("Global")) {
        ...
        if (globalGroup.hasKey("BundledWindecoPresetsImportedVersion")) {
            if (globalGroup.readEntry("BundledWindecoPresetsImportedVersion") == lingmoLongVersion()) {
                return;
            }
        }
    }
    ...
    for (QString libraryPath : QCoreApplication::libraryPaths()) {
        libraryPath += "/plasma/kcms/lingmo/presets";
    ...
    KConfigGroup globalGroup = presetsConfig->group("Global");
    globalGroup.writeEntry("BundledWindecoPresetsImportedVersion", lingmoLongVersion());
    presetsConfig->sync();
}
}
