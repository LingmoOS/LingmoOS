...
ifeq ($(LINGMO_STATIC_LIBS)$(LINGMO_TOOLCHAIN_HAS_THREADS),yy)
...
ifeq ($(call qstrip,$(LINGMO_TARGET_LOCALTIME)),)
PHP_LOCALTIME = UTC
else
# Not q-stripping this value, as we need quotes in the php.ini file
PHP_LOCALTIME = $(LINGMO_TARGET_LOCALTIME)
endif
...
ifeq ($(LINGMO_PACKAGE_PHP_EXT_OPCACHE),y)
...
PHP_CONF_OPTS += $(if $(LINGMO_PACKAGE_PHP_SAPI_CLI),--enable-cli,--disable-cli)
PHP_CONF_OPTS += $(if $(LINGMO_PACKAGE_PHP_SAPI_CGI),--enable-cgi,--disable-cgi)
PHP_CONF_OPTS += $(if $(LINGMO_PACKAGE_PHP_SAPI_FPM),--enable-fpm,--disable-fpm)

ifeq ($(LINGMO_PACKAGE_PHP_SAPI_APACHE),y)
...
ifeq ($(LINGMO_PACKAGE_APACHE_MPM_EVENT)$(LINGMO_PACKAGE_APACHE_MPM_WORKER),y)
...
### Extensions
PHP_CONF_OPTS += \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SOCKETS),--enable-sockets) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_POSIX),--enable-posix) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SESSION),--enable-session) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_DOM),--enable-dom) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SIMPLEXML),--enable-simplexml) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SOAP),--enable-soap) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_XML),--enable-xml) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_XMLREADER),--enable-xmlreader) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_XMLWRITER),--enable-xmlwriter) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_EXIF),--enable-exif) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_FTP),--enable-ftp) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_TOKENIZER),--enable-tokenizer) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_PCNTL),--enable-pcntl) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SHMOP),--enable-shmop) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SYSVMSG),--enable-sysvmsg) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SYSVSEM),--enable-sysvsem) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_SYSVSHM),--enable-sysvshm) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_ZIP),--with-zip) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_CTYPE),--enable-ctype) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_FILTER),--enable-filter) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_CALENDAR),--enable-calendar) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_FILEINFO),--enable-fileinfo) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_BCMATH),--enable-bcmath) \
	$(if $(LINGMO_PACKAGE_PHP_EXT_PHAR),--enable-phar)

ifeq ($(LINGMO_PACKAGE_PHP_EXT_LIBARGON2),y)
PHP_CONF_OPTS += --with-password-argon2=$(STAGING_DIR)/usr
PHP_DEPENDENCIES += libargon2
endif

ifeq ($(LINGMO_PACKAGE_PHP_EXT_LIBSODIUM),y)
PHP_CONF_OPTS += --with-sodium=$(STAGING_DIR)/usr
PHP_DEPENDENCIES += libsodium
endif

ifeq ($(LINGMO_PACKAGE_PHP_EXT_MBSTRING),y)
PHP_CONF_OPTS += --enable-mbstring
PHP_DEPENDENCIES += oniguruma
endif

ifeq ($(LINGMO_PACKAGE_PHP_EXT_OPENSSL),y)
...
ifeq ($(LINGMO_PACKAGE_PHP_EXT_LIBXML2),y)
PHP_CONF_ENV += php_cv_libxml_build_works=yes
PHP_CONF_OPTS += --with-libxml
PHP_DEPENDENCIES += libxml2
endif

ifeq ($(LINGMO_PACKAGE_PHP_EXT_ZIP),y)
PHP_DEPENDENCIES += libzip
endif

ifneq ($(LINGMO_PACKAGE_PHP_EXT_ZLIB)$(LINGMO_PACKAGE_PHP_EXT_ZIP),)
...
### PDO
ifeq ($(LINGMO_PACKAGE_PHP_EXT_PDO),y)
PHP_CONF_OPTS += --enable-pdo
ifeq ($(LINGMO_PACKAGE_PHP_EXT_PDO_SQLITE),y)
PHP_CONF_OPTS += --with-pdo-sqlite=$(STAGING_DIR)/usr
PHP_DEPENDENCIES += sqlite
PHP_CFLAGS += -DSQLITE_OMIT_LOAD_EXTENSION
endif
ifeq ($(LINGMO_PACKAGE_PHP_EXT_PDO_MYSQL),y)
PHP_CONF_OPTS += --with-pdo-mysql
endif
ifeq ($(LINGMO_PACKAGE_PHP_EXT_PDO_POSTGRESQL),y)
PHP_CONF_OPTS += --with-pdo-pgsql=$(STAGING_DIR)/usr
PHP_DEPENDENCIES += postgresql
endif
ifeq ($(LINGMO_PACKAGE_PHP_EXT_PDO_UNIXODBC),y)
PHP_CONF_OPTS += --with-pdo-odbc=unixODBC,$(STAGING_DIR)/usr
PHP_DEPENDENCIES += unixodbc
endif
endif

ifneq ($(LINGMO_PACKAGE_PHP_EXT_MYSQLI)$(LINGMO_PACKAGE_PHP_EXT_PDO_MYSQL),)
...
ifeq ($(LINGMO_PACKAGE_PCRE2_JIT),y)
...
ifeq ($(LINGMO_PACKAGE_PHP_EXT_FFI),y)
PHP_CONF_OPTS += --with-ffi
PHP_DEPENDENCIES += libffi
endif

ifeq ($(LINGMO_PACKAGE_PHP_SAPI_FPM),y)
...
define PHP_INSTALL_FIXUP
	...
	$(SED) 's%;date.timezone =.*%date.timezone = $(PHP_LOCALTIME)%' \
		$(TARGET_DIR)/etc/php.ini
	$(if $(LINGMO_PACKAGE_PHP_EXT_OPCACHE),
