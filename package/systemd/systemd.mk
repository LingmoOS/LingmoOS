...
#   arch and headers
# - If yes, propagate the dependencies to LINGMO_INIT_SYSTEMD
...
ifeq ($(LINGMO_PACKAGE_UTIL_LINUX_LIBFDISK),y)
...
ifeq ($(LINGMO_PACKAGE_LIBQRENCODE),y)
...
ifneq ($(LINGMO_PACKAGE_LIBGCRYPT)$(LINGMO_PACKAGE_LIBOPENSSL),)
SYSTEMD_CONF_OPTS += -Ddefault-dnssec=allow-downgrade
else
SYSTEMD_CONF_OPTS += -Ddefault-dnssec=no
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_HWDB),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_BINFMT),y)
SYSTEMD_CONF_OPTS += -Dbinfmt=true
else
SYSTEMD_CONF_OPTS += -Dbinfmt=false
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_UTMP),y)
SYSTEMD_CONF_OPTS += -Dutmp=true
else
SYSTEMD_CONF_OPTS += -Dutmp=false
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_VCONSOLE),y)
SYSTEMD_CONF_OPTS += \
	-Dvconsole=true \
	-Ddefault-keymap=$(call qstrip,$(LINGMO_PACKAGE_SYSTEMD_VCONSOLE_DEFAULT_KEYMAP))
else
SYSTEMD_CONF_OPTS += -Dvconsole=false
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_QUOTACHECK),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_MACHINED),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_IMPORTD),y)
SYSTEMD_CONF_OPTS += -Dimportd=true
else
SYSTEMD_CONF_OPTS += -Dimportd=false
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_HOMED),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_USERDB),y)
SYSTEMD_CONF_OPTS += -Duserdb=true
else
SYSTEMD_CONF_OPTS += -Duserdb=false
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_COREDUMP),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_PSTORE),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_OOMD),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_POLKIT),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_TIMESYNCD),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_SMACK_SUPPORT),y)
SYSTEMD_CONF_OPTS += -Dsmack=true
else
SYSTEMD_CONF_OPTS += -Dsmack=false
endif

ifeq ($(LINGMO_PACKAGE_SYSTEMD_HIBERNATE),y)
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_BOOT),y)
...
SYSTEMD_BOOT_EFI_ARCH = $(call qstrip,$(LINGMO_PACKAGE_SYSTEMD_BOOT_EFI_ARCH))
...
endif # LINGMO_PACKAGE_SYSTEMD_BOOT == y

SYSTEMD_FALLBACK_HOSTNAME = $(call qstrip,$(LINGMO_TARGET_GENERIC_HOSTNAME))
ifneq ($(SYSTEMD_FALLBACK_HOSTNAME),)
SYSTEMD_CONF_OPTS += -Dfallback-hostname=$(SYSTEMD_FALLBACK_HOSTNAME)
endif

SYSTEMD_DEFAULT_TARGET = $(call qstrip,$(LINGMO_PACKAGE_SYSTEMD_DEFAULT_TARGET))
...
ifneq ($(call qstrip,$(LINGMO_TARGET_GENERIC_GETTY_PORT)),)
...
#
# What we want to do
# * Enable a getty on $LINGMO_TARGET_GENERIC_GETTY_PORT
# * Set the baudrate for all units according to LINGMO_TARGET_GENERIC_GETTY_BAUDRATE
...
# * enable getty@xxx if  $LINGMO_TARGET_GENERIC_GETTY_PORT is a tty
# * enable serial-getty@xxx for other $LINGMO_TARGET_GENERIC_GETTY_PORT
...
define SYSTEMD_INSTALL_SERVICE_TTY
	...
	if [ $(LINGMO_TARGET_GENERIC_GETTY_PORT) = "console" ]; \
	then \
		: ; \
	elif echo $(LINGMO_TARGET_GENERIC_GETTY_PORT) | egrep -q 'tty[0-9]*$$'; \
	then \
		printf '[Install]\nDefaultInstance=%s\n' \
			$(call qstrip,$(LINGMO_TARGET_GENERIC_GETTY_PORT)) \
			>$(TARGET_DIR)/usr/lib/systemd/system/getty@.service.d/buildroot-console.conf; \
	else \
	...
	if [ $(call qstrip,$(LINGMO_TARGET_GENERIC_GETTY_BAUDRATE)) -gt 0 ] ; \
	then \
		$(SED) 's/115200/$(LINGMO_TARGET_GENERIC_GETTY_BAUDRATE),115200/' $(TARGET_DIR)/lib/systemd/system/getty@.service; \
		$(SED) 's/115200/$(LINGMO_TARGET_GENERIC_GETTY_BAUDRATE),115200/' $(TARGET_DIR)/lib/systemd/system/serial-getty@.service; \
		$(SED) 's/115200/$(LINGMO_TARGET_GENERIC_GETTY_BAUDRATE),115200/' $(TARGET_DIR)/lib/systemd/system/console-getty@.service; \
		$(SED) 's/115200/$(LINGMO_TARGET_GENERIC_GETTY_BAUDRATE),115200/' $(TARGET_DIR)/lib/systemd/system/container-getty@.service; \
	fi
endef
endif
...
ifeq ($(LINGMO_PACKAGE_SYSTEMD_CATALOGDB),y)
