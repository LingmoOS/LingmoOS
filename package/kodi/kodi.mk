...
ifeq ($(LINGMO_PACKAGE_KODI_PLATFORM_SUPPORTS_X11),y)
...
ifeq ($(LINGMO_arceb)$(LINGMO_arcle),y)
...
KODI_CONF_OPTS += \
	-DWITH_ARCH=mips$(if $(LINGMO_ARCH_IS_64),64) \
	-DWITH_CPU=mips$(if $(LINGMO_ARCH_IS_64),64)
else ifeq ($(LINGMO_powerpc)$(LINGMO_powerpc64le),y)
KODI_CONF_OPTS += \
	-DWITH_ARCH=powerpc$(if $(LINGMO_ARCH_IS_64),64) \
	-DWITH_CPU=powerpc$(if $(LINGMO_ARCH_IS_64),64)
else ifeq ($(LINGMO_or1k)$(LINGMO_powerpc64)$(LINGMO_riscv)$(LINGMO_sparc64)$(LINGMO_sh4)$(LINGMO_xtensa),y)
KODI_CONF_OPTS += -DWITH_ARCH=$(LINGMO_ARCH) -DWITH_CPU=$(LINGMO_ARCH)
else
# Kodi auto-detects ARCH, tested: arm, aarch64, i386, x86_64
# see project/cmake/scripts/linux/ArchSetup.cmake
KODI_CONF_OPTS += -DWITH_CPU=$(LINGMO_ARCH)
endif
...
ifeq ($(LINGMO_TOOLCHAIN_GCC_AT_LEAST_5),)
KODI_C_FLAGS += -std=gnu99
endif

ifeq ($(LINGMO_PACKAGE_KODI_MYSQL),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBUSB),y)
...
ifeq ($(LINGMO_PACKAGE_LIBXML2)$(LINGMO_PACKAGE_LIBXSLT),yy)
...
ifeq ($(LINGMO_PACKAGE_KODI_BLUEZ),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_DBUS),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBMICROHTTPD),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBSMBCLIENT),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBNFS),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBBLURAY),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBSHAIRPLAY),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_AVAHI),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBCEC),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LCMS2),y)
...
ifeq ($(LINGMO_PACKAGE_KODI_LIBVDPAU),y)
...
ifeq ($(LINGMO_PACKAGE_LIBUDFREAD),y)
