...
ifeq ($(LINGMO_microblaze)$(LINGMO_or1k):$(LINGMO_TOOLCHAIN_GCC_AT_LEAST_6),y:)
...
VLC_CONF_ENV += \
	ax_cv_check_cflags___fstack_protector_strong=$(if $(LINGMO_TOOLCHAIN_HAS_SSP),yes,no)
...
ifeq ($(LINGMO_PACKAGE_LIBDVBPSI),y)
...
ifeq ($(LINGMO_PACKAGE_LIBDVDREAD),y)
...
ifeq ($(LINGMO_PACKAGE_LIBMATROSKA),y)
...
ifeq ($(LINGMO_PACKAGE_LIBMPEG2),y)
...
ifeq ($(LINGMO_PACKAGE_LIBSIDPLAY2),y)
...
ifeq ($(LINGMO_PACKAGE_FFMPEG)$(LINGMO_PACKAGE_LIBVA),yy)
...
ifeq ($(LINGMO_PACKAGE_LIVE555),y)
VLC_CONF_OPTS += --enable-live555
VLC_DEPENDENCIES += live555
ifneq ($(LINGMO_PACKAGE_OPENSSL),y)
...
ifeq ($(LINGMO_PACKAGE_MINIZIP),y)
VLC_DEPENDENCIES += minizip
endif

ifeq ($(LINGMO_PACKAGE_MPG123),y)
...
ifeq ($(LINGMO_PACKAGE_MUSEPACK),y)
...
ifeq ($(LINGMO_PACKAGE_QT5BASE_WIDGETS)$(LINGMO_PACKAGE_QT5SVG),yy)
VLC_CONF_OPTS += --enable-qt
VLC_DEPENDENCIES += qt5base qt5svg
ifeq ($(LINGMO_PACKAGE_XLIB_LIBXEXT)$(LINGMO_PACKAGE_XLIB_LIBXINERAMA)$(LINGMO_PACKAGE_XLIB_LIBXPM),yyy)
...
ifeq ($(LINGMO_PACKAGE_SDL_IMAGE),y)
...
ifeq ($(LINGMO_PACKAGE_WAYLAND)$(LINGMO_PACKAGE_WAYLAND_PROTOCOLS),yy)
...
ifeq ($(LINGMO_PACKAGE_XCB_UTIL_KEYSYMS),y)
