...
GNUTLS_CONF_OPTS = \
	...
	--without-libz-prefix \
	--without-tpm \
	$(if $(LINGMO_PACKAGE_GNUTLS_TOOLS),--enable-tools,--disable-tools) \
	$(if $(LINGMO_PACKAGE_GNUTLS_ENABLE_SSL2),--enable,--disable)-ssl2-support \
	$(if $(LINGMO_PACKAGE_GNUTLS_ENABLE_GOST),--enable,--disable)-gost
GNUTLS_CONF_ENV = gl_cv_socket_ipv6=yes \
	ac_cv_header_wchar_h=$(if $(LINGMO_USE_WCHAR),yes,no) \
	gt_cv_c_wchar_t=$(if $(LINGMO_USE_WCHAR),yes,no) \
	gt_cv_c_wint_t=$(if $(LINGMO_USE_WCHAR),yes,no) \
...
ifeq ($(LINGMO_PACKAGE_GNUTLS_OPENSSL),y)
...
else ifeq ($(LINGMO_PACKAGE_CA_CERTIFICATES),y)
