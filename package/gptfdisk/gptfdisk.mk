...
GPTFDISK_TARGETS_$(LINGMO_PACKAGE_GPTFDISK_GDISK) += gdisk
GPTFDISK_TARGETS_$(LINGMO_PACKAGE_GPTFDISK_SGDISK) += sgdisk
GPTFDISK_TARGETS_$(LINGMO_PACKAGE_GPTFDISK_CGDISK) += cgdisk

GPTFDISK_DEPENDENCIES += util-linux
GPTFDISK_LDLIBS += -luuid

ifeq ($(LINGMO_PACKAGE_GPTFDISK_SGDISK),y)
GPTFDISK_DEPENDENCIES += host-pkgconf popt
GPTFDISK_SGDISK_LDLIBS += `$(PKG_CONFIG_HOST_BINARY) --libs popt`
endif
ifeq ($(LINGMO_PACKAGE_GPTFDISK_CGDISK),y)
