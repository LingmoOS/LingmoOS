...
NDISC6_BIN_$(LINGMO_PACKAGE_NDISC6_NAME2ADDR) += name2addr addr2name
NDISC6_BIN_$(LINGMO_PACKAGE_NDISC6_TCPSPRAY) += tcpspray tcpspray6

NDISC6_SBIN_$(LINGMO_PACKAGE_NDISC6_NDISC6) += ndisc6
NDISC6_SBIN_$(LINGMO_PACKAGE_NDISC6_RDISC6) += rdisc6
NDISC6_SBIN_$(LINGMO_PACKAGE_NDISC6_RDNSSD) += rdnssd
NDISC6_SBIN_$(LINGMO_PACKAGE_NDISC6_RLTRACEROUTE6) += rltraceroute6 tracert6
NDISC6_SBIN_$(LINGMO_PACKAGE_NDISC6_TCPTRACEROUTE6) += tcptraceroute6

define NDISC6_REMOVE_UNNEEDED
	rm -rf $(addprefix $(TARGET_DIR)/usr/bin/,$(NDISC6_BIN_)) \
		$(addprefix $(TARGET_DIR)/usr/sbin/,$(NDISC6_SBIN_))
	$(if $(LINGMO_PACKAGE_NDISC6_RDNSSD),,\
