...
GCC_VERSION = $(call qstrip,$(LINGMO_GCC_VERSION))

ifeq ($(LINGMO_GCC_VERSION_ARC),y)
GCC_SITE = $(call github,foss-for-synopsys-dwc-arc-processors,gcc,$(GCC_VERSION))
GCC_SOURCE = gcc-$(GCC_VERSION).tar.gz
else
# GCC_SITE = $(LINGMO_GNU_MIRROR:/=)/gcc/gcc-$(GCC_VERSION)
...
# tree, and potentially in LINGMO_GLOBAL_PATCH_DIR directories as well.
define HOST_GCC_APPLY_PATCHES
	for patchdir in \
	    package/gcc/$(GCC_VERSION) \
	    $(addsuffix /gcc/$(GCC_VERSION),$(call qstrip,$(LINGMO_GLOBAL_PATCH_DIR))) \
	    $(addsuffix /gcc,$(call qstrip,$(LINGMO_GLOBAL_PATCH_DIR))) ; do \
...
HOST_GCC_COMMON_DEPENDENCIES = \
	...
	host-mpc \
	host-mpfr \
	$(if $(LINGMO_BINFMT_FLAT),host-elf2flt)
...
HOST_GCC_COMMON_CONF_OPTS = \
	...
	--with-mpc=$(HOST_DIR) \
	--with-mpfr=$(HOST_DIR) \
	--with-pkgversion="Buildroot $(LINGMO_VERSION_FULL)" \
...
ifeq ($(LINGMO_TOOLCHAIN_BUILDROOT_MUSL)$(LINGMO_TOOLCHAIN_GCC_AT_LEAST_6),yy)
HOST_GCC_COMMON_CONF_OPTS += --disable-libmpx
endif

# quadmath support requires wchar
ifeq ($(LINGMO_USE_WCHAR)$(LINGMO_TOOLCHAIN_HAS_LIBQUADMATH),yy)
...
ifeq ($(LINGMO_TOOLCHAIN_BUILDROOT_UCLIBC)$(LINGMO_TOOLCHAIN_BUILDROOT_MUSL),y)
HOST_GCC_COMMON_CONF_OPTS += --disable-libsanitizer
endif

# libsanitizer is broken for SPARC
# https://bugs.busybox.net/show_bug.cgi?id=7951
ifeq ($(LINGMO_sparc)$(LINGMO_sparc64),y)
HOST_GCC_COMMON_CONF_OPTS += --disable-libsanitizer
endif
...
ifeq ($(LINGMO_mips64)$(LINGMO_mips64el):$(LINGMO_MIPS_NABI32),y:y)
HOST_GCC_COMMON_CONF_OPTS += --disable-libsanitizer
endif

# libsanitizer bundled in gcc 12 fails to build for mips32 due to
# mixing kernel and user struct stat.
ifeq ($(LINGMO_mips)$(LINGMO_mipsel):$(LINGMO_TOOLCHAIN_GCC_AT_LEAST_12),y:y)
HOST_GCC_COMMON_CONF_OPTS += --disable-libsanitizer
endif
...
ifeq ($(LINGMO_TOOLCHAIN_BUILDROOT_UCLIBC)$(LINGMO_PTHREADS)$(LINGMO_PTHREADS_NONE),yy)
...
ifeq ($(LINGMO_GCC_ENABLE_GRAPHITE),y)
...
ifeq ($(LINGMO_arm)$(LINGMO_armeb)$(LINGMO_mips)$(LINGMO_mipsel)$(LINGMO_mips64)$(LINGMO_mips64el)$(LINGMO_powerpc)$(LINGMO_sparc),y)
...
ifeq ($(LINGMO_POWERPC_CPU_HAS_SPE),y)
HOST_GCC_COMMON_CONF_OPTS += \
	--enable-obsolete \
	--enable-e500_double \
...
ifeq ($(LINGMO_powerpc)$(LINGMO_powerpc64),y)
...
ifeq ($(LINGMO_TOOLCHAIN_USES_MUSL)$(LINGMO_powerpc64),yy)
HOST_GCC_COMMON_CONF_OPTS += \
	--with-abi=elfv2 \
...
ifeq ($(LINGMO_TOOLCHAIN_USES_GLIBC)$(LINGMO_TOOLCHAIN_GCC_AT_LEAST_6)$(LINGMO_powerpc64le),yyy)
HOST_GCC_COMMON_CONF_OPTS += \
	--with-long-double-128
endif
...
HOST_GCC_COMMON_MAKE_OPTS += \
	gcc_cv_libc_provides_ssp=$(if $(LINGMO_TOOLCHAIN_HAS_SSP),yes,no)
...
# addition of gcc patches in LINGMO_GLOBAL_PATCH_DIR, we allow them to be
...
HOST_GCC_COMMON_CCACHE_HASH_FILES += \
	$(sort $(wildcard \
		...
		$(addsuffix /$($(PKG)_RAWNAME)/$(GCC_VERSION)/*.patch,$(call qstrip,$(LINGMO_GLOBAL_PATCH_DIR))) \
		$(addsuffix /$($(PKG)_RAWNAME)/*.patch,$(call qstrip,$(LINGMO_GLOBAL_PATCH_DIR))) \
		$(addsuffix /gcc/$(GCC_VERSION)/*.patch,$(call qstrip,$(LINGMO_GLOBAL_PATCH_DIR))) \
		$(addsuffix /gcc/*.patch,$(call qstrip,$(LINGMO_GLOBAL_PATCH_DIR)))))
...
# and a reference to the Buildroot git revision (LINGMO_VERSION_FULL),
# so substitute those away.
HOST_GCC_COMMON_TOOLCHAIN_WRAPPER_ARGS += -DBR_CCACHE_HASH=\"`\
	printf '%s\n' $(subst $(HOST_DIR),@HOST_DIR@,\
		$(subst --with-pkgversion="Buildroot $(LINGMO_VERSION_FULL)",,$($(PKG)_CONF_OPTS))) \
