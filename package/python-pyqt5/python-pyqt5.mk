...
PYTHON_PYQT5_MODULES = \
	QtCore QtGui \
	$(if $(LINGMO_PACKAGE_QT5BASE_DBUS),QtDBus) \
	$(if $(LINGMO_PACKAGE_QT5BASE_NETWORK),QtNetwork) \
	$(if $(LINGMO_PACKAGE_QT5BASE_OPENGL_LIB),QtOpenGL) \
	$(if $(LINGMO_PACKAGE_QT5BASE_PRINTSUPPORT),QtPrintSupport) \
	$(if $(LINGMO_PACKAGE_QT5BASE_XML),QtXml)
...
ifeq ($(LINGMO_PACKAGE_QT5DECLARATIVE_QUICK)$(LINGMO_PACKAGE_QT5BASE_OPENGL_LIB),yy)
PYTHON_PYQT5_MODULES += \
	QtQuick \
	$(if $(LINGMO_PACKAGE_QT5BASE_WIDGETS),QtQuickWidgets)
endif
endif

ifeq ($(LINGMO_PACKAGE_QT5ENGINIO),y)
PYTHON_PYQT5_DEPENDENCIES += qt5enginio
PYTHON_PYQT5_MODULES += Enginio
endif

ifeq ($(LINGMO_PACKAGE_QT5LOCATION),y)
PYTHON_PYQT5_DEPENDENCIES += qt5location
PYTHON_PYQT5_MODULES += QtPositioning
ifeq ($(LINGMO_PACKAGE_QT5DECLARATIVE_QUICK),y)
PYTHON_PYQT5_MODULES += QtLocation
endif
endif

ifeq ($(LINGMO_PACKAGE_QT5MULTIMEDIA),y)
PYTHON_PYQT5_DEPENDENCIES += qt5multimedia
PYTHON_PYQT5_MODULES += \
	QtMultimedia \
	$(if $(LINGMO_PACKAGE_QT5BASE_WIDGETS),QtMultimediaWidgets)
endif

ifeq ($(LINGMO_PACKAGE_QT5SENSORS),y)
...
PYTHON_PYQT5_QTCORE_DISABLE_FEATURES += $(if $(LINGMO_PACKAGE_QT5BASE_OPENGL),,PyQt_OpenGL)
PYTHON_PYQT5_QTCORE_DISABLE_FEATURES += $(if $(LINGMO_PACKAGE_QT5BASE_OPENGL_DESKTOP),,PyQt_Desktop_OpenGL)
PYTHON_PYQT5_QTCORE_DISABLE_FEATURES += $(if $(LINGMO_PACKAGE_OPENSSL),,PyQt_SSL)
...
# __init__.pyc is needed if LINGMO_PACKAGE_PYTHON_PYC_ONLY is set
