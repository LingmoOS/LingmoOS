...
XENOMAI_VERSION = $(call qstrip,$(LINGMO_PACKAGE_XENOMAI_VERSION))
ifeq ($(LINGMO_PACKAGE_XENOMAI_CUSTOM_TARBALL),y)
XENOMAI_TARBALL = $(call qstrip,$(LINGMO_PACKAGE_XENOMAI_CUSTOM_TARBALL_URL))
XENOMAI_SOURCE = $(notdir $(XENOMAI_TARBALL))
XENOMAI_SITE = $(patsubst %/,%,$(dir $(XENOMAI_TARBALL)))
else ifeq ($(LINGMO_PACKAGE_XENOMAI_CUSTOM_GIT),y)
XENOMAI_SITE = $(call qstrip,$(LINGMO_PACKAGE_XENOMAI_REPOSITORY))
...
ifeq ($(LINGMO_PACKAGE_XENOMAI)$(LINGMO_PACKAGE_XENOMAI_LATEST_VERSION),y)
...
ifeq ($(LINGMO_PACKAGE_XENOMAI_MERCURY),y)
XENOMAI_CONF_OPTS += --with-core=mercury
else
XENOMAI_CONF_OPTS += --with-core=cobalt
endif

ifeq ($(LINGMO_PACKAGE_XENOMAI_ENABLE_SMP),y)
XENOMAI_CONF_OPTS += --enable-smp
else
XENOMAI_CONF_OPTS += --disable-smp
endif

ifeq ($(LINGMO_PACKAGE_XENOMAI_ENABLE_REGISTRY),y)
XENOMAI_DEPENDENCIES += libfuse
XENOMAI_REGISTRY_PATH = $(call qstrip,$(LINGMO_PACKAGE_XENOMAI_ENABLE_REGISTRY_PATH))
...
XENOMAI_CONF_OPTS += $(call qstrip,$(LINGMO_PACKAGE_XENOMAI_ADDITIONAL_CONF_OPTS))
...
ifeq ($(LINGMO_PACKAGE_XENOMAI_TESTSUITE),y)
XENOMAI_CONF_OPTS += --enable-testsuite
else
XENOMAI_CONF_OPTS += --disable-testsuite
endif

ifeq ($(LINGMO_PACKAGE_XENOMAI_RTCAN),)
...
ifeq ($(LINGMO_PACKAGE_XENOMAI_ANALOGY),)
...
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_NATIVE_SKIN),,alchemy)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_POSIX_SKIN),,posix)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_PSOS_SKIN),,psos)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_RTAI_SKIN),,rtai)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_TESTSUITE),,smokey)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_UITRON_SKIN),,uitron)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_VXWORKS_SKIN),,vxworks)
XENOMAI_REMOVE_LIBS_LIST += $(if $(LINGMO_PACKAGE_XENOMAI_VRTX_SKIN),,vrtx)
