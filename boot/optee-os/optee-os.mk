...
OPTEE_OS_VERSION = $(call qstrip,$(LINGMO_TARGET_OPTEE_OS_VERSION))
OPTEE_OS_LICENSE = BSD-2-Clause
ifeq ($(LINGMO_TARGET_OPTEE_OS_LATEST),y)
...
ifeq ($(LINGMO_TARGET_OPTEE_OS_CUSTOM_TARBALL),y)
OPTEE_OS_TARBALL = $(call qstrip,$(LINGMO_TARGET_OPTEE_OS_CUSTOM_TARBALL_LOCATION))
OPTEE_OS_SITE = $(patsubst %/,%,$(dir $(OPTEE_OS_TARBALL)))
OPTEE_OS_SOURCE = $(notdir $(OPTEE_OS_TARBALL))
else ifeq ($(LINGMO_TARGET_OPTEE_OS_CUSTOM_GIT),y)
OPTEE_OS_SITE = $(call qstrip,$(LINGMO_TARGET_OPTEE_OS_CUSTOM_REPO_URL))
OPTEE_OS_SITE_METHOD = git
else
OPTEE_OS_SITE = $(call github,OP-TEE,optee_os,$(OPTEE_OS_VERSION))
endif

ifeq ($(LINGMO_TARGET_OPTEE_OS):$(LINGMO_TARGET_OPTEE_OS_LATEST),y:)
BR_NO_CHECK_HASH_FOR += $(OPTEE_OS_SOURCE)
endif

OPTEE_OS_DEPENDENCIES = host-openssl host-python3 host-python-pyelftools

ifeq ($(LINGMO_TARGET_OPTEE_OS_NEEDS_PYTHON_CRYPTOGRAPHY),y)
OPTEE_OS_DEPENDENCIES += host-python-cryptography
else
OPTEE_OS_DEPENDENCIES += host-python-pycryptodomex
endif

ifeq ($(LINGMO_TARGET_OPTEE_OS_NEEDS_DTC),y)
...
# variables
OPTEE_OS_MAKE_OPTS += PLATFORM=$(call qstrip,$(LINGMO_TARGET_OPTEE_OS_PLATFORM))
ifneq ($(call qstrip,$(LINGMO_TARGET_OPTEE_OS_PLATFORM_FLAVOR)),)
OPTEE_OS_MAKE_OPTS += PLATFORM_FLAVOR=$(call qstrip,$(LINGMO_TARGET_OPTEE_OS_PLATFORM_FLAVOR))
endif
OPTEE_OS_MAKE_OPTS += $(call qstrip,$(LINGMO_TARGET_OPTEE_OS_ADDITIONAL_VARIABLES))
...
OPTEE_OS_IMAGE_FILES = $(call qstrip,$(LINGMO_TARGET_OPTEE_OS_CORE_IMAGES))

OPTEE_OS_CUSTOM_DTS_PATH = $(call qstrip,$(LINGMO_TARGET_OPTEE_OS_CUSTOM_DTS_PATH))

ifeq ($(LINGMO_TARGET_OPTEE_OS_CORE),y)
...
endef
endif # LINGMO_TARGET_OPTEE_OS_CORE

ifeq ($(LINGMO_TARGET_OPTEE_OS_SERVICES),y)
...
endef
endif # LINGMO_TARGET_OPTEE_OS_SERVICES

ifeq ($(LINGMO_TARGET_OPTEE_OS_SDK),y)
...
endef
endif # LINGMO_TARGET_OPTEE_OS_SDK
...
ifeq ($(LINGMO_TARGET_OPTEE_OS)$(BR_BUILDING),yy)
ifeq ($(call qstrip,$(LINGMO_TARGET_OPTEE_OS_PLATFORM)),)
$(error No OP-TEE OS platform set. Check your LINGMO_TARGET_OPTEE_OS_PLATFORM setting)
endif

ifeq ($(LINGMO_TARGET_OPTEE_OS_CUSTOM_TARBALL),y)
ifeq ($(call qstrip,$(LINGMO_TARGET_OPTEE_OS_CUSTOM_TARBALL_LOCATION)),)
$(error No tarball location specified. Please check LINGMO_TARGET_OPTEE_OS_CUSTOM_TARBALL_LOCATION)
endif
endif

ifeq ($(LINGMO_TARGET_OPTEE_OS_CUSTOM_GIT),y)
ifeq ($(call qstrip,$(LINGMO_TARGET_OPTEE_OS_CUSTOM_REPO_URL)),)
$(error No repository specified. Please check LINGMO_TARGET_OPTEE_OS_CUSTOM_REPO_URL)
endif
endif

endif # LINGMO_TARGET_OPTEE_OS && LINGMO_BUILDING
