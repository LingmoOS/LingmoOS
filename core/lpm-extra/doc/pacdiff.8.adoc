...
backup entries found in the local lpm db. For every found file the option is
...
*-p, \--lpmdb*::
	Scan active config files from lpm database. (default)
...
*-c, \--cachedir <dir>*::
	Scan 'dir' instead as the lpm cache for 3-way merge base candidates.
