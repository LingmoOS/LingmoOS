...
{
public:
    explicit Atom(const QByteArray &name, bool onlyIfExists = false, xcb_connection_t *c = Lingmo::X11::connection())
        : m_connection(c)
        , m_retrieved(false)
...
public:
    Atoms()
        : xembedAtom("_XEMBED")
        , selectionAtom(xcb_atom_name_by_screen("_NET_SYSTEM_TRAY", Lingmo::X11::appScreen()))
