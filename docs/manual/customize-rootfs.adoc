...
Root filesystem overlays (+LINGMO_ROOTFS_OVERLAY+)::
+
A filesystem overlay is a tree of files that is copied directly
  ...
  feature, set config option +LINGMO_ROOTFS_OVERLAY+ (in the +System
  ...
  the copy.
+
When +LINGMO_ROOTFS_MERGED_USR+ is enabled, then the overlay must not
...
Post-build scripts (+LINGMO_ROOTFS_POST_BUILD_SCRIPT+)::
+
Post-build scripts are shell scripts called 'after' Buildroot builds
  ...
  post-build scripts in config option +LINGMO_ROOTFS_POST_BUILD_SCRIPT+ (in
...
The post-build scripts are run with the main Buildroot tree as current
  ...
  +LINGMO_ROOTFS_POST_SCRIPT_ARGS+ is not empty, these arguments will be
...
Custom target skeleton (+LINGMO_ROOTFS_SKELETON_CUSTOM+)::
+
...
To enable this feature, enable config option
  +LINGMO_ROOTFS_SKELETON_CUSTOM+ and set +LINGMO_ROOTFS_SKELETON_CUSTOM_PATH+
...
Custom skeletons don't need to contain the '/bin', '/lib' or '/sbin'
  ...
  When +LINGMO_ROOTFS_MERGED_USR+ is enabled, then the custom skeleton must
...
Post-fakeroot scripts (+LINGMO_ROOTFS_POST_FAKEROOT_SCRIPT+)::
+
