...
* +BUSYBOX_CONFIG_FILE=<path/to/.config>+, path to
  ...
  is the recommended way of setting it.
  +
* +LINGMO_CCACHE_DIR+ to override the directory where
  Buildroot stores the cached files when using ccache.
  +
* +LINGMO_DL_DIR+ to override the directory in which
  ...
  directory.
* +LINGMO_GRAPH_ALT+, if set and non-empty, to use an alternate color-scheme in
  build-time graphs
* +LINGMO_GRAPH_OUT+ to set the filetype of generated graphs, either +pdf+ (the
  default), or +png+.
* +LINGMO_GRAPH_DEPS_OPTS+ to pass extra options to the dependency graph; see
  xref:graph-depends[] for the accepted options
* +LINGMO_GRAPH_DOT_OPTS+ is passed verbatim as options to the +dot+ utility to
  draw the dependency graph.
* +LINGMO_GRAPH_SIZE_OPTS+ to pass extra options to the size graph; see
...
format. However, by passing the +LINGMO_GRAPH_OUT+ environment variable, you
...
--------------------------------
LINGMO_GRAPH_OUT=svg make graph-depends
--------------------------------

The +graph-depends+ behaviour can be controlled by setting options in the
+LINGMO_GRAPH_DEPS_OPTS+ environment variable. The accepted options are:
...
--------------------------------
LINGMO_GRAPH_DEPS_OPTS='-d 3 --no-transitive --colors=red,green,blue' make graph-depends
--------------------------------
...
format can be selected using the +LINGMO_GRAPH_OUT+ environment variable. The
only other format supported is PNG:

----------------
LINGMO_GRAPH_OUT=png make graph-build
----------------
...
Just like for the duration graph, a +LINGMO_GRAPH_OUT+ environment variable
is supported to adjust the output file format. See xref:graph-depends[]
for details about this environment variable.

Additionally, one may set the environment variable +LINGMO_GRAPH_SIZE_OPTS+
...
adjusted using the +LINGMO_JLEVEL+ configuration option.
...
. Enable the option +LINGMO_PER_PACKAGE_DIRECTORIES+ in the Buildroot
configuration

. Use +make -jN+ when starting the Buildroot build

Internally, the +LINGMO_PER_PACKAGE_DIRECTORIES+ will enable a mechanism
