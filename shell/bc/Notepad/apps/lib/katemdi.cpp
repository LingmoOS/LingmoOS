    ...
    , m_mw(mw)
{
    ...
    // hide tool views menu for Notepad mode
...
void MainWindow::startRestore(KConfigBase *config, const QString &group)
{
    ...
    // Notepad uses no sidebars, avoid all work beside windows sizes restoring above
    if (KateApp::isNotepad()) {
...
void MainWindow::saveSession(KConfigGroup &config)
{
    ...
    // Notepad uses no sidebars, avoid all work beside windows sizes saving
    if (KateApp::isNotepad()) {
        return;
    }
