...
{
    qmlRegisterAnonymousType<QAction>("org.kde.lingmo.plasmoid", 1);
}
...
void PlasmoidItem::init()
{
    ...
    connect(applet, &Lingmo::Applet::contextualActionsAboutToShow, this, &PlasmoidItem::contextualActionsAboutToShow);

    connect(applet, &Lingmo::Applet::titleChanged, this, [this]() {
    ...
    if (applet->containment()) {
        connect(applet->containment(), &Lingmo::Containment::screenChanged, this, &PlasmoidItem::screenChanged);

        // Screen change implies geo change for good measure.
        connect(applet->containment(), &Lingmo::Containment::screenChanged, this, &PlasmoidItem::screenGeometryChanged);

        connect(applet->containment()->corona(), &Lingmo::Corona::screenGeometryChanged, this, [this](int id) {
            if (id == AppletQuickItem::applet()->containment()->screen()) {
                Q_EMIT screenGeometryChanged();
            }
        });

        connect(applet->containment()->corona(), &Lingmo::Corona::availableScreenRegionChanged, this, &ContainmentItem::availableScreenRegionChanged);
        connect(applet->containment()->corona(), &Lingmo::Corona::availableScreenRectChanged, this, &ContainmentItem::availableScreenRectChanged);
    }
    ...
    connect(applet, &Lingmo::Applet::activated, this, [=, this]() {
    ...
    connect(applet, &Lingmo::Applet::destroyedChanged, this, &PlasmoidItem::destroyedChanged);
...
{
    if (Lingmo::Containment *c = applet()->containment()) {
        return c->screen();
    }
...
bool PlasmoidItem::eventFilter(QObject *watched, QEvent *event)
{
    if (event->type() == QEvent::MouseButtonPress) {
        ...
        Lingmo::Containment *c = applet()->containment();
        if (c) {
            const QString trigger = Lingmo::ContainmentActions::eventToString(event);
            Lingmo::ContainmentActions *plugin = c->containmentActions().value(trigger);
            if (!plugin) {
                return false;
            }
