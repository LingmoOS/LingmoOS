...
const char ThemePrivate::themeRcFile[] = "lingmorc";
...
{
    const QString baseName = QLatin1String(LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/") % theme;
    QString configPath = QSP::locate(QSP::GenericDataLocation, baseName + QLatin1String("/lingmorc"));
...
KPluginMetaData metaDataForTheme(const QString &theme)
{
    const QString packageBasePath =
        QSP::locate(QSP::GenericDataLocation, QLatin1String(LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/") % theme, QSP::LocateDirectory);
    if (packageBasePath.isEmpty()) {
        qWarning(LOG_LINGMO) << "Could not locate lingmo theme" << theme << "in" << LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/"
    ...
    if (QFileInfo::exists(packageBasePath + QLatin1String("/metadata.json"))) {
        ...
        qWarning(LOG_LINGMO) << "The theme" << theme
        ...
        qCWarning(LOG_LINGMO) << "Could not locate metadata for theme" << theme;
        return {};
    }
}
    ...
    , apiRevision(0)
{
    ...
    kSvgImageSet->setBasePath(QStringLiteral(LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/"));
...
KConfigGroup &ThemePrivate::config()
{
    if (!cfg.isValid()) {
        ...
        if (!useGlobal) {
            ...
            if (!app.isEmpty()) {
#ifndef NDEBUG
                // qCDebug(LOG_LINGMO) << "using theme for app" << app;
#endif
...
bool ThemePrivate::useCache()
{
    ...
    if (cacheTheme) {
        ...
        QString cacheFile = QLatin1String("lingmo_theme_") + themeName;
...
{
    QString subdir = QLatin1String(LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/") % theme % type % image;
...
void ThemePrivate::compositingChanged(bool active)
{
    ...
    if (compositingActive != active) {
        ...
        // qCDebug(LOG_LINGMO) << QTime::currentTime();
        scheduleThemeChangeNotification(PixmapCache | SvgElementsCache);
        if (active) {
...
{
    // qCDebug(LOG_LINGMO) << cachesToDiscard;
...
{
    qCDebug(LOG_LINGMO) << "settingsFile: " << file;
...
void ThemePrivate::settingsChanged(bool emitChanges)
{
    ...
    // qCDebug(LOG_LINGMO) << "Settings Changed!";
...
QColor ThemePrivate::color(Theme::ColorRole role, Theme::ColorGroup group) const
{
    ...
    // Before 5.0 Lingmo theme really only used Normal and Button
...
void ThemePrivate::setThemeName(const QString &tempThemeName, bool writeSettings, bool emitChanged)
{
    ...
    const QString colorsFile = realTheme
        ? QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                 QLatin1String(LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/") % theme % QLatin1String("/colors"))
        : QString();

    // qCDebug(LOG_LINGMO) << "we're going for..." << colorsFile << "*******************";
    ...
    tooltipColorScheme = KColorScheme(QPalette::Active, KColorScheme::Tooltip, colors);
    palette = KColorScheme::createApplicationPalette(colors);
    const QString wallpaperPath = QLatin1String(LINGMO_RELATIVE_DATA_INSTALL_DIR "/desktoptheme/") % theme % QLatin1String("/wallpapers/");
