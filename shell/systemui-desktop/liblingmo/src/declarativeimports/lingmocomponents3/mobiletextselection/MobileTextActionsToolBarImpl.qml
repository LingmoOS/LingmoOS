...
Popup {
    ...
    y: {
        ...
        if (desiredY >= 0) {
            ...
            return Math.min(Math.max(0, controlRoot.mapToItem(root.parent, 0, controlRoot.positionToRectangle(controlRoot.selectionEnd).y + Math.round(LingmoUI.Units.gridUnit * 1.5)).y), parent.height - root.height);
        }
    }
