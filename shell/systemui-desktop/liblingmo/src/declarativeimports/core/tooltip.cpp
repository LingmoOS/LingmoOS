...
ToolTip::ToolTip(QQuickItem *parent)
    ...
    , m_location(Lingmo::Types::Floating)
    ...
    , m_usingDialog(false)
{
    ...
    const QString configFile = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + QStringLiteral("/lingmorc");
...
{
    if (!file.endsWith(QLatin1String("lingmorc"))) {
        return;
    }

    KSharedConfig::openConfig(QStringLiteral("lingmorc"))->reparseConfiguration();
    loadSettings();
}

void ToolTip::loadSettings()
{
    KConfigGroup cfg = KConfigGroup(KSharedConfig::openConfig(QStringLiteral("lingmorc")), QStringLiteral("LingmoToolTips"));
...
void ToolTip::showToolTip()
{
    ...
    Lingmo::Types::Location location = m_location;
    if (m_location == Lingmo::Types::Floating) {
        QQuickItem *p = parentItem();
        while (p) {
            LingmoQuick::AppletQuickItem *appletItem = qobject_cast<LingmoQuick::AppletQuickItem *>(p);
            if (appletItem) {
...
Lingmo::Types::Location ToolTip::location() const
{
    return m_location;
}

void ToolTip::setLocation(Lingmo::Types::Location location)
{
