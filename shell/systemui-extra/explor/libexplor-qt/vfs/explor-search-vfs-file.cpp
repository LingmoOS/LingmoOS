...
#include "explor-search-vfs-file.h"
#include "explor-search-vfs-file-enumerator.h"
...
/* -- GFileIface -- */
static void explor_search_vfs_file_g_file_iface_init(GFileIface *iface);

static GFile *explor_search_vfs_file_dup(GFile *file);
static guint explor_search_vfs_file_hash(GFile *file);//unused
static gboolean explor_search_vfs_file_equal(GFile *file1, GFile *file2);//unused
static gboolean explor_search_vfs_file_is_native(GFile *file);//unused
static gboolean explor_search_vfs_file_has_uri_scheme(GFile *file, const char *uri_scheme);//unused
static char *explor_search_vfs_file_get_uri_sheceme(GFile *file);
static char *explor_search_vfs_file_get_basename(GFile *file);//unused
static char *explor_search_vfs_file_get_path(GFile *file);//unused
static char *explor_search_vfs_file_get_uri(GFile *file);
static char *explor_search_vfs_file_get_parse_name(GFile *file);//unused
static GFile *explor_search_vfs_file_get_parent(GFile *file);//unused
static gboolean explor_search_vfs_file_prefix_matches(GFile *prefix, GFile *file);//unused
static char *explor_search_vfs_file_get_relative_path(GFile *file, const char *relative_path);//unused
static GFile *explor_search_vfs_file_resolve_relative_path(GFile *file,
        const char *relative_path);
static GFile *explor_search_vfs_file_get_child_for_display_name (GFile *file,
        const char *display_name,
        GError **err);//unused

static GFileInfo *explor_search_vfs_file_query_info(GFile *file,
...
G_DEFINE_TYPE_EXTENDED(PeonySearchVFSFile,
                       explor_search_vfs_file,
                       G_TYPE_OBJECT,
                       0,
                       G_ADD_PRIVATE(PeonySearchVFSFile)
                       G_IMPLEMENT_INTERFACE(G_TYPE_FILE, explor_search_vfs_file_g_file_iface_init));
...
static void explor_search_vfs_file_class_init (PeonySearchVFSFileClass *klass)
{
...
GFileEnumerator *explor_search_vfs_file_enumerate_children(GFile *file,
        const char *attribute,
        GFileQueryInfoFlags flags,
        GCancellable *cancellable,
        GError **error)
{
    qDebug() << "explor_search_vfs_file_enumerate_children";
    auto search_vfs_file = PEONY_SEARCH_VFS_FILE(file);

    return explor_search_vfs_file_enumerate_children_internal(file, attribute, flags, cancellable, error);
}

GFileInfo *explor_search_vfs_file_query_info(GFile *file,
        const char *attributes,
        GFileQueryInfoFlags flags,
        GCancellable *cancellable,
        GError **error)
...
static void explor_search_vfs_file_init(PeonySearchVFSFile *self)
{
    PeonySearchVFSFilePrivate *priv = (PeonySearchVFSFilePrivate*)explor_search_vfs_file_get_instance_private(self);
    self->priv = priv;
    priv->uri = nullptr;
}

char *explor_search_vfs_file_get_uri(GFile *file)
{
    auto vfs_file = PEONY_SEARCH_VFS_FILE(file);
    return g_strdup(vfs_file->priv->uri);
}

GFile *explor_search_vfs_file_get_parent(GFile *file)
{
    Q_UNUSED(file);
    return nullptr;
}

gboolean explor_search_vfs_file_is_native(GFile *file)
{
    return false;
}
...
static void explor_search_vfs_file_g_file_iface_init(GFileIface *iface)
{
    qDebug() << "explor_search_vfs_file_g_file_iface_init";
    iface->dup = explor_search_vfs_file_dup;
    iface->get_parent = explor_search_vfs_file_get_parent;
    iface->is_native = explor_search_vfs_file_is_native;
    iface->enumerate_children = explor_search_vfs_file_enumerate_children;
    iface->query_info = explor_search_vfs_file_query_info;
    iface->get_uri = explor_search_vfs_file_get_uri;
    iface->get_path = explor_search_vfs_file_get_path;
    iface->resolve_relative_path = explor_search_vfs_file_resolve_relative_path;
    iface->get_uri_scheme = explor_search_vfs_file_get_uri_sheceme;
}

/// dup, get_parent and is_native(and so on) is used in explor-qt,
/// such as FileEnumerator and FileInfo, so i must need
/// implement these search vfs file's functions.
GFile *explor_search_vfs_file_dup(GFile *file)
{
...
GFile *explor_search_vfs_file_resolve_relative_path(GFile *file, const char *relative_path)
{
    Q_UNUSED(file);
...
GFile *explor_search_vfs_file_new_for_uri(const char *uri)
{
...
char *explor_search_vfs_file_get_path(GFile *file)
{
    return nullptr;
}

void explor_search_vfs_file_enumerator_parse_uri(PeonySearchVFSFileEnumerator *enumerator,
        const char *uri)
{
...
GFileEnumerator *explor_search_vfs_file_enumerate_children_internal(GFile *file,
        const char *attributes,
        GFileQueryInfoFlags flags,
        GCancellable *cancellable,
        GError **error)
{
    ...
    auto enumerator = PEONY_SEARCH_VFS_FILE_ENUMERATOR(g_object_new(PEONY_TYPE_SEARCH_VFS_FILE_ENUMERATOR,
                      "container", file,
                      nullptr));
    explor_search_vfs_file_enumerator_parse_uri(enumerator, vfs_file->priv->uri);
