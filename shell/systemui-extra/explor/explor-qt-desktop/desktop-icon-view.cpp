...
#define ITEM_POS_ATTRIBUTE "metadata::explor-qt-desktop-item-position"
#define PANEL_SETTINGS "org.lingmo.panel.settings"
#define LINGMO_STYLE_SETTINGS "org.lingmo.style"
#define RESTORE_ITEM_POS_ATTRIBUTE "metadata::explor-qt-desktop-restore-item-position"
#define RESTORE_EXTEND_ITEM_POS_ATTRIBUTE "metadata::explor-qt-desktop-restore-extend-item-position"
#define RESTORE_SINGLESCREEN_ITEM_POS_ATTRIBUTE "metadata::explor-qt-desktop-restore-singlescreen-item-position"
...
//        });
    ...
    m_explorDbusSer = new PeonyDbusService(this);
    m_explorDbusSer->DbusServerRegister();
...
void DesktopIconView::openFileByUri(QString uri)
{
    ...
    job->connect(job, &FileInfoJob::queryAsyncFinished, [=]() {
        if ((info->isDir() || info->isVolume() || info->isVirtual())) {
            ...
            p.startDetached("/usr/bin/explor", QStringList()<<strq<<"%U&");
#endif
        } else {
...
void DesktopIconView::setFileMetaInfoPos(const QString &uri, const QPoint &pos)
{
    ...
    if (metaInfo) {
        ...
        metaInfo->setMetaInfoStringList(ITEM_POS_ATTRIBUTE, topLeft);
        metaInfo->setMetaInfoInt("explor-qt-desktop-id", m_id);
    }
}
...
void DesktopIconView::setDefaultZoomLevel(ZoomLevel level)
{
    ...
    if (metaInfo) {
        ...
        metaInfo->setMetaInfoInt("explor-qt-desktop-zoom-level", int(m_zoom_level));
    }
...
DesktopIconView::ZoomLevel DesktopIconView::zoomLevel() const
{
    ...
    auto metaInfo = FileMetaInfo::fromUri("computer:///");
    if (metaInfo) {
        auto i = metaInfo->getMetaInfoInt("explor-qt-desktop-zoom-level");
        return ZoomLevel(i);
    }

    GFile *computer = g_file_new_for_uri("computer:///");
    GFileInfo *info = g_file_query_info(computer,
                                        "metadata::explor-qt-desktop-zoom-level",
                                        G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                        nullptr,
                                        nullptr);
    char* zoom_level = g_file_info_get_attribute_as_string(info, "metadata::explor-qt-desktop-zoom-level");
    if (!zoom_level) {
...
void DesktopIconView::saveExtendItemInfo()
{
    ...
    for (int i = 0; i < m_proxy_model->rowCount(); i++) {
        ...
        if (metaInfo) {
            ...
            metaInfo->setMetaInfoInt("explor-qt-desktop-id", 0);
        }
    }
}
    ...
    for (int i = 0; i < m_model->rowCount(); i++) {
        ...
        if (metaInfo) {
            ...
            if (list.count() == 3) {
                ...
                qDebug() << "[DesktopIconView::resetExtendItemInfo] uri:"<<uri<<" explor-qt-desktop-restore-extend-item-position:"<<list<<" id:"<<m_id;
                if (id.toInt() != m_id) {
                    continue;
                }
