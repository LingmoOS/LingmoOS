...
void DesktopMenu::openWindow(const QString &uri)
{
    ...
    p.startDetached();
#else
    p.startDetached("/usr/bin/explor", QStringList()<<"--show-folders"<<uri);
#endif
}
...
void DesktopMenu::openWindow(const QStringList &uris)
{
    ...
    p.startDetached();
#else
    p.startDetached("/usr/bin/explor", QStringList()<<"--show-folders"<<args);
#endif
}
...
void DesktopMenu::showProperties(const QString &uri)
{
    ...
    p.startDetached();
#else
    p.startDetached("/usr/bin/explor", QStringList()<<"--show-properties"<<url.toEncoded());
#endif
}
...
void DesktopMenu::showProperties(const QStringList &uris)
{
    ...
    if (uris.contains("trash:///") && uris.count() > 1) {
        ...
        QtConcurrent::run([=]() {
            ...
            p.startDetached();
#else
            p.startDetached("/usr/bin/explor", QStringList()<<"--show-properties"<<args);
#endif
        });
    }
