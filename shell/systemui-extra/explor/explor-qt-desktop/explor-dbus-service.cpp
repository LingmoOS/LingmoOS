...
{
    QDBusConnection::sessionBus().unregisterService("org.lingmo.explor");
    QDBusConnection::sessionBus().registerService("org.lingmo.explor");
    QDBusConnection::sessionBus().registerObject("/org/lingmo/explor", this, QDBusConnection :: ExportAllSlots | QDBusConnection::ExportAllSignals);
}

QString PeonyDbusService::GetSecurityConfigPath()
{
    QString jsonPath=QDir::homePath()+"/.config/explor-security-config.json";
