...
#define DirectoryViewPluginIface_iid "org.lingmo.explor-qt.plugin-iface.DirectoryViewPluginInterface"
 ...
 * \details
 * It is a few complex to implement a explor-qt's directory view's plugin.
 * There are 3 interface class must to be implemented.
 * 1. DirectoryViewPluginIface:
 * This is the entry of a plugin, explor-qt's application will dynamicly
 * load external view modules when it started. After a plugin loaded
 * it will be registered in explor-qt's application globally, then we can
 ...
 * In explor-qt the directory view is desgin to be extensive,
 * and every kind of view will have a global factory to create
 * the view's instance. the factories will be managed by explor-qt
 * in global, too.
 * A explor-qt's file item view will implement this interface.
 * \note
