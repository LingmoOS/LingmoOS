...
namespace {
const char *serverUnixPath = "unix:path=/tmp/org.lingmo.aisdk";
}
...
const std::string ServerProxy::getUnixPath() { return serverUnixPath; }
...
void ServerProxy::init() {
    ...
    connection_ = g_dbus_connection_new_for_address_sync(
        serverUnixPath, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT,
        nullptr, /* GDBusAuthObserver */
        nullptr, /* GCancellable */
        &error);
    if (connection_ == nullptr) {
        g_printerr("Error connecting to D-Bus address %s: %s\n",
                   serverUnixPath, error->message);
        g_error_free(error);
