...
const char *modelManagerServiceObjectPath = "/org/lingmo/aisdk/modelmanagerservice";
...
void ModelManagerServiceProxy::init() {
    ...
    delegate_ = aisdk_model_manager_service_proxy_new_sync(
        connection, G_DBUS_PROXY_FLAGS_NONE, nullptr, modelManagerServiceObjectPath,
        nullptr, &error);
    if (delegate_ == nullptr) {
        g_printerr("Error creating model manager service proxy %s: %s\n",
                   modelManagerServiceObjectPath, error->message);
        g_error_free(error);
    }
}
