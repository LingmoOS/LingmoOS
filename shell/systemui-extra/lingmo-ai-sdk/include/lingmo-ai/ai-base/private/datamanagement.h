...
*/
DataManagementResult data_management_create_session(
    DataManagementSession* session, DataManagementType type, int uid);
 ...
 * result 需要使用 data_management_free_check_file_format_result 释放，未被释放的 result
 * 会在 session 销毁时释放
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_check_file_format(
    DataManagementSession session, const char* filepath, char* result);

/*!
 * \brief 释放文件检查的结果
 * \param session 请求所使用的会话，通过 data_management_create_session 创建
 * \param result 要被释放的结果，通过 data_management_check_file_format 获得
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_free_check_file_format_result(
    DataManagementSession session, char* result);
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_similarity_search(
    DataManagementSession session, const char* search_conditions, char* search_result);
...
*/
DataManagementResult data_management_free_search_result(
    DataManagementSession session, char* search_result);
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_add_text_files(
    DataManagementSession session, const char* fileinfos);
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_add_image_files(
    DataManagementSession session, const char* fileinfos);
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_delete_files(
    DataManagementSession session, const char* fileinfos);
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_update_files_name(
    DataManagementSession session, const char* fileinfos);
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_update_vector_data(
    DataManagementSession session, const char* fileinfos);

/*!
 * \brief 获取当前会话所有文件及修改时间列表
 * \param fileinfos 文件列表，为 json 格式，包含文件的绝对路径和文件修改时间的UNIX时间戳
 * {
 *   [
 *     {
 *       "filepath": "/path/to/file3.jpeg",
 ...
 * \return 成功将返回 SearchResult::SEARCH_SUCCESS，失败则会返回对应错误码
*/
DataManagementResult data_management_get_all_fileinfos(
    DataManagementSession session, char* fileinfos);
