...
//G_DEFINE_TYPE(PeonySearchVFSFileEnumerator, explorer_search_vfs_file_enumerator, G_TYPE_FILE_ENUMERATOR)
G_DEFINE_TYPE_WITH_PRIVATE(PeonySearchVFSFileEnumerator,
                           explorer_search_vfs_file_enumerator,
                           G_TYPE_FILE_ENUMERATOR)

static void explorer_search_vfs_file_enumerator_parse_uri(PeonySearchVFSFileEnumerator *enumerator,
        const char *uri);

static gboolean explorer_search_vfs_file_enumerator_is_file_match(PeonySearchVFSFileEnumerator *enumerator, const QString &uri);

static void explorer_search_vfs_file_enumerator_add_directory_to_queue(PeonySearchVFSFileEnumerator *enumerator, const QString &directory_uri) {
...
static void explorer_search_vfs_file_enumerator_init(PeonySearchVFSFileEnumerator *self)
{
    PeonySearchVFSFileEnumeratorPrivate *priv = (PeonySearchVFSFileEnumeratorPrivate*)explorer_search_vfs_file_enumerator_get_instance_private(self);
    self->priv = priv;
...
static void explorer_search_vfs_file_enumerator_class_init (PeonySearchVFSFileEnumeratorClass *klass)
{
    ...
    while (!enumerate_queue->isEmpty() && !search_engine) {
        ...
        if (isDir && search_enumerator->priv->recursive) {
            if (!search_enumerator->priv->search_hidden) {
                if (!uri.contains("/.")) {
                    explorer_search_vfs_file_enumerator_add_directory_to_queue(search_enumerator, uri);
                }
            } else {
                explorer_search_vfs_file_enumerator_add_directory_to_queue(search_enumerator, uri);
            }
        ...
        //match
        if (explorer_search_vfs_file_enumerator_is_file_match(search_enumerator, uri)) {
...
gboolean explorer_search_vfs_file_enumerator_is_file_match(PeonySearchVFSFileEnumerator *enumerator, const QString &uri)
{
