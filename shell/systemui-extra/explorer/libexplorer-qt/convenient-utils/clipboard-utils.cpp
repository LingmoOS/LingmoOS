...
static bool m_is_explorer_cut = false;
...
ClipboardUtils::ClipboardUtils(QObject *parent) : QObject(parent)
{
    ...
    connect(QApplication::clipboard(), &QClipboard::dataChanged, [=]() {
        ...
        if (!data->hasFormat("explorer-qt/is-cut")) {
...
//    data->setData("explorer-qt/encoded-uris", string.toUtf8());
//    data->setText(string);
...
bool ClipboardUtils::isSearchTab()
{
    ...
    if (mimeData && mimeData->hasFormat("explorer-qt/is-search")) {
        isSearchTab = QVariant(mimeData->data("explorer-qt/is-search")).toBool();
    }
    return isSearchTab;
}
...
bool ClipboardUtils::isClipboardFilesBeCut()
{
    if (isClipboardHasFiles()) {
        auto data = QApplication::clipboard()->mimeData();
        if (data->hasFormat("explorer-qt/is-cut")) {
            QVariant var(data->data("explorer-qt/is-cut"));
            return var.toBool();
        }
    }
    return false;
...
QStringList ClipboardUtils::getClipboardFilesUris()
{
    ...
    auto explorerText = mimeData->data("explorer-qt/encoded-uris");

    if (!explorerText.isEmpty()) {
        qDebug() << "explorer text:" << explorerText;
        auto byteArrays = explorerText.split(' ');
