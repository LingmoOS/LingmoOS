...
FileOperationManager::FileOperationManager(QObject *parent) : QObject(parent)
{
    ...
    m_iface = new QDBusInterface("org.lingmo.explorer", "/org/lingmo/explorer", "org.lingmo.explorer", QDBusConnection::sessionBus());
    if (!m_iface->isValid()){
        qCritical() << "Create /org/lingmo/explorer Interface Failed " << QDBusConnection::sessionBus().lastError();
        return;
    }
...
void FileOperationManager::manuallyNotifyDirectoryChanged(FileOperationInfo *info)
{
    ...
    if (info->m_is_search && info->operationType() == FileOperationInfo::Move) {
        ...
        QDBusMessage msg = QDBusMessage::createSignal("/org/lingmo/explorer", "org.lingmo.explorer", "moveFilesToAnotherProcCompleted");
        msg << uris;
