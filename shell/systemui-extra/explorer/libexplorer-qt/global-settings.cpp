...
{
    m_settings = new QSettings("org.lingmo", "explorer-qt-preferences", this);
    m_explorerGSettings = new QGSettings("org.lingmo.explorer.settings", "/org/lingmo/explorer/settings/", this);
    ...
    if (QGSettings::isSchemaInstalled("org.lingmo.explorer.settings")) {
        connect(m_explorerGSettings, &QGSettings::changed, this, [=] (const QString &key) {
            m_cache.remove(key);
            m_cache.insert(key, m_explorerGSettings->get(key));
            Q_EMIT this->valueChanged(key);
        });

        for (auto key : m_explorerGSettings->keys()) {
            m_cache.remove(key);
            m_cache.insert(key, m_explorerGSettings->get(key));
        }
    }
    ...
    if (QGSettings::isSchemaInstalled("org.lingmo.explorer.settings")) {
        m_explorer_gsettings = new QGSettings("org.lingmo.explorer.settings", QByteArray(), this);
        ...
        if(!isExist(INIT_FOR_FIRST_TIME)){
             ...
             * 由于建行从explorer 3.2.3.0-0k0.1升级，此版本已经合入show-hidden-file的gsettings，但是没有合入#101227，导致升级到当前版本时走了错误的流程
             */
            bool currentShowHidden = m_explorer_gsettings->get(SHOW_HIDDEN_PREFERENCE).toBool();
            if (!currentShowHidden) {
                /* /usr/share/glib-2.0/schemas/org.lingmo.explorer.settings.gschema.xml文件首次初始化时，
                 * SHOW_HIDDEN_PREFERENCE字段为 "org.lingmo/explorer-qt-preferences" 文件中"show-hidden"的值*/
        ...
        connect(m_explorer_gsettings, &QGSettings::changed, this, [=](const QString &key) {
            ...
            if ((SHOW_HIDDEN_PREFERENCE == key) || (SHOW_FILE_EXTENSION == key) || key == DISPLAY_STANDARD_ICONS || key == USE_GLOBAL_DEFAULT_SORTING ||
                ...
                if (m_cache.value(key) != m_explorer_gsettings->get(key).toBool())
                {
                    m_cache.remove(key);
                    m_cache.insert(key, m_explorer_gsettings->get(key).toBool());
                }
                ...
                m_cache.remove(key);
                m_cache.insert(key, m_explorer_gsettings->get(key));
            }
        ...
        for (auto key : m_explorer_gsettings->keys()) {
            m_cache.remove(key);
            m_cache.insert(key, m_explorer_gsettings->get(key));
        }
        m_showCreateTime = m_cache.value(SHOW_CREATE_TIME).toBool();
...
void GlobalSettings::getLingmoUIStyle()
{
    ...
    if (QGSettings::isSchemaInstalled("org.lingmo.style")) {
        ...
        connect(styleGSettings, &QGSettings::changed, this, [=](const QString &key) {
            if (key == "explorerSideBarTransparency" && transparentConfigFromLINGMOStyle) {
                m_cache.remove(SIDEBAR_BG_OPACITY);
        ...
        if (transparentConfigFromLINGMOStyle) {
            m_cache.remove(SIDEBAR_BG_OPACITY);
            m_cache.insert(SIDEBAR_BG_OPACITY, styleGSettings->get("explorerSideBarTransparency").toString());
        }
...
{
    if (key == REMOTE_SERVER_REMOTE_IP || key == DEFAULT_WINDOW_SIZE || !m_explorerGSettings->keys().contains(key)) {
        m_cache.remove(key);
...
void GlobalSettings::forceSync(const QString &key)
{
    ...
    if (key.isNull()) {
        ...
        if (m_settings->allKeys().contains(key)) {
            m_cache.insert(key, m_settings->value(key));
        } else if (m_explorer_gsettings->keys().contains(key)) {
            m_cache.insert(key, m_explorer_gsettings ? m_explorer_gsettings->get(key) : QVariant());
        } else {
...
void GlobalSettings::setGSettingValue(const QString &key, const QVariant &value)
{
    ...
    const QStringList list = m_explorer_gsettings->keys();
    if (!list.contains(key)) {
        qWarning()<<"set gsettings failed"<<key<<value<<"not existed!";
        return;
    }

    m_explorer_gsettings->set(key, value);
    m_cache.remove(key);
    m_cache.insert(key, m_explorer_gsettings->get(key));
}
