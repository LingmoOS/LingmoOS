 ...
 * @details single application with c/s conmunication mode is a common desgin used for many file manager, like explorer, caja, and nautilus.
 * <pre>
...
class PeonyApplication : public SingleApplication
{
    Q_OBJECT
public:
    explicit PeonyApplication(int &argc, char *argv[], const char *applicationName = "explorer-qt");
    static void about();
    static void help();
    ...
    static qint64 explorer_start_time;
...
private:
    ...
    QCommandLineOption quitOption = QCommandLineOption(QStringList()<<"q"<<"quit", tr("Close all explorer-qt windows and quit"));
