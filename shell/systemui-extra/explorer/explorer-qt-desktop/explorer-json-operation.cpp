 ...
 *
 */
#include "explorer-json-operation.h"
...
int PeonyJsonOperation::getBWListInfo(QJsonDocument &jsonDoc, BWListInfo *bwListInfo)
{
    ...
    if (!jsonObj.contains("lingmo-explorer")) {
        qWarning("the json file is not lingmo-explorer");
        return -EINVAL;
    }

    QJsonObject explorerObj = jsonObj.value("lingmo-explorer").toObject();
    if (!explorerObj.contains("mode")) {
        qWarning("the json file does not contain model");
        return -EINVAL;
    }

    QString model = explorerObj.value("mode").toString();
    qDebug()<<"bw list mode:"<< model;
    if (BW_LIST_BLACK == model) {
        bwListInfo->setBWListModel(model);
        if (explorerObj.contains(BW_LIST_BLACK)) {
            QJsonValue bPkgValue = explorerObj.value(BW_LIST_BLACK);
            return bwPkgInfoParse(bPkgValue, bwListInfo);
        }
    } else if (BW_LIST_WHITE == model){
        bwListInfo->setBWListModel(model);
        if (explorerObj.contains(BW_LIST_WHITE)) {
            QJsonValue wPkgValue = explorerObj.value(BW_LIST_WHITE);
