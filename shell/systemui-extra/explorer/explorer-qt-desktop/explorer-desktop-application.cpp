...
qint64 PeonyDesktopApplication::explorer_desktop_start_time = 0;
...
void trySetDefaultFolderUrlHandler() {
    ...
    //not quit when we call "explorer-qt-desktop -q" in command line.
    //
    ...
    QTimer::singleShot(1000, []() {
        QtConcurrent::run([=]() {
            ...
            if (!hasPeonyQtAppInfo) {
                GAppInfo *explorer_qt = g_app_info_create_from_commandline("explorer",
                                     nullptr,
                                     G_APP_INFO_CREATE_SUPPORTS_URIS,
                                     nullptr);
                g_app_info_set_as_default_for_type(explorer_qt, "inode/directory", nullptr);
                g_object_unref(explorer_qt);
            }
            return;
        });
    });
...
PeonyDesktopApplication::PeonyDesktopApplication(int &argc, char *argv[], const QString &applicationName) : QtSingleApplication (applicationName, argc, argv)
{
    ...
    qDebug()<<"explorer desktop constructor start";
    setApplicationVersion(QString("v%1").arg(VERSION));
    //setApplicationDisplayName(tr("Peony-Qt Desktop"));

    QTranslator *t = new QTranslator(this);
    t->load("/usr/share/libexplorer-qt/libexplorer-qt_"+QLocale::system().name());
    QApplication::installTranslator(t);
    QTranslator *t2 = new QTranslator(this);
    t2->load("/usr/share/explorer-qt-desktop/explorer-qt-desktop_"+QLocale::system().name());
    ...
    setApplicationName(tr("explorer-qt-desktop"));
...
//    connect(this, &SingleApplication::screenRemoved, this, &PeonyDesktopApplication::screenRemovedProcess);
    ...
    connect(this, &PeonyDesktopApplication::emitFinish, this , [=](){
        ...
        QDBusMessage message = QDBusMessage::createMethodCall("org.gnome.SessionManager",
                                                              ...
                                                              "startupfinished");
        QList<QVariant> args;
        args.append("explorer-qt-desktop");
    ...
    qDebug()<<"explorer desktop application constructor end";
}
...
void PeonyDesktopApplication::parseCmd(QString msg, bool isPrimary)
{
    ...
    QCommandLineOption quitOption(QStringList()<<"q"<<"quit", tr("Close the explorer desktop window"));
        ...
        if (parser.isSet(daemonOption)) {
            if (!has_daemon) {
                ...
                connect(service, &Peony::FMDBusService::showItemsRequest, [=](const QStringList &urisList) {
                    ...
                    p.startDetached();
#else
                    p.startDetached("/usr/bin/explorer", QStringList()<<"--show-items"<<urisList, nullptr);
#endif
                });
                connect(service, &Peony::FMDBusService::showFolderRequest, [=](const QStringList &urisList) {
                    ...
                    p.startDetached();
#else
                    p.startDetached("/usr/bin/explorer", QStringList()<<"--show-folders"<<urisList, nullptr);
#endif
                });
                connect(service, &Peony::FMDBusService::showItemPropertiesRequest, [=](const QStringList &urisList) {
                    ...
                    p.startDetached();
#else
                    p.startDetached("/usr/bin/explorer", QStringList()<<"--show-properties"<<urisList, nullptr);
#endif
                });
            }
            has_daemon = true;
...
void guessContentTypeCallback(GObject* object, GAsyncResult *res,gpointer data)
{
    ...
    openFolderCmd = "/usr/bin/explorer " + QString(mountUri);
