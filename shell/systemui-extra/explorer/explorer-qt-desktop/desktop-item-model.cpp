...
{
    // do not redo layout new items while we start an operation with explorer's api.
        ...
        if (!exsited) {
            ...
            if (!view->isRenaming()) {
                ...
                if (metaInfo) {
                    ...
                    restoreInfo<<"";
                    metaInfo->setMetaInfoStringList("metadata::explorer-qt-desktop-restore-singlescreen-item-position", restoreInfo);
                }
...
bool DesktopItemModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
    ...
    if (data->hasFormat("explorer-qt/encoded-uris")) {
        if (!data->data("explorer-qt/encoded-uris").isEmpty()) {
    ...
    if (hasPeonyEncodedUris) {
        srcUris = QString(data->data("explorer-qt/encoded-uris")).split(" ");
    ...
    if (data->hasFormat("explorer-qt/is-search")) {
        bMoveFromSearchTab = QVariant(data->data("explorer-qt/is-search")).toBool();
    }
