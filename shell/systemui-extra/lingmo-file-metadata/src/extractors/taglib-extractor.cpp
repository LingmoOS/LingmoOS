...
static const QString PLUGIN_NAME = "Taglib";
...
void extractId3Thumbnails(const TagLib::ID3v2::Tag *Id3Tag, ExtractionResult *result) {
    if (!(result->inputFlags() & ExtractionResult::ExtractThumbnail)) {
        return;
    }
    if (ThumbnailUtils::needGenerateThumbnail(result, PLUGIN_NAME, VERSION)) {
        EmbeddedImageData::ImageTypes thumbnailTypes = {EmbeddedImageData::FrontCover};
        auto map = extractId3Cover(Id3Tag, thumbnailTypes);
        for (const auto& value : map) {
            if (!value.isEmpty()) {
                QImage thumbnail;
                thumbnail.loadFromData(value);
...
void extractFlacThumbnails(const TagLib::List<TagLib::FLAC::Picture *> picList, ExtractionResult* result) {
    if (!(result->inputFlags() & ExtractionResult::ExtractThumbnail)) {
        return;
    }
    if (ThumbnailUtils::needGenerateThumbnail(result, PLUGIN_NAME, VERSION)) {
        EmbeddedImageData::ImageTypes thumbnailTypes = {EmbeddedImageData::FrontCover};
        auto map = extractFlacCover(picList, thumbnailTypes);
        for (const auto& value : map) {
            if (!value.isEmpty()) {
                QImage thumbnail;
                thumbnail.loadFromData(value);
...
void extractMp4Thumbnail(TagLib::MP4::Tag* mp4Tags, ExtractionResult* result) {
    if (!(result->inputFlags() & ExtractionResult::ExtractThumbnail)) {
        return;
    }
    if (ThumbnailUtils::needGenerateThumbnail(result, PLUGIN_NAME, VERSION)) {
        EmbeddedImageData::ImageTypes thumbnailTypes = {EmbeddedImageData::FrontCover};
        auto map = extractMp4Cover(mp4Tags, thumbnailTypes);
        for (const auto& value : map) {
            if (!value.isEmpty()) {
                QImage thumbnail;
                thumbnail.loadFromData(value);
...
void extractApeThumbnail(const TagLib::APE::Tag* apeTags, ExtractionResult* result) {
    if (!(result->inputFlags() & ExtractionResult::ExtractThumbnail)) {
        return;
    }
    if (ThumbnailUtils::needGenerateThumbnail(result, PLUGIN_NAME, VERSION)) {
        EmbeddedImageData::ImageTypes thumbnailTypes = {EmbeddedImageData::FrontCover};
        auto map = extractApeCover(apeTags, thumbnailTypes);
        for (const auto& value : map) {
            if (!value.isEmpty()) {
                QImage thumbnail;
                thumbnail.loadFromData(value);
...
void extractAsfThumbnail(TagLib::ASF::Tag* asfTags, ExtractionResult* result) {
    if (!(result->inputFlags() & ExtractionResult::ExtractThumbnail)) {
        return;
    }
    if (ThumbnailUtils::needGenerateThumbnail(result, PLUGIN_NAME, VERSION)) {
        EmbeddedImageData::ImageTypes thumbnailTypes = {EmbeddedImageData::FileIcon | EmbeddedImageData::FrontCover};
        auto map = extractAsfCover(asfTags, thumbnailTypes);
        for (const auto& value : map) {
            if (!value.isEmpty()) {
                QImage thumbnail;
                thumbnail.loadFromData(value);
...
TaglibExtractor::TaglibExtractor(QObject *parent) : ExtractorPlugin(parent) {
    ...
    if (mimeType == QLatin1String("audio/mpeg") || mimeType == QLatin1String("audio/mpeg3") || mimeType == QLatin1String("audio/x-mpeg")) {
        ...
        if (file.isValid()) {
            ...
            if (file.hasID3v2Tag()) {
                ...
                extractId3Thumbnails(file.ID3v2Tag(), result);
        ...
        if (file.isValid()) {
            ...
            if (file.hasID3v2Tag()) {
                ...
                extractId3Thumbnails(file.tag(), result);
        ...
        if (file.isValid()) {
            ...
            if (file.APETag()) {
                ...
                extractApeThumbnail(file.APETag(), result);
            }
        }
        ...
        if (file.isValid()) {
            ...
            extractMp4Thumbnail(file.tag(), result);
        }
        ...
        if (file.isValid()) {
            ...
            extractFlacThumbnails(file.pictureList(), result);
        }
        ...
        if (file.isValid()) {
            ...
            if (file.tag()) {
                ...
                extractFlacThumbnails(file.tag()->pictureList(), result);
            }
        }
        ...
        if (file.isValid()) {
            ...
            if (asfTags) {
                ...
                extractAsfThumbnail(asfTags, result);
            }
        }
    }
