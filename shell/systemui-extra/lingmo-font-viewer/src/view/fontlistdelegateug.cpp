...
QPoint FontListDelegateug::m_collectPoint = QPoint(0, 0);
QSize FontListDelegateug::m_collectSize = QSize(0, 0);
...
FontListDelegateug::FontListDelegateug(FontListView *fontList)
{
    m_fontList = fontList;
    m_fontSize = m_fontList->getFontSize();
}

FontListDelegateug::~FontListDelegateug() {}

QSize FontListDelegateug::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(option);
...
void FontListDelegateug::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (index.isValid()) {
        painter->save();
...
//        const qreal radius = 7;
        ...
        QPainterPath path;
        path.moveTo(itemRect.topLeft() - QPointF(radius, 0));
        path.lineTo(itemRect.topRight() + QPointF(radius, 0));
        path.quadTo(itemRect.topRight(), itemRect.topRight() + QPointF(0, radius));
        path.lineTo(itemRect.bottomRight() + QPointF(0, -radius));
        path.quadTo(itemRect.bottomRight(), itemRect.bottomRight() + QPointF(radius, 0));
        path.lineTo(itemRect.bottomLeft() - QPointF(radius, 0));
        path.quadTo(itemRect.bottomLeft(), itemRect.bottomLeft() + QPointF(0, -radius));
        path.lineTo(itemRect.topLeft() + QPointF(0, radius));
        path.quadTo(itemRect.topLeft(), itemRect.topLeft() + QPointF(-radius, 0));
        ...
        // 字体信息
        painter->save();
        QPoint infomationPoint(itemRect.right() - 400, itemRect.top() + 8);
        ...
        optionText.setAlignment(Qt::AlignRight | Qt::AlignBottom); // 文字消息内容绘制
        ...
        // 展示字体
        painter->save();
        optionText.setAlignment(Qt::AlignRight | Qt::AlignTop);
        ...
        // 收藏图标
        QPoint collectPoint = QPoint(0, itemRect.top() + 15);
        ...
        if (option.state.testFlag(QStyle::State_HasFocus) || fontInstall) {
            FontListDelegate::m_collectPoint = collecteRectF.topRight().toPoint();
            ...
            optionText.setAlignment(Qt::AlignRight | Qt::AlignBottom);
            painter->drawText(infomationRect, fontInformation, optionText);
...
QSize FontListDelegateug::getTextRectSize(QString text, QFont font, int textWidth, int familyFontH)
{
    QSize resSize = QSize();
