...
class XDiscardEglPixmapRunnable : public QRunnable
{
public:
    XDiscardEglPixmapRunnable(uint, QFunctionPointer, EGLImageKHR);
    void run() override;
...
XDiscardEglPixmapRunnable::XDiscardEglPixmapRunnable(uint texture, QFunctionPointer deleteFunction, EGLImageKHR image)
    : QRunnable()
    , m_texture(texture)
...
void XDiscardEglPixmapRunnable::run()
{
...
bool XWindowThumbnail::nativeEventFilter(const QByteArray &eventType, void *message, long *result)
{
    Q_UNUSED(result)
...
bool XWindowThumbnail::fixHeight() const
{
    return m_fixHeight;
}

void XWindowThumbnail::setFixHeight(bool fixHeight)
{
...
bool XWindowThumbnail::fixWidth() const
{
    return m_fixWidth;
}

void XWindowThumbnail::setFixWidth(bool fixWidth)
{
...
QSGNode *XWindowThumbnail::updatePaintNode(QSGNode *oldNode, QQuickItem::UpdatePaintNodeData *updatePaintNodeData)
{
    ...
    QSizeF scaleSize = node->texture()->textureSize();
    if(m_fixHeight) {
    ...
    const QSizeF size(node->texture()->textureSize().scaled(scaleSize.toSize(), Qt::KeepAspectRatio));
...
void XWindowThumbnail::iconToTexture(WindowTextureProvider *textureProvider)
{
    QIcon icon;
    if (KWindowSystem::hasWId(m_winId)) {
        icon = KWindowSystem::icon(m_winId, boundingRect().width(), boundingRect().height());
    } else {
...
void XWindowThumbnail::windowToTexture(WindowTextureProvider *textureProvider)
{
...
bool XWindowThumbnail::windowToTextureGLX(WindowTextureProvider *textureProvider)
{
...
bool XWindowThumbnail::xcbWindowToTextureEGL(WindowTextureProvider *textureProvider)
{
...
void XWindowThumbnail::itemChange(QQuickItem::ItemChange change, const QQuickItem::ItemChangeData &data)
{
    switch (change) {
...
void XWindowThumbnail::releaseResources()
{
    ...
    // but the pointer is held by the XWindowThumbnail which is in the main thread
    if (m_glxPixmap != XCB_PIXMAP_NONE) {
    ...
    if (m_image != EGL_NO_IMAGE_KHR) {
        window()->scheduleRenderJob(new XDiscardEglPixmapRunnable(m_texture, m_eglDestroyImageKHR, m_image), m_renderStage);
        m_image = EGL_NO_IMAGE_KHR;
        m_texture = 0;
    }
}
