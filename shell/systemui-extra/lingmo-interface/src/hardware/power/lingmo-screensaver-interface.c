...
#include "lingmo-screensaver-interface.h"
...
void lingmo_hardware_power_idleactivationenabled(const bool in_arg)
{
    ...
    if (pError != NULL){
        print_error("Failed to call method lingmo_hardware_power_idleactivationenabled. Reason: %s.\n", pError->message);
        g_error_free(pError);
        }
...
void lingmo_hardware_power_lockenabled(const bool in_arg)
{
    ...
    if (pError != NULL){
        print_error("Failed to call method lingmo_hardware_power_lockenabled. Reason: %s.\n", pError->message);
        g_error_free(pError);
        }
