...
void addtoconninode(char *buffer) {
  ...
  if (matches != 5) {
    qCritical()<<QString("Unexpected buffer: '%1'\n").arg(buffer);
    return;
  }
  ...
  bzero(hashkey, HASHKEYSIZE * sizeof(char));
  bzero(local_string, 50);
  bzero(remote_string, 50);
...
// conninode = new HashTable (256);
  ...
  if (!addprocinfo("/proc/net/tcp")) {
    qDebug() << "Error: couldn't open /proc/net/tcp\n";
    return;
  }
  addprocinfo("/proc/net/tcp6");
  addprocinfo("/proc/net/udp");
