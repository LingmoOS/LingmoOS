...
static int nethogsmonitor_init() {
  ...
  if (devices == NULL) {
    qCritical() << "No devices to monitor";
    ...
    if (newhandle != NULL) {
      ...
      if (pc_loop_use_select) {
        ...
        if (fd != -1) {
          ...
          qCritical()<<QString("failed to get selectable_fd for %1\n").arg(current_dev->name);
        }
      }
    } else {
      qCritical()<<QString("ERROR: opening handler for device %1: %2\n").arg(current_dev->name).arg(strerror(errno));
      ++nb_failed_devices;
    }
  ...
  if (pc_loop_use_select) {
    ...
    if (self_pipe.first == -1 || self_pipe.second == -1) {
      qCritical() << "Error creating pipe file descriptors\n";
      pc_loop_use_select = false;
    } else {
...
static void nethogsmonitor_handle_update(NethogsMonitorCallback cb, void *pVoid) {
  refreshconninode();
  refreshcount++;
  ...
  while (curproc != NULL) {
    ...
    if ((curproc->getVal()->getLastPacket() + PROCESSTIMEOUT <=
      ...
      if (it != monitor_record_map.end()) {
        ...
        (*cb)(NETHOGS_APP_ACTION_REMOVE, &data, pVoid);
...
#undef NHM_UPDATE_ONE_FIELD
      ...
      if (data_change) {
        (*cb)(NETHOGS_APP_ACTION_SET, &data, pVoid);
      }
...
int nethogsmonitor_loop(NethogsMonitorCallback cb, void *pVoid) {
  if (monitor_run_flag) {
    return NETHOGS_STATUS_FAILURE;
  }
  ...
  bzero(userdata, sizeof(struct dpargs));
  ...
  while (monitor_run_flag) {
    ...
    while (current_handle != NULL) {
      ...
      if (retval < 0) {
        qDebug() << "Error dispatching: " << retval;
      } else if (retval != 0) {
        packets_read = true;
      } else {
    ...
    if (monitor_last_refresh_time + monitor_refresh_delay <= now) {
      ...
      nethogsmonitor_handle_update(cb, pVoid);
    }
    refresh_count++;
...
void nethogsmonitor_breakloop() {
  ...
  if (write(self_pipe.second, "x", 1) == -1) {
