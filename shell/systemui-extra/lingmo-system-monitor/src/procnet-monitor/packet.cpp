...
bool getLocal(const char *device, bool tracemode) {
  ...
  for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
    ...
    if (family == AF_INET) {
      ...
      if (tracemode || DEBUG) {
        qDebug()<<QString("Adding local address: %1\n").arg(inet_ntoa(addr->sin_addr));
      }
      ...
      if (tracemode || DEBUG) {
        char host[512];
        qDebug()<<QString("Adding local address: %1\n").arg(inet_ntop(AF_INET6, &addr->sin6_addr, host, sizeof(host)));
      }
    }
  }
  return true;
...
Packet::Packet(const Packet &old_packet) {
  ...
  if (old_packet.hashstring == nullptr)
    hashstring = nullptr;
  else
    hashstring = old_packet.hashstring;
  dir = old_packet.dir;
}
...
bool Packet::isOlderThan(timeval t) {
  //qDebug() << "Comparing " << time.tv_sec << " <= " << t.tv_sec;
...
HashKey Packet::gethashstring() {
  if (hashstring != nullptr) {
    return hashstring;
  }

  // TODO free this value in the Packet destructor
  hashstring = HashKey(new char[HASHKEYSIZE]);
  ...
  if (Outgoing()) {
    snprintf(hashstring.get(), HASHKEYSIZE * sizeof(char), "%s:%d-%s:%d",
  ...
  // if (DEBUG)
  //	qDebug() << "Returning newly created hash string: " << hashstring;
  return hashstring;
}
