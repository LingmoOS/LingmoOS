...
void Process::gettotal(u_int32_t *recvd, u_int32_t *sent) {
  ...
  // qDebug() << "Sum sent: " << sum_sent;
  // qDebug() << "Sum recv: " << sum_recv;
  *recvd = sum_recv;
  *sent = sum_sent;
}
...
Process *getProcess(unsigned long inode, const char *devicename) {
  ...
  if (node == NULL) {
    if (DEBUG || bughuntmode)
      qDebug() << "No PID information for inode " << inode ;
    return NULL;
  }
  ...
  if (!ROBUST && (retval != 0))
  {
          std::cerr << "Couldn't stat " << procdir ;
          assert (false);
  }
  */
...
Process *getProcess(Connection *connection, const char *devicename) {
  unsigned long inode = conninode[connection->refpacket->gethashstring().data()];

  if (inode == 0) {
    // no? refresh and check conn/inode table
    if (bughuntmode) {
      qDebug() << "?  new connection not in connection-to-inode table before "
                   "refresh, hash " << connection->refpacket->gethashstring().data()
                ;
    }
...
// haven't seen the connection->inode yet though).
...
#endif
    refreshconninode();
    inode = conninode[connection->refpacket->gethashstring().data()];
    if (bughuntmode) {
      if (inode == 0) {
        qDebug() << ":( inode for connection not found after refresh.\n";
      } else {
        qDebug() << ":) inode for connection found after refresh.\n";
      }
    }
#if REVERSEHACK
    if (inode == 0) {
      ...
      inode = conninode[reversepacket->gethashstring().data()];

      if (inode == 0) {
        delete reversepacket;
        if (bughuntmode || DEBUG)
          qDebug() << "LOC: " << connection->refpacket->gethashstring().data()
  ...
  if (proc == NULL) {
    proc = new Process(inode, "", connection->refpacket->gethashstring().data());
