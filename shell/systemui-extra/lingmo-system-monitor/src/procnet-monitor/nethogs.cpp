...
const char version[] = " version 0.8.5";
...
int process_tcp(u_char *userdata, const dp_header *header,
  ...
  case AF_INET6:
    ...
    break;
  default:
    //qDebug() << "Invalid address family for TCP packet: " << args->sa_family;
    return true;
  }
...
int process_udp(u_char *userdata, const dp_header *header,
  ...
  case AF_INET6:
    ...
    break;
  default:
    //qDebug() << "Invalid address family for UDP packet: " << args->sa_family;
    return true;
  }

  // if (DEBUG)
  //	qDebug() << "Got packet from " << packet->gethashstring().data();
