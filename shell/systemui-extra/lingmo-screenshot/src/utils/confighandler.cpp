...
bool ConfigHandler::verifyLaunchFile() {
    ...
    QString path = QDir::homePath() + "/.config/autostart/lingmo-screenshot.desktop";
    res = QFile(path).exists();
#elif defined(Q_OS_WIN)
    QSettings bootUpSettings(
                "HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run",
                QSettings::NativeFormat);
    res = bootUpSettings.value("lingmoscreenshot").toString() ==
...
void ConfigHandler::setStartupLaunch(const bool start) {
    ...
    QFile file(path + "lingmo-screenshot.desktop");
    if (start) {
        if (file.open(QIODevice::WriteOnly)) {
            QByteArray data("[Desktop Entry]\nName=lingmoscreenshot\nIcon=lingmo-screenshot"
                            "\nExec=lingmo-screenshot\nTerminal=false\nType=Application"
    ...
    if (start) {
        QString app_path =
                QDir::toNativeSeparators(QCoreApplication::applicationFilePath());
        bootUpSettings.setValue("lingmoscreenshot", app_path);
    } else {
        bootUpSettings.remove("lingmoscreenshot");
    }
#endif
