...
void FullScreenOverlay::initWayland()
{
    ...
    connect(registry, &Registry::lingmoShellAnnounced, this, [this, registry](quint32 name, quint32 version) {
        m_lingmoShellInterface = registry->createLingmoShell(name, version, this);

        m_lingmoShellSurface = m_lingmoShellInterface->createSurface(m_surface, this);
        m_lingmoShellSurface->setSkipTaskbar(true);
    });
...
bool FullScreenOverlay::event(QEvent *e)
{
    if (e->type() == QEvent::FocusIn || e->type() == QEvent::FocusOut) {
        ...
        if (KWindowSystem::isPlatformX11()) {
            ...
            if (m_lingmoShellSurface) {
                m_lingmoShellSurface->setSkipTaskbar(true);
                m_lingmoShellSurface->setSkipSwitcher(true);
            }
        }
    }
