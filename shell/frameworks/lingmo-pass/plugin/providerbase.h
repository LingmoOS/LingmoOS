...
private Q_SLOTS:
    void start();
    void onLingmoServiceRemovePasswordResult(KJob *job);
...
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
    std::unique_ptr<Lingmo::DataEngineConsumer> mEngineConsumer;
#else
    std::unique_ptr<Lingmo5Support::DataEngineConsumer> mEngineConsumer;
#endif
    QString mPath;
    QString mError;
    QString mSecret;
