...
#include <Lingmo/DataEngine>
#include <Lingmo/DataEngineConsumer>
#include <Lingmo/PluginLoader>
#include <Lingmo/Service>
#include <Lingmo/ServiceJob>
#else
...
const QString klipperDataEngine = QStringLiteral("org.kde.lingmo.clipboard");
...
void ProviderBase::start()
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        qCWarning(LINGMOPASS_LOG, "Failed to open password file: %s", qUtf8Printable(file.errorString()));
    ...
    connect(decryptJob, &QGpgME::DecryptJob::result, this, [this](const GpgME::DecryptionResult &result, const QByteArray &plainText) {
        if (result.error()) {
            qCWarning(LINGMOPASS_LOG, "Failed to decrypt password: %s", result.error().asString());
        ...
        if (data.isEmpty()) {
            qCWarning(LINGMOPASS_LOG, "Password file is empty!");
    ...
    if (error) {
        qCWarning(LINGMOPASS_LOG, "Failed to decrypt password: %s", error.asString());
...
void ProviderBase::removePasswordFromClipboard(const QString &password)
{
    ...
    case KlipperUtils::State::Missing:
        qCDebug(LINGMOPASS_LOG, "Klipper not detected in the system, will not attempt to clear the clipboard history");
        return;
    case KlipperUtils::State::SupportsPasswordManagerHint:
        ...
        // history.
        qCDebug(LINGMOPASS_LOG, "Klipper with support for x-kde-passwordManagerHint detected, will not attempt to clear the clipboard history");
        return;
    case KlipperUtils::State::Available:
        ...
        qCDebug(LINGMOPASS_LOG, "Old Klipper without x-kde-passwordManagerHint support detected, will attempt to remove the password from clipboard history");
        break;
    }

    if (!mEngineConsumer) {
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
        mEngineConsumer = std::make_unique<Lingmo::DataEngineConsumer>();
#else
        mEngineConsumer = std::make_unique<Lingmo5Support::DataEngineConsumer>();
#endif
    }
    ...
    if (service == nullptr) {
        qCWarning(LINGMOPASS_LOG, "Failed to obtain LingmoService for the password, falling back to clearClipboard()");
    ...
    connect(job, &KJob::result, this, &ProviderBase::onLingmoServiceRemovePasswordResult);
}

void ProviderBase::onLingmoServiceRemovePasswordResult(KJob *job)
{
    // Disconnect from the job: Klipper's ClipboardJob is buggy and emits result() twice
    disconnect(job, &KJob::result, this, &ProviderBase::onLingmoServiceRemovePasswordResult);
    QTimer::singleShot(0, this, [this]() {
        mEngineConsumer.reset();
    });
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
    auto serviceJob = qobject_cast<Lingmo::ServiceJob *>(job);
#else
    auto serviceJob = qobject_cast<Lingmo5Support::ServiceJob *>(job);
#endif
    if (serviceJob->error() != 0) {
        qCWarning(LINGMOPASS_LOG, "ServiceJob for clipboard failed: %s", qUtf8Printable(serviceJob->errorString()));
        clearClipboard();
        return;
    }
    // If something went wrong fallback to clearing the entire clipboard
    if (!serviceJob->result().toBool()) {
        qCWarning(LINGMOPASS_LOG, "ServiceJob for clipboard failed internally, falling back to clearClipboard()");
        clearClipboard();
        return;
    }

    qCDebug(LINGMOPASS_LOG, "Successfully removed password from Klipper");
}
