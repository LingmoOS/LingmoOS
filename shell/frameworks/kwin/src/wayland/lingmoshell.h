...
class LingmoShellInterfacePrivate;
class LingmoShellSurfaceInterfacePrivate;

/**
 * @brief Global for the org_kde_lingmo_shell interface.
 *
 * The LingmoShellInterface allows to add additional information to a SurfaceInterface.
 * It goes beyond what a ShellSurfaceInterface provides and is adjusted toward the needs
 * of the Lingmo desktop.
 *
 ...
 */
class KWIN_EXPORT LingmoShellInterface : public QObject
{
    Q_OBJECT

public:
    explicit LingmoShellInterface(Display *display, QObject *parent);
    virtual ~LingmoShellInterface();

Q_SIGNALS:
    /**
     * Emitted whenever a LingmoShellSurfaceInterface got created.
     */
    void surfaceCreated(KWin::LingmoShellSurfaceInterface *);

private:
    std::unique_ptr<LingmoShellInterfacePrivate> d;
};

/**
 * @brief Resource for the org_kde_lingmo_shell_surface interface.
 *
 * LingmoShellSurfaceInterface gets created by LingmoShellInterface.
 */
class KWIN_EXPORT LingmoShellSurfaceInterface : public QObject
{
    Q_OBJECT
public:
    virtual ~LingmoShellSurfaceInterface();

    /**
     * @returns the SurfaceInterface this LingmoShellSurfaceInterface got created for
     */
    ...
    /**
     * Describes possible roles this LingmoShellSurfaceInterface can have.
    ...
    Role role() const;
    /**
     * Describes how a LingmoShellSurfaceInterface with role @c Role::Panel should behave.
     *
     * Deprecated
     */
    ...
    /**
     * @returns The PanelBehavior for a LingmoShellSurfaceInterface with role @c Role::Panel
     * @see role
     *
    ...
    /**
     * Informs the LingmoShellSurfaceInterface that the auto-hiding panel got hidden.
    ...
    /**
     * Informs the LingmoShellSurfaceInterface that the auto-hiding panel got shown again.
     *
    ...
    /**
     * Whether a LingmoShellSurfaceInterface wants to have focus.
     *
     * By default some LingmoShell roles do not get focus, but the LingmoShellSurfaceInterface can
    ...
    /**
     * @returns The LingmoShellSurfaceInterface for the @p native resource.
     */
    static LingmoShellSurfaceInterface *get(wl_resource *native);
    static LingmoShellSurfaceInterface *get(SurfaceInterface *surface);
...
Q_SIGNALS:
     ...
     *
     * The compositor should inform the LingmoShellSurfaceInterface about the actual change.
...
private:
    friend class LingmoShellInterfacePrivate;
    explicit LingmoShellSurfaceInterface(SurfaceInterface *surface, wl_resource *resource);
    std::unique_ptr<LingmoShellSurfaceInterfacePrivate> d;
};

}

Q_DECLARE_METATYPE(KWin::LingmoShellSurfaceInterface::Role)
Q_DECLARE_METATYPE(KWin::LingmoShellSurfaceInterface::PanelBehavior)
