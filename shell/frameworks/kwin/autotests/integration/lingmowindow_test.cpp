...
static const QString s_socketName = QStringLiteral("wayland_test_kwin_lingmo-window-0");

class LingmoWindowTest : public QObject
{
    Q_OBJECT
private Q_SLOTS:
    void initTestCase();
    void init();
    void cleanup();
    void testCreateDestroyX11LingmoWindow();
    void testInternalWindowNoLingmoWindow();
    void testPopupWindowNoLingmoWindow();
    void testLockScreenNoLingmoWindow();
...
void LingmoWindowTest::initTestCase()
{
...
void LingmoWindowTest::cleanup()
{
    Test::destroyWaylandConnection();
}

void LingmoWindowTest::testCreateDestroyX11LingmoWindow()
{
    // this test verifies that a LingmoWindow gets unmapped on Client side when an X11 window is destroyed
    QSignalSpy lingmoWindowCreatedSpy(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated);
    ...
    QSignalSpy geometryChangedSpy(pw, &KWayland::Client::LingmoWindow::geometryChanged);

    QSignalSpy unmappedSpy(m_windowManagement->windows().first(), &KWayland::Client::LingmoWindow::unmapped);
...
void LingmoWindowTest::testInternalWindowNoLingmoWindow()
{
    // this test verifies that an internal window is not added as a LingmoWindow
    QSignalSpy lingmoWindowCreatedSpy(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated);
    HelperWindow win;
    win.setGeometry(0, 0, 100, 100);
    win.show();

    QVERIFY(!lingmoWindowCreatedSpy.wait(100));
}

void LingmoWindowTest::testPopupWindowNoLingmoWindow()
{
    // this test verifies that a popup window is not added as a LingmoWindow
    QSignalSpy lingmoWindowCreatedSpy(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated);
...
void LingmoWindowTest::testLockScreenNoLingmoWindow()
{
#if KWIN_BUILD_SCREENLOCKER
    // this test verifies that lock screen windows are not exposed to LingmoWindow
    QSignalSpy lingmoWindowCreatedSpy(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated);
    ...
    QVERIFY(lingmoWindowCreatedSpy.isEmpty());
    QVERIFY(!lingmoWindowCreatedSpy.wait(100));
...
void LingmoWindowTest::testDestroyedButNotUnmapped()
{
    // this test verifies that also when a ShellSurface gets destroyed without a prior unmap
    // the LingmoWindow gets destroyed on Client side
    QSignalSpy lingmoWindowCreatedSpy(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated);
    ...
    // this should create a lingmo window
    QVERIFY(lingmoWindowCreatedSpy.wait());
    QCOMPARE(lingmoWindowCreatedSpy.count(), 1);
    auto window = lingmoWindowCreatedSpy.first().first().value<KWayland::Client::LingmoWindow *>();
    QVERIFY(window);
...
WAYLANDTEST_MAIN(KWin::LingmoWindowTest)
#include "lingmowindow_test.moc"
