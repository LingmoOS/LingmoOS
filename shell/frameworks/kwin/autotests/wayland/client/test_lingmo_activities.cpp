...
{
    QSignalSpy enterRequestedSpy(m_windowInterface, &KWin::LingmoWindowInterface::enterLingmoActivityRequested);
    ...
    QSignalSpy activityEnteredSpy(m_window, &KWayland::Client::LingmoWindow::lingmoActivityEntered);

    // agree to the request
    m_windowInterface->addLingmoActivity(QStringLiteral("0-1"));
    QCOMPARE(m_windowInterface->lingmoActivities().length(), 1);
    QCOMPARE(m_windowInterface->lingmoActivities().first(), QStringLiteral("0-1"));
    ...
    QCOMPARE(m_window->lingmoActivities().length(), 1);
    QCOMPARE(m_window->lingmoActivities().first(), QStringLiteral("0-1"));

    // add another activity, server side
    m_windowInterface->addLingmoActivity(QStringLiteral("0-3"));
    activityEnteredSpy.wait();
    QCOMPARE(activityEnteredSpy.takeFirst().at(0).toString(), QStringLiteral("0-3"));
    QCOMPARE(m_windowInterface->lingmoActivities().length(), 2);
    QCOMPARE(m_window->lingmoActivities().length(), 2);
    QCOMPARE(m_window->lingmoActivities()[1], QStringLiteral("0-3"));

    // remove an activity
    QSignalSpy leaveRequestedSpy(m_windowInterface, &KWin::LingmoWindowInterface::leaveLingmoActivityRequested);
    ...
    QSignalSpy activityLeftSpy(m_window, &KWayland::Client::LingmoWindow::lingmoActivityLeft);

    // agree to the request
    m_windowInterface->removeLingmoActivity(QStringLiteral("0-1"));
    QCOMPARE(m_windowInterface->lingmoActivities().length(), 1);
    QCOMPARE(m_windowInterface->lingmoActivities().first(), QStringLiteral("0-3"));
    ...
    QCOMPARE(m_window->lingmoActivities().length(), 1);
    QCOMPARE(m_window->lingmoActivities().first(), QStringLiteral("0-3"));
}

QTEST_GUILESS_MAIN(TestActivities)
#include "test_lingmo_activities.moc"
