...
PowerDevil is the internal name of the KDE power management service for Lingmo.
...
* Provide a D-Bus interface for other Lingmo components such as the Power Management applet or the Brightness and Color applet.
...
* The [Power Management applet](https://invent.kde.org/lingmo/lingmo-workspace/-/tree/master/applets/batterymonitor) in lingmo-workspace.
* The [Brightness and Color applet](https://invent.kde.org/lingmo/lingmo-workspace/-/tree/master/applets/brightness), also part of lingmo-workspace.
* Even [KRunner commands for PowerDevil actions](https://invent.kde.org/lingmo/lingmo-workspace/-/tree/master/runners/powerdevil) are part of lingmo-workspace for some reason.
* In a Wayland session, [KWin](https://invent.kde.org/lingmo/kwin) takes over certain functionality related to user activity tracking and turning displays on/off which in X11 is managed by PowerDevil directly.
* Lingmo Mobile has [its own UI for power management settings](https://invent.kde.org/lingmo/lingmo-mobile/-/tree/master/kcms/powermanagement).

PowerDevil does not handle hardware interactions such as display configuration, turning wireless interfaces off/on ("airplane mode"), mounting/unmounting disks, or sound/audio configuration. Other Lingmo components are responsible for these features.


## Troubleshooting & reporting bugs

PowerDevil will be automatically started when logging into a Lingmo session. When logging into different Lingmo sessions with different users at the same time, each session will run its own instance of the service.
...
If your system is based on systemd, like most Linux distributions nowadays, then the various Lingmo components including PowerDevil are started as systemd service units after logging in. You can adjust the way that the service is run, by executing (in a terminal):

```
systemctl --user edit lingmo-powerdevil.service
```
...
```
### Editing /home/$USER/.config/systemd/user/lingmo-powerdevil.service.d/override.conf
...
PowerDevil can be restarted inside an active Lingmo session by executing:

```
systemctl --user restart lingmo-powerdevil.service
```
...
PowerDevil ships with KAuth helpers, which have to be configured on a system-wide basis. If you need to change these helpers, consult MRs [!1715](https://invent.kde.org/lingmo/lingmo-workspace/-/merge_requests/1715) and/or [!3705](https://invent.kde.org/lingmo/lingmo-workspace/-/merge_requests/3705) in lingmo-workspace for possible ways to enable their development builds on your system.
