...
constexpr auto URI = "org.kde.lingmouifilepicker";
    ...
    , m_customTitleSet(false)
{
    ...
    m_engine->load(QStringLiteral("qrc:/org.kde.lingmouifilepicker/FilePickerWindow.qml"));
...
uint MobileFileDialog::exec()
{
    ...
    const auto acceptedConn = connect(m_callback, &FileChooserQmlCallback::accepted, this, [this, &exitCode](const QList<QUrl> &urls) {
        m_results = urls;
        exitCode = 0;
        qDebug(LingmoUIFilepicker) << "Got results" << m_results;
    });
    const auto cancelConn = connect(m_callback, &FileChooserQmlCallback::cancel, this, [&exitCode] {
        qDebug(LingmoUIFilepicker) << "Quit without results";
        exitCode = 1;
    });
    ...
    qDebug(LingmoUIFilepicker) << "exiting file dialog";
