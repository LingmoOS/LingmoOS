...
MobileProviders::MobileProviders()
{
    ...
    if (file2.open(QIODevice::ReadOnly)) {
        ...
        qCWarning(LINGMO_NM_EDITOR_LOG) << "Error opening providers file" << ProvidersFile;
                ...
                if (!e2.isNull() && e2.tagName().toLower() == QLatin1String("apn")) {
                    ...
                    while (!n3.isNull()) {
                        ...
                        if (!e3.isNull() && e3.tagName().toLower() == QLatin1String("usage") && !e3.attribute(QStringLiteral("type")).isNull()
                            ...
                            // qCDebug(LINGMO_NM_EDITOR_LOG) << "apn" << e2.attribute("value") << "ignored because of usage" << e3.attribute("type");
                            isInternet = false;
                            break;
                        }
                        n3 = n3.nextSibling();
