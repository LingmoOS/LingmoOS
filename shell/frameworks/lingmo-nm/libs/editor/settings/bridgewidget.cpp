...
{
    qCDebug(LINGMO_NM_EDITOR_LOG) << "Adding bridged connection:" << action->data();
    ...
    connect(bridgeEditor.data(), &ConnectionEditorDialog::accepted, [bridgeEditor, this]() {
        ...
        // qCDebug(LINGMO_NM_EDITOR_LOG) << bridgeEditor->setting();
...
void BridgeWidget::bridgeAddComplete(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isValid()) {
        ...
        qCWarning(LINGMO_NM_EDITOR_LOG) << "Bridged connection not added:" << reply.error().message();
    }
}
...
void BridgeWidget::editBridge()
{
    ...
    if (connection) {
        qCDebug(LINGMO_NM_EDITOR_LOG) << "Editing bridged connection" << currentItem->text() << uuid;
...
void BridgeWidget::deleteBridge()
{
    ...
    if (connection) {
        // qCDebug(LINGMO_NM_EDITOR_LOG) << "About to delete bridged connection" << currentItem->text() << uuid;
