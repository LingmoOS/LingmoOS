...
{
    qCDebug(LINGMO_NM_EDITOR_LOG) << "Adding bonded connection:" << action->data();
    ...
    connect(bondEditor.data(), &ConnectionEditorDialog::accepted, [bondEditor, this]() {
        ...
        // qCDebug(LINGMO_NM_EDITOR_LOG) << bondEditor->setting();
...
void BondWidget::bondAddComplete(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isValid()) {
        ...
        qCWarning(LINGMO_NM_EDITOR_LOG) << "Bonded connection not added:" << reply.error().message();
    }
}
...
void BondWidget::editBond()
{
    ...
    if (connection) {
        // qCDebug(LINGMO_NM_EDITOR_LOG) << "Editing bonded connection" << currentItem->text() << uuid;
...
void BondWidget::deleteBond()
{
    ...
    if (connection) {
        // qCDebug(LINGMO_NM_EDITOR_LOG) << "About to delete bonded connection" << currentItem->text() << uuid;
