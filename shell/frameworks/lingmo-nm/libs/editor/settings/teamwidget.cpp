...
{
    qCDebug(LINGMO_NM_EDITOR_LOG) << "Adding teamed connection:" << action->data();
    qCDebug(LINGMO_NM_EDITOR_LOG) << "Master UUID:" << m_uuid;
    qCDebug(LINGMO_NM_EDITOR_LOG) << "Slave type:" << type();
    ...
    connect(teamEditor.data(), &ConnectionEditorDialog::accepted, [teamEditor, this]() {
        qCDebug(LINGMO_NM_EDITOR_LOG) << "Saving slave connection";
        // qCDebug(LINGMO_NM_EDITOR_LOG) << teamEditor->setting();
...
void TeamWidget::teamAddComplete(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isValid()) {
        ...
        qCWarning(LINGMO_NM_EDITOR_LOG) << "Teamed connection not added:" << reply.error().message();
    }
}
...
void TeamWidget::editTeam()
{
    ...
    if (connection) {
        qCDebug(LINGMO_NM_EDITOR_LOG) << "Editing teamed connection" << currentItem->text() << uuid;
...
void TeamWidget::deleteTeam()
{
    ...
    if (connection) {
        qCDebug(LINGMO_NM_EDITOR_LOG) << "About to delete teamed connection" << currentItem->text() << uuid;
...
void TeamWidget::importConfig()
{
    ...
    if (!filename.isEmpty()) {
        // qCDebug(LINGMO_NM_EDITOR_LOG) << "Importing" << filename;
        QFile file(filename);
