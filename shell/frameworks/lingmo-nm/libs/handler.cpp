...
#define AGENT_IFACE "org.kde.lingmonetworkmanagement"
...
QCoro::Task<void> Handler::activateConnectionInternal(const QString &connection, const QString &device, const QString &specificObject)
{
    ...
    if (!con) {
        qCWarning(LINGMO_NM_LIBS_LOG) << "Not possible to activate this connection";
        co_return;
    }
    ...
    if (con->settings()->connectionType() == NetworkManager::ConnectionSettings::Vpn) {
        ...
        if (vpnSetting) {
            qCDebug(LINGMO_NM_LIBS_LOG) << "Checking VPN" << con->name() << "type:" << vpnSetting->serviceType();

            // Check missing lingmo-nm VPN plugin
            ...
            const QList<KPluginMetaData> lingmoNmPlugins = KPluginMetaData::findPlugins(QStringLiteral("lingmo/network/vpn"), filter);

            const QString pluginBaseName = vpnSetting->serviceType().remove(QLatin1String("org.freedesktop.NetworkManager."));

            if (lingmoNmPlugins.empty()) {
                qCWarning(LINGMO_NM_LIBS_LOG) << "VPN" << vpnSetting->serviceType() << "not found, skipping";
                ...
                notification->setText(i18n("Lingmo is missing support for '%1' VPN connections.", pluginBaseName));
        ...
        if (nmModemDevice) {
            ...
            if (mmModemDevice) {
                ...
                if (gsmSetting && gsmSetting->pinFlags() == NetworkManager::Setting::NotSaved && modem && modem->unlockRequired() > MM_MODEM_LOCK_NONE) {
                    QDBusInterface managerIface(QStringLiteral("org.kde.lingmonetworkmanagement"),
...
QCoro::Task<void> Handler::deactivateConnectionInternal(const QString &_connection, const QString &device)
{
    ...
    if (!con) {
        qCWarning(LINGMO_NM_LIBS_LOG) << "Not possible to deactivate this connection";
        co_return;
    }
...
QCoro::Task<void> Handler::enableBluetooth(bool enable)
{
    ...
    if (!reply.isValid()) {
        qCWarning(LINGMO_NM_LIBS_LOG) << reply.error().message();
        co_return;
    }

    for (const QDBusObjectPath &path : reply.value().keys()) {
        const QString objPath = path.path();
        qCDebug(LINGMO_NM_LIBS_LOG) << "inspecting path" << objPath;
        const QStringList interfaces = reply.value().value(path).keys();
        qCDebug(LINGMO_NM_LIBS_LOG) << "interfaces:" << interfaces;
...
QCoro::Task<void> Handler::removeConnectionInternal(const QString &connection)
{
    ...
    if (!con || con->uuid().isEmpty()) {
        qCWarning(LINGMO_NM_LIBS_LOG) << "Not possible to remove connection " << connection;
        co_return;
    }
        ...
        if (device->type() == NetworkManager::Device::Wifi) {
            ...
            if (wifiDevice && wifiDevice->state() != NetworkManager::WirelessDevice::Unavailable) {
                ...
                if (!checkRequestScanRateLimit(wifiDevice)) {
                    ...
                    qCDebug(LINGMO_NM_LIBS_LOG) << "Rescheduling a request scan for" << wifiDevice->interfaceName() << "in" << timeout;
                ...
                qCDebug(LINGMO_NM_LIBS_LOG) << "Requesting wifi scan on device" << wifiDevice->interfaceName();
                ...
                if (!reply.isValid()) {
                    ...
                    qCWarning(LINGMO_NM_LIBS_LOG) << "Wireless scan on" << interface << "failed:" << reply.error().message();
                    scanRequestFailed(interface);
                } else {
                    qCDebug(LINGMO_NM_LIBS_LOG) << "Wireless scan on" << wifiDevice->interfaceName() << "succeeded";
                }
                decrementScansCount();
            }
        }
...
void Handler::decrementScansCount()
{
    if (m_ongoingScansCount == 0) {
        qCDebug(LINGMO_NM_LIBS_LOG) << "Extra decrementScansCount() called";
        return;
    }
    m_ongoingScansCount -= 1;
    if (m_ongoingScansCount == 0) {
...
QCoro::Task<void> Handler::createHotspotInternal()
{
    ...
    if (!wifiDev) {
        qCWarning(LINGMO_NM_LIBS_LOG) << "Failed to create hotspot: missing wireless device";
        co_return;
    }
...
bool Handler::checkRequestScanRateLimit(const NetworkManager::WirelessDevice::Ptr &wifiDevice)
{
    ...
    if (ret) {
        qCDebug(LINGMO_NM_LIBS_LOG) << "Last scan finished" << lastScan.msecsTo(now) << "ms ago and last request scan was sent" //
