...
SecretAgent::SecretAgent(QObject *parent)
    : NetworkManager::SecretAgent(QStringLiteral("org.kde.lingmo.networkmanagement"), NetworkManager::SecretAgent::Capability::VpnHints, parent)
...
NMVariantMapMap SecretAgent::GetSecrets(const NMVariantMapMap &connection,
                                        ...
                                        uint flags)
{
    qCDebug(LINGMO_NM_KDED_LOG) << Q_FUNC_INFO;
    qCDebug(LINGMO_NM_KDED_LOG) << "Path:" << connection_path.path();
    qCDebug(LINGMO_NM_KDED_LOG) << "Setting name:" << setting_name;
    qCDebug(LINGMO_NM_KDED_LOG) << "Hints:" << hints;
    qCDebug(LINGMO_NM_KDED_LOG) << "Flags:" << flags;

    const QString callId = connection_path.path() % setting_name;
    for (const SecretsRequest &request : std::as_const(m_calls)) {
        if (request == callId) {
            qCWarning(LINGMO_NM_KDED_LOG) << "GetSecrets was called again! This should not happen, cancelling first call" << connection_path.path()
                                          << setting_name;
...
void SecretAgent::SaveSecrets(const NMVariantMapMap &connection, const QDBusObjectPath &connection_path)
{
    qCDebug(LINGMO_NM_KDED_LOG) << Q_FUNC_INFO;
    qCDebug(LINGMO_NM_KDED_LOG) << "Path:" << connection_path.path();
    // qCDebug(LINGMO_NM_KDED_LOG) << "Setting:" << connection;
...
bool SecretAgent::processGetSecrets(SecretsRequest &request) const
{
    ...
    if (isVpn) {
        ...
        if (vpnSetting->serviceType() == QLatin1String("org.freedesktop.NetworkManager.ssh") && vpnSetting->data()["auth-type"] == QLatin1String("ssh-agent")) {
            ...
            qCDebug(LINGMO_NM_KDED_LOG) << Q_FUNC_INFO << "Sending SSH auth socket" << authSock;
    ...
    if (!requestNew && useWallet()) {
        if (m_wallet->isOpen()) {
            ...
            qCDebug(LINGMO_NM_KDED_LOG) << Q_FUNC_INFO << "Waiting for the wallet to open";
            return false;
        }
    }
    ...
    if (!Configuration::self().showPasswordDialog()) {
        ...
        sendError(SecretAgent::InternalError, QStringLiteral("Lingmo-nm did not know how to handle the request"), request.message);
        return true;
    }
}
...
bool SecretAgent::processSaveSecrets(SecretsRequest &request) const
{
    ...
    if (!request.saveSecretsWithoutReply) {
        ...
        if (!QDBusConnection::systemBus().send(reply)) {
            qCWarning(LINGMO_NM_KDED_LOG) << "Failed put save secrets reply into the queue";
        }
    }
...
bool SecretAgent::processDeleteSecrets(SecretsRequest &request) const
{
    ...
    QDBusMessage reply = request.message.createReply();
    if (!QDBusConnection::systemBus().send(reply)) {
        qCWarning(LINGMO_NM_KDED_LOG) << "Failed put delete secrets reply into the queue";
    }
...
bool SecretAgent::useWallet() const
{
    ...
    if (KWallet::Wallet::isEnabled()) {
        ...
        if (m_wallet) {
            ...
            qCWarning(LINGMO_NM_KDED_LOG) << "Error opening kwallet.";
        }
    } else if (m_wallet) {
        m_wallet->deleteLater();
        m_wallet = nullptr;
...
void SecretAgent::sendSecrets(const NMVariantMapMap &secrets, const QDBusMessage &message) const
{
    ...
    if (!QDBusConnection::systemBus().send(reply)) {
        qCWarning(LINGMO_NM_KDED_LOG) << "Failed put the secret into the queue";
    }
}

void SecretAgent::importSecretsFromPlainTextFiles()
{
    KConfig config(QStringLiteral("lingmo-networkmanagement"), KConfig::SimpleConfig);
