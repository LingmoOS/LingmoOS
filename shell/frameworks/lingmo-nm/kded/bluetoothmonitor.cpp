...
{
    qCDebug(LINGMO_NM_KDED_LOG) << "Adding BT connection:" << bdAddr << service;
    ...
    qCDebug(LINGMO_NM_KDED_LOG) << "Bdaddr == " << bdAddr;
    ...
    if (service == QLatin1String("nap")) {
        ...
        connect(mobileConnectionWizard.data(), &MobileConnectionWizard::accepted, [bdAddr, connectionName, mobileConnectionWizard]() {
            if (mobileConnectionWizard->getError() == MobileProviders::Success) {
                qCDebug(LINGMO_NM_KDED_LOG) << "Mobile broadband wizard finished:" << mobileConnectionWizard->type() << mobileConnectionWizard->args();
                if (mobileConnectionWizard->args().count() == 2) { // GSM or CDMA
                    qCDebug(LINGMO_NM_KDED_LOG) << "Creating new DUN connection for BT device:" << bdAddr;
                    ...
                    qCDebug(LINGMO_NM_KDED_LOG) << "Adding DUN connection" << connectionSettings;
