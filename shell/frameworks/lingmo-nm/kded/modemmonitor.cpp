    ...
    , d_ptr(new ModemMonitorPrivate)
{
    ...
    KSharedConfigPtr config = KSharedConfig::openConfig(QLatin1String("lingmo-nm"));
...
{
    Q_D(ModemMonitor);
    qCDebug(LINGMO_NM_KDED_LOG) << "unlockRequired == " << lock;
    ...
    if (d->dialog) {
        qCDebug(LINGMO_NM_KDED_LOG) << "PinDialog already running";
        return;
    }
...
#define MM_DIALOG(lockType, dialogType)                                                                                                                        \
    ...
    qCDebug(LINGMO_NM_KDED_LOG) << "Sending unlock code";
