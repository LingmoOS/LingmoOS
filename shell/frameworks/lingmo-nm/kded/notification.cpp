...
void Notification::onActiveConnectionStateChanged(NetworkManager::ActiveConnection::State state)
{
    ...
    if (state == NetworkManager::ActiveConnection::Activated) {
        ...
        if (m_justLaunched) {
            qCDebug(LINGMO_NM_KDED_LOG) << "Not emitting connection activated notification as the daemon was just launched";
            return;
        }
        ...
        if (foundConnection != m_activeConnectionsBeforeSleep.constEnd()) {
            qCDebug(LINGMO_NM_KDED_LOG) << "Not emitting connection activated notification as the connection was active prior to suspend";
            return;
        }
        ...
        if (m_preparingForSleep) {
            qCDebug(LINGMO_NM_KDED_LOG) << "Not emitting connection deactivated notification as we're about to suspend";
            return;
        }

        if (m_checkActiveConnectionOnResumeTimer && m_checkActiveConnectionOnResumeTimer->isActive()) {
            qCDebug(LINGMO_NM_KDED_LOG) << "Not emitting connection deactivated notification as we've just woken up from suspend";
            return;
        }
        ...
        qCWarning(LINGMO_NM_KDED_LOG) << "Unhandled active connection state change: " << state;
        return;
    }
...
void Notification::onVpnConnectionStateChanged(NetworkManager::VpnConnection::State state, NetworkManager::VpnConnection::StateChangeReason reason)
{
    ...
    if (state == NetworkManager::VpnConnection::Activated) {
        ...
        qCWarning(LINGMO_NM_KDED_LOG) << "Unhandled VPN connection state change: " << state;
        return;
    }
