...
void PasswordDialog::initializeUi()
{
    ...
    if (m_connectionSettings->connectionType() != NetworkManager::ConnectionSettings::Vpn) {
        ...
        if (m_neededSecrets.isEmpty()) {
            qCWarning(LINGMO_NM_KDED_LOG) << "list of secrets is empty!!!";
            m_hasError = true;
        ...
        if (!vpnSetting) {
            qCWarning(LINGMO_NM_KDED_LOG) << "Missing VPN setting!";
            m_hasError = true;
            m_error = SecretAgent::InternalError;
            ...
            if (result) {
                ...
                qCWarning(LINGMO_NM_KDED_LOG) << "Could not load VPN UI plugin" << result.errorText;
                m_hasError = true;
                m_error = SecretAgent::InternalError;
