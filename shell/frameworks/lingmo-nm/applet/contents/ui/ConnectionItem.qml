...
LingmoExtras.ExpandableListItem {
    ...
    property bool activating: ConnectionState === LingmoNM.Enums.Activating
    property bool deactivated: ConnectionState === LingmoNM.Enums.Deactivated
    property bool passwordIsStatic: (SecurityType === LingmoNM.Enums.StaticWep || SecurityType == LingmoNM.Enums.WpaPsk ||
                                     SecurityType === LingmoNM.Enums.Wpa2Psk || SecurityType == LingmoNM.Enums.SAE)
    property bool predictableWirelessPassword: !Uuid && Type === LingmoNM.Enums.Wireless && passwordIsStatic
    property bool showSpeed: mainWindow.expanded &&
                             ConnectionState === LingmoNM.Enums.Activated &&
    ...
    subtitle: itemText()
    isBusy: mainWindow.expanded && model.ConnectionState === LingmoNM.Enums.Activating
    isDefault: ConnectionState === LingmoNM.Enums.Activated
    defaultActionButtonAction: Action {
        id: stateChangeButton

        readonly property bool isDeactivated: model.ConnectionState === LingmoNM.Enums.Deactivated
    ...
    contextualActions: [
        Action {
            enabled: Uuid && Type === LingmoNM.Enums.Wireless && passwordIsStatic
    ...
    /* This generates the formatted text under the connection name
       ...
       "Disconnect"ed. */
    function itemText() {
        if (ConnectionState === LingmoNM.Enums.Activating) {
            if (Type === LingmoNM.Enums.Vpn) {
                return VpnState
            } else {
                return DeviceState
            }
        } else if (ConnectionState === LingmoNM.Enums.Deactivating) {
            if (Type === LingmoNM.Enums.Vpn) {
                return VpnState
            } else {
                return DeviceState
            }
        } else if (Uuid && ConnectionState === LingmoNM.Enums.Deactivated) {
            return LastUsed
        } else if (ConnectionState === LingmoNM.Enums.Activated) {
            if (showSpeed) {
    ...
    function setDelayModelUpdates(delay: bool) {
        appletProxyModel.setData(appletProxyModel.index(index, 0), delay, LingmoNM.NetworkModel.DelayModelUpdatesRole);
    }
    ...
    onDeactivatedChanged: {
        ...
        if (appletProxyModel.data(appletProxyModel.index(0, 0), LingmoNM.NetworkModel.SectionRole) !== "Available connections") {
