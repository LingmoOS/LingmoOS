    ...
    , m_handler(new Handler(this))
{
    ...
    for (QVariant arg : args) {
        if (arg.canConvert(QMetaType::QString)) {
            ...
            if (uuid.startsWith(uuidArgumentMarker)) {
                ...
                qDebug(LINGMO_NM_KCM_LOG) << "Selecting user connection:" << uuid;
                break;
            }
        }
    }
    ...
    if (!selectedConnection || !selectedConnection->isValid()) {
        ...
        if (activeConnection && activeConnection->isValid()) {
            ...
            qDebug(LINGMO_NM_KCM_LOG) << "Selecting active connection:" << selectedConnection->uuid();
        }
    }
    ...
    if (!selectedConnection || !selectedConnection->isValid()) {
        ...
        for (const NetworkManager::Connection::Ptr &connection : connectionList) {
            ...
            if (UiUtils::isConnectionTypeSupported(type)) {
                ...
                qDebug(LINGMO_NM_KCM_LOG) << "Selecting first connection:" << connection->uuid();
                break;
            }
        }
    }
    ...
    if (selectedConnection && selectedConnection->isValid()) {
        ...
        qDebug(LINGMO_NM_KCM_LOG) << "Cannot preselect a connection";
    }
    ...
    if (type == NetworkManager::ConnectionSettings::Vpn && vpnType == "imported") {
        ...
        connect(wizard.data(), &MobileConnectionWizard::accepted, [wizard, this]() {
            if (wizard->getError() == MobileProviders::Success) {
                qCDebug(LINGMO_NM_KCM_LOG) << "Mobile broadband wizard finished:" << wizard->type() << wizard->args();
                ...
                if (wizard->args().count() == 2) {
                    ...
                    if (wizard->type() == NetworkManager::ConnectionSettings::Gsm) {
                        ...
                        qCWarning(LINGMO_NM_KCM_LOG) << Q_FUNC_INFO << "Unhandled setting type";
                    }
                    // Generate new UUID
                    connectionSettings->setUuid(NetworkManager::ConnectionSettings::createNewUuid());
                    addConnection(connectionSettings);
                } else {
                    qCWarning(LINGMO_NM_KCM_LOG) << Q_FUNC_INFO << "Unexpected number of args to parse";
                }
            }
        });
...
void KCMNetworkmanagement::onRequestExportConnection(const QString &connectionPath)
{
    ...
    qCDebug(LINGMO_NM_KCM_LOG) << "Exporting VPN connection" << connection->name() << "type:" << vpnSetting->serviceType();
    ...
    if (result) {
        ...
        if (vpnPlugin->suggestedFileName(connSettings).isEmpty()) { // this VPN doesn't support export
            qCWarning(LINGMO_NM_KCM_LOG) << "This VPN doesn't support export";
            return;
        }
        ...
        if (!filename.isEmpty()) {
            if (auto result = vpnPlugin->exportConnectionSettings(connSettings, filename)) {
                ...
                qCWarning(LINGMO_NM_KCM_LOG) << "Failed to export VPN connection" << result.errorMessage();
            }
        }
    } else {
        qCWarning(LINGMO_NM_KCM_LOG) << "Error getting VpnUiPlugin for export:" << result.errorText;
    }
}
...
KCMNetworkmanagement::ImportResult KCMNetworkmanagement::importVpn()
{
    ...
    const QList<KPluginMetaData> services = KPluginMetaData::findPlugins(QStringLiteral("lingmo/network/vpn"));
    ...
    qCDebug(LINGMO_NM_KCM_LOG) << "Importing VPN connection " << filename << "extension:" << ext;
    ...
    if (WireGuardInterfaceWidget::supportedFileExtensions().contains(ext)) {
        ...
        if (error) {
            qCDebug(LINGMO_NM_KCM_LOG) << "Could not import WireGuard connection" << error->message;
        } else {
        ...
        // qCDebug(LINGMO_NM_KCM_LOG) << "Converted connection:" << connectionSettings;

        m_handler->addConnection(connectionSettings.toMap());
        // qCDebug(LINGMO_NM_KCM_LOG) << "Adding imported connection under id:" << conId;
    ...
    for (const KPluginMetaData &service : services) {
        ...
        if (vpnPlugin->supportedFileExtensions().contains(ext)) {
            qCDebug(LINGMO_NM_KCM_LOG) << "Found VPN plugin" << service.name() << ", type:" << service.value("X-NetworkManager-Services");

            VpnUiPlugin::ImportResult result = vpnPlugin->importConnectionSettings(filename);

            if (!result) {
                qWarning(LINGMO_NM_KCM_LOG) << "Failed to import" << filename << result.errorMessage();
