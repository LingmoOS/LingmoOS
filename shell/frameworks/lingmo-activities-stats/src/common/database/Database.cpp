...
public:
        ...
        : m_open(false)
    {
        ...
        if (!m_open) {
            qCWarning(LINGMO_ACTIVITIES_STATS_LOG) << "LingmoActivities: Database is not open: " << m_database.connectionName() << m_database.databaseName()
                                             << m_database.lastError();

            if (info.openMode == Database::ReadWrite) {
                qFatal("LingmoActivities: Opening the database in RW mode should always succeed");
            }
        }
    }
    ...
    ~QSqlDatabaseWrapper()
    {
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Closing SQL connection: " << m_connectionName;
    }
...
Database::Ptr Database::instance(Source source, OpenMode openMode)
{
    ...
    if (walResult != QLatin1String("wal")) {
        qCWarning(LINGMO_ACTIVITIES_STATS_LOG) << "LingmoActivities: Database can not be opened in WAL mode. Check the "
    ...
    qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "LingmoActivities: Database connection: " << ptr->d->database->connectionName()
