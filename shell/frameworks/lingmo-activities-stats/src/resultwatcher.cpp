...
#define DEBUG_MATCHERS 0
    ...
    {
#if DEBUG_MATCHERS
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Activity " << activity << "matching against" << query.activities();
#endif
    ...
    {
#if DEBUG_MATCHERS
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Agent " << agent << "matching against" << query.agents();
#endif
    ...
    {
#if DEBUG_MATCHERS
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Url " << url << "matching against" << urlFilters;
#endif
...
#if DEBUG_MATCHERS
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Type "
                                       << "...type..."
                                       << "matching against" << query.types();
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "ANY_TYPE_TAG" << ANY_TYPE_TAG;
#endif
    ...
    {
#if DEBUG_MATCHERS
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Resource has been linked: " << agent << resource << activity;
#endif
    ...
    {
#if DEBUG_MATCHERS
        qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Resource unlinked: " << agent << resource << activity;
#endif
...
void ResultWatcher::unlinkFromActivity(const QUrl &resource, const Terms::Activity &activity, const Terms::Agent &agent)
{
    ...
    for (const auto &activity : activities) {
        for (const auto &agent : agents) {
            qCDebug(LINGMO_ACTIVITIES_STATS_LOG) << "Unlink " << agent << resource << activity;
