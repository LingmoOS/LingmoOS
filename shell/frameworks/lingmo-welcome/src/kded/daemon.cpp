...
K_PLUGIN_CLASS_WITH_JSON(LingmoWelcomeDaemon, "kded_lingmo-welcome.json")

LingmoWelcomeDaemon::LingmoWelcomeDaemon(QObject *parent, const QList<QVariant> &)
    : KDEDModule(parent)
    , m_config(new KConfig(QStringLiteral("lingmo-welcomerc")), "General")
    , m_currentVersion(QVersionNumber::fromString(QString::fromLatin1(LINGMO_WELCOME_VERSION_STRING)))
    , m_previousVersion(QVersionNumber::fromString(m_config.readEntry("LastSeenVersion", QString::fromLatin1(LINGMO_WELCOME_VERSION_STRING))))
{
...
bool LingmoWelcomeDaemon::isSignificantUpgrade() const
{
...
void LingmoWelcomeDaemon::launch(const QStringList &args)
{
    auto *job = new KIO::CommandLauncherJob(QStringLiteral("lingmo-welcome"), args);
    job->setUiDelegate(new KNotificationJobUiDelegate(KJobUiDelegate::AutoErrorHandlingEnabled));
    job->setDesktopName(QStringLiteral("org.kde.lingmo-welcome"));
    job->start();
}
