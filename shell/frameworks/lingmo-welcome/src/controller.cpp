...
#include "config-lingmo-welcome.h"
#include "controller.h"
#include "lingmo-welcome-version.h"
...
Controller::Controller()
{
    ...
    // Version
    m_version = QVersionNumber::fromString(QString::fromLatin1(LINGMO_WELCOME_VERSION_STRING));
    m_patchVersion = m_version.microVersion();

    // Release URL
    if constexpr (LINGMO_WELCOME_VERSION_PATCH >= 90) {
        // Beta version
        m_releaseUrl = QStringLiteral("https://kde.org/announcements/lingmo/%1/%1.%2.90/?source=lingmo-welcome")
                           .arg(QString::number(m_version.majorVersion()), QString::number(m_version.minorVersion()));
    } else if constexpr (LINGMO_WELCOME_VERSION_PATCH >= 80) {
        // Development version
        m_releaseUrl = QStringLiteral("https://invent.kde.org/groups/lingmo/-/activity");
    } else {
        // Release version
        m_releaseUrl = QStringLiteral("https://kde.org/announcements/lingmo/%1/%1.%2.0/?source=lingmo-welcome")
                           .arg(QString::number(m_version.majorVersion()), QString::number(m_version.minorVersion()));
    }

    // Shown version string, matching desktop preview banner
    if constexpr (LINGMO_WELCOME_VERSION_MAJOR == 5 && LINGMO_WELCOME_VERSION_MINOR >= 80) {
        // Overrides for Lingmo 6.0 pre-release
        if constexpr (LINGMO_WELCOME_VERSION_PATCH == 80) {
            // Development
            m_shownVersion = i18n("6.0 Dev");
        } else if constexpr (LINGMO_WELCOME_VERSION_MINOR == 80) {
            // Alpha, 5.80.0
            m_shownVersion = i18n("6.0 Alpha");
        } else if constexpr (LINGMO_WELCOME_VERSION_MINOR == 90) {
            // Beta 1, 5.90.0
            m_shownVersion = i18n("6.0 Beta 1");
        } else if constexpr (LINGMO_WELCOME_VERSION_MINOR == 91) {
            // Beta 2, 5.91.0
            m_shownVersion = i18n("6.0 Beta 2");
        } else if constexpr (LINGMO_WELCOME_VERSION_MINOR == 92) {
            // RC1, 5.92.0
            m_shownVersion = i18nc("@label RC meaning Release Candidate", "6.0 RC1");
        } else if constexpr (LINGMO_WELCOME_VERSION_MINOR == 93) {
            // RC2, 5.93.0
        ...
        // and should be updated after the final Lingmo 6 release
        ...
        int major = (LINGMO_WELCOME_VERSION_MAJOR == finalMajor && LINGMO_WELCOME_VERSION_MINOR == finalMinor) ? LINGMO_WELCOME_VERSION_MAJOR + 1
                                                                                                               : LINGMO_WELCOME_VERSION_MAJOR;
        int minor = (LINGMO_WELCOME_VERSION_MAJOR == finalMajor && LINGMO_WELCOME_VERSION_MINOR == finalMinor) ? 0 : LINGMO_WELCOME_VERSION_MINOR + 1;
        const QString version = QStringLiteral("%1.%2").arg(QString::number(major), QString::number(minor));

        if constexpr (LINGMO_WELCOME_VERSION_PATCH == 80) {
            // Development version
            m_shownVersion = i18nc("@label %1 is the Lingmo version", "%1 Dev", version);
        } else if constexpr (LINGMO_WELCOME_VERSION_PATCH >= 90) {
            // Beta version
            if constexpr (LINGMO_WELCOME_VERSION_PATCH == 90) {
                m_shownVersion = i18nc("@label %1 is the Lingmo version", "%1 Beta", version);
            } else {
                constexpr int betaNumber = LINGMO_WELCOME_VERSION_PATCH - 89;
                m_shownVersion = i18nc("@label %1 is the Lingmo version, %2 is the beta release number", "%1 Beta %2", version, betaNumber);
            }
        }
    } else {
        // Release version
...
{
    KPluginMetaData data(QStringLiteral("lingmo/kcms/systemsettings/kcm_feedback"));
    return data.isValid();
}
...
{
    return QString::fromLatin1(LINGMO_WELCOME_INSTALL_DIR);
}
