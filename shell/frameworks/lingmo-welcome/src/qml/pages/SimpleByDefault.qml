...
GenericPage {
    ...
    description: xi18nc("@info:usagetip", "Lingmo is designed to be simple and usable out of the box. Things are where you would expect, and there is generally no need to configure anything before you can be comfortable and productive.<nl/><nl/>Below is a visual representation of a typical Lingmo Desktop; move the pointer over or click on something to learn about it!")
        ...
        MockDesktop {
            ...
            Item {
                ...
                Item {
                    ...
                    width: Math.min(Math.round(mockDesktop.width / 1.5), LingmoUI.Units.gridUnit * 25) + explanatoryLabel.anchors.margins * 2
                    height: explanatoryLabel.implicitHeight + explanatoryLabel.anchors.margins * 2

                    Behavior on height {
                        NumberAnimation {
                            // Don't animate the height if we aren't visible!
                            duration: explanatoryContainer.visible ? LingmoUI.Units.shortDuration : 0
                            easing.type: Easing.InOutQuad
                        }
                    }
