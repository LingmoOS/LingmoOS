...
function printDebug(payload, fn) {
    ...
    const hostLabelColor = "#3daee9"; // Ocean highlight color
...
function updateBrowserAction() {
    if (portStatus === "UNSUPPORTED_OS" || portStatus === "STARTUP_FAILED") {
        chrome.browserAction.setIcon({
            path: {
                "16": "icons/lingmo-disabled-16.png",
                "32": "icons/lingmo-disabled-32.png",
                "48": "icons/lingmo-disabled-48.png",
                "128": "icons/lingmo-disabled-128.png"
            }
        });
    }
    ...
    if (portLastErrorMessage && receivedMessageOnce) {
        ...
        chrome.browserAction.setBadgeBackgroundColor({ color: "#da4453" }); // ocean "negative" color
    } else {
        chrome.browserAction.setBadgeText({ text: "" });
    }
}
...
function connectHost() {
    port = chrome.runtime.connectNative("org.kde.lingmo.browser_integration");
