...
LingmoDesktopUnits::LingmoDesktopUnits(QObject *parent)
    : LingmoUI::Platform::Units(parent)
#if defined(Q_OS_WIN)
...
// Copy from lingmo-framework/src/declarativeimports/core/units.cpp, since we don't want to depend on lingmo-framework here
void LingmoDesktopUnits::updateAnimationSpeed()
{
    // Read the old longDuration value for compatibility
    KConfigGroup cfg = KConfigGroup(KSharedConfig::openConfig(QStringLiteral("lingmorc")), QStringLiteral("Units"));
...
#include "moc_lingmodesktopunits.cpp"
