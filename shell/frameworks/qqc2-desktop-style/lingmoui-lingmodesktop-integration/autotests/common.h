...
void initTestCase()
{
    ...
    if (qEnvironmentVariableIsSet("KDECI_BUILD")) {
        ...
        const QString lingmouiFolder(libraryDir.absolutePath() + QDir::separator() + u"kf6"_s + QDir::separator() + u"lingmoui" + QDir::separator()
                                     + u"platform"_s);
        QVERIFY2(libraryDir.mkpath(lingmouiFolder), qUtf8Printable(lingmouiFolder));
        const QString targetFilePath = lingmouiFolder + QDir::separator() + QFileInfo(libraryPath).fileName();
