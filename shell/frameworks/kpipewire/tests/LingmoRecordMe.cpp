...
LingmoRecordMe::LingmoRecordMe(Screencasting::CursorMode cursorMode, const QStringList &sources, bool doSelection, QObject *parent)
    : QObject(parent)
    ...
    , m_engine(new QQmlApplicationEngine(this))
{
    ...
    connect(qGuiApp, &QGuiApplication::screenAdded, this, &LingmoRecordMe::addScreen);

    if (m_sources.isEmpty() || m_sources.contains(QLatin1String("region"))) {
        connect(this, &LingmoRecordMe::workspaceChanged, this, [this] {
...
LingmoRecordMe::~LingmoRecordMe()
{
}

bool LingmoRecordMe::matches(const QString &value)
{
...
void LingmoRecordMe::addWindow(const QVariantList &uuid, const QString &appId)
{
...
void LingmoRecordMe::addStream(int nodeid, const QString &displayText, int fd, bool allowDmaBuf)
{
...
void LingmoRecordMe::startNode(int node)
{
    addStream(node, QStringLiteral("raw node %1").arg(node), 0, true);
}

void LingmoRecordMe::addScreen(QScreen *screen)
{
    auto f = [this, screen] {
    ...
    if (matches(screen->name())) {
        connect(this, &LingmoRecordMe::cursorModeChanged, screen, f);
        f();
...
void LingmoRecordMe::start(ScreencastingStream *stream, bool allowDmaBuf)
{
    qDebug() << "start" << stream;
    ...
    connect(this, &LingmoRecordMe::cursorModeChanged, stream, &ScreencastingStream::closed);
}

void LingmoRecordMe::setDuration(int duration)
{
    m_durationTimer->setInterval(duration);
}

void LingmoRecordMe::createVirtualMonitor()
{
    m_screencasting->createVirtualMonitorStream(QStringLiteral("recordme"), {1920, 1080}, 1, m_cursorMode);
}

void LingmoRecordMe::setRegionPressed(const QString &screenName, int x, int y)
{
...
void LingmoRecordMe::setRegionMoved(const QString &screenName, int x, int y)
{
    const auto screens = QGuiApplication::screens();
    auto screenIt = std::find_if(screens.begin(), screens.end(), [&screenName](auto screen) {
        return screen->name() == screenName;
...
void LingmoRecordMe::setRegionReleased(const QString &screenName, int x, int y)
{
...
void LingmoRecordMe::requestSelection()
{
    for (auto *screen : qApp->screens()) {
        ...
        view->showFullScreen();
        connect(this, &LingmoRecordMe::regionFinal, view, &QQuickView::deleteLater);
    }

    connect(this, &LingmoRecordMe::regionFinal, this, [this](const QRect &region) {
...
QRect LingmoRecordMe::region() const
{
    return m_region.normalized();
}

#include "moc_LingmoRecordMe.cpp"
