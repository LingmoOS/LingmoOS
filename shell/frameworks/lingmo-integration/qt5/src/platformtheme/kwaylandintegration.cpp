...
bool KWaylandIntegration::eventFilter(QObject *watched, QEvent *event)
{
    if (event->type() == QEvent::Expose) {
        ...
        if (w->property("org.kde.lingmo.integration.shellSurfaceCreated").isNull()) {
...
void KWaylandIntegration::shellSurfaceCreated(QWindow *w)
{
    ...
    w->setProperty("org.kde.lingmo.integration.shellSurfaceCreated", true);
    ...
    if (m_appMenuManager->isActive()) {
        ...
        w->setProperty("org.kde.lingmo.integration.appmenu", QVariant::fromValue(menu));
...
{
    w->setProperty("org.kde.lingmo.integration.shellSurfaceCreated", QVariant());

    auto appMenu = w->property("org.kde.lingmo.integration.appmenu").value<AppMenu *>();
    if (appMenu) {
        delete appMenu;
    }
    w->setProperty("org.kde.lingmo.integration.appmenu", QVariant());

    auto decoPallete = w->property("org.kde.lingmo.integration.palette").value<ServerSideDecorationPalette *>();
    if (decoPallete) {
        delete decoPallete;
    }
    w->setProperty("org.kde.lingmo.integration.palette", QVariant());
}
...
void KWaylandIntegration::installColorScheme(QWindow *w)
{
    ...
    auto palette = w->property("org.kde.lingmo.integration.palette").value<ServerSideDecorationPalette *>();
    if (!palette) {
        ...
        w->setProperty("org.kde.lingmo.integration.palette", QVariant::fromValue(palette));
    }
    if (palette) {
...
{
    auto menu = window->property("org.kde.lingmo.integration.appmenu").value<AppMenu *>();
    if (menu) {
