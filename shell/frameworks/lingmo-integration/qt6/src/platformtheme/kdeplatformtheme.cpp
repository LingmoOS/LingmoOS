...
// and org.kde.ocean for QGuiApplication unless it's otherwise set.
void KdePlatformTheme::setQtQuickControlsTheme()
{
    ...
    // org.kde.ocean style if it's installed.
    if (!qobject_cast<QApplication *>(qApp)) {
        if (qgetenv("QT_QUICK_CONTROLS_1_STYLE").right(7) == "Desktop") {
            qunsetenv("QT_QUICK_CONTROLS_1_STYLE");
        }
        // /org/kde/ocean/components is not installed by org.kde.ocean, but the impl folder is
        if (checkIfThemeExists(QStringLiteral("/org/kde/ocean/impl"))) {
...
{
    static bool usePortal = qEnvironmentVariableIntValue("LINGMO_INTEGRATION_USE_PORTAL") == 1;
    return usePortal;
}
