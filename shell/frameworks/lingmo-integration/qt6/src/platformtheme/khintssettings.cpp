...
static const QString defaultLookAndFeelPackage = QStringLiteral("org.kde.ocean.desktop");
    ...
    , mUsePortal(KSandbox::isInside())
{
    ...
    m_hints[QPlatformTheme::SystemIconThemeName] = readConfigValue(QStringLiteral("Icons"), QStringLiteral("Theme"), QStringLiteral("ocean"));
...
void KHintsSettings::slotNotifyChange(int type, int arg)
{
    ...
    case StyleChanged: {
        ...
        const QString theme = readConfigValue(cg, QStringLiteral("widgetStyle"), QStringLiteral("ocean")).toString();

        QStringList styleNames;
        if (theme != QStringLiteral("ocean")) {
            styleNames << theme;
        }
        styleNames << QStringLiteral("ocean") << QStringLiteral("oxygen") << QStringLiteral("fusion") << QStringLiteral("windows");
        const QString lnfStyle = readConfigValue(QStringLiteral("KDE"), QStringLiteral("widgetStyle"), QString()).toString();
...
void KHintsSettings::loadPalettes()
{
    ...
    if (mUsePortal && mKdeGlobalsPortal.contains(QStringLiteral("org.kde.kdeglobals.Colors:View"))) {
        ...
        QString path = QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                              QStringLiteral("lingmo/look-and-feel/") + looknfeel + QStringLiteral("/contents/colors"));
        if (!path.isEmpty()) {
            m_palettes[QPlatformTheme::SystemPalette] = new QPalette(KColorScheme::createApplicationPalette(KSharedConfig::openConfig(path)));
            return;
        }

        const QString scheme = readConfigValue(QStringLiteral("General"), QStringLiteral("ColorScheme"), QStringLiteral("OceanLight")).toString();
        path = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("color-schemes/") + scheme + QStringLiteral(".colors"));

        if (path.isEmpty()) {
            qWarning() << "Could not find color scheme" << scheme << "falling back to OceanLight";
            path = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("color-schemes/OceanLight.colors"));
        }
...
void KHintsSettings::updateCursorTheme()
{
    ...
    const QString cursorTheme = readConfigValue(mouseConfig, QStringLiteral("cursorTheme"), QStringLiteral("ocean_cursors")).toString();
