...
void DialogModule::init(const LingmoVault::Vault::Payload &payload)
{
    Q_UNUSED(payload);
}
...
LingmoVault::Vault::Payload CompoundDialogModule::fields() const
{
    LingmoVault::Vault::Payload result;
...
void CompoundDialogModule::init(const LingmoVault::Vault::Payload &payload)
{
    for (const auto &child : m_children) {
        child->init(payload);
    }
