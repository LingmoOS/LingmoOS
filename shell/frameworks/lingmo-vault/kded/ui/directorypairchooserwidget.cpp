...
LingmoVault::Vault::Payload DirectoryPairChooserWidget::fields() const
{
    return {
...
void DirectoryPairChooserWidget::init(const LingmoVault::Vault::Payload &payload)
{
    if (d->flags & DirectoryPairChooserWidget::AutoFillPaths) {
        const QString basePath = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QStringLiteral("/lingmo-vault");
