...
public:
    ...
    using ExpectedData = AsynQt::Expected<Data, LingmoVault::Error>;
    ExpectedData data;
    ...
    Private(Vault *parent, const Device &device)
        : q(parent)
        , config(KSharedConfig::openConfig(LINGMOVAULT_CONFIG_FILE))
        , device(device)
...
{
    const auto config = KSharedConfig::openConfig(LINGMOVAULT_CONFIG_FILE);
