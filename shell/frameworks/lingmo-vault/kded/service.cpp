...
K_PLUGIN_FACTORY_WITH_JSON(LingmoVaultServiceFactory, "lingmovault.json", registerPlugin<LingmoVaultService>();)

using namespace LingmoVault;

using AsynQt::Expected;

class LingmoVaultService::Private
{
public:
...
LingmoVaultService::LingmoVaultService(QObject *parent, const QVariantList &)
    : KDEDModule(parent)
    , d(new Private())
{
    connect(this, &KDEDModule::moduleRegistered, this, &LingmoVaultService::slotRegistered);

    // Close vaults that don't belong to the current activity
    connect(&d->kamd, &KActivities::Consumer::currentActivityChanged, this, &LingmoVaultService::onCurrentActivityChanged);

    // When an activity is deleted, remove it from all the vaults
    connect(&d->kamd, &KActivities::Consumer::activityRemoved, this, &LingmoVaultService::onActivityRemoved);

    // When activities are loaded, remove activities that no longer exist
    // the vaults
    connect(&d->kamd, &KActivities::Consumer::activitiesChanged, this, &LingmoVaultService::onActivitiesChanged);
...
LingmoVaultService::~LingmoVaultService()
{
}

LingmoVault::VaultInfoList LingmoVaultService::availableDevices() const
{
    LingmoVault::VaultInfoList result;
...
void LingmoVaultService::requestNewVault()
{
    const auto dialog = new VaultCreationWizard();

    connect(dialog, &VaultCreationWizard::createdVault, this, &LingmoVaultService::registerVault);

    dialog->show();
}

void LingmoVaultService::requestImportVault()
{
    const auto dialog = new VaultImportingWizard();

    connect(dialog, &VaultImportingWizard::importedVault, this, &LingmoVaultService::registerVault);

    dialog->show();
}

void LingmoVaultService::slotRegistered(const QDBusObjectPath &path)
{
    if (path.path() == QLatin1String("/modules/lingmovault")) {
        Q_EMIT registered();
    }
}

void LingmoVaultService::registerVault(Vault *vault)
{
    if (!vault->isValid()) {
    ...
    connect(vault, &Vault::statusChanged, this, &LingmoVaultService::onVaultStatusChanged);
    connect(vault, &Vault::messageChanged, this, &LingmoVaultService::onVaultMessageChanged);
    connect(vault, &Vault::infoChanged, this, &LingmoVaultService::onVaultInfoChanged);
...
void LingmoVaultService::forgetVault(Vault *vault)
{
    // Can not be open
...
void LingmoVaultService::onVaultStatusChanged(VaultInfo::Status status)
{
...
void LingmoVaultService::onVaultInfoChanged()
{
    const auto vault = static_cast<Vault *>(sender());
    Q_EMIT vaultChanged(vault->info());
}

void LingmoVaultService::onVaultMessageChanged(const QString &message)
{
    Q_UNUSED(message);
    const auto vault = static_cast<Vault *>(sender());
    Q_EMIT vaultChanged(vault->info());
...
void LingmoVaultService::openVault(const QString &device)
{
...
void LingmoVaultService::closeVault(const QString &device)
{
    if (auto vault = d->vaultFor(device)) {
        if (!vault->isOpened())
            return;
...
void LingmoVaultService::configureVault(const QString &device)
{
    if (auto vault = d->vaultFor(device)) {
...
void LingmoVaultService::forceCloseVault(const QString &device)
{
    if (auto vault = d->vaultFor(device)) {
        if (!vault->isOpened())
            return;
...
void LingmoVaultService::openVaultInFileManager(const QString &device)
{
...
bool LingmoVaultService::hasOpenVaults() const
{
    return !d->openVaults.isEmpty();
}

void LingmoVaultService::closeAllVaults()
{
...
void LingmoVaultService::forceCloseAllVaults()
{
    for (const auto &device : d->openVaults) {
...
void LingmoVaultService::deleteVault(const QString &device, const QString &name)
{
...
void LingmoVaultService::updateStatus()
{
...
void LingmoVaultService::onActivitiesChanged(const QStringList &knownActivities)
{
...
void LingmoVaultService::onCurrentActivityChanged(const QString &currentActivity)
{
    for (auto *vault : d->knownVaults.values()) {
...
void LingmoVaultService::onActivityRemoved(const QString &removedActivity)
{
    for (auto *vault : d->knownVaults.values()) {
        auto vaultActivities = vault->activities();
