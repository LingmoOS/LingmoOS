...
VaultsModel::Private::Private(VaultsModel *parent)
    : service("org.kde.kded6", "/modules/lingmovault", QDBusConnection::sessionBus())
    , serviceWatcher("org.kde.kded6", QDBusConnection::sessionBus())
    , q(parent)
{
    connect(&service, &org::kde::lingmovault::vaultAdded, this, &Private::onVaultAdded);
    connect(&service, &org::kde::lingmovault::vaultChanged, this, &Private::onVaultChanged);
    connect(&service, &org::kde::lingmovault::vaultRemoved, this, &Private::onVaultRemoved);
...
void VaultsModel::Private::onVaultAdded(const LingmoVault::VaultInfo &vaultInfo)
{
...
void VaultsModel::Private::onVaultChanged(const LingmoVault::VaultInfo &vaultInfo)
{
    const auto device = vaultInfo.device;
