    ...
    , m_lingmoshell(nullptr)
{
    ...
    connect(this, &Klipper::passivePopup, this, [this](const QString &caption, const QString &text) {
        if (m_notification) {
            ...
            // When Klipper is run as part of lingmo, we still need to pretend to be it for notification settings to work
    ...
    if (KWindowSystem::isPlatformWayland()) {
        ...
        connect(registry, &KWayland::Client::Registry::lingmoShellAnnounced, this, [registry, this](quint32 name, quint32 version) {
            if (!m_lingmoshell) {
                m_lingmoshell = registry->createLingmoShell(name, version);
            }
        });
...
bool Klipper::eventFilter(QObject *filtered, QEvent *event)
{
    ...
    if (menuWindow && event->type() == QEvent::Expose && menuWindow->isVisible()) {
        ...
        auto lingmoSurface = m_lingmoshell->createSurface(surface, menuWindow);
        lingmoSurface->openUnderCursor();
        lingmoSurface->setSkipTaskbar(true);
        lingmoSurface->setSkipSwitcher(true);
        lingmoSurface->setRole(KWayland::Client::LingmoShellSurface::Role::AppletPopup);
