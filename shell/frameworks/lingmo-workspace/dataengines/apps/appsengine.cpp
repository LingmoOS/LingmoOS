...
Lingmo5Support::Service *AppsEngine::serviceForSource(const QString &name)
{
    AppSource *source = dynamic_cast<AppSource *>(containerForSource(name));
    // if source does not exist, return null service
    if (!source) {
        return Lingmo5Support::DataEngine::serviceForSource(name);
    }
    ...
    Lingmo5Support::Service *service = source->createService();
...
K_PLUGIN_CLASS_WITH_JSON(AppsEngine, "lingmo-dataengine-apps.json")
