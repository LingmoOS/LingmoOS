...
public:
        ...
        : q(q)
    {
        ...
        m_nam.enableStrictTransportSecurityStore(true,
                                                 QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) + QLatin1String("/lingmoshell/hsts/"));
    }
    ...
    Lingmo5Support::DataEngine::Data m_data;
...
static QJsonArray accessPoints()
{
    ...
    const KConfigGroup config = KSharedConfig::openConfig()->group(QStringLiteral("org.kde.lingmo.geolocation.ip"));
...
void Ip::update()
{
    ...
    if (!NetworkManager::isNetworkingEnabled()) {
        setData(Lingmo5Support::DataEngine::Data());
        return;
    }
