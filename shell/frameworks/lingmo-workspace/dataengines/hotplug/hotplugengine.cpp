...
Lingmo5Support::Service *HotplugEngine::serviceForSource(const QString &source)
{
    ...
    while (it.hasNext()) {
        ...
        if (!predicates.isEmpty()) {
            if (sources().contains(udi)) {
                Lingmo5Support::DataEngine::Data data;
...
QVariantList HotplugEngine::actionsForPredicates(const QStringList &predicates) const
{
    ...
    for (const QString &desktop : predicates) {
        ...
        if (!services.isEmpty()) {
            Lingmo5Support::DataEngine::Data action;
...
void HotplugEngine::handleDeviceAdded(Solid::Device &device, bool added)
{
    ...
    if (!interestingDesktopFiles.isEmpty() || isEncryptedContainer) {
        ...
        if (!device.description().isEmpty()) {
            ...
            data.insert(QStringLiteral("text"), QString(device.vendor() + u' ' + device.product()));
        }
...
K_PLUGIN_CLASS_WITH_JSON(HotplugEngine, "lingmo-dataengine-hotplug.json")
