...
InteractiveConsole::InteractiveConsole(ConsoleMode mode, QWidget *parent)
    ...
    , m_lingmoAction(new QAction(QIcon::fromTheme(QStringLiteral("lingmo")), i18nc("Toolbar Button to switch to Lingmo Scripting Mode", "Lingmo"), this))
    ...
    , m_mode(mode)
{
    ...
    modeGroup->addAction(m_lingmoAction);
    modeGroup->addAction(m_kwinAction);
    m_lingmoAction->setCheckable(true);
    m_kwinAction->setCheckable(true);
    m_kwinAction->setChecked(mode == KWinConsole);
    m_lingmoAction->setChecked(mode == LingmoConsole);
    ...
    toolBar->addAction(m_lingmoAction);
    ...
    auto tryLoadingKatePart = [=, this]() -> KTextEditor::Document * {
        const auto loadResult = KPluginFactory::instantiatePlugin<KTextEditor::Document>(KPluginMetaData(QStringLiteral("kf6/parts/katepart")), this);
        if (!loadResult) {
        ...
        result->setHighlightingMode(QStringLiteral("JavaScript/LingmoDesktop"));
...
{
    if (m_lingmoAction->isChecked()) {
        m_mode = LingmoConsole;
...
void InteractiveConsole::loadScript(const QString &script)
{
    if (m_editorPart) {
        ...
        if (m_editorPart->openUrl(QUrl::fromLocalFile(script))) {
            m_editorPart->setHighlightingMode(QStringLiteral("JavaScript/LingmoDesktop"));
            return;
        }
    } else {
...
void InteractiveConsole::populateTemplatesMenu()
{
    ...
    auto templates = KPackage::PackageLoader::self()->findPackages(QStringLiteral("Lingmo/LayoutTemplate"), QString(), [](const KPluginMetaData &metaData) {
        return metaData.value(QStringLiteral("X-Lingmo-Shell")) == qApp->applicationName();
    });
...
{
    KPackage::Package package = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/LayoutTemplate"), action->data().toString());
    const QString scriptFile = package.filePath("mainscript");
    if (!scriptFile.isEmpty()) {
...
void InteractiveConsole::evaluateScript()
{
    ...
    if (m_mode == LingmoConsole) {
        QDBusMessage message = QDBusMessage::createMethodCall(s_lingmoShellService,
