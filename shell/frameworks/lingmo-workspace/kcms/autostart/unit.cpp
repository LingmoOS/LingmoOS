...
QStringList Unit::getLastJournalEntries(const QString &unit)
{
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        if (returnValue == 0) {
            ...
            // Adding color to the logs is done in c++ so we can't add LingmoUI color here so we add a string here
            //  and then replace it all with actual colors in qml code
            if (prio <= 3)
                line.append(u"<font color='LingmoUI.Theme.negativeTextColor'>");
            else if (prio == 4)
                line.append(u"<font color='LingmoUI.Theme.neutralTextColor'>");
            else
                line.append(u"<font>");
        }
