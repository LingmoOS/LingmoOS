...
// Then we have Lingmo-specific locations which run scripts
// .config/autostart-scripts which has scripts executed by lingmo_session (now migrated to .desktop files)
// .config/lingmo-workspace/shutdown which has scripts executed by lingmo-shutdown
// .config/lingmo-workspace/env which has scripts executed by startlingmo
...
std::optional<AutostartEntry> AutostartModel::loadDesktopEntry(const QString &fileName)
{
    ...
    const bool onlyInLingmo = lstEntry.contains(QLatin1String("KDE"));
    ...
    if (kind == XdgScripts) {
        ...
        return AutostartEntry{targetFileName, targetScriptDir, kind, enabled, fileName, onlyInLingmo, iconName};
    }
    return AutostartEntry{name, name, kind, enabled, fileName, onlyInLingmo, iconName};
}
...
void AutostartModel::load()
{
    ...
    loadScriptsFromDir(QStringLiteral("lingmo-workspace/env/"), AutostartModel::AutostartEntrySource::LingmoEnvScripts);

    loadScriptsFromDir(QStringLiteral("lingmo-workspace/shutdown/"), AutostartModel::AutostartEntrySource::LingmoShutdown);

    // Add unit objects for entries and set id to them
    for (AutostartEntry &entry : m_entries) {
        if (entry.source == AutostartModel::AutostartEntrySource::LingmoShutdown || entry.source == AutostartModel::AutostartEntrySource::LingmoEnvScripts) {
            continue;
        }
...
void AutostartModel::addScript(const QUrl &url, AutostartModel::AutostartEntrySource kind)
{
    ...
    if (kind == AutostartModel::AutostartEntrySource::XdgScripts) {
        ...
        for (const AutostartEntry &e : std::as_const(m_entries)) {
            if (e.source == AutostartModel::AutostartEntrySource::LingmoShutdown) {
                break;
            }
            ++lastLoginScript;
        }
    ...
    } else if (kind == AutostartModel::AutostartEntrySource::LingmoShutdown) {
        const QUrl destinationScript = QUrl::fromLocalFile(QDir(m_xdgConfigPath.filePath(QStringLiteral("lingmo-workspace/shutdown/"))).filePath(fileName));
...
void AutostartModel::insertScriptEntry(int index, const QString &name, const QString &targetFileDirPath, const QString &path, AutostartEntrySource kind)
{
    ...
    // Lingmo shutdown and Lingmo env scripts don't have units
    if (kind == AutostartModel::AutostartEntrySource::LingmoShutdown || kind == AutostartModel::AutostartEntrySource::LingmoEnvScripts) {
        delete unit;
        unit = nullptr;
    }
...
QHash<int, QByteArray> AutostartModel::roleNames() const
{
    ...
    roleNames.insert(OnlyInLingmo, QByteArrayLiteral("onlyInLingmo"));
