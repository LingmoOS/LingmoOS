    ...
    , m_haveThemeExplorerInstalled(false)
{
    ...
    m_haveThemeExplorerInstalled = !QStandardPaths::findExecutable(QStringLiteral("lingmothemeexplorer")).isEmpty();
...
void KCMDesktopTheme::installTheme(const QString &path)
{
    ...
    const QStringList arguments = {QStringLiteral("--type"), QStringLiteral("Lingmo/Theme"), QStringLiteral("--install"), path};
...
void KCMDesktopTheme::applyLingmoTheme(QQuickItem *item, const QString &themeName)
{
    if (!item) {
        return;
    }
    ...
    if (!imageSet) {
        imageSet = new KSvg::ImageSet(themeName, QStringLiteral("lingmo/desktoptheme"), this);
...
void KCMDesktopTheme::save()
{
    ...
    // Lingmo theme changes are known to be slow, so make the blend take a while
    msg << 1000;
...
{
    QProcess::startDetached(QStringLiteral("lingmothemeexplorer"), {QStringLiteral("-t"), theme});
}
...
void KCMDesktopTheme::processPendingDeletions()
{
    ...
    for (const QPersistentModelIndex &idx : persistentPendingDeletions) {
        ...
        const QString location = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1String("/lingmo/desktoptheme");
        auto *job = KPackage::PackageJob::uninstall(QStringLiteral("Lingmo/Theme"), pluginName, location);
