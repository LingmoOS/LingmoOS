...
std::unordered_map<QString, QString> KCMRegionAndLang::constructGlibcLocaleMap()
{
    ...
    const auto addToMap = [&localeMap](const QString &lingmoLocale, const QString &glibcLocale) {
        // We map the locale name and the lingmo name to a valid locale. This gives us flexibility for resolution
        ...
        localeMap.insert({lingmoLocale, toUTF8Locale(glibcLocale)});
    };

    auto lingmoLocales = KLocalizedString::availableDomainTranslations(QByteArrayLiteral("lingmoshell")).values();
    for (const auto &lingmoLocale : lingmoLocales) {
        auto baseLocale = lingmoLocale.split(u'_')[0].split(u'@')[0];
        if (baseLocaleMap.contains(baseLocale)) {
            const auto &prefixedLocales = baseLocaleMap[baseLocale];

            // if we have one to one match, use that. Eg. en_US to en_US
            auto fullMatch = std::find(prefixedLocales.begin(), prefixedLocales.end(), lingmoLocale);
            if (fullMatch != prefixedLocales.end()) {
                addToMap(lingmoLocale, *fullMatch);
                continue;
            }

            // language name with same country code has higher priority, eg. es_ES > es_PA, de_DE > de_DE@euro
            auto mainLocale = lingmoLocale + u'_' + lingmoLocale.toUpper();
            ...
            addToMap(lingmoLocale, prefixedLocales[closestMatchIndex]);
        }
    }
    return localeMap;
}
