...
int main(int argc, char **argv)
{
    ...
    QCoreApplication::setApplicationName(QStringLiteral("lingmo-apply-cursortheme"));
    QCoreApplication::setApplicationVersion(QStringLiteral("1.0"));
    QCoreApplication::setOrganizationDomain(QStringLiteral("kde.org"));
    KLocalizedString::setApplicationDomain(QByteArrayLiteral("lingmo-apply-cursortheme"));

    QCommandLineParser *parser = new QCommandLineParser;
    parser->addHelpOption();
    parser->setApplicationDescription(
        i18n("This tool allows you to set the mouse cursor theme for the current Lingmo session, without accidentally setting it to one that is either not "
             "available, or which is already set."));
    parser->addPositionalArgument(
        QStringLiteral("cursortheme"),
        i18n("The name of the cursor theme you wish to set for your current Lingmo session (passing a full path will only use the last part of the path)"));
    ...
    if (!parser->positionalArguments().isEmpty()) {
        ...
        if (settings->cursorTheme() == requestedTheme) {
            ...
            if (theme) {
                ...
                if (settings->save() && applyThemeAndSize(theme, parser->value(u"size"_s), ts)) {
                    ...
                    ts << i18n("Successfully applied the mouse cursor theme %1 to your current Lingmo session", theme->title()) << Qt::endl;
                } else {
                    ts << i18n("You have to restart the Lingmo session for your newly applied mouse cursor theme to display correctly.") << Qt::endl;
        ...
        for (int i = 0; i < model->rowCount(); ++i) {
            ...
            if (settings->cursorTheme() == theme->name()) {
                ts << QLatin1Char(' ') << i18n("(Current theme for this Lingmo session)");
            }
            ts << Qt::endl;
        }
    } else {
