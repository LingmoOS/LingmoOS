...
int main(int argc, char **argv)
{
    ...
    KLocalizedString::setApplicationDomain(QByteArrayLiteral("lingmo-apply-colorscheme"));

    QCommandLineParser *parser = new QCommandLineParser;
    parser->addHelpOption();
    parser->setApplicationDescription(
        i18n("This tool allows you to set the color scheme for the current Lingmo session, without accidentally setting it to one that is either not "
             "available, or which is already set."));
    parser->addPositionalArgument(
        QStringLiteral("colorscheme"),
        i18n("The name of the color scheme you wish to set for your current Lingmo session (passing a full path will only use the last part of the path)"));
    ...
    if (!parser->positionalArguments().isEmpty() && !parser->isSet(QStringLiteral("accent-color"))) {
        ...
        if (exitCode == 0) {
            if (settings->colorScheme() == requestedScheme) {
                ts << i18n("The requested theme \"%1\" is already set as the theme for the current Lingmo session.", requestedScheme) << Qt::endl;
                ...
                if (newSchemeIndex > -1) {
                    ...
                    ts << i18n("Successfully applied the color scheme %1 to your current Lingmo session", requestedScheme) << Qt::endl;
                } else {
