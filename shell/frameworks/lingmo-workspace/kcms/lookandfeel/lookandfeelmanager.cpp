...
#include "../../startkde/lingmoautostart/lingmoautostart.h"
...
LookAndFeelManager::Contents LookAndFeelManager::packageContents(const KPackage::Package &pkg) const
{
    ...
    if (!pkg.filePath("defaults").isEmpty()) {
        ...
        contents.setFlag(LingmoTheme, configProvides(conf, u"lingmorc/Theme"_s, u"name"_s));
        ...
        contents.setFlag(ShellPackage, configProvides(conf, u"lingmoshellrc/Shell"_s, u"ShellPackage"_s));
...
void LookAndFeelManager::setShellPackage(const QString &value)
{
    ...
    writeNewDefaults(QStringLiteral("lingmoshellrc"), QStringLiteral("Shell"), QStringLiteral("ShellPackage"), value);
    m_lingmoshellChanged = true;
}
...
void LookAndFeelManager::setLingmoTheme(const QString &theme)
{
    if (theme.isEmpty()) {
        return;
    }

    writeNewDefaults(QStringLiteral("lingmorc"), QStringLiteral("Theme"), QStringLiteral("name"), theme);
}
...
void LookAndFeelManager::save(const KPackage::Package &package, const KPackage::Package &previousPackage, Contents applyMask)
{
    ...
    if (itemsToApply.testFlag(DesktopLayout) && m_mode == Mode::Apply) {
        QDBusMessage message = QDBusMessage::createMethodCall(QStringLiteral("org.kde.lingmoshell"),
    ...
    if (!package.filePath("defaults").isEmpty()) {
        ...
        if (itemsToApply.testFlag(WidgetStyle)) {
            ...
            // Some global themes refer to ocean's widgetStyle with a lowercase b.
            if (widgetStyle == QStringLiteral("ocean")) {
                widgetStyle = QStringLiteral("Ocean");
            }
        ...
        if (itemsToApply.testFlag(LingmoTheme)) {
            group = KConfigGroup(conf, u"lingmorc"_s);
            group = KConfigGroup(&group, u"Theme"_s);
            setLingmoTheme(group.readEntry("name", QString()));
        }
        ...
        if (itemsToApply.testFlag(ShellPackage)) {
            group = KConfigGroup(conf, u"lingmoshellrc"_s);
        ...
        if (itemsToApply.testFlag(WindowDecoration)) {
            ...
            setWindowDecoration(group.readEntry("library", QStringLiteral(OCEAN_KDECORATION_PLUGIN_ID)),
                                group.readEntry("theme", QStringLiteral("Ocean")),
        ...
        if (m_lingmoshellChanged) {
            QDBusMessage message =
                QDBusMessage::createSignal(QStringLiteral("/LingmoShell"), QStringLiteral("org.kde.LingmoShell"), QStringLiteral("refreshCurrentShell"));
            ...
            // remove all the old package to autostart
            {
                ...
                if (qEnvironmentVariableIsSet("KDE_FULL_SESSION")) {
                    for (const QString &serviceFile : autostartServices) {
                        ...
                        LingmoAutostart as(serviceFile);
...
bool LookAndFeelManager::remove(const KPackage::Package &package, LookAndFeelManager::Contents contentsMask)
{
    ...
    if (itemsToRemove.testFlag(LingmoTheme) && itemsToRemove.testFlag(Icons)) {
        const QString themeName = configValue(config, u"lingmorc/Theme"_s, u"name"_s);
        QDir themeDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QDir::separator() + u"lingmo" + QDir::separator()
...
{
    // TODO: use pieces of cursor kcm when moved to lingmo-desktop
