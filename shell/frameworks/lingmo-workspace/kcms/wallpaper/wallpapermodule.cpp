...
WallpaperModule::WallpaperModule(QObject *parent, const KPluginMetaData &data)
    ...
    , m_config(KSharedConfig::openConfig(QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + u"/lingmo-org.kde.lingmo.desktop-appletsrc",
    ...
    , m_activityConsumer(new KActivities::Consumer(this))
{
    ...
    constexpr const char *uri = "org.kde.lingmo.kcm.wallpaper";

    qmlRegisterAnonymousType<QScreen>(uri, 1);
    qmlRegisterAnonymousType<KConfigPropertyMap>(uri, 1);
    // Only register types once
    [[maybe_unused]] static int configModelRegisterResult = qmlRegisterType<LingmoQuick::ConfigModel>("org.kde.lingmo.configuration", 2, 0, "ConfigModel");
    ...
    if (!connected) {
        qCFatal(KCM_WALLPAPER_DEBUG) << "Could not connect to dbus service org.kde.lingmoshell";
    }
...
void WallpaperModule::save()
{
    ...
    auto iface = new QDBusInterface(u"org.kde.lingmoshell"_s, u"/LingmoShell"_s, u"org.kde.LingmoShell"_s, QDBusConnection::sessionBus(), this);
    if (!iface->isValid()) {
...
QString WallpaperModule::wallpaperPluginSource()
{
    ...
    for (int i = 0; i < wallpaperPluginCount; ++i) {
        if (model->data(model->index(i), LingmoQuick::ConfigModel::PluginNameRole) == m_currentWallpaperPlugin) {
...
LingmoQuick::ConfigModel *WallpaperModule::wallpaperConfigModel()
{
