...
QList<QAction *> WallpaperFileItemAction::actions(const KFileItemListProperties &fileItemInfos, QWidget *parentWidget)
{
    ...
    auto menu = new QMenu(i18ndc("lingmo_wallpaper_org.kde.image", "@action:inmenu", "Set as Wallpaper"));
    menu->setIcon(QIcon::fromTheme(QStringLiteral("viewimage")));

    auto desktopAction = new QAction(i18ndc("lingmo_wallpaper_org.kde.image", "@action:inmenu Set as Desktop Wallpaper", "Desktop"));
    connect(desktopAction, &QAction::triggered, this, [this, filePath] {
        setAsDesktopBackground(filePath);
    });
    menu->addAction(desktopAction);

    auto lockAction = new QAction(i18ndc("lingmo_wallpaper_org.kde.image", "@action:inmenu Set as Lockscreen Wallpaper", "Lockscreen"));
    connect(lockAction, &QAction::triggered, this, [this, filePath] {
        setAsLockscreenBackground(filePath);
    });
    menu->addAction(lockAction);

    auto bothAction = new QAction(i18ndc("lingmo_wallpaper_org.kde.image", "@action:inmenu Set as both lockscreen and Desktop Wallpaper", "Both"));
...
void WallpaperFileItemAction::setAsDesktopBackground(const QString &file)
{
    ...
    auto message = QDBusMessage::createMethodCall(u"org.kde.lingmoshell"_s, u"/LingmoShell"_s, u"org.kde.LingmoShell"_s, u"evaluateScript"_s);
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, watcher]() {
        ...
        if (reply.isError()) {
            auto errorMessage = QString(xi18ndc("lingmo_wallpaper_org.kde.image",
                                                "@info %1 is the dbus error message",
                                                "An error occurred while attempting to set the Lingmo wallpaper:<nl/>%1"))
...
void WallpaperFileItemAction::setAsLockscreenBackground(const QString &file)
{
    ...
    if (screenLockerConfig->accessMode() != KConfig::ReadWrite) {
        auto errorMessage = QString(i18nd("lingmo_wallpaper_org.kde.image", "An error occurred while attempting to open kscreenlockerrc config file."));
        qWarning() << errorMessage;
        error(errorMessage);
        return;
    }
