...
#include <LingmoActivities/Stats/Query>
#include <LingmoActivities/Stats/ResultSet>
#include <LingmoActivities/Stats/ResultWatcher>
...
#define AGENT_APPLICATIONS QStringLiteral("org.kde.lingmo.favorites.applications")
#define AGENT_DOCUMENTS QStringLiteral("org.kde.lingmo.favorites.documents")
        ...
        if (ordering.length() == 0) {
            ...
            for (const auto &groupName : allGroups) {
                if (groupName.contains(QStringLiteral(".favorites.instance-"))
                    ...
                    // the group names look like "Favorites-org.kde.lingmo.kicker.favorites.instance-58-1bd5bb42-187c-4c77-a746-c9644c5da866"
...
void KAStatsFavoritesModel::portOldFavorites(const QStringList &_ids)
{
    ...
    if (qEnvironmentVariableIsSet("KDECI_BUILD")) {
        prepend = QStringList{
            QLatin1String("org.kde.lingmo.emojier.desktop"),
...
QString KAStatsFavoritesModel::activityNameForId(const QString &activityId) const
{
    ...
    // we are always synced with KAMD in lingmo
