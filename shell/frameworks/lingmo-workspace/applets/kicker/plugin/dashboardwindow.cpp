    ...
    , m_visualParentWindow(nullptr)
{
    ...
    setIcon(QIcon::fromTheme(QStringLiteral("lingmo")));

    connect(&m_theme, &Lingmo::Theme::themeChanged, this, &DashboardWindow::updateTheme);
        ...
        if (pSEvent->surfaceEventType() == QPlatformSurfaceEvent::SurfaceCreated) {
            if (KWindowSystem::isPlatformX11()) {
                ...
                if (m_lingmoshell) {
                    ...
                    auto *lingmoshellSurface = KWayland::Client::LingmoShellSurface::get(surface);

                    if (!lingmoshellSurface) {
                        lingmoshellSurface = m_lingmoshell->createSurface(surface, this);
                    }
