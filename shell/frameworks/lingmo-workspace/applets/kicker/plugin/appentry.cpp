...
QVariantList AppEntry::actions() const
{
    ...
    if (appletInterface) {
        systemImmutable = (appletInterface->property("immutability").toInt() == Lingmo::Types::SystemImmutable);
    }
...
bool AppEntry::run(const QString &actionId, const QVariant &argument)
{
    ...
    if (actionId.isEmpty()) {
        ...
        KActivities::ResourceInstance::notifyAccessed(QUrl(QString(u"applications:" + m_service->storageId())), QStringLiteral("org.kde.lingmo.kicker"));
