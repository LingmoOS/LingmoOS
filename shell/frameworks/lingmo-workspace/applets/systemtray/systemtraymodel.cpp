...
Lingmo::Types::ItemStatus BaseModel::calculateEffectiveStatus(bool canRender, Lingmo::Types::ItemStatus status, QString itemId) const
{
    if (!canRender) {
        return Lingmo::Types::ItemStatus::HiddenStatus;
    }

    bool forcedShown = m_showAllItems || m_shownItems.contains(itemId);
    bool forcedHidden = m_hiddenItems.contains(itemId);

    if (!forcedShown && status == Lingmo::Types::ItemStatus::HiddenStatus) {
        return Lingmo::Types::ItemStatus::HiddenStatus;
    } else if (forcedShown || (!forcedHidden && status != Lingmo::Types::ItemStatus::PassiveStatus)) {
        return Lingmo::Types::ItemStatus::ActiveStatus;
    } else {
        return Lingmo::Types::ItemStatus::PassiveStatus;
    }
}
...
static QString plasmoidCategoryForMetadata(const KPluginMetaData &metadata)
{
    ...
    return metadata.value(QStringLiteral("X-Lingmo-NotificationAreaCategory"));
}
    ...
    , m_plasmoidRegistry(plasmoidRegistry)
{
    ...
    for (const auto &info : appletMetaDataList) {
        if (!info.isValid() || info.value(QStringLiteral("X-Lingmo-NotificationAreaCategory")).isEmpty()) {
            continue;
        }
        appendRow(info);
    }
...
QVariant PlasmoidModel::data(const QModelIndex &index, int role) const
{
    ...
    Lingmo::Applet *applet = item.applet;
    ...
    if (role < static_cast<int>(Role::Applet)) {
        Lingmo::Types::ItemStatus status = Lingmo::Types::ItemStatus::UnknownStatus;
        if (applet) {
    ...
    case Role::Applet:
        return applet ? QVariant::fromValue(LingmoQuick::AppletQuickItem::itemForApplet(applet)) : QVariant();
    case Role::HasApplet:
        return applet != nullptr;
    default:
        return QVariant();
...
void PlasmoidModel::addApplet(Lingmo::Applet *applet)
{
    ...
    connect(applet, &Lingmo::Applet::statusChanged, this, [this, applet](Lingmo::Types::ItemStatus status) {
        Q_UNUSED(status)
...
void PlasmoidModel::removeApplet(Lingmo::Applet *applet)
{
    int idx = indexOfPluginId(applet->pluginMetaData().pluginId());
    if (idx >= 0) {
...
static Lingmo::Types::ItemStatus extractStatus(const StatusNotifierItemSource *sniData)
{
    QString status = sniData->status();
    if (status == QLatin1String("Active")) {
        return Lingmo::Types::ItemStatus::ActiveStatus;
    } else if (status == QLatin1String("NeedsAttention")) {
        return Lingmo::Types::ItemStatus::NeedsAttentionStatus;
    } else if (status == QLatin1String("Passive")) {
        return Lingmo::Types::ItemStatus::PassiveStatus;
    } else {
        return Lingmo::Types::ItemStatus::UnknownStatus;
    }
}
