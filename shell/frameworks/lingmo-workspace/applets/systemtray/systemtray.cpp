...
SystemTray::SystemTray(QObject *parent, const KPluginMetaData &data, const QVariantList &args)
    : Lingmo::Containment(parent, data, args)
{
    setHasConfigurationInterface(true);
    setContainmentDisplayHints(Lingmo::Types::ContainmentDrawsPlasmoidHeading | Lingmo::Types::ContainmentForcesSquarePlasmoids);
}
...
void SystemTray::init()
{
    ...
    connect(this, &Containment::appletAdded, this, [this](Lingmo::Applet *applet) {
...
void SystemTray::showStatusNotifierContextMenu(KJob *job, QQuickItem *statusNotifierIcon)
{
    ...
    Lingmo5Support::ServiceJob *sjob = qobject_cast<Lingmo5Support::ServiceJob *>(job);
    if (!sjob) {
        return;
    }
...
void SystemTray::onEnabledAppletsChanged()
{
    ...
    for (Lingmo::Applet *applet : appletsList) {
...
void SystemTray::startApplet(const QString &pluginId)
{
    ...
    if (m_configGroupIds.contains(pluginId)) {
        Applet *applet = Lingmo::PluginLoader::self()->loadApplet(pluginId, m_configGroupIds.value(pluginId), QVariantList());
        ...
        applet->setProperty("org.kde.lingmo:force-create", true);
        addApplet(applet);
        // create a new one automatic id, new config group
    } else {
        Applet *applet = createApplet(pluginId, QVariantList() << u"org.kde.lingmo:force-create"_s);
        if (applet) {
