...
void PlasmoidRegistry::init()
{
    QDBusConnection::sessionBus().connect(QString(),
                                          QStringLiteral("/KPackage/Lingmo/Applet"),
    ...
    for (const auto &info : Lingmo::PluginLoader::self()->listAppletMetaData(QString())) {
        registerPlugin(info);
    }
...
{
    if (!pluginMetaData.isValid() || pluginMetaData.value(QStringLiteral("X-Lingmo-NotificationAreaCategory")).isEmpty()) {
        return;
    }
