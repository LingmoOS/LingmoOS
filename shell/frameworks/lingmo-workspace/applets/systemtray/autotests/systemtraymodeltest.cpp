...
static const QString DEVICENOTIFIER_ID = QStringLiteral("org.kde.lingmo.devicenotifier.test");
static const QString MEDIACONROLLER_ID = QStringLiteral("org.kde.lingmo.mediacontroller.test");
...
void SystemTrayModelTest::testPlasmoidModel()
{
    ...
    QCOMPARE(model->data(idx, static_cast<int>(BaseModel::BaseRole::Status)), QVariant(Lingmo::Types::ItemStatus::UnknownStatus));
    QCOMPARE(model->data(idx, static_cast<int>(BaseModel::BaseRole::EffectiveStatus)), QVariant(Lingmo::Types::ItemStatus::HiddenStatus));
    ...
    // when: applet added
    model->addApplet(new Lingmo::Applet(nullptr, plasmoidRegistry->m_systemTrayApplets.value(MEDIACONROLLER_ID), QVariantList{}));
    // then: applet can be rendered
    QVERIFY(model->data(idx, static_cast<int>(BaseModel::BaseRole::CanRender)).toBool());
    QVERIFY(model->data(idx, static_cast<int>(PlasmoidModel::Role::HasApplet)).toBool());
    QCOMPARE(model->data(idx, static_cast<int>(BaseModel::BaseRole::EffectiveStatus)), QVariant(Lingmo::Types::ItemStatus::ActiveStatus));

    // and when: applet removed
    model->removeApplet(new Lingmo::Applet(nullptr, plasmoidRegistry->m_systemTrayApplets.value(MEDIACONROLLER_ID), QVariantList{}));
