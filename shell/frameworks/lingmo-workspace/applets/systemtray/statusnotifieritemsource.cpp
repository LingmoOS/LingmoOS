...
Q_GLOBAL_STATIC(Lingmo::Theme, s_theme)

class LingmoDBusMenuImporter : public DBusMenuImporter
{
public:
    LingmoDBusMenuImporter(const QString &service, const QString &path, KIconLoader *iconLoader, QObject *parent)
    ...
    , m_needsReRefreshing(false)
{
    ...
    connect(s_theme, &Lingmo::Theme::themeChanged, this, &StatusNotifierItemSource::reloadIcon);
}
...
Lingmo5Support::Service *StatusNotifierItemSource::createService()
{
...
void StatusNotifierItemSource::refreshCallback(QDBusPendingCallWatcher *call)
{
    ...
    if (reply.isError()) {
        ...
        // Overlay icon
        {
            ...
            if (!iconName.isEmpty()) {
                overlay = QIcon(new KIconEngine(iconName, KIconColors(Lingmo::Theme::globalPalette()), iconLoader()));
                if (!overlay.isNull()) {
        ...
        auto loadIcon = [this, &properties, &overlay](const QString &iconKey, const QString &pixmapKey) -> std::tuple<QIcon, QString> {
            ...
            if (!iconName.isEmpty()) {
                QIcon icon = QIcon(new KIconEngine(iconName, KIconColors(Lingmo::Theme::globalPalette()), iconLoader(), {m_overlayIconName}));
                if (!icon.isNull()) {
        ...
        if (qobject_cast<QApplication *>(QCoreApplication::instance()) && !m_menuImporter) {
            ...
            if (!menuObjectPath.isEmpty()) {
                if (menuObjectPath == QLatin1String("/NO_DBUSMENU")) {
                    ...
                    m_menuImporter = new LingmoDBusMenuImporter(m_statusNotifierItemInterface->service(), menuObjectPath, iconLoader(), this);
                    connect(m_menuImporter, &LingmoDBusMenuImporter::menuUpdated, this, [this](QMenu *menu) {
...
void StatusNotifierItemSource::reloadIcon()
{
    if (!m_iconName.isEmpty()) {
        m_icon = QIcon(new KIconEngine(m_iconName, KIconColors(Lingmo::Theme::globalPalette()), iconLoader(), {m_overlayIconName}));
    }

    if (!m_attentionIconName.isEmpty()) {
        m_attentionIcon = QIcon(new KIconEngine(m_attentionIconName, KIconColors(Lingmo::Theme::globalPalette()), iconLoader(), {m_overlayIconName}));
    }
