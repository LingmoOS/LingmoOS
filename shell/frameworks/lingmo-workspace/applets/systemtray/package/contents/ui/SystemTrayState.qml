...
QtObject {
    ...
    //this is to suppress expanded state change during Lingmo startup
    ...
    function setActiveApplet(applet, visualIndex) {
        if (visualIndex === undefined) {
            ...
            oldVisualIndex = (activeApplet && activeApplet.status === LingmoCore.Types.PassiveStatus) ? 9999 : newVisualIndex
    ...
    onExpandedChanged: {
        if (expanded) {
            ...
            Plasmoid.status = LingmoCore.Types.PassiveStatus;
            if (activeApplet) {
