...
QtObject {
    ...
    property int popupLocation: {
        ...
        case NotificationManager.Settings.CloseToWidget:
            ...
            } else if (plasmoid.location === LingmoCore.Types.BottomEdge) {
    ...
    readonly property rect screenRect: {
        ...
        if (visualParent && notificationSettings.popupPosition === NotificationManager.Settings.CloseToWidget
            && plasmoid.formFactor === LingmoCore.Types.Horizontal) {
    ...
    readonly property QtObject focusDialog: plasmoid ? plasmoid.focussedLingmoDialog : null
    onFocusDialogChanged: {
        if (focusDialog && !(focusDialog instanceof NotificationPopup)) {
            // keep around the last focusDialog so notifications don't jump around if there is an open but unfocused (eg pinned) Lingmo dialog
    ...
    property int popupWidth: (popupLocation & Qt.AlignHCenter) && !LingmoUI.Settings.isMobile ? LingmoUI.Units.gridUnit * 22 : LingmoUI.Units.gridUnit * 18
    property int popupEdgeDistance: LingmoUI.Units.gridUnit * 2
    // Reduce spacing between popups when centered so the stack doesn't intrude into the
    // view as much
    property int popupSpacing: (popupLocation & Qt.AlignHCenter) && !LingmoUI.Settings.isMobile ? LingmoUI.Units.smallSpacing : LingmoUI.Units.gridUnit
    ...
    property QtObject popupNotificationsModel: NotificationManager.Notifications {
        limit: plasmoid ? (Math.ceil(globals.screenRect.height / (LingmoUI.Units.gridUnit * 4))) : 0
        showExpired: false
        showDismissed: false
    ...
    property Instantiator popupInstantiator: Instantiator {
        ...
        delegate: NotificationPopup {
            ...
            type: model.urgency === NotificationManager.Notifications.CriticalUrgency
                  ...
                  ? LingmoCore.Dialog.CriticalNotification : LingmoCore.Dialog.Notification
