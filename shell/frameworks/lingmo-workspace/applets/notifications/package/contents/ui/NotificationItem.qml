...
ColumnLayout {
                ...
                if (notificationItem.notificationType === NotificationManager.Notifications.JobType) {
                    if (notificationItem.jobState === NotificationManager.Notifications.JobStateSuspended) {
                        if (notificationItem.summary) {
                            return i18ndc("lingmo_applet_org.kde.lingmo.notifications", "Job name, e.g. Copying is paused", "%1 (Paused)", notificationItem.summary);
                        }
                    } else if (notificationItem.jobState === NotificationManager.Notifications.JobStateStopped) {
                        if (notificationItem.jobError) {
                            if (notificationItem.summary) {
                                return i18ndc("lingmo_applet_org.kde.lingmo.notifications", "Job name, e.g. Copying has failed", "%1 (Failed)", notificationItem.summary);
                            } else {
                                return i18nd("lingmo_applet_org.kde.lingmo.notifications", "Job Failed");
                            }
                        } else {
                            if (notificationItem.summary) {
                                return i18ndc("lingmo_applet_org.kde.lingmo.notifications", "Job name, e.g. Copying has finished", "%1 (Finished)", notificationItem.summary);
                            } else {
                                return i18nd("lingmo_applet_org.kde.lingmo.notifications", "Job Finished");
                            }
                        }
                    }
                }
    ...
    SelectableLabel {
        ...
        Layout.fillWidth: true
        Layout.alignment: Qt.AlignTop
        readonly property real maximumHeight: LingmoUI.Units.gridUnit * notificationItem.maximumLineCount
        ...
        text: "<style>a { color: " + LingmoUI.Theme.linkColor + "; }</style>" + notificationItem.body
