...
void IconApplet::populate()
{
    ...
    const QString lingmoIconsFolderPath = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1String("/lingmo_icons");
    if (!QDir().mkpath(lingmoIconsFolderPath)) {
        setLaunchErrorMessage(i18n("Failed to create icon widgets folder '%1'", lingmoIconsFolderPath));
        return;
    }
    ...
    connect(statJob, &KIO::StatJob::finished, this, [=, this] {
        ...
        QString backingDesktopFile = lingmoIconsFolderPath + QLatin1Char('/');
        ...
        if (QFileInfo::exists(backingDesktopFile + desiredDesktopFileName)) {
            desiredDesktopFileName = KFileUtils::suggestName(QUrl::fromLocalFile(lingmoIconsFolderPath), desiredDesktopFileName);
        }
