...
PlasmoidItem {
    ...
    function switchSizeFromSize(formFactor, compactMax, fullMin) {
        ...
        if (Plasmoid.formFactor === LingmoCore.Types.Planar) {
            return -1
        }
        ...
        if (!Number.isFinite(compactMax)) {
            ...
            compactMax = LingmoUI.Units.iconSizes.enormous - 1
        }

        // Layout.minimumWidth will return -1 if it is not set, handle that.
        if (fullMin <= 0) {
            fullMin = LingmoUI.Units.iconSizes.enormous - 1
        }
    ...
    switchWidth: switchSizeFromSize(LingmoCore.Types.Horizontal, compactRepresentationItem?.Layout.maximumWidth ?? Infinity, fullRepresentationItem?.Layout.minimumWidth ?? -1)
    switchHeight: switchSizeFromSize(LingmoCore.Types.Vertical, compactRepresentationItem?.Layout.maximumHeight ?? Infinity, fullRepresentationItem?.Layout.minimumHeight ?? -1)

    preferredRepresentation: Plasmoid.formFactor === LingmoCore.Types.Planar ? fullRepresentation : null
