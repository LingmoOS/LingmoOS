...
Qt::Edges edgeFromLocation(Lingmo::Types::Location location)
{
    switch (location) {
...
void AppMenuApplet::trigger(QQuickItem *ctx, int idx)
{
    ...
    if (actionMenu) {
        ...
        actionMenu->setProperty("_ocean_menu_seamless_edges", QVariant::fromValue(edges));

        if (location() == Lingmo::Types::TopEdge) {
