...
QWidget *SwitchWindow::createConfigurationInterface(QWidget *parent)
{
    ...
    m_ui.setupUi(widget);
    widget->setWindowTitle(i18nc("lingmo_containmentactions_switchwindow", "Configure Switch Window Plugin"));
    switch (m_mode) {
    case AllFlat:
...
void SwitchWindow::makeMenu()
{
    ...
    if (m_mode == CurrentDesktop) {
        ...
        QAction *a = new QAction(i18nc("lingmo_containmentactions_switchwindow", "Windows"), this);
        a->setSeparator(true);
        m_actions << a;
        ...
        if (m_mode == AllFlat) {
            ...
            if (allDesktops.count()) {
                QAction *a = new QAction(i18nc("lingmo_containmentactions_switchwindow", "All Desktops"), this);
                a->setSeparator(true);
                m_actions << a;
                m_actions << allDesktops;
            }
            ...
            if (allDesktops.count()) {
                QMenu *subMenu = new QMenu(i18nc("lingmo_containmentactions_switchwindow", "All Desktops"));
...
K_PLUGIN_CLASS_WITH_JSON(SwitchWindow, "lingmo-containmentactions-switchwindow.json")
