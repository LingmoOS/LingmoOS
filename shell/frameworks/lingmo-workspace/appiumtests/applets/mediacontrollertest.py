...
WIDGET_ID: Final = "org.kde.lingmo.mediacontroller"
KDE_VERSION: Final = 6
...
class MediaControllerTests(unittest.TestCase):
    ...
    player_lingmo_browser_integration: subprocess.Popen | None = None
    ...
    def setUpClass(cls) -> None:
        ...
        options.set_capability("environ", {
            "LC_ALL": "en_US.UTF-8",
            "QT_FATAL_WARNINGS": "1",
            "QT_LOGGING_RULES": "qt.accessibility.atspi.warning=false;kf.lingmo.core.warning=false;kf.windowsystem.warning=false;kf.lingmoui.platform.warning=false;qt.qml.typeresolution.cycle.warning=false",
        })
    ...
    def test_touch_gestures(self) -> None:
        ...
        @see https://invent.kde.org/lingmo/lingmo-workspace/-/merge_requests/2438
        """
    ...
    def _cleanup_filter_lingmo_browser_integration(self) -> None:
        """
        ...
        if self.player_browser is not None:
            ...
            self.player_browser.wait(10)
            self.player_browser = None
        if self.player_lingmo_browser_integration is not None:
            self.player_lingmo_browser_integration.terminate()
            self.player_lingmo_browser_integration.wait(10)
            self.player_lingmo_browser_integration = None
        if self.player_b is not None:
            self.player_b.terminate()
            self.player_b.wait(10)
            self.player_b = None

    def test_filter_lingmo_browser_integration(self) -> None:
        """
        When Lingmo Browser Integration is installed, the widget should only show the player from p-b-i, and hide the player from the browser.
        """
        self.addCleanup(self._cleanup_filter_lingmo_browser_integration)
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "mediacontrollertest", "player_lingmo_browser_integration.json"), "r", encoding="utf-8") as f:
            pbi_json_data = json.load(f)
        pbi_json_data["metadata"][0]["kde:pid"] = self.player_browser.pid  # Simulate Lingmo Browser Integration
        with NamedTemporaryFile("w", encoding="utf-8", suffix=".json", delete=False) as temp_file:
            json.dump(pbi_json_data, temp_file)
            temp_file.flush()

            self.player_lingmo_browser_integration = subprocess.Popen(("python3", os.path.join(os.path.dirname(os.path.abspath(__file__)), "mediacontrollertest", "mediaplayer.py"), temp_file.name))
        ...
        session_bus: Gio.DBusConnection = Gio.bus_get_sync(Gio.BusType.SESSION)
        session_bus.call(f"org.mpris.MediaPlayer2.appiumtest.instance{str(self.player_browser.pid)}", Mpris2.OBJECT_PATH, Mpris2.PLAYER_IFACE.get_string(), "Play", None, None, Gio.DBusSendMessageFlags.NONE, 1000)
        session_bus.call(f"org.mpris.MediaPlayer2.appiumtest.instance{str(self.player_lingmo_browser_integration.pid)}", Mpris2.OBJECT_PATH, Mpris2.PLAYER_IFACE.get_string(), "Play", None, None, Gio.DBusSendMessageFlags.NONE, 1000)
        ...
        session_bus.call(f"org.mpris.MediaPlayer2.appiumtest.instance{str(self.player_lingmo_browser_integration.pid)}", Mpris2.OBJECT_PATH, Mpris2.PLAYER_IFACE.get_string(), "Pause", None, None, Gio.DBusSendMessageFlags.NONE, 1000)
        player_b_json_path: str = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mediacontrollertest", "player_b.json")
        self.player_b = subprocess.Popen(("python3", os.path.join(os.path.dirname(os.path.abspath(__file__)), "mediacontrollertest", "mediaplayer.py"), player_b_json_path))
        ...
        self._cleanup_filter_lingmo_browser_integration()
