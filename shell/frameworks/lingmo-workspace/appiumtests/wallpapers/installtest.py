...
BIN_PATH: Final = os.path.join(os.getenv("CMAKE_BINARY_DIR", "/usr/bin/"), "lingmo-apply-wallpaperimage")
DBUS_XML_PATH: Final = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, os.pardir, "shell/dbus/org.kde.LingmoShell.xml")


class OrgKdeLingmoshell:
    """
    D-Bus interfaces for org.kde.lingmoshell
    """

    BUS_NAME: Final = "org.kde.lingmoshell"
    OBJECT_PATH: Final = "/LingmoShell"
    INTERFACE_NAME: Final = "org.kde.LingmoShell"
...
class InstallWallpaperTest(unittest.TestCase):
    """
    Tests for lingmo-apply-wallpaperimage
    """

    loop_thread: GLibMainLoopThread
    lingmoshell_interface: OrgKdeLingmoshell

    @classmethod
    def setUpClass(cls) -> None:
        cls.loop_thread = GLibMainLoopThread()
        cls.loop_thread.start()
        cls.lingmoshell_interface = OrgKdeLingmoshell()

    def tearDown(self) -> None:
        self.lingmoshell_interface.script_set_event.clear()

    @classmethod
    def tearDownClass(cls) -> None:
        cls.lingmoshell_interface.quit()
    ...
    def test_0_install_image(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(self.lingmoshell_interface.script_set_event.wait(10))
            self.assertIn(image_path, self.lingmoshell_interface.script)
    ...
    def test_1_install_kpackage(self) -> None:
        ...
        self.assertTrue(self.lingmoshell_interface.script_set_event.wait(10))
        self.assertIn(package_path, self.lingmoshell_interface.script)
    ...
    def test_2_install_nonexistent_path(self) -> None:
        ...
        self.assertFalse(self.lingmoshell_interface.script_set_event.wait(1))
