...
public:
    ...
    void start() override
    {
        qCDebug(LINGMO_SESSION) << "Phase 0";
...
public:
    ...
    void start() override
    {
        qCDebug(LINGMO_SESSION) << "Phase 1";
...
public:
    ...
    void start() override
    {
        qCDebug(LINGMO_SESSION) << "Phase 2";
    ...
    : QObject(parent)
{
    ...
    if (qEnvironmentVariable("XDG_SESSION_TYPE") != QLatin1String("wayland")) {
        ...
        kwinWaylandJob.exec();
        // kslpash is only launched in lingmo-session from the wayland mode, for X it's in startlingmo-x11
...
void StartServiceJob::start()
{
    ...
    qCDebug(LINGMO_SESSION) << "Starting " << m_process->program() << m_process->arguments();
    if (!Startup::self()->startDetached(m_process)) {
        qCWarning(LINGMO_SESSION) << "error starting process" << m_process->program() << m_process->arguments();
        emitResult();
    }
    ...
    , m_process(new QProcess(this))
{
    ...
    connect(m_process, &QProcess::finished, [this](int exitCode) {
        qCInfo(LINGMO_SESSION) << "process job " << m_process->program() << "finished with exit code " << exitCode;
        emitResult();
    });
}
