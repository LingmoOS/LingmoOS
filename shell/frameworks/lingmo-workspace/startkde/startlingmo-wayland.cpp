...
int main(int argc, char **argv)
{
    ...
    // set XKB_DEFAULT_{MODEL,LAYOUT,VARIANT,OPTIONS} accordingly.
    {
        ...
        if (resultMessage.type() == QDBusMessage::ReplyMessage) {
            ...
            qCWarning(LINGMO_STARTUP) << "not a reply org.freedesktop.locale1" << resultMessage;
        }
    }
    runEnvironmentScripts();

    if (!qEnvironmentVariableIsSet("DBUS_SESSION_BUS_ADDRESS")) {
        out << "startlingmocompositor: Could not start D-Bus. Can you call qdbus?\n";
        return 1;
    }
    ...
    if (!startLingmoSession(true))
        return 4;

    // Anything after here is logout
    // It is not called after shutdown/restart
    waitForKonqi();
    out << "startlingmo-wayland: Shutting down...\n";

    // Keep for KF5; remove in KF6 (KInit will be gone then)
    runSync(QStringLiteral("kdeinit5_shutdown"), {});

    out << "startlingmocompositor: Shutting down...\n";
    stopSystemdSession();
    cleanupLingmoEnvironment(oldSystemdEnvironment);
    out << "startlingmocompositor: Done.\n";
