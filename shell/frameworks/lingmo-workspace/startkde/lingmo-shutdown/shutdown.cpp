...
void Shutdown::startLogout(KWorkSpace::ShutdownType shutdownType)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [closeSessionReply, watcher, this]() {
        ...
        if (closeSessionReply.isError()) {
            qCInfo(LINGMO_SESSION) << "ksmserver failed to complete logout with error" << closeSessionReply.error().name() << " continuining with shutdown";
            ksmServerComplete();
            return;
        }
...
void Shutdown::saveSession()
{
    ...
    if (c.readEntry("loginMode", "restorePreviousLogout") != QLatin1String("restoreSavedSession")) {
        qCWarning(LINGMO_SESSION) << "Manual save requested, but this does not match ksmserverrc settings. Ignoring";
        return;
    }
    ...
    int ret = QProcess::execute(QStringLiteral(LINGMO_FALLBACK_SESSION_SAVE_BIN));
    if (ret) {
        qCWarning(LINGMO_SESSION) << "lingmo-fallback-session-save failed with return code" << ret;
    }
}
...
void Shutdown::ksmServerComplete()
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher *watcher) {
        ...
        if (!reply.isValid()) {
            qCWarning(LINGMO_SESSION) << "KWin failed to complete logout";
...
bool Shutdown::usingSystemdManagedSession()
{
    ...
    msg << QStringLiteral("lingmo-workspace.target");
...
void Shutdown::runShutdownScripts()
{
    const QStringList shutdownFolders =
        QStandardPaths::locateAll(QStandardPaths::GenericConfigLocation, QStringLiteral("lingmo-workspace/shutdown"), QStandardPaths::LocateDirectory);
    for (const QString &shutDownFolder : shutdownFolders) {
        ...
        for (const QString &file : entries) {
            ...
            if (!file.endsWith(QLatin1Char('~')) && !file.endsWith(QLatin1String(".bak")) && (file[0] != QLatin1Char('%') || !file.endsWith(QLatin1Char('%')))
                ...
                qCDebug(LINGMO_SESSION) << "running shutdown script" << fullPath;
