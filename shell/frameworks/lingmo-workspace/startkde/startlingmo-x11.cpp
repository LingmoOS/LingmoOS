...
int main(int argc, char **argv)
{
    ...
    // Check if a Lingmo session already is running and whether it's possible to connect to X
    switch (kCheckRunning()) {
    case NoX11:
        out << "$DISPLAY is not set or cannot connect to the X server.\n";
        return 1;
    case LingmoRunning:
        messageBox(QStringLiteral("Lingmo seems to be already running on this display.\n"));
        return 1;
    case NoLingmoRunning:
        break;
    }
