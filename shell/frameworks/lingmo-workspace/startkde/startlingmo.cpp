...
void gentleTermination(QProcess *p)
{
    ...
    if (!p->waitForFinished(5000)) {
        p->kill();
        if (!p->waitForFinished(5000)) {
            qCWarning(LINGMO_STARTUP) << "Could not fully finish the process" << p->program();
        }
    }
}
...
int runSync(const QString &program, const QStringList &args, const QStringList &env)
{
    ...
    p.waitForFinished(-1);
    if (p.exitCode()) {
        qCWarning(LINGMO_STARTUP) << program << args << "exited with code" << p.exitCode();
    }
    return p.exitCode();
}
...
void setupCursor(bool wayland)
{
    ...
    if (!wayland) {
        ...
        const auto cursorTheme = inputCfg.readEntry("cursorTheme", QStringLiteral("ocean_cursors"));
...
// But it won't work if lingmo is not started by systemd.
...
void setupLingmoEnvironment()
{
    ...
    qputenv("XDG_MENU_PREFIX", "lingmo-");
    ...
    const QString currentLnf = KConfigGroup(&globals, QStringLiteral("KDE")).readEntry("LookAndFeelPackage", QStringLiteral("org.kde.ocean.desktop"));
    ...
    if (activeLnf.readLine() != currentLnf.toUtf8()) {
        KPackage::Package package = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/LookAndFeel"), currentLnf);
        LookAndFeelManager lnfManager;
        lnfManager.setMode(LookAndFeelManager::Mode::Defaults);
        lnfManager.save(package, KPackage::Package());
    }
    // check if colors changed, if so apply them and discard lingmo cache
    {
        LookAndFeelManager lnfManager;
        ...
        const QString colorScheme = generalGroup.readEntry("ColorScheme", QStringLiteral("OceanLight"));
...
static void dropSessionVarsFromSystemdEnvironment()
{
    ...
    auto reply = QDBusConnection::sessionBus().call(msg);
    if (reply.type() == QDBusMessage::ErrorMessage) {
        qCWarning(LINGMO_STARTUP) << "Failed to unset systemd environment variables:" << reply.errorName() << reply.errorMessage();
    }
}

// kwin_wayland can possibly also start dbus-activated services which need env variables.
// In that case, the update in startlingmo might be too late.
...
QProcess *setupKSplash()
{
    ...
    if (!desktopLockedAtStart) {
        ...
        if (ksplashCfg.readEntry("Engine", QStringLiteral("KSplashQML")) == QLatin1String("KSplashQML")) {
            p = new QProcess;
            p->setProcessChannelMode(QProcess::ForwardedChannels);
            p->start(QStringLiteral("ksplashqml"), {ksplashCfg.readEntry("Theme", QStringLiteral("Ocean"))});
        }
    }
    return p;
}
...
// This is independent of whether we use the Lingmo systemd boot
...
static void migrateUserScriptsAutostart()
{
    ...
    for (const auto &info : entries) {
        ...
        if (scriptName.endsWith(QLatin1Char('~')) || scriptName.endsWith(QLatin1String(".bak"))
            ...
            || (scriptName[0] == QLatin1Char('#') && scriptName.endsWith(QLatin1Char('#')))) {
            qCDebug(LINGMO_STARTUP) << "Not migrating backup autostart script" << scriptName;
            continue;
        }
        ...
        qCInfo(LINGMO_STARTUP) << "Migrated legacy autostart script" << scriptPath << "to" << desktopFile.fileName();

        if (info.isSymLink() && QFile::remove(scriptPath)) {
            qCInfo(LINGMO_STARTUP) << "Removed legacy autostart script" << scriptPath << "that pointed to" << info.symLinkTarget();
        }
    }
...
bool startLingmoSession(bool wayland)
{
    ...
    iface.setStage(QStringLiteral("startLingmo"));
    ...
    QObject::connect(&serviceWatcher, &QDBusServiceWatcher::serviceUnregistered, [&]() {
        ...
        bool lingmoSessionRunning = std::any_of(watchedServices.constBegin(), watchedServices.constEnd(), [](const QString &service) {
            return QDBusConnection::sessionBus().interface()->isServiceRegistered(service);
        });
        if (!lingmoSessionRunning) {
            e.quit();
        }
    });
    ...
    if (!useSystemdBoot()) {
        ...
        if (wayland) {
            lingmoSessionOptions << QStringLiteral("--no-lockscreen");
        } else {
            if (desktopLockedAtStart) {
                lingmoSessionOptions << QStringLiteral("--lockscreen");
            }
        }

        startLingmoSession->setProcessChannelMode(QProcess::ForwardedChannels);
        QObject::connect(startLingmoSession.get(), &QProcess::finished, &e, [&rc](int exitCode, QProcess::ExitStatus) {
            if (exitCode == 255) {
                // Startup error
                messageBox(QStringLiteral("startkde: Could not start lingmo_session. Check your installation.\n"));
                rc = false;
            }
        });

        startLingmoSession->start(QStringLiteral(CMAKE_INSTALL_FULL_BINDIR "/lingmo_session"), lingmoSessionOptions);
    } else {
        qCDebug(LINGMO_STARTUP) << "Using systemd boot";
        ...
        QDBusReply<QDBusObjectPath> reply = QDBusConnection::sessionBus().call(msg);
        if (!reply.isValid()) {
            qCWarning(LINGMO_STARTUP) << "Could not start systemd managed Lingmo session:" << reply.error().name() << reply.error().message();
            messageBox(QStringLiteral("startkde: Could not start Lingmo session.\n"));
            rc = false;
        } else {
            playStartupSound();
        }
...
{
    // Generally lingmo-workspace.target will have already marked itself as stopped
    // when the session ends gracefully.

    // However, we also need to handle the reverse case where the login manager restarts and
    // startlingmo is terminated first. This should trigger a cleanup of the systemd services we started.
    auto msg = QDBusMessage::createMethodCall(QStringLiteral("org.freedesktop.systemd1"),
                                              QStringLiteral("/org/freedesktop/systemd1"),
                                              QStringLiteral("org.freedesktop.systemd1.Manager"),
                                              QStringLiteral("StopUnit"));
    msg << QStringLiteral("lingmo-workspace.target") << QStringLiteral("fail");
    ...
    QMetaObject::invokeMethod(
        ...
        [c, error_code] {
            if (error_code != CA_SUCCESS) {
                qCWarning(LINGMO_STARTUP) << "Failed to cancel canberra context for audio notification:" << ca_strerror(error_code);
            }
...
void playStartupSound()
{
    ...
    // documented, I am left guessing that it is this way so we don't end up talking to lingmoshell while it is initing.

    KNotifyConfig notifyConfig(QStringLiteral("lingmo_workspace"), QStringLiteral("startkde"));
    ...
    if (soundName.isEmpty()) {
        qCWarning(LINGMO_STARTUP) << "Audio notification requested, but no sound file provided in notifyrc file, aborting audio notification";
        return;
    }
    ...
    if (!group.readEntry("Enable", true)) {
        qCDebug(LINGMO_STARTUP) << "Notification sounds are globally disabled";
        return;
    }
    ...
    if (int ret = ca_context_create(&context); ret != CA_SUCCESS) {
        qCWarning(LINGMO_STARTUP) << "Failed to initialize canberra context for startup sound:" << ca_strerror(ret);
        return;
    }

    // We aren't actually lingmoshell, for all intents and purpose we are part of it though from the user perspective.
    if (int ret = ca_context_change_props(context,
                                          CA_PROP_APPLICATION_NAME,
                                          "lingmoshell",
                                          CA_PROP_APPLICATION_ID,
                                          "org.kde.lingmoshell.desktop",
                                          CA_PROP_APPLICATION_ICON_NAME,
                                          "lingmoshell",
                                          nullptr);
        ret != CA_SUCCESS) {
        qCWarning(LINGMO_STARTUP) << "Failed to set application properties on canberra context for startup sound:" << ca_strerror(ret);
    }
    ...
    if (int ret = ca_context_play_full(context, 0 /* id */, props, canberraFinishCallback, nullptr); ret != CA_SUCCESS) {
        qCWarning(LINGMO_STARTUP) << "Failed to play startup sound with canberra:" << ca_strerror(ret);
    }
}
