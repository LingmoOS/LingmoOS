...
{
    setApplicationName(QStringLiteral("lingmo_waitforname"));
    ...
    if (!intIsOk) {
        qCWarning(LOG_LINGMO) << "Invalid timeout value";
        exit(-1);
    }
    if (timeout > 0) {
...
bool Waiter::waitForService()
{
    ...
    if (mMode == Waiter::WaitForRegistration) {
        if (sessionBus.interface()->isServiceRegistered(mService)) {
            qCDebug(LOG_LINGMO) << "WaitForName: Service" << mService << "is already registered";
            return false;
        }

        qCDebug(LOG_LINGMO) << "WaitForName: Waiting for appearance of service" << mService << "for" << dbusTimeoutSec << "seconds";
        ...
        if (!sessionBus.interface()->isServiceRegistered(mService)) {
            qCDebug(LOG_LINGMO) << "WaitForName: Service" << mService << "is not yet registered";
            return false;
        }
...
{
    qCDebug(LOG_LINGMO) << "WaitForName: Service was registered after" << (mTimeoutTimer.interval() - (mTimeoutTimer.remainingTime() / 1000)) << "seconds";
    mTimeoutTimer.stop();
    exit(0);
}
...
{
    qCDebug(LOG_LINGMO) << "WaitForName: Service was unregistered after" << (mTimeoutTimer.interval() - (mTimeoutTimer.remainingTime() / 1000)) << "seconds";
    mTimeoutTimer.stop();
    exit(0);
}
...
void Waiter::timeout()
{
    qCInfo(LOG_LINGMO) << "WaitForName: Service was not registered within timeout";
    exit(1);
}
