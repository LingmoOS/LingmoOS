...
KCMInit::KCMInit(const QCommandLineParser &args)
{
    if (args.isSet(QStringLiteral("list"))) {
        m_list = KPluginMetaData::findPlugins(QStringLiteral("lingmo/kcminit"));
        for (const KPluginMetaData &data : std::as_const(m_list)) {
    ...
    if (!positionalArguments.isEmpty()) {
        for (const auto &arg : positionalArguments) {
            ...
            if (!data.isValid()) {
                data = KPluginMetaData::findPluginById(QStringLiteral("lingmo/kcminit"), arg);
            }
...
{
    // lingmo-session startup waits for kcminit to finish running phase 0 kcms
