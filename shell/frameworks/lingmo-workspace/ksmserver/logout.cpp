...
void KSMServer::saveYourselfDone(KSMClient *client, bool success)
{
    ...
    // Always fake success to make Lingmo's logout not block with broken
...
void KSMServer::completeShutdownOrCheckpoint()
{
    ...
    for (KSMClient *c : std::as_const(pendingClients)) {
        if (!c->saveYourselfDone && !c->waitForPhase2) {
            return; // not done yet
        }
    }
    ...
    if (state == Shutdown) {
        KNotification *n = KNotification::event(QStringLiteral("exitkde"), QString(), QPixmap(), KNotification::DefaultEvent); // Lingmo says good bye
...
void KSMServer::startKillingSubSession()
{
    ...
    for (KSMClient *c : std::as_const(clientsToKill)) {
...
void KSMServer::signalSubSessionClosed()
{
    ...
    // so that lingmo can close its stuff
    state = Idle;
    qCDebug(KSMSERVER) << state;
