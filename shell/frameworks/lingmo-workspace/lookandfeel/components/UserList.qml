...
ListView {
    ...
    readonly property int userItemWidth: LingmoUI.Units.gridUnit * 8
    readonly property int userItemHeight: LingmoUI.Units.gridUnit * 9
    readonly property bool constrainText: count > 1
    property real fontSize: LingmoUI.Theme.defaultFont.pointSize + 2
    ...
    delegate: UserDelegate {
        ...
        name: {
            ...
            if (!model.session) {
                return i18ndc("lingmo_lookandfeel_org.kde.lookandfeel", "Nobody logged in on that session", "Unused")
            }


            let location = undefined
            if (model.isTty) {
                location = i18ndc("lingmo_lookandfeel_org.kde.lookandfeel", "User logged in on console number", "TTY %1", model.vtNumber)
            } else if (model.displayNumber) {
                location = i18ndc("lingmo_lookandfeel_org.kde.lookandfeel", "User logged in on console (X display number)", "on TTY %1 (Display %2)", model.vtNumber, model.displayNumber)
            }

            if (location !== undefined) {
                return i18ndc("lingmo_lookandfeel_org.kde.lookandfeel", "Username (location)", "%1 (%2)", displayName, location)
            }
