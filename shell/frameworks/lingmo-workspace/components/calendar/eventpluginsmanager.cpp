...
{
    const auto plugins = KPluginMetaData::findPlugins(QStringLiteral("lingmocalendarplugins"));
    for (const KPluginMetaData &plugin : plugins) {
        const QString prefix = plugin.fileName().left(plugin.fileName().lastIndexOf(QLatin1Char('/')));
        const QString configName = plugin.value(QStringLiteral("X-KDE-LingmoCalendar-ConfigUi"));
...
{
    QPluginLoader loader(QString(u"lingmocalendarplugins/" + QDir::cleanPath(pluginId)));

    if (!loader.load()) {
        qWarning() << "Could not create Lingmo Calendar Plugin: " << pluginId;
