...
public:
     ...
     */
    void appletAdded(Lingmo::Applet *applet);

    /**
     * A running applet is no more
     */
    void appletRemoved(Lingmo::Applet *applet);

    WidgetExplorer *q;
    QString application;
    Lingmo::Containment *containment;

    QHash<QString, int> runningApplets; // applet name => count
    // extra hash so we can look up the names of deleted applets
    QHash<Lingmo::Applet *, QString> appletNames;

    LingmoAppletItemModel itemModel;
...
void WidgetExplorerPrivate::initFilters()
{
    ...
    filterModel.addFilter(i18n("All Widgets"), KCategorizedItemsViewModels::Filter(), QIcon::fromTheme(QStringLiteral("lingmo")));
    ...
    for (int i = 0; i < itemModel.rowCount(); ++i) {
        if (LingmoAppletItem *p = dynamic_cast<LingmoAppletItem *>(itemModel.item(i))) {
...
QList<QObject *> WidgetExplorer::widgetsMenuActions()
{
    ...
    if (KAuthorized::authorize(KAuthorized::GHNS)) {
        action = new WidgetAction(QIcon::fromTheme(QStringLiteral("internet-services")), i18n("Download New Lingmo Widgets"), this);
...
void WidgetExplorerPrivate::initRunningApplets()
{
    ...
    Lingmo::Corona *c = containment->corona();
    ...
    QObject::connect(c, &Lingmo::Corona::screenAdded, q, [this](int screen) {
        screenAdded(screen);
    });
    QObject::connect(c, &Lingmo::Corona::screenRemoved, q, [this](int screen) {
        screenRemoved(screen);
    });

    const QList<Containment *> containments = c->containments();
    for (Containment *containment : containments) {
        if (containment->containmentType() == Lingmo::Containment::Desktop && containment->activity() != activitiesConsumer->currentActivity()) {
            continue;
        }
...
{
    QObject::connect(containment, &Lingmo::Containment::appletAdded, q, [this](Lingmo::Applet *a, const QRectF &) {
        appletAdded(a);
    });
    QObject::connect(containment, &Lingmo::Containment::appletRemoved, q, [this](Lingmo::Applet *a) {
        appletRemoved(a);
    });
...
void WidgetExplorerPrivate::removeContainment(Lingmo::Containment *containment)
{
...
void WidgetExplorerPrivate::appletAdded(Lingmo::Applet *applet)
{
    if (!applet->pluginMetaData().isValid()) {
        return;
    }
...
void WidgetExplorerPrivate::appletRemoved(Lingmo::Applet *applet)
{
...
void WidgetExplorer::setContainment(Lingmo::Containment *containment)
{
...
Lingmo::Corona *WidgetExplorer::corona() const
{
    if (d->containment) {
...
{
    const QString p = QStringLiteral(LINGMO_RELATIVE_DATA_INSTALL_DIR) + u"/plasmoids/" + pluginName;
...
void WidgetExplorer::immutabilityChanged(Lingmo::Types::ImmutabilityType type)
{
    if (type != Lingmo::Types::Mutable) {
        Q_EMIT shouldClose();
    }
}
...
void WidgetExplorer::openWidgetFile()
{
    ...
    dialog->setMimeTypeFilters({u"application/x-lingmo"_s});
    ...
    connect(dialog, &QFileDialog::fileSelected, [](const QString &packageFilePath) {
        ...
        auto job = KPackage::PackageJob::install(QStringLiteral("Lingmo/Applet"), packageFilePath);
...
void WidgetExplorer::uninstall(const QString &pluginName)
{
    static const QString packageRoot = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1Char('/')
        + QStringLiteral(LINGMO_RELATIVE_DATA_INSTALL_DIR) + u"/plasmoids/";
    KPackage::PackageJob *job = KPackage::PackageJob::uninstall(QStringLiteral("Lingmo/Applet"), pluginName, packageRoot);
    // This removes folders of packages that are *not* valid so the package job can't uninstall them
    // This is a bit dangerous so eventually we can drop this when we drop support for uninstalling lingmo5 plasmoids
    ...
    for (int i = 0; i < d->itemModel.rowCount(); ++i) {
        ...
        if (item->data(LingmoAppletItemModel::PluginNameRole).toString() == pluginName) {
