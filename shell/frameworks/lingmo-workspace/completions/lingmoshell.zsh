...
# keep this function in sync with plasmoidviewer from lingmo-sdk
function _lingmo_list_packages() {
  local expl global_dir descr="$1" purpose="$2" type="${3:-Lingmo/Applet}"
  ...
  packages=( ${(f)"$(
    _call_program lingmo-list-packages \
  ...
  if [[ "$purpose" == "containment" ]]; then
    ...
    containments=( ${${${(f)"$(
      ...
      grep --files-with-matches -R "Lingmo/Containment" $global_dir/*/metadata.json(N.,@r)
    ...
    # executable applets on their own, e.g. org.kde.lingmo.folder view.
  fi
...
_arguments -s \
  ...
  '(-n --no-respawn)'{-n,--no-respawn}'[Do not restart lingmo-shell automatically after a crash]' \
  '(-a --standalone)'{-a,--standalone}'[Load lingmoshell as a standalone application, needs the shell-plugin option to be specified]' \
  '(-p --shell-plugin)'{-p=,--shell-plugin=}'[Force loading the given shell plugin]::: _lingmo_list_packages plugins "shell plugin" Lingmo/Shell' \
