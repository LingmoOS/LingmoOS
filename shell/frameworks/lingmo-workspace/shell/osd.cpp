...
bool Osd::init()
{
    ...
    if (!m_osdObject) {
        m_osdObject = new LingmoQuick::SharedQmlEngine(this);
    }

    m_osdObject->setSource(url);

    if (m_osdObject->status() != QQmlComponent::Ready) {
        qCWarning(LINGMOSHELL) << "Failed to load OSD QML file" << url;
        auto fallbackUrl = m_corona->kPackage().fallbackPackage().fileUrl("osdmainscript");
        if (fallbackUrl.isEmpty() || fallbackUrl == url) {
            return false;
        }
        qCWarning(LINGMOSHELL) << "Trying fallback theme";
        m_osdObject->setSource(fallbackUrl);
        if (m_osdObject->status() != QQmlComponent::Ready) {
            qCWarning(LINGMOSHELL) << "Failed to load fallback OSD QML file" << fallbackUrl;
            return false;
        }
    }
