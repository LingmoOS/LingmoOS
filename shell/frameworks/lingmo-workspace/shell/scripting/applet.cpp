...
void Applet::setLocked(bool locked)
{
    ...
    app->setImmutability(locked ? Lingmo::Types::UserImmutable : Lingmo::Types::Mutable);
    KConfigGroup cg = app->config();
    if (!app->isContainment()) {
        cg = cg.parent();
    }
...
bool Applet::locked() const
{
    Lingmo::Applet *app = applet();
    if (!app) {
        return Lingmo::Types::Mutable;
    }

    return app->immutability() != Lingmo::Types::Mutable;
}

Lingmo::Applet *Applet::applet() const
{
    return nullptr;
}
