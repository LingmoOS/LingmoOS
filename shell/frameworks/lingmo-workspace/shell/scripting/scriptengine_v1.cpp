...
QJSValue ScriptEngine::V1::desktopById(const QJSValue &param) const
{
    ...
    for (const auto conts = m_engine->m_corona->containments(); Lingmo::Containment * c : conts) {
...
bool ScriptEngine::V1::loadTemplate(const QString &layout)
{
    ...
    auto filter = [&layout](const KPluginMetaData &md) -> bool {
        return md.pluginId() == layout && md.value(QStringLiteral("X-Lingmo-ContainmentCategories"), QStringList()).contains(QLatin1String("panel"));
    };
    QList<KPluginMetaData> offers = KPackage::PackageLoader::self()->findPackages(QStringLiteral("Lingmo/LayoutTemplate"), QString(), filter);
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qCWarning(LINGMOSHELL) << "Unable to load script file:" << path;
        return false;
    }
...
QJSValue ScriptEngine::V1::knownWallpaperPlugins(const QString &formFactor) const
{
    ...
    if (!formFactor.isEmpty()) {
        constraint.append(u"[X-Lingmo-FormFactors] ~~ '").append(formFactor).append(u'\'');
    }

    const QList<KPluginMetaData> wallpapers = KPackage::PackageLoader::self()->listPackages(QStringLiteral("Lingmo/Wallpaper"), QString());
...
QJSValue ScriptEngine::V1::configFile(const QJSValue &config, const QString &group)
{
    ...
    if (!config.isUndefined()) {
        if (config.isString()) {
            ...
            const Lingmo::Corona *corona = m_engine->corona();
...
void ScriptEngine::V1::setImmutability(const QString &immutability)
{
    ...
    if (immutability == QLatin1String("systemImmutable")) {
        m_engine->corona()->setImmutability(Lingmo::Types::SystemImmutable);
    } else if (immutability == QLatin1String("userImmutable")) {
        m_engine->corona()->setImmutability(Lingmo::Types::UserImmutable);
    } else {
        m_engine->corona()->setImmutability(Lingmo::Types::Mutable);
    }
...
QString ScriptEngine::V1::immutability() const
{
    ...
    case Lingmo::Types::SystemImmutable:
        return QLatin1String("systemImmutable");
    case Lingmo::Types::UserImmutable:
