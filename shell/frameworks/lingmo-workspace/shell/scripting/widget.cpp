...
Widget::Widget(Lingmo::Applet *applet, ScriptEngine *parent)
    : Applet(parent)
...
Lingmo::Applet *Widget::applet() const
{
    return d->applet;
}
...
int Widget::index() const
{
    ...
    Lingmo::Containment *c = d->applet->containment();
    if (!c) {
        return -1;
    }
...
{
    QQuickItem *appletItem = LingmoQuick::AppletQuickItem::itemForApplet(d->applet);
...
void Widget::setUserBackgroundHints(const QString &hint)
{
    ...
    if (ok) {
        applet()->setUserBackgroundHints(Lingmo::Types::BackgroundHints(value));
    }
}
