...
{
ScriptEngine::ScriptEngine(Lingmo::Corona *corona, QObject *parent)
    : QJSEngine(parent)
    , m_corona(corona)
{
    Q_ASSERT(m_corona);
...
QJSValue ScriptEngine::wrap(Lingmo::Applet *w)
{
    Widget *wrapper = new Widget(w, this);
    return newQObject(wrapper);
}

QJSValue ScriptEngine::wrap(Lingmo::Containment *c)
{
...
bool ScriptEngine::isPanel(const Lingmo::Containment *c)
{
    if (!c) {
        return false;
    }

    return c->containmentType() == Lingmo::Containment::Panel || c->containmentType() == Lingmo::Containment::CustomPanel;
}

Lingmo::Corona *ScriptEngine::corona() const
{
    return m_corona;
}
...
QStringList ScriptEngine::pendingUpdateScripts(Lingmo::Corona *corona)
{
    if (!corona->kPackage().isValid()) {
        qCWarning(LINGMOSHELL) << "Warning: corona package invalid";
        return QStringList();
    }
    ...
    const QStringList dirs =
        QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, u"lingmo/shells/" + appName + u"/contents/updates", QStandardPaths::LocateDirectory);
...
QList<Containment *> ScriptEngine::desktopsForActivity(const QString &id)
{
    ...
    for (const auto conts = m_corona->containments(); Lingmo::Containment * c : conts) {
...
Lingmo::Containment *ScriptEngine::createContainment(const QString &type, const QString &plugin)
{
    bool exists = false;
    const QList<KPluginMetaData> list = Lingmo::PluginLoader::listContainmentsMetaDataOfType(type);
    ...
    if (c) {
        ...
        c->updateConstraints(Lingmo::Applet::AllConstraints | Lingmo::Applet::StartupCompletedConstraint);
...
{
    Lingmo::Containment *c = createContainment(type, plugin);
