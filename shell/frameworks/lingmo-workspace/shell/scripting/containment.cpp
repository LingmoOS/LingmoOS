...
Containment::Containment(Lingmo::Containment *containment, ScriptEngine *engine)
    : Applet(engine)
...
QList<int> Containment::widgetIds() const
{
    ...
    if (d->containment) {
        for (const auto applets = d->containment->applets(); const Lingmo::Applet *applet : applets) {
            w.append(applet->id());
        }
    }
...
QJSValue Containment::widgetById(const QJSValue &paramId) const
{
    ...
    if (d->containment) {
        for (const auto applets = d->containment->applets(); Lingmo::Applet * w : applets) {
            if (w->id() == id) {
...
QJSValue Containment::addWidget(const QJSValue &v, qreal x, qreal y, qreal w, qreal h, const QVariantList &args)
{
    ...
    if (v.isString()) {
        ...
        if (geometry.x() >= 0 && geometry.y() >= 0) {
            containmentItem = LingmoQuick::AppletQuickItem::itemForApplet(d->containment);

            if (containmentItem) {
                QMetaObject::invokeMethod(containmentItem,
                                          "createApplet",
                                          Qt::DirectConnection,
                                          Q_RETURN_ARG(Lingmo::Applet *, applet),
...
QJSValue Containment::widgets(const QString &widgetType) const
{
    ...
    for (const auto applets = d->containment->applets(); Lingmo::Applet * widget : applets) {
...
Lingmo::Applet *Containment::applet() const
{
    return d->containment;
}

Lingmo::Containment *Containment::containment() const
{
    return d->containment;
}
