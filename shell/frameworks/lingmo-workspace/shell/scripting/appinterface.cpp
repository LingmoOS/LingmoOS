...
QList<int> AppInterface::activityIds() const
{
    ...
    for (Lingmo::Containment *c : containments) {
...
QList<int> AppInterface::panelIds() const
{
    ...
    for (const auto conts = m_env->corona()->containments(); Lingmo::Containment * c : conts) {
...
{
    return LINGMO_DESKTOP_SCRIPTING_VERSION;
}
...
{
    m_env->corona()->setImmutability(locked ? Lingmo::Types::UserImmutable : Lingmo::Types::Mutable);
}

bool AppInterface::coronaLocked() const
{
    return m_env->corona()->immutability() != Lingmo::Types::Mutable;
}
...
void AppInterface::sleep(int ms)
{
    ...
    qCWarning(LINGMOSHELL) << "Lingmo scripting sleep() is deprecated and does nothing!";
}
...
QStringList AppInterface::knownWidgetTypes() const
{
    if (m_knownWidgets.isEmpty()) {
        QStringList widgets;
        const QList<KPluginMetaData> plugins = Lingmo::PluginLoader::self()->listAppletMetaData(QString());
...
QStringList AppInterface::knownContainmentTypes(const QString &type) const
{
    ...
    const QList<KPluginMetaData> plugins = Lingmo::PluginLoader::listContainmentsMetaDataOfType(type);
