...
private:
    static QString lingmoLocationToString(const Lingmo::Types::Location location);

    Lingmo::Containment *containmentById(unsigned int id);
    QString containmentPreview(Lingmo::Containment *containment);
...
private:
    ...
    struct Data {
        unsigned int id;
        int screen;
        Lingmo::Types::Location edge;
        QString activity;
        bool changed = false;
        bool isActive = true;
        QString image;
        const Lingmo::Containment *containment;
    };
    QTimer *m_reloadTimer = nullptr;
    ...
    QHash<int, QHash<Lingmo::Types::Location, QList<int>>> m_edgeCount;
};
