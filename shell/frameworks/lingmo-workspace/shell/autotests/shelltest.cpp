...
void ShellTest::initTestCase()
{
    ...
    m_lingmoDir = QDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + u'/' + u"lingmo");
    m_lingmoDir.removeRecursively();

    copyDirectory(QFINDTESTDATA("data/testpanel"), m_lingmoDir.absolutePath() + u"/plasmoids/org.kde.lingmo.testpanel");
    ...
    m_corona->setShell(u"org.kde.lingmo.nano"_s);
    m_corona->init();
...
void ShellTest::cleanup()
{
    ...
    for (auto *c : m_corona->containments()) {
        if (c->containmentType() == Lingmo::Containment::Panel || c->screen() != 0) {
            c->destroy();
        }
    }
...
{
    QCOMPARE(m_corona->m_panelViews.size(), 0);
    auto panelCont = m_corona->addPanel(QStringLiteral("org.kde.lingmo.testpanel"));
    QCOMPARE(panelCont->pluginMetaData().pluginId(), QStringLiteral("org.kde.lingmo.testpanel"));
    // If the panel fails to load (on ci lingmo-desktop isn't here) we want the "failed" containment to be of panel type anyways
...
void ShellTest::testScreenRemoval()
{
    ...
    Lingmo::Containment *cont0 = m_corona->containmentForScreen(0, m_corona->m_activityController->currentActivity(), QString());
    QVERIFY(cont0);
    QCOMPARE(cont0->screen(), 0);
    Lingmo::Containment *cont1 = m_corona->containmentForScreen(1, m_corona->m_activityController->currentActivity(), QString());
    QVERIFY(cont1);
    QCOMPARE(cont1->screen(), 1);
    Lingmo::Containment *cont2 = m_corona->containmentForScreen(2, m_corona->m_activityController->currentActivity(), QString());
    QVERIFY(cont2);
    QCOMPARE(cont2->screen(), 2);

    QCOMPARE(m_corona->m_panelViews.size(), 0);
    auto panelCont = m_corona->addPanel(QStringLiteral("org.kde.lingmo.panel"));
...
void ShellTest::testReorderScreens()
{
    ...
    QList<Lingmo::Containment *> desktopContainments;
    QList<Lingmo::Containment *> panelContainments;
    ...
    QList<Lingmo::Containment *> desktopContainmentsAfter;
    QList<Lingmo::Containment *> panelContainmentsAfter;
...

    {
        ...
        for (PanelView *v : m_corona->m_panelViews) {
            Lingmo::Containment *cont = v->containment();
...
void ShellTest::testPanelSizeModes()
{
    ...
    panel->containment()->setLocation(Lingmo::Types::BottomEdge);
    QCOMPARE(panel->containment()->location(), Lingmo::Types::BottomEdge);
    ...
    // Set vertical panel
    panel->containment()->setLocation(Lingmo::Types::LeftEdge);
    panel->containment()->setFormFactor(Lingmo::Types::Vertical);
