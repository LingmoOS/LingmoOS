...
CurrentContainmentActionsModel::CurrentContainmentActionsModel(Lingmo::Containment *containment, QObject *parent)
    ...
    , m_tempConfigParent(QString(), KConfig::SimpleConfig)
{
    ...
    QHash<QString, Lingmo::ContainmentActions *> actions = containment->containmentActions();

    QHashIterator<QString, Lingmo::ContainmentActions *> i(actions);
    while (i.hasNext()) {
        i.next();
        ...
        m_plugins[i.key()] = Lingmo::PluginLoader::self()->loadContainmentActions(m_containment, i.value()->metadata().pluginId());
        m_plugins[i.key()]->setContainment(m_containment);
        KConfigGroup cfg(&m_baseCfg, i.key());
        m_plugins[i.key()]->restore(cfg);
        item->setData(m_plugins[i.key()]->metadata().rawData().value(QStringLiteral("X-Lingmo-HasConfigurationInterface")).toBool(),
...
QString CurrentContainmentActionsModel::mouseEventString(int mouseButton, int modifiers)
{
    ...
    const QString string = Lingmo::ContainmentActions::eventToString(mouse);
...
QString CurrentContainmentActionsModel::wheelEventString(QObject *quickWheelEvent)
{
    ...
    return Lingmo::ContainmentActions::eventToString(&wheel);
}
...
bool CurrentContainmentActionsModel::append(const QString &action, const QString &plugin)
{
    ...
    Lingmo::ContainmentActions *actions = Lingmo::PluginLoader::self()->loadContainmentActions(m_containment, plugin);
    ...
    item->setData(m_plugins[action]->metadata().rawData().value(QStringLiteral("X-Lingmo-HasConfigurationInterface")).toBool(), HasConfigurationInterfaceRole);
...
void CurrentContainmentActionsModel::update(int row, const QString &action, const QString &plugin)
{
    ...
    if (idx.isValid()) {
        ...
        if (!m_plugins.contains(action) || oldPlugin != plugin) {
            ...
            m_plugins[action] = Lingmo::PluginLoader::self()->loadContainmentActions(m_containment, plugin);
            m_plugins[action]->setContainment(m_containment);
            // empty config: the new one will ne in default state
            KConfigGroup tempConfig(&m_tempConfigParent, u"test"_s);
            m_plugins[action]->restore(tempConfig);
            setData(idx,
                    m_plugins[action]->metadata().rawData().value(QStringLiteral("X-Lingmo-HasConfigurationInterface")).toBool(),
                    HasConfigurationInterfaceRole);
        }
...
void CurrentContainmentActionsModel::showConfiguration(int row, QQuickItem *ctx)
{
    ...
    Lingmo::ContainmentActions *pluginInstance = m_plugins[action];
...
void CurrentContainmentActionsModel::save()
{
    ...
    QHashIterator<QString, Lingmo::ContainmentActions *> i(m_plugins);
    while (i.hasNext()) {
        i.next();
