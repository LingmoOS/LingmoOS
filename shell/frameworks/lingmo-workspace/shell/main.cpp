...
int main(int argc, char *argv[])
{
    ...
    if (qEnvironmentVariableIsSet("LINGMO_ENABLE_QML_DEBUG")) {
    ...
    // this works around a bug of Qt and the lingmoshell protocol
    // consider disabling when layer-shell lands
    qputenv("QT_WAYLAND_DISABLE_FIXED_POSITIONS", {});
    // this variable controls whether to reconnect or exit if the compositor dies, given lingmoshell does a lot of
    ...
    KLocalizedString::setApplicationDomain(QByteArrayLiteral("lingmoshell"));

    // The executable's path is added to the library/plugin paths.
    // This does not make much sense for lingmoshell.
    app.removeLibraryPath(QCoreApplication::applicationDirPath());

    KAboutData aboutData(QStringLiteral("lingmoshell"), QString(), QStringLiteral(PROJECT_VERSION), i18n("Lingmo Shell"), KAboutLicense::GPL);
    ...
    ShellCorona *corona = nullptr;
    {
        ...
        QCommandLineOption noRespawnOption(QStringList() << QStringLiteral("n") << QStringLiteral("no-respawn"),
                                           i18n("Do not restart lingmo-shell automatically after a crash"));
        ...
        qApp->setProperty("_lingmo_dbus_master", true);
        ...
        QObject::connect(corona, &ShellCorona::glInitializationFailed, &app, [&app]() {
            ...
            if (configGroup.readEntry("SceneGraphBackend") != QLatin1String("software")) {
                ...
                configGroup.sync();
                QProcess::startDetached(QStringLiteral("lingmoshell"), app.arguments());
            } else {
                QCoreApplication::setAttribute(Qt::AA_ForceRasterWidgets);
                QMessageBox::critical(nullptr,
                                      i18n("Lingmo Failed To Start"),
                                      i18n("Lingmo is unable to start as it could not correctly use OpenGL 2 or software fallback\nPlease check that your "
