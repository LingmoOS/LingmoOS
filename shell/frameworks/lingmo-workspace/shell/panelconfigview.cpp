...
#include <lingmoquick/popuplingmowindow.h>
...
PanelRulerView::PanelRulerView(Lingmo::Containment *containment, PanelView *panelView, PanelConfigView *mainConfigView)
    : LingmoWindow()
...
void PanelRulerView::syncPanelLocation()
{
    ...
    if (KWindowSystem::isPlatformX11()) {
        ...
        if (m_containment->formFactor() == Lingmo::Types::Horizontal) {
...
//////////////////////////////PanelConfigView
PanelConfigView::PanelConfigView(Lingmo::Containment *containment, PanelView *panelView)
    : LingmoQuick::PopupLingmoWindow()
    , m_containment(containment)
    , m_panelView(panelView)
    , m_sharedQmlEngine(std::make_unique<LingmoQuick::SharedQmlEngine>(this))
{
    ...
    connect(containment, &Lingmo::Containment::destroyedChanged, this, &QObject::deleteLater);
    connect(containment, &Lingmo::Containment::formFactorChanged, this, &PanelConfigView::syncGeometry);
    connect(containment, &Lingmo::Containment::locationChanged, this, &PanelConfigView::syncGeometry);
    ...
    connect(m_containment->corona(), &Lingmo::Corona::editModeChanged, this, [this](bool edit) {
        if (!edit) {
            hide();
        }
    });
...
void PanelConfigView::addPanelSpacer()
{
    ...
    c->evaluateScript(u"panel = panelById(" + QString::number(m_containment->id())
                      + QStringLiteral(");"
                                       "var spacers = panel.widgets(\"org.kde.lingmo.panelspacer\");"
                                       "if (spacers.length === 1) {"
                                       "    panel.addWidget(\"org.kde.lingmo.panelspacer\", 0,0,1,1);"
                                       "} else {"
                                       "    panel.addWidget(\"org.kde.lingmo.panelspacer\");"
                                       "}"));
}
...
void PanelConfigView::showEvent(QShowEvent *ev)
{
    ...
    PopupLingmoWindow::showEvent(ev);
}

void PanelConfigView::hideEvent(QHideEvent *ev)
{
    PopupLingmoWindow::hideEvent(ev);
...
void PanelConfigView::focusVisibilityCheck(QWindow *focusWindow)
{
    ...
    // Do not hide if the focus is on PopupLingmoWindows opened by the
    // panel, e.g. the settings popup that appear on applet hover.
    if (auto popup = qobject_cast<const PopupLingmoWindow *>(focusWindow)) {
    ...
    if (auto popup = qobject_cast<const LingmoQuick::Dialog *>(focusWindow)) {
        if (auto parent = popup->visualParent(); parent && parent->window() == m_panelView) {
            return;
        }
    }
    if (auto popup = qobject_cast<const PopupLingmoWindow *>(oldFocusWindow); popup && oldFocusWindow != this) {
        requestActivate();
        return;
    }
...
void PanelConfigView::focusInEvent(QFocusEvent *ev)
{
    ...
    PopupLingmoWindow::focusInEvent(ev);
}
