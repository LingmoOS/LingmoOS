...
#include <LingmoActivities/Consumer>
...
ShellCorona::ShellCorona(QObject *parent)
    ...
    , m_config(KSharedConfig::openConfig(QStringLiteral("lingmorc")))
    ...
    , m_shellContainmentConfig(nullptr)
{
    ...
    qmlRegisterUncreatableType<DesktopView>("org.kde.lingmo.shell", 2, 0, "Desktop", QStringLiteral("It is not possible to create objects of type Desktop"));
    qmlRegisterUncreatableType<PanelView>("org.kde.lingmo.shell", 2, 0, "Panel", QStringLiteral("It is not possible to create objects of type Panel"));

    qDBusRegisterMetaType<QColor>();

    KConfigGroup cg(KSharedConfig::openConfig(u"kdeglobals"_s), u"KDE"_s);
    const QString packageName = cg.readEntry("LookAndFeelPackage", QString());
    m_lookAndFeelPackage = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/LookAndFeel"), packageName);
...
{
#if USE_SCRIPTING
    connect(this, &Lingmo::Corona::containmentCreated, this, [this](Lingmo::Containment *c) {
        executeSetupPlasmoidScript(c, c);
    });
#endif

    connect(this, &Lingmo::Corona::availableScreenRectChanged, this, &Lingmo::Corona::availableScreenRegionChanged);
    connect(this, &Lingmo::Corona::editModeChanged, this, [this]() {
        QMapIterator<const Lingmo::Containment *, PanelView *> i(m_panelViews);
        while (i.hasNext()) {
            i.next();
    ...
    m_lnfDefaultsConfig = KConfigGroup(&m_lnfDefaultsConfig, QStringLiteral("org.kde.lingmo.desktop"));

    new LingmoShellAdaptor(this);

    QDBusConnection dbus = QDBusConnection::sessionBus();
    dbus.registerObject(QStringLiteral("/LingmoShell"), this);

    // Look for theme config in lingmorc, if it isn't configured, take the theme from the
    ...
    // catch when lingmorc changes, so we e.g. enable/disable the OSd
     ...
     *
     * The unique connection makes sure we don't reload lingmo if KAMD ever crashes and reloads, the signal is disconnected in the body of load
     */
...
void ShellCorona::setShell(const QString &shell)
{
    ...
    KConfigGroup lingmorc(m_config, themeGroupKey);
    themeName = lingmorc.readEntry(themeNameKey, themeName);
    ...
    if (!themeName.isEmpty()) {
        Lingmo::Theme *t = new Lingmo::Theme(this);
...
QByteArray ShellCorona::dumpCurrentLayoutJS() const
{
    ...
    auto isPanel = [](Lingmo::Containment *cont) {
        return (cont->formFactor() == Lingmo::Types::Horizontal || cont->formFactor() == Lingmo::Types::Vertical)
            && (cont->location() == Lingmo::Types::TopEdge || cont->location() == Lingmo::Types::BottomEdge || cont->location() == Lingmo::Types::LeftEdge
                || cont->location() == Lingmo::Types::RightEdge)
            && cont->pluginMetaData().pluginId() != QLatin1String("org.kde.lingmo.private.systemtray");
    };

    auto isDesktop = [](Lingmo::Containment *cont) {
    ...
    for (Lingmo::Containment *cont : containments) {
        if (!isPanel(cont)) {
            continue;
        }
        ...
        panelJson.insert(u"location",
                         location == Lingmo::Types::TopEdge         ? u"top"_s
                             : location == Lingmo::Types::LeftEdge  ? u"left"_s
                             : location == Lingmo::Types::RightEdge ? u"right"_s
                                                                    : /* Lingmo::Types::BottomEdge */ u"bottom"_s);
        ...
        if (appletsOrderStrings.length() == cont->applets().length()) {
            ...
            for (Lingmo::Applet *applet : applets) {
                QJsonObject appletJson;
    ...
    return
        QByteArray("var lingmo = getApiVersion(1);\n\n"
        "var layout = " + json.toJson() + ";\n\n"
        "lingmo.loadSerializedLayout(layout);\n");
}
...
void ShellCorona::loadLookAndFeelDefaultLayout(const QString &packageName)
{
    ...
    KSharedConfig::Ptr conf = KSharedConfig::openConfig(QLatin1String("lingmo-") + m_shell + QLatin1String("-appletsrc"), KConfig::SimpleConfig);
...
void ShellCorona::sanitizeScreenLayout(const QString &configFileName)
{
    ...
    QSet<Lingmo::Types::Location> panelLocations({Lingmo::Types::TopEdge, Lingmo::Types::BottomEdge, Lingmo::Types::LeftEdge, Lingmo::Types::RightEdge});
    ...
    for (const QString &idStr : cg.groupList()) {
        ...
        if (panelLocations.contains(Lingmo::Types::Location(contCg.readEntry(QStringLiteral("location"), 0)))) {
            if (lastScreen >= 0) {
...
void ShellCorona::load()
{
    ...
    if (activityStatus != KActivities::Controller::Running && !qApp->property("org.kde.KActivities.core.disableAutostart").toBool()) {
        if (activityStatus == KActivities::Controller::NotRunning) {
            qCWarning(LINGMOSHELL) << "Aborting shell load: The activity manager daemon (kactivitymanagerd) is not running.";
            qCWarning(LINGMOSHELL)
                << "If this Lingmo has been installed into a custom prefix, verify that its D-Bus services dir is known to the system for the daemon to be "
                   "activatable.";
        }
        return;
    }
    ...
    QString configFileName(u"lingmo-" + m_shell + u"-appletsrc");
    ...
    if (containments().isEmpty()) {
        ...
        std::copy_if(containments.constBegin(), containments.constEnd(), std::back_inserter(m_waitingPanels), [this](Lingmo::Containment *containment) {
            return ((containment->containmentType() == Lingmo::Containment::Panel || containment->containmentType() == Lingmo::Containment::CustomPanel)
    ...
    if (config()->isImmutable() || !KAuthorized::authorize(QStringLiteral("lingmo/lingmoshell/unlockedDesktop"))) {
        setImmutability(Lingmo::Types::SystemImmutable);
    } else {
        KConfigGroup coronaConfig(config(), u"General"_s);
        setImmutability((Lingmo::Types::ImmutabilityType)coronaConfig.readEntry("immutability", static_cast<int>(Lingmo::Types::Mutable)));
    }
}
...
void ShellCorona::screenInvariants() const
{
    ...
    if (m_desktopViewForScreen.isEmpty()) {
        qCWarning(LINGMOSHELL) << "no screens!!";
    }
}
#endif

void ShellCorona::showAlternativesForApplet(Lingmo::Applet *applet)
{
    ...
    auto *qmlObj = new LingmoQuick::SharedQmlEngine(this);
    ...
    if (!dialog) {
        qCWarning(LINGMOSHELL) << "Alternatives UI does not inherit from Dialog";
        delete qmlObj;
        return;
    }
    m_showingAlternatives = applet;
    connect(applet, &Lingmo::Applet::destroyedChanged, qmlObj, [qmlObj, this](bool destroyed) {
        if (!destroyed) {
            return;
        }
...
void ShellCorona::slotCyclePanelFocus()
{
    ...
    if (activePanel->containment()->status() != Lingmo::Types::AcceptingInputStatus) {
        activePanel->containment()->setStatus(Lingmo::Types::AcceptingInputStatus);
    } else {
        ...
        activePanel->containment()->setStatus(Lingmo::Types::PassiveStatus);
        ...
        if (activePanel != m_panelViews.last()) {
            ...
            if (viewIt != m_panelViews.cend()) {
                viewIt.value()->containment()->setStatus(Lingmo::Types::AcceptingInputStatus);
            } else {
...
void ShellCorona::loadDefaultLayout()
{
    ...
    if (!script.isEmpty()) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            qCDebug(LINGMOSHELL) << "evaluating pre-startup script:" << script;
    ...
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QString code = QString::fromLocal8Bit(file.readAll());
        qCDebug(LINGMOSHELL) << "evaluating startup script:" << script;
...
void ShellCorona::processUpdateScripts()
{
    ...
    for (const QString &script : scripts) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            qCWarning(LINGMOSHELL) << "Unable to open the script file" << script << "for reading";
        }
    }
#endif
}
...
QRect ShellCorona::screenGeometry(int id) const
{
    ...
    if (!screen) {
        qCWarning(LINGMOSHELL) << "requesting unexisting screen geometry" << id;
...
QRegion ShellCorona::_availableScreenRegion(int id) const
{
    ...
    if (!screen) {
        ...
        qCWarning(LINGMOSHELL) << "requesting unexisting screen region" << id;
        screen = m_screenPool->primaryScreen();
...
QRect ShellCorona::_availableScreenRect(int id) const
{
    ...
    if (!screen) {
        // each screen should have a view
        qCWarning(LINGMOSHELL) << "requesting unexisting screen available rect" << id;
        screen = m_screenPool->primaryScreen();
...
void ShellCorona::removeDesktop(DesktopView *desktopView)
{
    ...
    QMutableMapIterator<const Lingmo::Containment *, PanelView *> it(m_panelViews);
...
PanelView *ShellCorona::panelView(Lingmo::Containment *containment) const
{
...
#endif
    if (m_waylandWindowManagement && !m_previousLingmoWindow) {
        m_previousLingmoWindow = m_waylandWindowManagement->activeWindow();
    }
}
...
#endif
    if (m_previousLingmoWindow) {
        m_previousLingmoWindow->requestActivate();
    }
...
{
    m_previousWId = 0;
    m_previousLingmoWindow = nullptr;
}
...
void ShellCorona::addOutput(QScreen *screen)
{
    ...
    Lingmo::Containment *containment = createContainmentForActivity(m_activityController->currentActivity(), insertPosition);
    ...
    connect(containment, &Lingmo::Containment::uiReadyChanged, this, &ShellCorona::checkAllDesktopsUiReady);
...
void ShellCorona::checkAllDesktopsUiReady(bool ready)
{
    ...
    for (auto v : std::as_const(m_desktopViewForScreen)) {
        ...
        qCDebug(LINGMOSHELL) << "Lingmo Shell startup completed";
...
Lingmo::Containment *ShellCorona::createContainmentForActivity(const QString &activity, int screenNum)
{
    Lingmo::Containment *lastScreenCont = nullptr;
    Lingmo::Containment *orphanCont = nullptr;
    ...
    for (Lingmo::Containment *cont : containments) {
        ...
        // This piece of code always fails at startup, used only later in lingmo runtime
        if (cont->destroyed()) {
            continue;
        }
    ...
    Lingmo::Containment *containment = containmentForScreen(screenNum, activity, plugin, QVariantList());
    Q_ASSERT(containment);
...
{
    QList<Lingmo::Containment *> stillWaitingPanels;

    for (Lingmo::Containment *cont : std::as_const(m_waitingPanels)) {
...
{
    auto *cont = static_cast<Lingmo::Containment *>(obj);
...
void ShellCorona::handleContainmentAdded(Lingmo::Containment *c)
{
    connect(c, &Lingmo::Containment::showAddWidgetsInterface, this, &ShellCorona::toggleWidgetExplorer);
    connect(c, &Lingmo::Containment::appletAlternativesRequested, this, &ShellCorona::showAlternativesForApplet);
    connect(c, &Lingmo::Containment::appletCreated, this, [this, c](Lingmo::Applet *applet) {
        executeSetupPlasmoidScript(c, applet);
    });

    // When a containment is removed, remove the thumbnail as well
    connect(c, &Lingmo::Containment::destroyedChanged, this, [this, c](bool destroyed) {
        if (!destroyed) {
            return;
        }
...
void ShellCorona::executeSetupPlasmoidScript(Lingmo::Containment *containment, Lingmo::Applet *applet)
{
#if USE_SCRIPTING
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qCWarning(LINGMOSHELL) << "Unable to load script file:" << scriptFile;
        return;
    }

    QString script = QString::fromLocal8Bit(file.readAll());
    if (script.isEmpty()) {
        // qCDebug(LINGMOSHELL) << "script is empty";
        return;
    }
...
QVariantMap ShellCorona::wallpaper(uint screenNum)
{
    if (!m_desktopViewForScreen.contains(screenNum)) {
        qCWarning(LINGMOSHELL) << "wallpaper: unknown screen" << screenNum;
        return QVariantMap();
    }
    ...
    Lingmo::Containment *containment = containmentForScreen(screenNum, currentActivity, QString());
    if (!containment) {
        qCWarning(LINGMOSHELL) << "wallpaper: containment not found for screen" << screenNum << currentActivity;
        return QVariantMap();
    }
...
void ShellCorona::setWallpaper(const QString &wallpaperPlugin, const QVariantMap &parameters, uint screenNum)
{
    if (wallpaperPlugin.isEmpty()) {
        qCWarning(LINGMOSHELL) << "setWallpaper: Missing wallpaperPlugin parameter";
        return;
    }
    ...
    Lingmo::Containment *containment = containmentForScreen(screenNum, currentActivity, QString());
    if (!containment) {
        qCWarning(LINGMOSHELL) << "setWallpaper: containment not found for screen" << screenNum << currentActivity;
        return;
    }
    ...
    if (config) {
        ...
        for (const auto &itemName : items) {
            ...
            if (it != parameters.end()) {
                ...
                qCDebug(LINGMOSHELL) << "setWallpaper: setting" << itemName << it.value() << screenNum;
...
#if USE_SCRIPTING
    if (calledFromDBus()) {
        if (immutability() == Lingmo::Types::SystemImmutable) {
            sendErrorReply(QDBusError::Failed, QStringLiteral("Widgets are locked"));
            return QString();
        } else if (!KAuthorized::authorize(QStringLiteral("lingmo-desktop/scripting_console"))) {
...
void ShellCorona::checkActivities()
{
    ...
    // qCDebug(LINGMOSHELL) << "$%$%$#%$%$%Status:" << status;
    if (status != KActivities::Controller::Running) {
        // panic and give up - better than causing a mess
        qCDebug(LINGMOSHELL) << "ShellCorona::checkActivities is called whilst activity daemon is still connecting";
        return;
    }
    ...
    for (Lingmo::Containment *cont : conts) {
        if ((cont->containmentType() == Lingmo::Containment::Desktop || cont->containmentType() == Lingmo::Containment::Custom)
...
{
    //     qCDebug(LINGMOSHELL) << "Activity changed:" << newActivity;

    for (auto it = m_desktopViewForScreen.constBegin(); it != m_desktopViewForScreen.constEnd(); ++it) {
        Lingmo::Containment *c = createContainmentForActivity(newActivity, it.key());
...
void ShellCorona::activityAdded(const QString &id)
{
    ...
    if (m_activityContainmentPlugins.contains(id)) {
        qCWarning(LINGMOSHELL) << "Activity added twice" << id;
        return;
    }
...
void ShellCorona::activityRemoved(const QString &id)
{
    ...
    const QList<Lingmo::Containment *> containments = containmentsForActivity(id);
    for (auto cont : containments) {
        cont->destroy();
    }
}
...
void ShellCorona::insertActivity(const QString &id, const QString &plugin)
{
    ...
    if (!currentActivity.result()) {
        qCDebug(LINGMOSHELL) << "Failed to create and switch to the activity";
        return;
    }
    ...
    for (auto it = m_desktopViewForScreen.constBegin(); it != m_desktopViewForScreen.constEnd(); ++it) {
        Lingmo::Containment *c = createContainmentForActivity(id, (*it)->containment()->screen());
        if (c) {
...
Lingmo::Containment *ShellCorona::setContainmentTypeForScreen(int screen, const QString &plugin)
{
    // search but not create
    Lingmo::Containment *oldContainment = containmentForScreen(screen, m_activityController->currentActivity(), QString());
    ...
    Lingmo::Containment *newContainment = createContainmentDelayed(plugin);
    ...
    newContainment->updateConstraints(Lingmo::Applet::StartupCompletedConstraint);
    ...
    QTimer::singleShot(2500, oldContainment, &Lingmo::Applet::destroy);
...
void ShellCorona::checkAddPanelAction()
{
    ...
    const QList<KPluginMetaData> panelContainmentPlugins = Lingmo::PluginLoader::listContainmentsMetaDataOfType(QStringLiteral("Panel"));

    auto filter = [](const KPluginMetaData &md) -> bool {
        return !md.rawData().value(QStringLiteral("NoDisplay")).toBool()
            && md.value(QStringLiteral("X-Lingmo-ContainmentCategories"), QStringList()).contains(QLatin1String("panel"));
    };
    QList<KPluginMetaData> templates = KPackage::PackageLoader::self()->findPackages(QStringLiteral("Lingmo/LayoutTemplate"), QString(), filter);
...
// Used by the lingmo-desktop edit ToolBox
...
void ShellCorona::populateAddPanelsMenu()
{
    ...
    const QList<KPluginMetaData> templates = KPackage::PackageLoader::self()->findPackages(QStringLiteral("Lingmo/LayoutTemplate"), QString(), filter);
    for (const auto &tpl : templates) {
        sorted.insert(tpl.name(), qMakePair(emptyInfo, tpl));
    }

    QMapIterator<QString, QPair<KPluginMetaData, KPluginMetaData>> it(sorted);
    KPackage::Package package = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/LayoutTemplate"));
    while (it.hasNext()) {
        it.next();
        ...
        if (pair.first.isValid()) {
            ...
            if (!scriptFile.isEmpty()) {
                ...
                action->setData(QStringLiteral("lingmo-desktop-template:%1").arg(plugin.pluginId()));
            }
        }
    }
}
...
{
    const QList<KPluginMetaData> panelPlugins = Lingmo::PluginLoader::listContainmentsMetaDataOfType(QStringLiteral("Panel"));
...
#if USE_SCRIPTING
    if (plugin.startsWith(QLatin1String("lingmo-desktop-template:"))) {
        WorkspaceScripting::ScriptEngine scriptEngine(this);
        ...
        const QString templateName = plugin.right(plugin.length() - qstrlen("lingmo-desktop-template:"));
...
Lingmo::Containment *ShellCorona::addPanel(const QString &plugin)
{
    Lingmo::Containment *panel = createContainment(plugin);
    if (!panel) {
        return nullptr;
    }
    ...
    if (!wantedScreen) {
        qCWarning(LINGMOSHELL) << "Did not find a valid screen to place a new panel." << reply.error();
        wantedScreen = qGuiApp->focusWindow() ? qGuiApp->focusWindow()->screen() : m_screenPool->primaryScreen();
    }

    QList<Lingmo::Types::Location> availableLocations;
    availableLocations << Lingmo::Types::BottomEdge << Lingmo::Types::TopEdge << Lingmo::Types::LeftEdge << Lingmo::Types::RightEdge;
    ...
    Lingmo::Types::Location loc;
    if (availableLocations.isEmpty()) {
        loc = Lingmo::Types::TopEdge;
    } else {
    ...
    case Lingmo::Types::LeftEdge:
    case Lingmo::Types::RightEdge:
        panel->setFormFactor(Lingmo::Types::Vertical);
        break;
    default:
        panel->setFormFactor(Lingmo::Types::Horizontal);
        break;
    }
...
void ShellCorona::swapDesktopScreens(int oldScreen, int newScreen)
{
    for (auto *containment : containmentsForScreen(oldScreen)) {
        if (containment->containmentType() != Lingmo::Containment::Panel && containment->containmentType() != Lingmo::Containment::CustomPanel) {
            setScreenForContainment(containment, newScreen);
        }
    }
}

void ShellCorona::setScreenForContainment(Lingmo::Containment *containment, int newScreenId)
{
    ...
    if (containment->containmentType() == Lingmo::Containment::Panel || containment->containmentType() == Lingmo::Containment::CustomPanel) {
        ...
        Lingmo::Containment *contSwap = containmentForScreen(newScreenId, containment->activity(), u"org.kde.lingmo.folder"_s);
        Q_ASSERT(contSwap);
...
int ShellCorona::screenForContainment(const Lingmo::Containment *containment) const
{
    ...
    if (Lingmo::Applet *parentApplet = qobject_cast<Lingmo::Applet *>(containment->parent())) {
        if (Lingmo::Containment *cont = parentApplet->containment()) {
...
void ShellCorona::grabContainmentPreview(Lingmo::Containment *containment)
{
    ...
    if (viewToGrab) {
        ...
        if (result) {
            connect(result.data(), &QQuickItemGrabResult::ready, this, [this, result, containment]() {
                // DataLocation is lingmoshell, we need just "lingmo"
                QString path = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation);
                QDir dir(path);
                path += u"/lingmo/containmentpreviews/";
                dir.mkpath(path);
...
QString ShellCorona::containmentPreviewPath(Lingmo::Containment *containment) const
{
    const QString path = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + u"/lingmo/containmentpreviews/"
 ...
 *
 * lingmoshell uses both QtQuick and QtWidgets under a single roof, QtQuick is
 ...
 * application uses solely Qt widgets. See QTBUG-83972. For lingmo it doesn't
...
void ShellCorona::setupWaylandIntegration()
{
    ...
    connect(registry, &KWayland::Client::Registry::lingmoWindowManagementAnnounced, this, [this, registry](quint32 name, quint32 version) {
        m_waylandWindowManagement = registry->createLingmoWindowManagement(name, version, this);
    });
    registry->setup();
...
{
    auto activateLauncher = [](Lingmo::Applet *applet) -> bool {
        const auto provides = applet->pluginMetaData().value(QStringLiteral("X-Lingmo-Provides"), QStringList());
    ...
    for (auto *cont : containments()) {
        if ((screenId < 0 || cont->screen() == screenId)
            && (cont->containmentType() == Lingmo::Containment::Panel || cont->containmentType() == Lingmo::Containment::CustomPanel)) {
            const auto applets = cont->applets();
    ...
    for (auto *cont : containments()) {
        if ((screenId < 0 || cont->screen() == screenId) && cont->containmentType() == Lingmo::Containment::Desktop) {
            const auto applets = cont->applets();
            for (auto applet : applets) {
                if (activateLauncher(applet)) {
                    return;
...
{
    auto activateTaskManagerEntryOnContainment = [](const Lingmo::Containment *c, int index) {
        const auto &applets = c->applets();
        for (auto *applet : applets) {
            const auto &provides = applet->pluginMetaData().value(QStringLiteral("X-Lingmo-Provides"), QStringList());
            if (provides.contains(QLatin1String("org.kde.lingmo.multitasking"))) {
                if (QQuickItem *appletInterface = LingmoQuick::AppletQuickItem::itemForApplet(applet)) {
...
QString ShellCorona::defaultShell()
{
    ...
    const QString defaultValue = qEnvironmentVariable("LINGMO_DEFAULT_SHELL", u"org.kde.lingmo.desktop"_s);
...
{
    KSharedConfig::openConfig(QStringLiteral("lingmoshellrc"))->reparseConfiguration();
    //  FIXME:   setShell(defaultShell());
    QProcess::startDetached(u"lingmoshell"_s, {u"--replace"_s});
}
