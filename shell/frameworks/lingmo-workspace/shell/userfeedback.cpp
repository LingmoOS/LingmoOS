...
{
    m_provider->setProductIdentifier(QStringLiteral("org.kde.lingmoshell"));
    ...
    auto lingmoConfig = KSharedConfig::openConfig(QStringLiteral("LingmoUserFeedback"));
    m_provider->setTelemetryMode(KUserFeedback::Provider::TelemetryMode(
        lingmoConfig->group(QStringLiteral("Global")).readEntry("FeedbackLevel", int(KUserFeedback::Provider::NoTelemetry))));
}
