...
PanelView::PanelView(ShellCorona *corona, QScreen *targetScreen, QWindow *parent)
    : LingmoQuick::ContainmentView(corona, parent)
    , m_offset(0)
    , m_maxLength(0)
    , m_minLength(0)
    , m_contentLength(0)
    ...
    , m_backgroundHints(Lingmo::Types::StandardBackground)
{
    ...
    connect(&m_theme, &Lingmo::Theme::themeChanged, this, &PanelView::updateMask);
    connect(&m_theme, &Lingmo::Theme::themeChanged, this, &PanelView::updateAdaptiveOpacityEnabled);
    ...
    connect(m_corona, &Lingmo::Corona::editModeChanged, this, &PanelView::restoreAutoHide);

    m_lastScreen = targetScreen;
    connect(this, &PanelView::containmentChanged, this, &PanelView::refreshContainment);
    connect(this, &PanelView::locationChanged, this, [this](Lingmo::Types::Location location) {
        switch (location) {
        case Lingmo::Types::LeftEdge:
        case Lingmo::Types::RightEdge:
            containment()->setFormFactor(Lingmo::Types::Vertical);
            break;
        case Lingmo::Types::TopEdge:
        case Lingmo::Types::BottomEdge:
            containment()->setFormFactor(Lingmo::Types::Horizontal);
        }
        restore();
    });

    if (!m_corona->kPackage().isValid()) {
        qCWarning(LINGMOSHELL) << "Invalid home screen package";
    }

    m_strutsTimer.setSingleShot(true);
    connect(&m_strutsTimer, &QTimer::timeout, this, &PanelView::updateExclusiveZone);

    connect(m_corona, &Lingmo::Corona::editModeChanged, this, &PanelView::updateEditModeLabel);

    // Register enums
    qmlRegisterUncreatableMetaObject(PanelView::staticMetaObject, "org.kde.lingmo.shell.panel", 0, 1, "Global", QStringLiteral("Error: only enums"));
...
KConfigGroup PanelView::panelConfig(ShellCorona *corona, Lingmo::Containment *containment, QScreen *screen)
{
    if (!containment || !screen) {
        return KConfigGroup();
    }
    KConfigGroup views(corona->applicationConfig(), u"LingmoViews"_s);
    views = KConfigGroup(&views, QStringLiteral("Panel %1").arg(containment->id()));

    if (containment->formFactor() == Lingmo::Types::Vertical) {
...
KConfigGroup PanelView::panelConfigDefaults(ShellCorona *corona, Lingmo::Containment *containment, QScreen *screen)
{
    if (!containment || !screen) {
        return KConfigGroup();
    }
...
void PanelView::setOffset(int offset)
{
    ...
    if (formFactor() == Lingmo::Types::Vertical) {
...
int PanelView::minThickness() const
{
    ...
    } else if (formFactor() == Lingmo::Types::Horizontal) {
...
Lingmo::Types::BackgroundHints PanelView::backgroundHints() const
{
    return m_backgroundHints;
}

void PanelView::setBackgroundHints(Lingmo::Types::BackgroundHints hint)
{
...
void PanelView::setVisibilityMode(PanelView::VisibilityMode mode)
{
    ...
    disconnect(containment(), &Lingmo::Applet::activated, this, &PanelView::showTemporarily);
    if (edgeActivated()) {
        connect(containment(), &Lingmo::Applet::activated, this, &PanelView::showTemporarily);
    }
...
void PanelView::updateLayerWindow()
{
    ...
    if (formFactor() == Lingmo::Types::Horizontal) {
        switch (m_alignment) {
        case Qt::AlignLeft:
            anchors.setFlag(LayerShellQt::Window::AnchorLeft);
...
QRect PanelView::geometryByDistance(int distance) const
{
    ...
    QRect r(QPoint(0, 0),
            formFactor() == Lingmo::Types::Vertical ? QSize(totalThickness(), preferredSize().height()) : QSize(preferredSize().width(), totalThickness()));
...
void PanelView::restore()
{
    ...
    // the place for this config key is changed in Lingmo 5.9
    ...
    // is safe to directly write during lingmo startup, as there can be
    // resolution changes
    ...
    const int side = containment()->formFactor() == Lingmo::Types::Vertical ? screenSize.height() : screenSize.width();
...
void PanelView::showConfigurationInterface(Lingmo::Applet *applet)
{
    if (!applet || !applet->containment()) {
        return;
    }

    Lingmo::Containment *cont = qobject_cast<Lingmo::Containment *>(applet);
    ...
    if (isPanelConfig) {
        if (m_panelConfigView && m_panelConfigView->isVisible()) {
            ...
            auto positionConfigView = [this]() {
                QQuickItem *contObject = LingmoQuick::AppletQuickItem::itemForApplet(containment());
                QQuickItem *tb = contObject->property("toolBox").value<QQuickItem *>();
                if (tb && containment()->formFactor() != Lingmo::Types::Vertical) {
        ...
        m_appletConfigView = new LingmoQuick::ConfigView(applet);
...
void PanelView::restoreAutoHide()
{
    ...
    if (!edgeActivated()) {
        ...
        autoHide = false;
    } else if (containment() && containment()->status() >= Lingmo::Types::NeedsAttentionStatus && containment()->status() != Lingmo::Types::HiddenStatus) {
        autoHide = false;
    } else {
...
void PanelView::resizeEvent(QResizeEvent *ev)
{
    ...
    LingmoQuick::ContainmentView::resizeEvent(ev);
}
...
void PanelView::moveEvent(QMoveEvent *ev)
{
    updateEnabledBorders();
    m_strutsTimer.start(STRUTSTIMERDELAY);
    LingmoQuick::ContainmentView::moveEvent(ev);
...
void PanelView::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->key() == Qt::Key_Escape) {
        if (containment()->status() == Lingmo::Types::AcceptingInputStatus) {
            containment()->setStatus(Lingmo::Types::PassiveStatus);
        }
        // No return for Wayland
    }

    LingmoQuick::ContainmentView::keyPressEvent(event);
    if (event->isAccepted()) {
        return;
    }
...
{
    LingmoQuick::ContainmentView::showEvent(event);
...
{
    QQuickItem *containmentItem = LingmoQuick::AppletQuickItem::itemForApplet(containment());
...
void PanelView::updateMask()
{
    ...
    if (m_backgroundHints == Lingmo::Types::NoBackground) {
...
{
    restore();
    connect(containment(), &Lingmo::Containment::userConfiguringChanged, this, [this](bool configuring) {
        if (configuring) {
            showTemporarily();
        } else {
            m_unhideTimer.start();
    ...
    connect(containment(), &Lingmo::Applet::statusChanged, this, &PanelView::refreshStatus);
    connect(containment(), &Lingmo::Applet::appletDeleted, this, [this] {
        ...
        if (containment()->destroyed()) {
            KConfigGroup views(m_corona->applicationConfig(), u"LingmoViews"_s);
    ...
    connect(containment(), &Lingmo::Applet::userConfiguringChanged, this, &PanelView::updateExclusiveZone);
    ...
    connect(this, &PanelView::activeFocusItemChanged, containment(), [this] {
        if (containment()->status() == Lingmo::Types::AcceptingInputStatus && !activeFocusItem()) {
            // BUG 454729: avoid switching to PassiveStatus in keyboard navigation
            containment()->setStatus(Lingmo::Types::ActiveStatus);
        }
    });
}
...
void PanelView::refreshStatus(Lingmo::Types::ItemStatus status)
{
    if (status == Lingmo::Types::NeedsAttentionStatus) {
        showTemporarily();
    ...
    } else if (status == Lingmo::Types::AcceptingInputStatus) {
        ...
        if (status == Lingmo::Types::PassiveStatus) {
            m_corona->restorePreviousWindow();
        } else if (status == Lingmo::Types::ActiveStatus) {
