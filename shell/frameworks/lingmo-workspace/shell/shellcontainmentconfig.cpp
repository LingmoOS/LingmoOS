    ...
    , m_corona(corona)
{
    ...
    connect(m_corona, &Lingmo::Corona::screenAdded, m_reloadTimer, static_cast<void (QTimer::*)()>(&QTimer::start));
    connect(m_corona, &Lingmo::Corona::screenRemoved, m_reloadTimer, static_cast<void (QTimer::*)()>(&QTimer::start));
}
...
void ScreenPoolModel::load()
{
    ...
    for (auto *cont : m_corona->containments()) {
        connect(cont, &Lingmo::Containment::destroyedChanged, this, &ScreenPoolModel::load, Qt::UniqueConnection);
        if (!cont->destroyed()) {
    ...
    , m_activityConsumer(new KActivities::Consumer(this))
{
    ...
    connect(m_corona, &Lingmo::Corona::containmentAdded, m_reloadTimer, static_cast<void (QTimer::*)()>(&QTimer::start));
    connect(m_corona, &Lingmo::Corona::screenOwnerChanged, m_reloadTimer, static_cast<void (QTimer::*)()>(&QTimer::start));

    connect(m_corona, &ShellCorona::containmentPreviewReady, this, [this](Lingmo::Containment *containment, const QString &path) {
        int i = 0;
...
QVariant ShellContainmentModel::data(const QModelIndex &index, int role) const
{
    ...
    case ScreenRole:
        return d.screen;
    case EdgeRole:
        return ShellContainmentModel::lingmoLocationToString(d.edge);
    case EdgePositionRole:
        return qMax(0, m_edgeCount.value(d.screen).value(d.edge).indexOf(d.id));
    case PanelCountAtRightRole:
        return qMax(0, m_edgeCount.value(d.screen).value(Lingmo::Types::RightEdge).count());
    case PanelCountAtTopRole:
        return qMax(0, m_edgeCount.value(d.screen).value(Lingmo::Types::TopEdge).count());
    case PanelCountAtLeftRole:
        return qMax(0, m_edgeCount.value(d.screen).value(Lingmo::Types::LeftEdge).count());
    case PanelCountAtBottomRole:
        return qMax(0, m_edgeCount.value(d.screen).value(Lingmo::Types::BottomEdge).count());
    case ActivityRole: {
...
void ShellContainmentModel::moveContainementToScreen(unsigned int contId, int newScreen)
{
    ...
    if (cont->containmentType() == Lingmo::Containment::Panel || cont->containmentType() == Lingmo::Containment::CustomPanel) {
...
void ShellContainmentModel::load()
{
    ...
    for (const auto *cont : m_corona->containments()) {
        // Skip the systray
        if (qobject_cast<Lingmo::Applet *>(cont->parent())) {
            continue;
        }
        ...
        if (!m_edgeCount.contains(cont->lastScreen())) {
            m_edgeCount[cont->lastScreen()] = QHash<Lingmo::Types::Location, QList<int>>();
            m_edgeCount[cont->lastScreen()][cont->location()] = QList<int>();
        }
        m_edgeCount[cont->lastScreen()][cont->location()].append(cont->id());
        m_corona->grabContainmentPreview(const_cast<Lingmo::Containment *>(cont));
        Data d;
        d.id = cont->id();
        ...
        d.containment = cont;
        d.image = containmentPreview(const_cast<Lingmo::Containment *>(cont));
        ...
        if (cont->lastScreen() == m_screenId || (cont->lastScreen() == -1 && cont->screen() == m_screenId)) {
            ...
            connect(cont, &Lingmo::Containment::destroyedChanged, this, &ShellContainmentModel::load);
            connect(cont, &Lingmo::Containment::locationChanged, this, &ShellContainmentModel::load);
        }
    }
    endResetModel();
}
...
QString ShellContainmentModel::lingmoLocationToString(Lingmo::Types::Location location)
{
    switch (location) {
...
Lingmo::Containment *ShellContainmentModel::containmentById(unsigned int id)
{
    for (auto *cont : m_corona->containments()) {
        if (cont->id() == id) {
            return cont;
...
QString ShellContainmentModel::containmentPreview(Lingmo::Containment *containment)
{
    ...
    KSharedConfig::Ptr conf = KSharedConfig::openConfig(QLatin1String("lingmo-") + m_corona->shell() + QLatin1String("-appletsrc"), KConfig::SimpleConfig);
...
void ShellContainmentConfig::init()
{
    ...
    localizedContext->setTranslationDomain(u"lingmo_shell_"_s + m_corona->shell());
