...
DesktopView::DesktopView(Lingmo::Corona *corona, QScreen *targetScreen)
    : LingmoQuick::ContainmentView(corona, nullptr)
    , m_accentColor(Qt::transparent)
{
    ...
    QObject::connect(corona, &Lingmo::Corona::kPackageChanged, this, &DesktopView::coronaPackageChanged);
...
{
    // Lingmo 6 pre-release versions
    if constexpr (PROJECT_VERSION_MAJOR == 5 && PROJECT_VERSION_MINOR >= 80) {
        if constexpr (PROJECT_VERSION_PATCH == 80) {
            // Development
            return i18nc("@label %1 is the Lingmo version", "KDE Lingmo 6.0 Dev");
        } else if constexpr (PROJECT_VERSION_MINOR == 80) {
            // Alpha, 5.80.0
            return i18nc("@label %1 is the Lingmo version", "KDE Lingmo 6.0 Alpha");
        } else if constexpr (PROJECT_VERSION_MINOR == 90) {
            // Beta 1, 5.90.0
            return i18nc("@label %1 is the Lingmo version", "KDE Lingmo 6.0 Beta 1");
        } else if constexpr (PROJECT_VERSION_MINOR == 91) {
            // Beta 2, 5.91.0
            return i18nc("@label %1 is the Lingmo version", "KDE Lingmo 6.0 Beta 2");
        } else if constexpr (PROJECT_VERSION_MINOR == 92) {
            // RC1, 5.92.0
            return i18nc("@label %1 is the Lingmo version, RC meaning Release Candidate", "KDE Lingmo 6.0 RC1");
        } else if constexpr (PROJECT_VERSION_MINOR == 93) {
            // RC2, 5.93.0
            return i18nc("@label %1 is the Lingmo version, RC meaning Release Candidate", "KDE Lingmo 6.0 RC2");
        }
    }

    /*
     * Versions are reported as follows:
     *  Development, 5.27.80 -> KDE Lingmo 6.0 Dev
     *  Beta,        5.27.90 -> KDE Lingmo 6.0 Beta
     *  Development, 6.0.80  -> KDE Lingmo 6.1 Dev
     *  Beta,        6.0.90  -> KDE Lingmo 6.1 Beta
     *  Beta,        6.0.91  -> KDE Lingmo 6.1 Beta 2
     */

    // finalMajor, finalMinor is the final version in the line and
    // should be updated after the final Lingmo 6 release
    ...
    if constexpr (PROJECT_VERSION_PATCH == 80) {
        // Development version
        return i18nc("@label %1 is the Lingmo version", "KDE Lingmo %1 Dev", version);
    } else if constexpr (PROJECT_VERSION_PATCH >= 90) {
        // Beta version
        if constexpr (PROJECT_VERSION_PATCH == 90) {
            return i18nc("@label %1 is the Lingmo version", "KDE Lingmo %1 Beta", version);
        } else {
            constexpr int betaNumber = PROJECT_VERSION_PATCH - 89;
            return i18nc("@label %1 is the Lingmo version, %2 is the beta release number", "KDE Lingmo %1 Beta %2", version, betaNumber);
        }
    } else {
        // Unrecognised version
        return i18nc("@label %1 is the Lingmo version", "KDE Lingmo %1", WORKSPACE_VERSION_STRING);
    }
}
...
void DesktopView::showPreviewBannerMenu(const QPoint &pos)
{
    ...
    QAction *copyVersionAction = new QAction(QIcon::fromTheme(u"edit-copy-symbolic"_s), i18nc("@action:button", "Copy Lingmo Version"));
...
QVariantMap DesktopView::candidateContainmentsGraphicItems() const
{
    ...
    for (auto cont : corona()->containmentsForScreen(containment()->screen())) {
        map[cont->activity()] = QVariant::fromValue(LingmoQuick::AppletQuickItem::itemForApplet(cont));
    }
    return map;
}
...
bool DesktopView::event(QEvent *e)
{
    ...
    return LingmoQuick::ContainmentView::event(e);
}
...
void DesktopView::showConfigurationInterface(Lingmo::Applet *applet)
{
    if (m_configView) {
    ...
    if (cont && cont->isContainment() && cont->containmentType() == Lingmo::Containment::Desktop) {
        ...
        connect(this, &ContainmentView::containmentChanged, m_configView.data(), [this]() {
            if (containment()->property("wallpaperGraphicsObject").value<QObject *>()) {
                ...
                connect(containment(), &Lingmo::Containment::wallpaperGraphicsObjectChanged, this, [this] {
                    disconnect(static_cast<Lingmo::Containment *>(sender()), &Lingmo::Containment::wallpaperGraphicsObjectChanged, this, nullptr);
                    showConfigurationInterface(static_cast<Lingmo::Containment *>(sender()));
                });
            }
        });
    } else {
...
{
    if (m_containment) {
        disconnect(m_containment, &Lingmo::Containment::screenChanged, this, &DesktopView::slotScreenChanged);
    }

    m_containment = containment();

    if (m_containment) {
        connect(m_containment, &Lingmo::Containment::screenChanged, this, &DesktopView::slotScreenChanged);
...
void DesktopView::setAccentColorFromWallpaper(const QColor &accentColor)
{
    ...
    QDBusMessage applyAccentColor = QDBusMessage::createMethodCall(u"org.kde.lingmoshell.accentColor"_s, u"/AccentColor"_s, QString(), u"setAccentColor"_s);
