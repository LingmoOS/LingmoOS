...
class LingmoWindow : public QObject, public QtWayland::org_kde_lingmo_window
{
    Q_OBJECT
public:
    LingmoWindow(const QString &uuid, ::org_kde_lingmo_window *id)
        : org_kde_lingmo_window(id)
        , uuid(uuid)
    {
    }
    ~LingmoWindow()
    ...
    using state = QtWayland::org_kde_lingmo_window_management::state;
    const QString uuid;
    QString title;
    QString appId;
    QIcon icon;
...
protected:
    void org_kde_lingmo_window_unmapped() override
    {
        wasUnmapped = true;
        Q_EMIT unmapped();
    }
    void org_kde_lingmo_window_title_changed(const QString &title) override
    {
        this->title = title;
        Q_EMIT titleChanged();
    }
    void org_kde_lingmo_window_app_id_changed(const QString &app_id) override
    {
        appId = app_id;
        Q_EMIT appIdChanged();
    }
    void org_kde_lingmo_window_icon_changed() override
    {
        int pipeFds[2];
    ...
    void org_kde_lingmo_window_themed_icon_name_changed(const QString &name) override
    {
        icon = QIcon::fromTheme(name);
        Q_EMIT iconChanged();
    }
    void org_kde_lingmo_window_state_changed(uint32_t flags) override
    {
    ...
    void org_kde_lingmo_window_virtual_desktop_entered(const QString &id) override
    {
        virtualDesktops.push_back(id);
        Q_EMIT virtualDesktopEntered();
    }

    void org_kde_lingmo_window_virtual_desktop_left(const QString &id) override
    {
        virtualDesktops.removeAll(id);
        Q_EMIT virtualDesktopLeft();
    }
    void org_kde_lingmo_window_geometry(int32_t x, int32_t y, uint32_t width, uint32_t height) override
    {
        geometry = QRect(x, y, width, height);
        Q_EMIT geometryChanged();
    }
    void org_kde_lingmo_window_application_menu(const QString &service_name, const QString &object_path) override
    ...
    void org_kde_lingmo_window_activity_entered(const QString &id) override
    {
        activities.push_back(id);
        Q_EMIT activitiesChanged();
    }
    void org_kde_lingmo_window_activity_left(const QString &id) override
    {
        activities.removeAll(id);
        Q_EMIT activitiesChanged();
    }
    void org_kde_lingmo_window_pid_changed(uint32_t pid) override
    {
        this->pid = pid;
    }
    void org_kde_lingmo_window_resource_name_changed(const QString &resource_name) override
    {
        resourceName = resource_name;
    }
    void org_kde_lingmo_window_parent_window(::org_kde_lingmo_window *parent) override
    {
        LingmoWindow *parentWindow = nullptr;
        if (parent) {
            parentWindow = dynamic_cast<LingmoWindow *>(LingmoWindow::fromObject(parent));
        }
        setParentWindow(parentWindow);
    }
    void org_kde_lingmo_window_initial_state() override
    {
        Q_EMIT initialStateDone();
    }

private:
    void setParentWindow(LingmoWindow *parent)
    {
        ...
        if (parent && !parent->wasUnmapped) {
            ...
            parentWindowUnmappedConnection = QObject::connect(parent, &LingmoWindow::unmapped, this, [this] {
                setParentWindow(nullptr);
            });
            // QPointer nulling itself wouldn't cause the change signal to be emitted.
            parentWindowDestroyedConnection = QObject::connect(parent, &QObject::destroyed, this, &LingmoWindow::parentWindowChanged);
        } else {
...
class LingmoStackingOrder;

class LingmoWindowManagement : public QWaylandClientExtensionTemplate<LingmoWindowManagement>, public QtWayland::org_kde_lingmo_window_management
{
    Q_OBJECT
public:
    static constexpr int s_version = 17;
    LingmoWindowManagement()
        : QWaylandClientExtensionTemplate(s_version)
    {
        connect(this, &QWaylandClientExtension::activeChanged, this, [this] {
            ...
            } else if (QtWayland::org_kde_lingmo_window_management::version() >= ORG_KDE_LINGMO_WINDOW_MANAGEMENT_GET_STACKING_ORDER_SINCE_VERSION) {
                // fetch the stacking order
                org_kde_lingmo_window_management_stacking_order_changed_2();
            }
        });
        initialize();
    }
    ...
    void org_kde_lingmo_window_management_window_with_uuid(uint32_t id, const QString &uuid) override
    {
        Q_UNUSED(id)
        Q_EMIT windowCreated(new LingmoWindow(uuid, get_window_by_uuid(uuid)));
    }
    void org_kde_lingmo_window_management_stacking_order_uuid_changed(const QString &uuids) override
    {
        Q_EMIT stackingOrderChanged(uuids.split(QLatin1Char(';')));
    }
    void org_kde_lingmo_window_management_stacking_order_changed_2() override;
Q_SIGNALS:
    void windowCreated(LingmoWindow *window);
    void stackingOrderChanged(const QList<QString> &uuids);
};

class LingmoStackingOrder : public QtWayland::org_kde_lingmo_stacking_order
{
public:
    explicit LingmoStackingOrder(LingmoWindowManagement *windowManagement, ::org_kde_lingmo_stacking_order *id)
        : QtWayland::org_kde_lingmo_stacking_order(id)
        , m_windowManagement(windowManagement)
    {
    }
    ~LingmoStackingOrder()
    {
        org_kde_lingmo_stacking_order_destroy(object());
    }

    void org_kde_lingmo_stacking_order_window(const QString &uuid) override
    {
        m_uuids.push_back(uuid);
    }

    void org_kde_lingmo_stacking_order_done() override
    {
        Q_EMIT m_windowManagement->stackingOrderChanged(m_uuids);
        delete this;
    }

    LingmoWindowManagement *const m_windowManagement;
    QList<QString> m_uuids;
};

void LingmoWindowManagement::org_kde_lingmo_window_management_stacking_order_changed_2()
{
    new LingmoStackingOrder(this, org_kde_lingmo_window_management_get_stacking_order(object()));
}
...
public:
    ...
    QHash<LingmoWindow *, AppData> appDataCache;
    QHash<LingmoWindow *, QTime> lastActivated;
    LingmoWindow *activeWindow = nullptr;
    std::vector<std::unique_ptr<LingmoWindow>> windows;
    // key=transient child, value=leader
    QHash<LingmoWindow *, LingmoWindow *> transients;
    // key=leader, values=transient children
    QMultiHash<LingmoWindow *, LingmoWindow *> transientsDemandingAttention;
    std::unique_ptr<LingmoWindowManagement> windowManagement;
    ...
    void init();
    void initWayland();
    auto findWindow(LingmoWindow *window) const;
    void addWindow(LingmoWindow *window);

    const AppData &appData(LingmoWindow *window);

    QIcon icon(LingmoWindow *window);

    static QString mimeType();
    static QString groupMimeType();

    void dataChanged(LingmoWindow *window, int role);
    void dataChanged(LingmoWindow *window, const QList<int> &roles);
...
void WaylandTasksModel::Private::initWayland()
{
    ...
    windowManagement = std::make_unique<LingmoWindowManagement>();

    QObject::connect(windowManagement.get(), &LingmoWindowManagement::activeChanged, q, [this] {
        q->beginResetModel();
        windows.clear();
        q->endResetModel();
    });

    QObject::connect(windowManagement.get(), &LingmoWindowManagement::windowCreated, q, [this](LingmoWindow *window) {
        connect(window, &LingmoWindow::initialStateDone, q, [this, window] {
            addWindow(window);
        });
    });

    QObject::connect(windowManagement.get(), &LingmoWindowManagement::stackingOrderChanged, q, [this](const QList<QString> &order) {
        stackingOrder = order;
...
auto WaylandTasksModel::Private::findWindow(LingmoWindow *window) const
{
    return std::find_if(windows.begin(), windows.end(), [window](const std::unique_ptr<LingmoWindow> &candidate) {
        return candidate.get() == window;
    });
}

void WaylandTasksModel::Private::addWindow(LingmoWindow *window)
{
    ...
    auto removeWindow = [window, this] {
        ...
        auto it = std::find_if(windows.begin(), windows.end(), [window](const std::unique_ptr<LingmoWindow> &candidate) {
            return candidate.get() == window;
        });
        ...
        if (it != windows.end()) {
            ...
            const std::unique_ptr<LingmoWindow> removedWindow = std::move(*it);
            windows.erase(it);
            ...
            if (transients.remove(window)) {
                if (LingmoWindow *leader = transientsDemandingAttention.key(window)) {
    ...
    QObject::connect(window, &LingmoWindow::iconChanged, q, [window, this] {
        // The icon in the AppData struct might come from LingmoWindow if it wasn't
    ...
    if (window->windowState & LingmoWindow::state::state_active) {
        LingmoWindow *effectiveActive = window;
    ...
    QObject::connect(window, &LingmoWindow::activeChanged, q, [window, this] {
        const bool active = window->windowState & LingmoWindow::state::state_active;

        LingmoWindow *effectiveWindow = window;
    ...
    QObject::connect(window, &LingmoWindow::parentWindowChanged, q, [window, this] {
        LingmoWindow *leader = window->parentWindow.data();

        // Migrate demanding attention to new leader.
        if (window->windowState.testFlag(LingmoWindow::state::state_demands_attention)) {
    ...
    QObject::connect(window, &LingmoWindow::virtualDesktopEntered, q, [window, this] {
    ...
    QObject::connect(window, &LingmoWindow::virtualDesktopLeft, q, [window, this] {
        this->dataChanged(window, VirtualDesktops);
    ...
    QObject::connect(window, &LingmoWindow::applicationMenuChanged, q, [window, this] {
        this->dataChanged(window, QList<int>{ApplicationMenuServiceName, ApplicationMenuObjectPath});
    });

    QObject::connect(window, &LingmoWindow::activitiesChanged, q, [window, this] {
        this->dataChanged(window, Activities);
    });

    // Handle transient.
    if (LingmoWindow *leader = window->parentWindow.data()) {
        transients.insert(window, leader);
...
const AppData &WaylandTasksModel::Private::appData(LingmoWindow *window)
{
...
QIcon WaylandTasksModel::Private::icon(LingmoWindow *window)
{
...
void WaylandTasksModel::Private::dataChanged(LingmoWindow *window, int role)
{
    auto it = findWindow(window);
    if (it == windows.end()) {
        return;
...
void WaylandTasksModel::Private::dataChanged(LingmoWindow *window, const QList<int> &roles)
{
    auto it = findWindow(window);
    if (it == windows.end()) {
        return;
...
QVariant WaylandTasksModel::data(const QModelIndex &index, int role) const
{
    ...
    LingmoWindow *window = d->windows.at(index.row()).get();
    ...
    if (role == Qt::DisplayRole) {
        ...
        return window->windowState.testFlag(LingmoWindow::state::state_closeable);
    } else if (role == IsMovable) {
        return window->windowState.testFlag(LingmoWindow::state::state_movable);
    } else if (role == IsResizable) {
        return window->windowState.testFlag(LingmoWindow::state::state_resizable);
    } else if (role == IsMaximizable) {
        return window->windowState.testFlag(LingmoWindow::state::state_maximizable);
    } else if (role == IsMaximized) {
        return window->windowState.testFlag(LingmoWindow::state::state_maximized);
    } else if (role == IsMinimizable) {
        return window->windowState.testFlag(LingmoWindow::state::state_minimizable);
    } else if (role == IsMinimized || role == IsHidden) {
        return window->windowState.testFlag(LingmoWindow::state::state_minimized);
    } else if (role == IsKeepAbove) {
        return window->windowState.testFlag(LingmoWindow::state::state_keep_above);
    } else if (role == IsKeepBelow) {
        return window->windowState.testFlag(LingmoWindow::state::state_keep_below);
    } else if (role == IsFullScreenable) {
        return window->windowState.testFlag(LingmoWindow::state::state_fullscreenable);
    } else if (role == IsFullScreen) {
        return window->windowState.testFlag(LingmoWindow::state::state_fullscreen);
    } else if (role == IsShadeable) {
        return window->windowState.testFlag(LingmoWindow::state::state_shadeable);
    } else if (role == IsShaded) {
        return window->windowState.testFlag(LingmoWindow::state::state_shaded);
    } else if (role == IsVirtualDesktopsChangeable) {
        return window->windowState.testFlag(LingmoWindow::state::state_virtual_desktop_changeable);
        ...
        return window->windowState.testFlag(LingmoWindow::state::state_demands_attention) || d->transientsDemandingAttention.contains(window);
    } else if (role == SkipTaskbar) {
        return window->windowState.testFlag(LingmoWindow::state::state_skiptaskbar) || d->appData(window).skipTaskbar;
...
void WaylandTasksModel::requestActivate(const QModelIndex &index)
{
    ...
    window->set_state(LingmoWindow::state::state_active, LingmoWindow::state::state_active);
}
...
void WaylandTasksModel::requestToggleMinimized(const QModelIndex &index)
{
    ...
    if (window->windowState & LingmoWindow::state::state_minimized) {
        window->set_state(LingmoWindow::state::state_minimized, 0);
    } else {
        window->set_state(LingmoWindow::state::state_minimized, LingmoWindow::state::state_minimized);
    }
}
...
void WaylandTasksModel::requestToggleMaximized(const QModelIndex &index)
{
    ...
    if (window->windowState & LingmoWindow::state::state_maximized) {
        window->set_state(LingmoWindow::state::state_maximized | LingmoWindow::state::state_active, LingmoWindow::state::state_active);
    } else {
        window->set_state(LingmoWindow::state::state_maximized | LingmoWindow::state::state_active,
                          LingmoWindow::state::state_maximized | LingmoWindow::state::state_active);
    }
}
...
void WaylandTasksModel::requestToggleKeepAbove(const QModelIndex &index)
{
    ...
    if (window->windowState & LingmoWindow::state::state_keep_above) {
        window->set_state(LingmoWindow::state::state_keep_above, 0);
    } else {
        window->set_state(LingmoWindow::state::state_keep_above, LingmoWindow::state::state_keep_above);
    }
}
...
void WaylandTasksModel::requestToggleKeepBelow(const QModelIndex &index)
{
    ...
    if (window->windowState & LingmoWindow::state::state_keep_below) {
        window->set_state(LingmoWindow::state::state_keep_below, 0);
    } else {
        window->set_state(LingmoWindow::state::state_keep_below, LingmoWindow::state::state_keep_below);
    }
}
...
void WaylandTasksModel::requestToggleFullScreen(const QModelIndex &index)
{
    ...
    if (window->windowState & LingmoWindow::state::state_fullscreen) {
        window->set_state(LingmoWindow::state::state_fullscreen, 0);
    } else {
        window->set_state(LingmoWindow::state::state_fullscreen, LingmoWindow::state::state_fullscreen);
    }
}
...
void WaylandTasksModel::requestToggleShaded(const QModelIndex &index)
{
    ...
    if (window->windowState & LingmoWindow::state::state_shaded) {
        window->set_state(LingmoWindow::state::state_shaded, 0);
    } else {
        window->set_state(LingmoWindow::state::state_shaded, LingmoWindow::state::state_shaded);
    };
}
...
void WaylandTasksModel::requestActivities(const QModelIndex &index, const QStringList &activities)
{
    ...
    const auto lingmoActivities = window->activities;
    const auto oldActivities = QSet(lingmoActivities.begin(), lingmoActivities.end());
...
void WaylandTasksModel::requestPublishDelegateGeometry(const QModelIndex &index, const QRect &geometry, QObject *delegate)
{
    ...
    then calling LingmoWindow::setMinimizeGeometry in the applet backend,
