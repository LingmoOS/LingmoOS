...
#include <qwayland-org-kde-lingmo-virtual-desktop.h>
...
class LingmoVirtualDesktop : public QObject, public QtWayland::org_kde_lingmo_virtual_desktop
{
    Q_OBJECT
public:
    LingmoVirtualDesktop(::org_kde_lingmo_virtual_desktop *object, const QString &id)
        : org_kde_lingmo_virtual_desktop(object)
        , id(id)
    {
    }
    ~LingmoVirtualDesktop()
    {
...
protected:
    void org_kde_lingmo_virtual_desktop_name(const QString &name) override
    {
        this->name = name;
    }
    void org_kde_lingmo_virtual_desktop_done() override
    {
        Q_EMIT done();
    }
    void org_kde_lingmo_virtual_desktop_activated() override
    {
        Q_EMIT activated();
    }
};

class LingmoVirtualDesktopManagement : public QWaylandClientExtensionTemplate<LingmoVirtualDesktopManagement>,
                                       public QtWayland::org_kde_lingmo_virtual_desktop_management
{
    Q_OBJECT
public:
    LingmoVirtualDesktopManagement()
...
protected:
    void org_kde_lingmo_virtual_desktop_management_desktop_created(const QString &desktop_id, uint32_t position) override
    {
        Q_EMIT desktopCreated(desktop_id, position);
    }
    void org_kde_lingmo_virtual_desktop_management_desktop_removed(const QString &desktop_id) override
    {
        Q_EMIT desktopRemoved(desktop_id);
    }
    void org_kde_lingmo_virtual_desktop_management_rows(uint32_t rows) override
    {
...
public:
    ...
    std::vector<std::unique_ptr<LingmoVirtualDesktop>> virtualDesktops;
    std::unique_ptr<LingmoVirtualDesktopManagement> virtualDesktopManagement;
    quint32 rows;
...
{
    return std::find_if(virtualDesktops.begin(), virtualDesktops.end(), [&id](const std::unique_ptr<LingmoVirtualDesktop> &desktop) {
...
void VirtualDesktopInfo::WaylandPrivate::init()
{
    ...
    virtualDesktopManagement = std::make_unique<LingmoVirtualDesktopManagement>();

    connect(virtualDesktopManagement.get(), &LingmoVirtualDesktopManagement::activeChanged, this, [this] {
    ...
    connect(virtualDesktopManagement.get(), &LingmoVirtualDesktopManagement::desktopCreated, this, &WaylandPrivate::addDesktop);

    connect(virtualDesktopManagement.get(), &LingmoVirtualDesktopManagement::desktopRemoved, this, [this](const QString &id) {
        std::erase_if(virtualDesktops, [id](const std::unique_ptr<LingmoVirtualDesktop> &desktop) {
            return desktop->id == id;
        });
    ...
    connect(virtualDesktopManagement.get(), &LingmoVirtualDesktopManagement::rowsChanged, this, [this](quint32 rows) {
        this->rows = rows;
        Q_EMIT desktopLayoutRowsChanged();
    });
}
...
void VirtualDesktopInfo::WaylandPrivate::addDesktop(const QString &id, quint32 position)
{
    ...
    auto desktop = std::make_unique<LingmoVirtualDesktop>(virtualDesktopManagement->get_virtual_desktop(id), id);

    connect(desktop.get(), &LingmoVirtualDesktop::activated, this, [id, this]() {
        currentVirtualDesktop = id;
        Q_EMIT currentDesktopChanged();
    });

    connect(desktop.get(), &LingmoVirtualDesktop::done, this, [this]() {
        Q_EMIT desktopNamesChanged();
    });
...
QVariantList VirtualDesktopInfo::WaylandPrivate::desktopIds() const
{
    ...
    std::transform(virtualDesktops.cbegin(), virtualDesktops.cend(), std::back_inserter(ids), [](const std::unique_ptr<LingmoVirtualDesktop> &desktop) {
        return desktop->id;
    });
    return ids;
}
...
QStringList VirtualDesktopInfo::WaylandPrivate::desktopNames() const
{
    ...
    std::transform(virtualDesktops.cbegin(), virtualDesktops.cend(), std::back_inserter(names), [](const std::unique_ptr<LingmoVirtualDesktop> &desktop) {
        return desktop->name;
    });
    return names;
}
