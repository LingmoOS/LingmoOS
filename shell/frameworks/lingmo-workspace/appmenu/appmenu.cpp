    ...
    , m_appmenuDBus(new AppmenuDBus(this))
{
    ...
    if (qGuiApp->platformName() == QLatin1String("wayland")) {
        ...
        connect(&registry, &KWayland::Client::Registry::lingmoShellAnnounced, this, [this, &registry](quint32 name, quint32 version) {
...
void AppMenuModule::initMenuWayland()
{
    ...
    if (window && m_lingmoshell) {
        ...
        auto lingmoSurface = m_lingmoshell->createSurface(KWayland::Client::Surface::fromWindow(window), m_menu.data());
        lingmoSurface->setPosition(window->position());
...
bool AppMenuModule::eventFilter(QObject *object, QEvent *event)
{
    ...
    if (object == m_menu && event->type() == QEvent::Enter && m_lingmoshell) {
