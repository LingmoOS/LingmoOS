...
Q_DECLARE_METATYPE(Lingmo5Support::DataEngine::Data)
...
{
    setWindowTitle(i18n("Lingmo Engine Explorer"));
    ...
    m_title->setIcon(QIcon::fromTheme("lingmo"));
...
void EngineExplorer::dataUpdated(const QString &source, const Lingmo5Support::DataEngine::Data &data)
{
...
{
    m_engines->clear();
    QVector<KPluginMetaData> engines = Lingmo5Support::PluginLoader::listDataEngineMetaData(m_app);
...
void EngineExplorer::showEngine(const QString &name)
{
    ...
    if (auto res = KPluginFactory::instantiatePlugin<Lingmo5Support::DataEngine>(
            KPluginMetaData::findPluginById(QStringLiteral("lingmo5support/dataengine"), m_engineName))) {
        m_engine = res.plugin;
    } else {
        m_engineName.clear();
        updateTitle();
    ...
    connect(m_engine, &Lingmo5Support::DataEngine::sourceAdded, this, &EngineExplorer::addSource);
    connect(m_engine, &Lingmo5Support::DataEngine::sourceRemoved, this, &EngineExplorer::removeSource);
...
void EngineExplorer::showDataContextMenu(const QPoint &point)
{
    ...
    if (index.isValid()) {
        ...
        if (activated == service) {
            ...
            m_engine->connectSource(source, this);
            // Lingmo5Support::DataEngine::Data data = m_engine->query(source);
...
int EngineExplorer::showData(QStandardItem *parent, Lingmo5Support::DataEngine::Data data)
{
    int rowCount = 0;
    for (auto it = data.constBegin(); it != data.constEnd(); it++) {
...
void EngineExplorer::updateTitle()
{
    if (!m_engine || !m_engine->metadata().isValid()) {
        m_title->setIcon(QIcon::fromTheme("lingmo"));
        m_title->setText(i18n("Lingmo DataEngine Explorer"));
        return;
    }
