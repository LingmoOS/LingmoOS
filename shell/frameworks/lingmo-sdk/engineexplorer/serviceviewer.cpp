...
ServiceViewer::ServiceViewer(Lingmo5Support::DataEngine *engine, const QString &source, QWidget *parent)
    : QDialog(parent)
    , m_engine(engine)
    , m_service(nullptr)
    , m_source(source)
    ...
    , m_operationButton(new QPushButton(i18n("Start Operation"), this))
{
    ...
    QString engineName = i18nc("Lingmo engine with unknown name", "Unknown");
    QString serviceName = i18nc("Lingmo service with unknown name", "Unknown");
    ...
    setWindowTitle(i18nc("%1 is a Lingmo service name", "%1 Service Explorer", serviceName));
...
void ServiceViewer::startOperation()
{
    ...
    updateJobCount(1);
    Lingmo5Support::ServiceJob *job = m_service->startOperationCall(desc);
...
void ServiceViewer::operationResult(KJob *j)
{
    ...
    Lingmo5Support::ServiceJob *job = qobject_cast<Lingmo5Support::ServiceJob *>(j);
    if (!job) {
        return;
    }
