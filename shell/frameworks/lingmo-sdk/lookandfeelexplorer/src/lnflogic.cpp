...
LnfLogic::LnfLogic(QObject *parent)
    : QObject(parent)
    , m_themeName(QStringLiteral("org.kde.ocean.desktop"))
    , m_lnfListModel(new LnfListModel(this))
    , m_needsSave(false)
{
    m_package = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/LookAndFeel"));
}
...
{
    const QString metadataPath(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % pluginName
    ...
    cg.writeEntry("X-KDE-ServiceTypes", "Lingmo/LookAndFeel");
    ...
    cg.writeEntry("X-KDE-PluginInfo-Category", "Lingmo Look And Feel");
    ...
    dumpLingmoLayout(pluginName);
    dumpDefaultsConfigFile(pluginName);

    m_lnfListModel->reload();
}

void LnfLogic::dumpLingmoLayout(const QString &pluginName)
{
    QDBusMessage message = QDBusMessage::createMethodCall("org.kde.lingmoshell", "/LingmoShell", "org.kde.LingmoShell", "dumpCurrentLayoutJS");
    ...
    QObject::connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, pluginName](QDBusPendingCallWatcher *watcher) {
        ...
        watcher->deleteLater();
        if (watcher->isError()) {
            emit messageRequested(ErrorLevel::Error, i18n("Cannot retrieve the current Lingmo layout."));
            return;
        }

        const QString layout = msg.arguments().first().toString();
        QDir themeDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % pluginName);
        ...
        QFile layoutFile(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % pluginName
                         % QLatin1String("/contents/layouts/org.kde.lingmo.desktop-layout.js"));
        if (layoutFile.open(QIODevice::WriteOnly)) {
            layoutFile.write(layout.toUtf8());
            layoutFile.close();
        } else {
            qWarning() << "Impossible to write to org.kde.lingmo.desktop-layout.js";
            emit messageRequested(ErrorLevel::Error, i18n("Impossible to write to org.kde.lingmo.desktop-layout.js"));
            return;
        }
        emit messageRequested(ErrorLevel::Info, i18n("Lingmo Layout successfully duplicated"));
    });
}
...
void LnfLogic::dumpDefaultsConfigFile(const QString &pluginName)
{
    ...
    KConfig defaultsConfig(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % pluginName
                           % "/contents/defaults");
    ...
    defaultsConfigGroup.writeEntry("widgetStyle", systemCG.readEntry("widgetStyle", QStringLiteral("ocean")));
    ...
    defaultsConfigGroup.writeEntry("ColorScheme", systemCG.readEntry("ColorScheme", QStringLiteral("Ocean")));
    ...
    defaultsConfigGroup.writeEntry("Theme", systemCG.readEntry("Theme", QStringLiteral("ocean")));

    // lingmo theme
    defaultsConfigGroup = KConfigGroup(&defaultsConfig, "lingmorc");
    defaultsConfigGroup = KConfigGroup(&defaultsConfigGroup, "Theme");
    systemCG = KConfigGroup(KSharedConfig::openConfig(QStringLiteral("lingmorc")), "Theme");
    ...
    defaultsConfigGroup.writeEntry("cursorTheme", systemCG.readEntry("cursorTheme", QStringLiteral("ocean_cursors")));
    ...
    defaultsConfigGroup.writeEntry("LayoutName", systemCG.readEntry("LayoutName", QStringLiteral("org.kde.ocean.desktop")));

    defaultsConfigGroup = KConfigGroup(&defaultsConfig, "kwinrc");
    defaultsConfigGroup = KConfigGroup(&defaultsConfigGroup, "DesktopSwitcher");
    defaultsConfigGroup.writeEntry("LayoutName", systemCG.readEntry("DesktopLayout", QStringLiteral("org.kde.ocean.desktop")));
    ...
    defaultsConfigGroup.writeEntry("library", systemCG.readEntry("library", QStringLiteral("org.kde.ocean")));
...
void LnfLogic::dumpCurrentLingmoLayout()
{
    dumpLingmoLayout(m_themeName);
}
...
{
    return QFile::exists(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + "/lingmo/look-and-feel/" + m_themeName);
}
...
void LnfLogic::processThumbnail(const QString &path)
{
    ...
    QDir themeDir(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % m_themeName);
    if (!themeDir.mkpath("contents/previews")) {
        qWarning() << "Impossible to create the layouts directory in the look and feel package";
    }

    QFile imageFile(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % m_themeName
    ...
    QFile fullScreenImageFile(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) % QLatin1String("/lingmo/look-and-feel/") % m_themeName
