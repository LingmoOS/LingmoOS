...
int main(int argc, char **argv)
{
    ...
    app.setDesktopFileName(QStringLiteral("org.kde.lingmo.themeexplorer"));

    QCommandLineParser parser;
    parser.addVersionOption();
    parser.addHelpOption();
    parser.setApplicationDescription(i18n("Lingmo Theme Explorer"));
    ...
    const QString packagePath("org.kde.lingmo.themeexplorer");

    // usually we have an ApplicationWindow here, so we do not need to create a window by ourselves
    auto obj = new LingmoQuick::SharedQmlEngine();
    ...
    qmlRegisterAnonymousType<ThemeListModel>("org.kde.lingmo.sdk", 1);
    qmlRegisterAnonymousType<ColorEditor>("org.kde.lingmo.sdk", 1);
