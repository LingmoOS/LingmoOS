...
int main(int argc, char **argv)
{
    ...
    QApplication::setWindowIcon(QIcon::fromTheme(QStringLiteral("lingmo"), app.windowIcon()));

    QCommandLineParser parser;
    parser.setApplicationDescription(i18n("Run Lingmo widgets in their own window"));
    ...
    parser.addOption(QCommandLineOption(QStringList() << QStringLiteral("x") << QStringLiteral("xPosition"),
                                        i18n("Set the x position of the plasmoidviewer on the Lingmo desktop"),
                                        QStringLiteral("xPosition")));
    parser.addOption(QCommandLineOption(QStringList() << QStringLiteral("y") << QStringLiteral("yPosition"),
                                        i18n("Set the y position of the plasmoidviewer on the Lingmo desktop"),
    ...
    if (applet.isEmpty()) {
        qWarning() << "An applet name or path must be specified, e.g. --applet org.kde.lingmo.analogclock";
        return 1;
    }
