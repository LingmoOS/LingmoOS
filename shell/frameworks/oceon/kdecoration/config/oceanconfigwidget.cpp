...
#include "oceanconfigwidget.h"
#include "oceanexceptionlist.h"
...
ConfigWidget::ConfigWidget(QObject *parent, const KPluginMetaData &data, const QVariantList & /*args*/)
    ...
    , m_configuration(KSharedConfig::openConfig(QStringLiteral("oceanrc")))
    , m_changed(false)
{
    // configuration
    m_ui.setupUi(widget());
...
void ConfigWidget::save()
{
    ...
    // needed for ocean style to reload shadows
    {
        QDBusMessage message(QDBusMessage::createSignal("/OceanDecoration", "org.kde.Ocean.Style", "reparseConfiguration"));
        QDBusConnection::sessionBus().send(message);
    }
}
