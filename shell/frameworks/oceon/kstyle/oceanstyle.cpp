...
#include "oceantoolsareamanager.h"
...
using TabletModeWatcher = LingmoUI::Platform::TabletModeWatcher;
#else
#if __has_include(<LingmoUI/TabletModeWatcher>)
// the namespaced include is new in KF 5.91
#include <LingmoUI/TabletModeWatcher>
#else
#include <TabletModeWatcher>
#endif
using TabletModeWatcher = LingmoUI::TabletModeWatcher;
#endif
#include <QQuickWindow>
#endif

#include "ocean_logging.h"
...
Style::Style()
    ...
    , _tabBarData(std::make_unique<OceanPrivate::TabBarData>())
...
void Style::globalConfigurationChanged(int type, int arg)
{
    ...
    // lingmo-integration/src/platformtheme/khintssettings.h and fetched
    // from KGlobalConfig in kdelibs4support in lingmo-desktop/kcms/*,
    // seems to be agreed on by everything in lingmo is what sets the
    // animation duration
    if (type != 3) {
        return;
    }
...
QRect Style::toolButtonSubControlRect(const QStyleOptionComplex *option, SubControl subControl, const QWidget *widget) const
{
    ...
    const auto menuStyle = OceanPrivate::toolButtonMenuArrowStyle(toolButtonOption);
    ...
    case SC_ToolButtonMenu: {
        // check features
        if (menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::None) {
            return QRect();
        }

        auto menuRect(rect);
        if (menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::InlineSmall) {
    ...
    case SC_ToolButton: {
        if (menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::SubControl) {
...
QSize Style::toolButtonSizeFromContents(const QStyleOption *option, const QSize &contentsSize, [[maybe_unused]] const QWidget *widget) const
{
    ...
    const auto menuStyle = OceanPrivate::toolButtonMenuArrowStyle(toolButtonOption);
    if (menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::InlineLarge) {
        size.rwidth() += Metrics::MenuButton_IndicatorWidth;
    }
...
bool Style::drawIndicatorArrowPrimitive(ArrowOrientation orientation, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (inTabBar) {
        ...
        if (flat && menuStyle != OceanPrivate::ToolButtonMenuArrowStyle::None) {
            if (sunken && !mouseOver) {
...
bool Style::drawPanelButtonToolPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    const auto menuStyle = OceanPrivate::toolButtonMenuArrowStyle(option);
    if (menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::SubControl) {
...
{
    if (widget && !widget->property("_ocean_statusbar_separator").toBool() && widget->parent() && !widget->parent()->inherits("QMainWindow")) {
        return true;
    }
    auto rect(option->rect);
    const auto color(_helper->separatorColor(option->palette));
...
bool Style::drawMenuItemControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (menuItemOption->menuItemType == QStyleOptionMenuItem::Separator) {
        ...
        if (!menuItemOption->text.isEmpty()) { // icon is ignored on purpose
            ...
            painter->setFont(font);
            painter->setBrush(Qt::NoBrush);
            // 0.7 is from LingmoUI ListSectionHeader.
...
bool Style::drawGroupBoxComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (groupBoxOption->subControls & QStyle::SC_GroupBoxFrame) {
        ...
        OceanPrivate::PainterStateSaver pss(painter);
    ...
    if ((groupBoxOption->subControls & QStyle::SC_GroupBoxLabel) && !groupBoxOption->text.isEmpty()) {
        OceanPrivate::PainterStateSaver painterStateSaver(painter);
...
bool Style::drawToolButtonComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    } else if (menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::InlineSmall || menuStyle == OceanPrivate::ToolButtonMenuArrowStyle::InlineLarge) {
        copy.rect = menuRect;
        copy.state &= ~State_MouseOver;
        copy.state &= ~State_Sunken;
        copy.state &= ~State_On;
...
{
    if (!option) {
        qCWarning(OCEAN) << "Style::drawScrollBarComplexControl: Style can't draw scrollbar without options";
        return true;
    }
...
{
    if (qEnvironmentVariableIsSet("OCEAN_IS_TABLET_MODE")) {
        return qEnvironmentVariableIntValue("OCEAN_IS_TABLET_MODE");
    }
#if OCEAN_HAVE_QTQUICK
