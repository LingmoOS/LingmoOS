# ![Logo](https://lingmo-bigscreen.org/img/logo.png)

A big launcher giving you easy access to any installed apps and skills.
Controllable via voice or TV remote.

This project is using various open-source components like Lingmo Bigscreen, Mycroft AI and libcec.
...
Note that `kdesrc-build` doesn't automatically build `lingmo-nano` and `lingmo-settings`, so make sure to also build that before you run the shell.

<details>
<summary><b>Click here to see dependencies</b></summary>

### KDE Lingmo Dependencies

- lingmo-nano - https://invent.kde.org/lingmo/lingmo-nano
- lingmo-settings - https://invent.kde.org/lingmo-mobile/lingmo-settings
...
```
QT_QPA_PLATFORM=wayland dbus-run-session kwin_wayland "lingmoshell -p org.kde.lingmo.mycroft.bigscreen"
```
