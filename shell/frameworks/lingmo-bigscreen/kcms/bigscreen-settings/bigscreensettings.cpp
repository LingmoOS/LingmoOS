...
void BigscreenSettings::applyLingmoTheme(QQuickItem *item, const QString &themeName)
{
    if (!item) {
        return;
    }

    Lingmo::Theme *theme = m_themes[themeName];
    if (!theme) {
        theme = new Lingmo::Theme(themeName, this);
