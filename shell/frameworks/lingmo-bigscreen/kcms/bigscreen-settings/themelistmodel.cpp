...
void ThemeListModel::reload()
{
    ...
    QStringList themes;
    const QStringList &packs = QStandardPaths::locateAll(QStandardPaths::GenericDataLocation, "lingmo/desktoptheme", QStandardPaths::LocateDirectory);
    ...
    for (const QString &theme : std::as_const(themes)) {
        ...
        if (!hasPluginName) {
            // Lingmo Theme creates a KColorScheme out of the "color" file and falls back to system colors if there is none
