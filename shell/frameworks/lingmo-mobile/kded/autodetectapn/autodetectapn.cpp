...
K_PLUGIN_FACTORY_WITH_JSON(StartFactory, "kded_lingmo_mobile_autodetectapn.json", registerPlugin<AutoDetectAPN>();)

static const QLoggingCategory LOGGING_CATEGORY("lingmo-mobile-autodetectapn");
...
QCoro::Task<void> AutoDetectAPN::checkAndAddAutodetectedAPN()
{
    if (!KRuntimePlatform::runtimePlatform().contains(QStringLiteral("phone"))) {
        qCDebug(LOGGING_CATEGORY) << "Not running APN autodetection because this is not a Lingmo Mobile session...";
        co_return;
    }
...
{
    const QString providersFile = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("lingmo-mobile-apn-info/apns-full-conf.xml"));
