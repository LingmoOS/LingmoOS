    ...
    , m_colorsSettings{new ColorsSettings(this)}
{
    ...
    m_usingDarkTheme = m_colorsSettings->colorScheme() == "OceanDark";
}
...
{
    // use lingmo-apply-colorscheme since it has logic for notifying the shell of changes
    if (usingDarkTheme) {
        QProcess::execute("lingmo-apply-colorscheme", {QStringLiteral("OceanDark")});
    } else {
        QProcess::execute("lingmo-apply-colorscheme", {QStringLiteral("OceanLight")});
    }
