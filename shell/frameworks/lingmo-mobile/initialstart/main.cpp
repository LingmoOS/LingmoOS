...
int main(int argc, char *argv[])
{
    ...
    // start wizard
    KLocalizedString::setApplicationDomain("lingmo_org.kde.lingmo.mobileinitialstart");
    KAboutData aboutData(QStringLiteral("lingmo-mobile-initial-start"),
                         QStringLiteral("Initial Start"),
                         QStringLiteral(LINGMO_MOBILE_VERSION_STRING),
                         QStringLiteral(""),
                         KAboutLicense::GPL,
    ...
    if (!testWizard) {
        // if the wizard has already been run, or we aren't in lingmo mobile
        if (!Settings::self()->shouldStartWizard()) {
            qDebug() << "Wizard will not be started since either it has already been run, or the current session is not Lingmo Mobile.";
            return 0;
        }
    }
    ...
    engine.load(QUrl(QStringLiteral("qrc:org/kde/lingmo/mobileinitialstart/initialstart/qml/Main.qml")));
