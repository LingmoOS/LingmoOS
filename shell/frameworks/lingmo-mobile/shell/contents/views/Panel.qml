...
Item {
    ...
    // NOTE: Below is taken straight out of Lingmo Desktop so that we can
    //       support desktop panels properly, try to keep it in sync:
    //       lingmo-desktop/desktoppackage/contents/views/Panel.qml

    property Item containment

    property bool floatingPrefix: floatingPanelSvg.usedPrefix === "floating"
    readonly property bool verticalPanel: containment?.plasmoid?.formFactor === LingmoCore.Types.Vertical

    readonly property real spacingAtMinSize: Math.round(Math.max(1, (verticalPanel ? root.width : root.height) - LingmoUI.Units.iconSizes.smallMedium)/2)
    KSvg.FrameSvgItem {
        id: thickPanelSvg
        visible: false
        prefix: 'thick'
    ...
    readonly property bool rightEdge: containment?.plasmoid?.location === LingmoCore.Types.RightEdge
    readonly property bool bottomEdge: containment?.plasmoid?.location === LingmoCore.Types.BottomEdge
    ...
    readonly property int topPadding: Math.round(Math.min(thickPanelSvg.fixedMargins.top + LingmoUI.Units.smallSpacing, spacingAtMinSize));
    readonly property int bottomPadding: Math.round(Math.min(thickPanelSvg.fixedMargins.bottom + LingmoUI.Units.smallSpacing, spacingAtMinSize));
    readonly property int leftPadding: Math.round(Math.min(thickPanelSvg.fixedMargins.left + LingmoUI.Units.smallSpacing, spacingAtMinSize));
    readonly property int rightPadding: Math.round(Math.min(thickPanelSvg.fixedMargins.right + LingmoUI.Units.smallSpacing, spacingAtMinSize));
    ...
    Connections {
        ...
        function onActivated() {
            if (root.containment.plasmoid.status === LingmoCore.Types.AcceptingInputStatus) {
                root.containment.plasmoid.status = LingmoCore.Types.PassiveStatus;
            } else {
                root.containment.plasmoid.status = LingmoCore.Types.AcceptingInputStatus;
            }
        }
    }
    ...
    readonly property int floatingnessAnimationDuration: LingmoUI.Units.longDuration
    ...
    KSvg.FrameSvgItem {
        ...
        imagePath: containment?.plasmoid?.backgroundHints === LingmoCore.Types.NoBackground ? "" : "widgets/panel-background"
    }
    KSvg.FrameSvgItem {
    ...
    KSvg.FrameSvgItem {
        ...
        anchors.fill: floatingTranslucentItem
        imagePath: containment?.plasmoid?.backgroundHints === LingmoCore.Types.NoBackground ? "" : "solid/widgets/panel-background"
    }
    KSvg.FrameSvgItem {
        id: opaqueItem
    ...
    onStateTriggersChanged: {
        ...
        if (containment) {
            if (opaqueApplets) {
                containment.plasmoid.containmentDisplayHints |= LingmoCore.Types.ContainmentPrefersOpaqueBackground
            } else {
                containment.plasmoid.containmentDisplayHints &= ~LingmoCore.Types.ContainmentPrefersOpaqueBackground
            }
            if (floatingApplets) {
                containment.plasmoid.containmentDisplayHints |= LingmoCore.Types.ContainmentPrefersFloatingApplets
            } else {
                containment.plasmoid.containmentDisplayHints &= ~LingmoCore.Types.ContainmentPrefersFloatingApplets
            }
        }
    ...
    KSvg.FrameSvgItem {
        ...
        x: root.verticalPanel || !panel.activeFocusItem
            ? translucentItem.x
            : Math.max(panel.activeFocusItem.LingmoUI.ScenePosition.x, panel.activeFocusItem.LingmoUI.ScenePosition.x)
        y: root.verticalPanel && panel.activeFocusItem
            ? Math.max(panel.activeFocusItem.LingmoUI.ScenePosition.y, panel.activeFocusItem.LingmoUI.ScenePosition.y)
            : translucentItem.y
