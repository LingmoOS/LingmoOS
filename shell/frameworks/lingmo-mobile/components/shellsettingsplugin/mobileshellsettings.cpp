...
void MobileShellSettings::setNavigationPanelEnabled(bool navigationPanelEnabled)
{
    ...
    updateNavigationBarsInLingmo(navigationPanelEnabled);
}
...
void MobileShellSettings::setConvergenceModeEnabled(bool enabled)
{
    ...
    auto *job = new KIO::CommandLauncherJob(QStringLiteral("lingmo-mobile-envmanager --apply-settings"), {});
    job->setUiDelegate(new KNotificationJobUiDelegate(KJobUiDelegate::AutoErrorHandlingEnabled));
    job->setDesktopName(QStringLiteral("org.kde.lingmo-mobile-envmanager"));
    job->start();
}

void MobileShellSettings::updateNavigationBarsInLingmo(bool navigationPanelEnabled)
{
    // Do not update panels when not in Lingmo Mobile
    ...
    if (navigationPanelEnabled) {
        QString createNavigationPanelScript = R"(
            loadTemplate("org.kde.lingmo.mobile.defaultNavigationPanel");
        )";
        ...
        QString deleteNavigationPanelScript = R"(
            ...
            for (var i = 0; i < allPanels.length; i++) {
                if (allPanels[i].type === "org.kde.lingmo.mobile.taskpanel") {
                    allPanels[i].remove();
                }
            }
        )";
