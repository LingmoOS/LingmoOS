...
WallpaperPlugin::WallpaperPlugin(QObject *parent)
    ...
    , m_homescreenConfigFile{KSharedConfig::openConfig("lingmo-org.kde.lingmo.mobileshell-appletsrc", KConfig::SimpleConfig)}
    , m_lockscreenConfigFile{KSharedConfig::openConfig("kscreenlockerrc", KConfig::SimpleConfig)}
{
    ...
    const bool connected = QDBusConnection::sessionBus().connect(QStringLiteral("org.kde.lingmoshell"),
    ...
    if (!connected) {
        qWarning() << "Could not connect to dbus service org.kde.lingmoshell to listen to wallpaperChanged";
    }
...
LingmoQuick::ConfigModel *WallpaperPlugin::wallpaperPluginModel()
{
...
QString WallpaperPlugin::homescreenWallpaperPluginSource()
{
    ...
    for (int i = 0; i < wallpaperPluginCount; ++i) {
        if (model->data(model->index(i), LingmoQuick::ConfigModel::PluginNameRole) == m_homescreenWallpaperPlugin) {
            return model->data(model->index(i), LingmoQuick::ConfigModel::SourceRole).toString();
        }
    }
...
QString WallpaperPlugin::lockscreenWallpaperPluginSource()
{
    ...
    const auto wallpaperPluginCount = model->count();
    for (int i = 0; i < wallpaperPluginCount; ++i) {
        if (model->data(model->index(i), LingmoQuick::ConfigModel::PluginNameRole) == m_lockscreenWallpaperPlugin) {
            return model->data(model->index(i), LingmoQuick::ConfigModel::SourceRole).toString();
        }
    }
...
{
    auto message = QDBusMessage::createMethodCall(QLatin1String("org.kde.lingmoshell"),
                                                  QLatin1String("/LingmoShell"),
                                                  QLatin1String("org.kde.LingmoShell"),
...
QCoro::Task<void> WallpaperPlugin::loadHomescreenSettings()
{
    ...
    if (!map.contains("wallpaperPlugin")) {
        qWarning() << "wallpaperPlugin not found in response from org.kde.LingmoShell wallpaper(), could not retrieve wallpaper";
...
{
    auto packages = KPackage::PackageLoader::self()->listPackages(QStringLiteral("Lingmo/Wallpaper"), "lingmo/wallpapers");
    ...
    for (auto &metaData : packages) {
        if (metaData.pluginId() == wallpaperPlugin) {
            found = true;
            pkg = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/Wallpaper"), QFileInfo(metaData.fileName()).path());
            break;
        }
    }
...
{
    auto iface = new QDBusInterface("org.kde.lingmoshell", "/LingmoShell", "org.kde.LingmoShell", QDBusConnection::sessionBus(), this);
    if (!iface->isValid()) {
