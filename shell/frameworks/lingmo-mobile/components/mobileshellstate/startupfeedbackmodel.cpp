    ...
    : QAbstractListModel{parent}
{
    ...
    connect(WindowListener::instance(), &WindowListener::lingmoWindowCreated, this, &StartupFeedbackModel::onLingmoWindowOpened);
...
void StartupFeedbackModel::addApp(StartupFeedback *startupFeedback)
{
    ...
    // the Lingmo window opening and the visual (causes panels to flash background color)
...
void StartupFeedbackModel::onWindowOpened(KWayland::Client::LingmoWindow *window)
{
    if (!window) {
        return;
    }
    ...
    if (m_list.size() > indexToRemove) {
        ...
        if (window->isActive()) {
            ...
            connect(window, &KWayland::Client::LingmoWindow::activeChanged, this, removeFunction);
        }
    }
}

void StartupFeedbackModel::onLingmoWindowOpened(KWayland::Client::LingmoWindow *window)
{
...
void StartupFeedbackModel::onActiveWindowChanged(KWayland::Client::LingmoWindow *activeWindow)
{
