...
void WindowUtil::initWayland()
{
    if (!QGuiApplication::platformName().startsWith(QLatin1String("wayland"), Qt::CaseInsensitive)) {
        qWarning() << "Lingmo Mobile must use wayland! The current platform detected is:" << QGuiApplication::platformName();
        return;
    }
    ...
    connect(registry, &Registry::lingmoWindowManagementAnnounced, this, [this, registry](quint32 name, quint32 version) {
        ...
        connect(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated, this, [this](KWayland::Client::LingmoWindow *window) {
            Q_EMIT windowCreated(window);
        });
        connect(m_windowManagement, &KWayland::Client::LingmoWindowManagement::windowCreated, this, &WindowUtil::windowCreatedSlot);

        connect(m_windowManagement, &LingmoWindowManagement::showingDesktopChanged, this, &WindowUtil::updateShowingDesktop);
        connect(m_windowManagement, &LingmoWindowManagement::activeWindowChanged, m_activeWindowTimer, qOverload<>(&QTimer::start));
    ...
    connect(registry, &Registry::lingmoActivationFeedbackAnnounced, this, [this, registry](quint32 name, quint32 version) {
        ...
        connect(iface, &LingmoActivationFeedback::activation, this, [this](LingmoActivation *activation) {
            connect(activation, &LingmoActivation::applicationId, this, [this, activation](const QString &appId) {
                // do not show activation screen for the lingmoshell process
                if (appId == "org.kde.lingmoshell") {
                    return;
                }
                ...
                if (!servicesFound.isEmpty()) {
                    ...
                    connect(activation, &LingmoActivation::finished, this, [this, appId, iconName]() {
...
void WindowUtil::updateActiveWindow()
{
    ...
    using namespace KWayland::Client;
    if (m_activeWindow) {
        disconnect(m_activeWindow.data(), &LingmoWindow::closeableChanged, this, &WindowUtil::hasCloseableActiveWindowChanged);
        disconnect(m_activeWindow.data(), &LingmoWindow::unmapped, this, &WindowUtil::forgetActiveWindow);
    }
    ...
    if (m_activeWindow) {
        connect(m_activeWindow.data(), &LingmoWindow::closeableChanged, this, &WindowUtil::hasCloseableActiveWindowChanged);
        connect(m_activeWindow.data(), &LingmoWindow::unmapped, this, &WindowUtil::forgetActiveWindow);
    }
...
void WindowUtil::updateActiveWindowIsShell()
{
    ...
    if (activeWindow) {
        if (activeWindow->appId() == QStringLiteral("org.kde.lingmoshell") && !m_activeWindowIsShell) {
            m_activeWindowIsShell = true;
            Q_EMIT activeWindowIsShellChanged();
        } else if (activeWindow->appId() != QStringLiteral("org.kde.lingmoshell") && m_activeWindowIsShell) {
...
QList<KWayland::Client::LingmoWindow *> WindowUtil::windowsFromStorageId(const QString &storageId) const
{
...
void WindowUtil::windowCreatedSlot(KWayland::Client::LingmoWindow *window)
{
    QString storageId = window->appId() + QStringLiteral(".desktop");

    // ignore empty windows
    if (storageId == ".desktop" || storageId == "org.kde.lingmoshell.desktop") {
        return;
    }
