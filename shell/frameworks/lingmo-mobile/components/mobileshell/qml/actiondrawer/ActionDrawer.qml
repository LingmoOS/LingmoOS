...
Item {
     ...
     */
    readonly property real largePortraitThreshold: LingmoUI.Units.gridUnit * 35
    ...
    function updateState() {
        cancelAnimations();
        let openThreshold = LingmoUI.Units.gridUnit;

        if (root.offset <= 0) {
            // close immediately, so that we don't have to wait LingmoUI.Units.longDuration
            root.visible = false;
            close();
