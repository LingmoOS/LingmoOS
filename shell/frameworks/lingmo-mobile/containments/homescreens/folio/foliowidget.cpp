...
FolioWidget::FolioWidget(HomeScreen *parent, Lingmo::Applet *applet, int realGridWidth, int realGridHeight)
    : QObject{parent}
    , m_homeScreen{parent}
...
void FolioWidget::init()
{
    ...
    connect(m_homeScreen->widgetsManager(), &WidgetsManager::widgetAdded, this, [this](Lingmo::Applet *applet) {
        if (applet && static_cast<int>(applet->id()) == m_id) {
            setApplet(applet);
        }
    });
    connect(m_homeScreen->widgetsManager(), &WidgetsManager::widgetRemoved, this, [this](Lingmo::Applet *applet) {
        if (applet && static_cast<int>(applet->id()) == m_id) {
            setApplet(nullptr);
        }
    });
...
Lingmo::Applet *FolioWidget::applet() const
{
    return m_applet;
}

void FolioWidget::setApplet(Lingmo::Applet *applet)
{
    m_applet = applet;
    ...
    if (m_applet) {
        setVisualApplet(LingmoQuick::AppletQuickItem::itemForApplet(m_applet));
    } else {
        setVisualApplet(nullptr);
    }
}

LingmoQuick::AppletQuickItem *FolioWidget::visualApplet() const
{
    return m_quickApplet;
}

void FolioWidget::setVisualApplet(LingmoQuick::AppletQuickItem *quickItem)
{
