    ...
    function launchApp() {
        if (application.icon !== "" && !root.application.running) {
            MobileShellState.ShellDBusClient.openAppLaunchAnimationWithPosition(
                ...
                root.iconItem.LingmoUI.ScenePosition.x + root.iconItem.width/2,
                root.iconItem.LingmoUI.ScenePosition.y + root.iconItem.height/2,
    ...
    contentItem: Item {
        ...
        Rectangle {
            ...
            Behavior on scaleAmount {
                ...
                NumberAnimation { duration: LingmoUI.Units.longDuration; easing.type: Easing.InOutQuad }
            }
            Behavior on opacity {
                enabled: root.turnToFolderAnimEnabled
                NumberAnimation { duration: LingmoUI.Units.longDuration; easing.type: Easing.InOutQuad }
        ...
        DelegateAppIcon {
            ...
            Behavior on scaleAmount {
                enabled: root.turnToFolderAnimEnabled
                NumberAnimation { duration: root.turnToFolderAnimEnabled ? LingmoUI.Units.longDuration : 0; easing.type: Easing.InOutQuad }
            }
