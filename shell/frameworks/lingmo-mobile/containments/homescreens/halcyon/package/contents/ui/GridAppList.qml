...
MobileShell.GridView {
    ...
    cellWidth: gridView.effectiveContentWidth / Math.min(Math.floor(effectiveContentWidth / (LingmoUI.Units.iconSizes.huge + LingmoUI.Units.largeSpacing * 2)), 8)
    ...
    header: MobileShell.BaseItem {
        ...
        topPadding: LingmoUI.Units.gridUnit + Math.round(gridView.height * 0.1)
    ...
    PC3.Label {
        ...
        text: "M\nM"
        visible: false
        font.pointSize: LingmoUI.Theme.defaultFont.pointSize * 0.85
        font.weight: Font.Bold
    }
        ...
        onLaunch: (x, y, icon, title, storageId) => {
            if (icon !== "") {
                MobileShellState.ShellDBusClient.openAppLaunchAnimationWithPosition(
                    ...
                    title,
                    storageId,
                    delegate.iconItem.LingmoUI.ScenePosition.x + delegate.iconItem.width/2,
                    delegate.iconItem.LingmoUI.ScenePosition.y + delegate.iconItem.height/2,
