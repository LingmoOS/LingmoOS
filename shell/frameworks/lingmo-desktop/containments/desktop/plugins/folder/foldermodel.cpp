...
Q_LOGGING_CATEGORY(FOLDERMODEL, "lingmo.containments.desktop.folder.foldermodel")
...
bool FolderModel::isDir(const QModelIndex &index, const KDirModel *dirModel) const
{
    ...
    if (m_parseDesktopFiles && item.isDesktopFile()) {
        ...
        job->setProperty("org.kde.lingmo.folder_url", item.url());
...
void FolderModel::statResult(KJob *job)
{
    ...
    const QUrl &url = statJob->property("org.kde.lingmo.folder_url").toUrl();
...
Lingmo::Applet *FolderModel::applet() const
{
    return m_applet;
}

void FolderModel::setApplet(Lingmo::Applet *applet)
{
    if (m_applet != applet) {
        Q_ASSERT(!m_applet);
        ...
        if (applet) {
            ...
            if (containment) {
                ...
                if (corona) {
                    connect(corona, &Lingmo::Corona::screenRemoved, this, [this](int screenId) {
                        if (m_screen == screenId) {
                            m_screenMapper->removeScreen(screenId, m_currentActivity, resolvedUrl());
                        }
                    });
                    connect(corona, &Lingmo::Corona::screenAdded, this, [this](int screenId) {
                        if (m_screen == screenId) {
                ...
                connect(containment, &Lingmo::Containment::screenChanged, this, &FolderModel::setScreen);
            }
        }
