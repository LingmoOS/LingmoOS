...
LingmoComponents.ScrollView {
    ...
    LingmoComponents.TextArea {
        ...
        rightPadding: root.LingmoComponents.ScrollBar.vertical.visible ? root.LingmoComponents.ScrollBar.vertical.width : 0
        ...
        function getXY() {
            ...
            if (root.useListViewMode) {
                ...
                if (root.LingmoComponents.ScrollBar.vertical.policy === Qt.ScrollBarAlwaysOn
                    && gridView.effectiveLayoutDirection === Qt.RightToLeft) {
                    _x -= root.LingmoComponents.ScrollBar.vertical.width;
                }
                _y = pos.y + LingmoUI.Units.smallSpacing - editor.topPadding;
            }
            return [ _x, _y ];
        }
        ...
        function getWidth(addWidthVerticalScroller) {
            ...
            return(targetItem.label.parent.width - LingmoUI.Units.smallSpacing +
                (root.useListViewMode ? -(editor.leftPadding + editor.rightPadding + LingmoUI.Units.smallSpacing) : 0) +
                (addWidthVerticalScroller ? root.LingmoComponents.ScrollBar.vertical.width : 0));
        }
        ...
        function getHeight(addWidthHoriozontalScroller, init) {
            ...
            if (isPopup || init) {
                ...
                var maxHeight = LingmoUI.Units.iconSizes.sizeForLabels * (Plasmoid.configuration.textLines + 1) + editor.topPadding + editor.bottomPadding;
                _height = Math.min(realHeight, maxHeight);
            }
            return _height + (addWidthHoriozontalScroller ? root.LingmoComponents.ScrollBar.horizontal.height : 0);
        }
        ...
        function adjustSize() {
            if (isPopup) {
                if(contentWidth + editor.leftPadding + editor.rightPadding > root.width) {
                    ...
                    root.LingmoComponents.ScrollBar.horizontal.policy = Qt.ScrollBarAlwaysOn;
                    root.height = getHeight(true);
                } else {
                    root.LingmoComponents.ScrollBar.horizontal.policy = Qt.ScrollBarAlwaysOff;
                ...
                if(contentHeight + editor.topPadding + editor.bottomPadding > root.height) {
                    root.visible = targetItem !== null;
                    root.LingmoComponents.ScrollBar.vertical.policy = Qt.ScrollBarAlwaysOn;
                    root.width = getWidth(true);
                } else {
                    root.LingmoComponents.ScrollBar.vertical.policy = Qt.ScrollBarAlwaysOff;
                    root.width = getWidth();
                }
            }
