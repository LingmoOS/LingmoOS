    ...
    MouseEventListener {
        ...
        FocusScope {
            ...
            GridView {
                ...
                LingmoComponents.ScrollBar.vertical: LingmoComponents.ScrollBar {
                    id: verticalScrollBar
                }
                LingmoComponents.ScrollBar.horizontal: LingmoComponents.ScrollBar {}
                ...
                cellWidth: {
                    if (root.useListViewMode) {
                        ...
                        var iconWidth = iconSize + (2 * LingmoUI.Units.gridUnit) + (2 * LingmoUI.Units.smallSpacing);
                        if (root.isContainment && isRootView && scrollArea.viewportWidth > 0) {
                            var minIconWidth = Math.max(iconWidth, LingmoUI.Units.iconSizes.small * ((Plasmoid.configuration.labelWidth * 2) + 4));
                ...
                cellHeight: {
                    if (root.useListViewMode) {
                        return Math.ceil((Math.max(LingmoUI.Units.iconSizes.sizeForLabels, iconSize)
                            + Math.max(highlightItemSvg.margins.top + highlightItemSvg.margins.bottom,
                            listItemSvg.margins.top + listItemSvg.margins.bottom)) / 2) * 2;
                    } else {
                        // the smallSpacings are for padding
                        var iconHeight = iconSize + (LingmoUI.Units.gridUnit * Plasmoid.configuration.textLines) + (LingmoUI.Units.smallSpacing * 3);
                ...
                function makeIconSize() {
                    if (root.useListViewMode) {
                        return LingmoUI.Units.iconSizes.small;
                    }
                ...
                function rectangleSelect(x, y, width, height) {
                    ...
                    var cWidth = gridView.cellWidth - (2 * LingmoUI.Units.smallSpacing);
                    var cHeight = gridView.cellHeight - (2 * LingmoUI.Units.smallSpacing);
                    ...
                    for (var s = 0; s < stripes; s++) {
                        for (var i = 0; i < perStripe; i++) {
                            ...
                            // expensive work.
                            if (main.rubberBand.intersects(Qt.rect(itemX + LingmoUI.Units.smallSpacing, itemY + LingmoUI.Units.smallSpacing,
                                cWidth, cHeight))) {
