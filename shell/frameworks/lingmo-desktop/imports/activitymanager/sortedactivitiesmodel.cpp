...
static const char *s_lingmo_config = "lingmo-org.kde.lingmo.desktop-appletsrc";
...
public:
    BackgroundCache()
        : initialized(false)
        , lingmoConfig(KSharedConfig::openConfig(QString::fromLatin1(s_lingmo_config)))
    {
        using namespace std::placeholders;

        const QString configFile = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + QLatin1Char{'/'} + QLatin1String{s_lingmo_config};
    ...
    {
        if (!file.endsWith(QLatin1String{s_lingmo_config})) {
            return;
        }

        if (initialized) {
            lingmoConfig->reparseConfiguration();
            reload();
        }
    }
    ...
    void reload()
    {
        ...
        // containments that define activities in lingmo
        for (const auto &containmentId : lingmoConfigContainments().groupList()) {
            const auto containment = lingmoConfigContainments().group(containmentId);
    ...
    KConfigGroup lingmoConfigContainments()
    {
        return lingmoConfig->group(QStringLiteral("Containments"));
    }
    ...
    bool initialized;
    KSharedConfig::Ptr lingmoConfig;
};
