...
EmojiModel::EmojiModel()
{
    ...
    const QString dictName = QLatin1String{"lingmo/emoji/"} + bcp + QLatin1String{".dict"};
    ...
    for (qsizetype underscoreIndex = -1; (underscoreIndex = bcp.lastIndexOf(QLatin1Char('_'), underscoreIndex)) != -1; --underscoreIndex) {
        const QString genericDictName = QLatin1String{"lingmo/emoji/"} + QStringView(bcp).left(underscoreIndex) + QLatin1String{".dict"};
    ...
    const QString genericPath = QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("lingmo/emoji/en.dict"));
    dicts << genericPath;
