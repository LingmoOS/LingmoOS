...
for (var i in containments) {
    ...
    for (var j in cont.widgetIds) {
        ...
        let newType = ""
        if (widget.type == "org.kde.lingmo.systemloadviewer") {
            let geometry = widget.geometry;
            geometry.width = geometry.width/3
            
            widget.remove();
            cont.addWidget("org.kde.lingmo.systemmonitor.cpuusage", geometry.x, geometry.y, geometry.width, geometry.height);
            geometry.x += geometry.width;
            cont.addWidget("org.kde.lingmo.systemmonitor.memoryusage", geometry.x, geometry.y, geometry.width, geometry.height);
            geometry.x += geometry.width;

            let swapWidget = cont.addWidget("org.kde.lingmo.systemmonitor", geometry.x, geometry.y, geometry.width, geometry.height);
