    ...
    Loader {
        ...
        readonly property point pos: root.containment?.plasmoid.availableScreenRegion, shouldBeActive() && item !== null
            ? root.containment.adjustToAvailableScreenRegion(
                root.containment.width + root.containment.x - item.width - LingmoUI.Units.largeSpacing,
                root.containment.height + root.containment.y - item.height - LingmoUI.Units.largeSpacing,
                item.width + LingmoUI.Units.largeSpacing,
                item.height + LingmoUI.Units.largeSpacing)
            : Qt.point(0, 0)
