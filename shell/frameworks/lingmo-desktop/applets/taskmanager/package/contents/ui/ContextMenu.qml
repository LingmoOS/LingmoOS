...
LingmoExtras.Menu {
    ...
    onStatusChanged: {
        if (visualParent && get(atm.LauncherUrlWithoutIcon) != "" && status == LingmoExtras.Menu.Open) {
            activitiesDesktopsMenu.refresh();

        } else if (status == LingmoExtras.Menu.Closed) {
            menu.destroy();
        }
    }

    Component.onCompleted: {
        // Cannot have "Connections" as child of LingmoExtras.Menu.
    ...
    function loadDynamicLaunchActions(launcherUrl) {
        ...
        if (playerData && playerData.canControl && !(get(atm.WinIdList) !== undefined && get(atm.WinIdList).length > 1)) {
            ...
            // LingmoCore Menu doesn't actually handle icons or labels changing at runtime...
    ...
    LingmoExtras.MenuItem {
        ...
        property LingmoExtras.Menu _virtualDesktopsMenu: LingmoExtras.Menu {
     ...
     LingmoExtras.MenuItem {
        ...
        property LingmoExtras.Menu _activitiesDesktopsMenu: LingmoExtras.Menu {
    ...
    LingmoExtras.MenuItem {
        ...
        visible: visualParent
                     && !get(atm.IsLauncher)
                     && !get(atm.IsStartup)
                     && Plasmoid.immutability !== LingmoCore.Types.SystemImmutable
    ...
    LingmoExtras.MenuItem {
        ...
        property LingmoExtras.Menu _activitiesLaunchersMenu: LingmoExtras.Menu {
    ...
    LingmoExtras.MenuItem {
        ...
        property LingmoExtras.Menu moreMenu: LingmoExtras.Menu {
