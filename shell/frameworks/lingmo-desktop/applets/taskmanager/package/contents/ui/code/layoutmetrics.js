...
const iconMargin = Math.round(LingmoUI.Units.smallSpacing / 4);
const labelMargin = LingmoUI.Units.smallSpacing;

function horizontalMargins() {
    const spacingAdjustment = (tasks.plasmoid.pluginName === "org.kde.lingmo.icontasks") ? (LingmoUI.Settings.tabletMode ? 3 : tasks.plasmoid.configuration.iconSpacing) : 1
...
function adjustMargin(height, margin) {
    ...
    if (available < LingmoUI.Units.iconSizes.small) {
        return Math.floor((margin * (LingmoUI.Units.iconSizes.small / available)) / 3);
    }
...
function preferredMinHeight() {
    ...
    return LingmoUI.Units.iconSizes.sizeForLabels + 4;
}
...
function preferredMaxHeight() {
    if (tasks.vertical) {
        ...
        if (tasks.iconsOnly) {
            ...
            taskPreferredSize = Math.max(LingmoUI.Units.iconSizes.sizeForLabels,
        ...
        return verticalMargins() +
            Math.min(
                LingmoUI.Units.iconSizes.small * 3,
                LingmoUI.Units.iconSizes.sizeForLabels * 3);
    }
}

function preferredHeightInPopup() {
    return verticalMargins() + Math.max(LingmoUI.Units.iconSizes.sizeForLabels,
                                        LingmoUI.Units.iconSizes.medium);
}
...
function spaceRequiredToShowText() {
    ...
    return Math.round(LingmoUI.Units.gridUnit * 1.5);
}

function preferredMinLauncherWidth() {
    const baseWidth = tasks.vertical ? preferredMinHeight() : Math.min(tasks.height, LingmoUI.Units.iconSizes.small * 3);
...
function maximumContextMenuTextWidth() {
    return (LingmoUI.Units.iconSizes.sizeForLabels * 28);
}
