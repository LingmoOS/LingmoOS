...
class KCMTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        options.set_capability("environ", {
            "LC_ALL": "en_US.UTF-8",
            "QT_FATAL_WARNINGS": "1",
            "QT_LOGGING_RULES": "qt.accessibility.atspi.warning=false;qt.qml.typeresolution.cycle.warning=false;kf.lingmo.core.warning=false;kf.windowsystem.warning=false;kf.lingmoui.platform.warning=false;org.kde.klipper.debug=true",
        })
    ...
    def test_0_start_stop_service(self) -> None:
        ...
        self.assertIn("lingmo_accentcolor_service", loadedModules(session_bus))  # The service id is from lingmo-workspace

        stop_button.click()
        wait.until(EC.presence_of_element_located((AppiumBy.NAME, "Start Accent Color")))
        self.assertNotIn("lingmo_accentcolor_service", loadedModules(session_bus))
    ...
    def test_1_toggle_automatically_loading_service(self) -> None:
        ...
        wait = WebDriverWait(self.driver, 30)
        wait.until(lambda _: subprocess.check_output([f"kreadconfig{KDE_VERSION}", "--file", "kded5rc", "--group", "Module-lingmo_accentcolor_service", "--key", "autoload"]).decode(encoding="utf-8").strip() == "false")

        self.driver.find_element(AppiumBy.NAME, "Enable automatically loading Accent Color on startup").click()
        self.driver.find_element(AppiumBy.NAME, "Apply").click()
        wait.until(lambda _: subprocess.check_output([f"kreadconfig{KDE_VERSION}", "--file", "kded5rc", "--group", "Module-lingmo_accentcolor_service", "--key", "autoload"]).decode(encoding="utf-8").strip() == "true")
