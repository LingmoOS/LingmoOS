...
import cv2 as cv
from desktoptest import name_has_owner, start_lingmoshell
from gi.repository import Gio, GLib

KDE_VERSION: Final = 6
LINGMOSHELL_SERVICE_NAME: Final = "org.kde.lingmoshell"
...
class Bug482267Test(unittest.TestCase):
    ...
    def tearDownClass(cls) -> None:
        if cls.lingmoshell is not None:
            subprocess.check_output([f"kquitapp{KDE_VERSION}", "lingmoshell"], stderr=sys.stderr)
            cls.lingmoshell.wait(5)
    ...
    def test_0_wait_until_ready(self) -> None:
        """
        Start lingmoshell and wait until the DBus interface is ready
        """
        if not name_has_owner(self.session_bus, LINGMOSHELL_SERVICE_NAME):
            for _ in range(10):
                if name_has_owner(self.session_bus, LINGMOSHELL_SERVICE_NAME):
                    return
                print("waiting for lingmoshell to appear on the DBus session")
                time.sleep(1)
            self.fail("lingmoshell does not appear on DBus after 10 secs")
    ...
    def test_1_set_color_wallpaper(self) -> None:
        ...
        """
        assert self.session_bus is not None and self.lingmoshell is not None
        message: Gio.DBusMessage = Gio.DBusMessage.new_method_call(LINGMOSHELL_SERVICE_NAME, "/LingmoShell", "org.kde.LingmoShell", "setWallpaper")
        ...
        # Prepare to restart lingmoshell
        subprocess.check_output([f"kquitapp{KDE_VERSION}", "lingmoshell"], stderr=sys.stderr)
        self.assertEqual(self.lingmoshell.wait(5), 0)
        self.lingmoshell = None
    ...
    def test_2_signal_ksplash_when_desktop_is_ready(self):
        ...
        @see https://invent.kde.org/lingmo/liblingmo/-/commit/1b154b3eb2b7617067dfd05d961e53238458714e
        """
        ...
        _, _, self.lingmoshell = start_lingmoshell()
