...
{
    qmlRegisterAnonymousType<SplashScreenSettings>("org.kde.lingmo.splash.kcm", 0);
    qmlRegisterAnonymousType<QStandardItemModel>("org.kde.lingmo.splash.kcm", 0);
...
QList<KPackage::Package> KCMSplashScreen::availablePackages(const QString &component)
{
    ...
    for (const QString &path : dataPaths) {
        QDir dir(path + QStringLiteral("/lingmo/look-and-feel"));
...
void KCMSplashScreen::save()
{
    ...
    for (const QString &plugin : pendingDeletionPlugins) {
        ...
        KJob *uninstallJob = KPackage::PackageJob::uninstall(QStringLiteral("Lingmo/LookAndFeel"), plugin, m_packageRoot);
