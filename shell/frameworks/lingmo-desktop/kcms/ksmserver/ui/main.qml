...
KCM.SimpleKCM {
    ...
    header: ColumnLayout {
        ...
        LingmoUI.InlineMessage {
            Layout.fillWidth: true
            position: LingmoUI.InlineMessage.Position.Header
            type: kcm.error.length > 0 ? LingmoUI.MessageType.Error : LingmoUI.MessageType.Information
    ...
    LingmoUI.FormLayout {
        ...
        Item {
            ...
            LingmoUI.FormData.label: i18nc("@title:group", "General")
        }

        CheckBox {
            id: logoutScreenCheckbox
        LingmoUI.FormData.label: i18nc("@label beginning of the logical sentence 'Ask for confirmation on shutdown, restart, and logout.'", "Ask for confirmation:")
        ...
        Item {
            LingmoUI.FormData.isSection: true
            LingmoUI.FormData.label: i18nc("@title:group", "Session Restore")
        }
        ...
        RadioButton {
            id: loginRestore
            LingmoUI.FormData.label: i18n("On login, launch apps that were open:")
        ...
        RowLayout {
            ...
            LingmoUI.ContextualHelpButton {
                toolTipText: xi18nc("@info", "A <interface>Save Session</interface> button will appear in the <interface>Application Launcher</interface> menu. When you click it, Lingmo will remember the apps that are open and restore them on the next login. Click it again to replace the set of remembered apps.")
            }
        }
        RadioButton {
            id: loginEmpty
        ...
        ColumnLayout {
            LingmoUI.FormData.label: i18n("Ignored applications:")
            LingmoUI.FormData.buddyFor: ignoredAppsTextField
        ...
        Item {
            LingmoUI.FormData.isSection: true
            LingmoUI.FormData.label: i18nc("@title:group", "Firmware")
            visible: uefi.visible
        }

        CheckBox {
            id: uefi
            LingmoUI.FormData.label: i18nc("@label:check part of a sentence: After next restart enter UEFI/Firmware setup screen", "After next restart:")
