...
public:
    ...
    public:
        BackgroundCache()
            : initialized(false)
            , lingmoConfig(QStringLiteral("lingmo-org.kde.lingmo.desktop-appletsrc"))
        {
            using namespace std::placeholders;

            const QString configFile = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + QLatin1Char('/') + lingmoConfig.name();
        ...
        {
            if (!file.endsWith(lingmoConfig.name())) {
                return;
            }
        ...
        void reload(bool fullReload)
        {
            ...
            for (const auto &cont : lingmoConfigContainments().groupList()) {
                auto config = lingmoConfigContainments().group(cont);
