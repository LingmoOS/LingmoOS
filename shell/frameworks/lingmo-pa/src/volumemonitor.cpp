...
void VolumeMonitor::createStream()
{
    ...
    if (!(m_stream = pa_stream_new(Context::instance()->context(), "LingmoPA-VolumeMeter", &ss, nullptr))) {
        qCWarning(LINGMOPA) << "Failed to create stream";
        return;
    }
...
void VolumeMonitor::read_callback(pa_stream *s, size_t length, void *userdata)
{
    ...
    if (pa_stream_peek(s, &data, &length) < 0) {
        qCWarning(LINGMOPA) << "Failed to read data from stream";
        return;
    }
