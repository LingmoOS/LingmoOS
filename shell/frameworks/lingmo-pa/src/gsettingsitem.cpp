...
QVariant GSettingsItem::value(const QString &key) const
{
    ...
    case G_VARIANT_CLASS_STRING:
        ...
        break;
    default:
        qCWarning(LINGMOPA()) << "Unhandled variant type in value()";
    }
...
void GSettingsItem::set(const QString &key, const QVariant &val)
{
    ...
    case G_VARIANT_CLASS_STRING:
        ...
        break;
    default:
        qCWarning(LINGMOPA()) << "Unhandled variant type in set()";
    }
    ...
    : QObject(parent)
{
    ...
    if (!defaultSource) {
        qCWarning(LINGMOPA) << "No GSettings schemas are installed on the system";
        return;
    }

    auto *schema = g_settings_schema_source_lookup(defaultSource, schemaId, true /*recursive*/);
    if (!schema) {
        qCWarning(LINGMOPA) << "Settings schema" << schemaId << "is not installed";
        return;
    }
