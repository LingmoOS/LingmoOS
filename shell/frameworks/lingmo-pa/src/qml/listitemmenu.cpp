...
QMenu *ListItemMenu::createMenu()
{
    ...
    if (!m_visualParent || !m_visualParent->window()) {
        qCWarning(LINGMOPA) << "Cannot prepare menu without visualParent or a window";
        return nullptr;
    }
    ...
    if (auto *device = qobject_cast<PulseAudioQt::Device *>(m_pulseObject.data())) {
        ...
        if (m_cardModel) {
            ...
            if (card) {
                ...
                qCWarning(LINGMOPA) << "Failed to find card at" << device->cardIndex() << "for" << device->description() << device->index();
            }
        }
    }
