...
PotdClient::PotdClient(const KPluginMetaData &metadata, const QVariantList &args, QObject *parent)
    : QObject(parent)
    , m_metadata(metadata)
    , m_identifier(metadata.value(QStringLiteral("X-KDE-LingmoPoTDProvider-Identifier")))
    , m_args(args)
{
...
void PotdEngine::updateSource(bool refresh)
{
    ...
    for (const auto &[_, clientPair] : std::as_const(m_clientMap)) {
        ...
        m_updateCount++;
        qCDebug(WALLPAPERPOTD) << clientPair.client->m_metadata.value(QStringLiteral("X-KDE-LingmoPoTDProvider-Identifier")) << "starts updating wallpaper.";
...
void PotdEngine::loadPluginMetaData()
{
    ...
    for (const KPluginMetaData &metadata : plugins) {
        const QString identifier = metadata.value(QStringLiteral("X-KDE-LingmoPoTDProvider-Identifier"));
