...
void PotdBackend::saveImage()
{
    ...
    m_savedUrl = QUrl::fromLocalFile( //
        QFileDialog::getSaveFileName( //
            nullptr, //
            i18ndc("lingmo_wallpaper_org.kde.potd", "@title:window", "Save Today's Picture"), //
            path + "/" + sanitizeFileName(defaultFileName) + ".jpg", //
            i18ndc("lingmo_wallpaper_org.kde.potd", "@label:listbox Template for file dialog", "JPEG image (*.jpeg *.jpg *.jpe)"), //
            nullptr, //
    ...
    connect(copyJob, &KJob::finished, this, [this](KJob *job) {
        if (job->error()) {
            ...
            if (m_saveStatusMessage.isEmpty()) {
                m_saveStatusMessage = i18ndc("lingmo_wallpaper_org.kde.potd", "@info:status after a save action", "The image was not saved.");
            }
            m_saveStatus = FileOperationStatus::Failed;
            Q_EMIT saveStatusChanged();
        } else {
            m_saveStatusMessage = i18ndc("lingmo_wallpaper_org.kde.potd",
