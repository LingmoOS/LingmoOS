...
function itemPadding() { return LingmoUI.Units.smallSpacing / 2; }
...
{
  return LingmoUI.Units.iconSizes.small + 2 * itemPadding();
}

function minimumCellHeight()
{
  var h = LingmoUI.Units.iconSizes.small + 2 * itemPadding();
  if (showLauncherNames) {
    h += LingmoUI.Units.gridUnit * 2;
  }
    return h;
}
...
{
  return Math.max(root.width, LingmoUI.Units.iconSizes.medium + 20 * LingmoUI.Units.gridUnit);
}

function popupItemHeight()
{
  return LingmoUI.Units.iconSizes.medium + 2 * itemPadding();
}
