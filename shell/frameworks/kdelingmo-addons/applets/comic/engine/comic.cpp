...
{
    mProviders.clear();
    const auto comics = KPackage::PackageLoader::self()->listKPackages(QStringLiteral("Lingmo/Comic"));
    QList<ComicProviderInfo> providers;
    for (auto comicPackage : comics) {
        const KPluginMetaData comic = comicPackage.metadata();
        qCDebug(LINGMO_COMIC) << "ComicEngine::loadProviders()  service name=" << comic.name();
        ...
        if (file.isRelative()) {
            data.icon =
                QStandardPaths::locate(QStandardPaths::GenericDataLocation, QString::fromLatin1("lingmo/comics/%1/%2").arg(comic.pluginId(), comic.iconName()));
        } else {
...
bool ComicEngine::requestSource(const QString &identifier)
{
    ...
    if (!isOnline()) {
        qCDebug(LINGMO_COMIC) << "Currently offline, requested identifier was" << mIdentifierError;
        ...
        qCDebug(LINGMO_COMIC) << "No internet connection, using cached data";
        return true;
    }

    KPackage::Package pkg = KPackage::PackageLoader::self()->loadPackage(QStringLiteral("Lingmo/Comic"), parts[0]);
    ...
    QVariant data;
    const IdentifierType identifierType = stringToIdentifierType(pkg.metadata().value(QStringLiteral("X-KDE-LingmoComicProvider-SuffixType")));
...
void ComicEngine::finished(ComicProvider *provider)
{
    ...
    if (provider->image().isNull()) {
        qCWarning(LINGMO_COMIC) << "Provider returned null image" << provider->name();
        error(provider);
        return;
    }
...
QString ComicEngine::lastCachedIdentifier(const QString &identifier) const
{
    ...
    QString data = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1String("/lingmo_engine_comic/");
