...
public:
    ...
    {
        if (job->error()) {
            qCDebug(LINGMO_COMIC) << "Redirection job with id" << job->property("uid").toInt() << "finished with an error.";
        }
...
{
    qCDebug(LINGMO_COMIC) << "Requested page" << url << "with id" << id << "and additional metadata" << infos;
