...
{
    const QString dataDir = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1String("/lingmo_engine_comic/");
...
bool CachedProvider::storeInCache(const QString &identifier, const QImage &comic, const ComicMetaData &data)
{
    ...
    const QString dirPath = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1String("/lingmo_engine_comic/");
    ...
    // limit is on
    if (limit > 0) {
        qCDebug(LINGMO_COMIC) << QLatin1String("MaxComicLimit on.");
        int comicsToRemove = comics.count() - limit;
        QStringList::iterator it = comics.begin();
        while (comicsToRemove > 0 && it != comics.end()) {
            qCDebug(LINGMO_COMIC) << QLatin1String("Remove file") << (dirPath + (*it));
...
{
    if (limit < 0) {
        qCDebug(LINGMO_COMIC) << "Wrong limit, setting to default.";
        limit = CACHE_DEFAULT;
    }
    QSettings settings(identifierToPath(QLatin1String("comic_settings.conf")), QSettings::IniFormat);
