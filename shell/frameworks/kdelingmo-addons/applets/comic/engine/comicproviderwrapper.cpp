...
void ComicProviderWrapper::init()
{
    const QString path = QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                                QLatin1String("lingmo/comics/") + mProvider->pluginName() + QLatin1Char('/'),
                                                QStandardPaths::LocateDirectory);
    qCDebug(LINGMO_COMIC) << "ComicProviderWrapper::init() package is" << mProvider->pluginName() << " at " << path;

    if (!path.isEmpty()) {
        mPackage = new KPackage::Package(KPackage::PackageLoader::self()->loadPackageStructure(QStringLiteral("Lingmo/Comic")));
...
IdentifierType ComicProviderWrapper::identifierType() const
{
    ...
    const QString type = mProvider->description().value(QLatin1String("X-KDE-LingmoComicProvider-SuffixType"));
...
void ComicProviderWrapper::setNextIdentifier(const QJSValue &nextIdentifier)
{
    ...
    if (mNextIdentifier == mIdentifier) {
        ...
        qCWarning(LINGMO_COMIC) << "Next identifier is the same as the current one, clearing next identifier.";
    }
}
...
void ComicProviderWrapper::setPreviousIdentifier(const QJSValue &previousIdentifier)
{
    ...
    if (mPreviousIdentifier == mIdentifier) {
        ...
        qCWarning(LINGMO_COMIC) << "Previous identifier is the same as the current one, clearing previous identifier.";
    }
}
...
{
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Author").leftJustified(22, QLatin1Char('.')) << comicAuthor();
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Website URL").leftJustified(22, QLatin1Char('.')) << mWebsiteUrl;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Shop URL").leftJustified(22, QLatin1Char('.')) << mShopUrl;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Title").leftJustified(22, QLatin1Char('.')) << mTitle;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Additional Text").leftJustified(22, QLatin1Char('.')) << mAdditionalText;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Identifier").leftJustified(22, QLatin1Char('.')) << mIdentifier;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("First Identifier").leftJustified(22, QLatin1Char('.')) << mFirstIdentifier;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Last Identifier").leftJustified(22, QLatin1Char('.')) << mLastIdentifier;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Next Identifier").leftJustified(22, QLatin1Char('.')) << mNextIdentifier;
    qCDebug(LINGMO_COMIC) << QString::fromLatin1("Previous Identifier").leftJustified(22, QLatin1Char('.')) << mPreviousIdentifier;
...
QVariant ComicProviderWrapper::callFunction(const QString &name, const QJSValueList &args)
{
    if (m_engine) {
        ...
        if (mFuncFound) {
            ...
            if (val.isError()) {
                qCWarning(LINGMO_COMIC) << "Error when calling function" << name << "with arguments" << QVariant::fromValue(args) << val.toString();
                return QVariant();
            } else {
